 ...
 * @param <K>
 */
class GroupDataCacheImpl<T, K extends Serializable> extends AbstractDataCache<T, K> {
	// 实体Id对应的
	@SuppressWarnings("unused")
	private final ConcurrentHashMap<K, Serializable> keyEntityIdValueGroupId = new ConcurrentHashMap<>();
	// GroupId <==> 一个数据集合
	private final LoadingCache<Serializable, ConcurrentHashMap<K, T>> caches;

	public GroupDataCacheImpl(final AbstractCacheRepository<T, K> repository) {
		super(repository);
	...
	public void update(T entity) {
		...
		K entityId = this.getPrimaryIdValue(entity);
		if (!data.containsKey(entityId)) {
			throw new DataException("修改了一个不存在的Key:" + entity.toString() + ",info=" + ToStringBuilder.reflectionToString(entity));
		}
		data.put(entityId, entity);
	}
	...
	public List<T> loadAll(Serializable roleId, QueryFilter<T> filter) {
		...
		for (Entry<K, T> e : data.entrySet()) {
			T t = e.getValue();
			if (!filter.stopped() && filter.check(t)) {
				result.add(t);
			}
		}
		return result;
