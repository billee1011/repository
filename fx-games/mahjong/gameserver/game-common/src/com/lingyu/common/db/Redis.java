...
public class Redis implements IRediser {
	...
	public static void main(String[] args) throws InterruptedException {
		Redis redis = new Redis("192.168.1.21", 6379, 0);
		// return redis.call('set', KEYS[1], get_random_number())
		redis.del("test");
		redis.lpush("test", "a");
		redis.lpush("test", "b");
		redis.lpush("test", "c");
		...
		List<String> xx = redis.lrange("test", 0, -1);
		System.out.println(Arrays.toString(xx.toArray()));
		redis.ltrim("test", 0, 4);// 只保留5个.
		List<String> xx1 = redis.lrange("test", 0, -1);
		System.out.println(Arrays.toString(xx1.toArray()));

		// Redis redis = new Redis("192.168.1.21", 6384, 0);
		// for (int i = 0; i < 200000; i++) {
		// MonitorLogin login = new MonitorLogin();
		// login.setCityCode(1012);
		// login.setRoleId(RandomUtils.nextLong(Integer.MAX_VALUE,
		// Long.MAX_VALUE));
		// login.setRoleName("晨曦飞扬");
		// login.setUserId("23124554dffsf435656767");
		// String value = JSON.toJSONString(login);
		// redis.hset(RedisConstant.KEY_LOGIN_LOGOUT,
		// String.valueOf(login.getRoleId()), value);
		// }
		// Map<String, String>
		// map=redis.hgetAll(RedisConstant.KEY_LOGIN_LOGOUT);
		// logger.info("{}",map.size());

		// SuperVIP vip=new SuperVIP();
		// vip.setMoney(2000);
		// vip.setGirlUrl("http://avatar.csdn.net/4/7/E/1_jiangguilong2000.jpg");
		// vip.setQq(41157121);
		// vip.setVipLevel(14);
		// vip.setRatio(0.1f);
		// vip.setName("瑶瑶");
		// redis.set(RedisConstant.KEY_SUPERVIP, JSON.toJSONString(vip));
		...
		// list.add(info1);
		// list.add(info2);
		// ShardedJedisPool pool = new ShardedJedisPool(new
		// GenericObjectPoolConfig(), list);
		// // 从池中获取一个Jedis对象
		...
		// long start = System.nanoTime();
		// //Redis.migrate("192.168.1.21",6381, 0,"192.168.1.21", 6379 , 0,
		// 5000, "a","b");
