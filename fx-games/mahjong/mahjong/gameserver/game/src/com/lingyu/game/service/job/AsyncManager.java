...
public class AsyncManager {
		...
		for (String name : list) {
			...
			for (Method method : methods) {
				...
				if (schedule != null) {
					...
					if (wrapper != null) {
						logger.error("调度事件处理方法被多次定义 type={},method1={},method2={},methodIndex={}", type, wrapper.getMethod().getName(), method.getName(),methodIndex);
	 ...
	 * @param objectId 对象ID
	 * @param startTime 调度的有效开始时间
	 * @param interval 毫秒级 调度间隔
	 * @param args 参数值
	 */
	...
	public void schedule(byte scheduleType, long objectId, Date startTime, int interval, Object... args) {
		...
		logger.info("增加计划任务 scheduleType={},objectId={},args={},startTime={},interval={} ms",  scheduleType, objectId, args, startTime,
				interval);
	 ...
	 * @param scheduleType 调度类型
	 * @param objectId 对象ID
	 * @param delay 延迟多少时间执行一次 单位 毫秒级
	 * @param args 变量
	 * */
	public void scheduleOnce(byte scheduleType, long objectId, long delay, Object... args) {
		this.removeSchedule(scheduleType, objectId);
		// logger.debug("增加计划任务 stageId={},scheduleType={},objectId={},args={},delay={} ms",
...
//				stageExecutor.getExecutorService(groupName, objectId).execute(task);
//				break;
//			}
