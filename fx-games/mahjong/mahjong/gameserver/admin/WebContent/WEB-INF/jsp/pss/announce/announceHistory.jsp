...
<div class="pop_layer_cont">
	<div class="table_area" style="overflow: auto;">
		<div class="table_mask" id="cont_table_area" style="width: 100%;">
		  <c:choose>
		    <c:when test="${empty announceHistory}">
			  <div class="table_blank">
				<p class="text">没有历史公告</p>
		      </div>
			</c:when>
			<c:otherwise>
			<div>
			    <span id="announceHistoryError" class="tip_error"></span>
			</div>
				<div>
				  <table id="announceHistoryTable" style="width:900px;height:500px;border:1px solid #ccc;">
					<thead style="height: 31px">
						<tr>
							...
							<th style="width:10%" data-options="field:'exists'"><div class="op"><span class="table_title">状态</span></div></th>
							<th style="width:5%" data-options="field:'content'"><div class="op"><span class="table_title">公告内容</span></div></th>
						</tr>
					</thead>
						<tbody>
						 <c:forEach items="${announceHistory}" var="announcing" varStatus="status">
							<tr>
								...
								<td>
								    <c:choose>
								        <c:when test="${announcing.exists}">正常</c:when>
...
<script type="text/javascript">
    $(function(){
    	commonPagination('#announceHistoryTable');
    });
</script>