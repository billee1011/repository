...
public class AdminServerHandler extends SimpleChannelInboundHandler<Object[]> {
	...
	private static  AdminProcessor processor = AdminServerContext.getBean(AdminProcessorImpl.class);
	...
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		...
		if (session == null) {
			...
			int userId=SessionManager.getInstance().removeSession(ctx.channel());
			//chatMonitorManager.remove4ObserverList(userId);
			logger.info("{}/{} closed", ctx.channel(), session.getId());
			logger.info("用户登出 userId={},channel={},id={}",userId,session.getChannel(),session.getId());
		}
	}
