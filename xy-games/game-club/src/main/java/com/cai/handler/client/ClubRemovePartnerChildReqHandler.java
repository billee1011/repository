...
import static protobuf.clazz.ClubMsgProto.ClubPartnerCommon;

/**
 * Copyright@2015-2016 Hunan Qisheng Network Technology Co. Ltd.[SHEN-ZHEN]
 *
 * @author zhanglong 2018/8/14 16:01
 */
@ICmd(code = C2SCmd.CLUB_REMOVE_PARTNER_CHILD_REQ, desc = "亲友圈移除合伙人子成员")
public class ClubRemovePartnerChildReqHandler extends IClientExHandler<ClubPartnerCommon> {
	@Override
	protected void execute(ClubPartnerCommon req, TransmitProto topReq, C2SSession session) throws Exception {
		...
		club.runInReqLoop(() -> {
			...
			if (club.getIdentify(topReq.getAccountId()) == EClubIdentity.CREATOR) {
				canOperate = true;
			} else {
				if (operator.isPartner() && operator.getAccount_id() == req.getPartnerId()) {
					canOperate = true;
				}
			}
			if (!canOperate) {
			...
			if (targetMember == null || targetMember.getParentId() <= 0) {
				return;
			}
			...
			session.send(PBUtil.toS_S2CRequet(topReq.getAccountId(), S2CCmd.CLUB_REMOVE_PARTNER_CHILD_RSP, b));

			// 事件
			club.sendClubPartnerEventMsgToClient(operator, targetMember, partnerMember, ClubMsgProto.ClubJoinQuitMsgProto.MsgType.DEL_CHILD);
			//log
			ClubEventLog.event(new ClubEventLogModel(club.getClubId(), club.getOwnerId(), EClubEventType.DEL_CHILD)
