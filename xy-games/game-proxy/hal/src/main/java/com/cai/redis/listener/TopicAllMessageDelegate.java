		...
		try {
			...
			case RsResponseType.CMD_VALUE: {
				...
				//代理开房变化了
					...
					AccountRedis accountRedis = SpringService.getBean(RedisService.class).hGet(RedisConstant.ACCOUNT_REDIS, account.getAccount_id()+"", AccountRedis.class);
					...
					Collections.sort(list, new Comparator<PrxoyPlayerRoomModel>() {
						public int compare(PrxoyPlayerRoomModel p1, PrxoyPlayerRoomModel p2) {
							return ((Long)p2.getCreate_time()).compareTo((Long)p1.getCreate_time());//id 从大到小
						}
					});
