...
public class OtherSystemHandler extends ClientHandler<OtherSystemRequest> {
	...
	public void onRequest() throws Exception {
		...
		if (type == SYS_NOTICE) {
			...
			SysNoticeModel sysNoticeModel = SysNoticeDict.getInstance().getSysNoticeModelDictionary().get(game_id)
					.get(1);
			...
			// 是否开放
			SysParamModel sysParamModel1016 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id)
					.get(1016);
			if (sysParamModel1016.getVal1() == 1) {
				FastMap<Integer, MainUiNoticeModel> map = MainUiNoticeDict.getInstance().getMainUiNoticeDictionary()
						.get(game_id);
				if (map != null) {
		...
		// }
			...
			centerRMIServer.addAccountGold(account.getAccount_id(), every_share_gold, false,
					"每日分享得卡:" + every_share_gold, EGoldOperateType.DAY_SHARE);
			...
			if (cur_page <= 0)
				cur_page = 1;
			List<AccountRecommendModel> accountRecommendModelList = Lists
					.newArrayList(account.getAccountRecommendModelMap().values());
			// 排序
			...
			for (int i = 0; i < 10; i++) {
				...
				ShareInviteInfoResponse.Builder shareInviteInfoResponseBuilder = MessageResponse
						.getShareInviteInfoResponse(model);
