...
public class RoleLogService implements IRoleLogService {
	...
	public void test() {
		...
		for (int i = 0; i < 5; i++) {
			...
			roleLogBase.setRoleId(i + time);
			roleLogBase.setAfterNum(i + time);
			roleLogBase.setChangeNum(i + time);
		...
		Criteria criteria = Criteria.where("msgCode").is(Integer.valueOf(30)).and("roleId").is(Integer.valueOf(30));
