...
public class RoomHandler extends ClientHandler<RoomRequest> {
	...
	public void onRequest() throws Exception {
		...
		if(game_id == GameConstants.GAME_ID_OX)
			game_count = GameConstants.GAME_PLAYER_OX;
		else
			game_count = GameConstants.GAME_PLAYER;
...
//			}
			...
			if (sysParamModel != null && sysParamModel.getVal2() == 1) {
				...
				if(FvMask.has_any(game_rule_index, FvMask.mask(GameConstants.GAME_RULE_ROOM_PAY))){
					if (game_round == sysParamModel1007.getVal1()) {
						if (gold < sysParamModel1007.getVal2()) {
							send(MessageResponse.getMsgAllResponse("房卡不足").build());
							return;
						}
					}else if (game_round == sysParamModel1008.getVal1()) {
						if (gold < sysParamModel1008.getVal2()) {
							send(MessageResponse.getMsgAllResponse("房卡不足").build());
							return;
						}
					}else if (game_round == sysParamModel1009.getVal1()) {
						if (gold < sysParamModel1009.getVal2()) {
							send(MessageResponse.getMsgAllResponse("房卡不足").build());
							return;
						}
					}
				}
				if(FvMask.has_any(game_rule_index, FvMask.mask(GameConstants.GAME_RULE_AA_PAY))){
					if (game_round == sysParamModel1007.getVal1()) {
						if (gold < sysParamModel1007.getVal3()) {
							send(MessageResponse.getMsgAllResponse("房卡不足").build());
							return;
						}
					}else if (game_round == sysParamModel1008.getVal1()) {
						if (gold < sysParamModel1008.getVal3()) {
							send(MessageResponse.getMsgAllResponse("房卡不足").build());
							return;
						}
					}else if (game_round == sysParamModel1009.getVal1()) {
						if (gold < sysParamModel1009.getVal3()) {
							send(MessageResponse.getMsgAllResponse("房卡不足").build());
							return;
						}
					}
			...
			if (roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_FLS_LX
					|| roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_FLS_LX_TWENTY
					|| roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_HH_YX
					|| roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_PHZ_YX
					|| roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_FPHZ_YX
					||roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_SEVER_OX
					|| roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_SZOX 
					|| roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_LZOX
					|| roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_ZYQOX 
					|| roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_MSZOX
					|| roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_MFZOX
					|| roomRedisModel.getGame_type_index() == GameConstants.GAME_TYPE_TBOX) {// 福禄寿
				// 限制ip--福禄寿在logic加入房间
				// 三人场
				// if(!FvMask.has_any(roomRedisModel.getGame_rule_index(),
				// FvMask.mask(GameConstants.GAME_RULE_LIXIANG_FLS_IP))) {
				// if (account.getAccountModel().getClient_ip2() != null
				// &&
				// roomRedisModel.getIpSet().contains(account.getAccountModel().getClient_ip2()))
				// {
				// send(MessageResponse.getMsgAllResponse("不允许相同ip进入!").build());
				// return;
				// }
				// }
			} else {
			...
			Global.getService(Global.SERVER_LOGIC).execute(new Runnable(){
				...
				public void run() {
					...
					for (int k = 0; k < l; k++) {
						...
						recorde_common(grr, _player_result, player_result, playerResultFLSResponse,gamecount);
			...
			Global.getService(Global.SERVER_LOGIC).execute(new Runnable(){
				...
				public void run() {
					...
					if (request.hasCurPage()) {
						...
						room_record = MongoDBServiceImpl.getInstance().getProxyParentBrandListByAccountId(create_account_id,
								game_id);
						l = room_record.size();
					}
			...
			Global.getService(Global.SERVER_LOGIC).execute(new Runnable(){
				...
				public void run() {
					...
					if (target_brand_parent_id <= 0) {
						send(MessageResponse.getMsgAllResponse("牌局父Id错误").build());
						return;
					}
					...
					if (room_record != null) {
						...
						if (error_check) {
							send(MessageResponse.getMsgAllResponse("error_check错误").build());
							return;
						}
...
//			}
	...
	private void recorde_common(GameRoomRecord grr, PlayerResult _player_result,
			PlayerResultResponse.Builder player_result, PlayerResultFLSResponse.Builder playerResultFLSResponse,int game_count) {
