...
public abstract class ClientHandler<T extends GeneratedMessage> {
	...
	public void send(Response response)
	{
		try
		{
			ResponseWrapper responseWrapper = buildResponseWrapper(response);
	...
	private ResponseWrapper buildResponseWrapper(Response response)
	{
		int responseType = response.getResponseType().getNumber();
		ResponseWrapper responseWrapper = new ResponseWrapper(responseType, response);
