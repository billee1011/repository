 ...
 * 采用泛型T来定义config的目的在于在AbstractManager的子类中使用config时不需要转型为具体的
 * AbstractConfig的子类了。<br>
...
public abstract class AbstractService implements Comparable<AbstractService> {
	 ...
	 * 根据_userID初始化玩家相关数据
	 */
	public abstract void sessionCreate(C2SSession session);

	/**
	 * 根据_userID清理玩家相关数据
	 */
	public abstract void sessionFree(C2SSession session);
