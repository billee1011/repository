 ...
 */
public class WechatGameLoginRunnable implements Runnable {
	 ...
	 */
	public WechatGameLoginRunnable(C2SSession session, LoginRequest request, int game_index_tmp, String client_flag_tmp, String client_version_tmp) {
	...
	public void run() {
		...
		try {
			...
			String result = HttpClientUtils.get(URL + "?" + request.getAccessTokenJson());
			...
			if (accessTokenJson.containsKey("errcode") && accessTokenJson.getInteger("errcode") != 0) {
				session.send(MessageResponse.getMsgAllResponse(accessTokenJson.getString("errmsg")).build());
				return;
			}

			String openid = accessTokenJson.getString("openid");

			JSONObject userJson = JSON.parseObject(request.getAccessUserJson());
			...
			String unionid = String.format("MGAME_%s", openid);// 全平台唯一id //
																// 微信小游戏，没有unionid
