...
public class PlayerViewHandler extends IClientHandler<PlayerViewRequest> {
	 ...
	 */
	@Override
	protected void execute(PlayerViewRequest request, Request topRequest, C2SSession session) throws Exception {
		// 1,accountId 简单检测
		...
		if (null != lastReqTime_Second && (System.currentTimeMillis() / 1000) - lastReqTime_Second.intValue() < REQ_CD_SECOND) {
		...
		if (null == vo) {
			PlayerServiceImpl.getInstance().sendAccountMsg(session, MessageResponse.getMsgAllResponse(-1, "不存在该玩家!", ESysMsgType.INCLUDE_ERROR).build());
			return;
		}
