...
public class RedisService implements IRedisService {
	...
	/**
	 */
	private static final long WARN_TIME = 200L;
	...
	public long del(final String... keys) {
		...
		if (cost > WARN_TIME) {
			...
			if(cost>WARN_DB_TIME) {
				MongoDBServiceImpl.getInstance().server_error_log(0, ELogType.redisTimeLong, "redis耗时 del",
						0L, keyStr);
			}
		}
	...
	public void set(final byte[] key, final byte[] value, final long liveTime) {
		...
		if (cost > WARN_TIME) {
			...
			if(cost>WARN_DB_TIME) {
				MongoDBServiceImpl.getInstance().server_error_log(0, ELogType.redisTimeLong, "redis耗时--set",
						0L, keyStr);
			}
		}
	}
	...
	public String get(final String key) {
		...
		if (cost > WARN_TIME) {
			...
			if(cost>WARN_DB_TIME) {
				MongoDBServiceImpl.getInstance().server_error_log(0, ELogType.redisTimeLong, "逻辑服redis耗时--get",
						0L, key);
			}
		}
	...
	public void hSet(String key, String field, Object value) {
		...
		if (cost > WARN_TIME) {
			...
			if(cost>WARN_DB_TIME) {
				MongoDBServiceImpl.getInstance().server_error_log(0, ELogType.redisTimeLong, "逻辑服redis耗时--hSet cost="+cost,
						0L, key);
			}
		}
	}
	...
	public <T> T hGet(final String key, final String field, Class<T> elementType) {
		...
		if (cost > WARN_TIME) {
			...
			if(cost>WARN_DB_TIME) {
				MongoDBServiceImpl.getInstance().server_error_log(0, ELogType.redisTimeLong, "redis耗时--hGet cost="+cost,
						0L, key);
			}
		}
		return r;
