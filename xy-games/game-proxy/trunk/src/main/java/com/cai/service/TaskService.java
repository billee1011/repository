...
//
//			// 转盘活动清理免费抽奖
//			TurntableService.getInstance().clearFreeRewardedAccount();
		} catch (Exception e) {
	...
	public void resetTodayAccountParam() {
		try {
			...
			// for(C2SSession
			// session:SessionServiceImpl.getInstance().getSessionMap().values()){
			for (C2SSession session : C2SSessionService.getInstance().getAllOnlieSession()) {
				if (session.getAccount() == null)
					continue;
				if (session.getAccount().getAccountParamModelMap() == null)
					continue;
