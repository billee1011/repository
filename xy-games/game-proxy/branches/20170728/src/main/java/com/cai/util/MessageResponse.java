...
public class MessageResponse {
	...
	public static AccountResponse.Builder getAccountResponse(Account account) {
		...
		if (accountModel.getPt().equals(EPtType.WX.getId())) {
			...
			accountResponseBuilder
					.setHeadImgUrl(WxUtil.changHeadimgurl(accountWeixinModel.getHeadimgurl(), EWxHeadimgurlType.S132));
			// 性别
	...
	public static LogicRoomAccountItemRequest.Builder getLogicRoomAccountItemRequest(Session session) {
		...
		if (account.getAccountWeixinModel() != null) {
			buider.setAccountIcon(
					WxUtil.changHeadimgurl(account.getAccountWeixinModel().getHeadimgurl(), EWxHeadimgurlType.S132));
	 ...
	 * @return
	 */
	public static AccountPropertyResponse.Builder getAccountPropertyResponse(int prop_id, Integer val1, Integer val2,
			Integer val3, Integer val4, Integer val5, String str1, String str2, Long vallong1) {
	...
	public static List<AccountPropertyResponse> getSysAccountPropertyResponseList(int game_id, Account account) {
		...
		for (SysParamModel model : map.values()) {
			if (model.getSend_client() != 1)
				continue;
			AccountPropertyResponse.Builder accountPropertyResponseBuilder = getAccountPropertyResponse(model.getId(),
					model.getVal1(), model.getVal2(), model.getVal3(), model.getVal4(), model.getVal5(),
					model.getStr1(), model.getStr2(), null);
		...
		// 红点功能
		// 系统公告
		AccountParamModel accountParamModel = account.getAccountParamModelMap()
				.get(EPropertyType.RED_SYS_NOTIC.getId());
		if (accountParamModel != null) {
			...
			if (sysNoticeModel != null) {
				...
				AccountPropertyResponse.Builder accountPropertyResponseBuilder = getAccountPropertyResponse(
						EPropertyType.RED_SYS_NOTIC.getId(), flag, null, null, null, null, null, null, null);
				list.add(accountPropertyResponseBuilder.build());
			}
		}
	 ...
	 * @return
	 */
	public static List<AccountPropertyResponse> getSysAccountPropertyResponseList(int game_id) {
		// 系统参数的
		FastMap<Integer, SysParamModel> map = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id);
		List<AccountPropertyResponse> list = Lists.newArrayList();
		for (SysParamModel model : map.values()) {
	...
	public static RsAccountParamModelResponse.Builder getRsAccountParamModelResponse(
			AccountParamModel accountParamModel) {
		RsAccountParamModelResponse.Builder rsAccountParamModelResponseBuilder = RsAccountParamModelResponse
				.newBuilder();
	 ...
	 * @return
	 */
	public static ProxyRoomItemResponse.Builder getProxyRoomItemResponse(PrxoyPlayerRoomModel model,
