...
public class StoreHandler extends IClientHandler<StoreRequest> {
	 ...
	 */
	@Override
	protected void execute(StoreRequest request, Request topRequest, Session session) throws Exception {
		if (!request.hasType())
			return;
		...
		if (type == QUERY_FUNCTION_STATU) {
			...
			for (MoneyShopModel shopModel : shopModelList) {
				StoreGoodMoneyResponse.Builder storeGoodResponseBuilder = MessageResponse
						.getStoreMoneyResponse(shopModel);
				storeGoodResponseList.add(storeGoodResponseBuilder.build());
			}
			...
			AddGoldResultModel result = centerRMIServer.addAccountGold(account.getAccount_id(), -findModel.getPrice(),
					false, "兑换金币,shopID:" + shopID, EGoldOperateType.MONEY_DUIHUAN);
			if (result.isSuccess()) {
				centerRMIServer.addAccountMoney(account.getAccount_id(),
						findModel.getMoney() + findModel.getSendMoney(), false, "兑换金币,shopID:" + shopID,
						EMoneyOperateType.DUIHUAN_PROP);
			}
			StoreAllResponse.Builder storeAllResponseBuilder = StoreAllResponse.newBuilder();
			...
			Global.getWxPayService().execute(new Runnable() {
				...
				public void run() {
					...
					SysParamModel sysParamModel1014 = SysParamDict.getInstance()
							.getSysParamModelDictionaryByGameId(game_id).get(1014);
					...
					storeAllResponseBuilder.setPayBuyResponse(
							ptAPIServiceImpl.getPayBuyResponse("", gameOrderID, account.getGame_id()));
					...
					// 插入临时订单
					ptAPIServiceImpl.addCardLog(gameOrderID, account.getAccount_id(), account.getNickName(),
							account.getAccountModel().getIs_agent(), ESellType.GAME_PAY_CARD.getId(),
							shopModel.getShop_id(), shopModel.getGold(), shopModel.getSend_gold(), shopModel.getPrice(),
							0, ESellType.GAME_PAY_CARD.getName(), ESellType.GAME_PAY_CARD.getName(), 0 + "",
							ESellType.GAME_PAY_CARD.getName(), "", game_id, shopModel.getName());
				}
			...
			Global.getWxPayService().execute(new Runnable() {
				...
				public void run() {
					...
					if (StringUtils.isEmpty(gameOrderID) || StringUtils.isEmpty(receipt)) {
						logger.error("客户端 发送的  gameOrderID or receipt是空的!!!" + receipt + " gameOrderID=" + gameOrderID
