	...
	public void sendAccountMsg(Session session, Response response) {
		...
		wf.addListener(new ChannelFutureListener() {
			public void operationComplete(ChannelFuture future) throws Exception {
				if (!future.isSuccess()) {
					logger.warn("转发服给客户端消息失败:response:" + response.getResponseType());
				}
			}
		});
		...
		if (false&&account != null) {
			...
			buf.append(response.toByteArray().length).append("B").append("|sessionId:").append(session.getSessionId())
					.append("|accountId:" + account.getAccount_id()).append("|").append("转发服消息|")
					.append(response.toString());
			long v1 = response.getResponseType().getNumber();
			MongoDBServiceImpl.getInstance().player_log(account_id, ELogType.response, buf.toString(), v1, null, ip);
		}
		//
