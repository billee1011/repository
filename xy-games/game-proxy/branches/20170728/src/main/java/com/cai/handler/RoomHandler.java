...
import protobuf.clazz.ddz.DdzProto.DdzCallRsp;
...
public class RoomHandler extends IClientHandler<RoomRequest> {
	 ...
	 * 测试
	 */
	private static final int COMMON_TEST = 1001;

	@Override
	protected void execute(RoomRequest request, Request topRequest, Session session) throws Exception {
		...
		int _game_type_index = request.getGameTypeIndex();
		// 玩法
		int game_rule_index = request.getGameRuleIndex();
		game_count = RoomComonUtil.getMaxNumber(_game_type_index, game_rule_index);
		...
		if (type == CREATE_ROOM) {
			...
			if (account.getRoom_id() != 0) {
				// 验证一下
				session.send(MessageResponse.getMsgAllResponse("调试:已进入其它房间:" + account.getRoom_id()).build());
				return;
			}
			...
			if (sysParamModel != null && sysParamModel.getVal2() == 1) {
				...
				if (gold < findParam.getVal2()) {
					session.send(MessageResponse.getMsgAllResponse("房卡不足").build());
					return;
				}
			}
			...
			if (source_room_id != 0) {
				RoomRedisModel roomRedisModel = SpringService.getBean(RedisService.class).hGet(RedisConstant.ROOM, source_room_id + "",
			...
			Global.getService(Global.SERVER_LOGIC).execute(new Runnable() {
				...
				public void run() {
					...
					if (request.hasCurPage()) {
						...
						room_record = MongoDBServiceImpl.getInstance().getProxyParentBrandListByAccountId(create_account_id, game_id);
						l = room_record.size();
					}
...

		{
			...
			if (room_id > 0) {
				...
				Global.getService("").execute(new Runnable() {
					...
					public void run() {
						...
						LocationInfor reqlocationInfor = topRequest.getExtension(Protocol.roomRequest).getLocationInfor();
						if (reqlocationInfor.getPosX() == 0 || reqlocationInfor.getPosY() == 0) {
							return;
						}

						String position = PtAPIServiceImpl.getInstance().getbaiduPosition(game_id, reqlocationInfor.getPosX(),
								reqlocationInfor.getPosY());
			...
			// 金币场配置
			SysParamModel sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(5006);
			// 判断金币场是否开放
		...
		} else if (COMMON_TEST == type) {
			// byte[] body = PBUtil.toByteArray(request.getCommRequet());
			// DdzCallReq builder = DdzCallReq.parseFrom(body);
			// int id = builder.getId();
			// int result = builder.getResult();
			//
			// DdzCallRsp.Builder rsbBuilder = DdzCallRsp.newBuilder();
			// rsbBuilder.setId(id * 10);
			// rsbBuilder.setResult(result * 10);
			//
			// RoomResponse.Builder rspBuilder = RoomResponse.newBuilder();
			// rspBuilder.setType(type);
			// rspBuilder.setAppId(1000022);
			// rspBuilder.setCommResponse(PBUtil.toByteString(rsbBuilder));
			// Response.Builder responseBuilder = Response.newBuilder();
			// responseBuilder.setResponseType(ResponseType.ROOM);
			// responseBuilder.setExtension(Protocol.roomResponse,
			// rspBuilder.build());
			//
			// session.send(responseBuilder.build());
			DdzCallReq req = PBUtil.toObject(request, DdzCallReq.class);
			return;
		} else {
			// 消息再次封装
			Request logicRequest = MessageResponse.getLogicRequest(topRequest.toBuilder(), session).build();
	 ...
	 * @param player_result
	 */
	public void setRoomInfor(GameRoomRecord grr, PlayerResult _player_result, PlayerResultResponse.Builder player_result) {
	 ...
	 */
	private void recorde_common(GameRoomRecord grr, PlayerResult _player_result, PlayerResultResponse.Builder player_result,
	 ...
	 * 检查是否停服状态
	 */
	public boolean check_is_stop_service(Account account, Session session) {
		SysParamModel sysParamModel1000 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(1).get(1000);
		if (sysParamModel1000.getVal3() == 0 && account.getAccountModel().getIs_inner() == 0) {
	 ...
	 * 检查是否已经有了房间了
	 */
	public boolean check_is_had_room(Account account, RoomRequest request, Session session) {
		int source_room_id = account.getRoom_id();
		if (source_room_id != 0) {
			RoomRedisModel roomRedisModel = SpringService.getBean(RedisService.class).hGet(RedisConstant.ROOM, source_room_id + "",
					RoomRedisModel.class);
