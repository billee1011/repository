...
public class LoginHandler extends IClientHandler<LoginRequest> {
	 ...
	 * 获取子游戏差异包
	 */
	private static final int SUBAPP_UPDATE = 13;
	/**
	 * 推广员开始返利时间
	 */
	private static final long  RECOMMEND_RECIEVE_TIME =  1500102000000L;

	@Override
	protected void execute(LoginRequest request, Request topRequest, Session session) throws Exception {
		...
		if (type == WX_LOGIN) {
			...
			Global.getWxService().execute(new Runnable() {
				...
				public void run() {
					try {
						...
						//通过分享链接下载app，实行推广员返利
						try{
							if(account.getAccountModel().getCreate_time().getTime()>RECOMMEND_RECIEVE_TIME){
								Account account2 = centerRMIServer.getAccount(account.getAccount_id());
								long recommendId = account.getAccountModel().getRecommend_id();
								if(recommendId ==0){
									recommendId = account2.getAccountModel().getRecommend_id();
								}
								
								addRecommendPlayerIncome(account2, recommendId);
							}
						}catch(Exception e){
							logger.error(account.getAccount_id()+" 推荐用户返利失败 ");
						}
					} catch (Exception e) {
						PlayerServiceImpl.getInstance().sendAccountMsg(session,
			...
			try {
				...
				if (account_id == 0L) {
					this.sendSelfToketLoginFail(selfToken, session);
					return;
				}
	 ...
	 * @param account
	 */
	private void sendLastNotice(Account account, Session session) {
	 ...
	 * @param selfToken
	 */
	private void sendSelfToketLoginFail(String selfToken, Session session) {
		LoginResponse.Builder loginResponse = LoginResponse.newBuilder();
		loginResponse.setType(3);
	...
	private void loginError(int type, int errorCode, Session session) {
		LoginResponse.Builder loginResponse = LoginResponse.newBuilder();
