	...
	public void handleMessage(byte[] message) {
		...
		try {
			...
			case RsResponseType.ACCOUNT_UP_VALUE: {
				...
				@SuppressWarnings("unused")
				int new_inner = account.getAccountModel().getIs_inner();
				...
				if (new_password != null && !new_password.equals(old_password)) {
					...
					AccountPropertyResponse.Builder accountPropertyResponseBuilder = MessageResponse.getAccountPropertyResponse(
							EPropertyType.IS_NULL_AGENT_PW.getId(), is_null_agent_pw, null, null, null, null, null, null, null);
					accountPropertyListResponseBuilder.addAccountProperty(accountPropertyResponseBuilder);
				}
				if (old_money != new_money) {
					AccountPropertyResponse.Builder accountPropertyResponseBuilder = MessageResponse
							.getAccountPropertyResponse(EPropertyType.MONEY.getId(), null, null, null, null, null, null, null, new_money);
					accountPropertyListResponseBuilder.addAccountProperty(accountPropertyResponseBuilder);
				}
				// 签名
			...
			case RsResponseType.CMD_VALUE: {
				...
				if (rsCmdResponse.getType() == 1) {
					if (rsCmdResponse.hasAccountId()) {
						...
						if (session != null) {
							session.getChannel().close();
							MongoDBServiceImpl.getInstance().systemLog(ELogType.kickOnlineAccount, "踢玩家下线:account_id=" + account_id, account_id, null,
				...
				// 代理开房变化了
					...
					for (PrxoyPlayerRoomModel model : list) {
						...
						if (roomRedisModel != null) {
							ProxyRoomItemResponse.Builder proxyRoomItemResponseBuilder = MessageResponse.getProxyRoomItemResponse(model,
									roomRedisModel);
