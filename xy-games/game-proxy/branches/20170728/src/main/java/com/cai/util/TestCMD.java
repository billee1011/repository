				...
				Global.getWxPayService().execute(new Runnable() {
					...
					public void run() {
						...
						String prepayID = ptAPIServiceImpl.getPrepayId(gameOrderID, shopModel.getName(),
								shopModel.getPrice(), "192.168.1.29", shopID, 1);
						...
						SysParamModel sysParamModel5000 = SysParamDict.getInstance()
								.getSysParamModelDictionaryByGameId(1).get(5000);// 微信登录相关,str1=appid,str2=应用密钥
						SysParamModel sysParamModel5001 = SysParamDict.getInstance()
								.getSysParamModelDictionaryByGameId(1).get(5001);// 微信支付str1=商户号str2=key
						// 签名生成 临时订单
