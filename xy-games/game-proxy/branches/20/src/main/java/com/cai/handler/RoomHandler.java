	...
	public void onRequest() throws Exception {
		...
		if(type == CREATE_ROOM){
			...
			if(game_type_index==GameConstants.GAME_TYPE_ZZ){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_ZZ);
			}else if(game_type_index==GameConstants.GAME_TYPE_CS){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_CS);
			}else if(game_type_index==GameConstants.GAME_TYPE_HZ){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_HZ);
			}else if(game_type_index==GameConstants.GAME_TYPE_SHUANGGUI){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_SHUANGGUI);
			}else if(game_type_index==GameConstants.GAME_TYPE_ZHUZHOU){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_ZHUZHOU);
			}else if(game_type_index==GameConstants.GAME_TYPE_XTHH){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_XTHH);
			}else if(game_type_index==GameConstants.GAME_TYPE_HENAN_AY){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_HENAN_AY);
			}else if(game_type_index==GameConstants.GAME_TYPE_HENAN_LZ){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_HENAN_LZ);
			}else if(game_type_index==GameConstants.GAME_TYPE_HENAN){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_HENAN);
			}else if(game_type_index==GameConstants.GAME_TYPE_HENAN_HZ){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_HENAN_HZ);
			}else if(game_type_index==GameConstants.GAME_TYPE_FLS_LX){
				sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id).get(GameConstants.SYS_GAME_TYPE_FLS_LX);
			}
			...
			if(roomRedisModel.getGame_type_index()==GameConstants.GAME_TYPE_FLS_LX) {//福禄寿 限制ip
				if(!FvMask.has_any(roomRedisModel.getGame_rule_index(), FvMask.mask(GameConstants.GAME_RULE_LIXIANG_FLS_IP))) {
					if (account.getAccountModel().getClient_ip() != null
							&& roomRedisModel.getIpSet().contains(account.getAccountModel().getClient_ip())) {
...
//				roomResponse.addLocationInfor(topRequest.getExtension(Protocol.roomRequest).getLocationInfor());
//				responseBuilder.setExtension(Protocol.roomResponse, roomResponse.build());
//				send(responseBuilder.build());
			}
