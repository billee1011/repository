...
//			
				...
				for(ResponseWrapper responseWrapper : responseWrapperList){
					...
					wf.addListener(new ChannelFutureListener()
					{
						public void operationComplete(ChannelFuture future) throws Exception
						{
							if (!future.isSuccess())
							{
								logger.warn("转发服给客户端消息失败: request:" + wrapper.getRequest() + ",response:"+res);
							}
						}
					});
			...
			if(false && wrapper.getTopRequest().getRequestType()!=RequestType.HEAR){
				...
				if(account!=null){
					...
					MongoDBServiceImpl.getInstance().log(account_id, ELogType.request,buf.toString(), v1, hanlderProccesTime, ip);
				}
			}
