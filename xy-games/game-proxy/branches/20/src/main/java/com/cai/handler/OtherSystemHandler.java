...
//			SysParamModel sysParamModel2005 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(account.getGame_id()).get(2005);
...
//			centerRMIServer.addAccountGold(account.getAccount_id(), gold, false, "每日分享得卡:" + gold, EGoldOperateType.DAY_SHARE);
//			
...
//		}
			...
			accountParamModel.setDate1(now);
			ICenterRMIServer centerRMIServer = SpringService.getBean(ICenterRMIServer.class);
			centerRMIServer.addAccountGold(account.getAccount_id(),every_share_gold , false, "每日分享得卡:" + every_share_gold, EGoldOperateType.DAY_SHARE);
			...
			Collections.sort(accountRecommendModelList, new Comparator<AccountRecommendModel>() {
				public int compare(AccountRecommendModel p1, AccountRecommendModel p2) {
					return ((Long)p2.getCreate_time().getTime()).compareTo((Long)p1.getCreate_time().getTime());//id 从大到小
				}
			});
			
			List<ShareInviteInfoResponse> shareInviteInfoResponseBuilderList = Lists.newArrayList();
			for(AccountRecommendModel model : accountRecommendModelList){
...
//			AccountRecommendModel model = new AccountRecommendModel();
//			model.setAccount_id(1L);
//			model.setCreate_time(new Date());
//			model.setGold_num(8);
//			model.setTarget_account_id(40);
//			model.setTarget_name("test");
//			model.setTarget_icon("http://wx.qlogo.cn/mmopen/vUJicqzME8aB3Olnt0l620cBJqwXfUAecgvLEiaEyxicRJ3ZHr7LuJhYYPgxPyoQozqeQk5NAlMvODUOfVrHFs7EjJoVyicGZ4R5/46");
//			shareInviteInfoResponseBuilderList.add(MessageResponse.getShareInviteInfoResponse(model).build());
			...
			shareInviteViewResponseBuilder.setCurPage(1);
