	...
	public void onRequest() throws Exception {
		...
		if (type == MODIFY_PASSWD) {
			...
			if (!verify) {
				...
				MongoDBServiceImpl.getInstance().proxyGoldLog(account.getAccount_id(), targetAccount.getAccount_id(), nick_name, num, account.getAccountModel().getClient_ip(), 0, target_proxy_account);
			...
			List<ProxyGoldLogModel> proxyGoldLogModelList = MongoDBServiceImpl.getInstance().getProxyGoldLogModelList(page, account.getAccount_id(), target_account_id);
			...
			Global.getWxPayService().execute(new Runnable() {
				...
				public void run() {
					...
					goldCardAllResponseBuilder.setPayBuyResponse(ptAPIServiceImpl.getPayBuyResponse(prepayID, gameOrderID,account.getGame_id()));
					...
					ptAPIServiceImpl.addCardLog(gameOrderID, account.getAccount_id(), account.getNickName(), account.getAccountModel().getIs_agent(), ESellType.GAME_PAY_CARD.getId(), shopModel.getId(), shopModel.getGold(), shopModel.getSend_gold(), shopModel.getPrice(), 0, ESellType.GAME_PAY_CARD.getName(),
							ESellType.GAME_PAY_CARD.getName(), 0 + "", ESellType.GAME_PAY_CARD.getName(), "",account.getGame_id(),shopModel.getName());
			...
			if(recommend_id==account.getAccount_id()){
				send(MessageResponse.getMsgAllResponse("不能设置自己").build());
				return;
			}
			
			if(account.getAccountModel().getRecommend_id()!=0L){
				send(MessageResponse.getMsgAllResponse("你已经设置了推荐人").build());
				return;
			}
			...
			if(sysParamModel2000!=null && sysParamModel2000.getVal1()==1){
				if(targetAccount!=null){
					...
					if(flag){
						//给好友加金币
						centerRMIServer.addAccountGold(targetAccount.getAccount_id(), addGold, false, "分享好友下载,好友account_id:"+account.getAccount_id(), EGoldOperateType.FRIEND_DOWN);
						//给自己加金币,登录成功后的地方给
					}
