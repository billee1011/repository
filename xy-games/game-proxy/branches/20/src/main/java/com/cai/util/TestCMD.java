...
public class TestCMD {
	...
	public static void cmd(String cmd) {
		...
		if("0".equals(cmd)){
			...
			int k = SessionServiceImpl.getInstance().getSessionMap().size();
			int k2 = SessionServiceImpl.getInstance().getOnlineSessionMap().size();
			int k3 = SessionServiceImpl.getInstance().getOnlineAccountIdSessionIdMap().size();
			...
			JSONObject jsonObject = PtAPIServiceImpl.getInstance().wxGetAccessTokenByCode("xxsdfsdfsdf", 1);
			...
			for (int i = 0; i < 1000000; i++) {
				MongoDBServiceImpl.getInstance().log(1, ELogType.login, "登录测试", null, 1L, null);
			...
			ICenterRMIServer centerRMIServer = SpringService.getBean(ICenterRMIServer.class);
			Account account = centerRMIServer.getAndCreateAccount(EPtType.WX.getId(), "t2", "127.0.0.1","0","1.0.0");
		}
			...
			for(IpFirewallModel model :  FirewallServiceImpl.getInstance().getIpFirewallModelMap().values()){
				...
				buf.append("\n").append("ip:"+model.getIp()+"  \tlinkCount:"+model.getLinkCount())
				.append(" \tpass:"  + pass);
			}
			...
			List<BrandLogModel> list = MongoDBServiceImpl.getInstance().getParentBrandListByAccountId(101495L, 1);
			...
			for(int i=0;i<10;i++) {
				Global.getWxPayService().execute(new Runnable() {
					...
					public void run() {
						...
						SysParamModel sysParamModel5000 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(1)
								.get(5000);// 微信登录相关,str1=appid,str2=应用密钥
						SysParamModel sysParamModel5001 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(1)
								.get(5001);// 微信支付str1=商户号str2=key
						// 签名生成 临时订单
