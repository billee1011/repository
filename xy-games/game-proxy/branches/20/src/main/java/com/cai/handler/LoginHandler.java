...
public class LoginHandler extends ClientHandler<LoginRequest> {
	...
	public void onRequest() throws Exception {
		...
		// 现在只有1,2
		if (!(game_index == 1 || game_index == 2 || game_index==3 || game_index==4 || game_index==5)) {
			return;
		}
		...
		if (type == WX_LOGIN) {
			...
			Global.getWxService().execute(new Runnable() {
				...
				public void run() {
					try {
										...
										if(uid!=accountModel.getAccount_id()){
											...
											if(targetAccount!=null){
												if(accountModel.getRecommend_id()==0L){
													...
													//活动相关
													SysParamModel sysParamModel2004 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_index_tmp).get(2004);
													...
													if(flag){
														//给好友加金币
														centerRMIServer.addAccountGold(uid, addGold, false, "分享好友下载,好友account_id:"+account.getAccount_id(), EGoldOperateType.FRIEND_DOWN);
														//给自己加金币,登录成功后的地方给
													}
												}
											}
						...
						MongoDBServiceImpl.getInstance().log(account.getAccount_id(), ELogType.login, "微信登录:account_id:" + account.getAccount_id() + ",IPAddr:" + ip_addr, 1L, timer.get(), session.getClientIP());
			...
			try {
				...
				if (accountWeixinModel == null) {
					...
					MongoDBServiceImpl.getInstance().log(account.getAccount_id(), ELogType.login, "微信缓存登录:account_id:" + account.getAccount_id() + ",IPAddr:" + ip_addr, 2L, timer.get(), session.getClientIP());
		...
		// 快速登录
			...
			MongoDBServiceImpl.getInstance().log(account.getAccount_id(), ELogType.login, "快速登录:account_id:" + account.getAccount_id() + ",IPAddr:" + ip_addr, 3L, timer.get(), session.getClientIP());
			// 测试
			...
			if(sysParamModel2000!=null && sysParamModel2000.getVal1()==1){
				if(accountModel!=null && accountModel.getRecommend_id()!=0){
					...
					if(accountParamModel.getVal1()==null || accountParamModel.getVal1()==0){
						...
						centerRMIServer.addAccountGold(account.getAccount_id(), addGold, false, "通过分享下载,分享人account_id:"+accountModel.getRecommend_id(), EGoldOperateType.SHARE_DOWN);
					}
				}
			}
...
//			AccountPropertyResponse.Builder accountPropertyResponseBuilder = MessageResponse.getAccountPropertyResponse(EPropertyType.BACK_IP_LIST.getId(), 30001, null, null, "118.184.252.3", null, null);
...
//			PlayerServiceImpl.getInstance().sendAccountMsg(session, responseBuilder.build());
										...
										if (errCode == 0) {
											...
											if (accountWeixinModel.getNickname() != null) {
												if (!accountWeixinModel.getNickname().equals(old_nickname)) {
													AccountPropertyResponse.Builder accountPropertyResponseBuilder = MessageResponse.getAccountPropertyResponse(EPropertyType.ACCOUNT_NICK_NAME.getId(), null, null,null,null, null, accountWeixinModel.getNickname(), null, null);
													accountPropertyListResponseBuilder.addAccountProperty(accountPropertyResponseBuilder);
												}
											}
											if (accountWeixinModel.getHeadimgurl() != null) {
												if (!accountWeixinModel.getHeadimgurl().equals(old_headimgurl)) {
													AccountPropertyResponse.Builder accountPropertyResponseBuilder = MessageResponse.getAccountPropertyResponse(EPropertyType.ACCOUNT_HEAR_IMG.getId(), null, null,null,null, null, accountWeixinModel.getHeadimgurl(), null, null);
													accountPropertyListResponseBuilder.addAccountProperty(accountPropertyResponseBuilder);
												}
											}
			...
			MongoDBServiceImpl.getInstance().log(account.getAccount_id(), ELogType.clientNotifyIp, "客户端传ip:"+accountModel.getClient_ip2()+",地区:"+ip_addr, null, null, accountModel.getClient_ip());
		}
