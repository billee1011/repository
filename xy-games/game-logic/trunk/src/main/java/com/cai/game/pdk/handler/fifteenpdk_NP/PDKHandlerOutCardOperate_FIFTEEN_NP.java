...
public class PDKHandlerOutCardOperate_FIFTEEN_NP extends PDKHandlerOutCardOperate<PDK_FIFTEEN_Table_NP> {
	@Override
	public void exe(PDK_FIFTEEN_Table_NP table) {
		...
		table._opreate_times[_out_card_player]++;
		// 刷新手牌
		...
		if (table._is_call_banker && _out_card_player != table.GRR._banker_player) {
			table._current_player = GameConstants.INVALID_SEAT;
		}
		// 刷新玩家手牌
		table.operate_player_cards();

		int other_seat[] = new int[table.getTablePlayerNumber() - 1];
		int other_num = 0;
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if (i != _out_card_player) {
				other_seat[other_num++] = i;
			}
		}
		// 显示出牌
		...
		if (0 == table.GRR._card_count[_out_card_player]) {
			...
			if (cbCardType == GameConstants.PDK_CT_BOMB_CARD) {
				...
				table.send_effect_type(_out_card_player, table.EFFECT_DI_GOLD_FEI, other_seat,
		...
		if (table.GRR != null) {
			...
			if (table._game_status == GameConstants.GS_MJ_PLAY) {
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					if (table.has_rule(GameConstants.GAME_RULE_FANG_ZUO_BI)) {
						if (i == seat_index) {
							if (table._opreate_times[i] > 0 || table._current_player == seat_index) {
						...
						if (i == seat_index || table._ming_pai[i] == 1) {
							for (int j = 0; j < table.GRR._card_count[i]; j++) {
	...
	// 玩家不出
	public void no_out_card(PDK_FIFTEEN_Table_NP table) {
		int can_out_card_data[] = new int[table.get_hand_card_count_max()];
		...
		if (table._current_player == table._out_card_player) {
			...
			if (cbCardType == GameConstants.PDK_CT_BOMB_CARD) {
				...
				table.load_player_info_data(roomResponse2);
				table.send_response_to_room(roomResponse2);
				table.send_effect_type(_out_card_player, table.EFFECT_DI_GOLD_FEI, null, GameConstants.INVALID_SEAT);
			}
	...
	// 判断玩家出牌合法
	public int adjust_out_card_right(PDK_FIFTEEN_Table_NP table) {
		...
		if (!table._is_call_banker) {
			// 黑桃必须先出
			if (table.getTablePlayerNumber() == 3 && table.has_rule(GameConstants.GAME_RULE_SHOU_JU_HEITAO_SAN)
					&& table._cur_round == 1 && table._out_card_times[_out_card_player] == 0
					&& _out_card_player == table.GRR._banker_player && playernumber == table.getTablePlayerNumber()) {
			...
			} else if (table.getTablePlayerNumber() == 2 && table._cur_round == 1
	...
	// 放走包赔
	public boolean fang_zou_bao_pei(PDK_FIFTEEN_Table_NP table) {
	...
	// 必须出最大牌
	public boolean mustmax(PDK_FIFTEEN_Table_NP table) {
	...
	// 自动出牌
	public void adjustAutoOutCard(PDK_FIFTEEN_Table_NP table, int seat_index) {
		int auto_out_card_dely = 1;
		if (table.matchId != 0) {
