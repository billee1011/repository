...
public class MJHandlerPao_ShanXi extends AbstractMJHandler<Table_ShanXi> {
	...
	public boolean handler_pao_qiang(Table_ShanXi table, int seat_index, int pao, int qiang) {
		...
		if (table._game_status == GameConstants.GS_MJ_PAO) {
			...
			// 检测听牌
			for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
				table._playerStatus[i]._hu_card_count = table.get_hnhz_ting_card_new(table._playerStatus[i]._hu_cards, table.GRR._cards_index[i],
	...
	private void player_reconnect(Table_ShanXi table, int seat_index) {
