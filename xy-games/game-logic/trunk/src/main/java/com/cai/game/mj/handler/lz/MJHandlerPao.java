...
public class MJHandlerPao extends MJHandler {
	...
	public boolean handler_pao_qiang(MJTable table, int seat_index, int pao, int qiang) {
		...
		if (table._game_status == GameConstants.GS_MJ_PAO) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				table._playerStatus[i]._hu_card_count = table.get_lz_ting_card(i, table._playerStatus[i]._hu_cards, table.GRR._cards_index[i],
			...
			// 天胡状态
			table.exe_dispatch_card(table._current_player, GameConstants.CHR_HENAN_TIAN_HU, GameConstants.DELAY_SEND_CARD_DELAY);
		}
