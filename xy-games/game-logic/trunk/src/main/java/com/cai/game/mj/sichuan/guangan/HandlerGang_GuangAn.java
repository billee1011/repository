...
public class HandlerGang_GuangAn extends MJHandlerGang<AbstractSiChuanMjTable> {
	@Override
	...
	protected boolean exe_gang(AbstractSiChuanMjTable table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			if (table.is_mj_type(GameConstants.GAME_TYPE_LE_SHAN_YAO_JI)) {
				// 及时雨，弯杠不直接杠，不计算分
				if (_center_card != table._send_card_data && table.has_rule(Constants_SiChuan.GAME_RULE_JI_SHI_YU)) {
