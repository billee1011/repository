...
public class FLSGameLogic {
			...
			if ((value_index >= excursion[i]) && ((value_index - excursion[i]) <= 0)) {
				...
				if (this._magic_card_count > 0) {
					for (int m = 0; m < this._magic_card_count; m++) {
						// 吃牌不能包含有王霸
						if (get_magic_card_index(m) != GameConstants.MAX_FLS_INDEX && get_magic_card_index(m) >= first_index
	...
	public int analyse_gang_card_fls(int cards_index[], int card, WeaveItem WeaveItem[], int cbWeaveCount, GangCardResult gangCardResult,
			boolean check_weave) {
		// 设置变量
		int cbActionMask = GameConstants.WIK_NULL;
	...
	public boolean is_qing_hu(AnalyseItemTwenty analyseItem, WeaveItem[] weaveItems, int weaveCount, int cur_card) {
		...
		for (int i = 0; i < weaveCount; i++) {
			if ((weaveItems[i].weave_kind & (GameConstants.WIK_LEFT | GameConstants.WIK_CENTER | GameConstants.WIK_RIGHT)) != 0) {
			...
			if (weaveItems[i].weave_kind == GameConstants.WIK_PENG || weaveItems[i].weave_kind == GameConstants.WIK_ZHAO
	...
	public boolean is_hong_hu(AnalyseItemTwenty analyseItem, WeaveItem[] weaveItems, int weaveCount, int cur_card) {
		int hong = 0;
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			if (analyseItem.cbWeaveKind[i] == GameConstants.WIK_PENG || analyseItem.cbWeaveKind[i] == GameConstants.WIK_ZHAO)
				return false;
			if ((analyseItem.cbWeaveKind[i] & (GameConstants.WIK_LEFT | GameConstants.WIK_CENTER | GameConstants.WIK_RIGHT)) != 0) {
				for (int j = 0; j < 3; j++) {
					int cardValue = analyseItem.cbCardData[i][j];
	 ...
	 * @return
	 */
	public boolean isYankouTwenty(int cards_index[], AnalyseItemTwenty analyseItem) {
		...
		for (int i = 0; i < GameConstants.MAX_FLS_INDEX; i++) {
			...
			if (num == 1) {
				...
				int color = get_card_color(card_data);
				if (oldColor >= 0) {
					analyseItem.cbCardEye[1] = card_data;
					return color == oldColor;
				} else {
					oldColor = color;
					analyseItem.cbCardEye[0] = card_data;
				}
			} else if (num == 2) {
				int card_data = switch_to_card_data(i);
				analyseItem.eyeDui = true;
				analyseItem.cbCardEye[0] = card_data;
				analyseItem.cbCardEye[1] = card_data;
				return true;
			}
	...
	public boolean analyse_card(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, List<AnalyseItem> analyseItemArray, boolean has_feng,
		...
		if (cbCardCount == 1) {// 满天飞1张牌 胡牌的情况
			...
			analyseItem.cbCardEye[0] = cur_card;
			analyseItem.eyeCenterCard = cur_card;
	...
	// 分析扑克
	public boolean analyse_card_twenty(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, List<AnalyseItemTwenty> analyseItemArray,
			boolean has_feng, int cur_card) {
		// 计算数目
		int cbCardCount = get_card_count_by_index(cards_index);
		...
		if (cbLessKindItem == 0) {
			// 效验参数
			if (((cbCardCount == 2) && (cbWeaveCount == 6)) == false) {
				return false;
			}
			AnalyseItemTwenty analyseItem = new AnalyseItemTwenty();
			if (isYankouTwenty(cards_index, analyseItem)) {
				// 插入结果
				analyseItemArray.add(analyseItem);
				return true;
			}
		...
		if (cbKindItemCount >= cbLessKindItem) {
			...
			do {
				...
				if (bEnoughCard == true) {
					// 牌眼判断
					AnalyseItemTwenty analyseItem = new AnalyseItemTwenty();
					if (isYankouTwenty(cbCardIndexTemp, analyseItem)) {
						// 设置牌型
						for (int i = 0; i < cbLessKindItem; i++) {
							analyseItem.cbWeaveKind[i + cbWeaveCount] = pKindItem[i].cbWeaveKind;
							analyseItem.cbCenterCard[i + cbWeaveCount] = pKindItem[i].cbCenterCard;
