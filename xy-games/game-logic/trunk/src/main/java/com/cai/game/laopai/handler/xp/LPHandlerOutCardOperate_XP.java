...
public class LPHandlerOutCardOperate_XP extends LPHandlerOutCardOperate<LPTable_XP> {

	@Override
	public void exe(LPTable_XP table) {
		if (table._player_result.nao[_out_card_player] > 0) {
			return;
		}
		int outcard_count = 0;
		...
		if (bAroseAction == false) {
			...
			table.exe_add_discard(_out_card_player, 1, new int[] { real_card }, true,
	 ...
	 */
	@Override
	public boolean handler_operate_card(LPTable_XP table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {// 不能通炮-点了胡不一定能胡
			...
			if (table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_PENG)) {
				table._can_not_pen_index[seat_index][table._logic.switch_to_card_index(operate_card)]++;
			}
		}
		...
		// 变量定义
		int target_card = table._playerStatus[target_player].get_weave_card(target_action);
		...
		case GameConstants.WIK_LEFT: // 上牌操作
		{
			...
			table._out_card_index[target_player][table._logic.switch_to_card_index(target_card) + 1]++;
			table._out_card_index[target_player][table._logic.switch_to_card_index(target_card) + 2]++;
			// 记牌器
			table.operate_jipaiqi();
			return true;
		}
		...
		case GameConstants.WIK_PENG: // 碰牌操作
		{
			...
			table._out_card_index[target_player][table._logic.switch_to_card_index(target_card)]++;
			table._out_card_index[target_player][table._logic.switch_to_card_index(target_card)]++;
		...
		case GameConstants.WIK_GANG: // 杠牌操作
		{
			...
			table._out_card_index[target_player][table._logic.switch_to_card_index(target_card)] += 3;
		...
		case GameConstants.WIK_NULL: {
			...
			int real_card = _out_card_data;
			// 加到牌堆 没有人要
			table.exe_add_discard(this._out_card_player, 1, new int[] { real_card }, true, 0);
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_xp(target_player, _out_card_player, _out_card_data, false);
			// 记录
