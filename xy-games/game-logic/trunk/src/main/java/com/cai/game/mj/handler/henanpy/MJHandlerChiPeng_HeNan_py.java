...
public class MJHandlerChiPeng_HeNan_py extends MJHandlerChiPeng<MJTable_PY> {
	private GangCardResult m_gangCardResult;

	public MJHandlerChiPeng_HeNan_py() {
	...
	public void exe(MJTable_PY table) {
		...
		if (table.has_rule(GameConstants.GAME_RULE_PUYANG_YAOJIUPU)) {
			for (int j = 0; j < hand_card_count; j++) {
				if (table._logic.switch_to_card_index(cards[j]) % 9 == 0 && table._logic.switch_to_card_index(cards[j]) < 27) {
					cards[j] += GameConstants.CARD_ESPECIAL_TYPE_YAOJIU;
				} else if (table._logic.switch_to_card_index(cards[j]) % 9 == 8 && table._logic.switch_to_card_index(cards[j]) < 27) {
					cards[j] += GameConstants.CARD_ESPECIAL_TYPE_YAOJIU;
				}
			}
		}
	 ...
	 */
	@Override
	public boolean handler_player_out_card(MJTable_PY table, int seat_index, int card) {
