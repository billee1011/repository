...
import com.cai.game.schcpdss.SCHCPDSSGameLogic.AnalyseItem;
...
import com.cai.game.schcpdss.handler.dss.SCHCPHandlerChiPeng_DSS;
import com.cai.game.schcpdss.handler.dss.SCHCPHandlerChuLiFirstCard_DSS;
import com.cai.game.schcpdss.handler.dss.SCHCPHandlerDaYiPiao_DSS;
import com.cai.game.schcpdss.handler.dss.SCHCPHandlerDispatchAddCard_DSS;
import com.cai.game.schcpdss.handler.dss.SCHCPHandlerDispatchCard_DSS;
import com.cai.game.schcpdss.handler.dss.SCHCPHandlerDispatchFirstCard_DSS;
import com.cai.game.schcpdss.handler.dss.SCHCPHandlerDispatchTouCard_DSS;
import com.cai.game.schcpdss.handler.dss.SCHCPHandlerGang_DSS;
import com.cai.game.schcpdss.handler.dss.SCHCPHandlerOutCardOperate_DSS;
import com.cai.game.schcpdss.handler.dss.SCHCPHandlerPaiQuan_DSS;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class SCHCPDSSTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	private static Logger logger = Logger.getLogger(SCHCPDSSTable.class);
	...
	public SCHCPDSSGameLogic _logic = null;
	...
	public SCHCPDSSTable() {
		super(RoomType.HH, GameConstants.GAME_PLAYER);

		_logic = new SCHCPDSSGameLogic();
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_zhe_ding_card = false;
		_zhe_fu_card = false;
		_handler_dispath_card = new SCHCPHandlerDispatchCard_DSS();
		_handler_out_card_operate = new SCHCPHandlerOutCardOperate_DSS();
		_handler_gang = new SCHCPHandlerGang_DSS();
		_handler_chi_peng = new SCHCPHandlerChiPeng_DSS();
		_handler_dispath_add_card = new SCHCPHandlerDispatchAddCard_DSS();
		_handler_chuli_firstcards = new SCHCPHandlerChuLiFirstCard_DSS();
		_handler_dispath_firstcards = new SCHCPHandlerDispatchFirstCard_DSS();
		_handler_dispatch_toucards = new SCHCPHandlerDispatchTouCard_DSS();
		_handler_dayipiao_fen = new SCHCPHandlerDaYiPiao_DSS();
		_handler_quan_pai = new SCHCPHandlerPaiQuan_DSS();
	...
	protected boolean on_handler_game_start() {
		...
		if (_game_type_index == GameConstants.GAME_TYPE_QIONG_LAI_HONG) {
			return on_handler_game_start_qlhf();
		}
		_game_status = GameConstants.GS_MJ_PLAY;
		...
		if (DEBUG_CARDS_MODE || BACK_DEBUG_CARDS_MODE)
			test_cards();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			if (i == this._shang_zhuang_player && _player_result.pao[i] == 1)
				continue;
			_player_result.pao[i] = -1;
		}
		if (is_mj_type(GameConstants.GAME_TYPE_CP_DSS_DY) && this.getRuleValue(GameConstants.GAME_RULE_DSS_SHUANG_FAN) == 0) {
			game_start_cp();
		} else
			exe_quan_pai();
		GameSchedule.put(new AnimationRunnable(getRoom_id(), ID_TIMER_ANIMATION_START), 600, TimeUnit.MILLISECONDS);
	...
	private boolean game_start_cp() {
		...
		// 发送数据
		if (is_mj_type(GameConstants.GAME_TYPE_CP_DSS_QL)) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				for (int j = 0; j < GRR._card_count[i]; j++) {
					hand_cards[i][j] = GRR._cards_data[i][j];
				}
				int hand_card_count = GRR._card_count[i];
				gameStartResponse.addCardsCount(hand_card_count);
			}
		}
		...
		// FlashTime + standTime);
		if (is_mj_type(GameConstants.GAME_TYPE_CP_DSS_QL) || is_mj_type(GameConstants.GAME_TYPE_QIONG_LAI_HONG)) {
			...
			this.exe_da_yi_piao(this._cur_banker, GameConstants.WIK_NULL, 200);
		}
	...
	public void shuffle(int repertory_card[], int mj_cards[]) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (this._cur_banker == i)
				send_count = GameConstants.DSS_MAX_CP_COUNT;
	...
	public void test_cards() {
		...
		// testSameCard(new int[] { 0x42, 0x42, 0x42, 0xf8, 0xf8, 0x80 });
		/********
	...
	public void testRealyCard(int[] realyCards) {
		...
		if (this._cur_banker < 0 || this._cur_banker >= this.getTablePlayerNumber())
			this._cur_banker = 0;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			send_count = (GameConstants.DSS_MAX_CP_COUNT - 1);
			if (this._cur_banker == i)
	...
	public int analyse_chi_hu_card(int cards_index[], WeaveItem weaveItems[], int weave_count, int seat_index, int provider_index, int cur_card,
		...
		if (_game_type_index == GameConstants.GAME_TYPE_QIONG_LAI_HONG) {
			return analyse_chi_hu_card_cp_qlhf(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type,
					dispatch);
		}
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_cp_qlhf(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index,
		...
		boolean dui_zi_hu = true;
		for (int i = 0; i < GameConstants.MAX_CP_INDEX; i++) {
			if (cbCardIndexTemp[i] == 0) {
				continue;
			}
			if (cbCardIndexTemp[i] % 2 != 0) {
				dui_zi_hu = false;
				break;
			}
		}
		...
		for (int i = 0; i < GameConstants.MAX_CP_INDEX; i++) {
			...
			all_pai_count += cbCardIndexTemp[i];
			for (int j = 0; j < cbCardIndexTemp[i]; j++) {
				if (_logic.get_hong_dot(_logic.switch_to_card_data(i)) != 0) {
					hong_pai_count++;
				} else {
					hei_pai_count++;
				}
		...
		for (int w = 0; w < weaveCount; w++) {
			hong_pai_count += _logic.get_hong_pai_count(weaveItems[w].weave_kind, weaveItems[w].center_card);
			hei_pai_count += _logic.get_hei_pai_count(weaveItems[w].weave_kind, weaveItems[w].center_card);
		}
		all_pai_count = hei_pai_count + hong_pai_count;

		int cards[] = new int[GameConstants.DSS_MAX_CP_COUNT];
		int card_count = _logic.switch_to_cards_data(GRR._cards_index[seat_index], cards);
		for (int w = 0; w < weaveCount; w++) {
			cards[card_count++] = weaveItems[w].center_card;
			cards[card_count++] = weaveItems[w].center_card;
			cards[card_count++] = weaveItems[w].center_card;
		}
		for (int c = 0; c < card_count; c++) {
			cards[c] = _logic.get_card_value_cp(cards[c]);
		}
		int diff_card_count = 0;
		int diff_card = 0;
		int[] diff_cards = new int[6];
		cards = _logic.sortCard(cards, card_count);
		for (int c = 0; c < card_count; c++) {
			if (diff_card != cards[c]) {
				diff_cards[diff_card_count++] = cards[c];
				diff_card = cards[c];
			}
		}
		...
		if (dui_zi_hu) {
			...
			if (diff_card_count == 1 || (diff_card_count == 2 && diff_cards[0] + diff_cards[1] == 14)) {
				chiHuRight.opr_or(GameConstants.CHR_CP_TONG_MENG);
			}
			chiHuRight.opr_or(GameConstants.CHR_CP_THREE_DUI);
			return GameConstants.WIK_CHI_HU;
		}

		List<AnalyseItem> analyseItemArray = new ArrayList<AnalyseItem>();

		boolean bValue = _logic.analyse_card_qihf(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index, cur_card, analyseItemArray,
				dispatch);
		if (!bValue) {
		...
		for (int j = 0; j < 9; j++) {
			if (analyseItem.cbWeaveKind[j] == GameConstants.WIK_NULL)
				break;
			if (analyseItem.cbWeaveKind[j] == GameConstants.DZ_WIK_PENG || analyseItem.cbWeaveKind[j] == GameConstants.DSS_WIK_PENG)
				peng_count++;
	...
	public int analyse_chi_hu_card_cp_dss(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index, int cur_card,
		...
		if (all_pai_count != hei_pai_count) {
			if (is_mj_type(GameConstants.GAME_TYPE_CP_DSS_QL)) {
				if (this.getRuleValue(GameConstants.GAME_RULE_DSS_FOUR) == 1) {
					if (hong_dian_shu > 2 && temp_hu_xi < 30) {
						chiHuRight.set_empty();
						return GameConstants.DSS_WIK_NULL;
					}
				} else {
					if (hong_dian_shu > 2 && temp_hu_xi < 40) {
						chiHuRight.set_empty();
						return GameConstants.DSS_WIK_NULL;
					}
				}
			}
			if (is_mj_type(GameConstants.GAME_TYPE_CP_DSS_DY)) {
		...
		if (all_pai_count == hei_pai_count) {
			...
			if (hong_pai_count > 0 && hong_dian_shu <= 2) {
	...
	public boolean get_end() {
		...
		if (this.getTablePlayerNumber() == 3 && GRR._left_card_count <= 7)
			is_end = true;
		if (this.getTablePlayerNumber() == 4 && GRR._left_card_count <= 5)
			is_end = true;
		return is_end;
	}
	...
	public void process_chi_hu_player_score_schcp(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		if (!(chr.opr_and(GameConstants.CHR_CP_DI_HU)).is_empty()) {
			wFanShu += 1;
		}
		for (int i = 0; i < this._hu_weave_count[seat_index]; i++) {
			if (_hu_weave_items[seat_index][i].center_card == 0x12 || _hu_weave_items[seat_index][i].center_card == 0x0b) {
				di_fu += _logic.get_analyse_tuo_shu(this._hu_weave_items[seat_index][i].weave_kind, this._hu_weave_items[seat_index][i].center_card);
			} else
				all_hu_xi += _logic.get_analyse_tuo_shu(this._hu_weave_items[seat_index][i].weave_kind,
						this._hu_weave_items[seat_index][i].center_card);
			switch (this._hu_weave_items[seat_index][i].weave_kind) {
			case GameConstants.DSS_WIK_PENG: {
				if (this._hu_weave_items[seat_index][i].center_card == this._player_card_type) {
					wFanShu += 1;
				}
		...
		wFanShu += _logic.get_four_count(this._hu_weave_items[seat_index], this._hu_weave_count[seat_index],
				(this.getRuleValue(GameConstants.GAME_RULE_DSS_JSG) == 1));

		if (this._is_ting_pai[seat_index] == 1)
			wFanShu += 1;
		if (all_hu_xi % 10 != 0)
			this._hu_xi[seat_index] = (all_hu_xi / 10 + 1) * 10 + calculate_score;
		else
			this._hu_xi[seat_index] = all_hu_xi + calculate_score;

		if (calculate_score > 0)
			this._hu_xi[seat_index] = calculate_score;
		if (is_mj_type(GameConstants.GAME_TYPE_CP_DSS_QL)) {
			if (this.getRuleValue(GameConstants.GAME_RULE_DSS_THREE) == 1) {
				if (all_hu_xi == 40)
					wFanShu = 0;
			} else {
				if (all_hu_xi == 30)
			...
			if (all_hu_xi == 30 && this.getRuleValue(GameConstants.GAME_RULE_DSS_30_YA) == 1)
				wFanShu += 1;
		...
		if (zimo) {
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				if (this.getRuleValue(GameConstants.GAME_RULE_DSS_4_TFD) == 1)
					if (wFanShu > 4)
						wFanShu = 4;
				if (this.getRuleValue(GameConstants.GAME_RULE_DSS_5_TFD) == 1)
					if (wFanShu > 5)
						wFanShu = 5;
				if (this.getRuleValue(GameConstants.GAME_RULE_DSS_6_TFD) == 1)
					if (wFanShu > 6)
						wFanShu = 6;
				wFanShu = (int) Math.pow(2, wFanShu);
				if (wFanShu == 0)
					wFanShu = 1;
				float lChiHuScore = wFanShu * this._hu_xi[seat_index];
				if (this.getRuleValue(GameConstants.GAME_RULE_DSS_1000_FD) == 1)
					if (lChiHuScore > 1000)
						lChiHuScore = 1000;
				if (this.getRuleValue(GameConstants.GAME_RULE_DSS_2000_FD) == 1) {
					if (this._is_ting_pai[i] == 1 && is_fan_bei == true) {
						if (lChiHuScore > 2000)
							lChiHuScore = 2000;
					} else if (lChiHuScore > 1000)
						lChiHuScore = 1000;
				...
				if (this.getRuleValue(GameConstants.GAME_RULE_DSS_GGF) == 1) {
					if (fan_piao == 0)
						fan_piao = 1;
					else
						fan_piao *= 2;
				}
				if (this.getRuleValue(GameConstants.GAME_RULE_DSS_TTF) == 1) {
					if (fan_piao == 0)
						fan_piao = 1;
					else
						fan_piao += 1;
		...
		for (int i = 0; i < mj_count; i++) {
			...
			if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card(cbCardIndexTemp, weaveItem, cbWeaveCount, seat_index, provate_index, cbCurrentCard,
				...
				if (is_mj_type(GameConstants.GAME_TYPE_CP_DSS_QL)) {
					...
					if (this.getRuleValue(GameConstants.GAME_RULE_DSS_FOUR) == 1 && is_bao_ting == false) {
						if (this._hu_xi[seat_index] - hong_dot < 31)
							continue;
					} else if (is_bao_ting == false) {
						if (this._hu_xi[seat_index] - hong_dot < 41)
							continue;
					}
				} else if (is_mj_type(GameConstants.GAME_TYPE_CP_DSS_DY)) {
					boolean is_bao_ting = false; // 可以2000封顶
					if (!(chr.opr_and(GameConstants.CHR_CP_ALL_HEI)).is_empty()) {
					...
					if (is_bao_ting == false) {
						if (this._hu_xi[seat_index] - hong_dot < 30)
							continue;
					}
				}
	...
	public boolean operate_player_cards(int seat_index, int card_count, int cards[], int weave_count, WeaveItem weaveitems[]) {
		...
		if (weave_count > 0) {
			for (int j = 0; j < weave_count; j++) {
				...
				if ((weaveitems[j].weave_kind == GameConstants.DSS_WIK_PENG) && this._is_display == false) {
