...
public class HandlerOutCardOperate_NING_XIANG extends MJHandlerOutCardOperate<Table_NING_XIANG> {
	...
	public void exe(Table_NING_XIANG table) {
		...
		if (bAroseAction == false) {
			...
			if(table.GRR._left_card_count == 1){
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					int p = (next_player + i + table.getTablePlayerNumber()) % table.getTablePlayerNumber();
					if(table._playerStatus[p]._hu_card_count > 0){
						table._playerStatus[p].add_action(Constants_MJ_NING_XIANG.WIK_YAO_HAI_DI_PAI);
				...
				GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), _out_card_player, GameConstants.Game_End_DRAW), GameConstants.GAME_FINISH_DELAY,
						TimeUnit.SECONDS);
				return;
			}else{
				if(table.is_dao_pai[next_player])
					table.exe_qi_pai(next_player);
	...
	public boolean handler_operate_card(Table_NING_XIANG table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			...
			if(table._playerStatus[seat_index].has_action_by_code(Constants_MJ_NING_XIANG.WIK_YAO_HAI_DI_PAI)){
				table.operate_effect_action(seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { Constants_MJ_NING_XIANG.ACTION_GUO }, 1, GameConstants.INVALID_SEAT);
				for (int i = 1; i < table.getTablePlayerNumber(); i++) {
					if(seat_index == _out_card_player)
						break;
					int p = (seat_index + i + table.getTablePlayerNumber()) % table.getTablePlayerNumber();
					if(table._playerStatus[p]._hu_card_count > 0){
						table._playerStatus[p].add_action(Constants_MJ_NING_XIANG.WIK_YAO_HAI_DI_PAI);
						table.change_player_status(p, GameConstants.Player_Status_OPR_CARD);
						table.operate_player_action(p, false);
					...
					if(p == _out_card_player)
						break;
				}
				//流局
				table._cur_banker = (_out_card_player + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();
				GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), _out_card_player, GameConstants.Game_End_DRAW), GameConstants.GAME_FINISH_DELAY,
						TimeUnit.SECONDS);
				return true;
			}
	...
	private void handler_operate_tong_pao(Table_NING_XIANG table, int seat_index,
		...
		case Constants_MJ_NING_XIANG.WIK_YAO_HAI_DI_PAI: {
			table._playerStatus[seat_index].clean_action(Constants_MJ_NING_XIANG.WIK_YAO_HAI_DI_PAI);
			table._send_card_count++;
			table.last_dispatch_card_player = seat_index;
			table._send_card_data = table._repertory_card[table._all_card_len - table.GRR._left_card_count];
			table._send_card_data = table.DEBUG_CARDS_MODE ? 0x22 : table._send_card_data;
			--table.GRR._left_card_count;
			_out_card_player = seat_index;
			//客户端显示到桌面
			table.operate_out_card(seat_index, 1, new int[]{table._send_card_data},
	                GameConstants.OUT_CARD_TYPE_XIA_ZI, GameConstants.INVALID_SEAT);
			...
			// 检查牌型,听牌
			chr.opr_or_long(Constants_MJ_NING_XIANG.CHR_HAI_DI_LAO_YUE);
			int action = table.analyse_chi_hu_card(table.GRR._cards_index[seat_index], table.GRR._weave_items[seat_index],
					table.GRR._weave_count[seat_index], table._send_card_data, chr, card_type, seat_index);
			if (action != GameConstants.WIK_NULL) {
				/*GameSchedule.put(()->{
					handler_operate_card(table, seat_index, GameConstants.WIK_CHI_HU, table.DEBUG_CARDS_MODE ? 0x15 : _send_card_data);
				},2, TimeUnit.SECONDS);
				return;*/
				table._playerStatus[seat_index].add_action(GameConstants.WIK_CHI_HU);
				table._playerStatus[seat_index].add_chi_hu(table._send_card_data, seat_index);
				if(table._player_result.biaoyan[seat_index] == 1){
					handler_operate_card(table, seat_index, GameConstants.WIK_CHI_HU, table._send_card_data);
				}else{
					table.change_player_status(seat_index, GameConstants.Player_Status_OPR_CARD);
					table.operate_player_action(seat_index, false);
				}
			} else {
				checkOtherHu(table, seat_index, table._send_card_data);
			}
			return;
		}
        case GameConstants.WIK_NULL: {
        	...
        	if(table._send_card_data > 0){
    			...
    			if(checkOtherHu(table, seat_index, table._send_card_data))
    				return;
    		}
        	if(table.GRR._left_card_count == 1){
    			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
    				int p = (_current_player + i + table.getTablePlayerNumber()) % table.getTablePlayerNumber();
    				if(table._playerStatus[p]._hu_card_count > 0){
    					table._playerStatus[p].add_action(Constants_MJ_NING_XIANG.WIK_YAO_HAI_DI_PAI);
    					table.change_player_status(p, GameConstants.Player_Status_OPR_CARD);
    					table.operate_player_action(p, false);
				...
				//流局
				table._cur_banker = _current_player;
				GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), _out_card_player, GameConstants.Game_End_DRAW), GameConstants.GAME_FINISH_DELAY,
						TimeUnit.SECONDS);
				return;
			}else{
				if(table.is_dao_pai[_current_player])
					table.exe_qi_pai(_current_player);
        ...
        {
            int jie_pao_count = 0;
            boolean is_hai_di = !table.GRR._chi_hu_rights[seat_index].opr_and_long(Constants_MJ_NING_XIANG.CHR_HAI_DI_LAO_YUE).is_empty();
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if ((table.GRR._chi_hu_rights[i].is_valid() == false)) {
                    continue;
                }
                if(!table.GRR._chi_hu_rights[i].opr_and_long(Constants_MJ_NING_XIANG.CHR_HAI_DI_PAO).is_empty())
                	is_hai_di = true;
                jie_pao_count++;
            }

            if (jie_pao_count > 0) {
            	if(table.GRR._chi_hu_rights[seat_index].opr_and_long(Constants_MJ_NING_XIANG.CHR_HAI_DI_LAO_YUE).is_empty())
            		table.GRR._chi_hu_rights[_out_card_player].opr_or_long(Constants_MJ_NING_XIANG.CHR_FANG_PAO);
            	if (jie_pao_count > 1) {
                    // 通炮
                    table._cur_banker = _out_card_player;
                    table.set_niao_card(_out_card_player, is_hai_di);
                } else if (jie_pao_count == 1) {
                    table._cur_banker = target_player;
                    table.set_niao_card(target_player, is_hai_di);
                }
                for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                    if ((table.GRR._chi_hu_rights[i].is_valid() == false)) {
                        continue;
                    ...
                    if(_type == GameConstants.GANG_TYPE_HONG_ZHONG)
        				table.GRR._chi_hu_rights[i].opr_or_long(Constants_MJ_NING_XIANG.CHR_DI_HU);
                    // 效果
                    table.process_chi_hu_player_operate(i, operate_card, false);
                    if(!table.GRR._chi_hu_rights[i].opr_and_long(Constants_MJ_NING_XIANG.CHR_HAI_DI_LAO_YUE).is_empty()){
                    	table.process_chi_hu_player_score(i, i, operate_card, true);
                    }else if(!table.GRR._chi_hu_rights[i].opr_and_long(Constants_MJ_NING_XIANG.CHR_HAI_DI_PAO).is_empty()){
                    	table.process_chi_hu_player_score(i, table.last_dispatch_card_player, operate_card, false);
                    }else
	...
	private boolean checkOtherHu(Table_NING_XIANG table, int seat_index,
			int _send_card_data) {
		boolean flag = false;
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			int p = (seat_index + 1 + i + table.getTablePlayerNumber()) % table.getTablePlayerNumber();
			if(p == seat_index)
				break;
			table.GRR._chi_hu_rights[p].set_empty();
			// 检查牌型,听牌
			table.GRR._chi_hu_rights[p].opr_or_long(Constants_MJ_NING_XIANG.CHR_HAI_DI_PAO);
			int action1 = table.analyse_chi_hu_card(table.GRR._cards_index[p], table.GRR._weave_items[p],
					table.GRR._weave_count[p], _send_card_data, table.GRR._chi_hu_rights[p], Constants_MJ_NING_XIANG.HU_CARD_TYPE_DIAN_PAO, p);
			if(action1 != GameConstants.WIK_NULL){
				flag = true;
				table._playerStatus[p].add_action(GameConstants.WIK_CHI_HU);
				table._playerStatus[p].add_chi_hu(_send_card_data, seat_index);
			}
		}
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (playerStatus.has_action()) {
				if((playerStatus.has_chi_hu() && table._player_result.biaoyan[i] == 1)){
					handler_operate_card(table, i, GameConstants.WIK_CHI_HU, _send_card_data);
				}else{
					table.change_player_status(i, GameConstants.Player_Status_OPR_CARD);
					table.operate_player_action(i, false);
				}
		...
		if(!flag){
			...
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), seat_index, GameConstants.Game_End_DRAW), GameConstants.GAME_FINISH_DELAY,
					TimeUnit.SECONDS);
		}
		return flag;
	}
