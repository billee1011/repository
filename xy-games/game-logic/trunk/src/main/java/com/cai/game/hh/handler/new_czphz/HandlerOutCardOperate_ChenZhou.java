...
public class HandlerOutCardOperate_ChenZhou extends HHHandlerOutCardOperate<Table_New_ChenZhou> {
	...
	public void exe(Table_New_ChenZhou table) {
		...
		if (table.is_card_has_wei(_out_card_data)) { // 玩家射跑
			...
			table.shoot_count[_out_card_player]++;
		}
		...
		for (int p = 0; p < table.getTablePlayerNumber(); p++) {
			...
			action_hu[i] = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i], table.GRR._weave_count[i], i,
					_out_card_player, table._out_card_data, chr[i], card_type, hu_xi_chi, false);
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				if (table.has_rule(Constants_New_ChenZhou.GAME_RULE_TIAN_DI_HU)) {
					boolean first_out_card = table.check_first_out_card();
					if (first_out_card)
						chr[i].opr_or(Constants_New_ChenZhou.CHR_DI_HU);
				}

				tempPlayerStatus.add_action(GameConstants.WIK_CHI_HU);
				tempPlayerStatus.add_chi_hu(table._out_card_data, _out_card_player);
				...
				if (table.has_rule(Constants_New_ChenZhou.GAME_RULE_FANG_PAO_BI_HU)
		...
		if (ti_sao != GameConstants.WIK_PAO) { // 如果没人跑牌，判断有没有吃碰
			if (table.GRR._left_card_count > 0) { // 不是最后一张牌才判断吃碰
				bAroseAction = table.estimate_player_out_card_respond_chen_zhou(_out_card_player, table._out_card_data, false); // 有没有吃碰的
			}
		}
		...
		if (bAroseAction == false && bHupai == 0) {
			...
			table.exe_add_discard(_out_card_player, 1, new int[] { table._out_card_data }, true, table.time_for_add_discard);
	...
	public boolean handler_operate_card(Table_New_ChenZhou table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		case GameConstants.WIK_NULL: {
			...
			table._last_player = _current_player;
			table.exe_dispatch_card(_current_player, GameConstants.WIK_NULL, table.time_for_dispatch_card);
	...
	public boolean handler_player_be_in_room(Table_New_ChenZhou table, int seat_index) {
		...
		for (int x = 0; x < hand_card_count; x++) {
			if (table.is_card_has_wei(hand_cards[x])) { // 如果是偎的牌
				...
				if (table.shoot_count[_out_card_player] > 1) {
					hand_cards[x] += GameConstants.CARD_ESPECIAL_TYPE_CAN_SHOOT;
				} else {
					// 判断打出这张牌是否能听牌
					table.GRR._cards_index[_out_card_player][table._logic.switch_to_card_index(hand_cards[x])]--;
					boolean b_is_ting_state = table.is_ting_state(table.GRR._cards_index[_out_card_player], table.GRR._weave_items[_out_card_player],
