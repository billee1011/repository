...
public class HandlerOutCardOperate_JingDian_CS_QY extends MJHandlerOutCardOperate<Table_JingDian_CS_QY> {
	boolean has_chi_hu = false;
	...
	public void exe(Table_JingDian_CS_QY table) {
		...
		boolean bAroseAction = table.estimate_player_out_card_response(_out_card_player, _out_card_data);
	...
	public boolean handler_operate_card(Table_JingDian_CS_QY table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_CHI_HU: {
			table.GRR._chi_hu_rights[_out_card_player].opr_or(Constants_JingDian_CS_QY.CHR_FANG_PAO);
			...
			if (jie_pao_count > 0) {
				if (jie_pao_count > 1) {
					...
					table.set_niao_card(_out_card_player, true, false);
