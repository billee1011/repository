...
public class HandlerOutCardOperate_PingJiang extends MJHandlerOutCardOperate<Table_PingJiang> {
	...
	public void exe(Table_PingJiang table) {
		...
		int hand_card_count = table.switch_to_cards_data_bao_ting(table.GRR._cards_index[_out_card_player], cards, _out_card_player);
		//刷新手牌
		...
		if(!bAroseAction)
			bAroseAction = table.estimate_player_out_card_respond_chi(_out_card_player, _out_card_data);
		...
		if (bAroseAction == false) {
			...
			if(table.dami_cards.size() > 0){
				GameSchedule.put(()->{
					...
					handler_player_out_card(table, Integer.valueOf(out_card[1]), Integer.valueOf(out_card[0]));
				...
				if(table.GRR._left_card_count == 1){
					table.operate_effect_action(GameConstants.INVALID_SEAT, GameConstants.Effect_Action_Other, 1,
							new long[] { Constants_PING_JIANG.WIK_HAI_DI }, 1, GameConstants.INVALID_SEAT);
					table._playerStatus[next_player].add_action(Constants_PING_JIANG.WIK_HAI_DI);
					table.change_player_status(next_player, GameConstants.Player_Status_OPR_CARD);
	...
	public boolean handler_player_out_card(Table_PingJiang table, int seat_index, int card) {
		...
		table.exe_out_card(seat_index, card, 0);
	...
	public boolean handler_operate_card(Table_PingJiang table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			...
			//要海底牌
			if(table._playerStatus[seat_index].has_action_by_code(Constants_PING_JIANG.WIK_HAI_DI)){
				...
				int p = (seat_index + 1 + table.getTablePlayerNumber()) % table.getTablePlayerNumber();
				table._playerStatus[p].add_action(Constants_PING_JIANG.WIK_HAI_DI);
	...
	private void handler_operate_tong_pao(Table_PingJiang table, int seat_index,
		...
		boolean has_chi = false;
		if(table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_LEFT) ||
		table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_RIGHT) ||
		table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_CENTER))
			has_chi = true;
		if (operate_code == GameConstants.WIK_CHI_HU) {
		...
		case Constants_PING_JIANG.WIK_HAI_DI: {
			//客户端显示到桌面
			table.operate_show_card(seat_index, GameConstants.Show_Card_Center, 1, new int[] { table._repertory_card[table._all_card_len - table.GRR._left_card_count] },
	                GameConstants.INVALID_SEAT);
        ...
        case GameConstants.WIK_NULL: {
        	if(!has_chi){
        		boolean bAroseAction = table.estimate_player_out_card_respond_chi(_out_card_player, _out_card_data);
        		if (bAroseAction) {
        			int i = (_out_card_player + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();
        ...
        case GameConstants.WIK_CHI_HU: // 胡
        {
            ...
            if (jie_pao_count > 0) {
            	...
            	table.GRR._chi_hu_rights[_out_card_player].opr_or(Constants_PING_JIANG.CHR_FANG_PAO);
            	if (jie_pao_count > 1) {
                    // 通炮
            	...
            	table.set_niao_card();
                table.GRR._chi_hu_rights[_out_card_player].opr_or(Constants_MJ_XI_AN.CHR_FANG_PAO);
                for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                    if ((i == _out_card_player) || (table.GRR._chi_hu_rights[i].is_valid() == false)) {
                        continue;
                    }
	...
	public boolean handler_player_be_in_room(Table_PingJiang table, int seat_index) {
		...
		table.switch_to_cards_data_bao_ting(table.GRR._cards_index[seat_index], hand_cards, seat_index);
