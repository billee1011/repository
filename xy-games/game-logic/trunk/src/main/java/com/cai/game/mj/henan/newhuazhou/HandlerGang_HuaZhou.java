...
public class HandlerGang_HuaZhou extends MJHandlerGang<Table_HuaZhou> {
	public HandlerGang_HuaZhou() {
	}
	...
	public boolean exe_gang(Table_HuaZhou table) {
		int cbCardIndex = table._logic.switch_to_card_index(_center_card);
		int cbWeaveIndex = -1;

		// 清理晒张
		if (table.is_shai_zhang_card(_seat_index, _center_card)) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				table.cal_an_gang_score(i, 0 - 2 * GameConstants.CELL_SCORE);
				table.cal_an_gang_score(_seat_index, 2 * GameConstants.CELL_SCORE);
			}
			...
			table.cal_dian_gang_score(_provide_player, 0 - 3 * GameConstants.CELL_SCORE);
			table.cal_dian_gang_score(_seat_index, 3 * GameConstants.CELL_SCORE);
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
				table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += 1 * GameConstants.CELL_SCORE;
				table.GRR._gang_score[_seat_index].scores[cbGangIndex][i] -= 1 * GameConstants.CELL_SCORE;

				table.cal_ming_gang_score(i, 0 - 1 * GameConstants.CELL_SCORE);
				table.cal_ming_gang_score(_seat_index, 1 * GameConstants.CELL_SCORE);
			}
