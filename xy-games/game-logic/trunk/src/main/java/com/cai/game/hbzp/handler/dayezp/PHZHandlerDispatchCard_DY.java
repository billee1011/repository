 ...
 *
 */
public class PHZHandlerDispatchCard_DY extends PHZHandlerDispatchCard<HBPHZTable> {
	...
	public void exe(HBPHZTable table) {
		...
		if (an_long_count > 0) {
			...
			for (int i = 0; i < an_long_count; i++) {
				...
				table.GRR._weave_items[_seat_index][cbWeaveIndex].hu_xi = table._logic
						.get_weave_hu_xi(table.GRR._weave_items[_seat_index][cbWeaveIndex], table._fan_jiang_card);
		...
		int is_kai_zhao = GameConstants.WIK_NULL;
		int loop = 0;
		while (loop < table.getTablePlayerNumber()) {
			...
			action_pao[i] = table.estimate_player_respond_hh(i, _seat_index, _send_card_data, pao_type[i], true);
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				if (table.has_rule(GameConstants.GAME_RULE_QIANG_ZHI_HU_PAI)) {
					...
					table.handler_operate_card(i, GameConstants.WIK_ZI_MO, _send_card_data, -1);
					return;
				}
				if (action_pao[i] != GameConstants.WIK_HBZP_ZHAO) {
					...
					if (count != 0) {
						...
						tempPlayerStatus.add_hbzp_zhao(_send_card_data, GameConstants.WIK_HBZP_ZHAO, _seat_index);
					}
				...
				if (bHupai == 0)
					table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants.INVALID_SEAT, false);
				bHupai = 1;
				is_kai_zhao = GameConstants.WIK_ZI_MO;
			} else {
				chr[i].set_empty();
			}
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if ((action_pao[i] != GameConstants.WIK_NULL) && (bHupai == 0)) {
				is_kai_zhao = GameConstants.WIK_HBZP_ZHAO;
				table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants.INVALID_SEAT, false);
		...
		if (is_kai_zhao != GameConstants.WIK_HBZP_ZHAO) {
			bAroseAction = table.estimate_player_out_card_respond_PHZ(_seat_index, _send_card_data, true);
			table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants.INVALID_SEAT, false);
		}

		if ((bAroseAction == false) && (is_kai_zhao == GameConstants.WIK_NULL)) {
		...
		if (curPlayerStatus.has_action()) {// 有动作
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				GameSchedule.put(new OutCardRunnable(table.getRoom_id(), _seat_index, _send_card_data), GameConstants.DELAY_AUTO_OUT_CARD,
						TimeUnit.MILLISECONDS);
			} else if (is_kai_zhao == GameConstants.WIK_NULL) {
	...
	public boolean handler_operate_card(HBPHZTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		case GameConstants.WIK_NULL: {
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					int action = table.estimate_player_respond_hh(i, _seat_index, _send_card_data, pao_type, true);
				...
				int pai_count = table._logic.get_card_count_by_index(table.GRR._cards_index[_seat_index]);
		...
		case GameConstants.WIK_HBZP_ZHAO: // 杠牌操作
		{

			int pao_type[] = new int[1];
			int action = table.estimate_player_respond_hh(target_player, _seat_index, _send_card_data, pao_type, true);
			if (action != GameConstants.WIK_NULL) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			table.process_chi_hu_player_score_dy(target_player, _seat_index, operate_card, true);

			table.countChiHuTimes(target_player, _seat_index, true);
