...
public class HandlerGang_PuCheng extends MJHandlerGang<Table_PuCheng> {
	...
	@Override
	public boolean exe_gang(Table_PuCheng table) {
		int cbCardIndex = table._logic.switch_to_card_index(_center_card);
		int cbWeaveIndex = -1;

		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			table.is_chase_vaild = false;//最追结束
			// 暗杠
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
				table.player_game_score[_seat_index][i] += 2;
				table.player_game_score[i][_seat_index] -= 2;
			}

			table._player_result.an_gang_count[_seat_index]++;
		} else if (GameConstants.GANG_TYPE_JIE_GANG == _type) {
			table.player_game_score[_seat_index][_provide_player] += 1;
			table.player_game_score[_provide_player][_seat_index] -= 1;
			table._player_result.ming_gang_count[_seat_index]++;
		} else if (GameConstants.GANG_TYPE_ADD_GANG == _type) {// 放杠的人给分
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
				table.player_game_score[_seat_index][i] += 1;
				table.player_game_score[i][_seat_index] -= 1;
			}
			table._player_result.ming_gang_count[_seat_index]++;
		}
	...
	private void handler_operate_tong_pao(Table_PuCheng table, int seat_index,
        ...
        if (jie_pao_count > 0) {
            ...
            table.GRR._chi_hu_rights[_seat_index].opr_or(Constants_MJ_PC.CHR_BEI_QIANG_GANG);
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if ((table.GRR._chi_hu_rights[i].is_valid() == false)) {
                    continue;
                }
