...
import protobuf.clazz.lpsddz.lpsDdzRsp.AddTimesDDZ_lps;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class DDZ_LPS3_Table extends DDZTable {
	...
	protected static final int ID_TIMER_FENLUO_TO_CALL_BANKER = 3;// 分摞到叫庄
	protected static final int ID_TIMER_SWAP_TO_CALL_BANKER = 4;// 换三张到叫庄
	public DDZHandlerFenLuo_LPS3 _handlerFenLuo;
	public DDZHandlerSawpCard_LPS3 _handlerSwapCard;
	...
	public int m_turn_banker = GameConstants.INVALID_SEAT;
	public int m_player_boom[] = new int [getTablePlayerNumber()];
	
	
	public DDZ_LPS3_Table() {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		// 初始化基础牌局handle
		_handler_out_card_operate = new DDZHandlerOutCardOperate_LPS3();
		//_handler_call_banker = new DDZHandlerCallBanker();
		_handler_call_banker = new DDZHandlerCallBanker_LPS3();
		_handler_add_times = new DDZHandlerAddtimes_LPS3();
		_handler_call_banker = new DDZHandlerCallBanker_LPS3();
		_handlerFenLuo = new DDZHandlerFenLuo_LPS3();
		_handlerSwapCard = new DDZHandlerSawpCard_LPS3();
	...
	public void InitParam(){
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			for(int j = 0;j < 3;j++){
				m_swap_card[i][j] = GameConstants.INVALID_CARD;
			}
			for(int j = 0; j < GameConstants.CARD_KIND_DDZ;j++){
				m_remaindercard[i][j] = 0;
			}
		}
	...
	protected boolean on_handler_game_start() {
		...
		if (has_rule(GameConstants.GAME_RULE_PILE_SEND_LPS3)) {
			this._handler = _handlerFenLuo;
			this._handlerFenLuo.reset_status(m_turn_banker, _game_status);
			this._handlerFenLuo.exe(this);
			return true;
		}else{
	...
	public boolean game_start_ddzlps3() {
		...
		if(has_rule(GameConstants.GAME_RULE_SWAP_THREE_LPS3)){
			this._handler = _handlerSwapCard;
			this._handlerSwapCard.reset_status(GameConstants.INVALID_SEAT, _game_status);
			this._handlerSwapCard.exe(this);
			return true; 
		}
		
		return start_call_branker();

	}
	
	public boolean start_call_branker(){
	...
	public String get_boom_difen_des(int seat_index) {
		String des = "";
		if (has_rule(GameConstants.GAME_RUL_LIMIT_24_LPS3)) {
			des += "24倍封顶;";
		} else if (has_rule(GameConstants.GAME_RUL_LIMIT_48_LPS3)) {
			des += "48倍封顶;";
		} else if (has_rule(GameConstants.GAME_RUL_LIMIT_96_LPS3)) {
			des += "96倍封顶;";
		}else if (has_rule(GameConstants.GAME_RUL_LIMIT_192_LPS3)) {
			des += "192倍封顶;";
		}else if (has_rule(GameConstants.GAME_RUL_LIMIT_NOT_LPS3)) {
				des += "不封顶;";
		}
		if(has_rule(GameConstants.GAME_RULE_MUST_CALL_LPS3)){
			des += "双王四个二必叫;";
		}
		if(has_rule(GameConstants.GAME_RULE_SWAP_THREE_LPS3)){
			des += "换三张;";
		}
		if(has_rule(GameConstants.GAME_RULE_NOT_FOUR_TO_TWO_LPS3)){
			des += "不能四带二;";
		}
		if(has_rule(GameConstants.GAME_RULE_FREE_TIMES_LPS3)){
			des += "自由加倍;";
		}
		if(has_rule(GameConstants.GAME_RULE_FARMERS_FIRST_LPS3)){
			des += "农民优先;";
		}
		if(has_rule(GameConstants.GAME_RULE_NOT_TIMES_LPS3)){
			des += "不能加倍;";
		}
		if(has_rule(GameConstants.GAME_RULE_PILE_SEND_LPS3)){
			des += "分摞发牌;";
		}
		if(has_rule(GameConstants.GAME_RULE_SHUTUP_LPS3)){
			des += "禁言;";
		}
		if(has_rule(GameConstants.GAME_RULE_MEMORY_ROBOT_LPS3)){
			des += "记牌器;";
		}
	...
	public boolean animation_timer(int timer_id) {
		...
		case ID_TIMER_FENLUO_TO_CALL_BANKER:{
			finish_fen_luo();
			return true;
		}
		case ID_TIMER_SWAP_TO_CALL_BANKER:{
			finish_swap();
			return true;
		}
		}
	...
	public boolean exe_swap_card(){
		if (this._handlerSwapCard != null) {
			this._handler = this._handlerSwapCard;
			this._handlerSwapCard.exe(this);
		}
		return true;
	}
	
	public void finish_fen_luo(){
		for(int i = 0;i < getTablePlayerNumber(); i++){
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			...
			fenluo_result.setControlPlayer(-1);
			fenluo_result.setChooseIsfinish(true);
			fenluo_result.setUserCardCount(0);
	...
	public void finish_swap(){
		int seq = (int) (RandomUtil.getRandomNumber(Integer.MAX_VALUE) % 2);
		int itemp [] = new int[3];
		//顺时针
		if(seq == 0){
			itemp[0] = m_swap_card[0][0];
			itemp[1] = m_swap_card[0][1];
			itemp[2] = m_swap_card[0][2];
			for(int i = 0; i < 3;i++){
				m_swap_card[0][i] = m_swap_card[1][i];
				m_swap_card[1][i] = m_swap_card[2][i];
				m_swap_card[2][i] = itemp[i];
			}
		}else if(seq == 1){//逆时针
			itemp[0] = m_swap_card[2][0];
			itemp[1] = m_swap_card[2][1];
			itemp[2] = m_swap_card[2][2];
			for(int i = 0; i < 3;i++){
				m_swap_card[2][i] = m_swap_card[1][i];
				m_swap_card[1][i] = m_swap_card[0][i];
				m_swap_card[0][i] = itemp[i];
			}
		...
		for(int i = 0;i < getTablePlayerNumber();i++){
			for(int j = 0;j < 3;j++){
				GRR._cards_data[i][j + GRR._card_count[i]] = m_swap_card[i][j];
		...
		for(int i = 0; i < getTablePlayerNumber();i++){
			...
			swap.setCurPlayer(i);
			swap.setSwapSeq(seq);
			swap.setSwapIsfinish(true);
			swap.setUserCardCount(GRR._card_count[i]);
			...
			for(int j = 0;j < 3;j++){
				swap_card111.addItem(m_swap_card[i][j]);
			}
		...
		for(int i = 0;i < getTablePlayerNumber();i++){
			...
			for(int j = 0;j < 3;j++){
				swap_card11.addItem(m_swap_card[i][j]);
			}
			swap.addSwapCards(swap_card11);
		}
		swap.setUserCardsData(cards_card);
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			Dispath.addCardsData(cards_card1);
		}
		roomResponse1.setCommResponse(PBUtil.toByteString(Dispath));
	...
	//发底牌
	public void send_last_card(boolean fan_di){
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
	...
	public void send_add_times(int cur_player, int to_player) {
		...
		AddTimesDDZ_lps.Builder add_time_ddz = AddTimesDDZ_lps.newBuilder();
	...
	private void test_cards() {
		int cards[] = new int[] { 0x03,0x13,0x23,0x33,0x04,0x14,0x24,0x34,0x05,0x15,0x25,
				0x35,0x06,0x16,0x26,0x36,0x12,0x07,0x17,0x27,0x37,0x08,0x18,0x28,0x38,0x09,
				0x19,0x29,0x39,0x0a,0x1a,0x2a,0x3a,0x22,0x0b,0x1b,0x2b,0x3b,0x0c,0x1c,0x2c,
				0x3c,0x0d,0x1d,0x2d,0x3d,0x01,0x11,0x21,0x31,0x32,0x02,0x4F,0x4E };
		...
		//int di_cards[] = new int[] { 0x02, 0x4e, 0x4f };
		//for (int i = 0; i < _di_pai_card_count; i++) {
	...
	public void cal_score_ddz_jd(int end_score[], int chuntian[]) {
		...
		for(int i = 0;i < this.getTablePlayerNumber(); i++){
			...
			if(_add_times[this.GRR._banker_player] == 1){
				temp_times *= 2; 
				if(timelimit != -1){
					temp_times = temp_times >= timelimit ? timelimit : temp_times;
				}
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		...
		}else if(type == MsgConstants.REQUST_FEN_LUO_LPS){
			Fen_Luo_Request_lps req = PBUtil.toObject(room_rq, Fen_Luo_Request_lps.class);
			_handlerFenLuo.handler_fen_luo(this, seat_index, req.getLuoIndex());
			return true;
			
		}else if(type == MsgConstants.REQUST_SWAP_CARD_LPS){
			Swap_Card_Request_lps req = PBUtil.toObject(room_rq, Swap_Card_Request_lps.class);
			_handlerSwapCard.handler_swap_card(this, seat_index, req.getSwapCardList(), req.getSwapCardCount());
			return true;
		}
	...
	public void exe_dispath() {
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			ReDispath_lps.Builder re_dispath = ReDispath_lps.newBuilder();
	...
	public boolean operate_player_cards_flag(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		ReDispath_lps.Builder Dispath = ReDispath_lps.newBuilder();
