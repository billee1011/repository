...
public class HandlerOutCardOperate_GuangAn extends MJHandlerOutCardOperate<AbstractSiChuanMjTable> {
	@Override
	...
	public boolean handler_operate_card(AbstractSiChuanMjTable table, int seat_index, int operate_code, int operate_card) {
		...
		if (target_action == GameConstants.WIK_CHI_HU) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (table._playerStatus[i].is_respone() && (table._playerStatus[i]._perfrom_action == GameConstants.WIK_NULL
						|| table._playerStatus[i]._perfrom_action == GameConstants.WIK_CHI_HU)) {
					table._playerStatus[i].clean_action();
					table._playerStatus[i].clean_status();
					table.operate_player_action(i, true);
				} else {
					table._playerStatus[i].delete_action(GameConstants.WIK_CHI_HU);
				}
			}
		} else {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		case GameConstants.WIK_CHI_HU: {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				playerStatus = table._playerStatus[i];
				if (playerStatus.has_action() && !table.had_hu_pai[i]) {
					hasAction = true;
					break;
				}
			}
				...
				if (table.left_player_count == 1) {
					...
					if (hasAction) {
						for (int i = 0; i < table.getTablePlayerNumber(); i++) {
							...
							if (playerStatus.has_action()) {
								if (playerStatus._response == false) {
									table.change_player_status(i, GameConstants.Player_Status_OPR_CARD);
									table.operate_player_action(i, false);
								} else {
									playerStatus._response = false;
									handler_operate_card(table, i, playerStatus._perfrom_action, _out_card_data);
								}
							}
						}
					} else {
