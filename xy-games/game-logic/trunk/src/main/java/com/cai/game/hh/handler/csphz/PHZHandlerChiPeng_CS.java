...
public class PHZHandlerChiPeng_CS extends HHHandlerChiPeng<HHTable_CS> {

	@SuppressWarnings("unused")
	private GangCardResult m_gangCardResult;

	public PHZHandlerChiPeng_CS() {
	 ...
	 */
	@Override
	public boolean handler_operate_card(HHTable_CS table, int seat_index, int operate_code, int operate_card, int lou_pai) {
		PlayerStatus playerStatus = table._playerStatus[seat_index];

		// 效验操作
		if ((operate_code != GameConstants.WIK_NULL) && (playerStatus.has_action_by_code(operate_code) == false)) {
			table.log_info("HHHandlerChiPeng_syzp 没有这个操作:" + operate_code);
			return false;
		}
		...
		if (seat_index != _seat_index) {
			table.log_info("HHHandlerChiPeng_syzp 不是当前玩家操作" );
			return false;
		}
