...
public class PHZHandlerOutCardOperate_XiangXiang extends HHHandlerOutCardOperate<XiangXiangHHTable> {
	@Override
	public void exe(XiangXiangHHTable table) {
		...
		// 重明跑
		if (table.GRR._cannot_out_index[_out_card_player][table._logic.switch_to_card_index(_out_card_data)] != 0) {
			table._xian_ming_zhao[_out_card_player] = true;
		}
		//
		// 出牌记录
		...
		table.is_first_mo[_out_card_player] = false;
		// 用户切换
		...
		// 显示出牌
		table.operate_out_card(_out_card_player, 1, new int[] { _out_card_data }, GameConstants_XiangXiang.OUT_CARD_TYPE_MID,
		...
		int ti_pao = GameConstants_XiangXiang.WIK_NULL;
		for (int i = 0; i < GameConstants_XiangXiang.GAME_PLAYER_HH; i++) {
			if (i == _out_card_player)
				continue;
			int pao_type[] = new int[1];
			ti_pao = table.estimate_player_respond_phz(i, _out_card_player, _out_card_data, pao_type, false);
			if (ti_pao != GameConstants_XiangXiang.WIK_NULL) {
		...
		boolean bAroseAction = table.estimate_player_out_card_respond_PHZ_syzp(_out_card_player, _out_card_data, false);// ,
		...
		if (bAroseAction == false) {
			...
			//
			table.operate_out_card(_out_card_player, 0, null, GameConstants_XiangXiang.OUT_CARD_TYPE_MID, GameConstants_XiangXiang.INVALID_SEAT);
	...
	private boolean hasChiHu(XiangXiangHHTable table) {
		int i = 0;
		do {
			int operateIndex = (_out_card_player + i++) % table.getPlayerCount();
			if (operateIndex == _out_card_player) {
				continue;
			}

			ChiHuRight chr = table.GRR._chi_hu_rights[operateIndex];
			chr.set_empty();
			int hu_xi_chi[] = new int[] { 0 };
			int hu_xi = table.analyse_chi_hu_card(table.GRR._cards_index[operateIndex], table.GRR._weave_items[operateIndex],
					table.GRR._weave_count[operateIndex], operateIndex, _out_card_player, _out_card_data, chr, _type, hu_xi_chi, false);// 自

			if (hu_xi != GameConstants_XiangXiang.WIK_NULL) {
				table.operate_out_card(_out_card_player, 0, null, GameConstants_XiangXiang.OUT_CARD_TYPE_MID, GameConstants_XiangXiang.INVALID_SEAT);
				table._cur_banker = operateIndex;

				table._shang_zhuang_player = operateIndex;
				table.GRR._chi_hu_rights[_out_card_player].opr_or(GameConstants_XiangXiang.CHR_FANG_PAO);
				if (table.is_first_mo[operateIndex]) {
					table.GRR._chi_hu_rights[operateIndex].opr_or(GameConstants_XiangXiang.CHR_DI_HU_2);
				}
				if (GameConstants.DispatchCard_Type_Tian_Hu == _type) {
					table.GRR._chi_hu_rights[operateIndex].opr_or(GameConstants_XiangXiang.CHR_DI_HU);
				}
				table.GRR._chi_hu_rights[operateIndex].set_valid(true);
				table.process_chi_hu_player_operate(operateIndex, _out_card_data, false);
				table.process_chi_hu_player_score_phz(operateIndex, _out_card_player, _out_card_data, false);

				// 记录
				table._player_result.dian_pao_count[_out_card_player]++;
				table.countChiHuTimes(operateIndex, false);

				int delay = GameConstants_XiangXiang.GAME_FINISH_DELAY_FLS;
				if (table.GRR._chi_hu_rights[operateIndex].type_count > 2) {
					delay += table.GRR._chi_hu_rights[operateIndex].type_count - 2;
				}
				GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._cur_banker, GameConstants_XiangXiang.Game_End_NORMAL), delay,
						TimeUnit.SECONDS);
				return true;
			}
		} while (i < table.getPlayerCount());
	...
	public boolean handler_operate_card(XiangXiangHHTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		if ((operate_code != GameConstants_XiangXiang.WIK_NULL) && playerStatus.has_action_by_code(operate_code) == false)// 没有这个操作动作
		{
		...
		// if (operate_code == GameConstants_XiangXiang.WIK_CHI_HU) {
		// table.GRR._chi_hu_rights[seat_index].set_valid(true);// 胡牌生效
		// // 效果
		// //
		// table.process_chi_hu_player_operate_cs(seat_index,operate_card,1,false);
		//
		// } else if (operate_code == GameConstants_XiangXiang.WIK_NULL) {
		...
		for (int j = 0; j < table._playerStatus[last_player]._action_count; j++) {
			...
			case GameConstants_XiangXiang.WIK_DDX:
			case GameConstants_XiangXiang.WIK_EQS:
				if (target_action == GameConstants_XiangXiang.WIK_NULL)
					continue;
				if (flag == false)
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (table._playerStatus[i].has_action()) {
				for (int j = 0; j < table._playerStatus[i]._action_count; j++) {
					...
					case GameConstants_XiangXiang.WIK_DDX:
					case GameConstants_XiangXiang.WIK_EQS:
						if (!((target_action == GameConstants_XiangXiang.WIK_PENG) || (target_action == GameConstants_XiangXiang.WIK_ZI_MO)))
							continue;
						if (flag_temp == false)
					...
					case GameConstants_XiangXiang.WIK_PENG:
						if (!((target_action == GameConstants_XiangXiang.WIK_NULL)
								|| (target_action & eat_type) != GameConstants_XiangXiang.WIK_NULL))
							continue;
		...
		case GameConstants_XiangXiang.WIK_LEFT: // 上牌操作
		{
			...
			table.exe_chi_peng(target_player, _out_card_player, target_action, target_card, GameConstants_XiangXiang.CHI_PENG_TYPE_OUT_CARD,
					target_lou_code);
		...
		case GameConstants_XiangXiang.WIK_NULL: {
			...
			// 发牌
			table._last_player = _current_player;
			table.exe_dispatch_card(_current_player, GameConstants_XiangXiang.WIK_NULL, 500);

			return true;
		}
		case GameConstants_XiangXiang.WIK_CHI_HU: // 胡
		{
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == target_player) {
			...
			if (table.has_rule(GameConstants_XiangXiang.GAME_RULE_LIXIANG_FLS_ZHUANG)) {// 轮装
			...
			if (GameConstants.DispatchCard_Type_Tian_Hu == _type) {
				table.GRR._chi_hu_rights[target_player].opr_or(GameConstants_XiangXiang.CHR_DI_HU);
			}
	...
	public boolean handler_player_be_in_room(XiangXiangHHTable table, int seat_index) {
		...
		if (table._logic.is_magic_card(_out_card_data)) {
			real_card += GameConstants_XiangXiang.CARD_ESPECIAL_TYPE_HUN;
		}
		// 出牌
		table.operate_out_card(_out_card_player, 1, new int[] { real_card }, GameConstants_XiangXiang.OUT_CARD_TYPE_MID, seat_index);
