package com.cai.game.mj.ningxia;
...
public class MJTable_NingXia_HuaShui extends AbstractMJTable {
	...
	public boolean[] is_gang_valid = new boolean[getTablePlayerNumber()];

	protected MJHandlerPiao_NingXia_HuaShui _handler_piao;
	private int _magic_card_count;// 癞子牌数量

	public MJTable_NingXia_HuaShui(MJType game_type) {
		super(game_type);
	}
	...
	public int getTablePlayerNumber() {
		...
		if (has_rule(Constants_MJ_NXHS.GAME_RULE_PLAY_4)) {
			return 4; // 4人场
		} else {
			return 4; // 默认4人场
		}
	...
	protected void init_shuffle() {
		...
		if (has_rule(Constants_MJ_NXHS.GAME_RULE_DAI_FENG_PAI)) {
			int[] cards = Constants_MJ_NXHS.CARD_DATA_DAI_FENG;
			for (int i = 0; i < cards.length; i++) {
				cards_list.add(cards[i]);
			}

		} else if (has_rule(Constants_MJ_NXHS.GAME_RULE_HONG_ZHONG_LAI_ZI)) {
			int[] cards = Constants_MJ_NXHS.CARD_DATA_HONGZHONG;
			for (int i = 0; i < cards.length; i++) {
				cards_list.add(cards[i]);
			}
		} else {
			int[] cards = Constants_MJ_NXHS.CARD_DATA_NO_FENG;
			for (int i = 0; i < cards.length; i++) {
				cards_list.add(cards[i]);
			}
		}
	...
	public int analyse_chi_hu_card(int[] cards_index, WeaveItem[] weaveItems, int weave_count, int cur_card,
		...
		if (has_rule(Constants_MJ_NXHS.AME_RULE_HUNAN_258)) {
			need_258 = true;
		}
		...
		// 判断是否是豪华七对
		if (check_qi_xiao_dui != GameConstants.WIK_NULL && check_qi_xiao_dui == Constants_MJ_NXHS.CHR_HHQXD) {
			need_258 = false;
			can_win = true;
		...
		// 判断是不是清一色
		boolean is_qing_yi_se = this.is_qing_yi_se(cards_index, weaveItems, weave_count, cur_card, card_type);
		...
		if (card_type == Constants_MJ_NXHS.GAME_RULE_ZIMOHU) {
			chiHuRight.opr_or_xt(Constants_MJ_NXHS.CHR_TYPE_ZI_MO, need_to_multiply); // 自摸
		} else if (card_type == Constants_MJ_NXHS.GAME_RULE_DIAN_PAO_HU) {
			chiHuRight.opr_or_xt(Constants_MJ_NXHS.CHR_TYPE_DIAN_PAO, need_to_multiply); // 点炮
		} else if (card_type == Constants_MJ_NXHS.HU_CARD_TYPE_QIANG_GANG_HU) {
			chiHuRight.opr_or_xt(Constants_MJ_NXHS.CHR_TYPE_QIANG_GANG, need_to_multiply); // 抢杠胡
		} else if (card_type == Constants_MJ_NXHS.HU_GANG_SHANG_KAI_HUA) {
			chiHuRight.opr_or_xt(Constants_MJ_NXHS.CHR_TYPE_GANG_SHANG_KAI_HUA, need_to_multiply); // 杠上开花
		}

		if (has_qi_xiao_dui && !has_hao_hua_qi_xiao_dui) { // 七小对
			chiHuRight.opr_or_xt(Constants_MJ_NXHS.CHR_QI_XIAO_DUI, need_to_multiply);
		}
		if (has_qing_yi_se) { // 清一色
			chiHuRight.opr_or_xt(Constants_MJ_NXHS.CHR_QING_YI_SE, need_to_multiply);
		}

		if (has_hao_hua_qi_xiao_dui) { // 豪华七对
			chiHuRight.opr_or_xt(Constants_MJ_NXHS.CHR_HHQXD, need_to_multiply);
		}

		if (has_rule(Constants_MJ_NXHS.GAME_RULE_HUANG_ZHUANG_JIA_FAN) && is_shang_ju_huang_zhuang) {// 荒庄加番
			chiHuRight.opr_or_xt(Constants_MJ_NXHS.CHR_HUANG_ZHUANG_JIA_FAN, need_to_multiply);
		}
		if (has_rule(Constants_MJ_NXHS.GAME_RULE_HUANG_ZHUANG_JIA_BEI) && is_shang_ju_huang_zhuang) {// 荒庄加倍
			chiHuRight.opr_or_xt(Constants_MJ_NXHS.CHR_HUANG_ZHUANG_JIA_BEI, need_to_multiply);
		}
	...
	public boolean estimate_gang_respond(int seat_index, int card) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (playerStatus.is_chi_hu_round()/*
				...
				if (!has_rule(Constants_MJ_NXHS.GAME_RULE_ZIMOHU)) {
					action = analyse_chi_hu_card(GRR._cards_index[i], GRR._weave_items[i], cbWeaveCount, card, chr,
							Constants_MJ_NXHS.HU_CARD_TYPE_QIANG_GANG_HU, i, false);
				}
				...
				if (action != 0) {
					...
					chr.opr_or(Constants_MJ_NXHS.CHR_TYPE_QIANG_GANG);// 抢杠胡
					bAroseAction = true;
				}
			}
		}
	...
	public int get_ting_card(int[] cards, int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index) {
		...
		for (int i = 0; i < max_ting_count; i++) {
			...
			if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card(cbCardIndexTemp, weaveItem, cbWeaveCount, cbCurrentCard,
					chr, Constants_MJ_NXHS.GAME_RULE_ZIMOHU, seat_index, false)) {
				if (_logic.switch_to_card_data(this._logic.get_magic_card_index(0)) == cbCurrentCard
						&& has_rule(Constants_MJ_NXHS.GAME_RULE_HONG_ZHONG_LAI_ZI)) {
		...
		if (count == 0) {
			if (has_rule(Constants_MJ_NXHS.GAME_RULE_HONG_ZHONG_LAI_ZI)
				...
				if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card(cbCardIndexTemp, weaveItem, cbWeaveCount,
						GameConstants.HZ_MAGIC_CARD, chr, Constants_MJ_NXHS.GAME_RULE_ZIMOHU, seat_index, false)) {
			...
			/*
			 * if (has_rule(Constants_MJ_NXHS.GAME_RULE_HONG_ZHONG_LAI_ZI)&&!
			 * has_rule(Constants_MJ_NXHS.AME_RULE_HUNAN_258)) { // 有胡的牌，红中肯定能胡
			 * cards[count] =
			 * _logic.switch_to_card_data(this._logic.get_magic_card_index(0));
			 * count++; }
			 */
	...
	protected void onInitTable() {
		hu_before_bank = 0;
		_handler_chi_peng = new MJHandlerChiPeng_NingXia_HuaShui();
		_handler_dispath_card = new MJHandlerDispatchCard_NingXia_HuaShui();
		_handler_gang = new MJHandlerGang_NingXia_HuaShui();
		_handler_out_card_operate = new MJHandlerOutCardOperate_NingXia_HuaShui();

		_handler_piao = new MJHandlerPiao_NingXia_HuaShui();

		// 每次进来勾选的底分
		if (has_rule(Constants_MJ_NXHS.GAME_RULE_DI_FEN_1)) {
			di_fen = 1;
		} else if (has_rule(Constants_MJ_NXHS.GAME_RULE_DI_FEN_2)) {
			di_fen = 2;
		} else if (has_rule(Constants_MJ_NXHS.GAME_RULE_DI_FEN_3)) {
			di_fen = 3;
		} else if (has_rule(Constants_MJ_NXHS.GAME_RULE_DI_FEN_4)) {
			di_fen = 4;
		} else if (has_rule(Constants_MJ_NXHS.GAME_RULE_DI_FEN_5)) {
			di_fen = 5;
		} else {
			di_fen = 0;
		}
		...
		if (has_rule(Constants_MJ_NXHS.GAME_RULE_DAI_GEN_ZHUANG)) {
			is_gen_zhuang = true;
		}

		if (has_rule(Constants_MJ_NXHS.GAME_RULE_HONG_ZHONG_LAI_ZI)) { // 红中癞子
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				if (is_gen_zhuang && this.getGenZhuangCount() >= 1) {
					if (i == this.hu_before_bank) {
						gen_zhuang_fen[i] = -(this.getTablePlayerNumber() - 1);
					} else {
						gen_zhuang_fen[i] = 1;
					}
				}
	...
	protected boolean on_game_start() {
		...
		is_gang_valid = new boolean[getTablePlayerNumber()];// 默认每个玩家都是可以杠的默认为false
															// 当为true 的时候表示是不能杠的
	...
	public void process_chi_hu_player_score(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		int wFanShu = 1;
		if (!chr.opr_and(Constants_MJ_NXHS.CHR_QING_YI_SE).is_empty()) {
			wFanShu *= 3;
		}
		if (!chr.opr_and(Constants_MJ_NXHS.CHR_TYPE_GANG_SHANG_KAI_HUA).is_empty()) {
			wFanShu *= 3;
		}
		if (!chr.opr_and(Constants_MJ_NXHS.CHR_TYPE_QIANG_GANG).is_empty()) {
			wFanShu *= 3;
		}
		if (!chr.opr_and(Constants_MJ_NXHS.CHR_QI_XIAO_DUI).is_empty()) {
			wFanShu *= 3;
		}
		if (!chr.opr_and(Constants_MJ_NXHS.CHR_HHQXD).is_empty()) {
			wFanShu *= 6;
		}
		...
		if (zimo) {
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				float s = 0;
				// 这个 输家分数=（胡牌分数+下鱼子分数+杠牌分数+跟庄分数）*底分 杠分和跟庄的分数 单独抽出来 在游戏结束的时候算

				int pao_fen = 0;
				// 如果选择了单鱼 胡牌玩家的鱼分
				if (has_rule(Constants_MJ_NXHS.GAME_RULE_DAN_YU)) {
					pao_fen = gu_ding_yu; // 算鱼分的时候
				}
				// 如果选择了鱼吃鱼
				if (has_rule(Constants_MJ_NXHS.GAME_RULE_YU_CHI_YU)) {
					pao_fen = gu_ding_yu + gu_ding_yu; // 算鱼分的时候
				}

				// 荒庄加番和荒庄加倍为互斥 不能够同时存在
				if (has_rule(Constants_MJ_NXHS.GAME_RULE_HUANG_ZHUANG_JIA_BEI) && is_shang_ju_huang_zhuang) {// 荒庄加倍
					s = (lChiHuScore * 2 + pao_fen) * di_fen;
				} else if (has_rule(Constants_MJ_NXHS.GAME_RULE_HUANG_ZHUANG_JIA_FAN) && is_shang_ju_huang_zhuang) {// 荒庄加番
					s = (lChiHuScore + 2 + pao_fen) * di_fen;
				} else {
		...
		////////////////////////////////////////////////////// 点炮 算分
			...
			GRR._chi_hu_rights[real_player].opr_or(Constants_MJ_NXHS.CHR_FANG_PAO);
			GRR._chi_hu_rights[seat_index].opr_or(Constants_MJ_NXHS.CHR_JIE_PAO);
			...
			// 荒庄加番和荒庄加倍为互斥 不能够同时存在
			// 勾选荒庄加倍
			if (has_rule(Constants_MJ_NXHS.GAME_RULE_HUANG_ZHUANG_JIA_BEI) && is_shang_ju_huang_zhuang) {
				lChiHuScore = (lChiHuScore * 2 + pao_fen) * di_fen;
			} else if (has_rule(Constants_MJ_NXHS.GAME_RULE_HUANG_ZHUANG_JIA_FAN) && is_shang_ju_huang_zhuang) {
				lChiHuScore = (lChiHuScore + 3 + pao_fen) * di_fen;
			} else {
	...
	public void test_cards() {
		int[] cards_of_player1 = new int[] { 0x11, 0x15, 0x11, 0x15, 0x16, 0x16, 0x17, 0x17, 0x18, 0x18, 0x19, 0x19,
				0x35 };
		...
		// 0x23 };
		int[] cards_of_player0 = new int[] { 0x11, 0x15, 0x11, 0x15, 0x16, 0x16, 0x17, 0x17, 0x18, 0x18, 0x19, 0x19,
				0x35 };
		...
		for (int player = 0; player < this.getTablePlayerNumber(); player++) {
			...
			for (int typeIndex = 0; typeIndex < chrTypes; typeIndex++) {
				...
				if (GRR._chi_hu_rights[player].is_valid()) {
					...
					if (type == Constants_MJ_NXHS.CHR_TYPE_ZI_MO) {
						gameDesc.append(" 自摸");

					}

					if (type == Constants_MJ_NXHS.CHR_TYPE_QIANG_GANG) {
						gameDesc.append(" 抢杠胡");

					}

					if (type == Constants_MJ_NXHS.CHR_JIE_PAO) {
						gameDesc.append(" 接炮");
					}

					if (type == Constants_MJ_NXHS.CHR_QI_XIAO_DUI) {
						gameDesc.append(" 七小对");
					}
					if (type == Constants_MJ_NXHS.CHR_HHQXD) {
						gameDesc.append(" 豪华七小对");
					}
					if (type == Constants_MJ_NXHS.CHR_TYPE_GANG_SHANG_KAI_HUA) {
						gameDesc.append(" 杠上开花");
					}
					if (type == Constants_MJ_NXHS.CHR_QING_YI_SE) {
						gameDesc.append(" 清一色");
					}
					if (type == Constants_MJ_NXHS.CHR_HUANG_ZHUANG_JIA_FAN) {
						gameDesc.append(" 荒庄加番");
					}
					if (type == Constants_MJ_NXHS.CHR_HUANG_ZHUANG_JIA_BEI) {
						gameDesc.append(" 荒庄加倍");
					}

				} else if (type == Constants_MJ_NXHS.CHR_FANG_PAO) {
					gameDesc.append(" 放炮");
				}
			}
			...
			if (is_gen_zhuang && this.getGenZhuangCount() >= 1) {
				if (player == this.hu_before_bank) {
					gameDesc.append(" 跟庄" + "-" + (getTablePlayerNumber() - 1));
				} else {
	...
	public int is_qi_xiao_dui(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int cur_card, int type) {
		...
		if (_magic_card_count > 0) {
			...
			// 有四个红中并且 单牌数为0 豪华七小对
			if (count == 4 && cbReplaceCount <= 2) {
				return Constants_MJ_NXHS.CHR_HHQXD;// 豪华七对
			}
			if (count >= 2 && cbReplaceCount == 0) {
				return Constants_MJ_NXHS.CHR_HHQXD;// 豪华七对
			}
			// 有暗杠 并且没有单牌的为豪华七小对
			...
			if (count >= cbReplaceCount) {
				return Constants_MJ_NXHS.CHR_QI_XIAO_DUI;// 七对
			}
			if (doubleGenCount == 7) {
				return Constants_MJ_NXHS.CHR_QI_XIAO_DUI;// 七对
			}
	...
	public boolean is_qing_yi_se(int cards_index[], WeaveItem weaveItem[], int weaveCount, int cur_card,
		...
		boolean lai_zi_dang_ben_shen = false;// 癞子当本身
		if ((card_type == 2 || card_type == 3) && this._logic.is_magic_card(cur_card)) {
			lai_zi_dang_ben_shen = true;
		}
		for (int i = 0; i < GameConstants.MAX_INDEX; i++) {
	...
	public int analyse_gang_exclude_magic_card(int cards_index[], int card, WeaveItem WeaveItem[], int cbWeaveCount,
		...
		if (!this.is_gang_valid[_seat_index]) {
			// 手上杠牌
			for (int i = 0; i < GameConstants.MAX_INDEX; i++) {
				/*
	 ...
	 * 固定选鱼子数
	 */
	public void set_yu_zi() {
		if (has_rule(Constants_MJ_NXHS.GAME_RULE_YU_ZI_O)) {
			gu_ding_yu = 0;
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				this._player_result.pao[i] = 0;
			}
		} else if (has_rule(Constants_MJ_NXHS.GAME_RULE_YU_ZI_2)) {
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				this._player_result.pao[i] = 2;
			}
			gu_ding_yu = 2;
		} else if (has_rule(Constants_MJ_NXHS.GAME_RULE_YU_ZI_5)) {
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				this._player_result.pao[i] = 5;
			}
			gu_ding_yu = 5;
		} else if (has_rule(Constants_MJ_NXHS.GAME_RULE_YU_ZI_8)) {
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				this._player_result.pao[i] = 8;
			}
			gu_ding_yu = 8;
		} else {
			for (int i = 0; i < getTablePlayerNumber(); i++) {
