...
public class HandlerGang_ZYZJ extends MJHandlerGang<Table_ZYZJ> {
	...
	@Override
	public boolean exe_gang(Table_ZYZJ table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			table.GRR._weave_count[_seat_index]++;
			if (table.is_ji_card(_center_card)) {
				if (_center_card == GameConstants_ZYZJ.BA_TONG_CARD && table.has_rule(GameConstants_ZYZJ.GAME_RULE_WG_ZYZJ)) {
					table.ze_ren_ji[_provide_player] -= 2;
					table.ze_ren_ji[_seat_index] += 2;
				} else if(_center_card == GameConstants_ZYZJ.YJ_CARD){
					table.ze_ren_ji[_provide_player] -= 2;
					table.ze_ren_ji[_seat_index] += 2;
				}
				//for (int i = 0; i < 5; i++)
			...
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				...
				if ((cbCenterCard == _center_card) && (cbWeaveKind == GameConstants.WIK_PENG || 
						cbWeaveKind == GameConstants.WIK_SUO_PENG_1 || cbWeaveKind == GameConstants.WIK_SUO_PENG_2)) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			score = 3;
			if(table.has_rule(GameConstants_ZYZJ.GAME_RULE_SDG_ZYZJ) || table.has_rule(GameConstants_ZYZJ.GAME_RULE_EDG_ZYZJ)){
				if(_center_card == GameConstants_ZYZJ.YJ_CARD ){
					score = 7;
				}else if(_center_card == GameConstants_ZYZJ.BA_TONG_CARD){
					score = 11;
				}
			}
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	...
	public boolean handler_operate_card(Table_ZYZJ table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			table.hutype = 2;
			table.hupaiplayer[seat_index] = 1;
			table.fangpaoplayer[_seat_index] = 1;
		...
		if (jie_pao_count > 0) {
			...
			if(has_shao){
				table.process_reset_gang_score(_seat_index);
			}
			...
			if(table._playerStatus[_seat_index]._hu_card_count > 0){
				...
				table.process_ji_fen(_seat_index);
			}
