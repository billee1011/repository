 ...
 * 
 */
package com.cai.game.pdk.handler.znpdk;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PDK_ZN_Table extends PDKTable {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_cur_round = 0;
		_zha_niao_palyer = GameConstants.INVALID_SEAT;
		...
		_handler_out_card_operate = new PDKHandlerOutCardOperate_ZN();
		_bomb_cell_score = 10;
		if (this.matchId != 0) {
			_bomb_cell_score = 4;
		}
	...
	protected boolean on_handler_game_start() {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (DEBUG_CARDS_MODE || BACK_DEBUG_CARDS_MODE) {
				// TODO 去除
				// this.get_players()[i].enableRobot(this._game_type_index,
				// this);
			}
		}

		//
		// if(_zha_niao_palyer == GameConstants.INVALID_SEAT){
		// _zha_niao_palyer=(int)
		// (RandomUtil.getRandomNumber(Integer.MAX_VALUE)%getTablePlayerNumber());
		// if(_zha_niao_palyer != 3){
		// _hong_tao_palyer=3;
		...
		// }
		if (_zha_niao_palyer == GameConstants.INVALID_SEAT) {
			_zha_niao_palyer = RandomUtil.getRandomNumber(Integer.MAX_VALUE) % getTablePlayerNumber();
		}
		...
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				if (this.has_rule(GameConstants.GAME_RULE_DISPLAY_CARD)) {
					if (this._zha_niao_palyer == i) {
						gamestart_pdk.addCardCount(GRR._card_count[_hong_tao_palyer]);
					} else {
			...
			// 手牌--将自己的手牌数据发给自己
			Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
			if (this._zha_niao_palyer == play_index) {
				for (int j = 0; j < GRR._card_count[_hong_tao_palyer]; j++) {
					cards_card.addItem(GRR._cards_data[_hong_tao_palyer][j]);
				}
			} else {
				for (int j = 0; j < GRR._card_count[play_index]; j++) {
					cards_card.addItem(GRR._cards_data[play_index][j]);
			...
			gamestart_pdk.setZhaNiaoPlayer(this._zha_niao_palyer);
			gamestart_pdk.setHongTaoPlayer(this._hong_tao_palyer);
	...
	private void shuffle(int repertory_card[], int mj_cards[]) {
		...
		for (int i = 0; i < count; i++) {
			if (i == _zha_niao_palyer) {
				GRR._card_count[i] = 0;
				continue;
			}
			for (int j = 0; j < this.get_hand_card_count_max(); j++) {
				GRR._cards_data[i][j] = repertory_card[player_num * this.get_hand_card_count_max() + j];
				if (GRR._cards_data[i][j] == 0x2A) {
					_hong_tao_palyer = i;
				}
			}
	...
	private void test_cards() {
		int cards[] = new int[] { 39, 55, 38, 5, 4, 3, 60, 28, 23, 54, 25, 22, 19, 6, 44, 12, 40, 27, 24, 26, 43, 57,
				33, 10, 35, 45, 1, 42, 29, 53, 41, 51, 11, 20, 52, 8, 2, 13, 36, 17, 7, 59, 37, 56, 58, 61, 21, 9 };
		// int cards[] = new int[] {33, 52, 19, 42, 22, 24, 21, 9, 53, 26, 12,
		// 28, 20, 8, 1, 25, 7, 41, 57, 2, 23, 10, 27, 13, 3, 38, 11, 43, 6, 59,
		// 4, 29, 17, 44, 54, 39, 61, 5, 40, 56, 58, 36, 55, 51, 60, 45, 35,
		// 37};
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
		...
		int index = 0;
		for (int i = 0; i < this.getTablePlayerNumber() - 1; i++) {
			if (i == this._zha_niao_palyer) {
				continue;
			}
			for (int j = 0; j < get_hand_card_count_max(); j++) {
	...
	public void cal_score_pdk_jd(int end_score[]) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this.GRR._card_count[i] > 1) {
				...
				if (win_player == _hong_tao_palyer) {
					win_score += Math.abs(end_score[i]);
					end_score[i] *= 2;
				} else {
					if (_hong_tao_palyer == i) {
						end_score[_zha_niao_palyer] = end_score[i];
						win_score += Math.abs(end_score[_zha_niao_palyer]);
					}
					win_score += Math.abs(end_score[i]);
		...
		if (_hong_tao_palyer == win_player) {
			end_score[_zha_niao_palyer] = end_score[_hong_tao_palyer];
		}
		if (this.matchId == 0) {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean handler_game_finish_pdk_jd(int seat_index, int reason) {
		...
		if (reason == GameConstants.Game_End_NORMAL) {
			...
			game_end_pdk.setZhaNiaoPlayer(_zha_niao_palyer);
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				if (GRR != null && i != GRR._banker_player && this._out_card_times[i] == 0) {
					if (i == this._zha_niao_palyer) {
						if (_out_card_times[_hong_tao_palyer] == 0) {
		...
		if (GRR != null) {
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				if (i == this._zha_niao_palyer) {
					game_end_pdk.addCardCount(GRR._card_count[_hong_tao_palyer]);
					for (int j = 0; j < GRR._card_count[_hong_tao_palyer]; j++) {
						cards_card.addItem(GRR._cards_data[_hong_tao_palyer][j]);
					}
				} else {
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if (_cur_round >= _game_round) {// 局数到了
				...
				// if(this.get_players()[j].getAccount_id() ==
				// _init_account_id[i]){
		...
		_cur_banker = _zha_niao_palyer;
		_zha_niao_palyer = seat_index;
		if (_zha_niao_palyer != GameConstants.INVALID_SEAT) {
			handler_request_trustee(_zha_niao_palyer, false, 0);
		}
	...
	private void shuffle_players_data() {
		if (this._cur_banker != GameConstants.INVALID_SEAT && this._zha_niao_palyer != GameConstants.INVALID_SEAT) {
			// 换用户信息
			Player temp_player = new Player();
			temp_player = get_players()[_cur_banker];
			get_players()[_cur_banker] = get_players()[_zha_niao_palyer];
			get_players()[_zha_niao_palyer] = temp_player;
			get_players()[_cur_banker].set_seat_index(_cur_banker);
			get_players()[_zha_niao_palyer].set_seat_index(_zha_niao_palyer);

			float temp_score = _player_result.game_score[_cur_banker];
			_player_result.game_score[_cur_banker] = _player_result.game_score[_zha_niao_palyer];
			_player_result.game_score[_zha_niao_palyer] = temp_score;
			...
			if (this._handler != null) {
				...
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					...
					if (has_rule(GameConstants.GAME_RULE_DISPLAY_CARD)) {
						if (this._zha_niao_palyer == i) {
							tableResponse_pdk.addCardCount(GRR._card_count[_hong_tao_palyer]);
						} else {
				...
				tableResponse_pdk.setZhaNiaoPlayer(this._zha_niao_palyer);
				tableResponse_pdk.setHongTaoPlayer(this._hong_tao_palyer);
	...
	public boolean operate_player_cards() {
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				if (this.has_rule(GameConstants.GAME_RULE_DISPLAY_CARD)) {
					if (this._zha_niao_palyer == i) {
						refreshcards.addCardCount(GRR._card_count[_hong_tao_palyer]);
					} else {
