...
import com.cai.game.pdk.PDKGameLogic_XiangYang;
...
import protobuf.clazz.pdk_np.Pdk_npRsp.GameStart_PDK_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.MingPaiStart_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.Open_Less_Response_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.Opreate_RequestWsk_nppdk;
import protobuf.clazz.pdk_np.Pdk_npRsp.OutCardDataPdk_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.Piao_Score_Begin_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.PukeGameEndPdk_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.QiangGuanResponse_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.QiangGuanStart_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.QiePaiResponse_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.QiePaiStart_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.RefreshCardsPdk_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.TableResponse_PDK_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.ZhanJiResponse_NP;
import protobuf.clazz.pdk_np.Pdk_npRsp.card_arry_np;
import protobuf.clazz.pdk_np.Pdk_npRsp.effect_type_NP;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PDK_FIFTEEN_Table_NP extends PDKTable {
	...
	protected static final int ID_TIMER_MING_PAI = 4;
	protected static final int ID_TIMER_QIANG_GUAN = 5;
	protected static final int ID_TIMER_OUT_CARD = 6;

	protected static final int GAME_OPREATE_TYPE_PIOA_FEN = 1;
	protected static final int GAME_OPREATE_TYPE_PEOPLE_LESS = 2;// 少人模式
	protected static final int GAME_OPREATE_TYPE_QIE_PAI_BEGIN = 3;// 进入切牌
	protected static final int GAME_OPREATE_TYPE_QIE_PAI = 4;// 切牌
	protected static final int GAME_OPREATE_TYPE_QIE_PAI_NO = 5;// 不切
	protected static final int GAME_OPREATE_TYPE_QIANG_GUAN = 6;// 抢关
	protected static final int GAME_OPREATE_TYPE_QIANG_GUAN_NO = 7;// 不抢
	protected static final int GAME_OPREATE_TYPE_MING_PAI = 8;// 明牌
	protected static final int GAME_OPREATE_TYPE_MING_PAI_NO = 9;// 不明牌

	protected static final int EFFECT_DI_GOLD_FEI = 1;// 飞金币

	public PDK_FIFTEEN_Table_NP() {
	}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_ming_pai = new int[getTablePlayerNumber()];
		_opreate_times = new int[getTablePlayerNumber()];
		_game_end_score = new int[getTablePlayerNumber()][_game_round];
		_history_out_card = new int[getTablePlayerNumber()][this.get_hand_card_count_max()][this
		...
		_logic = new PDKGameLogic_XiangYang();
		_handler_out_card_operate = new PDKHandlerOutCardOperate_FIFTEEN_NP();
		_handler_call_banker = new PDKHandlerCallBnakerNP_FIFTEEN();
		_handler_ming_pai = new PDKHandlerMingPaiNP_FIFTEEN();
		this._logic._is_boom = true;
		this._logic.setRuleMap(ruleMap);
		_is_in_qie = false;
	...
	protected boolean on_handler_game_start() {
		...
		if (this.has_rule(GameConstants.GAME_RULE_PIAO_SCORE_ONE) || has_rule(GameConstants.GAME_RULE_PIAO_SCORE_TWO)
			...
			_game_status = GameConstants.GS_NPPDK_PIAO;
			piao_score_begin();
		} else {
			_game_status = GameConstants.GS_NPPDK_PLAY;
			if (!_is_in_qie) {
				if (!this.has_rule(GameConstants.GAME_RULE_MING_PAI_PRV)) {
					game_start_pkd();
				} else {
					this._handler = this._handler_ming_pai;
					this._handler.exe(this);
				}
			} else {
				RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
				QiePaiStart_NP.Builder qie_pai = QiePaiStart_NP.newBuilder();
	...
	public void piao_score_begin() {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		Piao_Score_Begin_NP.Builder piaofen = Piao_Score_Begin_NP.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PDK_PIAO_SCORE_BEGIN);// 201
		piaofen.setRoomInfo(this.getRoomInfo());
	...
	public void qiang_guan_begin(int to_player) {

		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		QiangGuanStart_NP.Builder qiang_guan = QiangGuanStart_NP.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PDK_QIANG_GUAN_BEGIN);// 201
		if (_game_status == GameConstants.GS_NPPDK_CALL) {
			qiang_guan.setCurrentPlayer(this.GRR._banker_player);
		} else {
			qiang_guan.setCurrentPlayer(GameConstants.INVALID_SEAT);
		}

		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			qiang_guan.addCallAction(this._call_action[i]);
		}
		roomResponse.setCommResponse(PBUtil.toByteString(qiang_guan));
	...
	public void qiang_guan_response(int seat_index, int cur_player, int action) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		QiangGuanResponse_NP.Builder qiang_guan_response = QiangGuanResponse_NP.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PDK_QIANG_GUAN_RESPONSE);// 201
		qiang_guan_response.setCallAction(action);
		qiang_guan_response.setCallPlayer(seat_index);
		qiang_guan_response.setCurrentPlayer(cur_player);
		roomResponse.setCommResponse(PBUtil.toByteString(qiang_guan_response));
		this.send_response_to_room(roomResponse);

	}

	public void ming_pai_begin(int to_player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		MingPaiStart_NP.Builder mign_pai_begin = MingPaiStart_NP.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PDK_MING_PAI_BEGIN);// 201
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			mign_pai_begin.addAction(this._ming_pai[i]);
		}
		mign_pai_begin.setRoomInfo(this.getRoomInfo());
		this.load_player_info_data_ming_pai(mign_pai_begin);
		roomResponse.setCommResponse(PBUtil.toByteString(mign_pai_begin));
		if (to_player == GameConstants.INVALID_SEAT) {
			this.send_response_to_room(roomResponse);
		} else {
			this.send_response_to_player(to_player, roomResponse);
	...
	public void zhan_ji_score(int to_player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		ZhanJiResponse_NP.Builder zhan_ji = ZhanJiResponse_NP.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PDK_ZHAN_JI_RESPONSE);// 201

		for (int i = 0; i < this._cur_round; i++) {
			Int32ArrayResponse.Builder score_list = Int32ArrayResponse.newBuilder();
			for (int j = 0; j < this.getTablePlayerNumber(); j++) {
				score_list.addItem(_game_end_score[j][i]);
			}
			zhan_ji.addScore(score_list);
		}

		roomResponse.setCommResponse(PBUtil.toByteString(zhan_ji));
		if (to_player == GameConstants.INVALID_SEAT) {
			this.send_response_to_room(roomResponse);
		} else {
			this.send_response_to_player(to_player, roomResponse);
	...
	public void send_effect_type(int seat_index, int type, int data[], int to_player) {
		// type=1：飞金币
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PDK_EFFECT_TYPE);
		effect_type_NP.Builder effect = effect_type_NP.newBuilder();
		effect.setTargetIndex(seat_index);
		effect.setType(type);
		if (data != null) {
			for (int i = 0; i < data.length; i++) {
				effect.addOrginIndex(data[i]);
			}
		}
	...
	protected void timerCallBack(SheduleArgs args) {
		...
		case ID_TIMER_MING_PAI: {
			this._handler = this._handler_ming_pai;
			this._handler.exe(this);
			return;
		}
		case ID_TIMER_QIANG_GUAN: {
		...
		case ID_TIMER_OUT_CARD: {
			...
			operate_out_card(GameConstants.INVALID_SEAT, 0, null, GameConstants.PDK_CT_ERROR,
	...
	public boolean game_start_pkd() {
		...
		if (this._cur_round == 1 || has_rule(GameConstants.GAME_RULE_HEITAO_SAN)) {
		...
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			...
			// 发送数据
			GameStart_PDK_NP.Builder gamestart_pdk = GameStart_PDK_NP.newBuilder();
			gamestart_pdk.setRoomInfo(this.getRoomInfo());
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				if (i == play_index || this._ming_pai[i] == 1) {
		...
		// 发送数据
		GameStart_PDK_NP.Builder gamestart_pdk = GameStart_PDK_NP.newBuilder();
		gamestart_pdk.setRoomInfo(getRoomInfo());
		if (this._cur_round == 1) {
			this.load_player_info_data_game_start(gamestart_pdk);
		}
		...
		if (this.has_rule(GameConstants.GAME_RULE_MING_PAI_NEXT)) {
			schedule(ID_TIMER_MING_PAI, SheduleArgs.newArgs(), 2000);
		} else {
			if (this.has_rule(GameConstants.GAME_RULE_QIANG_GUAN)) {
				schedule(ID_TIMER_QIANG_GUAN, SheduleArgs.newArgs(), 2000);
			} else {
				_game_status = GameConstants.GS_NPPDK_PLAY;
				schedule(ID_TIMER_OUT_CARD, SheduleArgs.newArgs(), 2000);
			}
		}
	...
	private void test_cards() {
		...
		int cards[] = new int[] { 0x05, 0x15, 0x25, 0x35, 0x06, 0x16, 0x26, 0x36, 0x07, 0x17, 0x37, 0x27, 0x08, 0x18,
				0x28, 0x38, 0x09, 0x05, 0x15, 0x25, 0x35, 0x06, 0x16, 0x26, 0x36, 0x07, 0x17, 0x37, 0x27, 0x08, 0x18,
				0x28, 0x38, 0x09, 0x05, 0x15, 0x25, 0x35, 0x06, 0x16, 0x26, 0x36, 0x07, 0x17, 0x37, 0x27, 0x08, 0x18,
				0x28, 0x38, 0x09, };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public void cal_score_pdk_jd(int end_score[], int seat_index) {
		...
		if (_is_call_banker) {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				int times = 1;
				if (this._ming_pai[i] == 1) {
					times *= 2;
				}
				if (this._ming_pai[this.GRR._banker_player] == 1) {
					times *= 2;
				}
				if (has_rule(GameConstants.GAME_RULE_FIFTEEN_COUNT)) {
					end_score[i] += 30 * times;
				} else {
					end_score[i] += 32 * times;
				}
				if (this._piao_fen[i] != -1 && this._piao_fen[GRR._banker_player] != -1) {
					end_score[i] += this._piao_fen[i] + this._piao_fen[GRR._banker_player];
				}
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				if (this._ming_pai[win_player] == 1) {
					times *= 2;
				}
				if (this.GRR._card_count[i] > 1) {
					if (i == this.GRR._banker_player) {
						// 反春天
						if (this.has_rule(GameConstants.GAME_RULE_PEOPLE_LESS) && this.getTablePlayerNumber() == 2
								&& this._out_card_times[i] == 1) {
							end_score[i] -= 30 * times;
						} else if (has_rule(GameConstants.GAME_RULE_TWO_PLAY) && this._out_card_times[i] == 1
								&& this.has_rule(GameConstants.GAME_RULE_KE_FAN_DE) && GRR._card_count[i] != 0) {
							end_score[i] -= 30 * times;

						} else {
							end_score[i] -= this.GRR._card_count[i] * times;
						}
					} else {
						if (this.has_rule(GameConstants.GAME_RULE_PEOPLE_LESS) && this.getTablePlayerNumber() == 2
								&& this._out_card_times[i] == 0) {
							end_score[i] -= this.GRR._card_count[i] * 2 * times;
						} else if (this._out_card_times[i] == 0 && this.has_rule(GameConstants.GAME_RULE_CHUN_TIAN)) {
							end_score[i] -= this.GRR._card_count[i] * 2 * times;
						} else {
							end_score[i] -= this.GRR._card_count[i] * times;
						}
	...
	public boolean handler_game_finish_pdk_jd(int seat_index, int reason) {
		...
		if (reason == GameConstants.Game_End_NORMAL) {
			cal_score_pdk_jd(end_score, seat_index);
		} else {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			_game_end_score[i][this._cur_round - 1] = end_score[i];
		}
		// 失分最多
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			...
			PukeGameEndPdk_NP.Builder game_end_pdk = PukeGameEndPdk_NP.newBuilder();
			game_end_pdk.setRoomInfo(getRoomInfo());
			...
			if (reason == GameConstants.Game_End_NORMAL) {
				...
				if (!this._is_call_banker) {
					...
					for (int i = 0; i < this.getTablePlayerNumber(); i++) {
						...
						if (this.has_rule(GameConstants.GAME_RULE_PEOPLE_LESS) && this.getTablePlayerNumber() == 2
								&& this._out_card_times[i] == 1 && GRR._card_count[i] > 1) {
							game_end_pdk.setFanDiPlayer(i);
						} else if ((has_rule(GameConstants.GAME_RULE_TWO_PLAY)
						...
						} else if (GRR != null && i != GRR._banker_player && this._out_card_times[i] == 0
								&& this.has_rule(GameConstants.GAME_RULE_CHUN_TIAN)) {
							if (this._bao_pei_palyer == GameConstants.INVALID_SEAT) {
								game_end_pdk.addChunTianPlayer(i);
							} else {
								if (this._bao_pei_palyer == i) {
			...
			if (GRR != null) {
				...
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					...
					Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
					card_arry_np.Builder out_card_array = card_arry_np.newBuilder();
					for (int j = 0; j < GRR._card_count[i]; j++) {
						cards_card.addItem(GRR._cards_data[i][j]);
					}
					for (int j = 0; j < this._out_card_times[i]; j++) {
						...
						for (int x = 0; x < this._history_out_count[i][j]; x++) {
							out_cards_card.addItem(this._history_out_card[i][j][x]);
						}
						out_card_array.addCardCount(_history_out_count[i][j]);
						out_card_array.addCardsData(out_cards_card);
					}
					game_end_pdk.addOutCardArray(out_card_array);
		...
		zhan_ji_score(GameConstants.INVALID_SEAT);
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status)
			...
			if (this._handler != null && !_is_in_qie) {
			...
			} else if (!_is_in_qie || _game_status == GameConstants.GS_NPPDK_PIAO) {
				...
				tableResponse_pdk.setRoomInfo(getRoomInfo());
				if (GRR != null) {
					...
					if (_game_status == GameConstants.GS_NPPDK_PLAY) {
						for (int i = 0; i < getTablePlayerNumber(); i++) {
						...
						tableResponse_pdk.setDisplayTime(-1);
					}
			...
			if (_is_in_qie && _game_status != GameConstants.GS_NPPDK_PIAO) {
				QiePaiStart_NP.Builder qie_pai = QiePaiStart_NP.newBuilder();
				roomResponse.setType(MsgConstants.RESPONSE_PDK_QIE_START);// 201
				qie_pai.setQiepaiChair(_qie_pai_seat);
				qie_pai.setDisplayTime(6);
			...
			if (_is_in_qie && _game_status != GameConstants.GS_NPPDK_PIAO) {
				QiePaiResponse_NP.Builder qie_resopnse = QiePaiResponse_NP.newBuilder();
		...
		zhan_ji_score(seat_index);
		return true;
	}

	public void Refresh_Open_Less(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		Open_Less_Response_NP.Builder open_less = Open_Less_Response_NP.newBuilder();
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player) {
		//// 发送玩家出牌 201 =玩家出牌(CMD_OutCard: operate_player,operate_card)
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		OutCardDataPdk_NP.Builder outcarddata = OutCardDataPdk_NP.newBuilder();
	...
	public void load_player_info_data_paifenbegin(Piao_Score_Begin_NP.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public void load_player_info_data_ming_pai(MingPaiStart_NP.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_game_start(GameStart_PDK_NP.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_game_end(PukeGameEndPdk_NP.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_reconnect(TableResponse_PDK_NP.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void ming_refresh_card() {
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			...
			// 刷新玩家手牌数量
			RefreshCardsPdk_NP.Builder refreshcards = RefreshCardsPdk_NP.newBuilder();
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				if (this.has_rule(GameConstants.GAME_RULE_DISPLAY_CARD)) {
	...
	public boolean operate_player_cards() {
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
				if (play_index == i || this._ming_pai[i] == 1) {
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		...
		if (type == MsgConstants.REQUST_NPPDK_OPERATE) {
			Opreate_RequestWsk_nppdk req = PBUtil.toObject(room_rq, Opreate_RequestWsk_nppdk.class);
			switch (req.getOpreateType()) {
			case GAME_OPREATE_TYPE_PIOA_FEN: {
				deal_piao_score(seat_index, req.getScoreIdnex());
				return true;
			}
			case GAME_OPREATE_TYPE_PEOPLE_LESS: {
				deal_people_less(seat_index, req.getScoreIdnex());
				return true;
			}
			case GAME_OPREATE_TYPE_QIE_PAI_BEGIN: {
				deal_qie_pai_begin(seat_index);
				return true;
			}
			case GAME_OPREATE_TYPE_QIE_PAI: {
				deal_qie_pai_opreate(seat_index, req.getCardIndex());
				return true;
			}
			case GAME_OPREATE_TYPE_QIE_PAI_NO: {
				deal_qie_pai_opreate_no(seat_index);
				return true;
			}
			case GAME_OPREATE_TYPE_QIANG_GUAN: {
				_handler_call_banker.handler_call_banker(this, seat_index, 1);
				return true;
			}
			case GAME_OPREATE_TYPE_QIANG_GUAN_NO: {
				_handler_call_banker.handler_call_banker(this, seat_index, 0);
				return true;
			}
			case GAME_OPREATE_TYPE_MING_PAI: {
				_handler_ming_pai.handler_ming_pai(this, seat_index, 1);
				return true;
			}
			case GAME_OPREATE_TYPE_MING_PAI_NO: {
				_handler_ming_pai.handler_ming_pai(this, seat_index, 0);
				return true;
			}
			}
		}
	...
	public void deal_qie_pai_opreate_no(int seat_index) {
		if (!_is_in_qie) {
			return;
		}
		...
		if (this._game_status != GameConstants.GS_NPPDK_PLAY) {
			return;
		}
	...
	public void deal_qie_pai_opreate(int seat_index, int card_index) {
		if (!_is_in_qie) {
			return;
		}
		if (_qie_pai_seat != seat_index) {
		...
		qie_result.setCardIndex(card_index);
	...
	public void deal_qie_pai_begin(int seat_index) {
		if (_qie_pai_seat != seat_index) {
			return;
		}
	...
	public void deal_piao_score(int seat_index, int index) {
		if (this._game_status != GameConstants.GS_NPPDK_PIAO) {
			return;
		}
		int score = _piao_fen_select[index];
		this._piao_fen[seat_index] = _piao_fen_select[index];
