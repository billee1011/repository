...
public class PHHandlerChuLiFirstCard_YX extends PHHandlerDispatchCard<PHTable> {
	    ...
	    for(int j = 0; j<table.getTablePlayerNumber();j++)
	    {
			...
			{			
				for(int i = 0; i< an_long_count[j];i++)
				{
                    ...
                    table._logic.calculate_game_weave_score(table._game_mid_score,table._game_weave_score,score,table.getTablePlayerNumber(),true,table.get_match_times());
...
//			table.operate_player_get_card(_seat_index, 0, null, GameConstants.INVALID_SEAT,false);
			...
			curPlayerStatus.set_handler_status(GameConstants.PH_STATUS_CHU_LI_CARD);
...
//			table.operate_player_get_card(_seat_index, 0, null, GameConstants.INVALID_SEAT,false);
		...
		if (action != GameConstants.WIK_NULL) {
			...
			{
				table._playerStatus[_seat_index].set_handler_status(GameConstants.PH_STATUS_CHU_LI_CARD);
...
//		assert handler_status == GameConstants.PH_STATUS_CHU_LI_CARD;
		if(handler_status != GameConstants.PH_STATUS_CHU_LI_CARD){
			table.log_error("DispatchCard handler_status = "+ handler_status+"!= "+GameConstants.PH_STATUS_CHU_LI_CARD+"seat_index = " + seat_index);
//			return false;
		}
