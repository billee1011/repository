...
///////////////////////////////////////////////////////////////////////////////////////////////
public class YYWHZTable extends PHZTable {
	/**
	 * 
	 */
	private static final long serialVersionUID = 7060061356475703643L;

	private static Logger logger = Logger.getLogger(YYWHZTable.class);
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_dispath_firstcards = new WHZHandlerDispatchFirstCard_YY();
		_handler_chuli_firstcards = new WHZHandlerChuLiFirstCard_YY();
		_handler_dispath_card = new WHZHandlerDispatchCard_YY();
		_handler_out_card_operate = new WHZHandlerOutCardOperate_YY();
		_handler_gang = new WHZHandlerGang_YY();
		_handler_chi_peng = new WHZHandlerChiPeng_YY();
		_handler_wai = new WHZHandlerWai_YY();
		_handler_liu = new WHZHandlerLiu_YY();
	}
	...
	private void test_cards() {
		int cards[] = new int[] { 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7,
				0x8, 0x9 };
		...
		int count = 0;
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			for (int j = 0; j < get_hand_card_count_max() - 1; j++) {
				int index = _logic.switch_to_card_index(cards[j]);
	...
	public int analyse_chi_hu_card_yywhz(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index,
		...
		boolean bValue = _logic.analyse_card_yywhz(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index,
		...
		for (int i = 0; i < analyseItemArray.size(); i++) {
			...
			for (int j = 0; j < 7; j++) {
				...
				temp_hu_xi += _logic.get_weave_hu_xi_yywhz_yywzh(weave_items);
				temp_total_hu_xi += _logic.get_weave_hu_xi_yywhz_yywzh(weave_items);
			}
			...
			for (int index = 0; index < GameConstants.MAX_HH_INDEX; index++) {
				int hand_count = _hand_card_index[seat_index][index];
				int mo_count = _mo_card_index[seat_index][index];
				int peng_count = _peng_card_index[seat_index][index];
				int chi_count = _chi_card_index[seat_index][index];

				// 散豪
				boolean is_san_hao = true;
				if (hand_count <= 1 && mo_count + hand_count + peng_count + chi_count == 4) {
					if (is_san_hao) {
						temp_total_hu_xi += 40;
					}
				} else {
				...
				if (!is_san_hao) {
					// 外豪
					if (hand_count >= 2 && mo_count + hand_count + peng_count + chi_count == 4
							&& (peng_count > 0 || chi_count > 0)) {
						temp_total_hu_xi += 10;
					}
					// 内毫
					if (hand_count >= 2 && hand_count + mo_count == 4 && mo_count > 0) {
						temp_total_hu_xi += 20;
					}
				}
				...
				// 清溜
				if (hand_count == 4) {
					boolean is_qing_liu = true;
					for (int j = 0; j < 7; j++) {
						for (int x = 0; x < 4; x++) {
							if (analyseItem.cbCardData[j][x] == _logic.switch_to_card_data(index)) {
								// for(int y=0;y<4;y++){
								// if(_hu_weave_items[seat_index][j].weave_card[y]
								// == GRR._chi_hu_card[seat_index][0] &&
								// _hu_weave_items[seat_index][j].weave_card[y]
								// != 0){
								// is_qing_liu=false;
								// break;
								// }
								// }
								if (analyseItem.cbWeaveKind[j] == GameConstants.WIK_YYWHZ_WAI
										|| analyseItem.cbWeaveKind[j] == GameConstants.WIK_PENG
										|| analyseItem.cbWeaveKind[j] == GameConstants.WIK_LEFT
										|| analyseItem.cbWeaveKind[j] == GameConstants.WIK_CENTER
										|| analyseItem.cbWeaveKind[j] == GameConstants.WIK_RIGHT
										|| analyseItem.cbWeaveKind[j] == GameConstants.WIK_EQS
										|| analyseItem.cbMenEye[0] == _logic.switch_to_card_data(index)
										|| analyseItem.cbMenEye[1] == _logic.switch_to_card_data(index)) {
									is_qing_liu = false;
									break;
								}
							}
		...
		if (this._init_left_count - GRR._left_card_count <= 2) {
			if (this._init_left_count - GRR._left_card_count <= 1) {
				bHuBaoting = false;
			}
			if (seat_index == GRR._banker_player) {
				if (has_rule(GameConstants.GAME_RULE_YYWHZ_BANKER_DI_HU)) {
					bHuBaoting = false;
				} else if (_user_out_card_count[seat_index] > 1) {
					bHuBaoting = false;
				}
			} else if (seat_index != GRR._banker_player) {
				bHuBaoting = false;
			}
		...
		if (_init_left_count - GRR._left_card_count == 2) {
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_BANKER_DI_HU)) {
				...
				if (weaveCount > 0) {
					if (weaveItems[0].weave_kind == GameConstants.WIK_YYWHZ_WAI
							|| weaveItems[0].weave_kind == GameConstants.WIK_YYWHZ_LIU_NEI) {
						chiHuRight.opr_or(GameConstants.CHR_DI_HU_WHZ);
					}
				} else {
					chiHuRight.opr_or(GameConstants.CHR_DI_HU_WHZ);
				}
		...
		// 全大
		if (_logic.calculate_da_pai_count(analyseItem) >= 20) {
			chiHuRight.opr_or(GameConstants.CHR_QUAN_DA_WHZ);
		}
		// 全小
		if (_logic.calculate_xiao_pai_count(analyseItem) >= 20) {
		...
		for (int j = 0; j < 7; j++) {
			...
			_hu_weave_items[seat_index][j].hu_xi = _logic.get_weave_hu_xi_yywhz_yywzh(_hu_weave_items[seat_index][j]);
		...
		if (analyseItem.curCardEye == true) {
			...
			if (analyseItem.cbMenEye[0] == cur_card || analyseItem.cbMenEye[1] == cur_card) {
	...
	public void process_chi_hu_player_score_yywhz(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		if (!(chr.opr_and(GameConstants.CHR_TIAN_HU_WHZ)).is_empty()) {
			// 天胡
			all_hu_xi += 100;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
			}
			da_hu = true;
		}
		if (!(chr.opr_and(GameConstants.CHR_DI_HU_WHZ)).is_empty()) {
			// 地胡
			all_hu_xi += 100;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_TING_HU_WHZ)).is_empty()) {
			// 听胡
			all_hu_xi += 60;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_SHI_SAN_HONG_WHZ)).is_empty()) {
			;
			// 十三红
			all_hu_xi += 80
					+ (_logic.calculate_hong_pai_count(_hu_weave_items[seat_index], _hu_weave_count[seat_index]) - 13)
							* 10;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
			}
		...
		if (!(chr.opr_and(GameConstants.CHR_YI_DIAN_HONG_WHZ)).is_empty()) {
			// 一点红
			all_hu_xi += 60;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_QUAN_HEI_WHZ)).is_empty()) {
			// 全黑
			all_hu_xi += 80;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_QUAN_DA_WHZ)).is_empty()) {
			// 全大
			all_hu_xi += 100;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_QUAN_XIAO_WHZ)).is_empty()) {
			// 全小
			all_hu_xi += 100;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_WU_DUI_WHZ)).is_empty()) {
			// 无对或十对
			all_hu_xi += 120;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_SHI_DUI_WHZ)).is_empty()) {
			// 无对或十对
			all_hu_xi += 120;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_YI_DUI_WHZ)).is_empty()) {
			// 一对或九对
			all_hu_xi += 100;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_JIU_DUI_WHZ)).is_empty()) {
			// 一对或九对
			all_hu_xi += 100;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_HANG_HANG_XI_WHZ)).is_empty()) {
			// 行行息
			all_hu_xi += 60;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_DUI_ZI_XI_WHZ)).is_empty()) {
			// 对子息
			all_hu_xi += 100;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (!(chr.opr_and(GameConstants.CHR_HAI_LAO_WHZ)).is_empty()) {
			// 海底捞
			all_hu_xi += 30;
			if (!has_rule(GameConstants.GAME_RULE_YYWHZ_HAO_102030)) {
				all_hu_xi += 10;
			}
		}
		if (!(chr.opr_and(GameConstants.CHR_QUAN_QIU_REN_WHZ)).is_empty()) {
			// 全求人
			all_hu_xi += 60;
			if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
				all_hu_xi += 20;
		...
		if (da_hu) {
			for (int i = 0; i < this._hu_weave_count[seat_index]; i++) {
				if (this._hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_YYWHZ_KAN
						|| this._hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_YYWHZ_WAI
						|| this._hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_YYWHZ_LIU_NEI
						|| this._hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_YYWHZ_LIU_WAI) {
					all_hu_xi += this._hu_weave_items[seat_index][i].hu_xi;
					this._hu_xi[seat_index] += this._hu_weave_items[seat_index][i].hu_xi;
				}
			...
			all_hu_xi += 1;
			this._hu_xi[seat_index] += 1;
		}

		int nwaihao = 0;
		// 外豪
		// for (int i = 0; i < this._hu_weave_count[seat_index]; i++) {
		// if(_hu_weave_items[seat_index][i].weave_kind ==
		// GameConstants.WIK_YYWHZ_LIU_WAI){
		// all_hu_xi +=20;
		// }
		// if(_hu_weave_items[seat_index][i].weave_kind ==
		// GameConstants.WIK_YYWHZ_WAI
		// &&
		// _mo_card_index[seat_index][_logic.switch_to_card_index(_hu_weave_items[seat_index][i].center_card)]
		// == 1){
		// all_hu_xi +=20;
		// }
		// }
		for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
			...
			int mo_count = _mo_card_index[seat_index][i];
			int peng_count = _peng_card_index[seat_index][i];
			int chi_count = _chi_card_index[seat_index][i];
			...
			if (hand_count == 4) {
				boolean is_qing_liu = true;
				for (int j = 0; j < this._hu_weave_count[seat_index]; j++) {
					for (int x = 0; x < 4; x++) {
						if (_hu_weave_items[seat_index][j].weave_card[x] == _logic.switch_to_card_data(i)) {
							// for(int y=0;y<4;y++){
							// if(_hu_weave_items[seat_index][j].weave_card[y]
							// == GRR._chi_hu_card[seat_index][0] &&
							// _hu_weave_items[seat_index][j].weave_card[y] !=
							...
							if (_hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_YYWHZ_WAI
									|| _hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_PENG
									|| _hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_LEFT
									|| _hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_CENTER
									|| _hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_RIGHT
									|| _hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_EQS
									|| _hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_YYWHZ_MENZI_GUANG) {
								is_qing_liu = false;
								break;
							}
						}
	...
	public int estimate_player_liu_nei_respond_yywhz(int cards_index[], WeaveItem WeaveItem[], int cbWeaveCount,
	...
	public int estimate_player_liu_wai_respond_yywhz(int cards_index[], WeaveItem WeaveItem[], int cbWeaveCount,
			int cur_card, GangCardResult gangCardResult, boolean is_dispath) {
		int cbActionMask = GameConstants.WIK_NULL;

		return _logic.check_liu_wai_yywhz(cards_index, WeaveItem, cbWeaveCount, cur_card, gangCardResult, is_dispath);
	}
	...
	public boolean operate_show_card(int seat_index, int type, int count, int cards[], WeaveItem weaveitems[],
		...
		if (is_mj_type(GameConstants.GAME_TYPE_WHZ_YY) && GRR != null) {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	private void progress_banker_select() {
		if (this._cur_round == 1) {
			if (this.club_id != 0) {
				_cur_banker = RandomUtil.getRandomNumber(Integer.MAX_VALUE) % this.getTablePlayerNumber();// 创建者的玩家为专家
			} else {
				_cur_banker = 0;
			}
	...
	private void set_result_describe(int seat_index) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			for (int x = 0; x < GameConstants.MAX_HH_INDEX; x++) {
				int hand_count = _hand_card_index[seat_index][x];
				int mo_count = _mo_card_index[seat_index][x];
				int peng_count = _peng_card_index[seat_index][x];
				int chi_count = _chi_card_index[seat_index][x];
				...
				if (hand_count == 4) {
					...
					for (int j = 0; j < this._hu_weave_count[seat_index]; j++) {
						for (int z = 0; z < 4; z++) {
							if (_hu_weave_items[seat_index][j].weave_card[z] == _logic.switch_to_card_data(x)) {
								// for(int y=0;y<4;y++){
								// if(_hu_weave_items[seat_index][j].weave_card[y]
								// == GRR._chi_hu_card[seat_index][0] &&
								// _hu_weave_items[seat_index][j].weave_card[y]!=0){
								// is_qing_liu=false;
								// }
								// }
			...
			for (int j = 0; j < l; j++) {
				...
				if (GRR._chi_hu_rights[i].is_valid()) {
					...
					if (type == GameConstants.CHR_TIAN_HU_WHZ) {
					...
					if (type == GameConstants.CHR_DI_HU_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",地胡  X120";
						} else {
							des += ",地胡  X100";
					...
					if (type == GameConstants.CHR_TING_HU_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",听胡  X80";
						} else {
							des += ",听胡  X60";
					...
					if (type == GameConstants.CHR_SHI_SAN_HONG_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",十三红  X" + (80 + (_logic.calculate_hong_pai_count(_hu_weave_items[seat_index],
									_hu_weave_count[seat_index]) - 13) * 10 + 20);
						} else {
							des += ",十三红  X" + (80 + (_logic.calculate_hong_pai_count(_hu_weave_items[seat_index],
									_hu_weave_count[seat_index]) - 13) * 10);
						}

						da_hu = true;
					}
					if (type == GameConstants.CHR_YI_DIAN_HONG_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",一点红   X80";
						} else {
							des += ",一点红   X60";
					...
					if (type == GameConstants.CHR_QUAN_HEI_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",全黑   X100";
						} else {
							des += ",全黑   X80";
					...
					if (type == GameConstants.CHR_QUAN_DA_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",全大   X120";
						} else {
							des += ",全大   X100";
					...
					if (type == GameConstants.CHR_QUAN_XIAO_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",全小   X120";
						} else {
							des += ",全小   X100";
					...
					if (type == GameConstants.CHR_WU_DUI_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",无对   X140";
						} else {
							des += ",无对   X120";
					...
					if (type == GameConstants.CHR_SHI_DUI_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",十对   X140";
						} else {
							des += ",十对   X120";
					...
					if (type == GameConstants.CHR_YI_DUI_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",一对   X120";
						} else {
							des += ",一对   X100";
					...
					if (type == GameConstants.CHR_JIU_DUI_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",九对   X120";
						} else {
							des += ",九对   X100";
					...
					if (type == GameConstants.CHR_HANG_HANG_XI_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",行行息   X80";
						} else {
							des += ",行行息   X60";
					...
					if (type == GameConstants.CHR_DUI_ZI_XI_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",对子息   X120";
						} else {
							des += ",对子息   X100";
					...
					if (type == GameConstants.CHR_HAI_LAO_WHZ) {
					...
					if (type == GameConstants.CHR_QUAN_QIU_REN_WHZ) {
						if (has_rule(GameConstants.GAME_RULE_YYWHZ_ADD_XI)) {
							des += ",全求人   X80";
						} else {
							des += ",全求人   X60";
	...
	public void set_niao_card(int seat_index, int card, boolean show) {
		...
		if (has_rule(GameConstants.GAME_RULE_FAN_XING)) {
			...
			this.operate_player_get_card(GameConstants.INVALID_SEAT, 1, new int[] { GRR._cards_data_niao[0] },
					GameConstants.INVALID_SEAT, false);
		}
		GRR._count_pick_niao = _logic.get_xing_pai_count(this._hu_weave_items[seat_index],
				this._hu_weave_count[seat_index], GRR._cards_data_niao[0]);
	...
	public int is_tian_hu(int seat_index, int provider, ChiHuRight chiHuRight, int cur_card_data) {
		...
		// 特殊天胡
		boolean wudui = _logic.wudui(cbCardIndexTemp);
		boolean shidui = _logic.shidui(cbCardIndexTemp);
		boolean yidui = _logic.yidui(cbCardIndexTemp);
		boolean jiudui = _logic.jiudui(cbCardIndexTemp);

		if (wudui) {
			chiHuRight.opr_or(GameConstants.CHR_WU_DUI_WHZ);
			return 2;
		}
		if (shidui) {
			chiHuRight.opr_or(GameConstants.CHR_SHI_DUI_WHZ);
			return 2;
		}
		if (yidui) {
			chiHuRight.opr_or(GameConstants.CHR_YI_DUI_WHZ);
			return 2;
		}
		if (jiudui) {
			chiHuRight.opr_or(GameConstants.CHR_JIU_DUI_WHZ);
			return 2;
		}
		if (action_hu != GameConstants.WIK_NULL) {
			return 1;
