...
public class SCHCPHandlerChuLiFirstCard_DZ extends SCHCPDZHandlerDispatchCard<SCHCPDZTable> {
			...
			if(table._is_qi == false){
				for(int i = 0; i<GameConstants.MAX_CP_INDEX;i++){
					if(table.GRR._cards_index[next_player][i] >=2 )
						if(table._logic.get_times_cards(table._logic.switch_to_card_data(i)) == 1)
						{
							is_qi = true;
							break;
						}
...
//					% table.getTablePlayerNumber();
			...
			table._shang_zhuang_player = GameConstants.INVALID_SEAT;
			// 流局
			table.handler_game_finish(0, GameConstants.Game_End_DRAW);
			return ;
		}
