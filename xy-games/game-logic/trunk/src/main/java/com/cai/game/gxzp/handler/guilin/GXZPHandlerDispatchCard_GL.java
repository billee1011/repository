 ...
 *
 */
public class GXZPHandlerDispatchCard_GL extends GXZPHandlerDispatchCard<GXZPTable> {
...
//		 _send_card_data = 0x08;
		...
		int ti_sao = table.estimate_player_ti_sao_respond_hh(_seat_index,_send_card_data);
		...
		while(loop < table.getTablePlayerNumber()){
			...
			action_pao[i] = table.estimate_player_respond_glzp(i,_seat_index ,_send_card_data, pao_type[i],true);
...
//				ti_sao = GameConstants.WIK_ZI_MO;
		...
		if(ti_sao != GameConstants.WIK_TUO&&bHupai == 0){				
...
//						table.operate_remove_discard(table._current_player, table.GRR._discard_count[table._current_player]);
	...
	public boolean handler_operate_card(GXZPTable table, int seat_index, int operate_code, int operate_card,int luoCode) {
		...
		case GameConstants.WIK_NULL:
		{
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				for(int i = 0; i< table.getTablePlayerNumber();i++){
					...
					int action = table.estimate_player_respond_glzp(i,_seat_index ,_send_card_data, pao_type,true);	
		...
		case GameConstants.WIK_TUO: // 杠牌操作
		{
			...
			int pao_type[] =  new int[1];
			int action = table.estimate_player_respond_glzp(target_player,_seat_index ,_send_card_data, pao_type,true);	
			if(action != GameConstants.WIK_NULL){
