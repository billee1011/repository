...
import com.cai.game.wsk.WSKGameLogic_HS;
...
public class WSKTable_HS extends AbstractWSKTable {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_out_card_operate = new WSKHandlerOutCardOperate_HS();
		_handler_call_banker = new WSKHandlerCallBnakerHS();
	...
	public void shuffle(int repertory_card[], int card_cards[]) {
		...
		for (int i = 0; i < count; i++) {
			for (int j = 0; j < this.get_hand_card_count_max(); j++) {
				...
				if (this._logic.GetCardValue(GRR._cards_data[i][j]) == 5 || this._logic.GetCardValue(GRR._cards_data[i][j]) == 10
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player, boolean is_deal) {
		...
		if (to_player == GameConstants.INVALID_SEAT) {
			for (int index = 0; index < this.getTablePlayerNumber(); index++) {
				...
				// outcarddata.setFriendSeatIndex(GameConstants.INVALID_SEAT);
				// }
				if (this._turn_out_card_count == 0 || this._logic.search_out_card(GRR._cards_data[index], GRR._card_count[index],
						this._turn_out_card_data, this._turn_out_card_count) > 0) {
			...
			// outcarddata.setFriendSeatIndex(GameConstants.INVALID_SEAT);
			// }
			if (this._turn_out_card_count == 0 || this._logic.search_out_card(GRR._cards_data[to_player], GRR._card_count[to_player],
					this._turn_out_card_data, this._turn_out_card_count) > 0) {
