 ...
 *
 */
public class MJHandlerOutCardBaoTing_YiYang_QY extends AbstractMJHandler<MJTable_YiYang_QY> {
	...
	public void exe(MJTable_YiYang_QY table) {
		...
		// 出牌
		// table.operate_out_card_bao_ting(_out_card_player, 1, new int[] {
	...
	public boolean handler_operate_card(MJTable_YiYang_QY table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			if (jie_pao_count > 0) {
				...
				// if (table.GRR._left_card_count > 4 && jie_pao_count > 1) {
				// table.set_niao_card(_out_card_player,
				// GameConstants.INVALID_VALUE, true, 0);// 抓鸟
				// }
				table.GRR._chi_hu_rights[_out_card_player].opr_or(GameConstants.CHR_FANG_PAO);
				if(table.is_mj_type(GameConstants.GAME_TYPE_MJ_YI_YANG_QIN_YOU)){
					if(jie_pao_count > 1)
						table.set_niao_card(_out_card_player, GameConstants.INVALID_VALUE, true, 0);// 抓鸟
					else
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					/*if (table._out_card_count == 1 && _out_card_player == table._cur_banker) {
						table.GRR._chi_hu_rights[i].opr_or(GameConstants.CHR_HUNAN_DI_HU);
					}*/
					

					// if (table.GRR._left_card_count > 4 && jie_pao_count == 1)
					// {
					// table.set_niao_card(i, GameConstants.INVALID_VALUE, true,
					// 0);// 抓鸟
					// }

					table._cur_banker = jie_pao_count > 1 ? _out_card_player : i;
