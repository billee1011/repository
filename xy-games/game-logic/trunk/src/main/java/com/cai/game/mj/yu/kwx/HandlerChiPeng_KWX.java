...
public class HandlerChiPeng_KWX extends MJHandlerChiPeng<Table_KWX> {
		...
		if (table.has_rule(GameConstants_KWX.GAME_RULE_AUTO_10)) {
			...
			table.future = GameSchedule.put(new Runnable() {
				...
				public void run() {
					...
					handler_check_auto_behaviour_not_gold(table, _seat_index, GameConstants.INVALID_VALUE);
				}
	...
	public boolean handler_player_be_in_room(Table_KWX table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (i != seat_index && table.player_liang[i] == 1) {
				...
				table.operate_show_card(i, GameConstants.Show_Card_LY_HU, hand_card_count, cards, GameConstants.INVALID_SEAT);
			}
		}
		...
		if (table.player_liang[seat_index] == 1) {
			...
			table.operate_show_card(seat_index, GameConstants.Show_Card_LY_HU, hand_card_count, cards, GameConstants.INVALID_SEAT);
		}
		return true;
	}
}
