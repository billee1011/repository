 ...
 *
 */
public class PHZHandlerDispatchCard_CS extends HHHandlerDispatchCard<HHTable_CS> {
	...
	public void exe(HHTable_CS table) {
		...
		//有炮必胡
		if(table.has_rule(GameConstants_CS.GAME_RULE_PLAYER_QING_ZHI_HU_PAI)){
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (table._playerStatus[i].has_chi_hu()) {
					table.handler_operate_card(i, GameConstants.WIK_ZI_MO, table._out_card_data, -1);
				}
			}
		}
