...
public class FLSHandlerDispatchCard_LX extends FLSHandlerDispatchCard {
	...
	public void exe(FLSTable table) {
		...
		if (table.send_count_when_started != 1 && cbCardCount % 3 != 0 && cbCardCount != 2) {
		...
		if ((cbCardCount - 1) % 3 != 0 && cbCardCount != 3) {
			table.log_error("出现多牌cbCardCount==" + cbCardCount + "and " + ThreadUtil.getStack());
		}
				...
				for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
					...
					if (table.GRR._left_card_count > left) {
						...
						if (is_ting == true) {
							...
							can_gang = table.is_fls_ting_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
									table.GRR._weave_count[_seat_index]);
