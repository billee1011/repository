...
import com.cai.game.schcpdz.SCHCPDZGameLogic.AnalyseItem;
...
import com.cai.game.schcpdz.handler.SCHCPDZHandlerFinish;
import com.cai.game.schcpdz.handler.SCHCPDZHandlerGang;
import com.cai.game.schcpdz.handler.SCHCPDZHandlerOutCardOperate;
import com.cai.game.schcpdz.handler.cqydr.SCHCPHandlerChiPeng_YDR;
import com.cai.game.schcpdz.handler.cqydr.SCHCPHandlerChuLiFirstCard_YDR;
import com.cai.game.schcpdz.handler.cqydr.SCHCPHandlerDispatchAddCard_YDR;
import com.cai.game.schcpdz.handler.cqydr.SCHCPHandlerDispatchCard_YDR;
import com.cai.game.schcpdz.handler.cqydr.SCHCPHandlerDispatchFirstCard_YDR;
import com.cai.game.schcpdz.handler.cqydr.SCHCPHandlerDispatchTouCard_YDR;
import com.cai.game.schcpdz.handler.cqydr.SCHCPHandlerGang_YDR;
import com.cai.game.schcpdz.handler.cqydr.SCHCPHandlerOutCardOperate_YDR;
import com.cai.game.schcpdz.handler.cqydr.SCHCPHandlerPiao_YDR;
import com.cai.game.schcpdz.handler.dz.SCHCPHandlerChiPeng_DZ;
import com.cai.game.schcpdz.handler.dz.SCHCPHandlerChuLiFirstCard_DZ;
import com.cai.game.schcpdz.handler.dz.SCHCPHandlerDispatchAddCard_DZ;
import com.cai.game.schcpdz.handler.dz.SCHCPHandlerDispatchCard_DZ;
import com.cai.game.schcpdz.handler.dz.SCHCPHandlerDispatchFirstCard_DZ;
import com.cai.game.schcpdz.handler.dz.SCHCPHandlerDispatchTouCard_DZ;
import com.cai.game.schcpdz.handler.dz.SCHCPHandlerGang_DZ;
import com.cai.game.schcpdz.handler.dz.SCHCPHandlerOutCardOperate_DZ;
import com.cai.game.schcpdz.handler.dz.SCHCPHandlerPiao_DZ;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class SCHCPDZTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	private static Logger logger = Logger.getLogger(SCHCPDZTable.class);
	...
	public SCHCPDZGameLogic _logic = null;
	...
	public int _qi_yang_count;
	public int _piao_fen[];
	public boolean _is_bao_zi[];
	public boolean _is_yang[];
	public boolean _is_system_bao_zi[];
	public boolean _is_qi;
	public int _table_cards[];
	public int _tu_huo_index[][];
	public boolean is_record;
	public int _qi_player;
	public boolean _is_start;
	public int _is_sha_index[];
	public boolean is_zhang_zhua_pai;
	public int _cur_real_round; //真实局数
	/**
	 * 牌桌上的组合扑克 (落地的牌组合)
	 */
	...
	public SCHCPDZHandler _handler;

	public SCHCPDZHandlerDispatchCard _handler_dispath_card_xiao;
	public SCHCPDZHandlerDispatchCard _handler_dispath_card;
	public SCHCPDZHandlerDispatchCard _handler_dispath_add_card;
	public SCHCPDZHandlerOutCardOperate _handler_out_card_operate;
	public SCHCPDZHandlerGang _handler_gang;
	public SCHCPDZHandlerChiPeng _handler_chi_peng;
	public SCHCPDZHandlerDispatchCard _handler_dispath_firstcards;
	public SCHCPDZHandlerDispatchCard _handler_chuli_firstcards;
	public SCHCPDZHandlerDispatchCard _handler_dispatch_toucards;
	public SCHCPDZHandler _handler_piao_fen;

	// public HHHandlerDispatchLastCard_YX _handler_dispath_last_card;

	public SCHCPDZHandlerFinish _handler_finish; // 结束

	public SCHCPDZTable() {
		super(RoomType.HH, GameConstants.GAME_PLAYER);

		_logic = new SCHCPDZGameLogic();
		...
		_is_sha_index = new int[this.getTablePlayerNumber()];

		_is_start = false;
		is_zhang_zhua_pai = true;
		card_for_fan_xing = 0;
		fan_xing_count = 0;
		...
		_is_bao_zi = new boolean[this.getTablePlayerNumber()];
		Arrays.fill(_is_bao_zi, false);
		_is_system_bao_zi = new boolean[this.getTablePlayerNumber()];
		Arrays.fill(_is_system_bao_zi, false);
		_is_yang = new boolean[this.getTablePlayerNumber()];
		Arrays.fill(_is_yang, false);
		_table_cards = new int[GameConstants.DZ_MAX_CP_INDEX];
		Arrays.fill(_table_cards, 0);
		_is_qi = false;
		...
		_guo_hu_pai_cards = new int[this.getTablePlayerNumber()][GameConstants.MAX_CP_INDEX];
		_guo_hu_xi = new int[this.getTablePlayerNumber()][GameConstants.MAX_CP_INDEX];
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
		...
		_is_display = false;
		_gang_card_data = new CardsData(GameConstants.DZ_MAX_CP_COUNT);
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_zhe_ding_card = false;
		_zhe_fu_card = false;
		if (is_mj_type(GameConstants.GAME_TYPE_CP_DZHUA)) {
			_handler_dispath_card = new SCHCPHandlerDispatchCard_DZ();
			_handler_out_card_operate = new SCHCPHandlerOutCardOperate_DZ();
			_handler_gang = new SCHCPHandlerGang_DZ();
			_handler_chi_peng = new SCHCPHandlerChiPeng_DZ();
			_handler_dispath_add_card = new SCHCPHandlerDispatchAddCard_DZ();
			_handler_chuli_firstcards = new SCHCPHandlerChuLiFirstCard_DZ();
			_handler_dispath_firstcards = new SCHCPHandlerDispatchFirstCard_DZ();
			_handler_dispatch_toucards = new SCHCPHandlerDispatchTouCard_DZ();
			_handler_piao_fen = new SCHCPHandlerPiao_DZ();
		} else if (is_mj_type(GameConstants.GAME_TYPE_CHQ_YDR)) {
			_handler_dispath_card = new SCHCPHandlerDispatchCard_YDR();
			_handler_out_card_operate = new SCHCPHandlerOutCardOperate_YDR();
			_handler_gang = new SCHCPHandlerGang_YDR();
			_handler_chi_peng = new SCHCPHandlerChiPeng_YDR();
			_handler_dispath_add_card = new SCHCPHandlerDispatchAddCard_YDR();
			_handler_chuli_firstcards = new SCHCPHandlerChuLiFirstCard_YDR();
			_handler_dispath_firstcards = new SCHCPHandlerDispatchFirstCard_YDR();
			_handler_dispatch_toucards = new SCHCPHandlerDispatchTouCard_YDR();
			_handler_piao_fen = new SCHCPHandlerPiao_YDR();
		}

		_handler_finish = new SCHCPDZHandlerFinish();
	...
	public boolean reset_init_data() {
		...
		if (_cur_round == 0&&_cur_real_round == 0) {
		...
		_fang_pao_index = GameConstants.INVALID_SEAT;
		_xing_count = 0;
		_xing_card = new int[GameConstants.DZ_MAX_CP_COUNT];
		_xing_card_player = new int[this.getTablePlayerNumber()][GameConstants.MAX_HH_COUNT];
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			has_shoot[i] = false;
			is_hands_up[i] = false;
		...
		GRR = new GameRoundRecord(this.getTablePlayerNumber(), GameConstants.DZ_MAX_CP_WEAVE, GameConstants.DZ_MAX_CP_COUNT,
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			_playerStatus[i] = new PlayerStatus(GameConstants.DZ_MAX_CP_COUNT);
		}
	...
	protected boolean on_handler_game_start() {
		...
		// {
		// this._cur_banker = (int)
		// (RandomUtil.getRandomNumber(Integer.MAX_VALUE)
		...
		_repertory_card = new int[GameConstants.DZ_CARD_COUNT];
		shuffle(_repertory_card, GameConstants.CARD_DATA_CP_DZ);

		// DEBUG_CARDS_MODE = true;
		// BACK_DEBUG_CARDS_MODE = true;
		if (DEBUG_CARDS_MODE || BACK_DEBUG_CARDS_MODE)
			test_cards();
		if (this.is_mj_type(GameConstants.GAME_TYPE_CP_DZHUA)) {
			_game_status = GameConstants.GS_MJ_PIAO;// 设置状态
			game_start_paio_fen();
		}
		if (this.is_mj_type(GameConstants.GAME_TYPE_CHQ_YDR)) {
			game_start_chq_ydr();
		}
	...
	public boolean game_start_chq_ydr() {
		...
		int hand_cards[][] = new int[playerCount][GameConstants.DZ_MAX_CP_COUNT];
		// 发送数据
		...
		for (int i = 0; i < playerCount; i++) {
			...
			for (int j = 0; j < GameConstants.DZ_MAX_CP_COUNT; j++) {
			...
			roomResponse.clearEspecialShowCards();
			roomResponse.addDouliuzi(this.caculate_mao_shu(i)); // 卯数
			roomResponse.addEspecialShowCards(this.caculate_tuo_shu(i));// 砣数
		...
		this.exe_chuli_first_card(this._cur_banker, GameConstants.WIK_NULL, 2300);
	...
	public void set_banker() {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_SET_BANKER);
		roomResponse.setOperatePlayer(this._cur_banker);
		this.send_response_to_room(roomResponse);
	}

	public void set_qi_player() {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_SET_QI_PLAYER);
		roomResponse.setOperatePlayer(this._qi_player);
		this.send_response_to_room(roomResponse);
	}
	...
	public boolean game_start_cp() {
		...
		this._game_status = GameConstants.GS_MJ_PLAY;// 设置状态
		GameStartResponse.Builder gameStartResponse = GameStartResponse.newBuilder();
		gameStartResponse.setBankerPlayer(-1);
		gameStartResponse.setCurrentPlayer(this._current_player);
		gameStartResponse.setLeftCardCount(this.GRR._left_card_count);
	...
	public void shuffle(int repertory_card[], int mj_cards[]) {
		...
		for (int i = 0; i < count; i++) {
			send_count = (GameConstants.DZ_MAX_CP_FIRST_COUNT - 1);
	...
	public int is_operate_type(int seat_index, int operate_code, int operate_card) {
		int operate_type = GameConstants.DZ_WIK_DUI_LEFT | GameConstants.DZ_WIK_DUI_CENTER | GameConstants.DZ_WIK_DUI_RIGHT
				| GameConstants.DZ_WIK_SINGLE_LEFT | GameConstants.DZ_WIK_SINGLE_CENTER | GameConstants.DZ_WIK_SINGLE_RIGHT
				| GameConstants.DZ_WIK_OTHER | GameConstants.DZ_WIK_PENG;
		if ((operate_code & operate_type) == 0)
			return 1;
		switch (operate_code) {
		case GameConstants.DZ_WIK_DUI_LEFT:
		case GameConstants.DZ_WIK_DUI_CENTER:
		case GameConstants.DZ_WIK_DUI_RIGHT: {
			int cbRemoveCard[] = new int[3];
			cbRemoveCard[0] = this._logic.get_kind_card(operate_card, operate_code);
			cbRemoveCard[1] = cbRemoveCard[0];
			if (!_logic.remove_cards_by_index(GRR._cards_index[seat_index], cbRemoveCard, 2)) {
				log_player_error(seat_index, "吃牌删除出错");
				return 0;
			}
			cbRemoveCard[2] = operate_card;

			int tuo_shu = caculate_tuo_shu(seat_index);
			int mao_shu = caculate_mao_shu(seat_index);
			for (int i = 0; i < cbRemoveCard.length; i++)
				if (_logic.get_hong_dot(cbRemoveCard[i]) != 0)
					tuo_shu++;
			for (int i = 0; i < 2; i++)
				GRR._cards_index[seat_index][_logic.switch_to_card_index(cbRemoveCard[i])]++;
			if(this._cur_banker == seat_index){
				
				if (tuo_shu < 34 || mao_shu < 6) {
					// 刷新自己手牌
					return GameConstants.DZ_WIK_NULL;
				}
			}
			if (tuo_shu < 33 || mao_shu < 6) {
				// 刷新自己手牌
				return GameConstants.DZ_WIK_NULL;
			}
			return operate_code;
		}
		case GameConstants.DZ_WIK_SINGLE_LEFT:
		case GameConstants.DZ_WIK_SINGLE_CENTER:
		case GameConstants.DZ_WIK_SINGLE_RIGHT: {
			int cbRemoveCard[] = new int[3];
			cbRemoveCard[0] = this._logic.get_kind_card(operate_card, operate_code);
			cbRemoveCard[1] = operate_card;
			if (!_logic.remove_cards_by_index(GRR._cards_index[seat_index], cbRemoveCard, 2)) {
				log_player_error(seat_index, "吃牌删除出错");
				return 0;
			}
		...
		case GameConstants.DZ_WIK_OTHER: {
			int cbRemoveCard[] = new int[3];
			int cards[] = new int[3];
			int cards_count = _logic.switch_to_value_to_card(7, cards);
			int remove_count = 0;
			for (int i = 0; i < cards_count; i++) {
				if (cards[i] == operate_card)
					continue;
		...
		case GameConstants.DZ_WIK_PENG: {
			int cbRemoveCard[] = new int[3];
			cbRemoveCard[0] = operate_card;
			cbRemoveCard[1] = operate_card;
			if (!_logic.remove_cards_by_index(GRR._cards_index[seat_index], cbRemoveCard, 2)) {
				log_player_error(seat_index, "吃牌删除出错");
				return 0;
			...
			int tuo_shu = caculate_tuo_shu(seat_index);
			int mao_shu = caculate_mao_shu(seat_index);
			if (_logic.get_hong_dot(cbRemoveCard[0]) != 0)
				tuo_shu += 8;
			else
				tuo_shu += 4;
	...
	public boolean is_bao_zi(int seat_index) {
		int tuo_shu = caculate_tuo_shu(seat_index);
		int mao_shu = caculate_mao_shu(seat_index);
		
		if ((seat_index==this._cur_banker&&tuo_shu < 34 )||(seat_index!=this._cur_banker&&tuo_shu < 33) || mao_shu < 7) {
			// 刷新自己手牌
			return true;
		}
	...
	public int caculate_tuo_shu(int seat_index) {
		int tuo_shu = this._logic.get_max_tuo_shu(GRR._weave_items[seat_index], GRR._weave_count[seat_index], GRR._cards_index[seat_index],
				this._table_cards);

		return tuo_shu;
	}

	public int caculate_mao_shu(int seat_index) {
		int mao_shu = GRR._weave_count[seat_index];
		int temp_card_index[] = new int[GameConstants.MAX_CP_INDEX];
		Arrays.fill(temp_card_index, 0);
		for (int i = 0; i < GameConstants.MAX_CP_INDEX; i++) {
			temp_card_index[i] = GRR._cards_index[seat_index][i];
		}
		for (int i = 0; i < GameConstants.MAX_CP_INDEX; i++) {
			if (temp_card_index[i] == 5) {
				mao_shu += 3;
				temp_card_index[i] = 0;
			}
			if (temp_card_index[i] == 4) {
				if(_tu_huo_index[seat_index][i]>0)
					mao_shu+=2;
				else
					mao_shu += 3;
				temp_card_index[i] = 0;
			...
			// if(_table_cards[i] != 3)
			// flag = true;
			// int dot = _logic.get_dot(_logic.switch_to_card_data(i));
			// int cards[] = new int[3];
			// int count = _logic.switch_to_value_to_card(dot,cards);
			// for(int j = 0; j< count&&flag == false;j++){
			// if(_table_cards[_logic.switch_to_card_index(cards[j])] != 5)
			// {
			// flag = true;
			// break;
			// }
		...
		for (int i = 2; i < 7; i++) {
			int count = 0;
			int cards[] = new int[3];
			count = _logic.switch_to_value_to_card(i, cards);
			int count_next = 0;
			int cards_next[] = new int[3];
			count_next = _logic.switch_to_value_to_card(14 - i, cards_next);
			int any_count = 0;
			int any_next_count = 0;
			int dui_zi_count = 0;
			int dui_zi_next_count = 0;
			for (int j = 0; j < count; j++) {
				int cur_index = _logic.switch_to_card_index(cards[j]);
				any_count += temp_card_index[cur_index];
				if (temp_card_index[cur_index] == 2)
					dui_zi_count++;
			}
			for (int j = 0; j < count_next; j++) {
				int cur_index = _logic.switch_to_card_index(cards_next[j]);
				any_next_count += temp_card_index[cur_index];
				if (temp_card_index[cur_index] == 2)
					dui_zi_next_count++;
			}
			if (any_count > any_next_count) {
				mao_shu += any_next_count;
				if (any_count - any_next_count > 1) {
					int temp_dui_count = (any_count - any_next_count) / 2;
					if (temp_dui_count > dui_zi_count)
						mao_shu += dui_zi_count;
					else
						mao_shu += temp_dui_count;
				}
			} else {
				mao_shu += any_count;
				if (any_next_count - any_count > 1) {
					int temp_dui_count = (any_next_count - any_count) / 2;
					if (temp_dui_count > dui_zi_next_count)
						mao_shu += dui_zi_next_count;
					else
						mao_shu += temp_dui_count;
				}
			}
		}
		int seven_cards[] = new int[3];
		int seven_count = _logic.switch_to_value_to_card(7, seven_cards);
		int seven_card_count = 0;
		for (int i = 0; i < seven_count; i++) {
			if (GRR._cards_index[seat_index][_logic.switch_to_card_index(seven_cards[i])] == 2) {
				mao_shu++;
			}
			if (GRR._cards_index[seat_index][_logic.switch_to_card_index(seven_cards[i])] == 1) {
				seven_card_count++;
			}
		}
		if (seven_card_count > 1)
			mao_shu++;
		return mao_shu;
	}
	...
	public int get_cur_index(int seat_index) {
		int loop = 0;
		int cur_index = seat_index % this.getTablePlayerNumber();
		while (loop++ < this.getTablePlayerNumber()) {
			cur_index = (seat_index + loop) % this.getTablePlayerNumber();
			if (this._is_yang[cur_index] == true)
				continue;
			if (this._is_bao_zi[cur_index] == true)
				continue;
			if (this._is_system_bao_zi[cur_index] == true)
				continue;
			break;
		}
		if (loop > this.getTablePlayerNumber()) {
			cur_index = seat_index % this.getTablePlayerNumber();
		}
		return cur_index;
	}

	public boolean get_is_kou_player(int seat_index) {
		boolean is_kou = false;
		if (this._is_yang[seat_index] == true)
			is_kou = true;
		if (this._is_bao_zi[seat_index] == true)
			is_kou = true;
		if (this._is_system_bao_zi[seat_index] == true)
			is_kou = true;
		return is_kou;
	}
...
//				 11, 102, 248, 4, 7, 80, 21, 67, 18, 32, 66, 128, 9, 102, 9, 19, 11, 4, 22, 80, 5, 11, 70, 216, 69, 10, 22, 22, 9, 4, 6, 6, 67, 67, 5, 69, 6, 70, 32, 128, 80, 32, 19, 102, 7, 9, 70, 32, 66, 67, 102, 216, 70, 248, 216, 10, 5, 4, 5, 7, 102, 11, 4, 10, 18, 7, 80, 69, 67, 18, 248, 22, 248, 22, 66, 19, 32, 70, 6, 21, 21, 19, 66, 5, 10, 216, 128, 21, 128, 18, 9, 7, 21, 69, 66, 216, 18, 128, 80, 19, 248, 69, 6, 10, 11
//		 };
//		 this._cur_banker = 2;
//		 // testSameCard(realyCards);
//		 testRealyCard(realyCards);
		...
		if (BACK_DEBUG_CARDS_MODE) {
			if (debug_my_cards != null) {
				if (debug_my_cards.length > GameConstants.DZ_MAX_CP_COUNT) {
	...
	public void testSameCard(int[] cards) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			int send_count = (GameConstants.DZ_MAX_CP_FIRST_COUNT - 1);
	...
	public boolean handler_game_finish_schcp(int seat_index, int reason) {
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL ||
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				if (this._is_yang[i] == true || this._is_system_bao_zi[i] == true)
					game_end.addCardsDataNiao(1); // 1 是卯
				else if (this._is_bao_zi[i] == true)
					game_end.addCardsDataNiao(2); // 2是包子
				else
					game_end.addCardsDataNiao(0); // 正常用户
			...
			for (int i = 0; i < count; i++) {
				game_end.addPao(this._piao_fen[i]); // -1 ,没有参与 0没有飘分， 1就是飘分
				...
				int zhao_cards[] = new int[GameConstants.DZ_MAX_CP_COUNT];
		...
		if (this._cur_banker != seat_index && is_mj_type(GameConstants.GAME_TYPE_CHQ_YDR))
	...
	public int analyse_chi_hu_card(int cards_index[], WeaveItem weaveItems[], int weave_count, int seat_index, int provider_index, int cur_card,
		...
		if (is_mj_type(GameConstants.GAME_TYPE_CP_DZHUA))
			return analyse_chi_hu_card_cp_dz(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type,
					dispatch);
		if (is_mj_type(GameConstants.GAME_TYPE_CHQ_YDR))
			return analyse_chi_hu_card_cp_ydr(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type,
					dispatch);
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_cp_ydr(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index, int cur_card,
			ChiHuRight chiHuRight, int card_type, boolean dispatch) {
		...
		boolean bValue = _logic.analyse_card(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index, cur_card, analyseItemArray,
				dispatch, true);
		if (!bValue) {
			if (cbCardIndexTemp[_logic.switch_to_card_index(cur_card)] == 3 && dispatch == false) {
				bValue = _logic.analyse_card(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index, cur_card, analyseItemArray,
						dispatch, false);
			}
			if (!bValue) {
		...
		analyseItem = analyseItemArray.get(0);
		boolean is_peng_peng_hu = true;
		int chi_type = GameConstants.DZ_WIK_SINGLE_LEFT | GameConstants.DZ_WIK_SINGLE_CENTER | GameConstants.DZ_WIK_SINGLE_RIGHT
				| GameConstants.DZ_WIK_DUI_LEFT | GameConstants.DZ_WIK_DUI_CENTER | GameConstants.DZ_WIK_DUI_RIGHT;
		for (int j = 0; j < 9; j++) {
			if (analyseItem.cbWeaveKind[j] == GameConstants.WIK_NULL)
				break;

			_hu_weave_items[seat_index][j].weave_kind = analyseItem.cbWeaveKind[j];
			if ((_hu_weave_items[seat_index][j].weave_kind & chi_type) != 0)
		...
		if (this._is_di_hu == false && seat_index != this._cur_banker) {
		...
		if (is_peng_peng_hu == true)
			chiHuRight.opr_or(GameConstants.CHR_CP_PENG_HU);
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_cp_dz(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index, int cur_card,
			ChiHuRight chiHuRight, int card_type, boolean dispatch) {
	...
	public int estimate_player_hua(int seat_index, int card_data) {
		int bAroseAction = GameConstants.DZ_WIK_NULL;
		// 内滑判断
		if ((bAroseAction == GameConstants.DZ_WIK_NULL)
				&& (_logic.check_nei_hua(this.GRR._cards_index[seat_index], card_data) != GameConstants.DZ_WIK_NULL)) {
			int action = GameConstants.DZ_WIK_PENG;
			...
			bAroseAction = GameConstants.DZ_WIK_PENG;
		}
		return bAroseAction;
	}

	// 玩家出版的动作检测 偷
	public int estimate_player_tu_huo(int seat_index) {
		int count = GameConstants.DZ_WIK_NULL;
		for (int i = 0; i < GameConstants.MAX_CP_INDEX; i++) {
			if (this.GRR._cards_index[seat_index][i] == 4 && this._tu_huo_index[seat_index][i] == 0) {
				this._playerStatus[seat_index].add_action(GameConstants.DZ_WIK_TU_HUO);
				this._playerStatus[seat_index].add_tu_huo(_logic.switch_to_card_data(i), GameConstants.DZ_WIK_TU_HUO, seat_index);
				count++;
				break;
			}
		}
	...
	// 玩家出版的动作检测 偷
	public int estimate_player_gang(int seat_index, int cur_card, boolean dispatch) {
		int action = GameConstants.DZ_WIK_NULL;
		int cur_index = _logic.switch_to_card_index(cur_card);

		if (this.GRR._cards_index[seat_index][cur_index] == 3) {
			this._playerStatus[seat_index].add_action(GameConstants.DZ_WIK_SHA);
			this._playerStatus[seat_index].add_dz_gang(cur_card, GameConstants.DZ_WIK_SHA, seat_index);
			action = GameConstants.DZ_WIK_SHA;
		}
	...
	// 玩家出版的动作检测 偷
	public int estimate_player_tu_huo_xiang(int seat_index, int cur_card, boolean dispatch) {
		int action = GameConstants.DZ_WIK_NULL;
		int cur_index = _logic.switch_to_card_index(cur_card);

		if (this.GRR._cards_index[seat_index][cur_index] == 4) {
			this._playerStatus[seat_index].add_action(GameConstants.DZ_WIK_TUX_HUO);
			this._playerStatus[seat_index].add_tu_huox(cur_card, GameConstants.DZ_WIK_TUX_HUO, seat_index);
			action = GameConstants.DZ_WIK_TUX_HUO;
		}

		return action;
	}

	public int estimate_player_tu_huo(int seat_index, int cur_card) {
		int action = GameConstants.DZ_WIK_NULL;
		int cur_index = _logic.switch_to_card_index(cur_card);

		if (this.GRR._cards_index[seat_index][cur_index] == 3) {
			this._playerStatus[seat_index].add_action(GameConstants.DZ_WIK_TU_HUO);
			this._playerStatus[seat_index].add_tu_huo(cur_card, GameConstants.DZ_WIK_TU_HUO, seat_index);
			action = GameConstants.DZ_WIK_TU_HUO;
		}
	...
	// 玩家出版的动作检测 偷
	public int estimate_player_ming_tu_huo(int seat_index, int cur_card) {
		int action = GameConstants.DZ_WIK_NULL;
		int cur_index = _logic.switch_to_card_index(cur_card);

		if (this.GRR._cards_index[seat_index][cur_index] == 4) {
			this._playerStatus[seat_index].add_action(GameConstants.DZ_WIK_MTU_HUO);
			this._playerStatus[seat_index].add_tu_huo(cur_card, GameConstants.DZ_WIK_MTU_HUO, seat_index);
			action = GameConstants.DZ_WIK_MTU_HUO;
		}
		if (this._is_sha_index[seat_index] != -1)
			if (this._is_sha_index[seat_index] == _logic.switch_to_card_index(cur_card)) {
				this._playerStatus[seat_index].add_action(GameConstants.DZ_WIK_MTU_HUO);
				this._playerStatus[seat_index].add_tu_huo(cur_card, GameConstants.DZ_WIK_MTU_HUO, seat_index);
				action = GameConstants.DZ_WIK_MTU_HUO;
			}
	...
	// 玩家出版的动作检测 偷
	public int estimate_player_ming_tu_huo(int seat_index) {
		int count = GameConstants.DZ_WIK_NULL;
		for (int i = 0; i < GameConstants.MAX_CP_INDEX; i++) {
			if (this.GRR._cards_index[seat_index][i] == 5) {
				this._playerStatus[seat_index].add_action(GameConstants.DZ_WIK_MTU_HUO);
				this._playerStatus[seat_index].add_tu_huo(_logic.switch_to_card_data(i), GameConstants.DZ_WIK_MTU_HUO, seat_index);
				if (_tu_huo_index[seat_index][i] > 0)
					count += 1;
				else
					count += 2;
		...
		if (this._is_sha_index[seat_index] != -1)
			if (this.GRR._cards_index[seat_index][this._is_sha_index[seat_index]] == 1) {
				this._playerStatus[seat_index].add_action(GameConstants.DZ_WIK_MTU_HUO);
				this._playerStatus[seat_index].add_tu_huo(_logic.switch_to_card_data(this._is_sha_index[seat_index]), GameConstants.DZ_WIK_MTU_HUO,
						seat_index);
	...
	public boolean get_end_dz() {
		boolean is_end = false;
		if (GRR._left_card_count <= 0) {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				if (this.getRuleValue(GameConstants.GAME_RULE_DZ_BAO_ZI_BU_PEI) == 1)
					continue;
				if (this._is_bao_zi[i] == false) {
					continue;
				}
				for (int j = 0; j < this.getTablePlayerNumber(); j++) {
					if (i == j)
						continue;
					this.GRR._game_score[j] -= -4;
					this.GRR._game_score[i] += -4;
				}
			}
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				this._game_score[i][this._cur_round - 1] = (int) this.GRR._game_score[i];
		...
		if (GRR._left_card_count <= 0) {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				for (int j = 0; j < this.getTablePlayerNumber(); j++) {
					if (i == j)
						continue;
					if (this.getRuleValue(GameConstants.GAME_RULE_DZ_BAO_ZI_PEI_4) == 1) {
						this.GRR._game_score[j] -= -4;
						this.GRR._game_score[i] += -4;
					} else {
						this.GRR._game_score[j] -= -2;
						this.GRR._game_score[i] += -2;
					}
				}
			}
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public int estimate_player_tou(int seat_index, int card_data, int operate_type) {
		int weave_kind = GameConstants.DZ_WIK_DUI_CENTER;
		// if(card_data == 0x12){
		//
		// switch(_ding_count[seat_index]){
		// case 1:
		// weave_kind = GameConstants.DZ_WIK_DH_TWO;
		// break;
		// case 2:
		// weave_kind = GameConstants.DZ_WIK_DH_THREE;
		// break;
		// case 3:
		// weave_kind = GameConstants.DZ_WIK_DH_FOUR;
		// break;
		// }
	...
	public int estimate_player_peng(int seat_index) {
		...
		for (int i = 0; i < GameConstants.MAX_CP_INDEX; i++) {
			if (GRR._cards_index[seat_index][i] == 3) {
				this._playerStatus[seat_index].add_action(GameConstants.DZ_WIK_PENG);
				this._playerStatus[seat_index].add_cp_peng(_logic.switch_to_card_data(i), GameConstants.DZ_WIK_PENG, seat_index);
				player_pass = 1;
		...
		if (player_pass == 1) {
			this._playerStatus[seat_index].add_action(GameConstants.DZ_WIK_NULL);
	...
	public boolean estimate_player_chi(int seat_index, int card, boolean bDisdatch) {
		...
		do {
			...
			if (next_index == 1)
				chiseat_index = this.get_cur_index(seat_index);

			next_index++;
			if (chiseat_index == seat_index && bDisdatch == false)
				continue;
			...
			if (_playerStatus[chiseat_index].lock_huan_zhang() == false) {
				...
				if (this.GRR._cards_index[chiseat_index][_logic.switch_to_card_index(card)] > 0
						&& this.GRR._cards_index[chiseat_index][_logic.switch_to_card_index(card)] < 3) {
					for (int i = 0; i < cards_count; i++) {
						...
						if (card == cards[i])
							continue;
						action = _logic.get_chi_action(card, cards[i], card);
				...
				for (int i = 0; i < cards_count; i++) {
					...
					cards_data[cards_data_count++] = cards[i];
					cards_data[cards_data_count++] = cards[i];
					action = _logic.get_chi_action(card, cards[i], cards[i]);
					if (this.GRR._cards_index[chiseat_index][_logic.switch_to_card_index(cards[i])] == 2) {
						playerStatus.add_action(action);
						playerStatus.add_cp_chi(card, action, seat_index);
						player_pass[chiseat_index] = 1;
				...
				boolean flag = true;
				for (int i = 0; i < cards_count && card_dot == 7; i++) {
					if (cards[i] == card)
						continue;
					if (this.GRR._cards_index[chiseat_index][_logic.switch_to_card_index(cards[i])] <= 0
							|| this.GRR._cards_index[chiseat_index][_logic.switch_to_card_index(cards[i])] >= 3) {
						flag = false;
						break;
					}

				}
				if (flag == true && card_dot == 7) {
					action = _logic.get_chi_action(cards[0], cards[1], cards[2]);
					playerStatus.add_action(action);
					playerStatus.add_cp_chi(card, action, seat_index);
					player_pass[chiseat_index] = 1;
					bAroseAction = true;
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (this._is_yang[i] == true)
				continue;
			if (this._is_bao_zi[i] == true)
				continue;
			if (this._is_system_bao_zi[i] == true)
				continue;
			// if (this._cannot_peng_index[i][_logic.switch_to_card_index(card)]
			// != 0)
			// continue;
			...
			if (_playerStatus[i].lock_huan_zhang() == false) {
				...
				if (action != 0) {
					playerStatus.add_action(action);
					playerStatus.add_cp_peng(card, GameConstants.DZ_WIK_PENG, seat_index);
					bAroseAction = true;
					player_pass[i] = 1;
				}
			}
	...
	public boolean estimate_player_chi_ydr(int seat_index, int card, boolean bDisdatch) {
		// 变量定义
		boolean bAroseAction = false;// 出现(是否)有
		...
		do {
			...
			// continue;
			int card_count = 0;
			boolean is_fan_card = false;
			int is_two_count = 0;
			int wFanShu = _logic.calculate_fan_count_ydr(GRR._weave_items[chiseat_index], GRR._weave_count[chiseat_index]);
			for (int j = 0; j < GameConstants.MAX_CP_INDEX; j++) {
				if (this.GRR._cards_index[chiseat_index][j] == 4 && _logic.get_times_cards_ydr(_logic.switch_to_card_data(j)) > 0)
					wFanShu++;
				if (this.GRR._cards_index[chiseat_index][j] == 3 && _logic.get_times_cards_ydr(_logic.switch_to_card_data(j)) > 0)
					wFanShu++;
				if (this.GRR._cards_index[chiseat_index][j] == 2 && _logic.get_times_cards_ydr(_logic.switch_to_card_data(j)) > 0
						&& this.GRR._cards_index[chiseat_index][j] + this._table_cards[j] != 5)
					is_two_count++;

				if (this.GRR._cards_index[chiseat_index][j] <= 2)
					card_count += this.GRR._cards_index[chiseat_index][j];
			}
			if (wFanShu == 0 &&is_two_count>0 && this._qi_player == chiseat_index) {
				is_fan_card = true;
			}
			if (card_count == 2)
				continue;
			...
			if (_playerStatus[chiseat_index].lock_huan_zhang() == false) {
				...
				int card_dot = _logic.get_dot(card);
				cards_count = _logic.switch_to_value_to_card(14 - card_dot, cards);
				if (is_fan_card == true && (this.GRR._cards_index[chiseat_index][_logic.switch_to_card_index(card)] == 2)
						&& _logic.get_times_cards_ydr(card) > 0)
					continue;
				if (this.GRR._cards_index[chiseat_index][_logic.switch_to_card_index(card)] > 0
						&& this.GRR._cards_index[chiseat_index][_logic.switch_to_card_index(card)] < 3) {
					for (int i = 0; i < cards_count; i++) {
						if (this.GRR._cards_index[chiseat_index][_logic.switch_to_card_index(cards[i])] > 2)
							continue;
						if (is_fan_card == true && (this.GRR._cards_index[chiseat_index][_logic.switch_to_card_index(cards[i])] == 2)
								&& _logic.get_times_cards_ydr(cards[i]) > 0)
							continue;
						int cards_data[] = new int[2];
						int cards_data_count = 0;
						cards_data[cards_data_count++] = card;
...
//			if (_is_yang[seat_index] == false && _is_bao_zi[seat_index] == false && _is_system_bao_zi[seat_index] == false && _is_start == true)
//				operate_effect_action(seat_index, GameConstants.EFFECT_ACTION_CP, 1, new long[] { GameConstants.CP_DISPLAY_KOU }, 1, seat_index);
	...
	public boolean is_game_end() {
		...
		if (flag == false) {
			...
			// _banker_select = (GRR._banker_player +
			// getTablePlayerNumber() + 1)
			// % getTablePlayerNumber();
			_shang_zhuang_player = GameConstants.INVALID_SEAT;
			// 流局
			handler_game_finish(this._cur_banker, GameConstants.Game_End_DRAW);
		}
		return flag;
	}
	...
	public boolean handler_yang_operate_card(int seat_index, boolean is_yang) {
		if (this._is_yang[seat_index] == true || this._is_bao_zi[seat_index] == true || this._is_system_bao_zi[seat_index] == true) {
			this.send_error_notify(seat_index, 2, "您已经是包子 或者 仰操作！");
			return true;
		}
		if(_is_start == true)
		{
			this.send_error_notify(seat_index, 2, "游戏中不能扣！");
			return true;
		}
		this._is_yang[seat_index] = true;
		this.operate_player_xiang_gong_flag(seat_index, true);
		operate_effect_action(seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { GameConstants.DZ_WIK_YANG }, 1,
				GameConstants.INVALID_SEAT);
		if (is_game_end() == false)
			return true;
		if (this._handler != null && this._playerStatus[seat_index].has_action()
				&& this._playerStatus[seat_index].get_status() == GameConstants.Player_Status_OPR_CARD) {
			this._handler.handler_operate_card(this, seat_index, 0, 0, -1);
		} else if (this._playerStatus[seat_index].get_status() == GameConstants.Player_Status_OUT_CARD) {
			...
			int next_player = get_cur_index(seat_index);

			_current_player = next_player;
			_last_player = next_player;
			// 没有人要就加入到牌堆
			exe_dispatch_card(next_player, GameConstants.WIK_NULL, 0);
		}
	...
	public void process_chi_hu_player_score_schcp(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		for (int i = 0; i < this._hu_weave_count[seat_index]; i++) {
			...
			switch (this._hu_weave_items[seat_index][i].weave_kind) {
			case GameConstants.DZ_WIK_PENG:
			case GameConstants.DZ_WIK_SHA_QIANG: {
		...
		wFanShu += ((all_hu_xi - 33)) / 10;
		if (this.getRuleValue(GameConstants.GAME_RULE_DZ_3_FENG_DING) == 1 && wFanShu > 3)
			wFanShu = 3;
		if (this.getRuleValue(GameConstants.GAME_RULE_DZ_4_FENG_DING) == 1 && wFanShu > 4)
			wFanShu = 4;
	 ...
	 * @param zimo
	 */
	public void process_chi_hu_player_score_chq_ydr(int seat_index, int provide_index, int operate_card, boolean zimo) {
		zimo = true;
		GRR._win_order[seat_index] = 1;
		_fang_pao_index = provide_index;
		if (zimo == false) {
		...
		if (!(chr.opr_and(GameConstants.CHR_CP_PENG_HU)).is_empty()) {
			wFanShu += 1;

		}
		wFanShu += _logic.calculate_fan_count_ydr(this._hu_weave_items[seat_index], this._hu_weave_count[seat_index]);
		if (this._cur_banker == seat_index)
			wFanShu += ((all_hu_xi - 34)) / 10;
		else
			wFanShu += ((all_hu_xi - 33)) / 10;
		if (this.getRuleValue(GameConstants.GAME_RULE_DZ_3_FENG_DING) == 1 && wFanShu > 3)
			wFanShu = 3;
		...
		float lChiHuScore = 0;
		if (this.getRuleValue(GameConstants.GAME_RULE_DZ_TTF) == 1)
			lChiHuScore = 1 + wFanShu;
		else if (this.getRuleValue(GameConstants.GAME_RULE_DZ_GGF) == 1)
			lChiHuScore = (float) Math.pow(2.0, wFanShu);
		////////////////////////////////////////////////////// 自摸 算分
		if (zimo) {
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				if (i == seat_index) {
	...
	public boolean operate_player_cards(int seat_index, int card_count, int cards[], int weave_count, WeaveItem weaveitems[]) {
		...
		roomResponse.addDouliuzi(this.caculate_mao_shu(seat_index)); // 卯数
		roomResponse.addEspecialShowCards(this.caculate_tuo_shu(seat_index));// 砣数
		// 手牌--将自己的手牌数据发给自己
	...
	public boolean operate_player_xiang_gong_flag(int seat_index, boolean is_ting) {
		...
		boolean is_yang = false;
		if (this._is_system_bao_zi[seat_index] == true)
			is_yang = true;
		if (this._is_yang[seat_index] == true)
			is_yang = true;
		roomResponse.setIsTing(is_yang); // true 显示仰
		boolean is_bao_zi = false;
		if (this._is_bao_zi[seat_index] == true)
			is_bao_zi = true;

		roomResponse.setIsXiangGong(is_bao_zi); // true 显示包子
		if (GRR != null)
			GRR.add_room_response(roomResponse);
		this.send_response_to_room(roomResponse);
		return true;
	...
	public boolean send_play_data(int seat_index) {
		...
		int hand_cards[] = new int[GameConstants.DZ_MAX_CP_COUNT];
	...
	public void runnable_add_discard(int seat_index, int card_count, int card_data[], boolean send_client) {
		...
		int index = 0;
		for (int i = 0; i < card_count; i++) {
			GRR._discard_count[index]++;

			GRR._discard_cards[index][GRR._discard_count[index] - 1] = card_data[i];
		}
		if (send_client == true) {
	...
	public boolean display_card(int seat_index, int card, boolean is_out_card) {
		...
		if (is_out_card == true) {
			...
			if (_ti_mul_long[seat_index] == 0 && is_bao_zi(seat_index)) {
				_is_system_bao_zi[seat_index] = true;
				if (is_game_end() == false)
					return true;
				operate_player_xiang_gong_flag(seat_index, true);
				operate_effect_action(seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { GameConstants.DZ_WIK_YANG }, 1,
						GameConstants.INVALID_SEAT);
			}
			if (get_is_kou_player(seat_index) == false) {
				...
				_last_player = next_player;
				_last_card = 0;
				exe_dispatch_card(next_player, GameConstants.DZ_WIK_NULL, 1000);
			...
			int hand_card_count = _logic.switch_to_cards_data(GRR._cards_index[seat_index], cards);
			operate_player_cards(seat_index, hand_card_count, cards, GRR._weave_count[seat_index], GRR._weave_items[seat_index]);
			exe_chuli_first_card(seat_index, GameConstants.WIK_NULL, 500);
	...
	public int getTablePlayerNumber() {
		...
		return GameConstants.DZ_GAME_PLAYER_THREE;
