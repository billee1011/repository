...
import com.cai.game.mj.fujian.pc.Table_PC;
...
public class HandlerGang_PC extends MJHandlerGang<Table_PC> {
	...
	@Override
	public boolean exe_gang(Table_PC table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			/*if (table.has_rule(GameConstants_PC.GAME_RULE_FAN_BEI_AN_GANG)) {
				score *= 2;
			}*/
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
			...
			int score = GameConstants.CELL_SCORE;
			/*if (table.has_rule(GameConstants_PC.GAME_RULE_FAN_BEI_DIAN_GANG)) {
				score *= 2;
			}*/
	...
	public boolean handler_operate_card(Table_PC table, int seat_index, int operate_code, int operate_card) {
		...
		if (jie_pao_count > 0) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				//抢杠胡算自摸
				table.process_chi_hu_player_score(i, _seat_index, operate_card, 2);
	...
	public boolean handler_player_be_in_room(Table_PC table, int seat_index) {
		...
		for (int i = 0; i < GameConstants.MAX_COUNT; i++) {
			if (hand_cards[i] == GameConstants_PC.HZ_MAGIC_CARD) {
				hand_cards[i] += GameConstants_PC.CARD_ESPECIAL_TYPE_LAI_ZI;
			}
