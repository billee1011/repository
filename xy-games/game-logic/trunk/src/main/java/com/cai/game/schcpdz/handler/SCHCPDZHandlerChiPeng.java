package com.cai.game.schcpdz.handler;
...
public class SCHCPDZHandlerChiPeng<T extends SCHCPDZTable> extends SCHCPDZHandler<T> {
	...
	public boolean handler_player_out_card(T table, int seat_index, int card) {
		...
		if (table._logic.is_valid_card(card) == false) {
			table.log_error("SCHCPHandlerChiPeng出牌,牌型出错");
			return false;
		}

		// 效验参数
		if (seat_index != _seat_index) {
			table.log_error("SCHCPHandlerChiPeng出牌,没到出牌");
			return false;
		}
		if(table._playerStatus[_seat_index].get_status() != GameConstants.Player_Status_OUT_CARD)
		{
			table.log_error("状态不对不能出牌SCHCPHandlerChiPeng");
			return false;
		}
		boolean flag = false;
		for(int i = 0; i< GameConstants.MAX_CP_INDEX;i++)
		{
			if(table.GRR._must_out_index[seat_index][i] != 0)
			{
				flag = true;
				break;
			}
		}
//		if(table.GRR._must_out_index[seat_index][table._logic.switch_to_card_index(card)] == 0&&flag == true)
//		{
//			// 刷新手牌
...
//		}
		if(table.GRR._cards_index[_seat_index][table._logic.switch_to_card_index(card)] >= 3)
		{
			...
			table.log_error(_seat_index+"出牌出错 SCHCPHandlerChiPeng "+_seat_index);
			return true;
		}
		// 删除扑克
		if (table._logic.remove_card_by_index(table.GRR._cards_index[_seat_index], card) == false) {
			table.log_error("出牌删除出错SCHCPHandlerChiPeng");
			return false;
		}
