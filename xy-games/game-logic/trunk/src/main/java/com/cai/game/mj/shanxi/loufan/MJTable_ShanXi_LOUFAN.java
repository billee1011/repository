package com.cai.game.mj.shanxi.loufan;
...
public class MJTable_ShanXi_LOUFAN extends AbstractMJTable {
	...
	protected MJHandlerOutCardBaoTing_ShanXi_LOUFAN _handler_bao_ting;

	public MJTable_ShanXi_LOUFAN() {
		super(MJType.GAME_TYPE_MJ_SXLF);
	}
	...
	protected void onInitTable() {
		_handler_chi_peng = new MJHandlerChiPeng_ShanXi_LOUFAN();
		_handler_dispath_card = new MJHandlerDispatchCard_ShanXi_LOUFAN();
		_handler_gang = new MJHandlerGang_ShanXi_LOUFAN();
		_handler_out_card_operate = new MJHandlerOutCardOperate_ShanXi_LOUFAN();
		_handler_bao_ting = new MJHandlerOutCardBaoTing_ShanXi_LOUFAN();

		SysParamModel sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(374).get(12000);
		delay = 260;
	...
	public int getTablePlayerNumber() {
		if (this.has_rule(GameConstants_SXLF.GAME_RULE_PLAYER_NUM_2)) {
			return 2;
		} else if (this.has_rule(GameConstants_SXLF.GAME_RULE_PLAYER_NUM_3)) {
			return 3;
		} else if (this.has_rule(GameConstants_SXLF.GAME_RULE_PLAYER_NUM_4)) {
			return 4;
		} else {
			return 4;
		}
	...
	public boolean exe_dispatch_card(final int seat_index, final int type, final int card, int delay) {
		if (delay > 0) {
			MJTable_ShanXi_LOUFAN mjTable_LUHE = this;
	...
	public int analyse_gang_exclude_magic_card(int cards_index[], int card, WeaveItem WeaveItem[], int cbWeaveCount,
			GangCardResult gangCardResult, int cards_abandoned_gang[], int feng_gang_count, int seat_index) {
		// 设置变量,
	...
	public boolean is_ting_card(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index) {
		...
		for (int i = 0; i < GameConstants.MAX_ZI_FENG; i++) {
			...
			if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card(cbCardIndexTemp, weaveItem, cbWeaveCount, cbCurrentCard,
					chr, GameConstants_SXLF.HU_CARD_TYPE_ZI_MO, seat_index, false))
				return true;
		}
		return false;
	}
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (playerStatus.is_chi_hu_round() && this._playerStatus[i].is_bao_ting()) {
				...
				action = analyse_chi_hu_card(GRR._cards_index[i], GRR._weave_items[i], cbWeaveCount, card, chr,
						GameConstants_SXLF.HU_CARD_TYPE_QIANGGANG, i, false);

				// 结果判断
				if (action != 0) {
					if (this.has_rule(GameConstants_SXLF.GAME_RULE_SHANXI_NENG_HU_BI_HU)) {
						_playerStatus[i].add_action(GameConstants_SXJY.WIK_HIDE_ACTION);
					}
					_playerStatus[i].add_action(GameConstants.WIK_CHI_HU);
					_playerStatus[i].add_chi_hu(card, seat_index);// 吃胡的组合
					// chr.opr_or_xt(GameConstants_SXLF.CHR_TYPE_QIANG_GANG,false);//
					// 抢杠胡
					bAroseAction = true;
				}
			}
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this._playerStatus[i].is_bao_ting()) {
				...
				if (GRR._left_card_count >= left_card) {
					if (_playerStatus[i].is_chi_hu_round()) {
						...
						if (can_hu_this_card) {
							...
							// 非风牌大于5的点数才能点炮
							if (card_value > 5 && card_color < 3) {
								action = analyse_chi_hu_card(GRR._cards_index[i], GRR._weave_items[i], cbWeaveCount,
										card, chr, GameConstants_SXLF.HU_CARD_TYPE_JIE_PAO, i, false);
							}
							// 风牌不作点数的限制
							if (card_color >= 3) {
								action = analyse_chi_hu_card(GRR._cards_index[i], GRR._weave_items[i], cbWeaveCount,
				...
				if (GRR._left_card_count > 0) {
					...
					// 碰牌判断
					if (this.has_rule(GameConstants_SXLF.GAME_RULE_SHANXI_LAO_PAI)) {
	...
	public int analyse_chi_hu_card(int[] cards_index, WeaveItem[] weaveItems, int weave_count, int cur_card,
		...
		if (check_is_feng_yi_se != GameConstants.WIK_NULL
				&& check_is_feng_yi_se == GameConstants_SXLF.CHR_HUNAN_FENG_YI_SE) {
			has_feng_yi_se = true;
		}
		// 判断是不是一条龙
		...
		// 判断是否是双豪华七对
		if (check_qi_xiao_dui != GameConstants.WIK_NULL && check_qi_xiao_dui == GameConstants_SXLF.CHR_HHQXD) {
			can_win = true;
		...
		if (card_value >= 3 && card_value <= 5 && card_color < 3) { // 3,4,5只能自摸的,
			if (card_type == GameConstants_SXLF.HU_CARD_TYPE_ZI_MO) {
				chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_TYPE_ZI_MO, need_to_multiply); // 自摸
			}
		} else {
			if (card_type == GameConstants_SXLF.HU_CARD_TYPE_ZI_MO) {
				chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_TYPE_ZI_MO, need_to_multiply); // 自摸
			} else if (card_type == GameConstants_SXLF.HU_CARD_TYPE_JIE_PAO) {
				chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_TYPE_DIAN_PAO, need_to_multiply); // 点炮
			} else if (card_type == GameConstants_SXLF.HU_CARD_TYPE_QIANG_GANG_HU) {
				chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_TYPE_QIANG_GANG, need_to_multiply); // 抢杠胡
			}
		}
		if (has_qi_xiao_dui) { // 七小对
			chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_QI_XIAO_DUI, need_to_multiply);
		}
		if (has_qing_yi_se) { // 清一色
			chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_QING_YI_SE, need_to_multiply);
		}
		if (has_yi_tiao_long) { // 一条龙
			chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_YI_TIAO_LONG, need_to_multiply);
		}
		if (has_feng_yi_se) { // 风一色
			chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_HUNAN_FENG_YI_SE, need_to_multiply);
		}
		if (has_hao_hua_qi_xiao_dui) { // 豪华七对
			chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_HHQXD, need_to_multiply);
		}
		if (has_qing_yi_se_qi_xiao_dui) { // 清一色七对
			chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_QYS_QXD, need_to_multiply);
		}
		if (has_qing_yi_se_yi_tiao_long) { // 清一色一条龙
			chiHuRight.opr_or_xt(GameConstants_SXLF.CHR_QYS_YTL, need_to_multiply);
		}
		return cbChiHuKind;
	}
	...
	public void process_chi_hu_player_score(int seat_index, int provide_index, int operate_card, boolean zimo,
		...
		if (this.has_rule(GameConstants_SXLF.GAME_RULE_SHANXI_QYS_YTL_QXD_FH_JF)) {
			if (!chr.opr_and(GameConstants_SXLF.CHR_QING_YI_SE).is_empty()) {
				is_twice_add_score_one = true;
				extraScore_one = 10;
			}

			if (!chr.opr_and(GameConstants_SXLF.CHR_YI_TIAO_LONG).is_empty()) {
				is_twice_add_score_one = true;
				extraScore_one = 10;
			}

			if (!chr.opr_and(GameConstants_SXLF.CHR_QI_XIAO_DUI).is_empty()) {
				is_twice_add_score_one = true;
				extraScore_one = 10;
			}
		...
		if (this.has_rule(GameConstants_SXLF.GAME_RULE_SHANXI_QYSYTL_QYSQXD_HHQXD)) {
			if (!chr.opr_and(GameConstants_SXLF.CHR_QYS_YTL).is_empty()) {
				is_twice_add_score_two = true;
				extraScore_two = 20;
			}

			if (!chr.opr_and(GameConstants_SXLF.CHR_QYS_QXD).is_empty()) {
				is_twice_add_score_two = true;
				extraScore_two = 20;
			}

			if (!chr.opr_and(GameConstants_SXLF.CHR_HHQXD).is_empty()) {
				is_twice_add_score_two = true;
				extraScore_two = 20;
			}

			if (!chr.opr_and(GameConstants_SXLF.CHR_HUNAN_FENG_YI_SE).is_empty()) {
				is_twice_add_score_two = true;
				extraScore_two = 20;
			}
	...
	protected void countCardType(ChiHuRight chiHuRight, int seat_index) {
		try {
			if (!(chiHuRight.opr_and(GameConstants_SXLF.CHR_SHI_SAN_YAO)).is_empty()) { // 十三幺
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.sx_shisanyao, "",
						_game_type_index, 0l, this.getRoom_id());
			}

			if (!(chiHuRight.opr_and(GameConstants_SXLF.CHR_QING_YI_SE)).is_empty()) { // 清一色
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.sx_qingyise, "",
						_game_type_index, 0l, this.getRoom_id());
			}

			if (!(chiHuRight.opr_and(GameConstants_SXLF.CHR_QI_XIAO_DUI)).is_empty()) { // 七小对
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.sx_qixiaodui, "",
						_game_type_index, 0l, this.getRoom_id());
			}

			if (!(chiHuRight.opr_and(GameConstants_SXLF.CHR_YI_TIAO_LONG)).is_empty()) {
				// 一条龙
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.sx_yitiaolong, "",
						_game_type_index, 0l, this.getRoom_id());
			}
						...
						if (no_bao_ting_dian_pao[i]) {// **********如果未报听情况下点炮，点炮者的杠做废*************
							...
							if (GRR._weave_items[i][j].type == GameConstants.GANG_TYPE_JIE_GANG) {
								...
								if (GameConstants.Game_End_DRAW != reason) {
									// 如果勾选了点杠包杠玩法
									if (this.has_rule(GameConstants_SXLF.GAME_RULE_SHANXI_DIAN_GANG_BAO_GANG)) {
										if (GRR._weave_items[i][j].is_lao_gang) { // 报听点杠三家扣分;
											for (int k = 0; k < this.getTablePlayerNumber(); k++) {
												if (i == k)
													continue;
												this.GRR._gang_score[i].scores[cbGangIndex][k] -= gang_fen
														* GameConstants.CELL_SCORE;
												this.GRR._gang_score[i].scores[cbGangIndex][i] += gang_fen
														* GameConstants.CELL_SCORE;
											}
										} else {// 不报听点杠包杠（包三家的杠）
											this.GRR._gang_score[i].scores[cbGangIndex][i] += 3 * gang_fen
													* GameConstants.CELL_SCORE;
											this.GRR._gang_score[i].scores[cbGangIndex][GRR._weave_items[i][j].provide_player] -= 3
							...
							if (GRR._weave_items[i][j].type == GameConstants.GANG_TYPE_AN_GANG) {
								if (GameConstants.Game_End_DRAW != reason) {
									for (int k = 0; k < this.getTablePlayerNumber(); k++) {
										...
										// 暗杠每人2分
										this.GRR._gang_score[i].scores[cbGangIndex][k] -= gang_fen * 2
												* GameConstants.CELL_SCORE;
										this.GRR._gang_score[i].scores[cbGangIndex][i] += gang_fen * 2
												* GameConstants.CELL_SCORE;
									}
								}
		...
		for (int player = 0; player < this.getTablePlayerNumber(); player++) {
			...
			if (GRR._chi_hu_rights[player].is_valid()) {
				for (int typeIndex = 0; typeIndex < chrTypes; typeIndex++) {
					...
					if (type == GameConstants_SXLF.CHR_TYPE_ZI_MO) {
						gameDesc.append(" 自摸");
					}
					if (type == GameConstants_SXLF.CHR_TYPE_DIAN_PAO) {
						gameDesc.append(" 接炮");
					}
					if (type == GameConstants_SXLF.CHR_QI_XIAO_DUI) {
						if (map.containsKey((long) GameConstants_SXLF.CHR_QI_XIAO_DUI)) {
							tmp_count = map.get((long) GameConstants_SXLF.CHR_QI_XIAO_DUI);
							gameDesc.append(" 七小对x" + tmp_count);
						} else {
							gameDesc.append(" 七小对");
						}
					}
					if (type == GameConstants_SXLF.CHR_QING_YI_SE) {
						if (map.containsKey((long) GameConstants_SXLF.CHR_QING_YI_SE)) {
							tmp_count = map.get((long) GameConstants_SXLF.CHR_QING_YI_SE);
							gameDesc.append(" 清一色x" + tmp_count);
						} else {
							gameDesc.append(" 清一色");
						}
					}
					if (type == GameConstants_SXLF.CHR_YI_TIAO_LONG) {
						if (map.containsKey((long) GameConstants_SXLF.CHR_YI_TIAO_LONG)) {
							tmp_count = map.get((long) GameConstants_SXLF.CHR_YI_TIAO_LONG);
					...
					if (type == GameConstants_SXLF.CHR_QYS_YTL) {
						if (map.containsKey((long) GameConstants_SXLF.CHR_QYS_YTL)) {
							tmp_count = map.get((long) GameConstants_SXLF.CHR_QYS_YTL);
					...
					if (type == GameConstants_SXLF.CHR_QYS_QXD) {
						if (map.containsKey((long) GameConstants_SXLF.CHR_QYS_QXD)) {
							tmp_count = map.get((long) GameConstants_SXLF.CHR_QYS_QXD);
					...
					if (type == GameConstants_SXLF.CHR_HHQXD) {
						if (map.containsKey((long) GameConstants_SXLF.CHR_HHQXD)) {
							tmp_count = map.get((long) GameConstants_SXLF.CHR_HHQXD);
					...
					if (type == GameConstants_SXLF.CHR_HUNAN_FENG_YI_SE) {
						if (map.containsKey((long) GameConstants_SXLF.CHR_HUNAN_FENG_YI_SE)) {
							tmp_count = map.get((long) GameConstants_SXLF.CHR_HUNAN_FENG_YI_SE);
					...
					if (type == GameConstants_SXLF.CHR_TYPE_QIANG_GANG) {
						gameDesc.append(" 抢杠胡");
					}
				}
			} else if (!GRR._chi_hu_rights[player].opr_and(GameConstants_SXLF.CHR_FANG_PAO).is_empty()) {
				gameDesc.append(" 放炮");
			}
	...
	public boolean check_gang_can_hu_out_card(int seat_index, int card) {
		...
		// 检查听牌
		int hu_card_count = 0;
		hu_card_count = this.get_ting_card(hu_cards, GRR._cards_index[seat_index], GRR._weave_items[seat_index],
	...
	public void test_cards() {
		int[] cards_of_player0 = new int[] { 0x13, 0x14, 0x15, 0x18, 0x25, 0x25, 0x25, 0x26, 0x26, 0x26, 0x21, 0x22,
				0x23 };
		int[] cards_of_player1 = new int[] { 0x32, 0x32, 0x32, 0x01, 0x02, 0x03, 0x07, 0x11, 0x12, 0x13, 0x14, 0x15,
				0x16 };
		int[] cards_of_player2 = new int[] { 0x01, 0x01, 0x02, 0x02, 0x03, 0x21, 0x21, 0x21, 0x12, 0x12, 0x12, 0x16,
				0x17 };
		int[] cards_of_player3 = new int[] { 0x04, 0x04, 0x04, 0x06, 0x06, 0x06, 0x09, 0x09, 0x13, 0x15, 0x16, 0x17,
				0x18 };
	...
	private boolean checkWangGuiWei(int[] cbCardIndexTemp, ChiHuRight chiHuRight, int curCard, int magicCount) {
		...
		for (Integer color : sameColorMap.keySet()) {
			...
			if (haveMagicCount >= tuple.getRight()) {
				...
				boolean flag = eyeRuleByFuzi(
						sameColorMap.get(color).toArray(new Integer[sameColorMap.get(color).size()]), haveMagicCount,
						color, chiHuRight, curCard, tuple, magicCount);
				if (flag) {
					if (tuple.getLeft() > 1) {
	 ...
	 * @return
	 */
	private boolean eyeRuleByFuzi(Integer[] cardDatas, int magicCount, int color, ChiHuRight chiHuRight, int curCard,
			Tuple<Integer, Integer> t, int handMagicCount) {
	...
	public static Tuple<Integer, Integer> ruleByFuzi(Integer[] cardDatas, int color) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (used[i] == 1) {
				...
				if (i + 2 < length && card == cardDatas[i + 1] && card == cardDatas[i + 2] && used[i] == 0
						&& used[i + 1] == 0 && used[i + 2] == 0) {
					needTemp = 0;
	...
	public int is_qi_xiao_dui(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int cur_card) {
		...
		if (nGenCount > 0) {
			...
			if (this.has_rule(GameConstants_SXLF.GAME_RULE_SHANXI_QYSYTL_QYSQXD_HHQXD)) {
				...
				return GameConstants_SXLF.CHR_HHQXD;// 豪华七对
			} else {
