...
import com.cai.game.pdk.PDKGameLogic_SC;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PDK_SR_Table_SC extends PDKTable {

	public int type_data[][];
	public int type_count[];

	public PDK_SR_Table_SC() {
		_logic = new PDKGameLogic_SC();
	}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		type_data = new int[getTablePlayerNumber()][get_hand_card_count_max()];
		type_count = new int[getTablePlayerNumber()];
		...
		_handler_out_card_operate = new PDKHandlerOutCardOperate_SR_SC();
		_bomb_cell_score = 10;
		if (this.matchId != 0) {
			_bomb_cell_score = 4;
		}
	...
	protected boolean on_handler_game_start() {
		...
		//
		if (!this.has_rule(GameConstants.GAME_RULE_PLAYER_FOUR)) {
			_repertory_card = new int[GameConstants.CARD_COUNT_PDK_SR_SC_ER];
			shuffle(_repertory_card, GameConstants.CARD_DATA_PDK_ER_SC);
		} else {
			_repertory_card = new int[GameConstants.CARD_COUNT_PDK_SR_SC];
			shuffle(_repertory_card, GameConstants.CARD_DATA_PDK_SR_SC);
		}
		...
		this._logic.get_card_type_award(this, type_data, type_count);
	...
	private boolean game_start_pkd() {
		...
		// 庄家每局确认
		if (this.has_rule(GameConstants.GAME_RULE_SHOU_JU_HEITAO_WU)
				|| has_rule(GameConstants.GAME_RULE_MIN_CARD_SHOU_CHU) || this._cur_round == 1) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				if (i == play_index) {
					for (int j = 0; j < GRR._card_count[play_index]; j++) {
						if (this.has_rule(GameConstants.GAME_RULE_FANG_ZUO_BI_SC) && play_index != _current_player) {
	...
	public int find_banker() {
		...
		int seat = -1;
		int min_card = GameConstants.INVALID_CARD;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			if (!has_rule(GameConstants.GAME_RULE_PLAYER_TWO)) {
				for (int j = 0; j < GRR._card_count[i]; j++) {
					if (GRR._cards_data[i][j] == _first_out_card)
						return i;
				}
			} else {
				if (min_card == GameConstants.INVALID_CARD) {
					min_card = GRR._cards_data[i][GRR._card_count[i] - 1];
					seat = i;
				} else if (this._logic.GetCardLogicValue(GRR._cards_data[i][GRR._card_count[i] - 1]) < _logic
						.GetCardLogicValue(min_card)) {
					min_card = GRR._cards_data[i][GRR._card_count[i] - 1];
					seat = i;
				} else if (_logic.GetCardLogicValue(GRR._cards_data[i][GRR._card_count[i] - 1]) == _logic
						.GetCardLogicValue(min_card)) {
					if (GRR._cards_data[i][GRR._card_count[i] - 1] < min_card) {
						min_card = GRR._cards_data[i][GRR._card_count[i] - 1];
						seat = i;
					}
				}
	...
	private void test_cards() {
		...
		int cards[] = new int[] { 0x05, 0x15, 0x25, 0x35, 0x06, 0x16, 0x26, 0x36, 0x07, 0x17, 0x37, 0x27, 0x08, 0x18,
				0x28, 0x38, 0x09, 0x1c, 0x29, 0x31, 0x0a, 0x1a, 0x2a, 0x3a, 0x1b, 0x2b, 0x0b, 0x3b, 0x0c, 0x19, 0x2c,
				0x3c, 0x0d, 0x1d, 0x2d, 0x3d, 0x01, 0x11, 0x21, 0x39 };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public void cal_score_pdk_jd(int end_score[], int awrd_score[]) {
		...
		/*
		 * if(chun_tian_num == 3){ for(int
		 * i=0;i<this.getTablePlayerNumber();i++){ end_score[i]*=2; } }
		 */
		// if (chun_tian_num == this.getTablePlayerNumber() - 1 &&
		// this.getTablePlayerNumber() != 2) {
		// for (int i = 0; i < this.getTablePlayerNumber(); i++) {
		// end_score[i] = end_score[i] / 2;
		// end_score[i] *= 3;
		// }
		// for (int i = 0; i < this.getTablePlayerNumber(); i++) {
		// if (i == win_player) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			if (!(GRR._chi_hu_rights[i].opr_and(GameConstants.CHR_QUAN_DA_PDK)).is_empty()) {
				for (int j = 0; j < this.getTablePlayerNumber(); j++) {
					...
					awrd_score[i] += extraScore;
					awrd_score[j] -= extraScore;
					end_score[j] -= extraScore;
					end_score[i] += extraScore;
				}
			}
			...
			if (this.type_count[i] > 0) {
				for (int j = 0; j < this.getTablePlayerNumber(); j++) {
					if (i == j) {
						continue;
					}
					awrd_score[i] += extraScore * type_count[i];
					awrd_score[j] -= extraScore * type_count[i];
					end_score[j] -= extraScore * type_count[i];
					end_score[i] += extraScore * type_count[i];
				}
			}
		}
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean handler_game_finish_pdk_jd(int seat_index, int reason) {
		...
		if (reason == GameConstants.Game_End_NORMAL) {
			cal_score_pdk_jd(end_score, awrd_score);
		}
		...
		if (reason == GameConstants.Game_End_NORMAL) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				/*
				 * if(GRR != null && i != GRR._banker_player &&
				 * this._out_card_times[i]==0){ if(this._bao_pei_palyer ==
				 * GameConstants.INVALID_SEAT){
				 * game_end_pdk.addChunTianPlayer(i); }else{
				...
				for (int j = 0; j < type_count[i]; j++) {
					cards_card.addItem(type_data[i][j]);
				}
				game_end_pdk.addCardsDataMingtang(cards_card);
			}
	...
	public int get_hand_card_count_max() {
		...
		return GameConstants.MAX_PDK_COUNT_SR_SC;
	}
