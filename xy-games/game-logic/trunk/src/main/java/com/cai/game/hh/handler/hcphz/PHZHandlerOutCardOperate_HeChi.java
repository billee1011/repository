...
public class PHZHandlerOutCardOperate_HeChi extends HHHandlerOutCardOperate<HeChiHHTable> {
	@Override
	public void exe(HeChiHHTable table) {
		...
		// 显示出牌
		table.operate_out_card(_out_card_player, 1, new int[] { _out_card_data }, GameConstants_HeChi.OUT_CARD_TYPE_MID,
		...
		int ti_pao = GameConstants_HeChi.WIK_NULL;
		for (int i = 0; i < GameConstants_HeChi.GAME_PLAYER_HH; i++) {
			if (i == _out_card_player)
				continue;
			int pao_type[] = new int[1];
			ti_pao = table.estimate_player_respond_phz(i, _out_card_player, _out_card_data, pao_type, false);
			if (ti_pao != GameConstants_HeChi.WIK_NULL) {
		...
		// 点炮胡，在此处处理
		if ((table.has_rule(GameConstants_HeChi.GAME_RULE_PAO_HU_MORE) || table.has_rule(GameConstants_HeChi.GAME_RULE_PAO_HU)) && hasChiHu(table)) {
			return;
		}

		// 玩家出牌 响应判断,是否有吃,碰,胡 swe
		boolean bAroseAction = table.estimate_player_out_card_respond_PHZ(_out_card_player, _out_card_data, false);//
		// 如果没有需要操作的玩家，派发扑克
		if (bAroseAction == false) {
			...
			//
			table.operate_out_card(_out_card_player, 0, null, GameConstants_HeChi.OUT_CARD_TYPE_MID, GameConstants_HeChi.INVALID_SEAT);
			...
			table.exe_dispatch_card(next_player, GameConstants_HeChi.WIK_NULL, dispatch_time);
		} else {
			// 等待别人操作这张牌
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	...
	private boolean hasChiHu(HeChiHHTable table) {
		int i = 0;
		boolean show = false;
		do {
			...
			int hu_xi = table.analyse_chi_hu_card(table.GRR._cards_index[operateIndex], table.GRR._weave_items[operateIndex],
					table.GRR._weave_count[operateIndex], operateIndex, _out_card_player, _out_card_data, chr, _type, hu_xi_chi, true);// 自

			if (hu_xi != GameConstants_HeChi.WIK_NULL) {
				show = true;
				table._cur_banker = operateIndex;

				table._shang_zhuang_player = operateIndex;
				table.set_niao_card(operateIndex, GameConstants_HeChi.INVALID_VALUE, true); // 番醒
				...
				if (!table.has_rule(GameConstants_HeChi.GAME_RULE_PAO_HU_MORE)) {
					break;
				}
			}
		...
		if (show) {
			...
			if (table.GRR._chi_hu_rights[_out_card_player].type_count > 2) {
				delay += table.GRR._chi_hu_rights[_out_card_player].type_count - 2;
			}
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._cur_banker, GameConstants_HeChi.Game_End_NORMAL), delay,
					TimeUnit.SECONDS);
		}
		return false;
	}
	...
	public boolean handler_operate_card(HeChiHHTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		if ((operate_code != GameConstants_HeChi.WIK_NULL) && playerStatus.has_action_by_code(operate_code) == false)// 没有这个操作动作
		{
		...
		// if (operate_code == GameConstants_HeChi.WIK_CHI_HU) {
		// table.GRR._chi_hu_rights[seat_index].set_valid(true);// 胡牌生效
		// // 效果
		// //
		// table.process_chi_hu_player_operate_cs(seat_index,operate_card,1,false);
		//
		// } else if (operate_code == GameConstants_HeChi.WIK_NULL) {
		...
		case GameConstants_HeChi.WIK_LEFT: // 上牌操作
		{
			// 删除扑克
			...
			table.exe_chi_peng(target_player, _out_card_player, target_action, target_card, GameConstants_HeChi.CHI_PENG_TYPE_OUT_CARD,
					target_lou_code);

			return true;
		}
		case GameConstants_HeChi.WIK_RIGHT: // 上牌操作
		{
			// 删除扑克
			int cbRemoveCard[] = new int[] { target_card - 1, target_card - 2 };
		...
		case GameConstants_HeChi.WIK_CENTER: // 上牌操作
		{
			// 删除扑克
		...
		case GameConstants_HeChi.WIK_XXD:// 吃小
		{
			// 删除扑克
		...
		case GameConstants_HeChi.WIK_DDX:// 吃大
		{
			// 删除扑克
			// 删除扑克
			int target_card_color = table._logic.get_card_color(target_card);
		...
		case GameConstants_HeChi.WIK_EQS:// 吃二七十
		{
			// 删除扑克
		...
		case GameConstants_HeChi.WIK_PENG: // 碰牌操作
		{
			// 删除扑克
			int cbRemoveCard[] = new int[] { target_card, target_card };
			if (!table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2)) {
		...
		case GameConstants_HeChi.WIK_NULL: {
			...
			// 发牌
			table._last_player = _current_player;
			table.exe_dispatch_card(_current_player, GameConstants_HeChi.WIK_NULL, 500);

			return true;
		}
		case GameConstants_HeChi.WIK_CHI_HU: // 胡
		{
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == target_player) {
			...
			if (table.has_rule(GameConstants_HeChi.GAME_RULE_LIXIANG_FLS_ZHUANG)) {// 轮装
	...
	public boolean handler_player_be_in_room(HeChiHHTable table, int seat_index) {
		...
		for (int j = 0; j < hand_card_count; j++) {
			if (table._logic.is_magic_card(cards[j])) {
				cards[j] += GameConstants_HeChi.CARD_ESPECIAL_TYPE_HUN;
			}
		}
		...
		if (table._logic.is_magic_card(_out_card_data)) {
			real_card += GameConstants_HeChi.CARD_ESPECIAL_TYPE_HUN;
		}
		// 出牌
		table.operate_out_card(_out_card_player, 1, new int[] { real_card }, GameConstants_HeChi.OUT_CARD_TYPE_MID, seat_index);
