...
import com.cai.game.ddz.DDZGameLogic_JX;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class DDZ_JX_Table extends DDZTable {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		// 初始化基础牌局handle
		_handler_out_card_operate = new DDZHandlerOutCardOperate_JX();
		_handler_call_banker = new DDZHandlerCallBanker_JX();
		_handler_add_times = new DDZHandlerAddtimes_JX();
	...
	public int get_hand_card_count_max() {
		if (this.has_rule(GameConstants.GAME_RULE_JX_DDZ_PLAYER_THREE)) {
			return GameConstants.JXDDZ_MAX_COUT;
		}
		return GameConstants.JXDDZ_MAX_COUT - 4;
	}

	public int get_no_banker_card_count() {
		return get_hand_card_count_max() - GameConstants.JXDDZ_DI_PAI_COUNT_JD;
	}
	...
	private void test_cards() {
		int cards[] = new int[] { 43, 39, 58, 37, 21, 29, 54, 27, 5, 18, 36, 78, 40, 61, 1, 12, 26, 53, 60, 3, 44, 79,
				94, 11, 49, 35, 56, 19, 10, 7, 4, 28, 42, 57, 23, 22, 41, 6, 52, 51, 33, 17, 20, 55, 13, 2, 25, 8, 38,
				24, 59, 9, 45 };
		...
		int di_cards[] = new int[] { 45, 17, 44, 2, 57 };
		for (int i = 0; i < _di_pai_card_count; i++) {
	...
	public void cal_score_ddz_jd(int end_score[], int chuntian[]) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			if (win_player == this.GRR._banker_player) {
				if (i != win_player) {
					if (this._user_times[i] > 5) {
						end_score[i] = (int) (-Math.pow(2, 5) * this.game_cell + (_user_times[i] - 5) * 6);
					} else {
	...
	public String get_boom_difen_des(int seat_index) {
		...
		if (seat_index == this.GRR._banker_player) {
			...
			des += "倍数：" + (int) Math.pow(2, this._user_times[seat_index]) + "倍;";
		}
		return des;
	}
