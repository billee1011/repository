...
public class MJHandlerDispatchCard_PC extends MJHandlerDispatchCard<Table_PC> {
	...
	// 发牌
	@Override
	public void exe(Table_PC table) {
		// 用户状态 清除
		...
		// 当牌堆剩下的牌只够飞苍蝇时荒庄结束
		if (table.GRR._left_card_count == table.get_ma_num()) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		int card_type = GameConstants_PC.HU_CARD_TYPE_ZI_MO;
		int action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
				table.GRR._weave_count[_seat_index], _send_card_data, chr, card_type, _seat_index);

		if (action != GameConstants_PC.WIK_NULL) {
			curPlayerStatus.add_action(GameConstants_PC.WIK_ZI_MO);
		...
		} else if (_send_card_data == GameConstants_PC.HZ_MAGIC_CARD) {
			show_send_card += GameConstants_PC.CARD_ESPECIAL_TYPE_LAI_ZI;
		}
		table.operate_player_get_card(_seat_index, 1, new int[] { show_send_card }, GameConstants_PC.INVALID_SEAT);
		...
		if (table.GRR._left_card_count > 0) {
			...
			if (cbActionMask != GameConstants_PC.WIK_NULL) {
				curPlayerStatus.add_action(GameConstants_PC.WIK_GANG);
	...
	@Override
	public boolean handler_operate_card(Table_PC table, int seat_index, int operate_code, int operate_card) {
		PlayerStatus playerStatus = table._playerStatus[seat_index];

		if ((operate_code != GameConstants_PC.WIK_NULL) && (playerStatus.has_action_by_code(operate_code) == false)) {
		...
		if (operate_code == GameConstants_PC.WIK_NULL) {
			...
			for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
				if (GameConstants_PC.GANG_TYPE_ADD_GANG == m_gangCardResult.type[i]) {
		...
		/*	if (_type == GameConstants_PC.DispatchCard_Type_Tian_Hu && table.has_rule(GameConstants_PC.GAME_RULE_DIAN_PAO)) {
				table.GRR._chi_hu_rights[_seat_index].opr_or(GameConstants_PC.CHR_TIAN_HU);
			}
*/
			...
			table.process_chi_hu_player_score(_seat_index, _seat_index, operate_card, 1);
	...
	@Override
	public boolean handler_player_out_card(Table_PC table, int seat_index, int card) {
		...
		if (_type == GameConstants_PC.DispatchCard_Type_Tian_Hu) {
			// 出牌
			table.exe_out_card(_seat_index, card, GameConstants_PC.OutCard_Type_Di_Hu);
		} else {
			// 出牌
	...
	@Override
	public boolean handler_player_be_in_room(Table_PC table, int seat_index) {

		table.operate_mai_ma_card(seat_index, false);
		...
		if ((out_ting_count > 0) && (seat_index == _seat_index)) {
			for (int j = 0; j < hand_card_count; j++) {
				...
				if (hand_cards[j] == GameConstants_PC.HZ_MAGIC_CARD) {
					hand_cards[j] += GameConstants_PC.CARD_ESPECIAL_TYPE_LAI_ZI;
				}
			}
		}
