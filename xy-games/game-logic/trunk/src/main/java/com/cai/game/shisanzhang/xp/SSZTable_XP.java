 ...
 * 
 */
package com.cai.game.shisanzhang.xp;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class SSZTable_XP extends SSZTable {
	/**
	 * 
	 */
	public boolean _is_jetton[];
	public boolean _is_call_banker[];
	public boolean _can_call_banker[];
	...
	public SSZTable_XP() {
		super(SSZType.GAME_TYPE_SSZ_XUPU);
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_can_call_banker = new boolean[getTablePlayerNumber()];
	...
	protected boolean on_handler_game_start() {
		...
		Arrays.fill(_can_call_banker, false);
		...
		if (this.has_rule(GameConstants.GAME_RULE_XP_SSZ_DI_FEN_ONE)) {
			game_cell = 1;
		} else if (this.has_rule(GameConstants.GAME_RULE_XP_SSZ_DI_FEN_TWO)) {
			game_cell = 2;
		} else if (this.has_rule(GameConstants.GAME_RULE_XP_SSZ_DI_FEN_THREE)) {
			game_cell = 3;
		} else if (this.has_rule(GameConstants.GAME_RULE_XP_SSZ_DI_FEN_FOUR)) {
			game_cell = 4;
		}
		...
		if (this._cur_banker == GameConstants.INVALID_SEAT) {
			Arrays.fill(_can_call_banker, true);
		} else {
			_can_call_banker[this._cur_banker] = true;
		}
	...
	public boolean game_start_sszjd() {
		...
		callbanker_ssz.setCurrentPlayer(this._current_player);
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			callbanker_ssz.addCanCallBanker(_can_call_banker[i]);
		}
	...
	public boolean Send_card() {
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			...
			roomResponse.setGameStatus(this._game_status);
			// 发送数据
			Send_Card_Data.Builder gamestart_ssz = Send_Card_Data.newBuilder();
			RoomInfoSsz.Builder room_info = getRoomInfoSsz();
			gamestart_ssz.setRoomInfo(room_info);
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				gamestart_ssz.addCardCount(GRR._card_count[i]);

				gamestart_ssz.addCardsData(cards_card);
			}
			gamestart_ssz.setBankerPlayer(this.GRR._banker_player);
			gamestart_ssz.setDisplayTime(10);

			roomResponse.setCommResponse(PBUtil.toByteString(gamestart_ssz));
		...
		gamestart_ssz.setBankerPlayer(_cur_banker);
		gamestart_ssz.setDisplayTime(10);
		roomResponse.setCommResponse(PBUtil.toByteString(gamestart_ssz));
	...
	private void test_cards() {
		...
		// 0x1d, 0x3d, 0x31, 0x32};
		int cards[] = new int[] { 0x22, 0x23, 0x34, 0x08, 0x28, 0x37, 0x0C, 0x0D, 0x0A, 0x2A, 0x3B, 0x1B, 0x36 };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean handler_game_finish_sszjd(int seat_index, int reason) {
		...
		int fitst_Win[] = new int[getTablePlayerNumber()];
		int second_Win[] = new int[getTablePlayerNumber()];
		int three_Win[] = new int[getTablePlayerNumber()];
		int end_score[] = new int[this.getTablePlayerNumber()];
		int xian_win_count = 0;
		Arrays.fill(end_score, 0);
		Arrays.fill(fitst_Win, 0);
		Arrays.fill(second_Win, 0);
		Arrays.fill(three_Win, 0);
		if (GRR != null && this.GRR._banker_player != GameConstants.INVALID_SEAT && reason == GameConstants.Game_End_NORMAL) {
			...
			// 奖励分
			if (this.has_rule(GameConstants.GAME_RULE_XP_SSZ_SPECAIL_CARS)) {
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					int CardList[] = new int[13];

					if (i == GRR._banker_player) {
						for (int j = 0; j < 3; j++) {
							CardList[j] = GRR._cards_data[i][j];
							// bNextList[j]=GRR._cards_data[i][j];
						}

						int card_type = _logic.GetCardType(CardList, 3);
						if (card_type == GameConstants.SSZ_CT_THREE) {
							end_score[i] += this.game_cell * (getTablePlayerNumber() - 1);
							for (int j = 0; j < getTablePlayerNumber(); j++) {
								if (j == GRR._banker_player) {
									continue;
								}
								end_score[j] -= this.game_cell;
							}
						}
						for (int j = 0; j < 5; j++) {
							CardList[j] = GRR._cards_data[i][3 + j];
						}
						card_type = _logic.GetCardType(CardList, 5);
						if (card_type == GameConstants.SSZ_CT_FIVE_THREE_DEOUBLE || card_type == GameConstants.SSZ_CT_FIVE_FOUR_ONE
								|| card_type == GameConstants.SSZ_CT_FIVE_STRAIGHT_FLUSH
								|| card_type == GameConstants.SSZ_CT_FIVE_STRAIGHT_FLUSH_FIRST_A) {
							end_score[i] += this.game_cell * (getTablePlayerNumber() - 1);
							for (int j = 0; j < getTablePlayerNumber(); j++) {
								if (j == GRR._banker_player) {
									continue;
						...
						for (int j = 0; j < 5; j++) {
							CardList[j] = GRR._cards_data[i][8 + j];
						}
						card_type = _logic.GetCardType(CardList, 5);
						if (card_type == GameConstants.SSZ_CT_FIVE_FOUR_ONE || card_type == GameConstants.SSZ_CT_FIVE_STRAIGHT_FLUSH
								|| card_type == GameConstants.SSZ_CT_FIVE_STRAIGHT_FLUSH_FIRST_A) {
							end_score[i] += this.game_cell * (getTablePlayerNumber() - 1);
							for (int j = 0; j < getTablePlayerNumber(); j++) {
								if (j == GRR._banker_player) {
						...
						if (card_type == GameConstants.SSZ_CT_THREE) {
							...
							end_score[GRR._banker_player] -= this.game_cell;
						}
						for (int j = 0; j < 5; j++) {
							CardList[j] = GRR._cards_data[i][3 + j];
						}
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				if (i != GRR._banker_player) {
					if (fitst_Win[i] + second_Win[i] + three_Win[i] > 0) {
						end_score[i] += _jetton_score[i] + this.game_cell;
						end_score[GRR._banker_player] -= _jetton_score[i] + this.game_cell;
						xian_win_count++;
					} else {
						end_score[i] -= _jetton_score[i] + this.game_cell;
						end_score[GRR._banker_player] += _jetton_score[i] + this.game_cell;
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			game_end_ssz.addFirstScore(fitst_Win[i]);
			game_end_ssz.addSecondScore(second_Win[i]);
			game_end_ssz.addThreeScore(three_Win[i]);
		...
		if (this.has_rule(GameConstants.GAME_RULE_XP_SSZ_BANKER_TURN)) {
			_cur_banker = (_cur_banker + 1) % this.getTablePlayerNumber();
		} else {
			if (xian_win_count == this.getTablePlayerNumber() - 1) {
				_cur_banker = (_cur_banker + 1) % this.getTablePlayerNumber();
			}
		}
		Arrays.fill(_is_open_card, false);
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status) && this.get_players()[seat_index] != null) {
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				tableResponse_ssz.addCanCallBanker(_can_call_banker[i]);
	...
	public boolean handler_call_banker(int seat_index, int call_banker) {
		if ((_cur_banker != GameConstants.INVALID_SEAT && seat_index != _current_player) || _game_status != GameConstants.GS_CALL_BANKER) {
			return false;
		}
		...
		if (call_banker == 0) {
			...
			_can_call_banker[seat_index] = false;

		} else {
			this.GRR._banker_player = seat_index;
			_current_player = seat_index;
			_cur_banker = seat_index;

			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				_is_call_banker[i] = true;
				_can_call_banker[i] = false;
			}
		}
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		...
		callbanker_result_ssz.setCurrentSeatIndex(_current_player);
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			callbanker_result_ssz.addCanCallBanker(_can_call_banker[i]);
		}
		...
		if (this.GRR._banker_player == GameConstants.INVALID_SEAT) {
			GameSchedule.put(new GameFinishRunnable(getRoom_id(), this.GRR._banker_player, GameConstants.Game_End_DRAW),
					GameConstants.GAME_FINISH_DELAY, TimeUnit.SECONDS);
		}
		return true;
	}
