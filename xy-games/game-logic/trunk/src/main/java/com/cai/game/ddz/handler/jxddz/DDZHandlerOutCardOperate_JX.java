...
public class DDZHandlerOutCardOperate_JX extends DDZHandlerOutCardOperate {
	@Override
	public void exe(DDZTable table) {
		...
		if (cbCardType == GameConstants.DDZ_CT_BOMB_CARD_JX || cbCardType == GameConstants.DDZ_CT_MISSILE_CARD_JX) {
			if (table._logic.get_card_value(_out_change_cards_data[0]) == 3 && _out_card_count == 4) {
				table._times *= 8;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	...
	public int adjust_out_card_right(DDZTable table) {
		table._logic.make_change_card(_out_change_cards_data, _out_card_count);

		table._logic.sort_card_date_list(_out_change_cards_data, _out_card_count);
		int cbCardType = table._logic.GetCardType(this._out_change_cards_data, this._out_card_count,
				this._out_cards_data);
		...
		if (cbCardType == GameConstants.DDZ_CT_THREE_JX || cbCardType == GameConstants.DDZ_CT_THREE_LINE_JX_ONE) {
