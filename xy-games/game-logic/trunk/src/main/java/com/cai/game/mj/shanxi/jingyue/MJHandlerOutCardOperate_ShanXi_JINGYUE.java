...
public class MJHandlerOutCardOperate_ShanXi_JINGYUE extends MJHandlerOutCardOperate<MJTable_ShanXi_JINGYUE> {
	...
	public boolean handler_operate_card(MJTable_ShanXi_JINGYUE table, int seat_index, int operate_code,
		...
		if (GameConstants.WIK_FENG_GANG == target_action) {
			target_card = _out_card_data + GameConstants_SXJY.CARD_ESPECIAL_TYPE_GANG_NAN_FENG;
		}
		...
		case GameConstants.WIK_FENG_GANG: {
			...
			// if(operate_card>=GameConstants_SXJY.DONG_FENG_CARD+GameConstants_SXJY.CARD_ESPECIAL_TYPE_GANG_NAN_FENG){
			table.exe_gang(target_player, _out_card_player, target_card, target_action,
					GameConstants_SXJY.GANG_TYPE_MING_FENG_GANG, false, false);
			// }
			return true;
		}
		...
		case GameConstants.WIK_CHI_HU: {
			...
			table.GRR._chi_hu_rights[_out_card_player].opr_or_xt(GameConstants_SXJY.CHR_TYPE_DIAN_PAO, false);
	...
	public boolean handler_player_be_in_room(MJTable_ShanXi_JINGYUE table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < 55; j++) {
				...
				// GameConstants_SXJY.CARD_ESPECIAL_TYPE_HIDE);
				// }else{
				int_array.addItem(table.GRR._discard_cards[i][j]);
				// }
			}
