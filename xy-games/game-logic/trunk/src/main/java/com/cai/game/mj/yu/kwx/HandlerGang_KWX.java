...
public class HandlerGang_KWX extends MJHandlerGang<Table_KWX> {
	...
	@Override
	public boolean exe_gang(Table_KWX table) {
		if (_double) {
		...
		if (table.player_liang[_seat_index] == 1) {
			...
			cards = new int[GameConstants_KWX.MAX_COUNT];
	...
	public boolean handler_operate_card(Table_KWX table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants_KWX.WIK_CHI_HU) {
			...
			if (table._playerStatus[seat_index].has_chi_hu()) {
				table.pass_hu_fan[seat_index] = table.pass_hu_fan[seat_index] > table.get_fan(seat_index, _seat_index) ? table.pass_hu_fan[seat_index]
						: table.get_fan(seat_index, _seat_index);
			}
		}
