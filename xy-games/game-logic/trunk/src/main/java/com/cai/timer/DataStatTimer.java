...
public class DataStatTimer extends TimerTask{
	...
	public void run() {
		try{
			...
			MongoDBServiceImpl.getInstance().systemLog(ELogType.roomNumber, null, roomNumber,null, ESysLogLevelType.NONE);
			...
			MongoDBServiceImpl.getInstance().systemLog_queue(ELogType.requestPool_new, buf.toString(), activeCount,queueSize, ESysLogLevelType.NONE);
			...
			countThreadPool(Global.getLogicThreadPool(),ELogType.requestLogicPool_NEW);
			
			countThreadPool(Global.getRoomPool(),ELogType.roomPool);
			
			countThreadPool(Global.getRoomPoolBACK(),ELogType.roomPoolback);
	...
	public static void countThreadPool(ThreadPoolExecutor scheduledThreadPoolExecutor,ELogType logType) {
		...
		int poolSize = scheduledThreadPoolExecutor.getCorePoolSize();
		StringBuilder buf = new StringBuilder();
		buf.append("计划执行任务:" + taskCount).append(",已执行任务:" + complementtaskCount).append(",活动线程数:" + activeCount)
				.append(",总线程数:").append(poolSize).append(",队列长度:" + queueSize).append("逻辑服"+SystemConfig.logic_index);
		MongoDBServiceImpl.getInstance().systemLog(logType, buf.toString(), activeCount, queueSize, ESysLogLevelType.NONE);
		
		if(queueSize>1000) {
			logger.error("Slowly process countThreadPool queueSize"+queueSize);
		}
