...
public class Table_JingDian_CS_QY extends NewAbstractMjTable {
	private static final long serialVersionUID = 6668261272281498768L;

	private HandlerGangDispatchCard_JingDian_CS_QY _handler_gang_dispatch_card;
	private HandlerGangDispatchCard_THJ_JingDian_CS_QY _handler_gang_dispatch_card_thj;
	private HandlerPiao_JingDian_CS_QY _handler_piao;
	private HandlerXiaoHu_JingDian_CS_QY _handler_xiao_hu;
	...
	public Table_JingDian_CS_QY() {
		super(MJType.GAME_TYPE_MJ_CS_QY);
	}
	...
	public boolean check_da_si_xi(int[] cards_index, ChiHuRight chr, int seat_index) {
		for (int i = 0; i < GameConstants.MAX_ZI; i++) {
			if (cards_index[i] == 4) {
				...
				chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_DA_SI_XI);
		...
		for (int i = 0; i < GameConstants.MAX_ZI; i++) {
			if (cards_index[i] == 4) {
				...
				if (can_win_zt_si_xi) {
					...
					// chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_ZT_DA_SI_XI);
	...
	public boolean check_ban_ban_hu(int[] cards_index, ChiHuRight chr) {
		...
		chr._show_all = true;
		chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_BAN_BAN_HU);
		return true;
	}
	...
	public boolean check_que_yi_se(int cards_index[], ChiHuRight chr) {
		...
		if (count < 3) {
			chr._show_all = true;
			chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_QUE_YI_SE);
			return true;
		} else
			return false;
	}
	...
	public boolean check_liu_liu_shun(int[] cards_index, ChiHuRight chr, int seat_index) {
		...
		for (int i = 0; i < GameConstants.MAX_ZI; i++) {
			...
			if (index_a != -1 && index_b != -1) {
				...
				chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_LIU_LIU_SHUN);
				return true;
			}
		}
		return false;
	...
	public boolean check_zt_liu_liu_shun(int[] cards_index, ChiHuRight chr, int card_data, int seat_index) {
		...
		for (int i = 0; i < GameConstants.MAX_ZI; i++) {
			...
			if (index_a != -1 && index_b != -1) {
				...
				// TODO 中途四喜判断，不能把chr的值加入放到这里，因为如果点了过，并不能直接set_empty();
				// 因为其他的小胡并不是中途的，如果set_empty()会把之前的小胡类型也清空掉了，所以放在发牌处理的show_zt_si_xi方法里
				// chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_ZT_LLS);
			...
			if (cards_index[i] >= 2) {
				...
				if (card_value <= 7) {
					if (cards_index[i + 1] >= 2 && cards_index[i + 2] >= 2) {
						...
						chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_JIE_JIE_GAO);
						return true;
					}
				}
			}
	...
	public boolean check_san_tong(int[] cards_index, ChiHuRight chr) {
		for (int i = 0; i < 9; i++) {
			if (cards_index[i] >= 2 && cards_index[i + 9] >= 2 && cards_index[i + 18] >= 2) {
				...
				chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_SAN_TONG);
				return true;
			}
		}
	...
	public boolean check_yi_zhi_hua(int[] cards_index, ChiHuRight chr) {
		...
		if (hand_card_count == 1 && tmp_cards_index[4] == 1) {
			chr._show_all = true;
			chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_YI_ZHI_HUA);
			return true;
		}
	...
	public boolean check_jin_tong_yu_nv(int[] cards_index, ChiHuRight chr) {
		...
		if (cards_index[index1] >= 2 && cards_index[index2] >= 2) {
			chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_JIN_TONG_YU_NV);
			return true;
		}
	...
	public boolean check_hou_ba_lun(int[] cards_index, ChiHuRight chr) {
		...
		if (cards_index[index1] >= 3) {
			chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_HOU_BA_LUN);
			return true;
		}
	...
	public boolean check_mei_hua_san_nong(int[] cards_index, ChiHuRight chr) {
		...
		if (cards_index[index1] >= 3) {
			chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_MEI_HUA_SAN_NONG);
			return true;
		}
	...
	public boolean check_yi_dian_hong(int[] cards_index, ChiHuRight chr) {
		...
		if (count == 1) {
			chr._show_all = true;
			chr.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_YI_DIAN_HONG);
			return true;
		}
	...
	public int analyse_chi_hu_card_xiaohu(int cards_index[], ChiHuRight chiHuRight, int seat_index) {
		...
		if (true || has_rule(Constants_JingDian_CS_QY.GAME_RULE_DA_SI_XI))
			has_da_si_xi = check_da_si_xi(cbCardIndexTemp, chiHuRight, seat_index);

		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_BAN_BAN_HU))
			has_ban_ban_hu = check_ban_ban_hu(cbCardIndexTemp, chiHuRight);

		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_QUE_YI_SE))
			has_que_yi_se = check_que_yi_se(cbCardIndexTemp, chiHuRight);

		if (true || has_rule(Constants_JingDian_CS_QY.GAME_RULE_LIU_LIU_SHUN))
			has_liu_liu_shun = check_liu_liu_shun(cbCardIndexTemp, chiHuRight, seat_index);

		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_JIE_JIE_GAO))
			has_jie_jie_gao = check_jie_jie_gao(cbCardIndexTemp, chiHuRight);

		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_SAN_TONG))
			has_san_tong = check_san_tong(cbCardIndexTemp, chiHuRight);

		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_YI_ZHI_HUA))
			has_yi_zhi_hua = check_yi_zhi_hua(cbCardIndexTemp, chiHuRight);

		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_JIN_TONG_YU_NV))
			has_jin_tong_yu_nv = check_jin_tong_yu_nv(cbCardIndexTemp, chiHuRight);

		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_HOU_BA_LUN))
			has_hou_ba_lun = check_hou_ba_lun(cbCardIndexTemp, chiHuRight);

		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_MEI_HUA_SAN_NONG))
			has_mei_hua_san_nong = check_mei_hua_san_nong(cbCardIndexTemp, chiHuRight);

		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_YI_DIAN_HONG))
	...
	public int analyse_qi_shou_hu_pai(int cards_index[], WeaveItem weaveItem[], int weaveCount, ChiHuRight chiHuRight, int card_type,
		...
		if (_logic.is_jiangjiang_hu_qi_shou(cards_index, weaveItem, weaveCount)) {
			chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_JIANG_JIANG_HU);
			has_big_win = true;
		}
		...
		// chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_QUAN_QIU_REN);
		// has_big_win = true;
		// }

		if (_logic.is_qing_yi_se_qishou(cards_index, weaveItem, weaveCount)) {
			chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_QING_YI_SE);
			has_big_win = true;
		}

		boolean has_men_qing = false;
		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_MEN_QING_ZI_MO) || has_rule(Constants_JingDian_CS_QY.GAME_RULE_MEN_QING_JIE_PAO)) {
		...
		if (is_peng_peng_hu) {
			...
			if (!exist_eat) {
				chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_PENG_PENG_HU);
				has_big_win = true;
			}
		}

		boolean has_jia_jiang_hu = false;
		if (getRuleValue(Constants_JingDian_CS_QY.GAME_RULE_JIA_JIANG_HU) != 0) {
			if (card_type == Constants_JingDian_CS_QY.HU_CARD_TYPE_GANG_KAI || card_type == Constants_JingDian_CS_QY.HU_CARD_TYPE_QIANG_GANG
					|| card_type == Constants_JingDian_CS_QY.HU_CARD_TYPE_GANG_PAO || is_hai_di) {
				if (!has_big_win && !has_258) {
					chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_JIA_JIANG_HU);
		...
		if (has_big_win || has_jia_jiang_hu) {
			if (has_men_qing)
				chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_MEN_QING);

			if (card_type == Constants_JingDian_CS_QY.HU_CARD_TYPE_GANG_KAI) {
				chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_ZI_MO);
				chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_GANG_KAI);
			} else if (card_type == Constants_JingDian_CS_QY.HU_CARD_TYPE_GANG_PAO) {
				chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_JIE_PAO);
				chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_GANG_PAO);
			} else if (card_type == Constants_JingDian_CS_QY.HU_CARD_TYPE_QIANG_GANG) {
				chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_QIANG_GANG);
			} else if (card_type == Constants_JingDian_CS_QY.HU_CARD_TYPE_ZI_MO) {
				chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_ZI_MO);
				if (getTablePlayerNumber() == 4) {
					if (GRR._left_card_count == 55 && !has_jia_jiang_hu)
						chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_TIAN_HU);
				...
				if (getTablePlayerNumber() == 4) {
					if (GRR._left_card_count == 55 && _out_card_count == 1 && !has_jia_jiang_hu)
						chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_DI_HU);
				} else if (getTablePlayerNumber() == 3) {
	...
	public int analyse_chi_hu_card(int[] cards_index, WeaveItem[] weaveItem, int weaveCount, int cur_card, ChiHuRight chiHuRight, int card_type,
		...
		if (_logic.is_dan_diao(cards_index, cur_card)) {
			if (getRuleValue(Constants_JingDian_CS_QY.GAME_RULE_ALL_OPEN) == 1 || has_258) {
				chiHuRight.opr_or_jd_cs(Constants_JingDian_CS_QY.CHR_QUAN_QIU_REN);
				has_big_win = true;
			}
		}
		...
		boolean has_men_qing = false;
		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_MEN_QING_ZI_MO)
				&& (card_type == Constants_JingDian_CS_QY.HU_CARD_TYPE_ZI_MO || card_type == Constants_JingDian_CS_QY.HU_CARD_TYPE_GANG_KAI)) {
			if (_logic.is_men_qing(weaveItem, weaveCount) != GameConstants.WIK_NULL) {
				has_men_qing = true;
			}
		} else if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_MEN_QING_JIE_PAO)) {
			if (_logic.is_men_qing(weaveItem, weaveCount) != GameConstants.WIK_NULL) {
				has_men_qing = true;
			}
		}
	...
	public boolean on_game_start_new() {
		...
		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_PIAO_FEN)) {
	...
	public int get_ting_card(int[] cards, int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index) {
		...
		for (int i = 0; i < l; i++) {
			...
			if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card(cbCardIndexTemp, weaveItem, cbWeaveCount, cbCurrentCard, chr,
					Constants_JingDian_CS_QY.HU_CARD_TYPE_ZI_MO, seat_index)) {
	...
	public int is_qi_xiao_dui(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int cur_card) {
		...
		if (nGenCount > 0) {
			if (nGenCount >= 2) {
				return (int) Constants_JingDian_CS_QY.CHR_SHH_QI_XIAO_DUI;
			}
			return (int) Constants_JingDian_CS_QY.CHR_HH_QI_XIAO_DUI;
		} else {
			return (int) Constants_JingDian_CS_QY.CHR_QI_XIAO_DUI;
		}
	}
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (gang_or_bu == GANG_STATE) {
				action = analyse_chi_hu_card(GRR._cards_index[i], GRR._weave_items[i], cbWeaveCount, card, chr,
						Constants_JingDian_CS_QY.HU_CARD_TYPE_QIANG_GANG, i);
			} else {
				action = analyse_chi_hu_card(GRR._cards_index[i], GRR._weave_items[i], cbWeaveCount, card, chr,
						Constants_JingDian_CS_QY.HU_CARD_TYPE_JIE_PAO, i);
			}
	...
	public boolean estimate_player_fan_pai_response_thj(int seat_index, int provide_index, int card, int[][] special_player_cards, int c) {
		...
		action = analyse_chi_hu_card(GRR._cards_index[seat_index], GRR._weave_items[seat_index], GRR._weave_count[seat_index], card,
				GRR._chi_hu_rights[seat_index], Constants_JingDian_CS_QY.HU_CARD_TYPE_GANG_PAO, seat_index);
	...
	public boolean estimate_player_out_card_response(int seat_index, int card) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (_playerStatus[i].is_chi_hu_round()) {
				...
				int card_type = Constants_JingDian_CS_QY.HU_CARD_TYPE_JIE_PAO;
	...
	public int get_niao_card_num(boolean is_hai_di) {
		int num = 0;
		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZHUA_1_NIAO))
			num = 1;
		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZHUA_2_NIAO))
			num = 2;
		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZHUA_4_NIAO))
			num = 4;
		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZHUA_6_NIAO))
			num = 6;
	...
	public int get_chi_hu_action_rank_cs(ChiHuRight chiHuRight) {
		...
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_MEN_QING)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_MEN_QING))
				wFanShu += 6;
		}

		boolean has_jia_jiang_hu = false;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_JIA_JIANG_HU)).is_empty()) {
			has_jia_jiang_hu = true;
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_JIA_JIANG_HU))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_PENG_PENG_HU)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_PENG_PENG_HU))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_JIANG_JIANG_HU)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_JIANG_JIANG_HU))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_QING_YI_SE)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_QING_YI_SE))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_HAI_DI_LAO_YUE)).is_empty() && !has_jia_jiang_hu) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_HAI_DI_LAO_YUE))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_HAI_DI_PAO)).is_empty() && !has_jia_jiang_hu) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_HAI_DI_PAO))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_QI_XIAO_DUI)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_QI_XIAO_DUI))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_HH_QI_XIAO_DUI)).is_empty()) {
			wFanShu += 12;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_HH_QI_XIAO_DUI))
				wFanShu += 12;
		}
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_GANG_KAI)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_GANG_KAI))
				wFanShu += 6;
		}
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_QIANG_GANG)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_QIANG_GANG))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_GANG_PAO)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_GANG_PAO))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_QUAN_QIU_REN)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_QUAN_QIU_REN))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_SHH_QI_XIAO_DUI)).is_empty()) {
			wFanShu += 18;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_SHH_QI_XIAO_DUI))
				wFanShu += 18;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_TIAN_HU)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_TIAN_HU))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_DI_HU)).is_empty()) {
			wFanShu += 6;
			if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_DI_HU))
				wFanShu += 6;
		}

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_DA_SI_XI)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_BAN_BAN_HU)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_QUE_YI_SE)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_LIU_LIU_SHUN)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_JIE_JIE_GAO)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_SAN_TONG)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_YI_ZHI_HUA)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_ZT_DA_SI_XI)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_ZT_LLS)).is_empty())
			wFanShu += 2;

		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_JIN_TONG_YU_NV)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_HOU_BA_LUN)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_MEI_HUA_SAN_NONG)).is_empty())
			wFanShu += 2;
		if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_YI_DIAN_HONG)).is_empty())
			wFanShu += 2;

		if (wFanShu == 0) {
			if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_JIE_PAO)).is_empty()) {
				wFanShu = 1;
				if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_JIE_PAO)) {
					wFanShu = 2;
				}
			}
		...
		if (wFanShu == 0) {
			if (!(chiHuRight.opr_and_long(Constants_JingDian_CS_QY.CHR_ZI_MO)).is_empty()) {
				wFanShu = 2;
				if (chiHuRight.is_mul_long(Constants_JingDian_CS_QY.CHR_ZI_MO)) {
					wFanShu = 4;
				}
			}
		}
	...
	protected void onInitTable() {
		_handler_chi_peng = new HandlerChiPeng_JingDian_CS_QY();
		_handler_dispath_card = new HandlerDispatchCard_JingDian_CS_QY();
		_handler_gang = new HandlerGang_JingDian_CS_QY();
		_handler_hai_di = new HandlerHaiDi_JingDian_CS_QY();
		_handler_out_card_operate = new HandlerOutCardOperate_JingDian_CS_QY();
		_handler_yao_hai_di = new HandlerYaoHaiDi_JingDian_CS_QY();

		_handler_xiao_hu = new HandlerXiaoHu_JingDian_CS_QY();
		_handler_piao = new HandlerPiao_JingDian_CS_QY();
		_handler_gang_dispatch_card = new HandlerGangDispatchCard_JingDian_CS_QY();
		_handler_gang_dispatch_card_thj = new HandlerGangDispatchCard_THJ_JingDian_CS_QY();

		_handler_qi_shou = new HandlerQiShou_JingDian_CS_QY();
	}
	...
	public void process_chi_hu_player_score(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		boolean isMutlpNiap = has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZHONG_NIAO_FAN_BEI);
		...
		if (isMutlpNiap) {
			if (zimo) {
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					...
					if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZHUANG_XIAN)) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (GRR._start_hu_right[i].is_valid()) {
				...
				for (int j = 0; j < l; j++) {
					...
					if (type == Constants_JingDian_CS_QY.CHR_DA_SI_XI) {
						des += " 大四喜";
					}
					if (type == Constants_JingDian_CS_QY.CHR_ZT_DA_SI_XI) {
						des += " 中途四喜";
					...
					if (type == Constants_JingDian_CS_QY.CHR_BAN_BAN_HU) {
						des += " 板板胡";
					}
					if (type == Constants_JingDian_CS_QY.CHR_QUE_YI_SE) {
						des += " 缺一色";
					}
					if (type == Constants_JingDian_CS_QY.CHR_LIU_LIU_SHUN) {
						des += " 六六顺";
					}
					if (type == Constants_JingDian_CS_QY.CHR_ZT_LLS) {
						des += " 中途六六顺";
					...
					if (type == Constants_JingDian_CS_QY.CHR_JIE_JIE_GAO) {
						des += " 节节高";
					}
					if (type == Constants_JingDian_CS_QY.CHR_SAN_TONG) {
						des += " 三同";
					}
					if (type == Constants_JingDian_CS_QY.CHR_YI_ZHI_HUA) {
						des += " 一枝花";
					}

					if (type == Constants_JingDian_CS_QY.CHR_JIN_TONG_YU_NV) {
						des += "  金童玉女";
					}
					if (type == Constants_JingDian_CS_QY.CHR_HOU_BA_LUN) {
						des += " 后八轮";
					}
					if (type == Constants_JingDian_CS_QY.CHR_MEI_HUA_SAN_NONG) {
						des += " 梅花三弄";
					}
					if (type == Constants_JingDian_CS_QY.CHR_YI_DIAN_HONG) {
						des += " 一点红";
					}
				}
			}
			...
			for (int j = 0; j < l; j++) {
				...
				if (GRR._chi_hu_rights[i].is_valid()) {
					if (type == Constants_JingDian_CS_QY.CHR_MEN_QING) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_MEN_QING)) {
							des += " 门清*2";
						} else {
							des += " 门清";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_PENG_PENG_HU) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_PENG_PENG_HU)) {
							des += " 碰碰胡*2";
						} else {
							des += " 碰碰胡";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_JIANG_JIANG_HU) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_JIANG_JIANG_HU)) {
							des += " 将将胡*2";
						} else {
							des += " 将将胡";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_QING_YI_SE) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_QING_YI_SE)) {
							des += " 清一色*2";
						} else {
							des += " 清一色";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_HAI_DI_LAO_YUE) {
						des += " 海底捞";
					}
					if (type == Constants_JingDian_CS_QY.CHR_HAI_DI_PAO) {
						des += " 海底炮";
					}
					if (type == Constants_JingDian_CS_QY.CHR_QI_XIAO_DUI) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_QI_XIAO_DUI)) {
							des += " 七小对*2";
						} else {
							des += " 七小对";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_HH_QI_XIAO_DUI) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_HH_QI_XIAO_DUI)) {
							des += " 豪华七小对*2";
						} else {
							des += " 豪华七小对";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_GANG_KAI) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_GANG_KAI)) {
							des += " 杠上开花*2";
						} else {
							des += " 杠上开花";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_QIANG_GANG) {
						des += " 抢杠胡";
					}
					if (type == Constants_JingDian_CS_QY.CHR_GANG_PAO) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_GANG_PAO)) {
							des += " 杠上炮*2";
						} else {
							des += " 杠上炮";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_QUAN_QIU_REN) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_QUAN_QIU_REN)) {
							des += " 全求人*2";
						} else {
							des += " 全求人";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_SHH_QI_XIAO_DUI) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_SHH_QI_XIAO_DUI)) {
							des += " 双豪华七小对*2";
						} else {
							des += " 双豪华七小对";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_TIAN_HU) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_TIAN_HU)) {
							des += " 天胡*2";
						} else {
							des += " 天胡";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_DI_HU) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_DI_HU)) {
							des += " 地胡*2";
						} else {
							des += " 地胡";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_JIA_JIANG_HU) {
						if (chr.is_mul_long(Constants_JingDian_CS_QY.CHR_JIA_JIANG_HU)) {
							des += " 假将胡*2";
						} else {
							des += " 假将胡";
						}
					}
					if (type == Constants_JingDian_CS_QY.CHR_ZI_MO) {
						if (dahu[i] == true) {
							des += " 大胡自摸";
						} else {
							des += " 小胡自摸";
					...
					if (type == Constants_JingDian_CS_QY.CHR_JIE_PAO) {
						if (dahu[i] == true) {
							des += " 大胡接炮";
						} else {
							des += " 小胡接炮";
					...
					if (type == Constants_JingDian_CS_QY.CHR_FANG_PAO) {
						if (has_da_hu == true) {
							des += " 大胡放炮";
						} else {
							des += " 小胡放炮";
	...
	public int getZuoPiaoScore() {
		int score = 0;
		if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZUO_PIAO_1)) {
			return 1;
		} else if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZUO_PIAO_2)) {
			return 2;
		} else if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZUO_PIAO_3)) {
			return 3;
		} else if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZUO_PIAO_4)) {
			return 4;
		} else if (has_rule(Constants_JingDian_CS_QY.GAME_RULE_ZUO_PIAO_5)) {
			return 5;
		}
		return score;
	}
