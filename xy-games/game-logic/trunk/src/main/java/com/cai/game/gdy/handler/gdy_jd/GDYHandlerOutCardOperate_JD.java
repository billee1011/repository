...
public class GDYHandlerOutCardOperate_JD extends GDYHandlerOutCardOperate<GDYTable_JD> {

	@Override
	public void exe(GDYTable_JD table) {
		...
		if (_out_type == 0) {
			...
			if (table.has_rule(GameConstants.GAME_RULE_GDY_MUST_JIE)) {
				if (table._logic.search_card_data(table._turn_out_card_data, table._turn_out_card_count,
		...
		if (is_all_magic == true && _out_card_count == 1) {
			...
			if (table.has_rule(GameConstants.GAME_RULE_GDY_KING_BOMB) && is_all_magic) {
			...
			// 判断出牌牌型
			card_type = table._logic.GetCardType_GDY(_out_change_cards_data, _out_card_count);
			...
			if (table._turn_out_card_count != 0) {
				...
				if (!table._logic.comparecarddata(_out_change_cards_data, _out_card_count, table._turn_out_card_data,
		...
		// 炸弹限制倍数
		if (card_type == GameConstants.GDY_CT_BOMB_3) {
			table._boom_times *= 2;
		} else if (card_type == GameConstants.GDY_CT_BOMB_4) {
			table._boom_times *= 4;
		} else if (card_type == GameConstants.GDY_CT_BOMB_5) {
			table._boom_times *= 8;
		} else if (card_type == GameConstants.GDY_CT_BOMB_6) {
			table._boom_times *= 16;
		} else if (card_type == GameConstants.GDY_CT_KING_BOMB) {
			table._boom_times *= 4;
		}
		...
		if (table.has_rule(GameConstants.GAME_RULE_GDY_CHU_FINISH)) {
			if (table.GRR._card_count[_out_card_player] == 0) {
				...
				GameSchedule.put(
						new GameFinishRunnable(table.getRoom_id(), table._win_player, GameConstants.Game_End_NORMAL),
				...
				if (table._win_player == GameConstants.INVALID_SEAT) {
					...
					if (table._current_player == table._win_player) {
						...
						GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._win_player,
								GameConstants.Game_End_NORMAL), 1000, TimeUnit.MILLISECONDS);
					} else {
						table.operate_out_card(_out_card_player, _out_card_count, _out_cards_data,
								_out_change_cards_data, card_type, table._is_shou_chu, GameConstants.INVALID_SEAT);
						table._out_card_player = _out_card_player;
	...
	@Override
	public boolean handler_player_be_in_room(GDYTable_JD table, int seat_index) {
		...
		if (table._current_player == seat_index) {
			if (table._turn_out_card_count == 0) {
				...
				if (table._logic.search_card_data(table._turn_out_card_data, table._turn_out_card_count,
						table.GRR._cards_data[seat_index], table.GRR._card_count[seat_index])) {
					tableResponse_gdy.setCurPlayerYaPai(1);
				} else {
