...
public class MJHandlerGang_CS_DispatchCard extends MJHandler {
	...
	public void exe(MJTable table) {
		...
		// 显示两张牌
		table.operate_out_card(_seat_index, GameConstants.CS_GANG_DRAW_COUNT, this._gang_card_data.get_cards(), GameConstants.OUT_CARD_TYPE_LEFT,
				...
				if (k == _seat_index) {
					...
					if (table.GRR._left_card_count > 2) {
						...
						if (cbActionMask != GameConstants.WIK_NULL) {// 有杠
							// boolean is_ting =
							// table.is_cs_ting_card(table.GRR._cards_index[k],
					...
					if (k == chi_seat_index) {
						bAroseAction = table.estimate_gang_cs_respond(k, _seat_index, bu_card, _double, true);// ,
																												// EstimatKind.EstimatKind_OutCard
					} else {
						bAroseAction = table.estimate_gang_cs_respond(k, _seat_index, bu_card, _double, false);// ,
																												// EstimatKind.EstimatKind_OutCard
					}
		...
		if (has_action == false) {
			...
			GameSchedule.put(new AddDiscardRunnable(table.getRoom_id(), _seat_index, this._gang_card_data.get_card_count(),
					this._gang_card_data.get_cards(), true, table.getMaxCount()), GameConstants.GANG_CARD_CS_DELAY, TimeUnit.MILLISECONDS);
