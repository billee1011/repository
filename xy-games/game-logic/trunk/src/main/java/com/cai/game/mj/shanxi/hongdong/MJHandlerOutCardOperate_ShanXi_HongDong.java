...
public class MJHandlerOutCardOperate_ShanXi_HongDong extends MJHandlerOutCardOperate<MJTable_HongDong> {
	...
	public void exe(MJTable_HongDong table) {
		...
		if (table._logic.is_magic_card(_out_card_data)) {
			table.operate_out_card(_out_card_player, 1, new int[] { _out_card_data + GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI },
		...
		// 检查听牌
		if (table._playerStatus[_out_card_player]._card_status == GameConstants.CARD_STATUS_BAO_TING
				|| table._playerStatus[_out_card_player]._card_status == Constants_MJ_SXHD.CARD_STATUS_YING_KOU) {
	...
	public boolean handler_player_be_in_room(MJTable_HongDong table, int seat_index) {
		...
		// 历史记录 tableResponse.setOutCardData(0);
