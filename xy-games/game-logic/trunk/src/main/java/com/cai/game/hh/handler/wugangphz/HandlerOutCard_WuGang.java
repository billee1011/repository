...
public class HandlerOutCard_WuGang extends HHHandlerOutCardOperate<Table_WuGang> {
    ...
    public void exe(Table_WuGang table) {
        ...
        for (int i = 0; i < table.getTablePlayerNumber(); i++) { // 出牌时判断，有没有射跑的
            if (i == _out_card_player)
                continue;
            ti_pao = table.estimate_player_respond_phz_chd(i, _out_card_player, table._out_card_data, pao_type[i],
                    false);
            if (ti_pao != GameConstants.WIK_NULL) {
                table.exe_gang(i, _out_card_player, table._out_card_data, ti_pao, pao_type[i][0], false, true, false,
                        1000);
                return;
            }
        }

        boolean bAroseAction = table.estimate_player_out_card_respond_chen_zhou(_out_card_player, table._out_card_data,
                false); // 有没有吃碰的

        if (bAroseAction == false) { // 如果没人吃碰
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                table._playerStatus[i].clean_action();
                table._playerStatus[i].clean_status();
            }
