...
public class Table_GY extends AbstractMJTable {
		...
		if (check_weave) {
			...
			for (int i = 0; i < cbWeaveCount; i++) {
				...
				for (int j = 0; j < GameConstants_MYGY.MAX_INDEX; j++) {
					...
					if (j != _logic.switch_to_card_index(_send_card_data)) {
						continue;
					}
	...
	public boolean estimate_gang_respond(int seat_index, int card) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (playerStatus.is_chi_hu_round()) {
				...
				// 结果判断
				if (action != 0) {
					_playerStatus[i].add_action(GameConstants_MYGY.WIK_MUSIT_CHI_HU);
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (_playerStatus[i].is_chi_hu_round()) {
				...
				if (can_hu_this_card) {
					...
					if (action != 0) {
						if (card_type == GameConstants_MYGY.HU_CARD_TYPE_RE_PAO) {
							_playerStatus[i].add_action(GameConstants_MYGY.WIK_MUSIT_CHI_HU);
							can_peng = false;
						} else {
	...
	private int get_hu_fen(ChiHuRight chr) {
		...
		if (!chr.opr_and(GameConstants_MYGY.CHR_QING_DI_LONG_HU).is_empty()) {
			fen += 30;
		}
		if (!chr.opr_and(GameConstants_MYGY.CHR_DI_LONG_HU).is_empty()) {
			fen += 20;
		}
		if (!chr.opr_and(GameConstants_MYGY.CHR_LONG_QI_DUI).is_empty()) {
			fen += 20;
	...
	public int analyse_chi_hu_card(int[] cards_index, WeaveItem[] weaveItems, int weave_count, int cur_card, ChiHuRight chiHuRight, int card_type,
		...
		int check_di_long_hu = analyse_di_long(cards_index, weave_count, weaveItems, cur_card, is_zimo);
		...
		if (!bValue && check_dui_zi_hu == GameConstants_MYGY.WIK_NULL && check_di_long_hu == GameConstants_MYGY.WIK_NULL) {
		...
		if (check_di_long_hu != GameConstants_MYGY.WIK_NULL) {
			if (!chiHuRight.opr_and(GameConstants_MYGY.CHR_QING_YI_SE).is_empty()) {
				chiHuRight.opr_or(GameConstants_MYGY.CHR_QING_DI_LONG_HU);
			} else {
				chiHuRight.opr_or(GameConstants_MYGY.CHR_DI_LONG_HU);
			}
		}
	 ...
	 * @return
	 */
	public int analyse_di_long(int[] cards_index, int weave_count, WeaveItem[] weave_items, int cur_card, boolean zimo) {
		int[] tmp_cards_index = Arrays.copyOf(cards_index, cards_index.length);
		boolean cur_card_is_weave = false;
		for (int w = 0; w < weave_count; w++) {
			if (weave_items[w].center_card == cur_card) {
				cur_card_is_weave = true;
			}
			if (weave_items[w].weave_kind == GameConstants_MYGY.WIK_GANG) {
				tmp_cards_index[_logic.switch_to_card_index(weave_items[w].center_card)]++;
				tmp_cards_index[_logic.switch_to_card_index(weave_items[w].center_card)]++;
				tmp_cards_index[_logic.switch_to_card_index(weave_items[w].center_card)]++;
				tmp_cards_index[_logic.switch_to_card_index(weave_items[w].center_card)]++;
			} else if (weave_items[w].weave_kind == GameConstants_MYGY.WIK_PENG) {
				tmp_cards_index[_logic.switch_to_card_index(weave_items[w].center_card)]++;
				tmp_cards_index[_logic.switch_to_card_index(weave_items[w].center_card)]++;
				tmp_cards_index[_logic.switch_to_card_index(weave_items[w].center_card)]++;
			}
		}
		if (!cur_card_is_weave) {
			return GameConstants_MYGY.WIK_NULL;
		}

		return is_qi_xiao_dui(tmp_cards_index, null, 0, cur_card, zimo);
	}
	...
	public void process_ji_fen() {
		for (int player = 0; player < getTablePlayerNumber(); player++) {
			...
			if (chong_feng_ji_seat_yj == player) {
				...
				if (jin_ji[0]) {
					s += 9;
					show_index_score[player][GameConstants_MYGY.INDEX_17_JT_CFJJ] += 9;
	...
	public void process_chi_hu_player_score(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		if (zimo) {
			if (!GRR._chi_hu_rights[seat_index].opr_and(GameConstants_MYGY.CHR_QING_GANG_HU).is_empty()) {
				...
				if (_playerStatus[seat_index].is_bao_ting()) {
					lChiHuScore += 10;
					show_index_score[seat_index][GameConstants_MYGY.INDEX_39_BAO_TING] += 10;
				}

				// 抢杠胡不论是选择什么自摸分数，都按照自摸算，分值一律按照自摸+1，并需要包赔其他玩家要付的胡牌分
				lChiHuScore++;
				...
				if (!chr.opr_and(GameConstants_MYGY.CHR_GNAG_KAI).is_empty()) {
					lChiHuScore += 1;
					show_index_score[seat_index][GameConstants_MYGY.INDEX_9_HT_GANG_SHNAG_HUA] += 1;
					show_index_card[seat_index][GameConstants_MYGY.INDEX_9_HT_GANG_SHNAG_HUA] = operate_card;
				}

				if (has_rule(GameConstants_MYGY.GAME_RULE_ZI_MO_ADD_1)) {
					lChiHuScore++;
					show_index_score[seat_index][GameConstants_MYGY.INDEX_0_HT_ZI_MO] += 1;
				}
				if (has_rule(GameConstants_MYGY.GAME_RULE_ZI_MO_ADD_2)) {
					lChiHuScore += 2;
					show_index_score[seat_index][GameConstants_MYGY.INDEX_0_HT_ZI_MO] += 2;
				}
				if (has_rule(GameConstants_MYGY.GAME_RULE_ZI_MO_FAN)) {
					show_index_score[seat_index][GameConstants_MYGY.INDEX_0_HT_ZI_MO] += lChiHuScore;
					lChiHuScore += lChiHuScore;
				}
	...
	public boolean SetMagiCards() {
		...
		if (DEBUG_MAGIC_CARD) {
			...
			card = magic_card_decidor;
			DEBUG_MAGIC_CARD = false;
		}

		GRR._especial_card_count = 1;
		GRR._especial_show_cards[0] = card;
		_logic.add_magic_card_index(_logic.switch_to_card_index(card));
		operate_show_card(this.GRR._banker_player, GameConstants.Show_Card_Center, 1, new int[] { card }, GameConstants.INVALID_SEAT);
