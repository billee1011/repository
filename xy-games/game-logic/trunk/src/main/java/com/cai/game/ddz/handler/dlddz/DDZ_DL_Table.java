 ...
 * 
 */
package com.cai.game.ddz.handler.dlddz;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class DDZ_DL_Table extends DDZTable {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		// 初始化基础牌局handle
		_handler_out_card_operate = new DDZHandlerOutCardOperate_DL();
		_handler_call_banker = new DDZHandlerCallBanker();
		_handler_add_times = new DDZHandlerAddtimes();
		_handler_call_banker = new DDZHandlerCallBanker_DL();
	...
	protected boolean on_handler_game_start() {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			this._player_result.chun_tian[i] = -1;
		}
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		if (this.isCoinRoom()) {
			ret = this.handler_game_finish_coin(seat_index, reason);
		} else {
	...
	public boolean handler_game_finish_coin(int seat_index, int reason) {
		...
		if (GRR != null) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				game_end_ddz.addCardCount(this._init_hand_card_count[i]);
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
				for (int j = 0; j < _init_hand_card_count[i]; j++) {
					cards_card.addItem(this._init_hand_card[i][j]);
				}
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (chuntian[i] != -1) {
				...
				if (chuntian[i] == GRR._banker_player) {
					this._player_result.chun_tian[chuntian[i]] = 2;
				} else {
					this._player_result.chun_tian[chuntian[i]] = 1;
				}
			}
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			...
			if (this._current_player != GameConstants.INVALID_SEAT) {
				if (_turn_out_card_count == 0 || this._current_player != index) {
					outcarddata.setDisplayTime(getDelay_play_card_time() / 1000);
					outcarddata.setCurPlayerYaPai(1);
				} else {
