...
public class HandlerDispatchCard_PING_XIANG extends MJHandlerDispatchCard<Table_PING_XIANG> {
	...
	public HandlerDispatchCard_PING_XIANG() {
	...
	public void exe(Table_PING_XIANG table) {
		if(table.is_gang_yao[_seat_index])
		...
		if (table.GRR._left_card_count == 0) {
			...
			// 得到上家
			int last_player = (_seat_index + table.getTablePlayerNumber() - 1) % table.getTablePlayerNumber();
			table._cur_banker = last_player;
		...
		// 判断是否是杠后抓牌,杠开花只算接杠
		int card_type = Constants_MJ_PING_XIANG_258.HU_CARD_TYPE_ZI_MO;
		if (_type == Constants_MJ_PING_XIANG_258.HU_CARD_TYPE_GANG_KAI_HUA) {
			card_type = Constants_MJ_PING_XIANG_258.HU_CARD_TYPE_GANG_KAI_HUA;
		}
		// 检查牌型,听牌
		...
		if (action != GameConstants.WIK_NULL) {
			...
			if(_type == Constants_MJ_PING_XIANG_258.HU_CARD_TYPE_GANG_KAI_HUA)
				curPlayerStatus.add_action_card(-1, _send_card_data, GameConstants.WIK_ZI_MO, _seat_index);
			else
		...
		if (table.GRR._left_card_count > 0) {
			...
			if (cbActionMask != GameConstants.WIK_NULL && !table.is_gang_yao[_seat_index]) {
				curPlayerStatus.add_action(GameConstants.WIK_GANG);
				for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
					curPlayerStatus.add_gang(m_gangCardResult.cbCardData[i], _seat_index, m_gangCardResult.isPublic[i]);
					if(table.is_can_gang_yao(_seat_index, m_gangCardResult.cbCardData[i])){
						if(!curPlayerStatus.has_action_by_code(Constants_MJ_PING_XIANG_258.WIK_GANG_YAO))
							curPlayerStatus.add_action(Constants_MJ_PING_XIANG_258.WIK_GANG_YAO);
						table.add_gang_yao(curPlayerStatus, m_gangCardResult.cbCardData[i], _seat_index, m_gangCardResult.isPublic[i]);
					}
				}
			}
		}
		...
		if (curPlayerStatus.has_action()) {
			if(table.has_rule(Constants_MJ_PING_XIANG_258.GAME_RULE_QIANG_ZHI_HU_PAI) && curPlayerStatus.has_chi_hu()){
				handler_operate_card(table, _seat_index, GameConstants.WIK_ZI_MO, _send_card_data);
				return;
			}
	...
	@Override
	public boolean handler_player_out_card(Table_PING_XIANG table, int seat_index, int card) {
	...
	public boolean handler_operate_card(Table_PING_XIANG table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: {
			...
			table.set_niao_card(seat_index, false);
