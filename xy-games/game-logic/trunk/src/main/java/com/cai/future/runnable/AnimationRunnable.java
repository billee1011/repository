...
public class AnimationRunnable extends BaseFuture {
	private static Logger logger = LoggerFactory.getLogger(AnimationRunnable.class);

	
	private int _room_id;
	private int _timer_id;
	public AnimationRunnable(int room_id,int timer_id){
		super(room_id);
		_room_id = room_id;
		_timer_id = timer_id;
	}
	...
	public void execute() {
		...
		try {
			...
			try{
				roomLock.lock();
				table.animation_timer(_timer_id);
			}finally{
				roomLock.unlock();
