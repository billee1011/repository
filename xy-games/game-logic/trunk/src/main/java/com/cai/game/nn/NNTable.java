...
import com.cai.game.nn.handler.kszox.NNHandlerAddJetton_kszOX;
import com.cai.game.nn.handler.kszox.NNHandlerCallBanker_kszOX;
import com.cai.game.nn.handler.kszox.NNHandlerOpenCard_kszOX;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class NNTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	public int _card_times[]; // 牌倍数
	public int _game_score[][];// 用户每局数据
	public int _last_card[]; // 看四张的最后一张牌
	...
	public NNTable() {
		...
		_last_card = new int[this.getTablePlayerNumber()];
		Arrays.fill(_last_card, 0);
		is_game_start = 0;
		_tui_zhu_times = 1;
		_own_room_seat = -1;
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_SEVER_OX) || is_mj_type(GameConstants.GAME_TYPE_SEVER_OX_LX)
				|| GameDescUtil.has_rule(gameRuleIndexEx, GameConstants.GAME_RULE_JDOX_BBOX)) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_SZOX) || is_mj_type(GameConstants.GAME_TYPE_SZOX_LX)
				|| GameDescUtil.has_rule(gameRuleIndexEx, GameConstants.GAME_RULE_JDOX_ZBOX)
		...
		if (is_mj_type(GameConstants.GAME_TYPE_LZOX) || is_mj_type(GameConstants.GAME_TYPE_LZOX_LX)
				|| GameDescUtil.has_rule(gameRuleIndexEx, GameConstants.GAME_RULE_JDOX_LBOX)) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_MSZOX) || is_mj_type(GameConstants.GAME_TYPE_MSZOX_LX)
				|| GameDescUtil.has_rule(gameRuleIndexEx, GameConstants.GAME_RULE_JDOX_MSOX)) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_MFZOX) || is_mj_type(GameConstants.GAME_TYPE_MFZOX_LX)
				|| GameDescUtil.has_rule(gameRuleIndexEx, GameConstants.GAME_RULE_JDOX_KFOX)) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_KSZOX)) {
			_handler_add_jetton = new NNHandlerAddJetton_kszOX();
			_handler_open_card = new NNHandlerOpenCard_kszOX();
			_handler_Call_banker = new NNHandlerCallBanker_kszOX();
		}
	...
	public boolean call_banker_KSZOX() {
		// 游戏开始
		this._game_status = GameConstants.GS_OX_CALL_BANKER;// 设置状态
		if (this._game_scheduled != null)
			this.kill_timer();
	...
	public boolean game_start_LZOX() {
		...
		if (this._cur_round == 1) {
			if (this._own_room_seat != -1) {
				this._cur_banker = this._own_room_seat;
			} else {
				int chairID[] = new int[GameConstants.GAME_PLAYER_OX];
				int chair_count = 0;
				for (int i = 0; i < GameConstants.GAME_PLAYER_OX; i++) {
		...
		for (int i = 0; i < GameConstants.GAME_PLAYER_OX; i++) {
			...
			if (i != this._cur_banker || GameDescUtil.has_rule(gameRuleIndexEx, GameConstants.GAME_RULE_JDOX_GDYF)) {
				for (int k = 0; k < GameConstants.GAME_PLAYER_OX; k++) {
					Int32ArrayResponse.Builder cards = Int32ArrayResponse.newBuilder();
					if ((i != this._cur_banker) && (this._player_status[k] == true)) {
						if (is_mj_type(GameConstants.GAME_TYPE_JDOX_YY)) {
							...
							_cur_jetton_count[k] = _jetton_count;
							if (_can_tuizhu_player[k] > 0) {
								_jetton_info_cur[k][this._jetton_count] = _can_tuizhu_player[k];
								cards.addItem(_jetton_info_cur[k][this._jetton_count]);
								_cur_jetton_count[k]++;
							}
						}
		...
		for (int i = 0; i < GameConstants.GAME_PLAYER_OX; i++) {
			...
			for (int k = 0; k < GameConstants.GAME_PLAYER_OX; k++) {
				...
				if (this._player_status[k] != true) {
					...
					if (k == i) {
						for (int j = 0; j < GameConstants.OX_MAX_CARD_COUNT; j++) {
							if (this.is_mj_type(GameConstants.GAME_TYPE_MFZOX) && _last_card[k] == GRR._cards_data[k][j])
								cards.addItem(GRR._cards_data[k][j] | 0x100);
							else
								cards.addItem(GRR._cards_data[k][j]);
	...
	public void open_card_ox(int seat_index) {
...

		{
			...
			for (int j = 0; j < GameConstants.OX_MAX_CARD_COUNT; j++) {
				if (_last_card[seat_index] == GRR._cards_data[seat_index][j] && is_mj_type(GameConstants.GAME_TYPE_MFZOX))
					cards.addItem(GRR._cards_data[seat_index][j] | 0x100);
				else
		...
		RoomResponse_OX.Builder roomResponse_ox = RoomResponse_OX.newBuilder();
		for (int j = 0; j < GameConstants.OX_MAX_CARD_COUNT; j++) {
			if (_last_card[seat_index] == GRR._cards_data[seat_index][j] && is_mj_type(GameConstants.GAME_RULE_KSZQZ_OX))
				cards.addItem(GRR._cards_data[seat_index][j] | 0x100);
			else
				cards.addItem(GRR._cards_data[seat_index][j]);
		}
			...
			for (int i = 0; i < GameConstants.GAME_PLAYER_OX; i++) {
				...
				if (this._can_tuizhu_player[i] == 0) {
					if (GRR._game_score[i] > 0) {
						int temp = 0;
						if ((int) GRR._game_score[i] + this._add_Jetton[i] > this._jetton_info_cur[i][0] * this._tui_zhu_times)
							temp = this._jetton_info_cur[i][0] * this._tui_zhu_times;
						else
							temp = (int) GRR._game_score[i] + this._add_Jetton[i];
						if (this._jetton_info_sever_ox[this._jetton_count - 1] >= temp)
	...
	private void shuffle(int repertory_card[], int mj_cards[]) {
		...
		for (int i = 0; i < GameConstants.GAME_PLAYER_OX; i++) {
			...
			_last_card[i] = GRR._cards_data[i][GameConstants.OX_MAX_CARD_COUNT - 1];
		}
		// test_cards();
		// 记录初始牌型
	...
	private void test_cards() {
		...
		// 0x02,0x22,0x32,0x38,0x28,0x05,0x15,0x25,0x07,0x17
		// };
	...
	public boolean handler_game_finish_tbox(int seat_index, int reason) {
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL ||
			...
			if (this.is_mj_type(GameConstants.GAME_TYPE_TBOX))
				record_info.banker_seat = -1;
			else
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				if (this.is_mj_type(GameConstants.GAME_TYPE_TBOX))
					record_info.jetton_player[i] = this.game_cell;
				else
					record_info.jetton_player[i] = this._add_Jetton[i];
				record_info.end_score[i] = (int) GRR._game_score[i];
				record_info.card_type[i] = _card_type_ox[i];
				int calculate_score = 0;
				if (has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_ONE))
					calculate_score = _logic.get_times_two(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index, _game_type_index);
				if (has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_TWO))
					calculate_score = _logic.get_times_one(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index, _game_type_index);
				if (this._player_status[i] == true)
					record_info.card_times[i] = calculate_score;
				else
					record_info.card_times[i] = 0;
				record_info.player_status[i] = this._player_status[i];
				record_info.call_banker[i] = this._call_banker[i];
				// Player player = this.get_players()[i];
				// if(player != null)
				// player.setGame_score((long)this._player_result.getGame_score()[i]);
				// record_info.player[i] = player;
	...
	public boolean handler_enter_room(Player player) {
		...
		if (get_players()[seat_index].getAccount_id() == this.getRoom_owner_account_id())
			this._own_room_seat = seat_index;
		// 从观察者列表移出
		...
		if (has_rule(GameConstants.GAME_RULE_CONTORL_START) && game_id == 9) {
	...
	public boolean handler_enter_room_observer(Player player) {
		...
		// 限制围观者数量，未来加到配置表控制
		if (player.getAccount_id() != getRoom_owner_account_id() && observers().count() >= (limitCount + this.getTablePlayerNumber())) {
		...
		observers().send(player, roomResponse);
		int game_id = SysGameTypeDict.getInstance().getGameIDByTypeIndex(_game_type_index);
		if (player.getAccount_id() == getRoom_owner_account_id() && game_id == 9) {
			if (this.is_game_start == 1) {
				control_game_start();
				return true;
			}
			...
			if ((_player_count >= 2) && (_player_count == _cur_count)) {
				if (this._cur_round == 0 && game_id == 9 && has_rule(GameConstants.GAME_RULE_CONTORL_START)) {
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status) && this.get_players()[seat_index] != null) {
			...
			// 游戏变量
			tableResponse.setSceneInfo(this._game_status);
	...
	public boolean gu_ding_add_jetton() {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this._add_Jetton[i] == 0) {
				...
				this._handler.handler_add_jetton(this, i, jetton);
			}
	...
	public boolean trustee_timer(int operate_id, int seat_index) {
		...
		if (operate_id == GameConstants.HJK_READY_TIMER) {
			...
			if (has_trustee(GameConstants.GAME_RULE_TG_NO_BNAKER, seat_index)) {
				this._handler.handler_call_banker(this, seat_index, 0);
			}
			if (has_trustee(GameConstants.GAME_RULE_TG_ONE_BEI, seat_index)) {
				this._handler.handler_call_banker(this, seat_index, 1);
			}
			if (has_trustee(GameConstants.GAME_RULE_TG_TWO_BEI, seat_index)) {
				this._handler.handler_call_banker(this, seat_index, 2);
			}
			if (has_trustee(GameConstants.GAME_RULE_TG_THREE_BEI, seat_index)) {
				this._handler.handler_call_banker(this, seat_index, 3);
			}
			if (has_trustee(GameConstants.GAME_RULE_TG_FOUR_BEI, seat_index)) {
				this._handler.handler_call_banker(this, seat_index, 4);
			}
			...
			if (is_mj_type(GameConstants.GAME_TYPE_JDOX_YY)) {
				...
				if (has_trustee(GameConstants.GAME_RULE_TG_IS_TUI_ZHU, seat_index) && this._can_tuizhu_player[seat_index] > 0) {
					this._handler.handler_add_jetton(this, seat_index, _cur_jetton_count[seat_index] - 1);
				}
				if (has_rule(GameConstants.GAME_RULE_ONE_TWO)) {
					if (has_trustee(GameConstants.GAME_RULE_TG_ONE_FEN, seat_index)) {
						this._handler.handler_add_jetton(this, seat_index, 0);
					}
					if (has_trustee(GameConstants.GAME_RULE_TG_TWO_FEN, seat_index)) {
						this._handler.handler_add_jetton(this, seat_index, 1);
					}
				}
				if (has_rule(GameConstants.GAME_RULE_TWO_FOUR)) {
					if (has_trustee(GameConstants.GAME_RULE_TG_TWO_FEN, seat_index)) {
			...
			if (this._player_status[seat_index] == false)
				return true;
			this._handler.handler_open_cards(this, seat_index, true);
		}
