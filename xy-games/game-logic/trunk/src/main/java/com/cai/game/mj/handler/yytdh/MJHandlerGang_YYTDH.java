...
public class MJHandlerGang_YYTDH extends MJHandlerGang<MJTable_YYTDH> {
    ...
    public void exe(MJTable_YYTDH table) {
        ...
        // 检查对这个杠有没有 胡
        boolean bAroseAction = table.estimate_gang_respond_yytdh(_seat_index, _center_card, _action);
    ...
    public boolean handler_operate_card(MJTable_YYTDH table, int seat_index, int operate_code, int operate_card) {
        ...
        if (operate_code == GameConstants.WIK_NULL) {
            ...
            table.process_chi_hu_player_operate_yytdh(seat_index, new int[] { _center_card }, 1, false);// 效果
        } else {
            table.log_player_error(seat_index, "出牌操作,没有动作");
            return false;
        }
        ...
        if (jie_pao_count > 0) {
            if (jie_pao_count > 1) {
                ...
                // 红中通炮就不加鸟了
                table.set_niao_card_yytdh(_provide_player, GameConstants.INVALID_VALUE, true, 0, true);// 结束后设置鸟牌
            } else if (jie_pao_count == 1) {
                table._cur_banker = target_player;

                table.set_niao_card_yytdh(target_player, GameConstants.INVALID_VALUE, true, 0, false);// 结束后设置鸟牌
            }
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if ((table.GRR._chi_hu_rights[i].is_valid() == false)) {// (i == _provide_player) ||
                    continue;
     ...
     ***/
    @Override
    protected boolean exe_gang(MJTable_YYTDH table) {
        ...
        } else if (GameConstants.GANG_TYPE_ADD_GANG == _type) {// 其他玩家给分
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if (i == _seat_index)
                    continue;
                int score = 1;
                 ...
                 * score;// m_pGameServiceOption->lCellScore*2;//配置参数乘
                 * table.GRR._gang_score[_seat_index].scores[cbGangIndex][table.GRR._weave_items
                 * [_seat_index][cbWeaveIndex].provide_player] = -score;//
                 * -m_pGameServiceOption->lCellScore*2;//配置参数乘
                 */ }
        ...
        if (_action == GameConstants.WIK_BU_ZHNAG) {
            ...
            // 从后面发两张牌给玩家
            table.exe_gang_yytdh(_seat_index, false);
        }
