...
public class MJHandlerPiao_KWX_XG extends AbstractMJHandler<Table_KWX_XG_2D> {
	...
	public boolean handler_pao_qiang(Table_KWX_XG_2D table, int seat_index, int pao, int qiang) {
		...
		if (table.has_rule(GameConstants_KWX.GAME_RULE_CHU_ZI) && table._cur_round == 1) {
			table.exe_handler_ch_zi();
			return true;
		}
	...
	private void player_reconnect(Table_KWX_XG_2D table, int seat_index) {
