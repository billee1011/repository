...
public class PHZHandlerDispatchFirstCard_CHD extends HHHandlerDispatchCard<HHTable> {
			...
			if(((ti_count == 2)&&( sao_count==4))||(ti_count >= 3)||(sao_count >= 5))
			{
	        	...
	        	if(all_hu_xi>=15)
	        	{
					...
					if (hong_pai_count >= 10 ) {
						table._hong_pai_count[i] = hong_pai_count;
					...
					{  
						table._tuan_yuan_count[i] =table._logic.calculate_tuan_yuan_count(table._hu_weave_items[i],table._hu_weave_count[i]);
