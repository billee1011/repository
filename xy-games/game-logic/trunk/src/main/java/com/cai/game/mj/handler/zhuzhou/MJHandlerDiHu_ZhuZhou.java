...
public class MJHandlerDiHu_ZhuZhou extends MJHandlerDiHu<MJTable> {
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			...
			table.GRR._chi_hu_rights[seat_index].opr_or(GameConstants.CHR_HUNAN_DI_HU);
			// 效果
		...
		for (int p = 0; p < GameConstants.GAME_PLAYER; p++) {
			...
			if (table._playerStatus[i].has_action()) {
				...
				// logger.info("cbUserActionRank:"+cbUserActionRank + "
				// cbTargetActionRank:"+cbTargetActionRank);
				// 动作判断 优先级最高的人和动作
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			if (jie_pao_count > 0) {
				if (jie_pao_count > 1) {
					...
					// MJGameConstants.GAME_PLAYER + 1+i) %
					// MJGameConstants.GAME_PLAYER;
					// if(huIndex[next_player]){
					// if(hu_player<0) {
					// hu_player=next_player;
					// //效果
					// table.process_chi_hu_player_operate(hu_player,
					// operate_card,false);
					// }else {
					...
					// 结束
					table._cur_banker = _out_card_player;// 出牌者坐庄
	...
	public boolean handler_player_out_card(MJTable table, int seat_index, int card) {
		...
		// 玩家出牌 响应判断,是否有吃碰杠补胡
		boolean bAroseAction = table.estimate_player_out_card_respond_zhuzhou(_out_card_player, _out_card_data);// ,
