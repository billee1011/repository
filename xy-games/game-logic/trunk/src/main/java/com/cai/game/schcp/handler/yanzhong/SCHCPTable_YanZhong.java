...
import com.cai.game.schcp.handler.yanzhong.SCHCPGameLogic_YanZhong.AnalyseItem;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class SCHCPTable_YanZhong extends SCHCPTable {
	/**
	 * 
	 */
	...
	private static Logger logger = Logger.getLogger(SCHCPTable_YanZhong.class);
	...
	public int _temp_peng_index[][]; // 碰变龙
	...
	public SCHCPGameLogic_YanZhong _logic = null;
	...
	public boolean _is_hu_seven_di;
	/**
	 * 牌桌上的组合扑克 (落地的牌组合)
	 */
	...
	public SCHCPTable_YanZhong() {

		_logic = new SCHCPGameLogic_YanZhong();
		...
		_temp_peng_index = new int[this.getTablePlayerNumber()][GameConstants.MAX_CP_INDEX];
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_dispath_card = new SCHCPHandlerDispatchCard_YanZhong();
		_handler_out_card_operate = new SCHCPHandlerOutCardOperate_YanZhong();
		_handler_gang = new SCHCPHandlerGang_YanZhong();
		_handler_chi_peng = new SCHCPHandlerChiPeng_YanZhong();
		_handler_dispath_add_card = new SCHCPHandlerDispatchAddCard_YanZhong();
		_handler_chuli_firstcards = new SCHCPHandlerChuLiFirstCard_YanZhong();
		_handler_dispath_firstcards = new SCHCPHandlerDispatchFirstCard_YanZhong();
		_handler_dispatch_toucards = new SCHCPHandlerDispatchTouCard_YanZhong();
		_handler_zhao = new SCHCPHandlerZhao_YanZhong();
		_handler_dispath_card_xiao = new SCHCPHandlerXiaoDispatchCard_YanZhong();
	...
	public void test_cards() {
		...
		// {0x20,0x66,0x12,0x0b,0x04,0x46,0x13,0x0a,0x05,0x09,0x50,0x45,0x43,0x43,0x43,0x42,0x42,0x42});
		if (BACK_DEBUG_CARDS_MODE) {
			if (debug_my_cards != null) {
				if (debug_my_cards.length > GameConstants.MAX_CP_COUNT) {
	...
	public int analyse_chi_hu_card(int cards_index[], WeaveItem weaveItems[], int weave_count, int seat_index, int provider_index, int cur_card,
			ChiHuRight chiHuRight, int card_type, boolean dispatch, boolean is_hu_seven) {
		if (this._ti_two_long[seat_index] == true) {
			return 0;
		}

		if (GameConstants.GAME_TYPE_CP_YANZH == _game_type_index) {
			return analyse_chi_hu_card_cp_yanzh(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type,
					dispatch, is_hu_seven);
		}
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_cp_yanzh(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index,
			int cur_card, ChiHuRight chiHuRight, int card_type, boolean dispatch, boolean is_hu_seven) {

		if ((is_hu_seven == false) && ((cur_card == 0x43 || cur_card == 0x16) && seat_index != this._xiao_jia)) {
		...
		if (seat_index == this._cur_banker) {
			...
			if ((is_hu_seven == false) && (temp_hu_xi >= 5 && dot == 7)) {
				chiHuRight.set_empty();
				return GameConstants.CP_WIK_NULL;
			} else if (temp_hu_xi < 5) {
				if (hei_pai_count + d_h_count != 6) {
					chiHuRight.set_empty();
					return GameConstants.CP_WIK_NULL;
				}
				if (hong_pai_count + d_h_count != 6) {
					chiHuRight.set_empty();
					return GameConstants.CP_WIK_NULL;
				}
			}
	...
	public int estimate_player_hua(int seat_index, int card_data) {
		...
		// {
		// if((GRR._weave_items[seat_index][i].weave_kind==GameConstants.DZ_WIK_PENG
		// ||GRR._weave_items[seat_index][i].weave_kind==GameConstants.CP_WIK_HUA
		// ||GRR._weave_items[seat_index][i].weave_kind==GameConstants.CP_WIK_CHE
		// ||GRR._weave_items[seat_index][i].weave_kind==GameConstants.CP_WIK_KAN)
		// && card_data == GRR._weave_items[seat_index][i].center_card){
		// cbWeaveIndex = i;
		// break;
		// }
		//
		...
		// if(cbWeaveIndex != -1)
		// {
		// this.exe_gang(seat_index, seat_index,
		// card_data,GameConstants.CP_WIK_LONG , GameConstants.CP_TYPE_HUA_LONG,
		// true, true, false, 1000);
		// bAroseAction = GameConstants.CP_WIK_LONG;
		// }
	...
	public int estimate_player_zhao(int seat_index, int card_data) {
		...
		int bAroseAction = GameConstants.CP_WIK_NULL;
		// if
		// (this._must_zhao_card[seat_index][_logic.switch_to_card_index(card_data)]
		// == 1) {
		// _playerStatus[seat_index].add_action(GameConstants.CP_WIK_ZHAO);
		// _playerStatus[seat_index].add_cp_zhao(card_data,
		// GameConstants.CP_WIK_ZHAO, seat_index);
		//
	...
	public boolean estimate_player_chi(int seat_index, int card, boolean bDisdatch) {
		...
		do {
			...
			next_index++;
			if (this._is_xiang_gong[chiseat_index] == true)
				continue;
			if (chiseat_index == this._xiao_jia)
				continue;
			playerStatus = this._playerStatus[chiseat_index];
			int chi_index = 0;
			if (this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_ZC_ZD) == 1) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (_playerStatus[i].lock_huan_zhang() == false) {
				...
				if (action != 0) {
					...
					// else if((card == 0x14||card ==
					// 0x16)&&this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_ZC_ZD)==1){
					// player_pass[i] = 0;
					// playerStatus.clean_action();
					// playerStatus.clean_status();
					// playerStatus.add_action(action);
					// playerStatus.add_cp_peng(card, GameConstants.CP_WIK_CHE,
					// seat_index);
					// bAroseAction = true;
					// }
					else {
	...
	public boolean no_card_out_game_end(int seat_index, int operate_card) {
		...
		// 显示出牌
		int next_player = (seat_index + getTablePlayerNumber() + 1) % getTablePlayerNumber();
		...
		//
		// this.operate_show_card(i, GameConstants.Show_Card_HU,
		// hand_card_count, cards, this.GRR._weave_items[i],
		...
		//
		// GameSchedule.put(new GameFinishRunnable(getRoom_id(), seat_index,
	...
	public void process_chi_hu_player_score_schcp(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		for (int i = 0; i < this._hu_weave_count[seat_index]; i++) {
			...
			case GameConstants.CP_WIK_LONG: {
				if (_logic.get_times_cards(this._hu_weave_items[seat_index][i].center_card) > 0) {
					wFanShu += 2;
				} else if (this._hu_weave_items[seat_index][i].center_card == 0x43 && this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_SSSF) == 1) {
					wFanShu += 2;
				} else
					wFanShu += 1;
				break;
		...
		if (seat_index == this._xiao_jia) {
			if (xiao_jia_hei_peng == 2)
				wFanShu = 1 + xiao_jia_hei_peng;
			else {
				wFanShu = xiao_jia_hei_peng;
			}
		...
		if (this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_SHKSF) == 1 && hei_peng_count >= 3)
			wFanShu += 1;
		...
		wFanShu += _logic.get_four_count(this._hu_weave_items[seat_index], this._hu_weave_count[seat_index],
				(this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_RYGY) == 1));
		if (zimo == false && this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_DP_JIA_1F) == 1)
			wFanShu++;
		if (this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_FD_5_F) == 1 && wFanShu > 5) {
			wFanShu = 5;
		} else if (this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_FD_4_F) == 1 && wFanShu > 4) {
			wFanShu = 4;
		} else if (this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_FD_3_F) == 1 && wFanShu > 3) {
			wFanShu = 3;
		}
		...
		if (zimo) {
			...
			// 胡牌分
			if (this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_DP_JIA_1F) == 1) {
				GRR._game_score[provide_index] -= lChiHuScore;
				GRR._game_score[seat_index] += lChiHuScore;
			} else {
				GRR._game_score[provide_index] -= lChiHuScore * (this.getTablePlayerNumber() - 1);
				GRR._game_score[seat_index] += lChiHuScore * (this.getTablePlayerNumber() - 1);
			}
	...
	public boolean operate_player_cards(int seat_index, int card_count, int cards[], int weave_count, WeaveItem weaveitems[]) {
		...
		int zhao_count = 0;
		for (int i = 0; i < GameConstants.MAX_CP_INDEX; i++) {
			cards_index[i] = this._zhao_card[seat_index][i];
			zhao_count += this._zhao_card[seat_index][i];
		}
		// 手牌数量
		roomResponse.setCardCount(card_count);
		roomResponse.setWeaveCount(weave_count);
		roomResponse.setOutCardCount(zhao_count);// 召牌数量
		// 组合牌
		if (weave_count > 0) {
			for (int j = 0; j < weave_count; j++) {
				WeaveItemResponse.Builder weaveItem_item = WeaveItemResponse.newBuilder();
		...
		for (int j = 0; j < card_count; j++) {
			if (cards_index[_logic.switch_to_card_index(cards[j])] > 0) {
				cards_index[_logic.switch_to_card_index(cards[j])]--;
				cards[j] |= 0x100;
			}
	...
	public int get_real_card(int seat_index, int card) {
		int card_index = _logic.switch_to_card_index(card & 0xff);

		if (this._zhao_card[seat_index][card_index] > 0) {
			this._zhao_card[seat_index][card_index]--;
			return card & 0xFF;
		}
		if (GRR._cannot_out_index[seat_index][card_index] > 0)
			return 0;
		return card & 0xFF;
	}
	...
	public void cannot_outcard(int seat_index, int card_data, int count, boolean send_client) {
		...
		if (this.getRuleValue(GameConstants.GAME_RULE_CP_YZ_LC_LD) == 1)
			return;
		if (GRR == null)
			return;// 最后一张
		if (seat_index == this._xiao_jia)
	...
	public void cannot_chicard(int seat_index, int card) {
		if (_logic.get_hong_dot(card) == 0) {
			...
			int temp_count = _logic.switch_to_value_to_card(_logic.get_dot(card), temp_cards);
			for (int i = 0; i < temp_count; i++) {
				if (this._cannot_chi_index[seat_index][_logic.switch_to_card_index(temp_cards[i])] == 0)
					this._cannot_chi_index[seat_index][_logic.switch_to_card_index(temp_cards[i])]++;
			}
		} else {
			int temp_cards[] = new int[3];
			int temp_count = _logic.switch_to_value_to_card(_logic.get_dot(card), temp_cards);
			for (int i = 0; i < temp_count; i++) {
				if (_logic.get_hong_dot(temp_cards[i]) != 0) {
					if (this._cannot_chi_index[seat_index][_logic.switch_to_card_index(temp_cards[i])] == 0)
						this._cannot_chi_index[seat_index][_logic.switch_to_card_index(temp_cards[i])]++;
				}
			}
	...
	public void cannot_hu_card(int seat_index, int card) {
		if (_logic.get_hong_dot(card) == 0) {
			int temp_cards[] = new int[3];
			int temp_count = _logic.switch_to_value_to_card(_logic.get_dot(card), temp_cards);
			for (int i = 0; i < temp_count; i++) {
				if (this._guo_hu_pai_cards[seat_index][_logic.switch_to_card_index(card)] == 0)
					this._guo_hu_pai_cards[seat_index][_logic.switch_to_card_index(card)]++;
			}
		} else {
			int temp_cards[] = new int[3];
			int temp_count = _logic.switch_to_value_to_card(_logic.get_dot(card), temp_cards);
	...
	public void cannot_pengcard(int seat_index, int card) {
		if (this._cannot_peng_index[seat_index][_logic.switch_to_card_index(card)] == 0)
			this._cannot_peng_index[seat_index][_logic.switch_to_card_index(card)]++;
		return;
	}
	...
	public void must_out_card() {
		...
		// return;// 最后一张
		//
		// boolean is_must_out_card[] = new
		// boolean[this.getTablePlayerNumber()];
		...
		// }
		// if (this._zhe_di_card == true && GRR._cards_index[i][20] == 1 &&
		// is_must_out_card[i] == false) {
		// is_must_out_card[i] = true;
		// cannot_outcard(i, _logic.switch_to_card_data(20), -1, true);
		// GRR._must_out_index[i][20]++;
		// }
		// }
		// if (GRR._cards_index[i][1] + GRR._cards_index[i][19] == 1 &&
		// is_must_out_card[i] == false) {
		// if (this._zhe_ding_card == true && GRR._cards_index[i][19] == 1 &&
		// is_must_out_card[i] == false) {
		// is_must_out_card[i] = true;
		// cannot_outcard(i, _logic.switch_to_card_data(19), -1, true);
		// GRR._must_out_index[i][19]++;
		// }
		// if (this._zhe_fu_card == true && GRR._cards_index[i][1] == 1 &&
		// is_must_out_card[i] == false) {
		// is_must_out_card[i] = true;
	...
	public void add_must_zhao(int seat_index, int card) {
		...
		// if (dot == 7)
		// is_zhao = true;
		// if (GRR._cards_index[seat_index][_logic.switch_to_card_index(card)]
		// == 1)
		// is_zhao = true;
		// if (is_zhao == true) {
		// if
	...
	public boolean is_zhao_card(int seat_index,int card){
		for(int i = 0; i<GRR._weave_count[seat_index];i++){
			if(GRR._weave_items[seat_index][i].weave_kind == GameConstants.CP_WIK_HUA||
					GRR._weave_items[seat_index][i].weave_kind == GameConstants.CP_WIK_KAN	)
			{
				if(GRR._weave_items[seat_index][i].center_card == card)
					return true;
	...
	public void add_zhao(int seat_index, int card) {
		...
		// }
		// if
		// (this._cannot_chi_index[seat_index][_logic.switch_to_card_index(card)]
		// == 1)
		// is_zhao = true;
		// if
		// (this._cannot_peng_index[seat_index][_logic.switch_to_card_index(card)]
		...
		// is_zhao = true;
		// if (is_zhao == true) {
		// if (this._zhao_card[seat_index][_logic.switch_to_card_index(card)] ==
		// 0)
