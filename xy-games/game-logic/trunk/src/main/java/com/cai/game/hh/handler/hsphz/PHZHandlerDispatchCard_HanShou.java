 ...
 *
 */
public class PHZHandlerDispatchCard_HanShou extends HHHandlerDispatchCard<HanShouHHTable> {
	...
	public void exe(HanShouHHTable table) {
		...
		// for (int p = 0; p < table.getTablePlayerNumber() &&
		// table.GRR._left_card_count == 0; p++) {
		// if
		// (table._playerStatus[p].has_action_by_code(GameConstants.WIK_ZI_MO))
		// {
		// handler_operate_card(table, p, GameConstants.WIK_ZI_MO,
		// _send_card_data, 0);
		// }
		// return;
		// }
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if (action_pao[i] != GameConstants.WIK_NULL && bHupai == 0) {
				...
				// GameConstants.INVALID_SEAT,
				// false);
				table.exe_gang(i, _seat_index, _send_card_data, action_pao[i], pao_type[i][0], true, true, false, 100);
				return;
		...
		if (ti_sao == GameConstants.WIK_PAO) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				for (int w = 0; w < table.GRR._weave_count[i]; w++) {
					if (table.GRR._weave_items[i][w].center_card == _send_card_data
							&& table.GRR._weave_items[i][w].weave_kind == GameConstants.WIK_WEI) {
						table._playerStatus[i].set_card_status(GameConstants.CARD_STATUS_CS_GANG);
					}
					if (table.GRR._weave_items[i][w].center_card != _send_card_data)
						continue;

					if (table.GRR._weave_items[i][w].weave_kind == GameConstants.WIK_PENG) {
						flag = true;
						break;
					}
		...
		// if (ti_sao != GameConstants.WIK_PAO && table.GRR._left_card_count !=
		// 0) {
		if (!flag && action_pao[_seat_index] != GameConstants.WIK_PAO && table.GRR._left_card_count != 0) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++)
			table._playerStatus[i].set_card_status(GameConstants.CARD_STATUS_NORMAL);
		...
		if (curPlayerStatus.has_action()) {// 有动作
			...
			// GameConstants.DELAY_AUTO_OUT_CARD_TRUTESS,
	...
	public boolean handler_operate_card(HanShouHHTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		case GameConstants.WIK_LEFT: // 上牌操作
		{
			int pao_type[][] = new int[table.getTablePlayerNumber()][1];
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				int pao = table.estimate_player_respond_phz(i, target_player, target_card, pao_type[i], true);
				if (pao != GameConstants.WIK_NULL) {
					table.exe_gang(i, target_player, target_card, pao, pao_type[i][0], true, true, false, 1000);
					return true;
				}
			}
			// 删除扑克
	 ...
	 */
	@Override
	public boolean handler_player_out_card(HanShouHHTable table, int seat_index, int card) {
		// 错误断言
		...
		if (table.GRR != null && table.GRR._cards_index[_seat_index][table._logic.switch_to_card_index(card)] >= 3) {
			// 刷新手牌
			int cards[] = new int[GameConstants.MAX_HH_COUNT];
			// 显示出牌
		...
		// 删除扑克
		if (table.GRR != null && table._logic.remove_card_by_index(table.GRR._cards_index[_seat_index], card) == false) {
