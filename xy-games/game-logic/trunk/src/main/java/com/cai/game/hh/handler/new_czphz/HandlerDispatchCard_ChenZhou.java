...
public class HandlerDispatchCard_ChenZhou extends HHHandlerDispatchCard<Table_New_ChenZhou> {
	...
	public void exe(Table_New_ChenZhou table) {
		...
		for (int p = 0; p < table.getTablePlayerNumber(); p++) {
			...
			action_hu[i] = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i], table.GRR._weave_count[i], i, _seat_index,
					table._send_card_data, chr[i], card_type, hu_xi_chi, true);

			action_pao[i] = table.estimate_player_respond_phz_chd(i, _seat_index, table._send_card_data, pao_type[i], true);
	...
	public boolean handler_operate_card(Table_New_ChenZhou table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		case GameConstants.WIK_NULL: {
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					int action = table.estimate_player_respond_phz_chd(i, _seat_index, table._send_card_data, pao_type, true);
					if (action != GameConstants.WIK_NULL) {
						table.exe_gang(i, _seat_index, table._send_card_data, action, pao_type[0], true, true, false, table.time_for_operate_dragon);
						return true;
					}
				}
				...
				table.exe_add_discard(_seat_index, 1, new int[] { table._send_card_data }, true, table.time_for_add_discard);
		...
		case GameConstants.WIK_PAO: {
			int pao_type[] = new int[1];
			int action = table.estimate_player_respond_phz_chd(target_player, _seat_index, table._send_card_data, pao_type, true);

			if (action != GameConstants.WIK_NULL) {
				table.exe_gang(target_player, _seat_index, table._send_card_data, action, pao_type[0], true, true, false,
						table.time_for_operate_dragon);
			}
