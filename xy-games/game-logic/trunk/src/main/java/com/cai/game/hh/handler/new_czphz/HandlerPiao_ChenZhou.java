...
public class HandlerPiao_ChenZhou extends HHHandler<Table_New_ChenZhou> {
	@Override
	public void exe(Table_New_ChenZhou table) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			roomResponse.setPaoDes("最多飘5分");
			table.send_response_to_player(i, roomResponse);
		}
	}

	public boolean handler_pao_qiang(Table_New_ChenZhou table, int seat_index, int pao, int qiang) {
	...
	private void player_reconnect(Table_New_ChenZhou table, int seat_index) {
