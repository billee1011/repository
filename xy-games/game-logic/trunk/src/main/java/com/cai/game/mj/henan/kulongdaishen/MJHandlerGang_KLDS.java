...
public class MJHandlerGang_KLDS extends MJHandlerGang<MJTable_KLDS> {
    @Override
     ...
     ***/
    @Override
    protected boolean exe_gang(MJTable_KLDS table) {
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                ...
                // -2*GameConstants.CELL_SCORE;// 暗杠，其他玩家扣分
                // table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] +=
                // 2*GameConstants.CELL_SCORE;//一共加分
            }
            ...
            // -1*GameConstants.CELL_SCORE;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
            ...
            // table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] =
            // GameConstants.CELL_SCORE;// m_pGameServiceOption->lCellScore*2;//配置参数乘
            // table.GRR._gang_score[_seat_index].scores[cbGangIndex][table.GRR._weave_items[_seat_index][cbWeaveIndex].provide_player]
            // = -1*GameConstants.CELL_SCORE;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
    ...
    public boolean handler_player_be_in_room(MJTable_KLDS table, int seat_index) {
        ...
        for (int j = 0; j < hand_card_count; j++) {
            if (table._logic.get_magic_card_count() > 0 && table._logic.is_magic_card(hand_cards[j])) {
