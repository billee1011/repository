...
import com.cai.game.wsk.WSKGameLogic_GF_BaWang;
...
public class WSKTable_GF_BaWang extends AbstractWSKTable {
	...
	public WSKGameLogic_GF_BaWang _logic;

	public WSKTable_GF_BaWang() {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		_logic = new WSKGameLogic_GF_BaWang();
		...
		_user_data_score = new int[this.getTablePlayerNumber()][get_hand_card_count_max()];
		...
		_handler_out_card_operate = new WSKHandlerOutCardOperate_GF_BaWang();
		_handler_call_banker = new WSKHandlerCallBnakerGF_BaWang();
	...
	protected boolean on_handler_game_start() {
		...
		_repertory_card = new int[WSKConstants.CARD_COUNT_WSK_GF_BA_WANG];
		shuffle(_repertory_card, WSKConstants.CARD_DATA_WSK_GF_BA_WANG);
		...
		if (!is_touxiang) {
			...
			if (!_is_yi_da_san) {
				...
				if (_friend_seat[_chuwan_shunxu[0]] == _chuwan_shunxu[1]) {
					...
					if (shang_you_score >= xia_you_score) {
						...
						if (shang_you_score == 200 && has_rule(GameConstants.GAME_RULE_WSK_GF_200_SCORE_FAN)) {
							score *= 3;
						} else {
							score *= 2;
						}
					} else {
						if (xia_you_score == 200 && has_rule(GameConstants.GAME_RULE_WSK_GF_200_SCORE_FAN)) {
							score *= 3;
						} else {
							score *= 2;
						}
					...
					if (shang_you_score >= xia_you_score) {
						...
						if (shang_you_score == 0) {
							score *= 3;
						} else if (xia_you_score == 200 && has_rule(GameConstants.GAME_RULE_WSK_GF_200_SCORE_FAN)) {
							score *= 3;
						} else {
							score *= 2;
						}
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				if (_fei_wang_count[i] > 0) {
					_fei_wang_count[i] = _fei_wang_count[i] > 6 ? 6 : _fei_wang_count[i];
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				int xi_qian_score = _logic.GetHandCardXianScore(GRR._cards_data[i], GRR._card_count[i],
						sheng_dang_biaozhi, _user_data.get(i), this._user_data_score[i]);
	...
	public void cal_score_mian_da(int end_score[], int dang_ju_fen[], int jia_fa_socre[]) {
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			int score = _logic.GetHandCardXianScore(GRR._cards_data[i], GRR._card_count[i], sheng_dang_biaozhi,
					_user_data.get(i), this._user_data_score[i]);
			for (int j = 0; j < getTablePlayerNumber(); j++) {
				if (i == j) {
					continue;
				}
	...
	protected void test_cards() {
		...
		int cards[] = new int[] { 40, 49, 60, 3, 59, 23, 42, 23, 11, 26, 36, 27, 29, 2, 26, 35, 17, 50, 58, 56, 4, 53,
				1, 61, 38, 24, 45, 8, 78, 21, 10, 7, 28, 12, 6, 43, 79, 2, 20, 13, 4, 79, 37, 25, 7, 79, 41, 34, 55, 44,
				50, 40, 45, 28, 33, 44, 29, 24, 61, 39, 18, 9, 3, 6, 54, 58, 22, 5, 17, 51, 33, 38, 1, 27, 20, 51, 22,
				42, 9, 41, 78, 78, 19, 39, 52, 36, 56, 25, 79, 53, 11, 52, 54, 55, 78, 5, 13, 12, 34, 59, 57, 43, 57,
				10, 8, 49, 19, 18, 35, 21, 60, 37 };
		int index = 0;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
