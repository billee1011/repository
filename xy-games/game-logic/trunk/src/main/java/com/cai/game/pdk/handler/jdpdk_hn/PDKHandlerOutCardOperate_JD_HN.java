...
public class PDKHandlerOutCardOperate_JD_HN extends PDKHandlerOutCardOperate<PDK_JD_Table_HN> {
	@Override
	public void exe(PDK_JD_Table_HN table) {
		if (_out_card_player != table._current_player || table.isPauseGame) {
			return;
		}
		...
		if (cbCardType == GameConstants.PDK_CT_BOMB_CARD) {
			...
			if (table.has_rule(GameConstants.GAME_RULE_CAL_SCORE_ONE)) {
				table._boom_times *= 2;
				if (table.has_rule(GameConstants.GAME_RULE_BOOM_LIMIT_ONE)) {
					table._boom_times = 2;
				} else if (table.has_rule(GameConstants.GAME_RULE_BOOM_LIMIT_TWO)) {
					if (table._boom_times > 4) {
						table._boom_times = 4;
					}
				} else if (table.has_rule(GameConstants.GAME_RULE_BOOM_LIMIT_THREE)) {
					if (table._boom_times > 8) {
						table._boom_times = 8;
					}
				} else {
					table._boom_times = 1;
		...
		if (0 == table.GRR._card_count[_out_card_player]) {
			int delay = 1;
			if (cbCardType == GameConstants.PDK_CT_BOMB_CARD && table.has_rule(GameConstants.GAME_RULE_CAL_SCORE_TWO)) {
	...
	@Override
	public boolean handler_player_be_in_room(PDK_JD_Table_HN table, int seat_index) {
	...
	@Override
	public boolean handler_god_observer_be_in_room(PDK_JD_Table_HN table, Player player) {
		...
		roomResponse.setCommResponse(PBUtil.toByteString(tableResponse_pdk));
		table.send_response_to_god_player(player, roomResponse);

		return true;
	}

	// 玩家不出
	public void no_out_card(PDK_JD_Table_HN table) {
		int can_out_card_data[] = new int[table.get_hand_card_count_max()];
	...
	// 判断玩家出牌合法
	public int adjust_out_card_right(PDK_JD_Table_HN table) {
		...
		if (cbCardType == GameConstants.PDK_CT_FOUR_LINE_TAKE_ONE) {
			if (!table.has_rule(GameConstants.GAME_RULE_FOUR_TAKE_ONE)) {
				table.send_error_notify(_out_card_player, 2, "请选择正确的牌型!");
				return GameConstants.PDK_CT_ERROR;
			}
		}
		if (cbCardType == GameConstants.PDK_CT_THREE || cbCardType == GameConstants.PDK_CT_PLANE_LOST) {
			if (table._turn_out_card_count != 0) {
				table.send_error_notify(_out_card_player, 2, "请选择正确的牌型!");
				return GameConstants.PDK_CT_ERROR;
			}
		...
		if (table.GRR._cur_round == 0 && table._out_card_times[_out_card_player] == 0
			...
			for (int i = 1; i <= _out_card_count; i++) {
				if (_out_cards_data[_out_card_count - i] == 0x23) {
					break;
				}
				if (i == _out_card_count) {
					table.send_error_notify(_out_card_player, 2, "首出请出红桃3!");
					return GameConstants.PDK_CT_ERROR;
				}
			}
		}
	...
	// 放走包赔
	public boolean fang_zou_bao_pei(PDK_JD_Table_HN table) {
	...
	// 必须出最大牌
	public boolean mustmax(PDK_JD_Table_HN table) {
	...
	// 自动出牌
	public void adjustAutoOutCard(PDK_JD_Table_HN table, int seat_index) {
		int auto_out_card_dely = 1;
		if (table.matchId != 0) {
			auto_out_card_dely = 2;
		}
