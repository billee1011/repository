...
public class LHQHandlerGang_HD extends HHHandlerGang<HHTable> {
...
//		}
			...
			{    
				...
				if(pai_count == 0)
				{
					...
					if (table.isTrutess(_seat_index)) {
						for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
							if (table.GRR._cards_index[_seat_index][i] > 0 && table.GRR._cards_index[_seat_index][i] < 3 
									&& table._logic.switch_to_card_data(i) != this._center_card) {
								table.handler_player_out_card(_seat_index, table._logic.switch_to_card_data(i));
								break;
							}
						}
					}
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table._shang_zhuang_player = target_player;
			table.process_chi_hu_player_score_lhq(target_player, this._provide_player, operate_card, false);
			table.process_chi_hu_player_operate_lhq(target_player, operate_card, false);
			

		
			table.countChiHuTimes_lhq(target_player,  _seat_index,false);
...
//			}
			...
			table.countChiHuTimes_lhq(target_player,_seat_index, true);
	 ...
	 * 
	 ***/
	protected boolean exe_gang(HHTable table) {
		...
		if(table._long_count[_seat_index] <=1&&_action != GameConstants.WIK_WEI)
		{
			for(int i = 0; i< GameConstants.MAX_HH_INDEX;i++)
			{
				if(table.GRR._cards_index[_seat_index][i] == 4)
		...
		if((_depatch == true)&&(table._ti_two_long[_seat_index] == false)&&(table._is_xiang_gong[_seat_index] == false)){
			...
			if(is_di_hu == true)
			{
				for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
					...
					if (!((cbWeaveKind == GameConstants.WIK_TI_LONG)||
							...
							(cbWeaveKind == GameConstants.WIK_WEI)||(cbWeaveKind == GameConstants.WIK_PAO))) {
						is_di_hu = false;
					}
			...
			if(action_hu != GameConstants.WIK_NULL){
				...
				if(table.has_rule(GameConstants.GAME_RULE_QIANG_ZHI_HU_PAI) || table.isTrutess(_seat_index))
				{
					int hucard_time = 600;
	        ...
	        if(pai_count == 0)
	        {
	        	...
	        	if(all_hu_xi>=max_hu_xi)
	        	{
					...
					if (table.has_rule(GameConstants.GAME_RULE_HONG_HEI_DIAN)) {
						...
						if (table.is_mj_type(GameConstants.GAME_TYPE_LHQ_QD)||table.is_mj_type(GameConstants.GAME_TYPE_LHQ_QD_SAPP)) {
							if (hong_pai_count >= 8) {
					  ...
					  if (table.has_rule(GameConstants.GAME_RULE_HAI_DI_LAO_YUE) && table.GRR._left_card_count == 0) {
			...
			if(is_di_hu == true)
			{
				for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
					...
					if (!((cbWeaveKind == GameConstants.WIK_TI_LONG)||
							...
							(cbWeaveKind == GameConstants.WIK_PAO)) {
						is_di_hu = false;
					}
					if(table.GRR._weave_count[_seat_index] >1 || _seat_index == table.GRR._banker_player)
						is_di_hu = false;
