...
public class HandlerCallBnaker_XTXZ extends AbstractHandler_XTXZ {
	@Override
	 ...
	 * @return
	 */
	public boolean handler_call_banker(Table_XTXZ table, int seat_index, int call_action) {
		...
		if (call_action == 0) {
			...
			if (is_finish) {
				// 所有人已经操作了‘不独’之后，开始叫牌
				table._game_status = GameConstants.GS_XTXZ_WSK_LIANG_PAI;
		...
		}else if(call_action == 2){//投降
			
			for(int i = 0;i < table.getTablePlayerNumber(); i++){
				if(i == seat_index){
					continue;
				}
				table.jiang_fa_socre[i] += 50;
				table.jiang_fa_socre[seat_index] -= 50;
		...
		callbanker_result.setHaveTouQiang(table.have_tou_qiang);
		...
		if (table._game_status == GameConstants.GS_XTXZ_WSK_LIANG_PAI) {
