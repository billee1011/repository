...
import protobuf.clazz.czwxox.czwxoxRsp.ButtonOperateCzwxox;
import protobuf.clazz.czwxox.czwxoxRsp.GameStartCzwxox;
import protobuf.clazz.czwxox.czwxoxRsp.JettonResultCzwxox;
import protobuf.clazz.czwxox.czwxoxRsp.OpenCardCzwxox;
import protobuf.clazz.czwxox.czwxoxRsp.Opreate_Czwxox_Request;
import protobuf.clazz.czwxox.czwxoxRsp.PlayerResultCzwxox;
import protobuf.clazz.czwxox.czwxoxRsp.PukeGameEndCzwxox;
import protobuf.clazz.czwxox.czwxoxRsp.RoomPlayerResponseCzwxox;
import protobuf.clazz.czwxox.czwxoxRsp.SelectdBankerCzwxox;
import protobuf.clazz.czwxox.czwxoxRsp.SelectdBankerResultCzwxox;
import protobuf.clazz.czwxox.czwxoxRsp.SendCardCzwxox;
import protobuf.clazz.czwxox.czwxoxRsp.TableResponseCzwxox;

///////////////////////////////////////////////////////////////////////////////////////////////
public class CZWXOXTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	private static Logger logger = Logger.getLogger(CZWXOXTable.class);
	...
	public CZWXOXGameLogic _logic = null;
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		int min_sit = 2;
		if (this.getRuleValue(GameConstants.GAME_RULE_WX_FOUR_START) == 1)
			min_sit = 4;
		else if (this.getRuleValue(GameConstants.GAME_RULE_WX_FIVE_START) == 1)
			min_sit = 5;
		else if (this.getRuleValue(GameConstants.GAME_RULE_WX_SIX_START) == 1)
			min_sit = 6;
		else
			min_sit = 2;
		this.setMinPlayerCount(min_sit); // 郴州五小牛最小开局人数为2
	...
	protected boolean on_handler_game_start() {
		...
		if (this.getRuleValue(GameConstants.GAME_RULE_WX_ROB_FOUR) == 1)
			this._banker_times = 4;
		else if (this.getRuleValue(GameConstants.GAME_RULE_WX_ROB_THREE) == 1)
			this._banker_times = 3;
		else if (this.getRuleValue(GameConstants.GAME_RULE_WX_ROB_TWO) == 1)
			this._banker_times = 2;
		else
			this._banker_times = 1;

		if (this.getRuleValue(GameConstants.GAME_RULE_WX_TUI_ZHU_TEN) == 1)
			this._tui_zhu_times = 10;
		else if (this.getRuleValue(GameConstants.GAME_RULE_WX_TUI_ZHU_FIVE) == 1)
			this._tui_zhu_times = 5;
		else if (this.getRuleValue(GameConstants.GAME_RULE_WX_TUI_ZHU_THREE) == 1)
			this._tui_zhu_times = 3;
		else if (this.getRuleValue(GameConstants.GAME_RULE_WX_TUI_ZHU_TWO) == 1)
			this._tui_zhu_times = 2;
		else
			this._tui_zhu_times = 0;
		for (int j = 0; j <= this._banker_times; j++)
			this._call_banker_info[j] = j;
		if (this.getRuleValue(GameConstants.GAME_RULE_WX_JETTON_FOUR) == 1) {
			_jetton_count_max = 3;
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
		...
		} else if (this.getRuleValue(GameConstants.GAME_RULE_WX_JETTON_THREE) == 1) {
			_jetton_count_max = 4;
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				Arrays.fill(this._jetton_score[i], 0);
				this._jetton_count[i] = _jetton_count_max;
		...
		} else if (this.getRuleValue(GameConstants.GAME_RULE_WX_JETTON_TWO) == 1) {
			_jetton_count_max = 3;
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				Arrays.fill(this._jetton_score[i], 0);
				this._jetton_count[i] = _jetton_count_max;
				this._jetton_score[i][0] = 2;
				this._jetton_score[i][1] = 3;
				this._jetton_score[i][2] = 5;
				if (this._can_tuizhu_player[i] > 0) {
					this._jetton_score[i][this._jetton_count[i]++] = this._can_tuizhu_player[i];
				}
			}
		...
		return game_start_Czwxox();
	}

	// 斗板牛开始
	public boolean game_start_Czwxox() {
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			...
			roomResponse.setType(MsgConstants.RESPONSE_CSWOXOX_GAME_START);
			roomResponse.setGameStatus(this._game_status);
			// 发送数据
			GameStartCzwxox.Builder gamestart = GameStartCzwxox.newBuilder();
	...
	public boolean update_button(int opr_type, int seat_index, int display_time, boolean is_grr) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_CSWOXOX_UPDATE_BTN);
		ButtonOperateCzwxox.Builder button_operate = ButtonOperateCzwxox.newBuilder();
		...
		if (opr_type == 2) {
			...
			// boolean is_ox = _logic.get_ox_card(temp_cards,
			// GameConstants.OX_MAX_CARD_COUNT, ruleMap);
			// if(is_ox == true)
			// {
			// for(int i = 0; i<3;i++)
			// {
	...
	public boolean send_call_banker_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_CSWOXOX_ROBOT_BANKER);
		SelectdBankerCzwxox.Builder robot_banker = SelectdBankerCzwxox.newBuilder();
	...
	public boolean set_banker_to_room() {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_CSWOXOX_ROBOT_BANKER_RESULT);
		SelectdBankerResultCzwxox.Builder robot_banker = SelectdBankerResultCzwxox.newBuilder();
	...
	public void send_jetton_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_CSWOXOX_JETTON);
		JettonResultCzwxox.Builder jetton_result = JettonResultCzwxox.newBuilder();
	...
	public boolean send_card(int card_count, int opr_type) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_CSWOXOX_SEND_CARD);
		SendCardCzwxox.Builder send_card = SendCardCzwxox.newBuilder();
			...
			for (int k = 0; k < this.getTablePlayerNumber(); k++) {
				...
				if (this._player_status[k] == false) {
					...
					if (card_count == GameConstants.OX_MAX_CARD_COUNT) {
						if (this.getRuleValue(GameConstants.GAME_RULE_WX_TIMES_TWO) == 1)
							this._player_times[i] = _logic.get_times_two(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
						if (this.getRuleValue(GameConstants.GAME_RULE_WX_TIMES_ONE) == 1)
	...
	public boolean open_card(int seat_index) {
		...
		// if (has_rule(GameConstants.GAME_RULE_WX_TIMES_TWO))
		// this._player_times[seat_index] =
		// _logic.get_times_two(GRR._cards_data[seat_index],
		// GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
		// if (has_rule(GameConstants.GAME_RULE_WX_TIMES_ONE))
		// this._player_times[seat_index] =
		// _logic.get_times_one(GRR._cards_data[seat_index],
		// GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
		//
		// this._ox_value[seat_index] =
		// _logic.get_card_type(GRR._cards_data[seat_index],
		// GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
		// this._str_ox_value[seat_index] =
		// _logic.get_card_ox_value(this._ox_value[seat_index]);
	...
	public void process_ox_calulate_end() {
		...
		if (this.getRuleValue(GameConstants.GAME_RULE_WX_NNSZ) == 1) {
			int _win_player_oxtb = process_tbox_calulate_end();
			int card_type = _logic.get_card_type(GRR._cards_data[_win_player_oxtb], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
			if (card_type >= GameConstants.CZWXOX_JIA_TEN)
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				if (this._can_tuizhu_player[i] == 0) {
					if (GRR._game_score[i] > 0) {
						if (this._tui_zhu_times * this._jetton_player[i] > this._jetton_score[i][this._jetton_count_max - 1])
	...
	public boolean send_open_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_CSWOXOX_OPEN_CARD);
		OpenCardCzwxox.Builder open_card = OpenCardCzwxox.newBuilder();
	...
	private void test_cards() {
		...
		// 0x15,0x09,0x31,0x03,0x04,0x17,0x07,0x27,0x33,0x24
		// };
	...
	public RoomPlayerResponseCzwxox.Builder load_player_info_data(int seat_index) {
		Player rplayer = this.get_players()[seat_index];
		if (rplayer == null)
			return null;
		RoomPlayerResponseCzwxox.Builder room_player = RoomPlayerResponseCzwxox.newBuilder();
		room_player.setAccountId(rplayer.getAccount_id());
		room_player.setHeadImgUrl(rplayer.getAccount_icon());
		room_player.setIp(rplayer.getAccount_ip());
		room_player.setUserName(rplayer.getNick_name());
	...
	public void load_player_info_data_game_start(GameStartCzwxox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_game_end(PukeGameEndCzwxox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_reconnect(TableResponseCzwxox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		ret = this.handler_game_finish_Czwxox(seat_index, reason);

		return ret;
	}

	public boolean handler_game_finish_Czwxox(int seat_index, int reason) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_CSWOXOX_GAME_END);
		GameEndResponse.Builder game_end = GameEndResponse.newBuilder();
		PukeGameEndCzwxox.Builder game_end_czwxox = PukeGameEndCzwxox.newBuilder();
		RoomInfo.Builder room_info = getRoomInfo();
		game_end_czwxox.setRoomInfo(room_info);
		...
		this.load_player_info_data_game_end(game_end_czwxox);
		game_end_czwxox.setGameRound(_game_round);
		game_end_czwxox.setCurRound(_cur_round);
		game_end_czwxox.setDisplayTime(this._idi_ready_card_time);
		if (GRR != null) {
			this.operate_player_data();
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				game_end.addGameScore(GRR._game_score[i]);
				game_end_czwxox.addEndScore((int) GRR._game_score[i]);
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
				for (int j = 0; j < GameConstants.OX_MAX_CARD_COUNT; j++) {
					cards_card.addItem(GRR._cards_data[i][j]);
				}
				game_end_czwxox.addCardsData(cards_card);
				game_end_czwxox.addStrOxValue(this._str_ox_value[i]);
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if (_cur_round >= _game_round) {// 局数到了
				...
				game_end_czwxox.setPlayerResult(this.process_player_result_Czwxox(reason));
		...
		game_end_czwxox.setWinner(seat_index);
		game_end_czwxox.setReason(real_reason);
		////////////////////////////////////////////////////////////////////// 得分总的
		game_end.setEndType(real_reason);
		game_end.setGameRound(_game_round);
		game_end.setCurRound(_cur_round);
		game_end.setCommResponse(PBUtil.toByteString(game_end_czwxox));
	...
	public boolean handler_enter_room(Player player) {
		...
		// && GameDescUtil.has_rule(gameRuleIndexEx,
		// GameConstants.GAME_RULE_JDOX_NOKEJIN)) {
		//
		// boolean flag = false;
		// for (int i = 0; i < getTablePlayerNumber(); i++) {
		// if (this._player_status[i] == true)
		...
		// }
		// } else {
		if (this.getRuleValue(GameConstants.GAME_RULE_WX_FORBIN_JOIN_GAME) == 1) {
	...
	public boolean handler_player_ready(int seat_index, boolean is_cancel) {
		...
		if (this.getRuleValue(GameConstants.GAME_RULE_WX_FOUR_START) == 1)
			...
			min_sit = 2;
		if ((_player_count >= min_sit) && (_player_count == _cur_count))
			handler_game_start();
	...
	public boolean table_response_observer(Player player) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_CSWOXOX_RECONNECT_DATA);

		TableResponseCzwxox.Builder tableResponse_Czwxox = TableResponseCzwxox.newBuilder();
		load_player_info_data_reconnect(tableResponse_Czwxox);
		RoomInfo.Builder room_info = getRoomInfo();
		tableResponse_Czwxox.setRoomInfo(room_info);

		tableResponse_Czwxox.setBankerPlayer(_cur_banker);
		int display_time = _cur_operate_time - ((int) (System.currentTimeMillis() / 1000L) - _operate_start_time);
		tableResponse_Czwxox.setDisplayTime(display_time);

		for (int i = 0; i < getTablePlayerNumber(); i++) {
			if ((this.getRuleValue(GameConstants.GAME_RULE_WX_GDZJ) == 1 || this.getRuleValue(GameConstants.GAME_RULE_WX_NNSZ) == 1)
					&& this._cur_round != 1) {
				if (i == this._next_banker)
					tableResponse_Czwxox.addCallButton(this._banker_times);
			} else
				tableResponse_Czwxox.addCallButton(this._player_call_banker[i]);
			tableResponse_Czwxox.addJettonScore(this._jetton_player[i]);
			tableResponse_Czwxox.addOpenCard(this._open_card_player[i]);
			if (this._open_card_player[i] == true) {
				tableResponse_Czwxox.addOxValue(this._ox_value[i]);
				tableResponse_Czwxox.addTimes(this._player_times[i]);
			} else {
				tableResponse_Czwxox.addOxValue(-1);
				tableResponse_Czwxox.addTimes(0);
			}
		}
		tableResponse_Czwxox.setSceneStatus(_game_status);
		if (_game_status == GameConstants.GS_OX_CALL_BANKER) {

			tableResponse_Czwxox.setBankerPlayer(-1);
			int card_count = 4;
			if (has_rule(GameConstants.GAME_RULE_WX_MPQZ)) {
				for (int k = 0; k < this.getTablePlayerNumber(); k++) {
					Int32ArrayResponse.Builder cards = Int32ArrayResponse.newBuilder();
					if (this._player_status[k] == false) {
						for (int j = 0; j < card_count; j++) {
					...
					tableResponse_Czwxox.addCardsData(k, cards);
				}
			}
		...
		roomResponse.setCommResponse(PBUtil.toByteString(tableResponse_Czwxox));
			...
			if (_game_status == GameConstants.GS_OX_OPEN_CARD) {
				...
				for (int k = 0; k < this.getTablePlayerNumber(); k++) {
					...
					if (this._player_status[k] == false) {
						...
						if (k == seat_index) {
							tableResponse_Czwxox.setCurOxValue(this._ox_value[seat_index]);
							tableResponse_Czwxox.setCurTimes(this._player_times[seat_index]);
						}
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		if (type == MsgConstants.REQUST_CZWXOX_OPERATE) {

			Opreate_Czwxox_Request req = PBUtil.toObject(room_rq, Opreate_Czwxox_Request.class);
			// 逻辑处理
	...
	public boolean handler_release_room(Player player, int opr_code) {
		...
		case GameConstants.Release_Room_Type_QUIT: {// 玩家未开始游戏 退出
			...
			if (this.getRuleValue(GameConstants.GAME_RULE_WX_FOUR_START) == 1)
				...
				min_sit = 2;
			if ((player_count >= min_sit) && (player_count == _cur_count))
				handler_game_start();
			// 通知代理
	...
	private PlayerResultCzwxox.Builder process_player_result_Czwxox(int reason) {
		int win_idx = 0;
		float max_score = 0;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			int winner = -1;
		...
		PlayerResultCzwxox.Builder player_result = PlayerResultCzwxox.newBuilder();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public int getTablePlayerNumber() {
		if (this.getRuleValue(GameConstants.GAME_RULE_WX_TEN_PLAYER) == 1)
			return 10;
		else if (this.getRuleValue(GameConstants.GAME_RULE_WX_EIGHT_PLAYER) == 1)
			return 8;
		return 6;
	}
	...
	public boolean animation_timer(int timer_id) {
		...
		case ID_TIMER_ANIMATION_START: {
			if (has_rule(GameConstants.GAME_RULE_WX_NNSZ)) // 牛牛上庄不是轮庄
			{
			...
			} else if (has_rule(GameConstants.GAME_RULE_WX_GDZJ)) {
				if (this._cur_round == 1) {
				...
				if (this._game_scheduled != null)
					this.kill_timer();
				this.set_timer(GameConstants.HJK_ADD_JETTON_TIMER, _idi_jetton_score_time, true);
			} else if (has_rule(GameConstants.GAME_RULE_WX_MPQZ)) {
				send_card(4, 0);
				this._game_status = GameConstants.GS_OX_CALL_BANKER;
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					if (this._player_status[i] == false)
			...
			} else if (has_rule(GameConstants.GAME_RULE_WX_ZYQZ)) {
				this._game_status = GameConstants.GS_OX_CALL_BANKER;
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					if (this._player_status[i] == false)
						continue;
