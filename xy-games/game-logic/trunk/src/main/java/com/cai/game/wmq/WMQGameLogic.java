...
public class WMQGameLogic {

	private static Logger logger = Logger.getLogger(WMQGameLogic.class);
   ...
   public boolean check_lou_weave(int cards_index[],int cur_card,int action,int type_count[],int type_eat_count,LouWeaveItem lou_weave_item[],
    ...
    public int calculate_weave_dui_zi_hu_count(WeaveItem weaveItems[],int  weaveCount)
    {
    	int dui_zi_count = 0;
    	for (int j = 0; j < 7; j++) {
			if (weaveItems[j].weave_kind == GameConstants.WIK_NULL)
				break;
			switch(weaveItems[j].weave_kind)
			{
			case GameConstants.WIK_KAN:
	      	case GameConstants.WIK_WEI:
	    	case GameConstants.WIK_CHOU_WEI:
    ...
    public boolean is_hua_man_yuan(AnalyseItem analyseItem){
    	...
    	if(analyseItem.cbCardEye!=0)
    	{
    		if(color_hei(analyseItem.cbCardEye)==false)
    			hong_zi_count++;
    	}
    	if(hong_zi_count == 7)
    ...
    public int  get_analyse_tuan_yuan(AnalyseItem analyseItem,int cbAnalyseIndexTemp[],int tuan_yuan[],boolean is_hong)
    {
    	int count = 0;
    	for(int j = 0; j < 7; j++)
    	{
	    	...
	    	case GameConstants.WIK_CHOU_WEI:
	    	case GameConstants.WIK_PENG:
	    	{	if(cbAnalyseIndexTemp[this.switch_to_card_index(analyseItem.cbCenterCard[j])]==4)
	    		{
	    			if(color_hei(analyseItem.cbCenterCard[j]) == is_hong)
	    			{
	    				tuan_yuan[this.switch_to_card_index(analyseItem.cbCenterCard[j])] = 1;
		    			count++;
	    			}
    ...
    public int  get_weave_tuan_yuan(WeaveItem weaveItems[],int  weaveCount,int cbAnalyseIndexTemp[],int tuan_yuan[],boolean is_hong)
    {
    	int count = 0;
     	for (int j = 0; j < 7; j++) {
			if (weaveItems[j].weave_kind == GameConstants.WIK_NULL)
	    	...
	    	case GameConstants.WIK_CHOU_WEI:
	    	case GameConstants.WIK_PENG:
	    	{	if(cbAnalyseIndexTemp[this.switch_to_card_index(weaveItems[j].center_card)]==4)
	    		{
	    			if(color_hei(weaveItems[j].center_card) == is_hong)
	    			{
	    				tuan_yuan[this.switch_to_card_index(weaveItems[j].center_card)] = 1;
		    			count++;
	    			}
    ...
    public boolean is_hua_man_yuan_weave(WeaveItem weaveItems[],int  weaveCount){
    	boolean is_hua_man_yuan  = false;
    	int hong_zi_count = 0;
    	for (int j = 0; j < 7; j++) {
			if (weaveItems[j].weave_kind == GameConstants.WIK_NULL)
	    	...
	    	case GameConstants.WIK_CHOU_WEI:
	    	case GameConstants.WIK_PENG:
	    	{	if(color_hei(weaveItems[j].center_card)==false)
	    			hong_zi_count++;
	    			break;
	    	}
	    	case GameConstants.WIK_LEFT:
	    	{
    			...
    			if(color_hei(weaveItems[j].center_card+1)==false)
	    			hong_zi_count++;
	    		
    			if(color_hei(weaveItems[j].center_card+2)==false)
	    			hong_zi_count++;
			    break;
	    	} 
	    	case GameConstants.WIK_CENTER:
    ...
    public void weave_item_to_card(WeaveItem weave_items[],int weave_count,int cbAnalyseIndexTemp[])
    {
    	for (int j = 0; j < weave_count; j++) {
	    	...
	    	case GameConstants.WIK_PENG:
	    	{
	    		int card_index = this.switch_to_card_index(weave_items[j].center_card);
    ...
    public boolean is_jie_mei_zhuo_hu(int cbAnalyseIndexTemp[])
    {
    	...
    	for(int i = 0; i< GameConstants.MAX_HH_INDEX; i++)
    	{
    		if(cbAnalyseIndexTemp[i] == 1)
    			{
    				ying_hu_count ++;
    				if(i>0&&cbAnalyseIndexTemp[i-1]==1&&i-1!=9)
    					is_jie_mei = true;	
    			}
    		
    	}
       if(is_jie_mei  == true&&ying_hu_count==2)
    	   return true;
       else
    	   return false;
    ...
    public boolean is_die_shun_zhuo_hu(int cbAnalyseIndexTemp[])
    {
    	...
    	for(int i = 0; i< GameConstants.MAX_HH_INDEX; i++)
    	{
    		if(cbAnalyseIndexTemp[i] == 1)
    			{
    				ying_hu_count++;
    				if(i>1&&cbAnalyseIndexTemp[i-1]==1&&cbAnalyseIndexTemp[i-2]==1&&(!(i-1==9||i-2==9)))
    					is_die_shun = true;	
    			}
    		
    	}
       if(is_die_shun  == true&&ying_hu_count ==3)
    	   return true;
       else
    	   return false;
    	
    }
    public boolean is_zu_sun_zhuo_hu(int cbAnalyseIndexTemp[])
    {

    	int ying_hu_count = 0;
    	boolean  is_zu_sun = false;
    	for(int i = 0; i< GameConstants.MAX_HH_INDEX; i++)
    	{
    		if(cbAnalyseIndexTemp[i] == 1)
    			{
    				ying_hu_count ++;
    				if(i>2&&cbAnalyseIndexTemp[i-1]==1&&cbAnalyseIndexTemp[i-2]==1&&cbAnalyseIndexTemp[i-3]==1&&(!(i-1==9||i-2==9||i-3==9)))
    					is_zu_sun = true;	
    			}
    		
    	}
       if(is_zu_sun  == true &&ying_hu_count >= 4)
    	   return true;
       else
    	   return false;
    }
    public boolean is_two_jie_mei_zhuo(int cbAnalyseIndexTemp[])
    {
    ...
    public boolean is_zhen_ba_peng_tou(int tuan_yuan[])
    {
     	...
     	for(int i = 0; i<GameConstants.MAX_HH_INDEX;i++)
     	{
     		if(tuan_yuan[7]== 1&& tuan_yuan[17] == 1)
     		{
     			is_zhen_ba = true;
     		}
     	}
    ...
    public boolean is_jia_ba_peng_tou( int tuan_yuan[])
    {
    	...
    	for(int i = 0; i< GameConstants.MAX_HH_INDEX; i++)
    	{
    		if(i!= 7&&i<10&&tuan_yuan[i] == 1&&tuan_yuan[i+10] == 1)
			{
    ...
    public boolean is_long_bai_wei( int tuan_yuan[])
    {
    	boolean  is_long_bai_wei = false;

    	
		if(tuan_yuan[0] == 1&&tuan_yuan[9] == 1)
		{
		
			is_long_bai_wei = true;	
		}
		if(tuan_yuan[10] == 1&&tuan_yuan[19] == 1)
		{
    ...
    public boolean piao_dui_weave(WeaveItem weave_items[],int weave_count)
    {
    	for (int j = 0; j < weave_count; j++) {
			if (weave_items[j].weave_kind == GameConstants.WIK_NULL)
				break;
    ...
    public boolean is_ji_ding(int cur_card,AnalyseItem analyseItem,int weaveCount)
    {
    	...
    	for (int j = 0; j < 6-weaveCount; j++) {
			...
			{
			case GameConstants.WIK_LEFT:
			{	if(analyseItem.cbCenterCard[j+weaveCount]+1 == cur_card )
	    			return true;
				break;
			}
	    	case GameConstants.WIK_CENTER:
    ...
    public boolean is_bian_ding(int cur_card,AnalyseItem analyseItem,int weaveCount)
    {
    	if(cur_card != analyseItem.cbCardEye)
    		return false;
    	for (int j = 0; j < 6-weaveCount; j++) {
			...
			case GameConstants.WIK_CENTER:
			case GameConstants.WIK_RIGHT:
			{	if(analyseItem.cbCardData[j+weaveCount][0] == cur_card
			||analyseItem.cbCardData[j+weaveCount][2] == cur_card)
	    			return true;
				break;
			}
    ...
    public boolean is_yi_tiao_long(AnalyseItem analyseItem)
    {
    	int xiao_index[] = new int[10];
    	int da_index[] = new int[10];
    	Arrays.fill(xiao_index, 0);
    	Arrays.fill(da_index, 0);
    	for(int i = 0; i<6;i++)
    	{
			...
			case GameConstants.WIK_CENTER:
			case GameConstants.WIK_RIGHT:
			{	if(analyseItem.cbCardData[i][0] == 0x01||analyseItem.cbCardData[i][0] == 0x04||analyseItem.cbCardData[i][0] == 0x07)
				{
					for(int j = 0; j<3;j++)
					{
						xiao_index[this.switch_to_card_index(analyseItem.cbCardData[i][j])] ++;
					}
				}
				else if(analyseItem.cbCardData[i][0] == 0x11||analyseItem.cbCardData[i][0] == 0x14||analyseItem.cbCardData[i][0] == 0x17)
				{
					for(int j = 0; j<3;j++)
					{
						da_index[this.switch_to_card_index(analyseItem.cbCardData[i][j])-10] ++;
					}
				}
			}
			}
    	}
    	boolean is_xiao = true;
    	boolean is_da = true;
    	for(int i = 0; i < 9; i++)
    	{
    		if(xiao_index[i] == 0)
    		...
    		if(da_index[i] == 0)
    			is_da = false;
    		if(is_xiao == false && is_da == false)
    			break;
    			
    	}
    	if(is_xiao&& analyseItem.cbCardEye == 0x0a)
    		return true;
    	else if(is_da&& analyseItem.cbCardEye == 0x1a)
    		return true;
    	else return false;
    }
    public boolean is_ga_nuan_da(int cur_card,AnalyseItem analyseItem,int weaveCount){
    	if(cur_card != analyseItem.cbCardEye)
    		return false;
    	int card = 0;
    	if(weaveCount==0)
    		return false;
    	if(analyseItem.cbWeaveKind[weaveCount -1] == GameConstants.WIK_WEI
    			||analyseItem.cbWeaveKind[weaveCount -1] == GameConstants.WIK_CHOU_WEI)
    		card = analyseItem.cbCenterCard[weaveCount -1] ; 
    	if(card == 0)
    		return false;
    	if(analyseItem.cbCardEye == cur_card && ( cur_card + 1  == card ||cur_card  -1 == card ))
    		return true;
    	return false;
    }
    public boolean is_two_long_xi_zhu(AnalyseItem analyseItem)
    {
    	int da_hong = 0;
    	int xiao_hong = 0;
    	for(int i = 0; i< 6;i++)
    	{
			...
			case GameConstants.WIK_CENTER:
			case GameConstants.WIK_RIGHT:
			{	if(analyseItem.cbCardData[i][1] == 0x02)
				{
					xiao_hong ++;
				}
				else if (analyseItem.cbCardData[i][1] == 0x12)
				{
					da_hong ++;
				}
			}
    	...
    	if(xiao_hong == 1&& da_hong == 1)
    		return true;
    	return false;
    }
    public boolean is_kan_bian(int cur_card,AnalyseItem analyseItem,int weaveCount)
    {
    	...
    	for (int j = 0; j < 6-weaveCount; j++) {
			...
			{
			case GameConstants.WIK_LEFT:
			{	if(analyseItem.cbCenterCard[j+weaveCount] == cur_card && analyseItem.cbCenterCard[j+weaveCount]%16 == 8 )
	    			return true;
				if(analyseItem.cbCenterCard[j+weaveCount]+2 == cur_card && (analyseItem.cbCenterCard[j+weaveCount]+2)%16 == 3 )
	    			return true;
				if(analyseItem.cbCenterCard[j+weaveCount]+1 == cur_card )
					return true;
				break;
	    	...
	    	case GameConstants.WIK_RIGHT:
	    	{
	    		if(analyseItem.cbCenterCard[j+weaveCount] == cur_card && analyseItem.cbCenterCard[j+weaveCount] %16 == 3 )
	    			return true;
	    		if(analyseItem.cbCenterCard[j+weaveCount]-1 == cur_card )
					return true;
	    		break;
    ...
    public boolean is_sx_wu_qian_nian(AnalyseItem analyseItem,WeaveItem weaveItems[],
    		int weaveCount,int cbMingIndexTemp[],int cur_card ){
    	int weave_hu_xi = 0;
		...
		for (int j =0 ; j < 6-weaveCount; j++) {
			if (analyseItem.cbWeaveKind[j] == GameConstants.WIK_NULL)
				continue;
			handItem.cbCenterCard[j] = analyseItem.cbCenterCard[weaveCount+j];
			handItem.cbWeaveKind[j] = analyseItem.cbWeaveKind[weaveCount+j] ;
    	}
		handItem.cbCardEye = analyseItem.cbCardEye;
		analyse_item_to_card(weaveItem,weaveItemTemp);
		analyse_item_to_card(handItem,handItemTemp);
		if(get_all_hu_xi_awq(weaveItem,weaveItemTemp,cbMingIndexTemp,cur_card,weaveCount,false) == 5&&get_all_hu_xi_awq(handItem,handItemTemp,cbMingIndexTemp,cur_card,weaveCount,true) == 5)
		{
			return true;
		}
    ...
    public boolean is_dui_dao_hu(int _hu_cards[],int count, int cur_card,AnalyseItem analyseItem, int weaveCount)
    {
    	int  dui_dao_hu = 0;
    	for (int j = 0; j < 6-weaveCount; j++) {
			if (analyseItem.cbWeaveKind[j+weaveCount] == GameConstants.WIK_NULL)
       	...
       	if(cur_card == 0&&weaveCount>0)
      	{
 
      		if(analyseItem.cbWeaveKind[weaveCount-1] == GameConstants.WIK_WEI||analyseItem.cbWeaveKind[weaveCount-1] == GameConstants.WIK_CHOU_WEI)
      		{
      			dui_dao_hu++;
    ...
    public boolean is_ge_shan_da_niu(int _hu_cards[],int count, int cur_card,AnalyseItem analyseItem, int weaveCount)
    {
    	int  ge_san_da_niu = 0;
    	int card = -10;
    	for (int j = 0; j < 6-weaveCount; j++) {
      	...
      	if(Math.abs(analyseItem.cbCardEye - card ) == 2 )
      		ge_san_da_niu++;
    	if(ge_san_da_niu>=2)
    		return true;
    	return false;
    }
    public boolean is_huo_zhuo_xiao_san(int cur_card,AnalyseItem analyseItem, int weaveCount)
    {
    	if(cur_card == 0x03)
    		return true;
       	if(cur_card == 0&&weaveCount>0)
      	{
      		...
      		if(analyseItem.cbWeaveKind[weaveCount-1] == GameConstants.WIK_WEI||analyseItem.cbWeaveKind[weaveCount-1] == GameConstants.WIK_CHOU_WEI)
      		{
      			if(analyseItem.cbCenterCard[weaveCount-1] == 0x03)
      			{
      				return true;
      			}
    ...
    public boolean is_two_hong_two_hei_peng(AnalyseItem analyseItem, int weaveCount)
    {
    	int hong_peng_count = 0;
    	...
    	if(hong_peng_count == 2 && hei_peng_count == 2)
    		return true;
    	return false;
    }
    public boolean is_xing_lian_xing(AnalyseItem analyseItem)
    {
    	for(int i = 0; i<5;i++)
    	{    		
    		if(analyseItem.cbWeaveKind[i] == GameConstants.WIK_LEFT||analyseItem.cbWeaveKind[i] == GameConstants.WIK_CENTER||
    				analyseItem.cbWeaveKind[i] == GameConstants.WIK_RIGHT)
    		{
    			for(int j = i+1; j<6;j++)
    			{
    				if(analyseItem.cbWeaveKind[j] == GameConstants.WIK_LEFT||analyseItem.cbWeaveKind[j] == GameConstants.WIK_CENTER||
    	    				analyseItem.cbWeaveKind[j] == GameConstants.WIK_RIGHT)
    				{
    					int count = 0;
    					if(analyseItem.cbCardData[i][0] == analyseItem.cbCardData[j][2])
    						count ++;
    					if(analyseItem.cbCardData[i][2] == analyseItem.cbCardData[j][0])
    						count ++;
    					if(count == 1)
    						return true;
    				}
    ...
    public boolean is_yuan_yuan_ding(AnalyseItem analyseItem,int cur_card)
    {
    	int card_index = 0;
    	if(cur_card !=analyseItem.cbCardEye)
    		return false;
    ...
    public boolean is_bei_kao_bei(int _hu_cards[],int count, int cur_card,AnalyseItem analyseItem, int weaveCount)
    {
    	...
    	for (int j = 0; j < 6-weaveCount; j++) {
       	...
       	if(cur_card == 0&&weaveCount>0)
      	{
      		...
      		if(analyseItem.cbWeaveKind[weaveCount-1] == GameConstants.WIK_WEI||analyseItem.cbWeaveKind[weaveCount-1] == GameConstants.WIK_CHOU_WEI)
      		{
      			if(analyseItem.cbCenterCard[weaveCount-1]%16  == analyseItem.cbCardEye%16)
      			{
      				bei_kao_bei++;
      			}
      	...
      	else if(analyseItem.cbCardEye%16 == cur_card%16 )
    		bei_kao_bei++;
    	if(bei_kao_bei>=2)
    		return true;
    	return false;
    ...
    public boolean is_shou_qian_shou(int _hu_cards[],int count, int cur_card,AnalyseItem analyseItem, int weaveCount)
    {
    	int  shou_qian_shou = 0;
      	...
      	if(cur_card == 0&&weaveCount>0)
      	{
      		...
      		if(analyseItem.cbWeaveKind[weaveCount-1] == GameConstants.WIK_WEI ||analyseItem.cbWeaveKind[weaveCount-1] == GameConstants.WIK_CHOU_WEI)
      		{
      			if(analyseItem.cbCenterCard[weaveCount-1] +1 == analyseItem.cbCardEye)
      			{
      				shou_qian_shou++;
      			}
      			else if(analyseItem.cbCenterCard[weaveCount-1] -1 == analyseItem.cbCardEye)
      			{
      				shou_qian_shou++;
      			}
      		}
      	}
      	else if((analyseItem.cbCardEye)+1 == cur_card )
  		{
  			shou_qian_shou++;
  		}
      	else if((analyseItem.cbCardEye)-1 == cur_card )
    ...
    {    	
    	...
    	for (int j = 0; j < 6-weaveCount; j++) {
      	...
      	if(cur_card == 0&&weaveCount>0)
      	{
      		...
      		if(analyseItem.cbWeaveKind[weaveCount-1] == GameConstants.WIK_WEI||analyseItem.cbWeaveKind[weaveCount-1] == GameConstants.WIK_CHOU_WEI)
      		{
      			if(analyseItem.cbCardEye == 0x01)
    	    	{
    	    		if(analyseItem.cbCenterCard[weaveCount-1]  == 0x0a)
    	    			feng_bai_wei ++ ;
    	    	}
    	    	if(analyseItem.cbCardEye == 0x0a)
    	    	{
    	    		if(analyseItem.cbCenterCard[weaveCount-1]  == 0x01)
    	    			feng_bai_wei++;
    	    	}
    	    	if(analyseItem.cbCardEye == 0x11)
    	    	{
    	    		if(analyseItem.cbCenterCard[weaveCount-1]  == 0x1a)
    	    			feng_bai_wei ++ ;
    	    	}
    	    	if(analyseItem.cbCardEye == 0x1a)
    	    	{
    	    		if(analyseItem.cbCenterCard[weaveCount-1]  == 0x11)
    	    			feng_bai_wei++;
    	    	}
      		}
      	}
    ...
    public int get_all_hu_xi_weave(WeaveItem weave_items[], int weave_count,int cbWeaveIndexTemp[],boolean is_cur){
    	int hu_xi = 0;
    	for (int j = 0; j < weave_count; j++) {
			if (weave_items[j].weave_kind == GameConstants.WIK_NULL)
				break;
	    	...
	    	{	
    			...
    			{
	    			if(cbWeaveIndexTemp[switch_to_card_index(weave_items[j].center_card)] == 4)
	    			{
	    					hu_xi += 6;
	    			}
	    			else
    ...
    public int get_all_hu_xi_awq(AnalyseItem analyseItem,int cbAnalyseIndexTemp[],int cbMingIndexTemp[],int cur_card,int weave_count,boolean is_hand_card)
    {
    	int hu_xi = 0;
    	boolean  zhuan_wan = false; 
    	boolean hu_xi_card = false;
    	for (int j = 0; j < 7; j++) {
			if (analyseItem.cbWeaveKind[j] == GameConstants.WIK_NULL)
				break;
			switch(analyseItem.cbWeaveKind[j])
	    	...
	    	{	
	    		if(color_hei(analyseItem.cbCenterCard[j]) == true)
    			{
    				...
    				if(cbAnalyseIndexTemp[switch_to_card_index(analyseItem.cbCenterCard[j])] == 4)
    				{
        					hu_xi += 6;
    				}else
    					hu_xi += 3;
	    	...
	    	case GameConstants.WIK_LEFT:
	    	{
	            ...
	            for(int i =  card_index ; i<=card_index+2;i++ )
	            {
	            	if((this.switch_to_card_data(i) == analyseItem.cbCardEye)&&zhuan_wan!=true)
	            	{
	    	    		...
	    	    		if(this.switch_to_card_data(i)== cur_card)
							hu_xi_card = true;
	    	    		break;
	            	}
    	...
    	if(cur_card == 0 && is_hand_card == false)
    	{
    		if(analyseItem.cbWeaveKind[weave_count -1] == GameConstants.WIK_WEI || analyseItem.cbWeaveKind[weave_count -1] == GameConstants.WIK_CHOU_WEI)
    		{
    			if(hu_xi == 5)
    				hu_xi =6;
    		}
    	}
    	else if(is_hand_card == true &&hu_xi == 5 )
    	{
    		if(hu_xi_card == true)
    		{
    			hu_xi = 6;
    ...
    public int get_cal_all_hu_xi_awq(AnalyseItem analyseItem,int cbAnalyseIndexTemp[],int cbMingIndexTemp[])
    {
    	int hu_xi = 0;
    	boolean  zhuan_wan = false; 
    	for (int j = 0; j < 7; j++) {
	...
	public int get_analyse_da_card(AnalyseItem analyseItem){
		int count = 0;
		for(int i = 0; i<7;i++){
			switch(analyseItem.cbWeaveKind[i])
			{
			...
			case GameConstants.WIK_AN_LONG:
			case GameConstants.WIK_PAO:
				if(analyseItem.cbCenterCard[i] > 16)
					count +=4;
				  break;
			case GameConstants.WIK_SAO:			
			case GameConstants.WIK_PENG:
			...
			case GameConstants.WIK_LEFT:
				if(analyseItem.cbCenterCard[i] > 16)
					count +=1;
				if(analyseItem.cbCenterCard[i]+1 > 16)
					count +=1;
				if(analyseItem.cbCenterCard[i]+2 > 16)
					count +=1;
				  break;
			case GameConstants.WIK_CENTER:
				if(analyseItem.cbCenterCard[i] > 16)
		...
		if(analyseItem.cbCardEye >16)
		{
			count +=2;
		}
		return count ;
	}
	public int get_analyse_xiao_card(AnalyseItem analyseItem){
		int count = 0;
		for(int i = 0; i<7;i++){
			switch(analyseItem.cbWeaveKind[i])
			{
		...
		if(analyseItem.cbCardEye <  16 && analyseItem.cbCardEye != 0)
		{
			count +=2;
		}
		return count ;
	...
	public int calculate_tuan_yuan_count(WeaveItem hu_weave_item[],int hu_weave_count){
		...
		{
			if(!(hu_weave_item[i].weave_kind == GameConstants.WIK_PAO
				|| hu_weave_item[i].weave_kind == GameConstants.WIK_TI_LONG
				|| hu_weave_item[i].weave_kind == GameConstants.WIK_AN_LONG))
				continue;
			for(int j = i+1; j< hu_weave_count;j++)
			{
				if(!(hu_weave_item[j].weave_kind == GameConstants.WIK_PAO
						|| hu_weave_item[j].weave_kind == GameConstants.WIK_TI_LONG
	...
	public boolean analyse_card_wmq(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index,int cur_card,
			...
			for(int i = 0; i<GameConstants.MAX_HH_INDEX; i++)
			{
				...
				for(int j = 3; j<cards_index[i]; j++)
				{
					...
					kindItem[cbKindItemCount].cbValidIndex[1] = i;
					kindItem[cbKindItemCount].cbValidIndex[2] = i;
					if(((cur_card == switch_to_card_data(i)) && (seat_index != provider_index) &&(j ==3))||((cur_card == switch_to_card_data(i)) && (seat_index != provider_index) &&(j==4)) )
						kindItem[cbKindItemCount].cbWeaveKind = GameConstants.WIK_PENG;
					else 
						kindItem[cbKindItemCount].cbWeaveKind = GameConstants.WIK_KAN;
					kindItem[cbKindItemCount].cbCenterCard = switch_to_card_data(i);
				...
				if(cards_index[i] >= 3)
				{
					...
					kindItem[cbKindItemCount].cbValidIndex[1] = i;
					kindItem[cbKindItemCount].cbValidIndex[2] = i;
					if((cur_card == switch_to_card_data(i)) && (seat_index != provider_index) &&(cards_index[i] ==3) )
						kindItem[cbKindItemCount].cbWeaveKind = GameConstants.WIK_PENG;
					else 
						kindItem[cbKindItemCount].cbWeaveKind = GameConstants.WIK_KAN;
					kindItem[cbKindItemCount].cbCenterCard = switch_to_card_data(i);
					...
					if (bEnoughCard == true) {
						...
						if ((bNeedCardEye == false) || (cbCardEye != 0)) {
							...
							for (int i = 0; i < cbLessKindItem; i++) {
								...
								weave_item.weave_kind  = pKindItem[i].cbWeaveKind;
								weave_item.center_card = pKindItem[i].cbCenterCard;
								analyseItem.cbPublicCard[cbWeaveCount + i] = 100;
								analyseItem.cbCardData[cbWeaveCount + i][0] = switch_to_card_data(
										pKindItem[i].cbValidIndex[0]);
								analyseItem.cbCardData[cbWeaveCount + i][1] = switch_to_card_data(
										pKindItem[i].cbValidIndex[1]);
...
//							return false;
	...
	public int get_action_rank(int player_action) {
		...
		if(player_action == GameConstants.WIK_ZHE_WMQ)
		{
			return 5;
		}
		return 0;
	...
	public int get_chi_hu_action_rank_xzb_wmq(int seat_index,int da_pai_count, int xiao_pai_count, int ying_hu_count,int chun_ying_count,int hong_pai_count,ChiHuRight chiHuRight) {
		...
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_DUO_HONG_WMQ)).is_empty()){
			hu_xi += 20*(hong_pai_count-10);
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_YING_HU_WMQ)).is_empty()){
			hu_xi += ying_hu_count*20;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_CHUN_YING_WMQ)).is_empty()){
			hu_xi += chun_ying_count*100;
		}
	...
	public int get_chi_hu_action_rank_qmt_wmq(int seat_index,int da_pai_count, int xiao_pai_count, int ying_hu_count,int chun_ying_count,int hong_pai_count,ChiHuRight chiHuRight) {
		...
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_ZHA_DAN_WMQ)).is_empty()){
			hu_xi += ying_hu_count*150;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_FBW_WMQ)).is_empty()){
			hu_xi += 50;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_ZI_MO_WMQ)).is_empty()){
			hu_xi += 30;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_ONE_TEN_TIAN_HU)).is_empty()){
			hu_xi += 150;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_HEI_ZHA_DAN_WMQ)).is_empty()){
			hu_xi += chun_ying_count*100;
		}
		return hu_xi;
	}
	...
	public int get_chi_hu_action_rank_lmt_wmq(int seat_index,int da_pai_count, int xiao_pai_count, int ying_hu_count,int chun_ying_count,int hong_pai_count,ChiHuRight chiHuRight) {
		...
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_YING_HU_WMQ)).is_empty()){
			hu_xi += ying_hu_count*10;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_DUO_HONG_WMQ)).is_empty()){
			hu_xi += 10*(hong_pai_count-10);
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_DA_ZI_HU_WMQ)).is_empty()){
			hu_xi += 50;
		}
	...
	public int get_chi_hu_ying_xi_dzb_wmq_s(int seat_index,ChiHuRight chiHuRight)
	{
	...
	public int get_chi_hu_action_rank_dzb_wmq_s(int seat_index,int da_pai_count, int xiao_pai_count, int ying_hu_count,int chun_ying_count,int hong_pai_count,int zhe_zhe_count,ChiHuRight chiHuRight) {
		int hu_xi = 0;
		
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_MAN_YUAN_HUA_WMQ)).is_empty()){
			hu_xi += 150;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_2_MAN_YUAN_HUA)).is_empty()){
			hu_xi += 120;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_HONG_HU_WMQ)).is_empty()){
			hu_xi += 20;
		...
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_TWO_JIE_MEI_ZHUO)).is_empty()){
			hu_xi += 400;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_ZU_SUN_ZHUO)).is_empty()){
			hu_xi += 500;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_HAI_DI_HU_WMQ)).is_empty()){
			hu_xi += 50;
		...
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_BIAN_DING)).is_empty()){
			hu_xi += 50;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_TUI_DAO_HU)).is_empty()){
			hu_xi += 30;
		}
...
//		}
		...
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_XING_LIAN_XING)).is_empty()){
			hu_xi += 30;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_HUO_ZHOU_3)).is_empty()){
			hu_xi += 30;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_2_HONG_2_HEI)).is_empty()){
			hu_xi += 30;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_ZHE_ZHE_HU)).is_empty()){
			hu_xi += zhe_zhe_count*30;
		}
		return hu_xi;
	}

	public int get_chi_hu_ying_xi_xzb_wmq_s(int seat_index,ChiHuRight chiHuRight)
	{
		int fanshu = 0;
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_DUI_ZI_HU_WMQ)).is_empty()){
			fanshu += 6;
	...
	public int get_chi_hu_action_rank_xzb_wmq_s(int seat_index,int da_pai_count, int xiao_pai_count, int ying_hu_count,int chun_ying_count,int hong_pai_count,ChiHuRight chiHuRight) {
		int hu_xi = 0;
		...
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_YUAN_YUAN_DI)).is_empty()){
			hu_xi += 100;
		}
//		if(!(chiHuRight.opr_and_long(GameConstants.CHR_ZHE_ZHE_HU)).is_empty()){
//			hu_xi += zhe_zhe_count*30;
//		}
	...
	public int get_chi_hu_ying_xi_qmt_wmq_s(int seat_index,ChiHuRight chiHuRight)
	{
		int fanshu = 0;
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_DUI_ZI_HU_WMQ)).is_empty()){
			fanshu += 10;
	...
	public int get_chi_hu_action_rank_qmt_wmq_special_s(int special_type[]){
		int hu_xi = 0;
		if(special_type[GameConstants.CHR_WMQ_SPECIAL_GA_NUAN_DA] == 1)
			hu_xi = 30;
		return hu_xi;		
	}
	public int get_chi_hu_action_rank_qmt_wmq_s(int seat_index,int da_pai_count, int xiao_pai_count, int ying_hu_count,int chun_ying_count,int hong_pai_count,int zhe_zhe_count,ChiHuRight chiHuRight) {
		int hu_xi = 0;
		...
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_GE_SHAN_DA_NIU)).is_empty()){
			hu_xi += 30;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_ONE_TEN_TIAN_HU)).is_empty()){
			hu_xi += 150;
	...
	public int get_chi_hu_ying_xi_lmt_wmq_s(int seat_index,ChiHuRight chiHuRight)
	{
		int fanshu = 0;
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_DUI_ZI_HU_WMQ)).is_empty()){
			fanshu += 10;
	...
	public int get_chi_hu_action_rank_lmt_wmq_s(int seat_index,int da_pai_count, int xiao_pai_count, int ying_hu_count,int chun_ying_count,int hong_pai_count,int zhe_zhe_count,ChiHuRight chiHuRight) {
	int hu_xi = 0;
		...
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_MM_CAI_DAN_CHE)).is_empty()){
			hu_xi += 50;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_BIAN_DING)).is_empty()){
			hu_xi += 50;
		...
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_2_LONG_XI_ZHU)).is_empty()){
			hu_xi += 50;
		}
		if(!(chiHuRight.opr_and_long(GameConstants.CHR_XING_LIAN_XING)).is_empty()){
			hu_xi += 30;
	...
	public boolean is_zha_dan_tian_hu(int cards_index[],int card_count ,boolean is_hei )
	{
		boolean flag = false;
		boolean is_boom = false;
		for(int i = 0; i<card_count;i++ )
		{
			if(cards_index[i] != 4)
				continue;
			if(color_hei(this.switch_to_card_data(i))== is_hei)
					continue;
			int color = this.get_card_color(this.switch_to_card_data(i));
			flag = false;
			for(int j = 0; j<5;++j)
			{
				if(j == 2)
				...
				if(0!=cards_index[j+i -2]&&color == this.get_card_color(this.switch_to_card_data(j+i -2)))
				{
					flag = true;
					break;
				}
	...
	public int is_zha_dan_tian_hu_count(int cards_index[],int card_count ,boolean is_hei )
	{
		boolean flag = false;
		int is_count = 0;
		for(int i = 0; i<card_count;i++ )
	...
	public boolean is_not_ten_xi(int cards_index[],int card_count)
	{
		...
		case 1:
		{
			...
			{
				int color = this.get_card_color(switch_to_card_data(anaylse_card.cbMulCardData[0]));
				for(int i = 0; i<5;++i)
				{
					if(i == 2)
						continue;
					...
					if(0!=cards_index[anaylse_card.cbMulCardData[0]+i -2]&&(color==get_card_color(switch_to_card_data(anaylse_card.cbMulCardData[0]+i -2))))
						return false;
				}
			}
		...
		case 2:
		{
			//一个红对子，一个黑对子， 19-4 = 15 一个还有一个红字， 就可以 凑成红字 可以 红转弯，所以一定是有10息的 3+4+3 = 10
			if(anaylse_heicard.cbMulCount == 2)
			{
				...
				{
					if(cards_index[anaylse_heicard.cbMulCardData[i]] >= 3)
					{
						int color = this.get_card_color(switch_to_card_data(anaylse_heicard.cbMulCardData[i]));
						for(int j = 0; j<5;++j)
						{
							if(j == 2)
								continue;
							if(anaylse_heicard.cbMulCardData[i]+j -2 <0|| anaylse_heicard.cbMulCardData[i]+j-2>19)
								continue;
							
							if((color!=get_card_color(switch_to_card_data(anaylse_heicard.cbMulCardData[i]+j -2))))
								continue;
							boolean flag = false;
							for(int k = 0 ; k < anaylse_heicard.cbMulCount; k++)
							{
								if(k != i)
									if(anaylse_heicard.cbMulCardData[k] == anaylse_heicard.cbMulCardData[i]+j -2&&cards_index[anaylse_heicard.cbMulCardData[i]+j -2]==2)
									{
										flag = true;
										break;
									}
							}
							if(flag == false&&0!=cards_index[anaylse_heicard.cbMulCardData[i]+j -2])
							{
								tuan_yuan ++;
								break;
							}
					...
					else if((eat_type = check_chi_wmq(cards_index, this.switch_to_card_data(anaylse_heicard.cbMulCardData[i])))!= GameConstants.WIK_NULL)
					{
						boolean  is_left_chi = false;
						boolean is_center_chi = false;
						boolean is_right_chi = false;
						
						if ((eat_type & GameConstants.WIK_LEFT) != 0) {

							for(int j = 0; j<anaylse_heicard.cbMulCount; j++ )
							{
								if(i!=j)
								{
									boolean flag = false;
									if(anaylse_heicard.cbMulCardData[i]+1 == anaylse_heicard.cbMulCardData[j])
									{
										if(cards_index[anaylse_heicard.cbMulCardData[j]]!=2)
											is_left_chi = true;
										flag = true;	
									}
									if(anaylse_heicard.cbMulCardData[i]+2 == anaylse_heicard.cbMulCardData[j])
									{
										if(cards_index[anaylse_heicard.cbMulCardData[j]]!=2)
											is_left_chi = true;
										flag = true;
						...
						if ((eat_type & GameConstants.WIK_CENTER) != 0) {
						...
						if ((eat_type & GameConstants.WIK_RIGHT) != 0) {
						...
						if(is_left_chi|is_right_chi|is_center_chi)
						{
							tuan_yuan ++;
						}
				...
				//只要 有一个团员就一定是 满足10息 5+3+3  并且如果 不能团员 ，3（偎）+3（偎）+3（加红字转弯） = 9 ，是一定没有10息的
				if(tuan_yuan >= 1)
				{
					return false;
	...
	public static void main(String[] args) {
		// 插入扑克
		WMQGameLogic logic = new WMQGameLogic();
