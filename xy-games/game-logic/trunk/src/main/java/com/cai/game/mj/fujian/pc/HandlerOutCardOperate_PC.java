...
public class HandlerOutCardOperate_PC extends MJHandlerOutCardOperate<Table_PC> {
	...
	public void exe(Table_PC table) {
		...
		for (int i = 0; i < GameConstants.MAX_COUNT; i++) {
			if (cards[i] == GameConstants_PC.HZ_MAGIC_CARD) {
				cards[i] += GameConstants_PC.CARD_ESPECIAL_TYPE_LAI_ZI;
			}
		}
		...
		// 跟庄处理
		if (table.has_rule(GameConstants_PC.GAME_RULE_SHAO_ZHUANG) && table.isCanGenZhuang()) {
	...
	public boolean handler_operate_card(Table_PC table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_PENG: {
			...
			// 碰完后，不可以立马开杠，若玩家可以直杠，但是选择先碰，则之后也不允许再杠；
			if (table._playerStatus[target_player].has_action_by_code(GameConstants.WIK_GANG)) {
				table._playerStatus[target_player].add_cards_abandoned_gang(table._logic.switch_to_card_index(target_card));
			}
		...
		case GameConstants.WIK_CHI_HU: {
			...
			if (jie_pao_count > 0) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					if (_type == GameConstants_PC.OutCard_Type_Di_Hu && table.has_rule(GameConstants_PC.GAME_RULE_DIAN_PAO)) {
						table.GRR._chi_hu_rights[i].opr_or(GameConstants_PC.CHR_DI_HU);
					}
					table.set_niao_card(_out_card_player, 0, true, 0);
					table.GRR._chi_hu_card[i][0] = target_card;
					table.process_chi_hu_player_operate(i, operate_card, false);
					table.process_chi_hu_player_score(i, _out_card_player, _out_card_data, 3);
	...
	public boolean handler_player_be_in_room(Table_PC table, int seat_index) {
		...
		for (int i = 0; i < GameConstants.MAX_COUNT; i++) {
			if (hand_cards[i] == GameConstants_PC.HZ_MAGIC_CARD) {
				tableResponse.addCardsData(hand_cards[i] + GameConstants_PC.CARD_ESPECIAL_TYPE_LAI_ZI);
			} else {
