...
public class MJHandlerGang_HeNan_smx extends MJHandlerGang<MJTable_SMX> {
    @Override
    ...
    public boolean handler_operate_card(MJTable_SMX table, int seat_index, int operate_code, int operate_card) {
        ...
        if (jie_pao_count > 0) {
            ...
            table.process_chi_hu_player_score_henan_smx(target_player, _seat_index, _center_card, false);
     ...
     ***/
    @Override
    protected boolean exe_gang(MJTable_SMX table) {
        ...
        // 河南麻将,检查听牌
        table._playerStatus[_seat_index]._hu_card_count = table.get_henan_smx_ting_card(
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                ...
                if (GameDescUtil.has_rule(table.getGameRuleIndexEx(),
                        GameConstants.GAME_RULE_HENAN_SMX_ANGANG_DOUBLE)) {
                    score *= 2;
                }
