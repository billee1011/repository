...
public class SCHCPDSSGameLogic_MS {
	...
	public boolean  is_wu_cheng(int card_index[],int cbWeaveCount){
		if(cbWeaveCount != 0)
			return false;
		for(int i = 0; i<GameConstants.DSS_MAX_CP_INDEX;i++ )
		{
			int i_dot = this.get_dot(this.switch_to_card_data(i));
			if(i_dot == 7)
			{
				if(card_index[i] >1)
					return false;
			...
			if(card_index[i] == 0)
				continue;
			for(int j = 0; j<GameConstants.DSS_MAX_CP_INDEX; j++)
			{
				if(card_index[j] > 2)
					return false;
				if((card_index[j] == 0||i==j))
					continue;
				int j_dot = this.get_dot(this.switch_to_card_data(j));
				if(i_dot + j_dot == 14)
					return false;
	...
	public int get_action_rank(int player_action) {
		...
		if (player_action == GameConstants.DSS_WIK_WUCHENG) {
			return 50;
		}
		// 吃胡牌等级
