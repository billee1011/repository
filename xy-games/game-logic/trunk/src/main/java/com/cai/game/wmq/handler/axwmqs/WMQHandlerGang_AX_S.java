...
public class WMQHandlerGang_AX_S extends WMQHandlerGang<WMQTable> {
	...
	public boolean handler_operate_card(WMQTable table, int seat_index, int operate_code, int operate_card,int luoCode) {
		...
		if ((operate_code != GameConstants.WIK_NULL) && (operate_code != GameConstants.WIK_ZI_MO)&&(operate_code != GameConstants.WIK_ZHE_WMQ))// 没有这个操作动作
		{
		...
		if(operate_code == GameConstants.WIK_NULL){
			...
			{
				 if(_action == GameConstants.WIK_WEI||_action == GameConstants.WIK_CHOU_WEI)
		...
		if(operate_code == GameConstants.WIK_NULL)
		{
			...
			{
				table._zhe_zhe_count[target_player] = 0;
			}
		}
		table.operate_zhe_zhe_count(target_player, true);
		...
		case GameConstants.WIK_NULL:
		{
			...
			{    
				...
				if(pai_count == 0||table.is_can_out_card(_seat_index)== false)
				{
...
//			}
			if(_depatch == true)
				table.operate_player_get_card(this._provide_player, 1, new int[] { _center_card }, -2,false);
			
			table._shang_zhuang_player = _seat_index;
			
			table.process_chi_hu_player_score_wmq_s(_seat_index, _provide_player, operate_card, true);
...
//			table.countChiHuTimes_wmq(_seat_index, true);
	 ...
	 * 
	 ***/
	protected boolean exe_gang(WMQTable table) {
		...
		if((_depatch == true)&&(table._ti_two_long[_seat_index] == false)&&(table._is_xiang_gong[_seat_index] == false)){
			...
			if(action_hu != GameConstants.WIK_NULL){
				...
				table._hu_pai_action[_seat_index] = _action;
				 if(table._hu_pai_max_hu[_seat_index] == 10)
				 {
		...
		table._zhe_zhe_count[_seat_index] = 0;
		table.operate_zhe_zhe_count(_seat_index , true);
...
//			table.operate_player_get_card(_seat_index, 1, new int[] { _center_card }, GameConstants.INVALID_SEAT,false);
//			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
//				table.GRR._chi_hu_card[i][0] = GameConstants.INVALID_VALUE;
//			}
//			// 显示胡牌
//			for (int i = 0; i <  table.getTablePlayerNumber(); i++) {
//				int cards1[] = new int[GameConstants.MAX_HH_COUNT];
//				int hand_card_count1 = table._logic.switch_to_cards_data(table.GRR._cards_index[i], cards);
//
//				table.operate_show_card(i, GameConstants.Show_Card_HU, hand_card_count1, cards1, table.GRR._weave_items[i],
...
//		}
	...
	public boolean handler_player_be_in_room(WMQTable table,int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if( i != seat_index)
				tableResponse.addHuXi(table._game_other_mid_score[i]);
			else
		...
		if(table._zhe_zhe_count[seat_index]>0)
			table.operate_zhe_zhe_count(_seat_index , false);
		return true;
	}
}
