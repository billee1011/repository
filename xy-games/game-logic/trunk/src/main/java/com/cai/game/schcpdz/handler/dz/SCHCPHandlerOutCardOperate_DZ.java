...
public class SCHCPHandlerOutCardOperate_DZ extends SCHCPDZHandlerOutCardOperate<SCHCPDZTable> {
	@Override
	...
	public boolean handler_player_be_in_room(SCHCPDZTable table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < GameConstants.MAX_WEAVE_HH; j++) {
				...
				if (seat_index != i) {
					if ((table.GRR._weave_items[i][j].weave_kind == GameConstants.DZ_WIK_MTU_HUO) ) {
