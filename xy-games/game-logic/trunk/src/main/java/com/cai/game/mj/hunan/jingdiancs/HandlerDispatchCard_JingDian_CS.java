...
public class HandlerDispatchCard_JingDian_CS extends MJHandlerDispatchCard<Table_JingDian_CS> {
    ...
    public void exe(Table_JingDian_CS table) {
        ...
        if (table.has_rule(Constants_JingDian_CS.GAME_RULE_ZHONG_TU_SI_XI)) {
        ...
        if (table.has_rule(Constants_JingDian_CS.GAME_RULE_ZHONG_TU_LLS)) {
        ...
        if (check_bu_gang) {
            ...
            int card_type = Constants_JingDian_CS.HU_CARD_TYPE_ZI_MO;
    ...
    private void show_zt_si_xi(Table_JingDian_CS table, int seat_index) {
        ...
        if (table._playerStatus[_seat_index].has_action_by_code(GameConstants.WIK_ZT_SI_XI)) {
            ...
            start_hu_right.opr_or_jd_cs(Constants_JingDian_CS.CHR_ZT_DA_SI_XI);

            table.operate_effect_action(seat_index, GameConstants.EFFECT_ACTION_TYPE_HU, 1,
                    new long[] { Constants_JingDian_CS.CHR_ZT_DA_SI_XI }, 1, GameConstants.INVALID_SEAT);
    ...
    private void show_zt_liu_liu_shun(Table_JingDian_CS table, int seat_index) {
        ...
        if (table._playerStatus[_seat_index].has_action_by_code(GameConstants.WIK_ZT_LLS)) {
            // TODO 中途四喜判断，不能把chr的值加入放到这里，因为如果点了过，并不能直接set_empty();
            // 因为其他的小胡并不是中途的，如果set_empty()会把之前的小胡类型也清空掉了，所以放在发牌处理的show_zt_si_xi方法里
            start_hu_right.opr_or_jd_cs(Constants_JingDian_CS.CHR_ZT_LLS);

            table.operate_effect_action(seat_index, GameConstants.EFFECT_ACTION_TYPE_HU, 1,
                    new long[] { Constants_JingDian_CS.CHR_ZT_LLS }, 1, GameConstants.INVALID_SEAT);
