...
public class MJHandlerChiPeng_ShanXi extends MJHandlerChiPeng<Table_ShanXi> {
	private GangCardResult m_gangCardResult;

	public MJHandlerChiPeng_ShanXi() {
	...
	public void exe(Table_ShanXi table) {
				...
				for (int i = 0; i < tmp_hand_card_count; i++) {
					for (int j = 0; j < ting_count; j++) {
						if (tmp_cards[i] == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
							...
							break;
						}else if (table._logic.is_magic_card(tmp_cards[i])) {
		...
		if (table.GRR._left_card_count > llcard) {
			int cbActionMask = table._logic.analyse_gang_card_all(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], m_gangCardResult, true);
