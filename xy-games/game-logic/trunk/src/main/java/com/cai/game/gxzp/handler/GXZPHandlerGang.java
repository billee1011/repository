...
public class GXZPHandlerGang<T extends GXZPTable> extends GXZPHandler<T> {
	...
	public boolean handler_operate_card(T table,int seat_index, int operate_code, int operate_card,int lou_pai){
		...
		if(jie_pao_count>0){
			...
			for(int i=0; i<table.getTablePlayerNumber();i++){
				...
				table.process_chi_hu_player_score_glzp(i,_seat_index, _center_card,false);
