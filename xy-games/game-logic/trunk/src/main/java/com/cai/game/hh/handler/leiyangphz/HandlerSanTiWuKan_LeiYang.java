...
public class HandlerSanTiWuKan_LeiYang extends HHHandlerDispatchCard<Table_LeiYang> {
    ...
    public void exe(Table_LeiYang table) {
            ...
            if ((ti_count >= 3) || (sao_count >= 5)) {
                ...
                for (int j = 0; j < GameConstants.MAX_HH_INDEX; j++) {
                    if ((i == table._current_player) && (j == send_index))
                        table.GRR._cards_index[i][j]++;
                    if (table.GRR._cards_index[i][j] == 4) {
                        int card = table._logic.switch_to_card_data(j);
                        int card_color = table._logic.get_card_color(card);

                        if (0 == card_color) {
                            table.player_ti_count[i][0]++;
                        } else {
                            table.player_ti_count[i][1]++;
                        }
                ...
                curPlayerStatus.add_chi_hu(0, i);

                table.handler_operate_card(i, GameConstants.WIK_CHI_HU, 0, -1);
        ...
        if (is_hu == false) {
            ...
            if(!table.has_rule(Constants_LeiYang.GAME_RULE_UP_NO_VOICE)){
            	table._handler = table._handler_hands_up;
            	table._handler_hands_up.exe(table);
            }else{
    ...
    public boolean handler_operate_card(Table_LeiYang table, int seat_index, int operate_code, int operate_card,
        ...
        case GameConstants.WIK_CHI_HU: {
            ...
            // } else {
            // table.GRR._chi_hu_rights[seat_index].opr_or(Constants_LeiYang.CHR_DI_HU);
            // }
