...
public class MJHandlerDispatchCard_LUHE extends MJHandlerDispatchCard<Table_LUHE> {
	...
	public void exe(Table_LUHE table) {
		...
		if (table.GRR._left_card_count > 0) {
			...
			int cbActionMask = table._logic.analyse_gang_hong_zhong_luhe(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
	...
	public boolean handler_operate_card(Table_LUHE table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants_HZLZG.WIK_NULL) {
			...
			for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
				if( m_gangCardResult.type[i] == GameConstants.GANG_TYPE_ADD_GANG){
					WeaveItem[] weaveItems = table.GRR._weave_items[seat_index];
					for (int u = 0; u < table.GRR._weave_count[seat_index]; u++) {
						if (weaveItems[u].weave_kind == GameConstants.WIK_PENG
								&& m_gangCardResult.cbCardData[i] == weaveItems[u].center_card) {
							weaveItems[u].can_add_gang_invaild();
						}
					}
				}
			}
		...
		case GameConstants.WIK_ZI_MO: {
			...
			table.process_chi_hu_player_score_luhe(_seat_index, _seat_index, operate_card, true,false);
