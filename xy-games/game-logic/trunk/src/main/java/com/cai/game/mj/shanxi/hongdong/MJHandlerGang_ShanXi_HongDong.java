...
public class MJHandlerGang_ShanXi_HongDong extends MJHandlerGang<MJTable_HongDong> {
	...
	public void exe(MJTable_HongDong table) {
		...
		// 这里会发生抢杠胡 洪洞麻将没有抢杠胡
		// bAroseAction = table.estimate_gang_respond(_seat_index,
		// _center_card);
	...
	public boolean handler_operate_card(MJTable_HongDong table, int seat_index, int operate_code, int operate_card) {
		...
		int one = (_seat_index + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber() + 1;// 点炮者逆时针的下个玩家
		int two = (_seat_index + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber() + 2;// 点炮者逆时针的下下个玩家
		int three = (_seat_index + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber() + 3;// 点炮者逆时针的下下下个玩家
	 ...
	 ***/
	@Override
	protected boolean exe_gang(MJTable_HongDong table) {
		...
		if (table.player_magic_card[_seat_index] == table.get_real_card(_center_card)) {
		...
		if (table._magic_peng_gang_color[_seat_index] != null && table._magic_peng_gang_color[_seat_index].getRight() == -1
				&& table._logic.get_card_color(_center_card) != 3) {
			table._magic_peng_gang_color[_seat_index].setRight(table._logic.get_card_color(_center_card));
		}
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				...
				if ((table.get_real_card(cbCenterCard) == _center_card) && (cbWeaveKind == GameConstants.WIK_PENG)) {
		...
		if (table._logic.is_magic_card(_center_card)) {
			table.GRR._weave_items[_seat_index][cbWeaveIndex].center_card = _center_card + GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI;
		} else {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			table._player_result.zhi_gang_count[_seat_index]++;
		}
		table.operate_player_cards(_seat_index, hand_card_count, cards, weave_count, weaves);

		if (table.GRR._left_card_count > 0) {
			table.exe_dispatch_card(_seat_index, Constants_MJ_SXHS.HU_GANG_SHANG_KAI_HUA, 0);
		} else {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
