 ...
 *
 */
public class PHZHandlerDispatchCard_AnHua extends HHHandlerDispatchCard<AnHuaHHTable> {

	@Override
	public void exe(AnHuaHHTable table) {
		// 用户状态
		...
		if (ti_sao == GameConstants.WIK_PAO) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				for (int w = 0; w < table.GRR._weave_count[i]; w++) {
					// if (table.GRR._weave_items[i][w].center_card ==
					// _send_card_data
					// && table.GRR._weave_items[i][w].weave_kind ==
					// GameConstants.WIK_WEI) {
	 ...
	 */
	@Override
	public boolean handler_operate_card(AnHuaHHTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		if (operate_code == GameConstants.WIK_NULL) {
			if (playerStatus.has_zi_mo() == true && !table.has_rule(GameConstants_AnHua.GAME_RULE_GUO_HU_KE_HU)) {
				int index = -1;
	...
	@Override
	public boolean handler_player_be_in_room(AnHuaHHTable table, int seat_index) {
