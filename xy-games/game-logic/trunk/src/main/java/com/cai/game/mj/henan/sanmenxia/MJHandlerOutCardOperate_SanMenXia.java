...
public class MJHandlerOutCardOperate_SanMenXia extends MJHandlerOutCardOperate<MJTable_SanMenXia> {
	...
	public void exe(MJTable_SanMenXia table) {
		...
		if(ting_count == 1 && ting_cards[0] == -1 && table.has_rule(GameConstants.GAME_RULE_ZI_MO_HU_SMX) && table.has_rule(GameConstants.GAME_RULE_BIAO_YAN_SMX)){
			if(table._player_result.biaoyan[_out_card_player] == 0){
				table._player_result.biaoyan[_out_card_player]++;
				table.operate_player_data();
				//table.process_player_biaoyan(_out_card_player,GameConstants.Effect_Action_Other,GameConstants.INVALID_SEAT);
				
			}
			else if(table._logic.is_magic_card(_out_card_data) && table._player_result.biaoyan[_out_card_player] != 0){
				table._player_result.biaoyan[_out_card_player]++;
				table.process_player_biaoyan(_out_card_player,GameConstants.Effect_Action_Other,GameConstants.INVALID_SEAT);
			}
			else if(!table._logic.is_magic_card(_out_card_data) && table._player_result.biaoyan[_out_card_player] != 0){
				table._player_result.biaoyan[_out_card_player] = 0;
				table.operate_player_data();
			}
		}
		else{
