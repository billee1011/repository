...
public class SJHandlerOutCardOperate_XP_240 extends SJHandlerOutCardOperate<SJTable_XP_240> {

	@Override
	public void exe(SJTable_XP_240 table) {
	...
	public int adjust_out_card_right(SJTable_XP_240 table) {
	...
	public void table_pai_socre(SJTable_XP_240 table) {
		int pai_score = 0;
		...
		for (int i = 0; i < _out_card_count; i++) {
			...
			if (value == 5) {
				pai_score += 5;
			} else if (value == 10 || value == 13 || value == 14 || value == 15) {
				pai_score += 10;
			}
		}
	...
	@Override
	public boolean handler_player_be_in_room(SJTable_XP_240 table, int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJConstants.RESPONSE_XPSJ_RECONNECT_DATA);
		roomResponse.setGameStatus(table._game_status);
		// 发送数据
		TableResponse_Xpsj.Builder tableResponse = TableResponse_Xpsj.newBuilder();
