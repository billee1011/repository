...
public class MJHandlerDispatchCard_ShanXi_LOUFAN extends MJHandlerDispatchCard<MJTable_ShanXi_LOUFAN> {
	...
	public void exe(MJTable_ShanXi_LOUFAN table) {
		...
		if (table.has_rule(GameConstants_SXLF.GAME_RULE_SHANXI_LAO_PAI)) {
		...
		if (table._playerStatus[_seat_index].is_bao_ting()) {
			action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
					table.GRR._weave_count[_seat_index], _send_card_data, chr, GameConstants_SXLF.HU_CARD_TYPE_ZI_MO,
					_seat_index, false);
		}
		if (GameConstants.WIK_NULL != action && table._playerStatus[_seat_index].is_bao_ting()) {
			if (table.has_rule(GameConstants_SXLF.GAME_RULE_SHANXI_NENG_HU_BI_HU)) {
