...
public class PHZHandlerChiPeng_XiangXiang extends HHHandlerChiPeng<XiangXiangHHTable> {

	private GangCardResult m_gangCardResult;

	public PHZHandlerChiPeng_XiangXiang() {
	...
	public void exe(XiangXiangHHTable table) {
		...
		int eat_type = GameConstants_XiangXiang.WIK_LEFT | GameConstants_XiangXiang.WIK_CENTER | GameConstants_XiangXiang.WIK_RIGHT
				...
				| GameConstants_XiangXiang.WIK_YWS;
		...
		if (_lou_card == -1 || (eat_type & _action) == 0)
			...
			table.operate_effect_action(_seat_index, GameConstants_XiangXiang.EFFECT_ACTION_TYPE_ACTION, 1,
					new long[] { GameConstants_XiangXiang.WIK_LOU }, 1, GameConstants_XiangXiang.INVALID_SEAT);
		if (_type == GameConstants_XiangXiang.CHI_PENG_TYPE_OUT_CARD) {
			// 删掉出来的那张牌
			// table.operate_out_card(this._provider, 0,
			// null,MJGameConstants_XiangXiang.OUT_CARD_TYPE_MID,MJGameConstants_XiangXiang.INVALID_SEAT);
			// table.operate_remove_discard(this._provider,
			// table.GRR._discard_count[_provider]);
			table.operate_out_card(this._provider, 0, null, GameConstants_XiangXiang.OUT_CARD_TYPE_MID, GameConstants_XiangXiang.INVALID_SEAT);
		}
		if (_type == GameConstants_XiangXiang.CHI_PENG_TYPE_DISPATCH) {
			table.log_info(table._last_player + "CHI_PENG_TYPE_DISPATCH");
			table.operate_player_get_card(table._last_player, 0, null, GameConstants_XiangXiang.INVALID_SEAT, false);
		}
		...
		if (table._ti_two_long[_seat_index] == false) {
			curPlayerStatus.set_status(GameConstants_XiangXiang.Player_Status_OUT_CARD);// 出牌状态
			...
			table._last_card = 0;
			table.exe_dispatch_card(next_player, GameConstants_XiangXiang.WIK_NULL, 500);
	 ...
	 */
	@Override
	public boolean handler_operate_card(XiangXiangHHTable table, int seat_index, int operate_code, int operate_card, int lou_pai) {
		...
		if (operate_code == GameConstants_XiangXiang.WIK_NULL) {
			...
			table._playerStatus[_seat_index].set_status(GameConstants_XiangXiang.Player_Status_OUT_CARD);
			table.operate_player_status();
