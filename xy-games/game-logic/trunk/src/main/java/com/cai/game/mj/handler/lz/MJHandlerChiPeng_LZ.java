...
public class MJHandlerChiPeng_LZ extends com.cai.game.mj.handler.MJHandlerChiPeng<MJTable> {
	private GangCardResult m_gangCardResult;

	public MJHandlerChiPeng_LZ() {
	...
	public void exe(MJTable table) {
		...
		for (int i = 0; i < GameConstants.MAX_INDEX - GameConstants.CARD_HUA_COUNT; i++) {
			...
			if (count > 0) {
				...
				// 检查打出哪一张牌后可以听牌
				table._playerStatus[_seat_index]._hu_out_card_ting_count[ting_count] = table.get_lz_ting_card(_seat_index,
	...
	public boolean handler_player_be_in_room(MJTable table, int seat_index) {
		...
		if (seat_index == _seat_index) {
			// 自己打什么牌可以听
			int ting_count = table._playerStatus[seat_index]._hu_out_card_count;
			if (ting_count > 0) {
