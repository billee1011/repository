 ...
 *
 */
public class SCHCPHandlerPaiQuan_DSSMS extends SCHCPDSSHandler<SCHCPDSSTable_MS> {
...
//		--table.GRR._left_card_count;
		...
		if(table._game_round == 15)
		{
			
			table._dss_card_type_count = (table._cur_round-1)/3;
			
			
		}
		else{
			table._dss_card_type_count = (int) (RandomUtil.getRandomNumber(Integer.MAX_VALUE) % 5);
		}
		
		if(table.DEBUG_CARDS_MODE)table._dss_card_type_count=0;//
		
		
		_player_card_type = table._dss_card_type[table._dss_card_type_count];
		
		
		//显示
		table.operate_fan_jiang(-1,GameConstants.Show_Card_Center, 1,new int[]{_player_card_type},GameConstants.INVALID_SEAT);

		
		table._player_card_type = _player_card_type;
...
//		table.send_response_to_player(seat_index, roomResponse);
		...
		//显示
		table.operate_fan_jiang(-1,GameConstants.Show_Card_Center, 1,new int[_player_card_type],seat_index);
