 ...
 * 湖南醴陵麻将Table
 * 
 * @author WalkerGeek date: 2017年9月19日 下午2:58:26 <br/>
 */
public class MJTable_LLTH extends AbstractMJTable {
	...
	public MJTable_LLTH() {
		super(MJType.GAME_TYPE_HUNAN_LILING_TH_TYPE);
	}

	public MJHandlerQiShouHongZhong_LLTH _handler_qishou_hongzhong;

	public MJHandlerPiao_LLTH handlerPiao_LL;

	public int[] piao_score; // 飘分
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_old(int cards_index[], WeaveItem weaveItems[], int weaveCount, int cur_card,
			ChiHuRight chiHuRight, int card_type) {
	...
	/*
	 * public int get_hz_ting_card_old(int[] cards, int cards_index[], WeaveItem
	 * weaveItem[], int cbWeaveCount, boolean dai_feng) {
	 * 
	 * // 复制数据 int cbCardIndexTemp[] = new int[GameConstants.MAX_INDEX]; for
	 * (int i = 0; i < GameConstants.MAX_INDEX; i++) { cbCardIndexTemp[i] =
	 * cards_index[i]; }
	 * 
	 * ChiHuRight chr = new ChiHuRight();
	 * 
	 * int count = 0; int cbCurrentCard;
	 * 
	 * int l = GameConstants.MAX_INDEX - GameConstants.CARD_HUA_COUNT -
	 * GameConstants.CARD_FENG_COUNT; int ql = l; if (dai_feng) { l +=
	 * GameConstants.CARD_FENG_COUNT; ql += (GameConstants.CARD_FENG_COUNT - 1);
	 * } for (int i = 0; i < l; i++) { if (this._logic.is_magic_index(i))
	 * continue; cbCurrentCard = _logic.switch_to_card_data(i); chr.set_empty();
	 * if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card(cbCardIndexTemp,
	 * weaveItem, cbWeaveCount, cbCurrentCard, chr,
	 * GameConstants.HU_CARD_TYPE_ZIMO)) { cards[count] = cbCurrentCard;
	 * count++; } }
	 * 
	 * l -= 1; if (count == 0) { // 红中能不能胡 // cbCurrentCard = //
	 * _logic.switch_to_card_data(this._logic.get_magic_card_index(0)); // if(
	 * MJGameConstants.WIK_CHI_HU == analyse_chi_hu_card_hz( //
	 * cbCardIndexTemp,weaveItem,cbWeaveCount,cbCurrentCard,chr,MJGameConstants.
	 * HU_CARD_TYPE_ZIMO // ) ){ // cards[count] = cbCurrentCard; // count++; //
	 * } } else if (count > 0 && count < ql) { // 有胡的牌。红中肯定能胡
	 * if(this._logic.get_magic_card_count() != 0){ cards[count] =
	 * _logic.switch_to_card_data(this._logic.get_magic_card_index(0)); count++;
	 * } } else { // 全听 count = 1; cards[0] = -1; }
	 * 
	 * return count; }
	 */
	...
	public void set_niao_card(int seat_index, int card, boolean show, int add_niao) {
		...
		for (int i = 0; i < GRR._count_niao; i++) {
			if(has_rule_ex(GameConstants.GAME_RULE_ZHUANG_NIAO)){
			...
			if (has_rule_ex(GameConstants.GAME_RULE_ZHUANG_NIAO)) {
				seat = (GRR._banker_player + (nValue - 1) % getTablePlayerNumber()) % getTablePlayerNumber();
			} else {
	...
	protected void onInitTable() {
		_handler_dispath_card = new MJHandlerDispatchCard_LLTH();
		_handler_out_card_operate = new MJHandlerOutCardOperate_LLTH();
		_handler_gang = new MJHandlerGang_LLTH();
		_handler_chi_peng = new MJHandlerChiPeng_LLTH();

		_handler_qishou_hongzhong = new MJHandlerQiShouHongZhong_LLTH();
		handlerPiao_LL = new MJHandlerPiao_LLTH();
	}
	...
	public void process_chi_hu_player_score(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		if (zimo) {
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				// 记录胡牌分数 不算飘分，鸟分
				GRR._game_hu_score[i] -= s;
				GRR._game_hu_score[seat_index] += s;

				//不中算全中
				if(has_rule_ex(GameConstants.GAME_RULE_NIAO_BU_ZHONG_SUAN_QUAN_ZHONG) && GRR._count_pick_niao == 0){
					GRR._count_pick_niao = 6;
				}
				...
				s += niao * niaoScore;
				// 飘分
				int piao = ((_player_result.pao[i] < 0 ? 0 : _player_result.pao[i])
		...
		////////////////////////////////////////////////////// 点炮 算分
			...
			// 记录胡牌分数 不算飘分，鸟分
			GRR._game_hu_score[provide_index] -= s;
			GRR._game_hu_score[seat_index] += s;
			//不中算全中
			if(has_rule_ex(GameConstants.GAME_RULE_NIAO_BU_ZHONG_SUAN_QUAN_ZHONG) && GRR._count_pick_niao == 0){
				GRR._count_pick_niao = 6;
			...
			s += niao * niaoScore;
			// 飘分
			int piao = ((_player_result.pao[provide_index] < 0 ? 0 : _player_result.pao[provide_index])
					+ (_player_result.pao[seat_index] < 0 ? 0 : _player_result.pao[seat_index]));
			// 飘分记录
			...
			for (int j = 0; j < l; j++) {
				...
				if (GRR._chi_hu_rights[i].is_valid()) {
					...
					if (type == GameConstants.CHR_ZI_MO) {
						...
						//不中算全中
						if(has_rule_ex(GameConstants.GAME_RULE_NIAO_BU_ZHONG_SUAN_QUAN_ZHONG)){
							des += " 不中算全中";
						}
					}
	...
	protected boolean on_handler_game_finish(int seat_index, int reason) {
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL || reason
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				// 没有小胡:使用此字段传递结算飘分 game_end.addStartHuRight(rv[0]);
				game_end.addStartHuRight(piao_score[i]);

				game_end.addProvidePlayer(GRR._provider[i]);
				game_end.addGameScore(GRR._game_score[i]);// 放炮的人？
				game_end.addGangScore(lGangScore[i]);// 杠牌得分
				game_end.addStartHuScore((int) GRR._game_hu_score[i]); // 使用小胡分字段修改胡牌牌型分
