...
public class MJTable_KouDian extends AbstractMJTable {
	...
	protected boolean bBaoGangBaoHu = false;
	...
	protected MJHandlerOutCardBaoTing_KouDian _handler_out_card_bao_ting; // 报听
	...
	// protected boolean bAutoHuPai = false;

	public MJTable_KouDian() {
		super(MJType.GAME_TYPE_SXKD);
	}

	@Override
	protected void onInitTable() {
		_handler_gang = new MJHandlerGang_ShanXi_KouDian();
		_handler_chi_peng = new MJHandlerChiPeng_ShanXi_KouDian();
		_handler_out_card_bao_ting = new MJHandlerOutCardBaoTing_KouDian();
		_handler_dispath_card = new MJHandlerDispatchCard_ShanXi_KouDian();
		_handler_out_card_operate = new MJHandlerOutCardOperate_ShanXi_KouDian();
	...
	public boolean animation_timer(int timer_id) {
		...
		case ID_AUTO_ZIMO_HU: {
			if (bAutoZiMoHuPai && autozimoplyer != GameConstants.INVALID_SEAT
					&& autocard != GameConstants.INVALID_SEAT) {
				/*
				 * GRR._chi_hu_rights[autoplyer].set_valid(true);
				 * GRR._chi_hu_card[autoplyer][0] = autocard; if(autoplyer ==
				 * _cur_banker){ _cur_banker = autoplyer; } else{ _cur_banker =
				 * (_cur_banker + GameConstants.GAME_PLAYER + 1) %
				 * GameConstants.GAME_PLAYER; }
				 * 
				 * process_chi_hu_player_operate(autoplyer, autocard, true);
				 * process_chi_hu_player_score_ll(autoplyer, autoplyer,
				 * autocard, true); for (int i = 0; i < getTablePlayerNumber();
				 * i++) { if (i == autoplyer) { continue; } _biaoyan_count[i] =
				 * 0; } // 记录 _player_result.zi_mo_count[autoplyer]++;
				 * GameSchedule.put(new GameFinishRunnable(getRoom_id(),
				 * autoplyer, GameConstants.Game_End_NORMAL),
				 * GameConstants.GAME_FINISH_DELAY, TimeUnit.SECONDS);
				 */

				this._handler_dispath_card.handler_operate_card(this, autozimoplyer, GameConstants.WIK_ZI_MO,
						this.autocard);
				break;
			}
		}
		case ID_AUTO_FANGPAO_HU: {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				if (this.bAutoHuPai[i] && autocard != GameConstants.INVALID_SEAT) {
					this._handler_out_card_operate.handler_operate_card(this, i, GameConstants.WIK_CHI_HU,
							this.autocard);
				}
			}
			break;
	...
	public void shuffle(int repertory_card[], int mj_cards[]) {
		...
		if (count <= 0) {
			count = has_rule(GameConstants.GAME_RULE_HUNAN_THREE) ? GameConstants.GAME_PLAYER - 1
					: GameConstants.GAME_PLAYER;
		}
	...
	public boolean show_tou_zi() {
		...
		return operate_tou_zi_effect(tou_zi_dian_shu[0], tou_zi_dian_shu[1], tou_zi_dian_shu[2], tou_zi_dian_shu[3],
				time_for_animation, time_for_fade);
	}

	public boolean operate_tou_zi_effect(int tou_zi_one, int tou_zi_two, int tou_zi_three, int tou_zi_four,
			int time_for_animate, int time_for_fade) {
	...
	protected void test_cards() {
		int cards[] = new int[] { 0x06, 0x06, 0x08, 0x08, 0x13, 0x14, 0x15, 0x17, 0x19, 0x27, 0x28, 0x22, 0x36, 0x11,
				0x06, 0x12, 0x12, 0x13, 0x13, 0x14, 0x15, 0x16, 0x19, 0x21, 0x33, 0x33, 0x21, 0x21, 0x22, 0x22, 0x23,
				0x23, 0x26, 0x27, 0x31, 0x32, 0x33, 0x33, 0x34, 0x01, 0x06, 0x11, 0x12, 0x03, 0x13, 0x21, 0x22, 0x23,
				0x32, 0x33, 0x33, 0x34 };
	...
	public boolean SetMagiCards() {
		...
		operate_show_card(this.GRR._banker_player, GameConstants.Show_Card_Center, icount, cards,
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_shanxi_ll(int cards_index[], WeaveItem weaveItems[], int weaveCount, int cur_card,
		...
		// 3、4、5、只能自摸
		if (iCardValue < 6
				&& (card_type == GameConstants.HU_CARD_TYPE_PAOHU || card_type == GameConstants.HU_CARD_TYPE_QIANGGANG
		...
		if (bValue) {
			...
			if (!has_rule(GameConstants.GAME_RULE_DAN_HAO) && !has_rule(GameConstants.GAME_RULE_SHUANG_HAO)) {
				if (_logic.is_qing_yi_se(cbCardIndexTemp, weaveItems, weaveCount, cur_card)
						&& has_rule(GameConstants.GAME_RULE_TE_SHU_HU)) {
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_not_lai(int cards_index[], WeaveItem weaveItems[], int weaveCount, int cur_card,
			ChiHuRight chiHuRight, int card_type, boolean lastcardlaizi) {
		...
		if (lastcardlaizi) {
			bValue = AnalyseCardUtil.analyse_win_by_cards_index_shanxill(cards_index,
	...
	public int get_ting_card(int[] cards, int cards_index[], WeaveItem weaveItem[], int cbWeaveCount) {
		...
		for (int i = 0; i < GameConstants.MAX_ZI_FENG; i++) {
			...
			if (_logic.is_magic_index(i)) {
				if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card_not_lai(cbCardIndexTemp, weaveItem, cbWeaveCount,
						cbCurrentCard, chr, GameConstants.HU_CARD_TYPE_PAOHU, true)) {
				...
				if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card_not_lai(cbCardIndexTemp, weaveItem, cbWeaveCount,
						cbCurrentCard, chr, GameConstants.HU_CARD_TYPE_PAOHU, false)) {
		...
		if (count > 0) {
			for (int j = 0; j < _logic.get_magic_card_count(); j++) {
				cards[count++] = _logic.switch_to_card_data(_logic.get_magic_card_index(j))
	...
	public boolean estimate_gang_respond_ll(int seat_index, int card) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			// 可以胡的情况 判断,单吊耗子不能抢杠胡
			if (playerStatus.is_chi_hu_round() && _playerStatus[i].is_bao_ting()
					&& _playerStatus[i]._hu_card_count < 25) {
				// 吃胡判断
				ChiHuRight chr = GRR._chi_hu_rights[i];// playerStatus._chiHuRight;
				chr.set_empty();
				int cbWeaveCount = GRR._weave_count[i];
				action = analyse_chi_hu_card_shanxi_ll(GRR._cards_index[i], GRR._weave_items[i], cbWeaveCount, card,
						chr, GameConstants.HU_CARD_TYPE_QIANGGANG);
			...
			if (GRR._left_card_count > llcard) {
				...
				if (action != 0) {
					if (_playerStatus[i].is_bao_ting()) {
						...
						int ting_count = get_ting_card(ting_cards, cbCardIndexTemp, GRR._weave_items[i],
								GRR._weave_count[i] + 1);
						if (ting_count > 0) {
	...
	protected boolean on_handler_game_finish(int seat_index, int reason) {
		...
		if (GRR != null) {
			if (reason == GameConstants.Game_End_NORMAL) {
				...
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					...
					lGangScore[i] += an_gang_laizi_score[i];
			...
			if (reason == GameConstants.Game_End_DRAW) {
				for (int player = 0; player < getTablePlayerNumber(); player++) {
					GRR._game_score[player] = an_gang_laizi_score[player];
					;
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if ((_cur_round >= _game_round && (!is_sys())) || (bBaiFenPoChan && PoChan())) {// 局数到了
				end = true;
				...
				if (GRR._end_type != GameConstants.Game_End_DRAW) {
					...
					for (int tmpPlayer = 0; tmpPlayer < getTablePlayerNumber(); tmpPlayer++) {
						for (int w = 0; w < GRR._weave_count[tmpPlayer]; w++) {
							...
							if (GRR._weave_items[tmpPlayer][w].public_card == 0
									&& _logic.is_magic_card(GRR._weave_items[tmpPlayer][w].center_card)) {
								if (tmpPlayer == player)
									an_gang[player]++;
							}
						}
	...
	// 为报听去杠分
	public void process_gang_score_ll(int provide_index) {
		/*
		 * this.weibaotingfangpao = provide_index; for (int i = 0; i <
		 * getTablePlayerNumber(); i++) { if(i == provide_index){ for (int j =
		 * 0; j < GRR._gang_score[i].gang_count; j++) { for (int k = 0; k <
		 * getTablePlayerNumber(); k++) { GRR._gang_score[i].scores[j][k] = 0;//
		 * 杠牌，每个人的分数 } } } }
		 */
