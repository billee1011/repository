...
public class KickRunnable extends BaseFuture{
	private static Logger logger = LoggerFactory.getLogger(KickRunnable.class);

	
	private int _room_id;
	
	public KickRunnable(int room_id){
		super(room_id);
		_room_id = room_id;
	}
	...
	public void execute() {
		...
		try {
			...
			try{
				roomLock.lock();
				table.kickout_not_ready_player();
			}finally{
				roomLock.unlock();
