...
public class MJHandlerPao_HeNanxx extends MJHandlerPao_HeNan {
	...
	public boolean handler_pao_qiang(MJTable table, int seat_index, int pao, int qiang) {
		...
		if (table._game_status == GameConstants.GS_MJ_PAO) {
			...
			// 检测听牌
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				table._playerStatus[i]._hu_card_count = table.get_hn_xin_xiang_ting_card_new(table._playerStatus[i]._hu_cards,
