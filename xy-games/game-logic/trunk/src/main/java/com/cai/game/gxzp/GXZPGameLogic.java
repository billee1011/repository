...
public class GXZPGameLogic {

	private static Logger logger = Logger.getLogger(GXZPGameLogic.class);
	 ...
	 * @return
	 */
	public boolean is_shi_ba_xiao(int[] cards_index) {
		int count = 0;
		for (int i = 0; i < 10; i++) {
			count += cards_index[i];
		}
	...
	// 跑牌判断
	public int check_tuo(int card_index[], int cur_card) {
		// 参数效验
		if (is_valid_card(cur_card) == false) {
			return GameConstants.WIK_NULL;
		}
		// 碰牌判断
		return (card_index[switch_to_card_index(cur_card)] == 3) ? GameConstants.WIK_TUO : GameConstants.WIK_NULL;
	}
	...
	public int get_weave_hu_xi(WeaveItem weave_item, boolean is_hong) {
	...
	public boolean is_sx_wu_qian_nian(AnalyseItem analyseItem, WeaveItem weaveItems[], int weaveCount, int cbMingIndexTemp[]) {
		...
		int handItemTemp[] = new int[GameConstants.MAX_GXZP_COUNT];
		int weaveItemTemp[] = new int[GameConstants.MAX_GXZP_COUNT];
	...
	public int get_analyse_hu_xi(int cbWeaveKind, int center_card, boolean is_hong) {
		switch (cbWeaveKind) {
		case GameConstants.WIK_TI_LONG:
		case GameConstants.WIK_AN_LONG:
		case GameConstants.WIK_AN_LONG_LIANG:
	...
	// 分析扑克
	public boolean analyse_card_glzp(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index, int cur_card,
		...
		for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
			if (cards_index[i] == 4) {
				...
				if ((seat_index == provider_index) || (switch_to_card_index(cur_card) != i))
					...
					pao_WeaveItem[pao_count].weave_kind = GameConstants.WIK_TUO;
				b_pao = true;
				cbCardCount -= 4;
				pao_count++;
			...
			for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
				if (cards_index[i] == 2) {
					...
					for (int j = 0; j < cbWeaveCount; j++) {
						...
						analyseItem.hu_xi[j] = get_weave_hu_xi(weaveItem[j], yws_type);
						// if((cur_card ==
					...
					if (b_pao == true) {
						for (int k = 0; k < pao_count; k++) {
							...
							analyseItem.hu_xi[count] = get_weave_hu_xi(pao_WeaveItem[k], yws_type);
		...
		for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
			...
			// for (int j = 1; j <= cards_index[i]; j++) {
			// if (((cards_index[i] < 3) || (i ==
			// switch_to_card_index(cur_card)))
			// && ((cards_index[i + 4] >= j) && ((cards_index[i + 4] < 3) || ((i
			// + 4) == switch_to_card_index(cur_card))))
			// && ((cards_index[i + 9] >= j) && ((cards_index[i + 9] < 3) || ((i
			// + 9) == switch_to_card_index(cur_card))))) {
			// kindItem[cbKindItemCount].cbCardIndex[0] = i;
				...
				if (bEnoughCard == true) {
					...
					if ((bNeedCardEye == false) || (cbCardEye != 0)) {
						...
						for (int i = 0; i < cbLessKindItem; i++) {
							...
							analyseItem.hu_xi[i + cbWeaveCount] = get_weave_hu_xi(weave_item, yws_type);
	...
	public int get_action_rank(int player_action) {
		...
		// 跑等级
		if (player_action == GameConstants.WIK_TUO) {
			return 40;
		}
		// 吃胡牌等级
	...
	public boolean sort_card_list(int card_data[]) {
		...
		// 数目过虑
		if (cbCardCount == 0 || cbCardCount > GameConstants.MAX_GXZP_COUNT)
			return false;
	...
	public static void main(String[] args) {
		// 插入扑克
		GXZPGameLogic logic = new GXZPGameLogic();
	...
	// 分析扑克
	public boolean analyse_card_yyzhz(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, List<AnalyseItem> analyseItemArray) {
		// 计算数目
		int cbCardCount = get_card_count_by_index(cards_index);
		// 变量定义
		int cbKindItemCount = 0;
