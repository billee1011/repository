...
public class TBHandlerCallBanker extends BTZHandlerCallBanker<TBTable> {

	@Override
	public void exe(TBTable table) {
	 ...
	 * @param open_flag
	 */
	public boolean handler_call_banker(TBTable table, int seat_index, int call_banker) {
	...
	@Override
	public boolean handler_player_be_in_room(TBTable table, int seat_index) {
		...
		if (table.observers().count() > 0) {
			RoomUtil.send_response_to_observer(table, roomResponse);
		}
		return true;
	}

	@Override
	public boolean handler_observer_be_in_room(TBTable table, Player player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(BTZConstants.RESPONSE_RECONNECT_DATA);
		...
		table.load_room_info_data(roomResponse);
		table.load_player_info_data(roomResponse);
		tableResponse.setPlayerStatus(table._player_status[0]);
		tableResponse.setBankerPlayer(-1);
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
