...
public class MJHandlerGang_HY extends MJHandlerGang<MJTable_HY> {
    ...
    public boolean handler_operate_card(MJTable_HY table, int seat_index, int operate_code, int operate_card) {
        ...
        if (jie_pao_count > 0) {
            ...
            if (jie_pao_count > 1) {
                ...
                // 红中通炮就不加鸟了
                player_seat = _provide_player;
                // table.set_niao_card_hy(_provide_player,GameConstants.INVALID_VALUE,true,0,true);//
                // 结束后设置鸟牌

            } else if (jie_pao_count == 1) {
                table._cur_banker = target_player;
                player_seat = target_player;
                // table.set_niao_card_hy(target_player,GameConstants.INVALID_VALUE,true,0,false);//
                // 结束后设置鸟牌
            }
     ...
     ***/
    @Override
    protected boolean exe_gang(MJTable_HY table) {
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            ...
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                ...
                table.GRR._gang_score[_seat_index].scores[cbGangIndex][i] = -score;// 明杠，其他玩家扣分
                table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += score;// 一共加分
            }
        ...
        } else if (_action == GameConstants.WIK_YAO_YI_SE) {
            // 从后面发两张牌给玩家
