...
public class HandlerDispatchCard_XY extends MJHandlerDispatchCard<Table_XY> {
	...
	public void exe(Table_XY table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			int jiao_count = 0;
			int next_banker_player = table._cur_banker;
			for(int i = 0;i < table.getTablePlayerNumber();i++){
				if(table._playerStatus[i]._hu_card_count > 0){
					jiao_count++;
				}else if(table._playerStatus[i]._hu_card_count <= 0){
					next_banker_player = i;
				}
			}
			if(jiao_count == table.getTablePlayerNumber() - 1){
				table._cur_banker = next_banker_player;
			}else if(table.getTablePlayerNumber() == 4 && jiao_count == 2){
	...
	@Override
	public boolean handler_player_out_card(Table_XY table, int seat_index, int card) {
