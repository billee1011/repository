...
public class MJHandlerOutCardOperate_ShanXi_XiangYuan extends MJHandlerOutCardOperate<MJTable_ShanXi_XiangYuan> {
	...
	public boolean handler_operate_card(MJTable_ShanXi_XiangYuan table, int seat_index, int operate_code,
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			...
			// false);
			table.hu_pai_people++;
			table.is_zm_or_dp[seat_index] = 2;
			table.dian_pao = _out_card_player;
		}
		...
		// table._playerStatus[target_player]._action);
		// }
		//
		// // 动作判断 优先级最高的人和动作
		...
		case GameConstants.WIK_CHI_HU: {
			...
			if (jie_pao_count > 0) {
				...
				/*
				 * if (jie_pao_count > 1) { table._cur_banker =
				 * _out_card_player; } else { table._cur_banker = target_player;
				 * }
				 */
