...
public class HandlerDispatchCard_HuangZhou extends MJHandlerDispatchCard<Table_HuangZhou> {
	...
	public HandlerDispatchCard_HuangZhou() {
	...
	public void exe(Table_HuangZhou table) {
		...
		if (_type != Constants_HuangZhou.LIANG_LAI_ZI) {
			...
			if (table._logic.is_magic_card(_send_card_data)) {
				...
				if (table._player_result.nao[_seat_index] == 0) {
					if (table.qi_shou_lai_zi[_seat_index] <= 1 && table.total_lai_zi[_seat_index] == 2) {
						if (!table.is_men_qing(table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index])) {
							int real_card = _send_card_data;
							if (table._logic.is_magic_card(real_card)) {
								real_card += GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI;
							}

							// TODO 抓的这张癞子并没有往GRR和手牌数据里添加，断线重连时得注意一下
							table.operate_player_get_card_without_record(_seat_index, 1, new int[] { real_card }, GameConstants.INVALID_SEAT);

							// TODO: 起手癞子数<=1，中间过程抓到第二张癞子，并且已经开口了，必须选择亮或不亮癞子
							table.exe_liang_lai_zi(_seat_index, _send_card_data, Constants_HuangZhou.LIANG_LAI_ZI);
							return;
						} else {
							// TODO:
							// 起手癞子数<=1，中间过程抓到第二张癞子，并且没开口，这种情况是和起手癞子数大于1一样的
							table.qi_shou_lai_zi[_seat_index] = 2;
						}
					}
				}
			}
		...
		int card_type = Constants_HuangZhou.HU_CARD_TYPE_ZI_MO;
		if (table.gang_status) {
			card_type = Constants_HuangZhou.HU_CARD_TYPE_GANG_KAI;
		}
	...
	@Override
	public boolean handler_player_out_card(Table_HuangZhou table, int seat_index, int card) {
	...
	public boolean handler_player_be_in_room(Table_HuangZhou table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			// TODO 不管是不是亮癞子之后再进的这里，还是正常抓牌进的这里，都要删一下，因为已经加到手牌数据里了
			if (i == _seat_index) {
