...
public class PHZHandlerOutCardOperate_ER extends HHHandlerOutCardOperate<HHTable_ER> {
	@Override
	public void exe(HHTable_ER table) {
	...
	public boolean handler_operate_card(HHTable_ER table, int seat_index, int operate_code, int operate_card, int luoCode) {
		// 效验状态
		PlayerStatus playerStatus = table._playerStatus[seat_index];

		// 是否已经响应
		if (playerStatus.has_action() == false) {
			table.log_player_error(seat_index, "HHHandlerOutCardOperate_cs 出牌,玩家操作已失效");
			return true;
		}

		// 是否已经响应
		if (playerStatus.is_respone()) {
			table.log_player_error(seat_index, "HHHandlerOutCardOperate_cs 出牌,玩家已操作");
			return true;
		}

		if ((operate_code != GameConstants.WIK_NULL) && playerStatus.has_action_by_code(operate_code) == false)// 没有这个操作动作
		{
			table.log_player_error(seat_index, "HHHandlerOutCardOperate_cs 出牌操作,没有动作");
			return true;
		}
		if ((operate_card != table._out_card_data)) {
			table.log_player_error(seat_index, "HHHandlerOutCardOperate_cs 操作牌，与当前牌不一样");
			return true;
		}
	...
	@Override
	public boolean handler_player_be_in_room(HHTable_ER table, int seat_index) {
