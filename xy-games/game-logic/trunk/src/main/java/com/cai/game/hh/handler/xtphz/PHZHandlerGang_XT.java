...
public class PHZHandlerGang_XT extends HHHandlerGang<HHTable> {
...
//			}
			...
			table.process_chi_hu_player_score_phz_xt(_seat_index, _provide_player, operate_card, true);
	...
	protected boolean exe_gang(HHTable table) {
		...
		if(an_long_count > 0 )
		{
			...
			table._xt_display_an_long[_seat_index] = true;
...
//		}
		...
		if((_depatch == true)&&(table._ti_mul_long[_seat_index] == 0)&&(table._is_xiang_gong[_seat_index] == false)){
			...
			if(is_ting_hu == true)
			{
				for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
					...
					if (!((cbWeaveKind == GameConstants.WIK_TI_LONG)||
							...
							(cbWeaveKind == GameConstants.WIK_CHOU_WEI))) {
						is_ting_hu = false;
					}
			...
			if(action_hu != GameConstants.WIK_NULL){
				table._guo_hu_xt[_seat_index] = _provide_player;
	        ...
	        if(pai_count == 0&&table._guo_hu_xt[_seat_index] == -1)
	        {
	        	int all_hu_xi = 0;
	        	for(int i = 0; i<table.GRR._weave_count[_seat_index];i++ )
	        	{
...
//	        	if(table.has_rule(GameConstants.GAME_RULE_TEN_HU_XI_XT))
//    			{
//	        		if (all_hu_xi >= 10) {
//	    				b_hu_xi =true;
//	    			}	
//    			}
//	        	else if (table.has_rule(GameConstants.GAME_RULE_YOUXIAN_PHZ_SELECT_HUXI)) {
//	    			if (all_hu_xi >= 18) {
//	    				b_hu_xi =true;
//	    			}
//	    		}
//	        	else if(table.has_rule(GameConstants.GAME_RUEL_FIFTEEN_HU_XI_XT))
	        	{
	        		if (all_hu_xi >= 15) 
	    					b_hu_xi =true;
	    		}
	    		if(b_hu_xi== true)
	        	{
					...
					if(table.has_rule(GameConstants.GAME_RULE_JI_BENMING_TANG)||table.has_rule(GameConstants.GAME_RULE_ALL_MING_TANG))
					{
						...
						if(table.has_rule(GameConstants.GAME_RULE_JI_BENMING_TANG))
						{
						...
						if(table.has_rule(GameConstants.GAME_RULE_ALL_MING_TANG))
						{
							if (hong_pai_count > 10 && hong_pai_count<13 ) {
								table._hong_pai_count[_seat_index] = hong_pai_count;
								chr.opr_or(GameConstants.CHR_TEN_HONG_PAI);
							...
							if (hong_pai_count  == 10) {
								table._hong_pai_count[_seat_index] = hong_pai_count;
								chr.opr_or(GameConstants.CHR_JIA_DIAN_HU);
							}
							if(table.has_rule(GameConstants.GAME_RULE_SI_QI_ZHANG_HONG_HU)){
							...
							if(card_count == 1){
								chr.opr_or(GameConstants.CHR_SHUA_HOU);
							}
...
//		{
//			if(table.GRR._must_out_index[seat_index][table._logic.switch_to_card_index(card)] == 0)
//			{
//				// 刷新手牌
//				int cards[] = new int[GameConstants.MAX_HH_COUNT];
//				// 显示出牌
