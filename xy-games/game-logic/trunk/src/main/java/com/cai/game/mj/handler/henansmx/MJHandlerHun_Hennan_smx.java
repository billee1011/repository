 ...
 *
 */
public class MJHandlerHun_Hennan_smx  extends AbstractMJHandler<MJTable_SMX> {
	...
	public void exe(MJTable_SMX table) {
		...
		if(table.DEBUG_CARDS_MODE)_da_dian_card=0x17;//
		//显示
		...
		if(GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HENAN_SMX_SHANG_HUN)){
			int card_next=0;
		...
		//延迟调度打点结束（发牌）
		GameSchedule.put(new RevomeHunMiddleCardRunnable_SMX(table.getRoom_id(), _banker),
				3, TimeUnit.SECONDS);
