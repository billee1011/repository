...
public class HandlerGang_JiYuan extends MJHandlerGang<Table_JiYuan> {
    ...
    public boolean exe_gang(Table_JiYuan table) {
        ...
        int gangScore = table.get_di_fen();
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if (i == _seat_index) {
                    continue;
    ...
    public boolean handler_operate_card(Table_JiYuan table, int seat_index, int operate_code, int operate_card) {
        ...
        for (int i = 0; i < table.getTablePlayerNumber(); i++) {
            ...
            jie_pao_count++;
            table.jiePao[i]++;
            table.chr[i].opr_or(Constants_MJ_JiYuan.CHR_BA_TOU_GANG);
        }

        if (jie_pao_count > 0) {
            table.dianPao[_provide_player] += jie_pao_count;
            table.chr[_provide_player].opr_or(Constants_MJ_JiYuan.CHR_FANG_PAO); // 放炮
            if (operate_card == 0) {
                operate_card = _center_card;
            }
