...
public class HandlerDispatchCard_WuYuan extends MJHandlerDispatchCard<Table_WuYuan> {
	...
	public void exe(Table_WuYuan table) {
		...
		if (ting_count > 0) {
			...
			for (int i = 0; i < hand_card_count; i++) {
				...
				if (cards[i] > GameConstants.CARD_ESPECIAL_TYPE_TING) {
					if (table._logic.is_magic_card(cards[i] - GameConstants.CARD_ESPECIAL_TYPE_TING)) {
						cards[i] += GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI;
					}
				} else if (cards[i] < GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI) {
		...
		if (table._logic.is_magic_card(_send_card_data)) {
			table.mo_bao_pai[_seat_index]++;
			table.operate_player_get_card(_seat_index, 1, new int[] { real_card + GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI },
	...
	@Override
	public boolean handler_player_out_card(Table_WuYuan table, int seat_index, int card) {
		if (card > GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI + GameConstants.CARD_ESPECIAL_TYPE_TING) {
			card = card - (GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI + GameConstants.CARD_ESPECIAL_TYPE_TING);
		}
		if (GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI + GameConstants.CARD_ESPECIAL_TYPE_TING > card && card > GameConstants.CARD_ESPECIAL_TYPE_TING) {
			card = card - GameConstants.CARD_ESPECIAL_TYPE_TING;
		}
		if (GameConstants.CARD_ESPECIAL_TYPE_TING > card && card > GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI) {
			card = card - GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI;
		}
	...
	public boolean handler_operate_card(Table_WuYuan table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO:
			...
			// table.set_niao_card(seat_index, GameConstants.INVALID_VALUE,
			// true); // 抓鸟
	...
	public boolean handler_player_be_in_room(Table_WuYuan table, int seat_index) {
		...
		if ((out_ting_count > 0) && (seat_index == _seat_index)) {
			for (int j = 0; j < hand_card_count; j++) {
				...
				if (hand_cards[j] > GameConstants.CARD_ESPECIAL_TYPE_TING) {
					if (table._logic.is_magic_card(hand_cards[j] - GameConstants.CARD_ESPECIAL_TYPE_TING)) {
		...
		for (int i = 0; i < out_ting_count; i++) {
			...
			if (table._logic.is_magic_card(table._playerStatus[seat_index]._hu_out_card_ting[i])) {
				roomResponse.addOutCardTing(table._playerStatus[seat_index]._hu_out_card_ting[i] + GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI
		...
		int ting_cards[] = Arrays.copyOf(table._playerStatus[seat_index]._hu_cards, table._playerStatus[seat_index]._hu_cards.length);
		...
		if (ting_count > 0) {
			for (int i = 0; i < ting_count; i++) {
				...
				} else if (ting_cards[i] > 0 && ting_cards[i] < GameConstants.CARD_ESPECIAL_TYPE_TING) {
		...
		if (_send_card_data == table.joker_card_1) {
			table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data + GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI }, seat_index);
		} else {
