...
public class MJHandlerChiPeng_YD extends MJHandlerChiPeng<MJTable_YD> {
	...
	public void exe(MJTable_YD table) {
		...
		if (_card == table.baoPai) {
			table.GRR._weave_items[_seat_index][wIndex].center_card = _card + GameConstants.CARD_ESPECIAL_TYPE_BAO;
		} else {
		...
		// 打出的宝牌吃碰杠都不算
		if (_card == table.baoPai) {
			table.baoPaiCount[_provider]--;
			if (table.baoPaiCount[_provider] < 0) {
				table.baoPaiCount[_provider] = 0;
			}
		}
		if (table.baoPaiCount[_provider] == 0) {
			table.baoPaiScore[_provider] = 0;
		} else if (table.baoPaiCount[_provider] == 1) {
			table.baoPaiScore[_provider] = 5;
		} else if (table.baoPaiCount[_provider] == 2) {
			table.baoPaiScore[_provider] = 15;
		} else if (table.baoPaiCount[_provider] == 3) {
			table.baoPaiScore[_provider] = 45;
		} else if (table.baoPaiCount[_provider] == 4) {
			table.baoPaiScore[_provider] = 135;
		}
		...
		if (SysParamServerUtil.is_new_algorithm(3000, 3000, 1)) {
			...
			if (ting_count > 0) {
				...
				for (int i = 0; i < tmp_hand_card_count; i++) {
					...
					if (tmp_cards[i] == table.baoPai) {
						tmp_cards[i] += GameConstants.CARD_ESPECIAL_TYPE_BAO;
					}
				}
				table.changeCard(tmp_cards);
		...
		if (table.GRR._left_card_count > 0) {
			// 只检测手上的牌能不能杠
			int cbActionMask = table._logic.analyse_gang_card_all_yd(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
			...
			for (int j = 0; j < hand_card_count; j++) {
				for (int k = 0; k < out_ting_count; k++) {
					if (hand_cards[j] == table._playerStatus[seat_index]._hu_out_card_ting[k]) {
						...
						break;
					} else if (hand_cards[j] == table._logic.switch_to_card_data(table.baoPai)) {
