...
class LouWeaveItem {
	...
	public int nLouWeaveKind[][] = new int[50][10];
	public int nCount;
	public int nSendCount[] = new int[50];
...
public class HHGameLogic {
	...
	public boolean remove_card_by_data_yong_zhou(int cards[], int card_data) {
		int card_count = cards.length;
		...
		// 定义变量
		int cbDeleteCount = 0;
		int cbTempCardData[] = new int[GameConstants.MAX_YONG_ZHOU_HH_COUNT];
	...
	public boolean remove_card_by_index_xpphz(int cards_index[], int card) {
		// 效验扑克
		int card_index = switch_to_card_index_xpphz(card);
		if (card_index < 0 || card_index >= 40) {
			return false;
		}
	...
	// 删除扑克
	public boolean remove_cards_by_index_xpphz(int cards_index[], int cards[], int card_count) {
		// 删除扑克
		for (int i = 0; i < card_count; i++) {
			// 先删除小的 再删除大的 如果都没有 就还原
			if (remove_card_by_index_xpphz(cards_index, toLowCard(cards[i])) == false) {
				if (remove_card_by_index_xpphz(cards_index, toUpCard(cards[i])) == false) {
					// 还原删除
					for (int j = 0; j < i; j++) {
						cards_index[j]++;
					}
	...
	public int check_chi_phz_syzp(int cards_index[], int cur_card, int type_count[], int type_eat_count[], boolean yws_type) {
		int eat_type = 0;
		int count = 0;
		int cur_card_index = switch_to_card_index(cur_card);// 当前牌索引
		// 三牌判断
		...
		/*
		 * // 一五十吃 if (yws_type == true) { if ((card_value == 1) || (card_value
		 * == 5) || (card_value == 10)) { int excursion[] = { 0, 4, 9 }; int
		 * acceptIndex = (get_card_color(cur_card) == 0) ? 0 : 10; int i = 0;
		 * for (; i < excursion.length; i++) { int index = acceptIndex +
		 * excursion[i]; if ((index != cur_card_index) && ((cards_index[index]
		 * == 0) || (cards_index[index] >= 3))) break; } if (i ==
		 * excursion.length) {
		 * 
		 * eat_type |= GameConstants.WIK_YWS; type_count[count++] =
		 * GameConstants.WIK_YWS; } } }
		 */
		// 顺子吃
	...
	public int get_hong_count(WeaveItem weaveitem[], int weave_count) {
		int count = 0;
		for (int i = 0; i < weave_count; i++) {
			...
			case GameConstants.WIK_AN_LONG:
			case GameConstants.WIK_PAO:
				if (!color_hei(weaveitem[i].center_card))
					count += 4;
				continue;
			case GameConstants.WIK_SAO:
			case GameConstants.WIK_PENG:
			...
			case GameConstants.WIK_LEFT:
				for (int j = 0; j < 3; j++) {
					if (!color_hei(weaveitem[i].center_card + j))
						count += 1;
				}
				continue;
			case GameConstants.WIK_CENTER:
				for (int j = 0; j < 3; j++) {
					if (!color_hei(weaveitem[i].center_card + j - 1))
						count += 1;
				}
				continue;
			case GameConstants.WIK_RIGHT:
	...
	public int get_hong_count(AnalyseItem analyseItem) {
		int count = 0;
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			switch (analyseItem.cbWeaveKind[i]) {
			case GameConstants.WIK_TI_LONG:
			case GameConstants.WIK_AN_LONG:
			case GameConstants.WIK_PAO:
				if (!color_hei(analyseItem.cbCenterCard[i]))
					count += 4;
				continue;
			case GameConstants.WIK_SAO:
			case GameConstants.WIK_PENG:
			...
			case GameConstants.WIK_EQS:
				for (int j = 0; j < 3; j++) {
					if (!color_hei(analyseItem.cbCardData[i][j]))
						count += 1;
				}
				continue;
			}
		...
		if ((analyseItem.curCardEye != false) && (!color_hei(analyseItem.cbCardEye)))
			count += 2;
		return count;
	}

	public int get_hei_count(AnalyseItem analyseItem) {
		int count = 0;
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			switch (analyseItem.cbWeaveKind[i]) {
			case GameConstants.WIK_TI_LONG:
	...
	public int get_weave_hu_xi_xpphz(WeaveItem weave_item) {
		switch (weave_item.weave_kind) {
		case GameConstants.WIK_TI_LONG:
		case GameConstants.WIK_AN_LONG:
		case GameConstants.WIK_AN_LONG_LIANG:
			return (get_card_color(weave_item.center_card) % 2 != 0) ? 12 : 9;
		case GameConstants.WIK_PAO:
			return (get_card_color(weave_item.center_card) % 2 != 0) ? 9 : 6;
		case GameConstants.WIK_SAO:
		case GameConstants.WIK_CHOU_SAO:
		case GameConstants.WIK_KAN:
		case GameConstants.WIK_WEI:
		...
		case GameConstants.WIK_CHOU_XIAO:
		case GameConstants.WIK_CHOU_WEI:
			return (get_card_color(weave_item.center_card) % 2 != 0) ? 6 : 3;
		case GameConstants.WIK_PENG:
			return (get_card_color(weave_item.center_card) % 2 != 0) ? 3 : 1;
		case GameConstants.WIK_EQS:
		case GameConstants.WIK_YWS:
			return (get_card_color(weave_item.center_card) % 2 != 0) ? 6 : 3;
		case GameConstants.WIK_LEFT: {
	...
	public boolean is_sx_wu_qian_nian(AnalyseItem analyseItem, WeaveItem weaveItems[], int weaveCount, int cbMingIndexTemp[]) {
		int weave_hu_xi = 0;
		AnalyseItem handItem = new AnalyseItem();
		AnalyseItem weaveItem = new AnalyseItem();
		int handItemTemp[] = new int[GameConstants.MAX_HH_COUNT];
		int weaveItemTemp[] = new int[GameConstants.MAX_HH_COUNT];

		for (int i = 0; i < weaveCount; i++) {
			weaveItem.cbCenterCard[i] = weaveItems[i].center_card;
			weaveItem.cbWeaveKind[i] = weaveItems[i].weave_kind;
		}
		for (int j = 0; j < 6 - weaveCount; j++) {
			if (analyseItem.cbWeaveKind[j] == GameConstants.WIK_NULL)
				continue;
			handItem.cbCenterCard[j] = analyseItem.cbCenterCard[j];
			handItem.cbWeaveKind[j] = analyseItem.cbWeaveKind[j];
		}
		analyse_item_to_card(weaveItem, weaveItemTemp);
		analyse_item_to_card(handItem, handItemTemp);
		if (get_all_hu_xi_awq(weaveItem, weaveItemTemp, cbMingIndexTemp) == 5 && get_all_hu_xi_awq(handItem, handItemTemp, cbMingIndexTemp) == 5) {
			return true;
		}
	...
	public boolean analyse_card_phz_with_invisible_magic(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index,
			int provider_index, int cur_card, List<AnalyseItem> analyseItemArray, boolean has_feng, int hu_xi[]) {
		int cbCardCount = get_card_count_by_index(cards_index);
		hu_xi[0] = 0;
		...
		if (cbCardCount == 0 || cbCardCount % 3 == 0)
			return false;

		int cbLessKindItem = (cbCardCount) / 3;
		boolean bNeedCardEye = (cbCardCount % 3 == 1);
				...
				if (bEnoughCard == true) {
					...
					if ((bNeedCardEye == false) || (cbCardEye != 0)) {
						...
						if (cbCardEye != 0) {
							...
							analyseItem.invisibleMagicCard = cbCardEye;
							analyseItem.invisibleMagicCardKind = 7;

							analyseItemArray.add(analyseItem);
						} else {
							int tmp_hand_cards[] = new int[hh_count];
							int tmp_hand_card_count = switch_to_cards_data(cbCardIndexTemp, tmp_hand_cards);
							if (tmp_hand_card_count == 2) {
								if (tmp_hand_cards[0] == tmp_hand_cards[1]) { // 成坎
									if (cur_card == 0) { // 从偎牌走的胡牌判断
										analyseItem.curCardEye = true;
										analyseItem.cbCardEye = tmp_hand_cards[0];

										analyseItem.cbWeaveKind[cbWeaveCount - 1] = GameConstants.WIK_TI_LONG;
										analyseItem.cbCenterCard[cbWeaveCount - 1] = weaveItem[cbWeaveCount - 1].center_card;
										analyseItem.hu_xi[cbWeaveCount - 1] += 6;
										hu_xi[0] += 6;
										get_weave_card(GameConstants.WIK_TI_LONG, weaveItem[cbWeaveCount - 1].center_card,
												analyseItem.cbCardData[cbWeaveCount - 1]);

										analyseItem.invisibleMagicCard = weaveItem[cbWeaveCount - 1].center_card;
										analyseItem.invisibleMagicCardKind = cbWeaveCount - 1;

										analyseItemArray.add(analyseItem);
									} else {
										boolean tmp_has_kan = false;

										for (int i = 0; i < cbLessKindItem; i++) { // 分析手牌是否有坎
											if (pKindItem[i].cbWeaveKind == GameConstants.WIK_KAN) {
												tmp_has_kan = true;

												analyseItem.curCardEye = true;
												analyseItem.cbCardEye = tmp_hand_cards[0];

												analyseItem.cbWeaveKind[i + cbWeaveCount] = GameConstants.WIK_TI_LONG;
												analyseItem.cbCenterCard[i + cbWeaveCount] = pKindItem[i].cbCenterCard;
												analyseItem.hu_xi[i + cbWeaveCount] += 6;
												hu_xi[0] += 6;
												get_weave_card(GameConstants.WIK_TI_LONG, pKindItem[i].cbCenterCard,
														analyseItem.cbCardData[i + cbWeaveCount]);

												analyseItem.invisibleMagicCard = pKindItem[i].cbCenterCard;
												analyseItem.invisibleMagicCardKind = i + cbWeaveCount;

												analyseItemArray.add(analyseItem);
											}
										}

										if (tmp_has_kan == false) {
											if ((cur_card == tmp_hand_cards[0]) && cards_index[switch_to_card_index(cur_card)] == 3)
												analyseItem.cbWeaveKind[count] = GameConstants.WIK_KAN;
											else if (cur_card != tmp_hand_cards[0])
												analyseItem.cbWeaveKind[count] = GameConstants.WIK_KAN;
											else
												analyseItem.cbWeaveKind[count] = GameConstants.WIK_PENG;

											analyseItem.cbCenterCard[count] = tmp_hand_cards[0];

											analyseItem.cbCardData[count][0] = tmp_hand_cards[0];
											analyseItem.cbCardData[count][1] = tmp_hand_cards[0];
											analyseItem.cbCardData[count][2] = tmp_hand_cards[0];

											WeaveItem weave_item = new WeaveItem();
											weave_item.weave_kind = analyseItem.cbWeaveKind[count];
											weave_item.center_card = analyseItem.cbCenterCard[count];
											analyseItem.hu_xi[count] = get_weave_hu_xi(weave_item);
											hu_xi[0] += analyseItem.hu_xi[count];

											analyseItem.invisibleMagicCard = tmp_hand_cards[0];
											analyseItem.invisibleMagicCardKind = count;

											analyseItemArray.add(analyseItem);
										}
									}
								} else if ((tmp_hand_cards[0] + 16 == tmp_hand_cards[1]) || (tmp_hand_cards[0] == tmp_hand_cards[1] + 16)) { // 成绞
									analyseItem.cbWeaveKind[count] = GameConstants.WIK_XXD;

									analyseItem.cbCenterCard[count] = tmp_hand_cards[0];

									analyseItem.cbCardData[count][0] = tmp_hand_cards[0];
									analyseItem.cbCardData[count][1] = tmp_hand_cards[0];
									analyseItem.cbCardData[count][2] = tmp_hand_cards[1];
									...
									analyseItemArray.add(analyseItem);
								} else if ((tmp_hand_cards[0] + 1 == tmp_hand_cards[1] && tmp_hand_cards[1] <= 0x0a)
										|| (tmp_hand_cards[0] + 1 == tmp_hand_cards[1] && tmp_hand_cards[1] <= 0x1a)) { // 成顺子
									analyseItem.cbWeaveKind[count] = GameConstants.WIK_LEFT;

									if (tmp_hand_cards[0] == 0x01 || tmp_hand_cards[0] == 0x11) { // 第一张牌的值是1
										analyseItem.cbCenterCard[count] = tmp_hand_cards[0];

										analyseItem.cbCardData[count][0] = tmp_hand_cards[0];
										analyseItem.cbCardData[count][1] = tmp_hand_cards[1];
										analyseItem.cbCardData[count][2] = tmp_hand_cards[1] + 1;

										analyseItem.invisibleMagicCard = tmp_hand_cards[1] + 1;
									} else {
										analyseItem.cbCenterCard[count] = tmp_hand_cards[0] - 1;

										analyseItem.cbCardData[count][0] = tmp_hand_cards[0] - 1;
										analyseItem.cbCardData[count][1] = tmp_hand_cards[0];
										analyseItem.cbCardData[count][2] = tmp_hand_cards[1];

										analyseItem.invisibleMagicCard = tmp_hand_cards[0] - 1;
									}
									analyseItem.invisibleMagicCardKind = count;
									...
									analyseItemArray.add(analyseItem);
								} else if ((tmp_hand_cards[0] + 2 == tmp_hand_cards[1] && tmp_hand_cards[1] <= 0x0a)
										|| (tmp_hand_cards[0] + 2 == tmp_hand_cards[1] && tmp_hand_cards[1] <= 0x1a)) {
									analyseItem.cbWeaveKind[count] = GameConstants.WIK_LEFT;

									analyseItem.cbCenterCard[count] = tmp_hand_cards[0];

									analyseItem.cbCardData[count][0] = tmp_hand_cards[0];
									analyseItem.cbCardData[count][1] = tmp_hand_cards[0] + 1;
									analyseItem.cbCardData[count][2] = tmp_hand_cards[1];
									...
									analyseItemArray.add(analyseItem);
								} else if (tmp_hand_cards[0] == 0x02 && tmp_hand_cards[1] == 0x07) { // 成小二七十
									analyseItem.cbWeaveKind[count] = GameConstants.WIK_EQS;

									analyseItem.cbCenterCard[count] = 0x02;

									analyseItem.cbCardData[count][0] = 0x02;
									analyseItem.cbCardData[count][1] = 0x07;
									analyseItem.cbCardData[count][2] = 0x0a;

									analyseItem.invisibleMagicCard = 0x0a;
									analyseItem.invisibleMagicCardKind = count;
									...
									analyseItemArray.add(analyseItem);
								} else if (tmp_hand_cards[0] == 0x02 && tmp_hand_cards[1] == 0x0a) {
									analyseItem.cbWeaveKind[count] = GameConstants.WIK_EQS;
									...
									analyseItem.invisibleMagicCard = 0x07;
									analyseItem.invisibleMagicCardKind = count;
									...
									analyseItemArray.add(analyseItem);
								} else if (tmp_hand_cards[0] == 0x07 && tmp_hand_cards[1] == 0x0a) {
									analyseItem.cbWeaveKind[count] = GameConstants.WIK_EQS;
									...
									analyseItem.invisibleMagicCard = 0x02;
									analyseItem.invisibleMagicCardKind = count;
									...
									analyseItemArray.add(analyseItem);
								} else if (tmp_hand_cards[0] == 0x12 && tmp_hand_cards[1] == 0x17) { // 成大二七十
									analyseItem.cbWeaveKind[count] = GameConstants.WIK_EQS;

									analyseItem.cbCenterCard[count] = 0x12;

									analyseItem.cbCardData[count][0] = 0x12;
									analyseItem.cbCardData[count][1] = 0x17;
									analyseItem.cbCardData[count][2] = 0x1a;

									analyseItem.invisibleMagicCard = 0x1a;
									analyseItem.invisibleMagicCardKind = count;
									...
									analyseItemArray.add(analyseItem);
								} else if (tmp_hand_cards[0] == 0x12 && tmp_hand_cards[1] == 0x1a) {
									analyseItem.cbWeaveKind[count] = GameConstants.WIK_EQS;
									...
									analyseItem.invisibleMagicCard = 0x17;
									analyseItem.invisibleMagicCardKind = count;
									...
									analyseItemArray.add(analyseItem);
								} else if (tmp_hand_cards[0] == 0x17 && tmp_hand_cards[1] == 0x1a) {
									analyseItem.cbWeaveKind[count] = GameConstants.WIK_EQS;
									...
									analyseItem.invisibleMagicCard = 0x12;
									analyseItem.invisibleMagicCardKind = count;
		...
		cbLessKindItem -= 1;
		// 组合分析
		if (cbKindItemCount >= cbLessKindItem && cbLessKindItem >= 0) {
			// 变量定义
			int cbCardIndexTemp[] = new int[GameConstants.MAX_HH_INDEX];
			...
			do {
				...
				// TODO: 特殊情况下剩余四张牌时
				int tmp_card_count = get_card_count_by_index(cbCardIndexTemp);
				if (tmp_card_count != 4)
					bEnoughCard = false;
	...
	public int get_chi_index(int weave_kind){
		switch(weave_kind){
		case GameConstants.WIK_LEFT:
			return 0;
		case GameConstants.WIK_CENTER:
	...
	public boolean delete_chi_index(int weave_kind,int card,int card_index[]){
		
		if(this.is_valid_card(card) == false)
			return false;
		int cbCardBuffer[] = new int[3];
		 get_weave_card(weave_kind, card, cbCardBuffer);
		 for(int i = 0 ;i< 3;i++){
			 card_index[this.switch_to_card_index(cbCardBuffer[i])]--;
		 }
	...
	// 分析扑克
	public boolean analyse_card_xpphz(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index, int cur_card,
			List<AnalyseItem> analyseItemArray, boolean has_feng, int hu_xi[]) {
		// 计算数目
		int cbCardCount = get_card_count_by_index(cards_index);
		hu_xi[0] = 0;
		...
		for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
			if (cards_index[i] == 4) {
				cards_index[i] = 0;
				pao_WeaveItem.center_card = switch_to_card_data(i);
				if ((seat_index == provider_index) || (switch_to_card_index_xpphz(cur_card) != i))
					pao_WeaveItem.weave_kind = GameConstants.WIK_TI_LONG;
				else
					pao_WeaveItem.weave_kind = GameConstants.WIK_PAO;
				b_pao = true;
		...
		if ((cbLessKindItem == 0) && (bNeedCardEye == true)) {
			for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
				if (cards_index[i] == 2) {
					...
					// 变量定义
					AnalyseItem analyseItem = new AnalyseItem();
					analyseItem.change_Weave_length(10, 4);
					...
					for (int j = 0; j < cbWeaveCount; j++) {
						analyseItem.cbWeaveKind[j] = weaveItem[j].weave_kind;
						analyseItem.cbCenterCard[j] = weaveItem[j].center_card;
						analyseItem.hu_xi[j] = get_weave_hu_xi_xpphz(weaveItem[j]);
						hu_xi[0] += analyseItem.hu_xi[j];
						get_weave_card(weaveItem[j].weave_kind, weaveItem[j].center_card, analyseItem.cbCardData[j]);
						count++;
					}
					if (b_pao == true) {
						analyseItem.cbWeaveKind[count] = pao_WeaveItem.weave_kind;
						analyseItem.cbCenterCard[count] = pao_WeaveItem.center_card;
						analyseItem.hu_xi[count] = get_weave_hu_xi_xpphz(pao_WeaveItem);
						hu_xi[0] += analyseItem.hu_xi[count];
						get_weave_card(pao_WeaveItem.weave_kind, pao_WeaveItem.center_card, analyseItem.cbCardData[count]);
					}
					analyseItem.cbCardEye = switch_to_card_data(i);
		...
		for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
			...
			if ((card_date & GameConstants.LOGIC_MASK_VALUE) == 0x02) {
				for (int j = 1; j <= cards_index[i]; j++) {
					if (((cards_index[i] < 3) || (i == switch_to_card_index_xpphz(cur_card)))
							&& ((cards_index[i + 5] >= j) && ((cards_index[i + 5] < 3) || ((i + 5) == switch_to_card_index_xpphz(cur_card))))
							&& ((cards_index[i + 8] >= j) && ((cards_index[i + 8] < 3) || ((i + 8) == switch_to_card_index_xpphz(cur_card))))) {
						kindItem[cbKindItemCount].cbCardIndex[0] = i;
						kindItem[cbKindItemCount].cbCardIndex[1] = i + 5;
						kindItem[cbKindItemCount].cbCardIndex[2] = i + 8;
						kindItem[cbKindItemCount].cbValidIndex[0] = i;
			...
			if ((i < (GameConstants.MAX_HH_INDEX - 2)) && (cards_index[i] > 0) && ((i % 10) <= 7)) {
				for (int j = 1; j <= cards_index[i]; j++) {
					...
					if (((cards_index[i] < 3) || (i == switch_to_card_index_xpphz(cur_card)))
							&& (cards_index[i + 1] >= j && (cards_index[i + 1] < 3 || ((i + 1) == switch_to_card_index_xpphz(cur_card))))
							&& (cards_index[i + 2] >= j && (cards_index[i + 2] < 3 || ((i + 2) == switch_to_card_index_xpphz(cur_card))))) {
						kindItem[cbKindItemCount].cbCardIndex[0] = i;
						kindItem[cbKindItemCount].cbCardIndex[1] = i + 1;
						kindItem[cbKindItemCount].cbCardIndex[2] = i + 2;
						kindItem[cbKindItemCount].cbValidIndex[0] = i;
				...
				if (bEnoughCard == true) {
					...
					if ((bNeedCardEye == false) || (cbCardEye != 0)) {
						...
						for (int i = 0; i < cbLessKindItem; i++) {
							...
							weave_item.weave_kind = pKindItem[i].cbWeaveKind;
							weave_item.center_card = pKindItem[i].cbCenterCard;
							analyseItem.hu_xi[i + cbWeaveCount] = get_weave_hu_xi_xpphz(weave_item);
							hu_xi[0] += analyseItem.hu_xi[i + cbWeaveCount];
							analyseItem.cbCardData[cbWeaveCount + i][0] = switch_to_card_data(pKindItem[i].cbValidIndex[0]);
							analyseItem.cbCardData[cbWeaveCount + i][1] = switch_to_card_data(pKindItem[i].cbValidIndex[1]);
							analyseItem.cbCardData[cbWeaveCount + i][2] = switch_to_card_data(pKindItem[i].cbValidIndex[2]);
	...
	// 分析扑克
	public boolean analyse_card_WC(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index, int cur_card,
			List<AnalyseItem> analyseItemArray, boolean has_feng, int hu_xi[]) {
		// 计算数目
		int cbCardCount = get_card_count_by_index(cards_index);
		hu_xi[0] = 0;
		...
		if (cbKindItemCount >= cbLessKindItem) {
			...
			do {
				...
				if (bEnoughCard == true) {
					...
					boolean hu = false;
					int cbCardIndexTempCount = get_card_count_by_index(cbCardIndexTemp);
					if (cbCardIndexTempCount == 1) {
						hu = true;
						use_magic = true;
						for (int i = 0; i < hh_count; i++) {
							if (cbCardIndexTemp[i] == 1) {
								cbCardEye = switch_to_card_data(i);// 牌眼
								analyseItem.invisibleMagicCardKind = GameConstants.WIK_DUI_ZI;
								analyseItem.invisibleMagicCard = switch_to_card_data(i);
								break;
							}
						}
					} else {
						...
						if (!hu) {
							for (int i = 0; i < hh_count; i++) {
								int eat_type = check_chi(cbCardIndexTemp, switch_to_card_data(i), new int[20], new int[20]);
								if (eat_type != 0) {
									hu = true;
									use_magic = true;

									analyseItem.invisibleMagicCardKind = eat_type;
									analyseItem.invisibleMagicCard = switch_to_card_data(i);

									int n = count++;
									analyseItem.cbWeaveKind[n] = eat_type;
									analyseItem.cbCenterCard[n] = switch_to_card_data(i);
									get_weave_card(eat_type, switch_to_card_data(i), analyseItem.cbCardData[n]);
									break;
								}
							}
						}
					...
					if (hu) {
						for (int i = 0; i < cbWeaveCount; i++) {
							int n = count++;
							analyseItem.cbWeaveKind[n] = weaveItem[i].weave_kind;
							analyseItem.cbCenterCard[n] = weaveItem[i].center_card;
							analyseItem.hu_xi[n] = weaveItem[i].hu_xi;
							hu_xi[0] += weaveItem[i].hu_xi;
							get_weave_card(weaveItem[i].weave_kind, weaveItem[i].center_card, analyseItem.cbCardData[n]);
						}
						// 设置牌型
						int hand_index = -1;
						boolean flag = false;
						for (int i = 0; i < cbLessKindItem; i++) {
							...
							analyseItem.cbWeaveKind[n] = pKindItem[i].cbWeaveKind;
							analyseItem.cbCenterCard[n] = pKindItem[i].cbCenterCard;
							WeaveItem weave_item = new WeaveItem();
							weave_item.weave_kind = pKindItem[i].cbWeaveKind;
							weave_item.center_card = pKindItem[i].cbCenterCard;
							analyseItem.hu_xi[n] = get_weave_hu_xi(weave_item);
							hu_xi[0] += analyseItem.hu_xi[i + cbWeaveCount];
							analyseItem.cbCardData[n][0] = switch_to_card_data(pKindItem[i].cbValidIndex[0]);
							analyseItem.cbCardData[n][1] = switch_to_card_data(pKindItem[i].cbValidIndex[1]);
							analyseItem.cbCardData[n][2] = switch_to_card_data(pKindItem[i].cbValidIndex[2]);

						}

						if (b_pao == true) {
							int n = count++;
							analyseItem.cbWeaveKind[n] = pao_WeaveItem.weave_kind;
							analyseItem.cbCenterCard[n] = pao_WeaveItem.center_card;
							analyseItem.hu_xi[n] = get_weave_hu_xi(pao_WeaveItem);
							hu_xi[0] += analyseItem.hu_xi[n];
							get_weave_card(pao_WeaveItem.weave_kind, pao_WeaveItem.center_card, analyseItem.cbCardData[n]);
						}

						if (cur_card == 0 && !use_magic && cbWeaveCount > 0 && (GameConstants.WIK_WEI == weaveItem[cbWeaveCount - 1].weave_kind
								|| GameConstants.WIK_CHOU_WEI == weaveItem[cbWeaveCount - 1].weave_kind)) {
							for (int i = 0; i < count; i++) {
								if (weaveItem[cbWeaveCount - 1].weave_kind == analyseItem.cbWeaveKind[i]
										&& weaveItem[cbWeaveCount - 1].center_card == analyseItem.cbCenterCard[i]) {
									analyseItem.cbWeaveKind[i] = GameConstants.WIK_TI_LONG;
									analyseItem.invisibleMagicCard = analyseItem.cbCenterCard[i];
									analyseItem.invisibleMagicCardKind = analyseItem.cbWeaveKind[i];
									use_magic = true;
									break;
								}
							}
						}
						if (!use_magic && hand_index != -1) {
							for (int i = hand_index; i < hand_index + cbLessKindItem; i++) {
								if (analyseItem.cbWeaveKind[i] == GameConstants.WIK_KAN) {
									use_magic = true;
									analyseItem.cbWeaveKind[i] = GameConstants.WIK_TI_LONG;
									analyseItem.invisibleMagicCard = analyseItem.cbCenterCard[i];
									analyseItem.invisibleMagicCardKind = analyseItem.cbWeaveKind[i];
							...
							if (!use_magic && cbCardEye != 0) {
								int cbCardEyeColor = get_card_color(cbCardEye);
								int curCardColor = get_card_color(cur_card);
								if (curCardColor == cbCardEyeColor || cbCardEyeColor < curCardColor) {
									for (int i = hand_index; i < hand_index + cbLessKindItem; i++) {
										if (analyseItem.cbCenterCard[i] == cur_card && analyseItem.cbWeaveKind[i] == GameConstants.WIK_PENG) {
											use_magic = true;
											analyseItem.cbWeaveKind[i] = GameConstants.WIK_PAO;
											analyseItem.invisibleMagicCard = analyseItem.cbCenterCard[i];
											analyseItem.invisibleMagicCardKind = analyseItem.cbWeaveKind[i];
											break;
										}
				...
				// 设置索引
				int cbLessKindItemTmp = cbLessKindItem == 0 ? 1 : cbLessKindItem;
				int cbKindItemCountTmp = cbKindItemCount == 0 ? 1 : cbKindItemCount;
				if (cbIndex[cbLessKindItemTmp - 1] == (cbKindItemCountTmp - 1)) {
					int i = cbLessKindItemTmp - 1;
					for (; i > 0; i--) {
						if ((cbIndex[i - 1] + 1) != cbIndex[i]) {
							int cbNewIndex = cbIndex[i - 1];
							for (int j = (i - 1); j < cbLessKindItemTmp; j++)
								cbIndex[j] = cbNewIndex + j - i + 2;
							break;
						}
					}
					...
					cbIndex[cbLessKindItemTmp - 1]++;
			} while (true);
		}

		// 组合分析
		int cbTempLessKindItem = cbLessKindItem - 1;
		if (cbKindItemCount >= cbTempLessKindItem && cbTempLessKindItem >= 0) {
			// 变量定义
			int cbCardIndexTemp[] = new int[GameConstants.MAX_HH_INDEX];
			...
			do {
				...
				for (int i = 0; i < cbTempLessKindItem; i++) {
					pKindItem[i].cbWeaveKind = kindItem[cbIndex[i]].cbWeaveKind;
					pKindItem[i].cbCenterCard = kindItem[cbIndex[i]].cbCenterCard;
					for (int j = 0; j < 3; j++) {
						pKindItem[i].cbCardIndex[j] = kindItem[cbIndex[i]].cbCardIndex[j];
				...
				// 设置组合
				int count = 0;
				for (int i = 0; i < cbTempLessKindItem * 3; i++) {
					// 存在判断
					int cbCardIndex = pKindItem[i / 3].cbValidIndex[i % 3];
				...
				if (bEnoughCard == true) {
					...
					int cbCardIndexTempCount = get_card_count_by_index(cbCardIndexTemp);
					boolean is_two = true;
					int cbWeicard[] = new int[] { -1, -1 };
					int cbduiCard[] = new int[] { -1, -1 };
					int Wei_count = 0;
					int dui_count = 1;
					// 变量定义
					int tmp[] = new int[GameConstants.MAX_HH_INDEX];
					if (cbCardIndexTempCount == 4) {
						if (seat_index != provider_index) {
							for (int i = 0; i < hh_count; i++) {
								if (cbCardIndexTemp[i] == 2) {
									cbWeicard[Wei_count++] = i;
									cbduiCard[dui_count--] = i;
								} else if (cbCardIndexTemp[i] == 3) {
									cbWeicard[Wei_count++] = i;
									cbduiCard[dui_count--] = i;
									tmp[i]++;
									is_two = false;
								} else if (cbCardIndexTemp[i] != 0) {
									tmp[i]++;
									is_two = false;
								}
							}
					...
					// if (cbCardIndexTemp[i] == 1) {
					// cbCardEye = switch_to_card_data(i);// 牌眼
					// analyseItem.invisibleMagicCardKind =
					// GameConstants.WIK_DUI_ZI;
					// analyseItem.invisibleMagicCard = switch_to_card_data(i);
					// break;
					// }
					// }
					...
					// if (!hu) {
					// for (int i = 0; i < hh_count; i++) {
					// int eat_type = check_chi(cbCardIndexTemp,
					// switch_to_card_data(i), new
					// int[20],
					// new int[20]);
					// if (eat_type != 0) {
					// hu = true;
					...
					// analyseItem.cbWeaveKind[n] = eat_type;
					// analyseItem.cbCenterCard[n] = switch_to_card_data(i);
					// get_weave_card(eat_type, switch_to_card_data(i),
					// analyseItem.cbCardData[n]);
					// break;
					// }
					// }
					// }
					...
					if (is_two) {
						for (int j = 0; j < 2; j++) {
							...
							WeaveItem wei_WeaveItem = new WeaveItem();
							wei_WeaveItem.center_card = switch_to_card_data(cbWeicard[j]);
							if (cur_card != wei_WeaveItem.center_card || cards_index[switch_to_card_index(cur_card)] == 3) {
								wei_WeaveItem.weave_kind = GameConstants.WIK_KAN;
							} else if (seat_index == provider_index) {
								wei_WeaveItem.weave_kind = GameConstants.WIK_WEI;
							} else {
								wei_WeaveItem.weave_kind = GameConstants.WIK_PENG;
							}
							b_wei = true;
							hu_xi[0] = 0;
							...
							if (b_wei == true) {
								int n = count++;
								analyseItem.cbWeaveKind[n] = wei_WeaveItem.weave_kind;
								analyseItem.cbCenterCard[n] = wei_WeaveItem.center_card;
								analyseItem.hu_xi[n] = get_weave_hu_xi(wei_WeaveItem);
								hu_xi[0] += analyseItem.hu_xi[n];
								get_weave_card(wei_WeaveItem.weave_kind, wei_WeaveItem.center_card, analyseItem.cbCardData[n]);
								analyseItem.invisibleMagicCard = analyseItem.cbCenterCard[n];
								analyseItem.invisibleMagicCardKind = analyseItem.cbWeaveKind[n];
							}
							// if(hu_xi[0]<15)
							// return false;
							// 插入结果
							...
							analyseItem.curCardEye = true;
							analyseItem.cbCardEye = switch_to_card_data(cbduiCard[j]);

							// 插入结果
							analyseItemArray.add(analyseItem);
						}
					} else if (cbduiCard[1] != -1) {
						// 牌眼判断
						AnalyseItem analyseItem = new AnalyseItem();
						count = 0;
						boolean flag = false;
						for (int i = 0; i < hh_count; i++) {
							int eat_type = check_chi(tmp, switch_to_card_data(i), new int[20], new int[20]);
							if (eat_type != 0) {
								analyseItem.invisibleMagicCardKind = eat_type;
								analyseItem.invisibleMagicCard = switch_to_card_data(i);
								int n = count++;
						...
						if (flag) {
							...
							analyseItem.curCardEye = true;
							analyseItem.cbCardEye = switch_to_card_data(cbduiCard[1]);
				...
				// 设置索引
				int cbLessKindItemTmp = cbTempLessKindItem == 0 ? 1 : cbTempLessKindItem;
				int cbKindItemCountTmp = cbKindItemCount == 0 ? 1 : cbKindItemCount;
				if (cbIndex[cbLessKindItemTmp - 1] == (cbKindItemCountTmp - 1)) {
					int i = cbLessKindItemTmp - 1;
					for (; i > 0; i--) {
	...
	public int get_action_rank_yongzhou(int player_action, int win_type) {
		if (win_type == 1 && (player_action == GameConstants.WIK_WANG_ZHA || player_action == GameConstants.WIK_WANG_CHUANG
				|| player_action == GameConstants.WIK_WANG_DIAO)) {
			return 50;
		}
		...
		if (win_type == 2 && (player_action == GameConstants.WIK_WANG_ZHA || player_action == GameConstants.WIK_WANG_CHUANG
				|| player_action == GameConstants.WIK_WANG_DIAO)) {
			return 5;
		}
	...
	public int get_action_rank_with_pao_hu(int player_action) {
		// 自摸牌等级
		if (player_action == GameConstants.WIK_ZI_MO) {
			return 60;
		}
	...
	public int get_action_list_rank_yongzhou(int action_count, int action[], int win_type) {
		int MAX_HH_INDEX = 0;

		for (int i = 0; i < action_count; i++) {
			int index = get_action_rank_yongzhou(action[i], win_type);
			if (MAX_HH_INDEX < index) {
				MAX_HH_INDEX = index;
			}
		}
	...
	public int get_action_list_rank_with_pao_hu(int action_count, int action[]) {
		int MAX_HH_INDEX = 0;

		for (int i = 0; i < action_count; i++) {
			int index = get_action_rank_with_pao_hu(action[i]);
			if (MAX_HH_INDEX < index) {
				MAX_HH_INDEX = index;
			}
	...
	public int get_chi_hu_action_rank_yl(int seat_index, int da_pai_count, int xiao_pai_count, int hong_pai_count, ChiHuRight chiHuRight) {
		int wFanShu = 0;

		if (!(chiHuRight.opr_and(GameConstants.CHR_TEN_HONG_PAI)).is_empty()) {
			wFanShu += 4 + (hong_pai_count - 10);
		}
		...
		if (!(chiHuRight.opr_and(GameConstants.CHR_TUAN_CHD)).is_empty()) {
			wFanShu += 8;
		}
		if (!(chiHuRight.opr_and(GameConstants.CHR_DUI_ZI_HU)).is_empty()) {
			wFanShu += 8;
	...
	public int get_chi_hu_action_rank_hgw468(int seat_index, int da_pai_count, int xiao_pai_count, int hong_pai_count, ChiHuRight chiHuRight) {
		int wFanShu = 0;
	...
	// 有效判断
	public boolean is_valid_card_yongzhou(int card) {
	...
	public int switch_to_card_index_yongzhou(int card) {
		if (is_valid_card_yongzhou(card) == false) {
			return Constants_YongZhou.MAX_CARD_INDEX;
		}

		if (card == Constants_YongZhou.MAGIC_CARD)
			return Constants_YongZhou.MAX_CARD_INDEX - 1;
	...
	public int switch_to_card_index_xpphz(int card) {
		int color = get_card_color(card);
		int value = get_card_value(card);
		int index = color * 10 + value - 1;
		return index;
	 ...
	 * @return
	 */
	public int switch_to_cards_index_yongzhou(int cards_data[], int start_index, int card_count, int cards_index[]) {
		for (int i = 0; i < card_count; i++) {
			cards_index[switch_to_card_index_yongzhou(cards_data[start_index + i])]++;
		}
		return card_count;
	}
	 ...
	 * @return
	 */
	public int switch_to_cards_data_yongzhou(int cards_index[], int cards_data[]) {
		// 转换扑克
		int cbPosition = 0;
		...
		for (int i = 0; i < Constants_YongZhou.MAX_CARD_INDEX; i++) {
			if (cards_index[i] > 0) {
				for (int j = 0; j < cards_index[i]; j++) {
					cards_data[cbPosition++] = switch_to_card_data_yongzhou(i);
				}
			}
		}
	...
	public int switch_to_card_data_yongzhou(int card_index) {
		if (card_index == Constants_YongZhou.MAX_CARD_INDEX - 1) {
			return Constants_YongZhou.MAGIC_CARD;
		}
		return ((card_index / 10) << 4) | (card_index % 10 + 1);
	}
	...
	public static class AnalyseItem {
		...
		public int invisibleMagicCard; // 隐形王牌实际存的值
		public int invisibleMagicCardKind; // 隐形王牌实际存位置

		/**
		 * 修改组合数组长度 (为了修改溆浦的箍臭超过7个组合)
		 * 
		 * @param maxWeaverCount
		 *            最大组合数
		 * @param maxCardCount
		 *            最大真实牌局数
		 * @author WalkerGeek
		 */
		public void change_Weave_length(int maxWeaverCount, int maxCardCount) {
			this.cbWeaveKind = new int[maxWeaverCount];// 组合类型
			this.cbCenterCard = new int[maxWeaverCount];// 中心扑克
			this.cbCardData = new int[maxWeaverCount][maxCardCount]; // 实际扑克
			this.hu_xi = new int[maxWeaverCount];// 计算胡息
		}
