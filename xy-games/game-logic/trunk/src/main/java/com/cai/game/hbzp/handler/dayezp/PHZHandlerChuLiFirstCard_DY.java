...
public class PHZHandlerChuLiFirstCard_DY extends PHZHandlerDispatchCard<HBPHZTable> {
...
//		if(table.has_rule(GameConstants.GAME_RULE_DI_ERZI_LIANG_PAI))
		{
			...
			for(int j = 0; j<table.getTablePlayerNumber();j++){
				...
				for(int i = 0; i< an_long_count[j];i++)
				{
					...
					table.GRR._weave_items[j][cbWeaveIndex].weave_kind = GameConstants.WIK_HBZP_LONG;
					table.GRR._weave_items[j][cbWeaveIndex].provide_player = j;
					table.GRR._weave_items[j][cbWeaveIndex].hu_xi = table._logic.get_weave_hu_xi(table.GRR._weave_items[j][cbWeaveIndex],table._fan_jiang_card);
					table.GRR._weave_count[j]++;
					table._long_count[j]++;
					// 删除手上的牌
					table.GRR._cards_index[j][an_long_Index[j][i]] = 0;
					
					table.GRR._card_count[j] = table._logic.get_card_count_by_index(table.GRR._cards_index[_seat_index]);
			...
			if(bHupai == 0){
				...
				// 加到手牌
				for(int i = 0; i<table.getTablePlayerNumber();i++){
					if(an_long_count[i] > 0 )
					{
						int _action = GameConstants.WIK_HBZP_LONG;
						//效果
						table.operate_effect_action(i,GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[]{_action}, 1,GameConstants.INVALID_SEAT);
...
//				}
				for(int i = 0; i< table.getTablePlayerNumber();i++){
					if(an_long_count[i] >= 2)	
					{
						table._ti_mul_long[i] = an_long_count[i]-1;
						table._is_cannot_kai_zhao[i] = -1; 
					}
					if(table._long_count[i] == 5)
						table._is_cannot_kai_zhao[i] = 2;
				}
...
//					table.operate_remove_discard(table._current_player, table.GRR._discard_count[table._current_player]);
					...
					if(table._is_cannot_kai_zhao[_seat_index] <= 0)
					{
						if(table._ti_mul_long[_seat_index]>1)
						{
