 ...
 * 
 */
package com.cai.game.shisanzhang.ssz_zz;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class SSZTable_ZZ extends SSZTable {
	/**
	 * 
	 */
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		if (this.has_rule(GameConstants.GAME_RULE_SSZ_JD_DEAL_CARD_60)) {
			_deal_card_time = getRuleValue(GameConstants.GAME_RULE_SSZ_JD_DEAL_CARD_60);
		} else if (this.has_rule(GameConstants.GAME_RULE_SSZ_JD_DEAL_CARD_40)) {
			_deal_card_time = getRuleValue(GameConstants.GAME_RULE_SSZ_JD_DEAL_CARD_40);
		} else if (this.has_rule(GameConstants.GAME_RULE_SSZ_JD_DEAL_CARD_25)) {
			_deal_card_time = getRuleValue(GameConstants.GAME_RULE_SSZ_JD_DEAL_CARD_25);
		} else {
			_deal_card_time = 0;
		}
	...
	public boolean game_start_sszzz() {
	...
	private void test_cards() {
		...
		int cards[] = new int[] { 18, 10, 12, 2, 43, 24, 20, 27, 6, 61, 25, 34, 54, 8, 13, 44, 17, 22, 55, 42, 28, 19, 40, 38, 7, 23, 5, 9, 37, 36, 4,
				49, 58, 26, 3, 33, 39, 51, 41, 50, 21, 52, 45, 29, 1, 11, 59, 57, 35, 53, 60, 56 };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			for (int j = 0; j < get_hand_card_count_max(); j++) {
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		ret = this.handler_game_finish_sszzz(seat_index, reason);
		return ret;
	}

	public boolean handler_game_finish_sszzz(int seat_index, int reason) {
		...
		if (GRR != null && reason == GameConstants.Game_End_NORMAL) {
			// 计算分数
			cal_score_ssz_zz(end_score, fitst_Win, second_Win, three_Win, special_score, extera_score_first, extera_score_second, extera_score_three,
	...
	// 结算分数
	public void cal_score_ssz_zz(int end_score[], int fitst_Win[][], int second_Win[][], int three_Win[][], int special_score[][],
		...
		if (!has_rule(GameConstants.GAME_RULE_SSZ_JD_ZHUANG_TONGBI)) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				if (_is_special[i] || _is_special[GRR._banker_player]) {
					...
					int score = _logic.get_special_score_zz(special_type);
					int score_toher = _logic.get_special_score_zz(special_type_ohter);
					if (special_type != GameConstants.SSZ_CT_INVALID || special_type_ohter != GameConstants.SSZ_CT_INVALID) {
						if (score > score_toher) {
							special_score[i][GRR._banker_player] += score;
							special_score[GRR._banker_player][i] -= score;
							end_score[i] += score;
							end_score[GRR._banker_player] -= score;
						} else if (score < score_toher) {
							special_score[GRR._banker_player][i] += score_toher;
							special_score[i][GRR._banker_player] -= score_toher;
							end_score[GRR._banker_player] += score_toher;
							end_score[i] -= score_toher;
						}
					}
					continue;
				}
				...
				int type = _logic.CompareCard(bFirstList, bNextList, 3, 3, true, has_rule(GameConstants.GAME_RULE_SSZ_BI_HUA_SE));
				if (type == 1) {
				...
				type = _logic.CompareCard(bFirstList, bNextList, 5, 5, true, has_rule(GameConstants.GAME_RULE_SSZ_BI_HUA_SE));
				if (type == 1) {
