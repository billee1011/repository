...
public class MJHandlerDispatchCard_HeNanpds extends MJHandlerDispatchCard<MJTable> {
	...
	public void exe(MJTable table) {
		...
		if (table.has_rule(GameConstants.GAME_RULE_HENAN_LCI)) {
			// 不留底玩法 剩余0张 结算
			llcard = 0;
		}
		...
		if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HENAN_CHU_FENG_BAO_TING)
			...
			if (table._playerStatus[_seat_index].is_bao_ting() == true) {
				...
				int start = GameConstants.MAX_ZI;
				int end = GameConstants.MAX_ZI + 4;
				if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HENAN_BAO_TING)) {
					start = 0;
					if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HENAN_DAIFENG))
						end = GameConstants.MAX_ZI_FENG;
					else
						end = GameConstants.MAX_ZI;
				}

				for (int i = start; i < end; i++) {
					count = table.GRR._cards_index[_seat_index][i];
					if ((count == 1 && i != send_card_index) || count > 1) {
		...
		if ((GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HENAN_CHU_FENG_BAO_TING)
			...
			action = table.analyse_chi_hu_card_henan(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
			...
			if (cbActionMask != GameConstants.WIK_NULL) {// 有杠
				...
				for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
					if ((table._playerStatus[_seat_index].is_bao_ting() == false) || (table._playerStatus[_seat_index].is_bao_ting() == true && table
							.check_gang_huan_zhang(GameConstants.GAME_TYPE_HENAN_PDS, _seat_index, m_gangCardResult.cbCardData[i]) == false)) {
						has_gang = true;
						// 加上杠
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			table.process_chi_hu_player_score_henan_pds(_seat_index, _seat_index, operate_card, true);
