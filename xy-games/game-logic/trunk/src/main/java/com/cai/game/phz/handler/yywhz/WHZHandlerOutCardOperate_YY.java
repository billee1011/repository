...
public class WHZHandlerOutCardOperate_YY extends PHZHandlerOutCardOperate<YYWHZTable> {
	@Override
	public void exe(YYWHZTable table) {
		...
		if (table.GRR._cannot_out_index[_out_card_player][table._logic.switch_to_card_index(_out_card_data)] >= 1) {
			table.log_error(_out_card_player + "出牌出错  WHZHandlerOutCardOperate_YY " + _out_card_data);

			// 刷新手牌
			int cards[] = new int[GameConstants.MAX_HH_COUNT];
			// 刷新自己手牌
			int hand_card_count = table._logic.switch_to_cards_data(table.GRR._cards_index[_out_card_player], cards);
			table.operate_player_cards(_out_card_player, hand_card_count, cards,
					table.GRR._weave_count[_out_card_player], table.GRR._weave_items[_out_card_player]);

			table._playerStatus[_out_card_player].set_status(GameConstants.Player_Status_OUT_CARD);// 出牌状态
		...
		bAroseAction = table.estimate_player_out_card_respond_yywhz(_out_card_player, _out_card_data,
		...
		if (bAroseAction == false) {
			...
			table.cannot_chi_card(next_player, _out_card_data, table.GRR._cards_index[next_player]);
			//
	...
	public boolean handler_operate_card(YYWHZTable table, int seat_index, int operate_code, int operate_card,
		...
		//
		// table.process_chi_hu_player_operate(target_player, operate_card,
		// false);
		// table.process_chi_hu_player_score_yywhz(target_player,
		// _out_card_player, _out_card_data, false);
		//
		// // 记录
		// // table._player_result.jie_pao_count[target_player]++;
		// table._player_result.dian_pao_count[_out_card_player]++;
		// table.countChiHuTimes_wmq(target_player, false);
		//
		//
		...
		// }
		// GameSchedule.put(
		// new GameFinishRunnable(table.getRoom_id(), table._banker_select,
