...
public class PHHandleAddFirstCard_YX extends PHHandlerDispatchCard<PHTable> {
	...
	public void exe(PHTable table) {
		...
		table.operate_player_cards(_seat_index, hand_card_count, cards, table.GRR._weave_count[_seat_index],
				table.GRR._weave_items[_seat_index],1);
		...
		for(int j = 0; j<table.getTablePlayerNumber();j++)
		{
			...
			if(an_long_count[j] +an_sao_count[j] >0 )
			{
				table._send_card_data = 0;
				table.exe_chuli_first_card(_seat_index,GameConstants.WIK_NULL,1200);
				return ; 
			}
		}
...
//			table.operate_player_get_card(_seat_index, 0, null, GameConstants.INVALID_SEAT,false);
			...
			curPlayerStatus.set_handler_status(GameConstants.PH_STATUS_ADD_FIRST_CARD);
...
//			table.operate_player_get_card(_seat_index, 0, null, GameConstants.INVALID_SEAT,false);
		...
		if (action != GameConstants.WIK_NULL) {
			...
			if(table.has_rule(GameConstants.GAME_RULE_QZ_HU_PAI_ON)||table._warning[_seat_index] == 2)
			{
				 table._playerStatus[_seat_index].set_handler_status(GameConstants.PH_STATUS_ADD_FIRST_CARD);
				 table._handler_status_index[_seat_index] = GameConstants.PH_STATUS_ADD_FIRST_CARD;
...
//			table.operate_player_get_card(_seat_index, 0, null, GameConstants.INVALID_SEAT,false);
	...
	//	assert handler_status == GameConstants.PH_STATUS_ADD_FIRST_CARD;
		if(handler_status != GameConstants.PH_STATUS_ADD_FIRST_CARD){
			table.log_error("DispatchCard handler_status = "+ handler_status+"!= "+GameConstants.PH_STATUS_ADD_FIRST_CARD+"seat_index = " + seat_index);
//			return false;
		}
