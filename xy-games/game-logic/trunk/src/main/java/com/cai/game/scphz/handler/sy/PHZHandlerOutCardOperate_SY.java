package com.cai.game.scphz.handler.sy;
...
public class PHZHandlerOutCardOperate_SY extends PHZHandlerOutCardOperate<SCPHZTable> {
	@Override
	public void exe(SCPHZTable table) {
		...
		while(loop < table.getTablePlayerNumber()){
			...
			loop ++;
			action_pao[i] = table.estimate_player_respond_yj(i,_out_card_player ,_out_card_data, pao_type[i],false);
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				int all_hu_xi = 0;
				table._guo_hu_hu_xi[i][table._logic.switch_to_card_index(_out_card_data)] = all_hu_xi;
		...
		if (ting_count > 0) {
			...
			table._ting_card[_out_card_player] = true;
		} else {
			ting_cards[0] = 0;
		...
		for(int i = 0; i<table.getTablePlayerNumber();i++){ 
			if(i==_out_card_player) continue;
			 if((action_pao[i] != GameConstants.WIK_NULL)&& (bHupai == 0||action_hu[i] == GameConstants.WIK_NULL)){
				 int pao_type_out[] = new int[1];
					ti_pao = table.estimate_player_respond_yj(i,_out_card_player ,_out_card_data, pao_type_out,false);
					if(ti_pao != GameConstants.WIK_NULL){
						table._playerStatus[i].add_action(GameConstants.WIK_PAO);
						table._playerStatus[i].add_pao(_out_card_data, i);
					}
			 	}
		...
		if(ti_pao == GameConstants.WIK_NULL )
		{
			...
			bAroseAction = table.estimate_player_out_card_respond_yj(_out_card_player,_out_card_data,false);// ,
		}
																													// EstimatKind.EstimatKind_OutCar
		for(int i = 0; i < table.getTablePlayerNumber();i++){
			if(table._playerStatus[i].has_action_by_code(GameConstants.WIK_PENG)){
				table._playerStatus[i].delete_action(GameConstants.WIK_NULL);
				if(bHupai == 0&&table.getRuleValue(GameConstants.GAME_RULE_PHZ_SY_PK_NO_DISPLAYER) == 1)
				{
					int cbRemoveCard[] = new int[] { _out_card_data, _out_card_data };
					if (!table._logic.remove_cards_by_index(table.GRR._cards_index[i], cbRemoveCard, 2)) {
						table.log_player_error(i, "碰牌删除出错");
						return ;
					}
					 table.exe_chi_peng(i, _out_card_player, GameConstants.WIK_PENG, _out_card_data, GameConstants.CHI_PENG_TYPE_OUT_CARD, -1);
					 return ;
				}
			}
			if(action_pao[i]!=GameConstants.WIK_NULL){
				if(bHupai == 0&&table.getRuleValue(GameConstants.GAME_RULE_PHZ_SY_PK_NO_DISPLAYER) == 1)
				{
					table.exe_gang(i, _out_card_player, _out_card_data, action_pao[i], pao_type[i][0], false, true, false, 1000);
					return ;
				}
			}
		}																											// EstimatKind.EstimatKind_OutCar
...
//		}
		...
		case GameConstants.WIK_PAO: // 杠牌操作
		{
			...
			int pao_type[] = new int[1];
			int action = table.estimate_player_respond_yj(target_player, this._out_card_player, operate_card, pao_type, false);
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_sy(target_player, _out_card_player, _out_card_data, false);
