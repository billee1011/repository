...
public class HandlerDispatchCard_XiAn extends MJHandlerDispatchCard<Table_XiAn> {
	...
	public HandlerDispatchCard_XiAn() {
	...
	public void exe(Table_XiAn table) {
		...
		// 流局处理
		if (table.GRR._left_card_count <= table.getLiuJuCardCount()) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		// 判断是否是杠后抓牌,杠开花只算接杠
		int card_type = Constants_MJ_XI_AN.HU_CARD_TYPE_ZI_MO;
		// 检查牌型，是否是七对，4鬼，听牌
	...
	@Override
	public boolean handler_player_out_card(Table_XiAn table, int seat_index, int card) {
