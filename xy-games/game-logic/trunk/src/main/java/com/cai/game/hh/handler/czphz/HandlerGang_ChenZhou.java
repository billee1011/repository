...
public class HandlerGang_ChenZhou extends HHHandlerGang<Table_ChenZhou> {
	...
	@Override
	protected boolean exe_gang(Table_ChenZhou table) {
		...
		if ((table._long_count[_seat_index] <= 1 || GameConstants.SAO_TYPE_MINE_SAO == _type)
			...
			if (pai_count == 0) { // 如果是第1个提跑或者是偎牌，如果没牌可以出了，判断能不能胡，不能胡，设置成相公
				...
				if (b_hu_xi == true) { // 如果能胡
					...
					if (_seat_index != _provide_player) {
						chr.opr_or(Constants_ChenZhou.CHR_JEI_PAO_HU);
					} else {
						chr.opr_or(Constants_ChenZhou.CHR_ZI_MO);
					}
					...
					if (table.has_rule(Constants_ChenZhou.GAME_RULE_QIANG_ZHI_HU)) { // 如果有强制胡牌
					    if (_seat_index != _provide_player) {
	                        GameSchedule.put(new Runnable() {
	                            @Override
	                            public void run() {
