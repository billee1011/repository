...
public class PHZHandlerOutCardOperate_YJ extends PHZHandlerOutCardOperate<SCPHZTable> {
	@Override
	public void exe(SCPHZTable table) {
		...
		while(loop < table.getTablePlayerNumber()){
			...
			{
				fan_shu = table._logic.get_chi_hu_action_rank_yj(chr[i]);
			}
			if (action_hu[i] != GameConstants.WIK_NULL&&fan_shu>table._fan_shu[i]) {
				int all_hu_xi = 0;
		...
		for(int i = 0; i < table.getTablePlayerNumber();i++){
			...
			if((action_hu[i]!=GameConstants.WIK_NULL&&!(chr[i].opr_and_long(GameConstants.CHR_YJ_SKHU)).is_empty())){
				table._playerStatus[i].delete_action(GameConstants.WIK_PAO);
			}
		}																											// EstimatKind.EstimatKind_OutCar
...
//		}
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_yj(target_player, _out_card_player, _out_card_data, false);
