...
public class MJHandlerYaoHaiDi_ZhuZhouWang extends AbstractMJHandler<Table_ZhuZhouWang> {

	private static Logger logger = Logger.getLogger(MJHandlerYaoHaiDi_ZhuZhouWang.class);
	 ...
	 * @param _card_data
	 */
	public void reset_status(int _seat_index, int type) {
		_type = type;
	...
	public void exe(Table_ZhuZhouWang table) {
		...
		table.operate_out_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants.OUT_CARD_TYPE_XIA_ZI,
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			card_type = GameConstants_ZhuZhouWang.HU_CARD_TYPE_JIE_PAO;
			if (_type == GameConstants_ZhuZhouWang.HU_CARD_TYPE_GANG_KAI) {
				card_type = GameConstants_ZhuZhouWang.HU_CARD_TYPE_GANG_SHANG_PAO;
			}
			action = table.analyse_chi_hu_card_pao(table.GRR._cards_index[i], table.GRR._weave_items[i],
					table.GRR._weave_count[i], _send_card_data, chr, card_type, i);
	...
	public boolean handler_operate_card(Table_ZhuZhouWang table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			if (seat_index == _seat_index) {
				table.GRR._chi_hu_rights[seat_index].opr_or(GameConstants_ZhuZhouWang.CHR_HAI_DI_LAO_YUE);
			} else {
				table.GRR._chi_hu_rights[seat_index].opr_or(GameConstants_ZhuZhouWang.CHR_HAI_DI_PAO);
			}
			// 结束信息
			table.GRR._chi_hu_card[seat_index][0] = target_card;
			// 要删除手上的牌,因为这张海底并没有入手
			// table.process_chi_hu_player_operate_cs(_seat_index, new int[] {
			// _send_card_data }, 1, false);
			// table.process_chi_hu_player_score_cs(_seat_index, _seat_index, true);

			table.process_chi_hu_player_operate(seat_index, _send_card_data, false);

			table.process_chi_hu_player_score(seat_index, _seat_index, _send_card_data, true);
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				table.GRR._chi_hu_rights[i].opr_or(GameConstants_ZhuZhouWang.CHR_HAI_DI_PAO);

				// int cbWeaveItemCount = table.GRR._weave_count[i];
				// WeaveItem pWeaveItem[] = table.GRR._weave_items[i];
				//
				// table.GRR._chi_hu_kind[i] =
				// table.analyse_chi_hu_card(table.GRR._cards_index[i], pWeaveItem,
				...
				table.process_chi_hu_player_operate(i, _send_card_data, false);
