...
public class HandleChiPeng_SongYuan extends AbstractMJHandler<MjTable_SongYuan> {
	...
	public void exe(MjTable_SongYuan table) {
		...
		if (currentPlayerStatus.has_action()) {
			...
			if(table.is_huan_bao() && table.GRR._left_card_count > 4 && table.m_fan_bao_state){
				int huan_bao_payler = GameConstants.INVALID_SEAT;
				for(int i = 0;i < table.getTablePlayerNumber(); i++){
					huan_bao_payler = (_seat_index + GameConstants.GAME_PLAYER + i) % GameConstants.GAME_PLAYER;
					if(table._playerStatus[huan_bao_payler].is_bao_ting()){
						break;
					}
				}
				if(huan_bao_payler == GameConstants.INVALID_SEAT){
					huan_bao_payler = _seat_index;
				}
