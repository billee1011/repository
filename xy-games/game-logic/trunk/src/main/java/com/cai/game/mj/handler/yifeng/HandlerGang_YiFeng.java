...
public class HandlerGang_YiFeng extends MJHandlerGang<Table_YiFeng> {
    ...
    public void exe(Table_YiFeng table) {
        ...
        if(GameConstants.GANG_TYPE_ADD_GANG == _type && !table.bHaveAnGang){
    ...
    public boolean exe_gang(Table_YiFeng table) {
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if (i == _seat_index)
                    continue;
                iSore = 2 * GameConstants.CELL_SCORE;
                //对火
                if(table.has_rule(GameConstants.GAME_RULE_GUI_HUO)){
                	if(table._player_result.qiang[_seat_index] == 1){
                		iSore *= 2;
                	}
                	...
                	if(table._player_result.qiang[_seat_index] == 1 || table._player_result.qiang[i] == 1){
                		iSore *= 2;
                	}
                }
                table.GRR._gang_score[_seat_index].scores[cbGangIndex][i] -= iSore;
                table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += iSore;
            }
            table.bHaveAnGang = true;
            ...
            if(table.has_rule(GameConstants.GAME_RULE_GUI_HUO)){
            	...
            	if(table._player_result.qiang[_provide_player] == 1){
            		iSore *= 2;
            	}
            }
            else{
            	if(table._player_result.qiang[_seat_index] == 1 || table._player_result.qiang[_provide_player] == 1){
            		iSore *= 2;
            	}
            }
        	
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += iSore;
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][_provide_player] -= iSore;
            ...
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][provide_index] -= 3 * GameConstants.CELL_SCORE;*/
        	for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if (i == _seat_index)
                    continue;
                iSore =  GameConstants.CELL_SCORE;
                if(table.has_rule(GameConstants.GAME_RULE_GUI_HUO)){
                	if(table._player_result.qiang[_seat_index] == 1){
                		iSore *= 2;
                	}
