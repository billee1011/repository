 ...
 */
public class MJHandlerGang_YingJing extends MJHandlerGang<MJTable_YingJing> {
	...
	public boolean handler_operate_card(MJTable_YingJing table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_NULL) {
			...
			/*if(isFrist){
				isFrist = false;
				//添加放炮动画
				table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_HU, 1, new long[]{GameConstants_YINGJING.CHR_FANG_PAO}, 1,
		...
		case GameConstants.WIK_CHI_HU: {
			...
			if (jie_pao_count > 0) {
				...
				table.operate_player_cards(_seat_index, hand_card_count, cards, 0, null,false);
	 ...
	 ***/
	@Override
	protected boolean exe_gang(MJTable_YingJing table) {
...
//			}
		...
		table.operate_player_cards(_seat_index, hand_card_count, cards, weave_count, weaves,false);
		//廊起杆牌清理
		table.rmLangQiGang(_seat_index, _center_card);
	...
	public int getEffectByActionType(int type){
		/*if (type == GameConstants.GANG_TYPE_AN_GANG){
			return GameConstants_YINGJING.WIK_AN_GANG;
		}else if (type == GameConstants.GANG_TYPE_JIE_GANG){
			return GameConstants_YINGJING.WIK_JIE_GANG;
		}else if (type == GameConstants.GANG_TYPE_ADD_GANG){
			return GameConstants_YINGJING.WIK_ADD_GANG;
		}*/
