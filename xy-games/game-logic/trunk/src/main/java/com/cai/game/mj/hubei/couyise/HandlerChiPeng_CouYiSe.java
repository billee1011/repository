...
public class HandlerChiPeng_CouYiSe extends MJHandlerChiPeng<Table_CouYiSe> {
	...
	public void exe(Table_CouYiSe table) {
		...
		// TODO 注意action为WIK_PENG、WIK_SUO_PENG时的手牌处理
		...
		if (ting_count > 0) {
			...
			int tmp_hand_card_count = table._logic.switch_to_cards_data_couyise(table.GRR._cards_index[_seat_index], tmp_cards, table.pi_zi);
			...
			for (int i = 0; i < tmp_hand_card_count; i++) {
				...
				if (tmp_cards[i] < GameConstants.CARD_ESPECIAL_TYPE_TING) {
					...
					} else if (tmp_cards[i] == table.pi_zi) {
						tmp_cards[i] += GameConstants.CARD_ESPECIAL_TYPE_PI_ZI;
					} else if (tmp_cards[i] == Constants_CouYiSe.HZ_CARD) {
