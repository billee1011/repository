...
public class HandlerDispatchCard_HUAI_HUA extends MJHandlerDispatchCard<Table_HUAI_HUA> {
	...
	public HandlerDispatchCard_HUAI_HUA() {
	...
	public void exe(Table_HUAI_HUA table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			// 得到下家
			int next_player = (table._cur_banker + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();
		...
		// 判断是否是杠后抓牌,杠开花只算接杠
		int card_type = Constants_HUAI_HUA.HU_CARD_TYPE_ZI_MO;
		if (_type == Constants_HUAI_HUA.HU_CARD_TYPE_GANG_KAI_HUA && table.has_rule(Constants_HUAI_HUA.GAME_RULE_GANG_SHANG_HUA)) {
			card_type = Constants_HUAI_HUA.HU_CARD_TYPE_GANG_KAI_HUA;
		}
		// 检查牌型,听牌
		...
		// 显示牌
		//if (_type == Constants_HUAI_HUA.HU_CARD_TYPE_GANG_KAI_HUA) {
			...
			if (cbActionMask != GameConstants.WIK_NULL && !table.is_gang_yao[_seat_index]) {
				...
				for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
					...
					if(table.is_can_gang_yao(_seat_index, m_gangCardResult.cbCardData[i])){
						if(!curPlayerStatus.has_action_by_code(Constants_HUAI_HUA.WIK_GANG_YAO))
							curPlayerStatus.add_action(Constants_HUAI_HUA.WIK_GANG_YAO);
	...
	@Override
	public boolean handler_player_out_card(Table_HUAI_HUA table, int seat_index, int card) {
	...
	public boolean handler_operate_card(Table_HUAI_HUA table, int seat_index, int operate_code, int operate_card) {
		...
		case Constants_HUAI_HUA.WIK_GANG_YAO: {
