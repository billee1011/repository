...
private static Logger logger = Logger.getLogger(NNGameLogic.class);
	...
	public int get_card_type(int card_date[],int card_count, Map<Integer, Integer> ruleMaps){
		...
		if(this.getRuleValue(ruleMaps,GameConstants.GAME_RULE_TONG_HUA_SHUN) == 1)
		{
		...
		for(int i=0;i<card_count;i++)
		{
			...
			if(this.getRuleValue(ruleMaps,GameConstants.GAME_RULE_OX_WUXIAONIU)==1){
				if(sum <= 10)
					return  GameConstants.OX_WUXIAONIU_VALUE;
			}
			if(this.getRuleValue(ruleMaps,GameConstants.GAME_RULE_OX_BOOM)==1){
				if(four_count == 4)
		...
		if(this.getRuleValue(ruleMaps,GameConstants.GAME_RULE_HU_LU_NIU)==1)
		{
		...
		if((same_color==true)&& this.getRuleValue(ruleMaps,GameConstants.GAME_RULE_TONG_HUA_NIU)==1) return GameConstants.OX_TONG_HUA_VALUE;
		if((line_card == true)&&this.getRuleValue(ruleMaps,GameConstants.GAME_RULE_SHUN_ZI_NIU)==1) return GameConstants.OX_SHUN_ZI_VALUE;
	...
	public boolean get_ox_card(int card_date[],int card_count,Map<Integer, Integer> ruleMaps){
		...
		if((same_color==true)&&this.getRuleValue(ruleMaps, GameConstants.GAME_RULE_TONG_HUA_NIU)==1) return true;
		if((line_card == true)&&getRuleValue(ruleMaps, GameConstants.GAME_RULE_SHUN_ZI_NIU)==1) return true;
		if(this.getRuleValue(ruleMaps,GameConstants.GAME_RULE_OX_WUHUANIU)==1){
			...
			,Map<Integer, Integer> ruleMaps)
	{
		...
		//点数判断
		 if (first_type>next_type)
			...
			return -1;
		if(this.getRuleValue(ruleMaps, GameConstants.GAME_RULE_EQUAL_PING)==1 )
		{
			if(first_type==next_type)
				return 0;
			else if (first_type>next_type)
		...
		if(this.getRuleValue(ruleMaps, GameConstants.GAME_RULE_ZHUANG_WIN)==1){
		...
		if(this.getRuleValue(ruleMaps, GameConstants.GAME_RULE_HUA_SE_COMPARE)==1){
