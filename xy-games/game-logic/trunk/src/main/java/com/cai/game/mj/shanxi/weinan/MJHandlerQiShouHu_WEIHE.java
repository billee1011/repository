...
public class MJHandlerQiShouHu_WEIHE extends AbstractMJHandler<MJTable_WEIHE> {
	...
	public boolean handler_player_be_in_room(MJTable_WEIHE table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < 55; j++) {
				if (table._logic.is_magic_card(table.GRR._discard_cards[i][j])) {
					// 癞子
					int_array.addItem(table.GRR._discard_cards[i][j] + GameConstants.CARD_ESPECIAL_TYPE_BAO);
				} else {
		...
		table._logic.switch_to_cards_data_sichuan(table.GRR._cards_index[seat_index], hand_cards,table.ding_que_pai_se[seat_index]);
