...
public class HandlerOutCardOperate_HuaRong extends MJHandlerOutCardOperate<Table_HuaRong> {
	...
	public void exe(Table_HuaRong table) {
        ...
        if (table._logic.is_magic_card(real_card)) {
            ...
            table.player_out_magic_count[_out_card_player]++;
            table._player_result.haspiao[_out_card_player]++;
        ...
        int show_card =  table._logic.is_magic_card(_out_card_data)?_out_card_data+GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI:(table._logic.is_lai_gen_card(_out_card_data)?_out_card_data+GameConstants.CARD_ESPECIAL_TYPE_LAI_GEN:_out_card_data); 
		table.operate_out_card(_out_card_player, 1, new int[] {show_card}, GameConstants.OUT_CARD_TYPE_MID,
				GameConstants.INVALID_SEAT);
		//得到打出那些牌听牌
	...
	private void handler_operate_bu_tong_pao(Table_HuaRong table, int seat_index,
			int operate_code, int operate_card) {
		int target_player = seat_index;
		int target_action = operate_code;
		int target_p = 0;
		...
		case GameConstants.WIK_XIAO_CHAO_TIAN: {
			table.remove_discard_after_operate(_out_card_player, _out_card_data);
			table.exe_gang(target_player, _out_card_player, target_card, target_action, GameConstants.GANG_TYPE_LAI_ZI_PI_ZI_GANG, false, false);
			return;
		}
		case GameConstants.WIK_NULL: {
		...
		/*case GameConstants.WIK_CHI_HU: {
			...
			table.GRR._chi_hu_rights[_out_card_player].opr_or(Constants_MJ_HUA_RONG.CHR_FANG_PAO);
			...
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._cur_banker, GameConstants.Game_End_NORMAL),
					GameConstants.GAME_FINISH_DELAY, TimeUnit.MILLISECONDS);
