...
public class MJHandlerOutCardOperate_ShanXi_LANXIAN extends MJHandlerOutCardOperate<MJTable_ShanXi_LANXIAN> {
	...
	public boolean handler_operate_card(MJTable_ShanXi_LANXIAN table, int seat_index, int operate_code,
		...
		// if(operate_card==GameConstants_SXLX.DONG_FENG_CARD+GameConstants_SXLX.CARD_ESPECIAL_TYPE_GANG_NAN_FENG){
		// no_feng_operate_card=operate_card-GameConstants_SXLX.CARD_ESPECIAL_TYPE_GANG_NAN_FENG;
		// }
		...
		case GameConstants.WIK_CHI_HU: {
			...
			table.GRR._chi_hu_rights[_out_card_player].opr_or_xt(GameConstants_SXLX.CHR_TYPE_DIAN_PAO, false);
	...
	public boolean handler_player_be_in_room(MJTable_ShanXi_LANXIAN table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < 55; j++) {
				...
				// GameConstants_SXLX.CARD_ESPECIAL_TYPE_HIDE);
				// }else{
				int_array.addItem(table.GRR._discard_cards[i][j]);
				// }
			}
