 ...
 */
public class MJHandlerOutCardOperate_XN extends MJHandlerOutCardOperate<MJTable_XN> {
	...
	public void exe(MJTable_XN table) {
		...
		// 显示出牌
		table.operate_out_card(_out_card_player, 1, new int[] { table.changeCard(_out_card_data)  }, GameConstants.OUT_CARD_TYPE_MID,
				GameConstants.INVALID_SEAT);
		
		//摔金
		if(table._logic.is_magic_card(_out_card_data)){
			table.operate_effect_action(_out_card_player, GameConstants.Effect_Action_Other, 1, new long[]{GameConstants_XiangNing.EFFECT_ACTION_OTHER_SHUAI_JING}, 1000, GameConstants.INVALID_SEAT);
			//记录摔金次数
	...
	public boolean handler_operate_card(MJTable_XN table, int seat_index, int operate_code, int operate_card) {
		...
		if (table._playerStatus[seat_index].has_chi_hu() && operate_code != GameConstants.WIK_CHI_HU) {
			if(table.has_rule(GameConstants_XiangNing.GAME_RULE_MJ_XN_BAO_TING_BAO_HU)){
