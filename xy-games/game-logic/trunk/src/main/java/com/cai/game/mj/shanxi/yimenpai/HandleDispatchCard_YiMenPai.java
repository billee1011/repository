...
public class HandleDispatchCard_YiMenPai extends MJHandlerDispatchCard<Table_YiMenPai> {
	...
	public HandleDispatchCard_YiMenPai() {
	...
	public void exe(Table_YiMenPai table) {
		...
		if (table.GRR._left_card_count <= 0) {
			...
			if (table.hasRuleLunZhuang) {
				table._cur_banker = (table.GRR._banker_player + 1) % table.getTablePlayerNumber();
			}
			...
			if (cbActionMask != GameConstants.WIK_NULL) {
				...
				for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
					if (table.is_bao_ting[_seat_index]) {
						...
						boolean is_changed = table.check_gang_huan_zhang(_seat_index, m_gangCardResult.cbCardData[i]);
		...
		if (table.GRR._left_card_count <= 0 && _type == UniversalConstants.DISPATCH_CARD_TYPE_GANG && action != GameConstants.WIK_CHI_HU) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				table.GRR._chi_hu_card[i][0] = UniversalConstants.INVALID_VALUE;
			}
	...
	@Override
	public boolean handler_player_out_card(Table_YiMenPai table, int seat_index, int card) {
