...
public class HandlerGang_YangZhong extends MJHandlerGang<Table_YangZhong> {
	...
	public boolean exe_gang(Table_YangZhong table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			int card_color = table._logic.get_card_color(_center_card);
			int card_value = table._logic.get_card_value(_center_card);

			if (card_color == 3) {
				if (card_value <= 4) {
					if (card_value == table.men_hua_men_feng[_seat_index]) {
						table.player_di_fen[_seat_index] += 4;
					} else {
						table.player_di_fen[_seat_index] += 3;
					}
				} else {
					table.player_di_fen[_seat_index] += 3;
				}
			...
			if (card_color == 3) {
				...
				if (card_value <= 4) {
					if (card_value == table.men_hua_men_feng[_seat_index]) {
						table.player_di_fen[_seat_index] += 3 - 2;
					} else {
						table.player_di_fen[_seat_index] += 2 - 1;
					}
				} else {
					table.player_di_fen[_seat_index] += 2 - 1;
				}
