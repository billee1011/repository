 ...
 *
 */
public class DispatchAddCardRunnable extends BaseFuture{
	
	private static Logger logger = LoggerFactory.getLogger(DispatchAddCardRunnable.class);
	...
	private int _type;
	public DispatchAddCardRunnable(int room_id,int seat_index,int type,boolean tail){
		super(room_id);
		_room_id = room_id;
	...
	public void execute() {
		try {
			...
			try{
				roomLock.lock();
				table.dispatch_card_add_data(_seat_index,_type, _tail);
			}finally{
				roomLock.unlock();
