...
public class PDKHandlerOutCardOperate_JDDL extends PDKHandlerOutCardOperate {
	@Override
	public void exe(PDKTable table) {
		...
		if (0 == table.GRR._card_count[_out_card_player]) {
			...
			if (cbCardType == GameConstants.PDK_CT_BOMB_CARD) {
				...
				// 上报牌型
				coin_card_type_up(table, cbCardType, _out_card_player, _out_cards_data, _out_card_count);
			}
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	...
	public void coin_card_type_up(PDKTable table, int type, int seat_index, int out_card_data[], int out_card_count) {
		if (!table.isCoinRoom()) {
			return;
		}
		// 上报牌型
		Player player = table.get_players()[seat_index];

		table.triggerTypeEvent(ETriggerType.ING, player.getAccount_id(), type);
	}
	...
	@Override
	public boolean handler_god_observer_be_in_room(PDKTable table, Player player) {
	...
	public void no_out_card(PDKTable table) {
		...
		if (table._current_player == table._out_card_player) {
			...
			if (cbCardType == GameConstants.PDK_CT_BOMB_CARD) {
				...
				// 上报牌型
				coin_card_type_up(table, cbCardType, table._out_card_player, _out_cards_data, _out_card_count);
			}
