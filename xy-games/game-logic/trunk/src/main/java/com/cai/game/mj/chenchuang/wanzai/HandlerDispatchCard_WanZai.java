...
public class HandlerDispatchCard_WanZai extends MJHandlerDispatchCard<Table_WanZai> {
	...
	public HandlerDispatchCard_WanZai() {
	...
	public void exe(Table_WanZai table) {
		// 跟庄判断
		if (table.is_chase_vaild && table.banker_out_card_count > 0 && _seat_index == table._cur_banker) {
			if (table.out_same_pai_count < 3)
				table.is_chase_vaild = false;
			else
				table.out_same_pai_count = 0;
		}
		...
		// 判断是否是杠后抓牌,杠开花只算接杠
		int card_type = Constants_WanZai.HU_CARD_TYPE_ZI_MO;
		/*
		 * if (_type == 3) { card_type =
		 * Constants_WanZai.HU_CARD_TYPE_GANG_SHANG_KAI_HUA; }
		 */
		// 检查牌型，是否是七对，4鬼，听牌
	...
	@Override
	public boolean handler_player_out_card(Table_WanZai table, int seat_index, int card) {
		...
		if (_type == Constants_WanZai.HU_CARD_TYPE_TIAN_HU) {
			// 出牌
			table.exe_out_card(_seat_index, card, Constants_WanZai.HU_CARD_TYPE_TIAN_HU);
		} else {
			// 出牌
	...
	public boolean handler_operate_card(Table_WanZai table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: {
			if (_type == Constants_WanZai.HU_CARD_TYPE_TIAN_HU) {
				...
				chiHuRight.opr_or_long(Constants_WanZai.CHR_TIAN_HU);
