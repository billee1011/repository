...
import com.cai.future.runnable.ChiPengCardRunnable;
...
import com.cai.game.scphz.SCPHZGameLogic.AnalyseItem;
...
import com.cai.game.scphz.handler.PHZHandlerFinish;
import com.cai.game.scphz.handler.PHZHandlerGang;
import com.cai.game.scphz.handler.PHZHandlerOutCardOperate;
import com.cai.game.scphz.handler.msh.PHZHandlerChiPeng_MS;
import com.cai.game.scphz.handler.msh.PHZHandlerChuLiFirstCard_MS;
import com.cai.game.scphz.handler.msh.PHZHandlerDispatchCard_MS;
import com.cai.game.scphz.handler.msh.PHZHandlerDispatchFirstCard_MS;
import com.cai.game.scphz.handler.msh.PHZHandlerGang_MS;
import com.cai.game.scphz.handler.msh.PHZHandlerOutCardOperate_MS;
import com.cai.game.scphz.handler.sy.PHZHandlerChiPeng_SY;
import com.cai.game.scphz.handler.sy.PHZHandlerChuLiFirstCard_SY;
import com.cai.game.scphz.handler.sy.PHZHandlerDispatchCard_SY;
import com.cai.game.scphz.handler.sy.PHZHandlerDispatchFirstCard_SY;
import com.cai.game.scphz.handler.sy.PHZHandlerGang_SY;
import com.cai.game.scphz.handler.sy.PHZHandlerOutCardOperate_SY;
import com.cai.game.scphz.handler.sy.PHZHandlerPaiQuan_SY;
import com.cai.game.scphz.handler.yj.PHZHandlerChiPeng_YJ;
import com.cai.game.scphz.handler.yj.PHZHandlerChuLiFirstCard_YJ;
import com.cai.game.scphz.handler.yj.PHZHandlerDispatchCard_YJ;
import com.cai.game.scphz.handler.yj.PHZHandlerDispatchFirstCard_YJ;
import com.cai.game.scphz.handler.yj.PHZHandlerGang_YI;
import com.cai.game.scphz.handler.yj.PHZHandlerOutCardOperate_YJ;
import com.cai.game.scphz.handler.yj.PHZHandlerPaiQuan_YJ;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class SCPHZTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	private static Logger logger = Logger.getLogger(SCPHZTable.class);
	...
	public int _fan_shu[]; // 番数
	public int _ji_card; // 翻鸡
	public int _ji_count; // 鸡分数

	// 胡息
	// public HuCardInfo _hu_card_info;

	public SCPHZGameLogic _logic = null;
	...
	public int _change_banker_count; // 换庄次数
	/**
	 * 牌桌上的组合扑克 (落地的牌组合)
	 */
	...
	public PHZHandler _handler_quan_pai;
	...
	public SCPHZTable() {
		...
		_guo_hu_pai_cards = new int[this.getTablePlayerNumber()][GameConstants.MAX_HH_INDEX];
		_guo_hu_xi = new int[this.getTablePlayerNumber()][GameConstants.MAX_HH_INDEX];
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_player_card_type = 0x11;
		_change_banker_count = 0;
		// 不能吃，碰
		_cannot_chi = new int[this.getTablePlayerNumber()][30];
		_cannot_chi_count = new int[this.getTablePlayerNumber()];
		_cannot_peng = new int[this.getTablePlayerNumber()][30];
		...
		if (is_mj_type(GameConstants.GAME_TYPE_MS_EQS)) {
			_handler_dispath_card = new PHZHandlerDispatchCard_MS();
			_handler_out_card_operate = new PHZHandlerOutCardOperate_MS();
			_handler_gang = new PHZHandlerGang_MS();
			_handler_chi_peng = new PHZHandlerChiPeng_MS();

			_handler_chuli_firstcards = new PHZHandlerChuLiFirstCard_MS();
			_handler_dispath_firstcards = new PHZHandlerDispatchFirstCard_MS();

		}
		if (is_mj_type(GameConstants.GAME_TYPE_YJ_EQS)) {
			_handler_dispath_card = new PHZHandlerDispatchCard_YJ();
			_handler_out_card_operate = new PHZHandlerOutCardOperate_YJ();
			_handler_gang = new PHZHandlerGang_YI();
			_handler_chi_peng = new PHZHandlerChiPeng_YJ();

			_handler_chuli_firstcards = new PHZHandlerChuLiFirstCard_YJ();
			_handler_dispath_firstcards = new PHZHandlerDispatchFirstCard_YJ();
			_handler_quan_pai = new PHZHandlerPaiQuan_YJ();
		}
		if (is_mj_type(GameConstants.GAME_TYPE_PHZ_SY_DE)) {
			_handler_dispath_card = new PHZHandlerDispatchCard_SY();
			_handler_out_card_operate = new PHZHandlerOutCardOperate_SY();
			_handler_gang = new PHZHandlerGang_SY();
			_handler_chi_peng = new PHZHandlerChiPeng_SY();

			_handler_chuli_firstcards = new PHZHandlerChuLiFirstCard_SY();
			_handler_dispath_firstcards = new PHZHandlerDispatchFirstCard_SY();
			_handler_quan_pai = new PHZHandlerPaiQuan_SY();
		}
...
//		if (DEBUG_CARDS_MODE || BACK_DEBUG_CARDS_MODE)
		...
		if (is_mj_type(GameConstants.GAME_TYPE_MS_EQS))
			game_start_HH();
		else if (is_mj_type(GameConstants.GAME_TYPE_YJ_EQS)) {
			exe_quan_pai();
			GameSchedule.put(new AnimationRunnable(getRoom_id(), ID_TIMER_ANIMATION_START), 600, TimeUnit.MILLISECONDS);
		} else if (is_mj_type(GameConstants.GAME_TYPE_PHZ_SY_DE)) {
			game_start_HH();
		}
		return true;
...
//				 22, 10, 25, 23, 22, 1, 18, 26, 17, 26, 3, 4, 17, 19, 25, 5, 26, 19, 2, 18, 10, 25, 4, 26, 8, 9, 9, 18, 6, 1, 22, 23, 5, 4, 7, 21, 19, 1, 10, 9, 20, 7, 3, 2, 24, 17, 9, 7, 22, 7, 21, 21, 25, 3, 23, 8, 6, 5, 3, 24, 10, 18, 2, 6, 20, 24, 24, 21, 1, 20, 19, 4, 5, 2, 23, 8, 17, 8, 20, 6
//		
//		
//		 };
//		 this._cur_banker = 0;
...
//		 DEBUG_CARDS_MODE = true;
		 //		 this.testSameCard(new int[]{
		 //		 0x02,0x02,0x02,0x07,0x07,0x07,0x07,0x0a,0x0a,0x0a,0x0a,0x11,0x12,0x13,0x14,0x15,0x16,0x03,0x03,0x03
		 //		 });
		 //		 DEBUG_CARDS_MODE = true;
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_PHZ_SY_DE)) {
			ret = this.handler_game_finish_sy(seat_index, reason);
		}
		if (is_sys()) {
	...
	public boolean handler_game_finish_hh(int seat_index, int reason) {
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL ||
			...
			if (seat_index != -1)
				game_end.setCellScore(GRR._weave_count[seat_index]); // 显示的牌分两个部分
			for (int i = 0; i < count; i++) {
		...
		if (seat_index != this._cur_banker) {
			this._change_banker_count++;
			if (this._change_banker_count % this.getTablePlayerNumber() == 0)
				_player_card_type = (_player_card_type & 0x10) + _player_card_type % 16 % 4 + 1;
	...
	public boolean handler_game_finish_sy(int seat_index, int reason) {
		int real_reason = reason;
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL ||
			...
			// 番鸡
			game_end.addGangCount(this._ji_card);
			game_end.addGangScore(this._ji_count);
			GRR._end_type = reason;
	...
	public boolean handler_game_finish_yj(int seat_index, int reason) {
		int real_reason = reason;
			...
			for (int i = 0; i < count; i++) {
				...
				if (reason != GameConstants.Game_End_NORMAL) {
					...
					if (i == seat_index) {
						...
						for (int j = GRR._weave_count[i]; j < _hu_weave_count[i]; j++) {
							WeaveItemResponse.Builder weaveItem_item = WeaveItemResponse.newBuilder();
							weaveItem_item.setProvidePlayer(_hu_weave_items[i][j].provide_player);
							weaveItem_item.setPublicCard(_hu_weave_items[i][j].public_card);
							weaveItem_item.setWeaveKind(_hu_weave_items[i][j].weave_kind);
	...
	public int analyse_chi_hu_card(int cards_index[], WeaveItem weaveItems[], int weave_count, int seat_index, int provider_index, int cur_card,
		...
		if (GameConstants.GAME_TYPE_MS_EQS == _game_type_index) {
			return analyse_chi_hu_card_ms(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type, hu_xi);
		}
		if (GameConstants.GAME_TYPE_YJ_EQS == _game_type_index) {
			return analyse_chi_hu_card_yj(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type, hu_xi,
					dispatch);
		}
		if (GameConstants.GAME_TYPE_PHZ_SY_DE == _game_type_index) {
			return analyse_chi_hu_card_sy(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type, hu_xi,
					dispatch);
		}
		return 0;
	}
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_sy(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index, int cur_card,
		...
		boolean bValue = _logic.analyse_card_sy(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index, cur_card, analyseItemArray,
				false, hu_xi, _is_tian_hu);

		if (cur_card != 0) {
			for (int i = 0; i < weaveCount; i++) {
				if ((cur_card == weaveItems[i].center_card) && ((weaveItems[i].weave_kind == GameConstants.WIK_WEI))) {

					int index = _logic.switch_to_card_index(cur_card);
					cbCardIndexTemp[index]--;
					int temp_index = analyseItemArray.size();

					boolean temp_bValue = _logic.analyse_card_sy(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index, cur_card,
							analyseItemArray, false, hu_xi, _is_tian_hu);
					bValue = temp_bValue;
					if (temp_index < analyseItemArray.size()) {
						...
						for (; temp_index < analyseItemArray.size(); temp_index++) {
							...
							hu_xi[0] = 0;
							for (int j = 0; j < 7; j++) {
								if ((cur_card == analyseItem.cbCenterCard[j]) && (analyseItem.cbWeaveKind[j] == GameConstants.WIK_WEI)) {
		...
		if (bValue == true) {
			...
			if (analyseItem.curCardEye == true) {
				...
				if (analyseItem.cbCardEye == cur_card) {
					this.GRR._count_pick_niao = GameConstants.WIK_DUI_ZI;
				}
			}

			cbChiHuKind = GameConstants.WIK_CHI_HU;
			if (hong_pai_count >= 10)
				chiHuRight.opr_or_long(GameConstants.CHR_SY_TEN_HONG);

			if (max_hu_xi == 0)
				chiHuRight.opr_or_long(GameConstants.CHR_SY_0_HU);
			else if (max_hu_xi % 10 == 0) {
				chiHuRight.opr_or_long(GameConstants.CHR_SY_KA_HU);
			}
			if (max_hu_xi == 22)
				chiHuRight.opr_or_long(GameConstants.CHR_SY_TAI_HU);
			if (GRR._left_card_count == 0 && dispatch == true)
				chiHuRight.opr_or_long(GameConstants.CHR_SY_HAI_DI);
			if (_is_tian_hu == true && dispatch == true && this._cur_banker == seat_index)
				chiHuRight.opr_or_long(GameConstants.CHR_SY_TIAN_HU);
			if (dispatch == true && provider_index == seat_index)
				chiHuRight.opr_or_long(GameConstants.CHR_SY_ZI_MO);
			if (_is_di_hu == true && dispatch == false)
				chiHuRight.opr_or_long(GameConstants.CHR_SY_DI_HU);
			if (hei_pai_count == all_cards_count)
				chiHuRight.opr_or_long(GameConstants.CHR_SY_WU_HU);
			int zhao_count = _logic.is_weave_hu(this._hu_weave_items[seat_index], this._hu_weave_count[seat_index], GameConstants.WIK_PAO);
			int long_count = _logic.is_weave_hu(this._hu_weave_items[seat_index], this._hu_weave_count[seat_index], GameConstants.WIK_AN_LONG);

			if (zhao_count > 0) {
				chiHuRight.opr_or_long(GameConstants.CHR_SY_ZHAO_HU);
			}
			if (long_count > 0) {
				chiHuRight.opr_or_long(GameConstants.CHR_SY_LONG_HU);
			}
			int is_er_count = 0;
			int is_qi_count = 0;
			int is_shi_count = 0;
			for (int i = 0; i < _hu_weave_count[seat_index]; i++) {
				if (_hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_PENG
						|| _hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_KAN) {
					if (_hu_weave_items[seat_index][i].center_card == 0x12) {
						is_er_count += 3;
						chiHuRight.opr_or_long(GameConstants.CHR_SY_DA_ER_HU);

					}
					if (_hu_weave_items[seat_index][i].center_card == 0x17) {
						is_qi_count += 3;
						chiHuRight.opr_or_long(GameConstants.CHR_SY_DA_QI_HU);

					}
					if (_hu_weave_items[seat_index][i].center_card == 0x1a) {
						is_shi_count += 3;
						chiHuRight.opr_or_long(GameConstants.CHR_SY_DA_TEN_HU);

					}
				}
				if (_hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_AN_LONG
						|| _hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_PAO) {
					if (_hu_weave_items[seat_index][i].center_card == 0x12) {
						chiHuRight.opr_or_long(GameConstants.CHR_SY_DA_ER_HU);
						chiHuRight.opr_or_long(GameConstants.CHR_SY_TUO_DA_ER);

					}
					if (_hu_weave_items[seat_index][i].center_card == 0x17) {
						chiHuRight.opr_or_long(GameConstants.CHR_SY_DA_QI_HU);
						chiHuRight.opr_or_long(GameConstants.CHR_SY_TUO_DA_QI);

					}
					if (_hu_weave_items[seat_index][i].center_card == 0x1a) {
						chiHuRight.opr_or_long(GameConstants.CHR_SY_DA_TEN_HU);
						chiHuRight.opr_or_long(GameConstants.CHR_SY_TUO_DA_SHI);

					}
				}
				if (_hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_EQS) {
					if (_hu_weave_items[seat_index][i].center_card > 0x10) {
						is_er_count += 1;
						is_qi_count += 1;
						is_shi_count += 1;
					}
				}
				if (_hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_LEFT) {
					if (_hu_weave_items[seat_index][i].center_card < 0x10)
						continue;
					for (int j = 0; j < this.getTablePlayerNumber(); j++) {
						if (_hu_weave_items[seat_index][i].center_card + j == 0x12)
							is_er_count += 1;
						if (_hu_weave_items[seat_index][i].center_card + j == 0x17)
							is_qi_count += 1;
						if (_hu_weave_items[seat_index][i].center_card + j == 0x1a)
							is_shi_count += 1;
					}
				}
				if (_hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_CENTER) {
					if (_hu_weave_items[seat_index][i].center_card < 0x10)
						continue;
					for (int j = -1; j < 2; j++) {
						if (_hu_weave_items[seat_index][i].center_card + j == 0x12)
				...
				if (_hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_RIGHT) {
					if (_hu_weave_items[seat_index][i].center_card < 0x10)
						continue;
					for (int j = -2; j < 1; j++) {
						if (_hu_weave_items[seat_index][i].center_card + j == 0x12)
				...
				if (_hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_DDX) {
					if (_logic.get_card_value(_hu_weave_items[seat_index][i].center_card) == 2)
						is_er_count += 2;
					if (_logic.get_card_value(_hu_weave_items[seat_index][i].center_card) == 7)
						is_qi_count += 2;
					if (_logic.get_card_value(_hu_weave_items[seat_index][i].center_card) == 10)
						is_shi_count += 2;
				}
				if (_hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_XXD) {
					if (_logic.get_card_value(_hu_weave_items[seat_index][i].center_card) == 2)
						is_er_count += 1;
					if (_logic.get_card_value(_hu_weave_items[seat_index][i].center_card) == 7)
						is_qi_count += 1;
				...
				if (_hu_weave_items[seat_index][i].weave_kind == GameConstants.WIK_DUI_ZI) {
					if (_hu_weave_items[seat_index][i].center_card == 0x12)
						is_er_count += 2;
					if (_hu_weave_items[seat_index][i].center_card == 0x17)
						is_qi_count += 2;
	...
	public void setChiHuRight(int seat_index, ChiHuRight chiHuRight) {
		int zhao_count = _logic.is_weave_hu(this._hu_weave_items[seat_index], this._hu_weave_count[seat_index], GameConstants.WIK_PAO);
		int long_count = _logic.is_weave_hu(this._hu_weave_items[seat_index], this._hu_weave_count[seat_index], GameConstants.WIK_AN_LONG);
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_yj(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index, int cur_card,
			ChiHuRight chiHuRight, int card_type, int hu_xi_hh[], boolean dispatch) {
		...
		List<AnalyseItem> analyseItemArray = new ArrayList<AnalyseItem>();
		int kai_count = _logic.get_kai_count(weaveItems, weaveCount);
		int cur_index = _logic.switch_to_card_index(cur_card);
		...
		int hu_xi[] = new int[1];
		hu_xi[0] = 0;
		boolean bValue = _logic.analyse_card_yj(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index, cur_card, analyseItemArray,
				false, hu_xi);
		...
		if (cur_card != 0) {
			for (int i = 0; i < weaveCount; i++) {
				if ((cur_card == weaveItems[i].center_card) && ((weaveItems[i].weave_kind == GameConstants.WIK_WEI))) {
					...
					boolean temp_bValue = _logic.analyse_card_yj(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index, cur_card,
		...
		if (bValue == true) {
			...
			boolean hang_hang_xi_count = true;
			for (int j = 0; j < _hu_weave_count[seat_index]; j++) {
				if (_hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_PENG
						&& _hu_weave_items[seat_index][j].center_card == this._player_card_type)
					is_quan_feng = true;
				if ((_hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_KAN
						|| _hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_PAO
						|| _hu_weave_items[seat_index][j].weave_kind == GameConstants.WIK_AN_LONG)
						&& _hu_weave_items[seat_index][j].center_card == this._player_card_type)
					is_zi_an = true;
				if (_hu_weave_items[seat_index][j].hu_xi == 0 && _hu_weave_items[seat_index][j].weave_kind != GameConstants.WIK_DUI_ZI) {
					hang_hang_xi_count = false;
				}
				_da_pai_count[seat_index] += _logic.get_da_card(_hu_weave_items[seat_index][j].weave_kind,
			...
			if (is_quan_feng == true) {
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_QUANFENG);
			}
			if (is_zi_an == true) {
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_ZIAN);
			}
			if (hong_pai_count >= 10 && hong_pai_count < 13)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_XHHU);
			if (hong_pai_count >= 13 && hong_pai_count < 15)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_ZHHU);
			if (hong_pai_count >= 15)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_DHHHU);
			if (GRR._left_card_count == 0 && dispatch == true)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_HDHU);

			if (hang_hang_xi_count == true)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_XPHU);

			if (_is_tian_hu == true)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_THU);
			if (_is_di_hu == true && dispatch == false)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_DIHU);
			if (hong_pai_count >= 13 && hong_pai_count < 14)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_ZHHU);
			if (hei_pai_count == all_cards_count)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_DHEIHU);
			if (_xiao_pai_count[seat_index] >= 18)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_XIAOHU);
			if (_logic.is_man_chuan_hua(_hu_weave_items[seat_index], _hu_weave_count[seat_index]))
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_MCH);
			// if(max_hu_xi>= 24 && hang_hang_xi_count == true)
			// chiHuRight.opr_or_long(GameConstants.CHR_YJ_DPHU);
			if (max_hu_xi >= 24 && max_hu_xi < 27)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_ESS_HU);
			else if (max_hu_xi >= 27 && max_hu_xi < 30)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_EQSHU);
			else if (max_hu_xi >= 30)
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_SSHU);
		}
		if (kai_count == 4) {
			cbChiHuKind = GameConstants.WIK_CHI_HU;
			int fan_count = _logic.get_chi_hu_action_rank_yj(chiHuRight);
			if (fan_count < 3) {
				chiHuRight.set_empty();
				chiHuRight.opr_or_long(GameConstants.CHR_YJ_SKHU);
				_hu_weave_count[seat_index] = 0;
			}
		}
		if (_logic.get_chi_hu_action_rank_yj(chiHuRight) == 0)
			chiHuRight.opr_or_long(GameConstants.CHR_YJ_LANHU);
		return cbChiHuKind;
	}
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_ms(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index, int cur_card,
		...
		if (hang_hang_xi_count == 7) {
			chiHuRight.opr_or_long(GameConstants.CHR_MSEQS_KUN_HU);
		}
		if (max_hu_xi == 10)
			chiHuRight.opr_or_long(GameConstants.CHR_MSEQS_DI_HU);
		if (max_hu_xi > 10 && max_hu_xi <= 20)
			chiHuRight.opr_or_long(GameConstants.CHR_MSEQS_CHI_PIAO_HU);
		if (max_hu_xi > 20 && max_hu_xi <= 30)
			chiHuRight.opr_or_long(GameConstants.CHR_MSEQS_DA_HU);
		if (max_hu_xi > 30)
			chiHuRight.opr_or_long(GameConstants.CHR_MSEQS_KUN_DA_HU);
	...
	// 玩家出版的动作检测 跑
	public int estimate_player_respond_yj(int seat_index, int provider_index, int card_data, int pao_type[], boolean dispatch) {
		// 变量定义
		int bAroseAction = GameConstants.WIK_NULL;// 出现(是否)有
		pao_type[0] = 0;
	...
	// 玩家出牌的动作检测
	public boolean estimate_player_out_card_respond_yj(int seat_index, int card, boolean bDisdatch) {
		// 变量定义
		boolean bAroseAction = false;// 出现(是否)有
		...
		do {
			...
			if (this._playerStatus[chi_seat_index].has_action_by_code(GameConstants.WIK_PENG))
				continue;
			int chi_index = 0;
	...
	public boolean estimate_player_out_card_respond_PHZ(int seat_index, int card, boolean bDisdatch) {
		...
		do {
			...
			if (_playerStatus[chi_seat_index].lock_huan_zhang() == false) {
				...
				int type_count[] = new int[10];
				int type_eat_count[] = new int[1];
				boolean yws_type = has_rule(GameConstants.GAME_RULE_YWS_CHD);
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status) && this.get_players()[seat_index] != null) {
			...
			if (this._player_card_type != 0x10) {
	...
	public void countChiHuTimes(int _seat_index, boolean isZimo) {
		...
		if (this.is_mj_type(GameConstants.GAME_TYPE_PHZ_SY_DE)) {
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_TEN_HONG)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_KA_HU)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_TAI_HU)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and(GameConstants.CHR_SY_ZHAO_HU)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}

			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_LONG_HU)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_DA_ER_HU)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_DA_QI_HU)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_DA_TEN_HU)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_TUO_DA_ER)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_TUO_DA_QI)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_TUO_DA_SHI)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_WU_HU)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_ZI_MO)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_TIAN_HU)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_DI_HU)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
			if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_HAI_DI)).is_empty()) {
				_player_result.ming_tang_count[_seat_index]++;
			}
		}
	 ...
	 * @param zimo
	 */
	public void process_chi_hu_player_score_yj(int seat_index, int provide_index, int operate_card, boolean zimo) {
		GRR._chi_hu_card[seat_index][0] = operate_card;
		_provider_hu = provide_index;
		GRR._win_order[seat_index] = 1;
		// 引用权位
		...
		int calculate_score = cell_score;

		int wFanShu = _logic.get_chi_hu_action_rank_yj(chr);// 番数
		if (this.getRuleValue(GameConstants.GAME_RULE_PHZ_YJ_3_FD) == 1 && wFanShu > 3)
			wFanShu = 3;
		if (this.getRuleValue(GameConstants.GAME_RULE_PHZ_YJ_4_FD) == 1 && wFanShu > 4)
			wFanShu = 4;
		...
		wFanShu = (int) Math.pow(2.0, wFanShu);
		float lChiHuScore = wFanShu * calculate_score;
		////////////////////////////////////////////////////// 自摸 算分
		if (zimo) {
			for (int i = 0; i < getTablePlayerNumber(); i++) {
	 ...
	 * @param zimo
	 */
	public void process_chi_hu_player_score_sy(int seat_index, int provide_index, int operate_card, boolean zimo) {
		GRR._chi_hu_card[seat_index][0] = operate_card;
		_provider_hu = provide_index;
		GRR._win_order[seat_index] = 1;
		// 引用权位
		...
		if (zhao_count > 0) {
			chr.opr_or_long(GameConstants.CHR_SY_ZHAO_HU);
			calculate_score += zhao_count;
		}
		if (long_count > 0) {
			chr.opr_or_long(GameConstants.CHR_SY_LONG_HU);
			calculate_score += long_count * 2;
		}
		int wFan = _logic.get_chi_hu_action_rank_sy(chr);
		calculate_score += wFan;
		if (!(chr.opr_and_long(GameConstants.CHR_SY_KA_HU)).is_empty()) {
			calculate_score += this._hu_xi[seat_index] / 10;
		}
		if (!(chr.opr_and_long(GameConstants.CHR_SY_HAI_DI)).is_empty()) {
			if (this.getRuleValue(GameConstants.GAME_RULE_PHZ_SY_10_HD) == 1)
				calculate_score += 10;
			else
				calculate_score += 5;

		}
		calculate_score += this._ji_count;
		// 统计
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < l; j++) {
				...
				if (GRR._chi_hu_rights[i].is_valid()) {
					if (is_mj_type(GameConstants.GAME_TYPE_HH_CHD) || is_mj_type(GameConstants.GAME_TYPE_HH_CHD_DT)) {
					...
					if (type == GameConstants.CHR_THIRTEEN_HONG_PAI_CHD) {
						des += ",红乌×4";
					}
	 ...
	 * 结算描述
	 */
	private void set_result_describe_yj() {
		// 杠牌，每个人的分数
		int lGangScore[] = new int[getTablePlayerNumber()];
		for (int i = 0; i < getTablePlayerNumber(); i++) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < l; j++) {
				type = GRR._chi_hu_rights[i].type_list[j];
				if (GRR._chi_hu_rights[i].is_valid()) {
					if (type == GameConstants.CHR_YJ_LANHU) {
						des += ",烂胡";
					}
					if (type == GameConstants.CHR_YJ_QUANFENG) {
						des += ",圈峰 + 1番";
					}

					if (type == GameConstants.CHR_YJ_ESS_HU) {
						des += ",二十四胡 + 1番";
					}
					if (type == GameConstants.CHR_YJ_XHHU) {
						des += ",小红胡 + 1番";
					}
					if (type == GameConstants.CHR_YJ_HDHU) {
						des += ",海底胡 + 1番";
					}

					if (type == GameConstants.CHR_YJ_ZIAN) {
						des += ",自案 + 2番";
					}
					if (type == GameConstants.CHR_YJ_MCH) {
						des += ",满串花+ 2番";
					}
					if (type == GameConstants.CHR_YJ_XPHU) {
						des += ",铺胡+ 2番";
					}
					if (type == GameConstants.CHR_YJ_EQSHU) {
						des += ",二十七胡+ 2番";
					}
					if (type == GameConstants.CHR_YJ_THU) {
						des += ",天胡+ 2番";
					}
					if (type == GameConstants.CHR_YJ_DIHU) {
						des += ",天点地+ 2番";
					}
					if (type == GameConstants.CHR_YJ_ZHHU) {
						des += ",中红胡+ 2番";
					}

					if (type == GameConstants.CHR_YJ_XHEIHU) {
						des += ",小黑胡+ 3番";
					}
					if (type == GameConstants.CHR_YJ_DHEIHU) {
						des += ",大黑胡+ 3番";
					}
					if (type == GameConstants.CHR_YJ_XIAOHU) {
						des += ",十八小+ 3番";
					}
					if (type == GameConstants.CHR_YJ_DPHU) {
						des += ",大铺胡+ 3番";
					}
					if (type == GameConstants.CHR_YJ_HHU) {
						des += ",红胡+ 3番";
					}
					if (type == GameConstants.CHR_YJ_DHHHU) {
						des += ",大红胡+ 3番";
					}
					if (type == GameConstants.CHR_YJ_SSHU) {
						des += ",三十胡+ 3番";
					}
					if (type == GameConstants.CHR_YJ_SHEHU) {
						des += ",三合胡+ 3番";
					}
					if (type == GameConstants.CHR_YJ_SKHU) {
						des += ",四开胡+ 3番";
					}
					if (type == GameConstants.CHR_YJ_WANHU) {
						des += ",五案胡+ 3番";
					}
				}
			}
	 ...
	 * 结算描述
	 */
	private void set_result_describe_sy() {
		// 杠牌，每个人的分数
		int lGangScore[] = new int[getTablePlayerNumber()];
		for (int i = 0; i < getTablePlayerNumber(); i++) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < l; j++) {
				type = GRR._chi_hu_rights[i].type_list[j];
				if (GRR._chi_hu_rights[i].is_valid()) {
					if (type == GameConstants.CHR_SY_0_HU) {
						des += ",零胡+ 2分";
					}
					if (type == GameConstants.CHR_SY_TEN_HONG) {
						des += ",红牌+ 1分";
					}

					if (type == GameConstants.CHR_SY_KA_HU) {
						des += ",卡胡 + " + (this._hu_xi[i] / 10) + "分";
					}
					if (type == GameConstants.CHR_SY_TAI_HU) {
						des += ",台胡 + 1分";
					}
					if (type == GameConstants.CHR_SY_HAI_DI) {
						if (this.getRuleValue(GameConstants.GAME_RULE_PHZ_SY_10_HD) == 1)
							des += ",海底胡 + 10分";
						else
							des += ",海底胡 + 5分";
					...
					if (type == GameConstants.CHR_SY_ZHAO_HU) {
						des += ",招胡 + " + _logic.is_weave_hu(this._hu_weave_items[i], this._hu_weave_count[i], GameConstants.WIK_PAO) + "分";

					}
					if (type == GameConstants.CHR_SY_LONG_HU) {
						des += ",垅胡 + " + _logic.is_weave_hu(this._hu_weave_items[i], this._hu_weave_count[i], GameConstants.WIK_AN_LONG) * 2 + "分";

					}
					if (type == GameConstants.CHR_SY_DA_ER_HU) {
						des += ",大贰胡+ 1分";
					}
					if (type == GameConstants.CHR_SY_DA_QI_HU) {
						des += ",大柒胡+ 1分";
					}
					if (type == GameConstants.CHR_SY_DA_TEN_HU) {
						des += ",大拾胡+ 1分";
					}
					if (type == GameConstants.CHR_SY_TUO_DA_ER) {
						des += ",托大贰+ 1分";
					}
					if (type == GameConstants.CHR_SY_TUO_DA_QI) {
						des += ",托大柒+ 1分";
					}

					if (type == GameConstants.CHR_SY_TUO_DA_SHI) {
						des += ",托大拾+ 1分";
					}
					if (type == GameConstants.CHR_SY_WU_HU) {
						des += ",乌胡+ 4分";
					}
					if (type == GameConstants.CHR_SY_ZI_MO) {
						des += ",自摸+ 1分";
					}
					if (type == GameConstants.CHR_SY_TIAN_HU) {
						des += ",天胡+ 6分";
					}
					if (type == GameConstants.CHR_SY_DI_HU) {
						des += ",地胡+ 5分";
					}
				}
			}
	...
	public void set_result_describe(int seat_index) {
		if (this.is_mj_type(GameConstants.GAME_TYPE_MS_EQS)) {
			set_result_describe_hh();
		}
		if (this.is_mj_type(GameConstants.GAME_TYPE_YJ_EQS)) {
			set_result_describe_yj();
		}
		if (this.is_mj_type(GameConstants.GAME_TYPE_PHZ_SY_DE)) {
			set_result_describe_sy();
		}
	...
	public void set_niao_card(int seat_index, int card, boolean show) {
		...
		this._ji_card = this._repertory_card[this._all_card_len - this.GRR._left_card_count];
		--this.GRR._left_card_count;
		int ben_xing_count = 0;
		if (this.getRuleValue(GameConstants.GAME_RULE_PHZ_SY_DAN_JI) == 1) {
			int ji_card = 0;
			if (_logic.get_card_value(this._ji_card) == 10) {
				if (_logic.get_card_color(this._ji_card) == 0)
					ji_card = 0x01;
				else
					ji_card = 0x11;

			} else
				ji_card = this._ji_card + 1;

			ben_xing_count = _logic.get_xing_pai_count(this._hu_weave_items[seat_index], this._hu_weave_count[seat_index], ji_card);
		} else if (this.getRuleValue(GameConstants.GAME_RULE_PHZ_SY_MAN_JI) == 1) {
			int ji_card = 0;
			if (_logic.get_card_value(this._ji_card) == 10) {
				if (_logic.get_card_color(this._ji_card) == 0)
					ji_card = 0x01;
			...
			ben_xing_count += _logic.get_xing_pai_count(this._hu_weave_items[seat_index], this._hu_weave_count[seat_index], (ji_card + 16) % 32);
			ben_xing_count += _logic.get_xing_pai_count(this._hu_weave_items[seat_index], this._hu_weave_count[seat_index], (ji_card) % 32);
		} else if (this.getRuleValue(GameConstants.GAME_RULE_PHZ_SY_SZ_JI) == 1) {
			ben_xing_count = _logic.get_card_value(this._ji_card);
		}
		this.operate_player_get_card(GameConstants.INVALID_SEAT, 1, new int[] { this._ji_card }, GameConstants.INVALID_SEAT, false);
		this._ji_count = ben_xing_count;
		return;
	}
	...
	public  boolean runnable_chi_peng_card_data(int seat_index, int provide_player, int center_card, int action, int depatch){
		_last_player = provide_player;
		this._handler = this._handler_chi_peng;
		this._handler_chi_peng.reset_status(seat_index, provide_player, action, center_card, depatch, -1);
		this._handler.exe(this);
	...
	public boolean exe_chi_peng(int seat_index, int provider, int action, int card, int type, int lou_operate) {
		...
		if (delay > 0) {
			GameSchedule.put(new ChiPengCardRunnable(this.getRoom_id(), seat_index, provider, card, action, type), delay,
					TimeUnit.MILLISECONDS);
		} else {
			// 是否有抢杠胡
