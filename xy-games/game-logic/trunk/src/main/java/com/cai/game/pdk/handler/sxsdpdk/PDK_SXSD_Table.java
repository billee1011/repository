...
import com.cai.game.pdk.PDKGameLogic_SXSD;
...
import protobuf.clazz.pdk_all.PdkRsp.Boom_Count_pdk;
...
import protobuf.clazz.pdk_all.PdkRsp.card_arry;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PDK_SXSD_Table extends PDKTable {

	public static final int CARD_HONG_TIAO_4 = 0x24;
	public boolean m_yin_bao_stata = false; // 引爆状态
	public PDKGameLogic_SXSD _logic = null;
	public PDKHandlerYinBaoOperate_SXSD _handle_yibao;

	public PDK_SXSD_Table() {
		_logic = new PDKGameLogic_SXSD();
	}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_out_card_operate = new PDKHandlerOutCardOperate_SXSD();
		_bomb_cell_score = 0;
		if (this.matchId != 0) {
			_bomb_cell_score = 4;
		}
		_first_out_card = 0x35;
		_handler_finish = new PDKHandlerFinish();
		_handle_yibao = new PDKHandlerYinBaoOperate_SXSD();
	...
	protected boolean on_handler_game_start() {
		...
		// 发牌
		if (this.has_rule(GameConstants.GAME_RULE_NOT_TKAE_SXSD)) {
			_repertory_card = new int[GameConstants.CARD_COUNT_PDK_NOT_3];
			shuffle(_repertory_card, GameConstants.CARD_DATA_SXSD_NOT_3);
		} else if (this.has_rule(GameConstants.GAME_RULE_MAI_FOUR_SXSD)) {
			_repertory_card = new int[GameConstants.CARD_COUNT_PDK_MAI_4];
			shuffle(_repertory_card, GameConstants.CARD_DATA_SXSD_MAI_4);
		} else if (this.has_rule(GameConstants.GAME_RULE_FOUR_SXSD)) {
			_repertory_card = new int[GameConstants.CARD_COUNT_PDK_MAI_4];
			shuffle(_repertory_card, GameConstants.CARD_DATA_SXSD_MAI_4);
		}
		...
		this._logic.get_card_type_award(this);
	...
	private boolean game_start_pkd() {
		...
		if (GRR._banker_player == -1) {
			this.log_error("game start error");
			return false;
		}

		int cur_banker = GRR._banker_player;
		if (need_to_yinbao(GRR._banker_player)) {
			cur_banker = -1;
			_auto_yibao_scheduled = GameSchedule.put(new PDKAutoYinBaoRunnable(getRoom_id(), GRR._banker_player, this,
					GameConstants.PDK_CT_BOMB_CARD, new int[] { 0x04, 0x14, 0x24, 0x34 }, 4), 2000,
	...
	public int find_banker() {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			for (int j = 0; j < GRR._card_count[i]; j++) {
				if (GRR._cards_data[i][j] == CARD_HONG_TIAO_4) {
					return i;
				}
			}
	...
	private void shuffle(int repertory_card[], int mj_cards[]) {
		...
		for (int i = 0; i < count; i++) {
			...
			_logic.sort_card_date_list_SXSD(GRR._cards_data[i], GRR._card_count[i]);
		}
		if (count == 2) {
			for (int j = 0; j < this.get_hand_card_count_max(); j++) {
	...
	private void test_cards() {
		// int cards[] = new int[] { 0x35, 0x16, 0x18, 0x37, 0x38, 0x18, 0x17,
		// 0x37, 0x36, 0x36 };
		int cards[] = new int[] { 0x04, 0x14, 0x24, 0x05, 0x15, 0x25, 0x01, 0x21, 0x31, 24, 36, 38, 33, 42, 52, 5, 43,
				28, 8, 58, 45, 12, 27, 6, 17, 41, 10, 57, 44, 26, 35, 22, 60, 13, 23, 4, 34, 50, 37, 1, 11, 40, 7, 18,
				21, 53, 9, 56, 51, 39, 20 };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public void cal_score_pdk_sxsd(int end_score[], int awrd_score[]) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this.GRR._card_count[i] > 1) {
				if (this._out_card_times[i] == 0) {
					end_score[i] -= 20;
					end_score[win_player] += 20;
				} else {
					end_score[i] -= this.GRR._card_count[i];
					end_score[win_player] += this.GRR._card_count[i];
				}
			}
		}
	...
	public boolean handler_game_finish_pdk_jd(int seat_index, int reason) {
		...
		int awrd_score[] = new int[this.getTablePlayerNumber()];
		Arrays.fill(end_score, 0);
		Arrays.fill(awrd_score, 0);
		// 计算分数
		if (reason == GameConstants.Game_End_NORMAL) {
			cal_score_pdk_sxsd(end_score, awrd_score);
		}
		...
		if (reason == GameConstants.Game_End_NORMAL) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				int extraScore = 0;
				if (has_rule(GameConstants.GAME_RULE_MING_TANG_10)) {
					extraScore = 10;
				} else if (has_rule(GameConstants.GAME_RULE_MING_TANG_20)) {
					extraScore = 20;
				} else {
					extraScore = 10;
				}

				if (_out_beichuntian[i] != GameConstants.INVALID_SEAT) {
					// game_end_pdk.addChunTianPlayer(i);
				}
				if (_out_beibaichun[i] != GameConstants.INVALID_SEAT) {
					// game_end_pdk.addBaiChunPlayer(i);
				}
				if (!(GRR._chi_hu_rights[i].opr_and(GameConstants.CHR_QUAN_DA_PDK)).is_empty()) {
					game_end_pdk.addQuanDaPlayer(extraScore * (getTablePlayerNumber() - 1));
				} else {
					game_end_pdk.addQuanDaPlayer(0);
				}
				if (!(GRR._chi_hu_rights[i].opr_and(GameConstants.CHR_QUAN_XIAO_PDK)).is_empty()) {
					game_end_pdk.addQuanXiaoPlayer(extraScore * (getTablePlayerNumber() - 1));
				} else {
					game_end_pdk.addQuanXiaoPlayer(0);
				}
				if (!(GRR._chi_hu_rights[i].opr_and(GameConstants.CHR_QUAN_DAN_PDK)).is_empty()) {
					game_end_pdk.addQuanDanPlayer(extraScore * (getTablePlayerNumber() - 1));
				} else {
					game_end_pdk.addQuanDanPlayer(0);
				}
				if (!(GRR._chi_hu_rights[i].opr_and(GameConstants.CHR_QUAN_SHUANG_PDK)).is_empty()) {
					game_end_pdk.addQuanShuangPlayer(extraScore * (getTablePlayerNumber() - 1));
				} else {
					game_end_pdk.addQuanShuangPlayer(0);
				}
				if (!(GRR._chi_hu_rights[i].opr_and(GameConstants.CHR_QUAN_HEI_PDK)).is_empty()) {
					game_end_pdk.addQuanHeiPlayer(extraScore * (getTablePlayerNumber() - 1));
				} else {
					game_end_pdk.addQuanHeiPlayer(0);
				}
				if (!(GRR._chi_hu_rights[i].opr_and(GameConstants.CHR_QUAN_HONG_PDK)).is_empty()) {
					game_end_pdk.addQuanHongPlayer(extraScore * (getTablePlayerNumber() - 1));
				} else {
					game_end_pdk.addQuanHongPlayer(0);
				}
		...
		if (GRR != null) {
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
				card_arry.Builder out_card_array = card_arry.newBuilder();
				for (int j = 0; j < GRR._card_count[i]; j++) {
					cards_card.addItem(GRR._cards_data[i][j]);
				}
				for (int j = 0; j < _out_card_times[i]; j++) {
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if (_cur_round >= _game_round) {// 局数到了
				...
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					...
					game_end_pdk.addAllEndScore((int) _player_result.game_score[i]);
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status)
			...
			if (this._handler != null) {
				...
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					...
					if (has_rule(GameConstants.GAME_RULE_DISPLAY_CARD_SC)) {
				...
				for (int j = 0; j < GRR._card_count[seat_index]; j++) {
					if (this.has_rule(GameConstants.GAME_RULE_FANG_ZUO_BI_SC) && seat_index != _current_player) {
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player) {
		...
		if (_current_player != GameConstants.INVALID_SEAT) {
			...
			int can_out_card_count = _logic.Player_Can_out_card_SXSD(GRR._cards_data[_current_player],
	 ...
	 */
	//
	public boolean operate_out_card_yinbao(int seat_index, int count, int cards_data[], int type, int to_player) {
		if (seat_index == GameConstants.INVALID_SEAT) {
			return false;
		}
		//// 发送玩家出牌 201 =玩家出牌(CMD_OutCard: operate_player,operate_card)
		...
		if (_current_player != GameConstants.INVALID_SEAT) {
			for (int i = 0; i < GRR._card_count[_current_player]; i++) {
				outcarddata.addUserCanOutData(GRR._cards_data[_current_player][i]);
			}
			outcarddata.setUserCanOutCount(GRR._card_count[_current_player]);
		}
	...
	public boolean operate_player_cards() {
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				if (this.has_rule(GameConstants.GAME_RULE_DISPLAY_CARD_SC)) {
	...
	public void auto_yin_bao(int seat_index, int out_type, int[] cards_data, int card_count) {
		this._handler = _handle_yibao;
		this._handle_yibao.reset_status(seat_index, cards_data, card_count, out_type);
		this._handle_yibao.exe(this);
	}
	...
	public boolean handler_request_trustee(int get_seat_index, boolean isTrustee, int Trustee_type) {
		...
		if (istrustee[get_seat_index] && this._current_player == get_seat_index) {
			...
			if (out_card_count != 0) {
				_logic.sort_card_date_list_SXSD(card_data, out_card_count);
	...
	public int get_hand_card_count_max() {
		...
		if (this.has_rule(GameConstants.GAME_RULE_NOT_TKAE_SXSD)) {
			return GameConstants.MAX_COUNT_NOT_3;
		} else if (this.has_rule(GameConstants.GAME_RULE_MAI_FOUR_SXSD)) {
			return GameConstants.MAX_COUNT_MAI_4;
		} else if (this.has_rule(GameConstants.GAME_RULE_FOUR_SXSD)) {
			return GameConstants.MAX_COUNT_FOUR;
		}
		return GameConstants.MAX_COUNT_NOT_3;
	}
	...
	// 首出
	public boolean can_frist_out(int seat_index, int out_card[], int out_count) {
		boolean is_first = false;
		for (int i = 0; i < out_count; i++) {
			if (out_card[i] == CARD_HONG_TIAO_4) {
				is_first = true;
				break;
			}
		}
		if (!is_first) {
			send_error_notify(seat_index, 2, "必须先出红桃4!");
			return false;
		}

		int card_4 = 0;
		for (int i = 0; i < GRR._card_count[seat_index]; i++) {
			if (this._logic.GetCardLogicValue_SXSD(GRR._cards_data[seat_index][i]) == 4) {
				card_4++;
			}
		}
		if (card_4 == 4) {
			if (out_count != 4) {
				send_error_notify(seat_index, 2, "必须出4个四引爆！");
				return false;
			} else {
				for (int k = 0; k < 4; k++) {
					if (this._logic.GetCardLogicValue_SXSD(out_card[k]) != 4) {
						send_error_notify(seat_index, 2, "必须出4个四引爆！");
						return false;
					}
				}
				m_yin_bao_stata = true;
			}
		}
		return true;
	}
	...
	// 引爆状态，有炸弹不出，返回错误
	public boolean yin_bao(int seat_index, int out_card[], int out_count, int type) {
		if (_logic.GetAllBomCard_SXSD(GRR._cards_data[seat_index], GRR._card_count[seat_index]) > 0
				&& type != GameConstants.PDK_CT_BOMB_CARD) {
			return false;
		}
		return true;
	}
	...
	// 22比见33
	public boolean c22_to_c33(int[] turn_card, int turn_count, int[] out_card, int out_count, int count_2,
			int our_palyer) {
		if (turn_count != count_2) {
			return false;
		}
		for (int i = 0; i < turn_count; i++) {
			int card_value = _logic.GetCardLogicValue_SXSD(turn_card[i]);
			if (card_value != 15) {
				return false;
			}
		}
		...
		int count_3 = 0;
		for (int i = 0; i < GRR._card_count[our_palyer]; i++) {
			if (_logic.GetCardLogicValue_SXSD(GRR._cards_data[our_palyer][i]) == 16) {
				count_3++;
			}
		}
		if (count_3 >= 2) {
			if (out_count != count_2) {
				return true;
			} else {
				for (int j = 0; j < out_count; j++) {
					if (_logic.GetCardLogicValue_SXSD(out_card[j]) != 16) {
						return true;
					}
				}
			}
	...
	public boolean need_to_yinbao(int seat_index) {
		int card_4 = 0;
		for (int i = 0; i < GRR._card_count[seat_index]; i++) {
			if (this._logic.GetCardLogicValue_SXSD(GRR._cards_data[seat_index][i]) == 4) {
				card_4++;
	...
	public void send_boom_info() {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PDK_SEND_BOOM);// 发送炸弹信息
		Boom_Count_pdk.Builder b = Boom_Count_pdk.newBuilder();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			b.addBoomCount(_boom_num[i]);
		}
