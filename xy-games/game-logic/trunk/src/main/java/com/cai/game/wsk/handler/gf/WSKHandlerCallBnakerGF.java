...
public class WSKHandlerCallBnakerGF extends WSKHandlerCallBnaker<WSKTable_GF> {

	@Override
	public void exe(WSKTable_GF table) {
	}

	public boolean handler_call_banker(WSKTable_GF table, int seat_index, int call_action) {
			...
			outer: if (is_finish) {
				...
				if (table.has_rule(GameConstants.GAME_RULE_WSK_GF_CALL_HONGTAO_SAN)) {
					...
					if (table.has_rule(GameConstants.GAME_RULE_WSK_GF_TOU_XIANG)) {
						...
						table.set_timer(WSKTable_GF.TIME_OUT_NO_TOU_XIANG, 20);
					} else {
