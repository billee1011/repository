...
public class MJHandlerGang_SXTDH extends MJHandlerGang<MJTable_SXTDH> {
	...
	public void exe(MJTable_SXTDH table) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			table.change_player_status(i, Constants_SXTuiDaoHu.INVALID_VALUE);
		}

		table._playerStatus[_seat_index].chi_hu_round_valid();

		// 效果
		if (Constants_SXTuiDaoHu.GANG_TYPE_AN_GANG == _type) {
			table.operate_effect_action(_seat_index, Constants_SXTuiDaoHu.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { Constants_SXTuiDaoHu.WIK_AN_GANG }, 1,
					Constants_SXTuiDaoHu.INVALID_SEAT);
		}else {
			table.operate_effect_action(_seat_index, Constants_SXTuiDaoHu.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { Constants_SXTuiDaoHu.WIK_MING_GANG }, 1,
					Constants_SXTuiDaoHu.INVALID_SEAT);
		}

		//自己暗杆和别人打的牌自己再杠 这两种情况是不会发生胡牌的 
		if ((Constants_SXTuiDaoHu.GANG_TYPE_AN_GANG == _type) || (Constants_SXTuiDaoHu.GANG_TYPE_JIE_GANG == _type) ) {
			this.exe_gang(table);
			return;
		}
		...
		if (bAroseAction == false) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if (playerStatus.has_chi_hu()) {
					table.change_player_status(i, Constants_SXTuiDaoHu.Player_Status_OPR_CARD);
	...
	@Override
	public boolean exe_gang(MJTable_SXTDH table) {
		int  no_feng_operate_card=_center_card;
		...
		} else if (Constants_SXTuiDaoHu.GANG_TYPE_JIE_GANG == _type) {
			// 别人打的牌，谁点谁给
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
		...
		} else if (Constants_SXTuiDaoHu.GANG_TYPE_ADD_GANG == _type) {
			// 看看是不是有碰的牌，明杠，三个人开钱 	
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				int cbWeaveKind = table.GRR._weave_items[_seat_index][i].weave_kind;
				int cbCenterCard = table.GRR._weave_items[_seat_index][i].center_card;
				if ((cbCenterCard == no_feng_operate_card) && (cbWeaveKind == Constants_SXTuiDaoHu.WIK_PENG)) {
        ...
        if (Constants_SXTuiDaoHu.GANG_TYPE_ADD_GANG != _type) {
		...
		// 刷新手牌包括组合
		int cards[] = new int[Constants_SXTuiDaoHu.MAX_COUNT];
		int hand_card_count = table._logic.switch_to_cards_data(table.GRR._cards_index[_seat_index], cards);

		WeaveItem weaves[] = new WeaveItem[Constants_SXTuiDaoHu.MAX_WEAVE];
		...
		for (int i = 0; i < weave_count; i++) {
			...
			weaves[i].provide_player = table.GRR._weave_items[_seat_index][i].provide_player + Constants_SXTuiDaoHu.WEAVE_SHOW_DIRECT;
		}
		...
		// 从后面发一张牌给玩家
//		 table.exe_dispatch_card(_seat_index, Constants_SXTuiDaoHu.HU_CARD_TYPE_GANG_KAI, 0);
    ...
    public boolean handler_operate_card(MJTable_SXTDH table, int seat_index, int operate_code, int operate_card) {
        ...
        if ((operate_code != Constants_SXTuiDaoHu.WIK_NULL) && (operate_code != Constants_SXTuiDaoHu.WIK_CHI_HU))// 没有这个操作动作
        {
            table.log_player_error(seat_index, "出牌操作,没有动作");
            return false;
        }

        if ((operate_code != Constants_SXTuiDaoHu.WIK_NULL) && (operate_card != _center_card)) {
		...
		if (table._playerStatus[seat_index].has_action_by_code(Constants_SXTuiDaoHu.WIK_CHI_HU)
				&& operate_code != Constants_SXTuiDaoHu.WIK_CHI_HU) {
			table._playerStatus[seat_index].add_cards_abandoned_hu(operate_card);
			table._playerStatus[seat_index].chi_hu_round_invalid();
		}
        
        if (operate_code == Constants_SXTuiDaoHu.WIK_NULL) {
            table.record_effect_action(seat_index, Constants_SXTuiDaoHu.EFFECT_ACTION_TYPE_ACTION, 1,
                    new long[] { Constants_SXTuiDaoHu.WIK_NULL }, 1);
        ...
        } else if (operate_code == Constants_SXTuiDaoHu.WIK_CHI_HU) {
        ...
        // 选择了不胡
        if (target_action == Constants_SXTuiDaoHu.WIK_NULL) {
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
        ...
        if (jie_pao_count > 0) {
            ...
            GameSchedule.put(
                    new GameFinishRunnable(table.getRoom_id(), _seat_index, Constants_SXTuiDaoHu.Game_End_NORMAL),
	...
	public boolean handler_player_be_in_room(MJTable_SXTDH table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < Constants_SXTuiDaoHu.MAX_WEAVE; j++) {
		...
		int hand_cards[] = new int[Constants_SXTuiDaoHu.MAX_COUNT];
		table._logic.switch_to_cards_data(table.GRR._cards_index[seat_index], hand_cards);

		for (int i = 0; i < Constants_SXTuiDaoHu.MAX_COUNT; i++) {
		...
		// 效果
		table.operate_effect_action(_seat_index, Constants_SXTuiDaoHu.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { _action }, 1, seat_index);
