...
public class MJHandlerPaoQiang_KLDS extends AbstractMJHandler<MJTable_KLDS> {

	// 执行炮呛
	@Override
	public void exe(MJTable_KLDS table) {
		table.pao = new int[table.getTablePlayerNumber()]; // 跑
		table.ziba = new int[table.getTablePlayerNumber()]; // 自拔
		table.duanmen = new int[table.getTablePlayerNumber()]; // 断门

		table._game_status = GameConstants.GS_MJ_PAO_QIANG;// 设置状态
	...
	public boolean handler_pao_qiang(MJTable_KLDS table, int seat_index, int pao, int zi_ba, int duan_men) {
		...
		table._player_result.ziba[seat_index] = zi_ba;
		table._player_result.duanmen[seat_index] = duan_men;
		table.pao[seat_index] = pao;
		table.ziba[seat_index] = zi_ba;
		table.duanmen[seat_index] = duan_men;
	...
	private void player_reconnect(MJTable_KLDS table, int seat_index) {
