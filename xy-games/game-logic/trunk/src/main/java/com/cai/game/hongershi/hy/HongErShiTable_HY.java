...
public class HongErShiTable_HY extends HongErShiTable {

	private static final long serialVersionUID = -9044543068113413354L;
	//
	...
	// public HandlerChiPeng _handler_chi_peng;
	// public boolean[] player_bao_ting;
	...
	public boolean estimate_player_mo_card_response(int seat_index, int card, int operate_card, boolean dipatch) {

		boolean bAroseAction = false;

		// if (super.player_bao_ting[seat_index] && check_si_dui(seat_index)) {
		// return false;
		// }
		...
		if (cards_value_index[logic.get_card_value(card) - 1] == 3) {
			...
			m_gangCardResult[seat_index].cbCardData[index] = card;
			m_gangCardResult[seat_index].isPublic[index] = 1;// 接杠
			...
			_playerStatus[seat_index].add_gang_with_suo_pai(card, seat_index, 1, HongErShiConstants.WIK_GANG);
			bAroseAction = true;
		}
		for (int i = 0; i < 14; i++) {
			if (cards_value_index[i] == 4) {
				int index = m_gangCardResult[seat_index].cbCardCount++;
				m_gangCardResult[seat_index].cbCardData[index] = i + 1;
				m_gangCardResult[seat_index].isPublic[index] = 1;//
				m_gangCardResult[seat_index].type[index] = GameConstants.GANG_TYPE_AN_GANG;

				_playerStatus[seat_index].add_action(HongErShiConstants.WIK_GANG);
				_playerStatus[seat_index].add_action(GameConstants.WIK_NULL);
				_playerStatus[seat_index].add_gang_with_suo_pai(i + 1, seat_index, 1, HongErShiConstants.WIK_GANG);
				bAroseAction = true;
			}
			...
			for (int w = 0; w < GRR._weave_count[seat_index]; w++) {
				if ((GRR._weave_items[seat_index][w].weave_kind == HongErShiConstants.WIK_PENG
						|| GRR._weave_items[seat_index][w].weave_kind == HongErShiConstants.WIK_AN_PENG)
						&& logic.get_card_value(GRR._weave_items[seat_index][w].center_card) == i + 1) {
					if (dipatch && logic.get_card_value(GRR._weave_items[seat_index][w].center_card) == logic.get_card_value(operate_card)) {
						break;
					}
					int index = m_gangCardResult[seat_index].cbCardCount++;
					m_gangCardResult[seat_index].cbCardData[index] = i + 1; // 值
					m_gangCardResult[seat_index].isPublic[index] = 1;
					if (GRR._weave_items[seat_index][w].weave_kind == HongErShiConstants.WIK_AN_PENG) {
						m_gangCardResult[seat_index].type[index] = GameConstants.GANG_TYPE_JIE_GANG;
					} else {
						m_gangCardResult[seat_index].type[index] = GameConstants.GANG_TYPE_ADD_GANG;
					}
		...
		for (int w = 0; w < GRR._weave_count[seat_index]; w++) {
			if ((GRR._weave_items[seat_index][w].weave_kind == HongErShiConstants.WIK_PENG
					|| GRR._weave_items[seat_index][w].weave_kind == HongErShiConstants.WIK_AN_PENG)
					&& logic.get_card_value(GRR._weave_items[seat_index][w].center_card) == logic.get_card_value(card)) {
				int index = m_gangCardResult[seat_index].cbCardCount++;
				m_gangCardResult[seat_index].cbCardData[index] = card;
				m_gangCardResult[seat_index].isPublic[index] = 1;
				if (GRR._weave_items[seat_index][w].weave_kind == HongErShiConstants.WIK_AN_PENG) {
		...
		// TODO： 暗碰
		if (cards_value_index[logic.get_card_value(card) - 1] == 2) {
			if ((check_si_dui(seat_index) && player_bao_ting[seat_index]) || (player_bao_ting[seat_index] && logic.get_card_value(card) != 7)) {
			} else {
				_playerStatus[seat_index].add_action(HongErShiConstants.WIK_AN_PENG);
				_playerStatus[seat_index].add_action(GameConstants.WIK_NULL);
				_playerStatus[seat_index].add_tou(card, HongErShiConstants.WIK_AN_PENG, 0);
				bAroseAction = true;
			}
		}
		for (int i = 0; i < 14; i++) {
			if (cards_value_index[i] == 3) {
				for (int c = 0; c < GRR._card_count[seat_index]; c++) {
					if (i == logic.get_card_value(GRR._cards_data[seat_index][c]) - 1) {
						_playerStatus[seat_index].add_action(HongErShiConstants.WIK_AN_PENG);
						_playerStatus[seat_index].add_action(GameConstants.WIK_NULL);
						_playerStatus[seat_index].add_tou(GRR._cards_data[seat_index][c], HongErShiConstants.WIK_AN_PENG, 0);
						bAroseAction = true;
						break;
					}
				}
	...
	public boolean estimate_player_card_respond(int seat_index, int card, boolean bDisdatch) {
		...
		for (int p = 0; p < getTablePlayerNumber(); p++) {
			...
			if (action != GameConstants.WIK_NULL) {
				if ((check_si_dui(p) && player_bao_ting[p]) || (player_bao_ting[p] && logic.get_card_value(card) != 7)) {
				} else {
	...
	public boolean estimate_player_bao_ting() {
		...
		if (!GameDescUtil.has_rule(gameRuleIndexEx, HongErShiConstants.RULE_BAO_TING)) {
			return bAroseAction;
		}
	...
	public int analyse_chi_hu_card(int seatIndex, int provider_index, int card, int type, ChiHuRight chr, boolean dispatch) {
		...
		for (int w = 0; w < GRR._weave_count[seatIndex]; w++) {
			if (GRR._weave_items[seatIndex][w].weave_kind == HongErShiConstants.WIK_TOU) {
				if (GameDescUtil.has_rule(gameRuleIndexEx, HongErShiConstants.RULE_7_KING_RED)) {
					for (int c = 0; c < GRR._weave_items[seatIndex][w].weave_card.length; c++) {
						if (GRR._weave_items[seatIndex][w].weave_card[c] == 0
								|| GRR._weave_items[seatIndex][w].weave_card[c] == HongErShiConstants.MAGIC_CARD_KING) {
							continue;
						}
						if ((logic.get_card_color(GRR._weave_items[seatIndex][w].weave_card[c]) % 2) == 1) {
							...
							red_7_king += logic.get_card_value(GRR._weave_items[seatIndex][w].weave_card[c]) > 10 ? 1
									: logic.get_card_value(GRR._weave_items[seatIndex][w].weave_card[c]);
						} else {
							black += logic.get_card_value(GRR._weave_items[seatIndex][w].weave_card[c]) > 10 ? 1
									: logic.get_card_value(GRR._weave_items[seatIndex][w].weave_card[c]);
							black_7_king += logic.get_card_value(GRR._weave_items[seatIndex][w].weave_card[c]) > 10 ? 1
									: logic.get_card_value(GRR._weave_items[seatIndex][w].weave_card[c]);
						}
					}
				}
		...
		if (red == 0 || red - red_7_king == 0) { // 全
			chr.opr_or(HongErShiConstants.WIK_ALL_BALCK);
			can_jie_pao = true;
			check_magic_hu = true;
		}
		if (black == 0 || black - black_7_king == 0) { // 全黑
		...
		if (type == HongErShiConstants.WIK_TIAN_HU) {
			...
			if (has_rule(HongErShiConstants.RULE_YI_FAN_QI_HU) && !can_jie_pao && is_si_dui_HY(cardsIndex) == GameConstants.WIK_NULL
					&& GRR._weave_count[seatIndex] == weave_count_chi) {
				chr.set_empty();
				return GameConstants.WIK_NULL;
			}
			...
			if (!can_jie_pao && is_si_dui_HY(cardsIndex) == GameConstants.WIK_NULL && type != HongErShiConstants.WIK_QIANG_GANG
		...
		int code = is_si_dui_HY(cardsIndex);
		...
		// for (int w = 0; w < GRR._weave_count[seatIndex]; w++) {
		// if (GRR._weave_items[seatIndex][w].weave_kind !=
		// HongErShiConstants.WIK_TOU) {
		// continue;
		// }
		// for (int c = 0; c < GRR._weave_items[seatIndex][w].weave_card.length;
		// c++) {
		// if (GRR._weave_items[seatIndex][w].weave_card[c] ==
		// HongErShiConstants.MAGIC_CARD_KING) {
		// continue;
		// }
		//
		// if
		// ((logic.get_card_color(GRR._weave_items[seatIndex][w].weave_card[c])
		// % 2) == 1) {
		// red +=
		// logic.get_card_value(GRR._weave_items[seatIndex][w].weave_card[c]) >
		// 10 ? 1
		...
		// }
		// }
		if (red > 0 && red < 20 && !check_magic_hu) {
			chr.set_empty();
			return GameConstants.WIK_NULL;
		}
	...
	public int analyse_chi_hu_card_bao_ting(int seatIndex, int provider_index, int card, int type, ChiHuRight chr, boolean dispatch) {
		...
		if (type == HongErShiConstants.WIK_TIAN_HU) {
			...
			if (is_si_dui_HY(cardsIndex) == GameConstants.WIK_NULL && type != HongErShiConstants.WIK_QIANG_GANG
					&& GRR._weave_count[seatIndex] == weave_count_chi && has_rule(HongErShiConstants.RULE_YI_FAN_QI_HU)) {
				chr.set_empty();
				return GameConstants.WIK_NULL;
			}
	...
	public int is_si_dui_HY(int[] cards_value_index) {
		...
		if (count == 4) {
			if (long_dui >= 1 && has_rule(HongErShiConstants.RULE_LONG_SI_DUI)) {
	...
	public int getFanShu(int seat_index, ChiHuRight chr, WeaveItem weaveItem[], int cbWeaveCount, boolean dispatch) {
		...
		for (int w = 0; w < cbWeaveCount; w++) {
			if (weaveItem[w].weave_kind == HongErShiConstants.WIK_TOU) {
				for (int c = 0; c < weaveItem[w].weave_card.length; c++) {
					if (weaveItem[w].weave_card[c] == 0) {
						continue;
					}
					wfanShu += 1;
				}
	...
	public void set_result_describe(int seat_index) {
		...
		for (int player = 0; player < this.getTablePlayerNumber(); player++) {
			...
			if (player_bao_ting[player] && GameConstants.Game_End_DRAW != GRR._end_type) {
				if (GRR._chi_hu_rights[player].is_valid()) {
					result.append("报听 3番\n");
				} else {
	...
	public void test_cards() {
		int[] cards_of_player0 = new int[] { 0x04, 0x24, 0x05, 0x25, 0x06, 0x26, 0x17 };
		int[] cards_of_player1 = new int[] { 0x04, 0x24, 0x05, 0x25, 0x06, 0x26, 0x17 };
		int[] cards_of_player2 = new int[] { 0x04, 0x24, 0x05, 0x25, 0x06, 0x26, 0x17 };
		...
		// int[] realyCards = new int[] { 41, 79, 35, 51, 37, 79, 34, 9, 55, 79,
		// 22, 43, 6, 5, 79, 79, 52, 3, 19, 79, 58, 79, 27, 53, 29, 54, 79, 2,
		// 59,
		// 8, 56, 79, 42, 17, 24, 49, 40, 44, 7, 61, 79, 39, 21, 11, 38, 79, 1,
		// 28, 26, 13, 57, 10, 36, 33, 50, 12, 23, 20, 45, 60, 79, 4, 25,
		// 18 };
	...
	public boolean check_si_dui(int seat_index) {
		...
		for (int m = 0; m < max_ting_count; m++) {
			...
			if (is_si_dui_HY(cardsIndex) != GameConstants.WIK_NULL) {
				return true;
			}
		}
		return false;
		...
		for (int w = 0; w < weaveCount; w++) {
			...
			case HongErShiConstants.WIK_TOU: {
				...
				for (int c = 0; c < weaveItems[w].weave_card.length; c++) {
					if (weaveItems[w].weave_card[c] == 0) {
						continue;
					}
					count[3]++;
				}
