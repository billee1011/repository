...
public class PHHandlerDispatchFirstCard_YX extends PHHandlerDispatchCard<PHTable> {
...
//		_send_card_data = 0x18;
		...
		int hu_pai_type[] = new int[table.getTablePlayerNumber()];
		int loop = 0;
...
//        			table.GRR._cards_index[_seat_index][table._logic.switch_to_card_index(table._send_card_data)]++;
		   		...
		   		if(sao_count == 5)
	    		{
		   			if(hu_action != GameConstants.CHR_SHL_HU_PH && hu_action != GameConstants.CHR_XQD_HU_PH)
		   			{
		   				...
		   				curPlayerStatus.add_action(GameConstants.WIK_ZI_MO_WH);
		   				hu_pai_type[i] = GameConstants.WIK_ZI_MO_WH;
	    		...
	    		if(ti_count >= 2)
	    		{
	    			...
	    			curPlayerStatus.add_action(GameConstants.WIK_ZI_MO_SL);
	    			hu_pai_type[i] = GameConstants.WIK_ZI_MO_SL;
					curPlayerStatus.add_zi_mo(hu_card, i);	
	    		...
	    		if(dui_count == 7)
	    		{
	    			if(hu_action != GameConstants.CHR_SHL_HU_PH)
	    			{
	    				...
	    				curPlayerStatus.add_action(GameConstants.WIK_ZI_MO_QXD);
	    				hu_pai_type[i] = GameConstants.WIK_ZI_MO_QXD;
						curPlayerStatus.add_zi_mo(hu_card, i);	
		...
		if(table._playerStatus[_seat_index].has_action() == false)
		{
			...
			if(hu_action != GameConstants.CHR_SHL_HU_PH && hu_action != GameConstants.CHR_XQD_HU_PH&&hu_action != GameConstants.CHR_WU_PFU_HU_PH)
			{		
				...
				int action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
    					table.GRR._weave_count[_seat_index],_seat_index,_seat_index,_send_card_data, chr, card_type,hu_xi,true);
				if(action != GameConstants.WIK_NULL)
				{
					curPlayerStatus.add_action(GameConstants.WIK_ZI_MO);
					hu_pai_type[_seat_index] = GameConstants.WIK_ZI_MO;
		...
		if(hu_action != GameConstants.WIK_NULL)
		{
			...
			while(loop_index < table.getTablePlayerNumber()){
				...
				{
					table.GRR._cards_index[_seat_index][table._logic.switch_to_card_index(table._send_card_data)]++;
					table._playerStatus[i].set_handler_status(GameConstants.PH_STATUS_FRIST_DISPATCH);
					...
					GameSchedule.put(new HuPaiRunnable(table.getRoom_id(), i, hu_pai_type[i] ,_send_card_data),
...
//		assert handler_status == GameConstants.PH_STATUS_FRIST_DISPATCH;
		if(handler_status != GameConstants.PH_STATUS_FRIST_DISPATCH){
			table.log_error("DispatchCard handler_status = "+ handler_status+"!= "+GameConstants.PH_STATUS_FRIST_DISPATCH+"seat_index = " + seat_index);
//			return false;
		}
		...
		if(operate_code == GameConstants.WIK_NULL){
			if(playerStatus.has_zi_mo() == true&&table.has_rule(GameConstants.GAME_RULE_QZ_HU_PAI_ON)||table._warning[_seat_index] == 2)
			{
...
//				table.operate_player_get_card(_seat_index, 0, null, GameConstants.INVALID_SEAT,false);
			...
			curPlayerStatus.set_handler_status(GameConstants.PH_STATUS_FRIST_DISPATCH);
		...
		case GameConstants.WIK_ZI_MO_WH:
		case GameConstants.WIK_ZI_MO_SL:
		case GameConstants.WIK_ZI_MO_QXD:
