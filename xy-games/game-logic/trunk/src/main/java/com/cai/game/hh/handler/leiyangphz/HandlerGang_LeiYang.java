...
public class HandlerGang_LeiYang extends HHHandlerGang<Table_LeiYang> {
    ...
    @Override
    protected boolean exe_gang(Table_LeiYang table) {
        // 计算提龙的分值
        if (GameConstants.PAO_TYPE_TI_MINE_LONG == _type || GameConstants.PAO_TYPE_MINE_SAO_LONG == _type) {
            if (0 == table._logic.get_card_color(_center_card)) { // 小字提龙
                table.player_ti_count[_seat_index][0]++;
            } else { // 大字提龙
                table.player_ti_count[_seat_index][1]++;
            }
        ...
        for (int x = 0; x < hand_card_count; x++) {
            if (table.is_card_has_wei(cards[x])) { // 如果是偎的牌
                ...
                if (b_is_ting_state ||(table.is_hands_up[_seat_index] && table.out_status[_seat_index] != 0))
...
//            }
        ...
        if ((table._long_count[_seat_index] <= 1 || GameConstants.SAO_TYPE_MINE_SAO == _type)
            ...
            if (_seat_index != _provide_player) {
                if (_depatch == true)
                    card_type = Constants_LeiYang.HU_CARD_FAN_PAI_OTHER;
                else
                    card_type = Constants_LeiYang.HU_CARD_JIE_PAO;
            }
            ...
            action_hu = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index],
                    table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], _seat_index,
                    _provide_player, 0, chr, card_type, hu_xi, _depatch == true ? true : false);
            ...
            if (pai_count == 0) { // 如果是第1个提跑或者是偎牌，如果没牌可以出了，判断能不能胡，不能胡，设置成相公
                ...
                if (all_hu_xi >= 10 && !(table.is_hands_up[_seat_index] && table.out_status[_seat_index] != 0)) {
                    b_hu_xi = true;
                }
                ...
                if (b_hu_xi == true) { // 如果能胡
                    ...
                    if (hong_pai_count == 0) {
                    	chr.opr_or(Constants_LeiYang.CHR_HEI_HU);
            		} else if (hong_pai_count == 1 && table.has_rule(Constants_LeiYang.GAME_RULE_YI_DIAN_HONG)) {
            			chr.opr_or(Constants_LeiYang.CHR_DIAN_HU);
            		} else if (hong_pai_count >= 13) {
            			chr.opr_or(Constants_LeiYang.CHR_HONG_HU);
            		}
                    ...
                    if (table.has_rule(Constants_LeiYang.GAME_RULE_QIANG_ZHI_HU)) { // 如果有强制胡牌
                        if (_seat_index != _provide_player) {
                            GameSchedule.put(new Runnable() {
                                @Override
                                public void run() {
