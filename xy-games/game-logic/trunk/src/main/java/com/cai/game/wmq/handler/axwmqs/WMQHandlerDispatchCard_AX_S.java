 ...
 *
 */
public class WMQHandlerDispatchCard_AX_S extends WMQHandlerDispatchCard<WMQTable> {
		...
		while (loop < table.getTablePlayerNumber()) {
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				if (table.has_rule(GameConstants.GAME_RULE_QIANG_HU_PAI)) {
					...
					if (table.getRuleValue(GameConstants.GAME_RULE_XIAO_ZHUO_BAN) != 1) {

						tempPlayerStatus.add_action(GameConstants.WIK_ZHE_WMQ);
						tempPlayerStatus.add_zhe(_send_card_data, GameConstants.WIK_ZHE_WMQ, i);
					}
		...
		if (curPlayerStatus.has_action()) {// 有动作
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				if (bAroseAction == false) {
					...
					SysParamModel sysParamModel1104 = SysParamServerDict.getInstance()
							.getSysParamModelDictionaryByGameId(SysGameTypeDict.getInstance().getGameIDByTypeIndex(table._game_type_index)).get(1104);
	...
	public boolean handler_operate_card(WMQTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if (table._playerStatus[i].get_perform() == GameConstants.WIK_ZHE_WMQ) {
				table.operate_effect_action(i, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { GameConstants.WIK_ZHE_WMQ }, 1,
						GameConstants.INVALID_SEAT);
		...
		case GameConstants.WIK_NULL: {
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				SysParamModel sysParamModel1105 = SysParamServerDict.getInstance()
						.getSysParamModelDictionaryByGameId(SysGameTypeDict.getInstance().getGameIDByTypeIndex(table._game_type_index)).get(1105);
