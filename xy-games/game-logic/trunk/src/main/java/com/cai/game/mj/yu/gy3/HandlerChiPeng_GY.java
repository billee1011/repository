...
public class HandlerChiPeng_GY extends MJHandlerChiPeng<Table_GY> {
	...
	public void exe(Table_GY table) {
		...
		if (table.is_ji_card(_card)) {
			if (_card == GameConstants_GY.BA_TONG_CARD && table.chong_feng_ji_seat_bt == -2) {
				table.responsibility_ji[1].setLeft(_provider);
				table.responsibility_ji[1].setRight(_seat_index);
				// table._player_result.duanmen[_seat_index] = 1;
				table._player_result.duanmen[_provider] = 1;
			} else if (_card == GameConstants_GY.YJ_CARD && table.chong_feng_ji_seat_yj == -2) {
				table.responsibility_ji[0].setLeft(_provider);
			...
			table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { GameConstants_GY.WIK_ZE_REN_JI }, 1,
