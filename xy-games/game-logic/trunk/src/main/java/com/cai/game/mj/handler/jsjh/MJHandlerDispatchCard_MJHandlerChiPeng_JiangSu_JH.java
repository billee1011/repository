...
public class MJHandlerDispatchCard_MJHandlerChiPeng_JiangSu_JH extends MJHandlerDispatchCard<MJTable_JiangSu_JH> {
    ...
    public void exe(MJTable_JiangSu_JH table) {
        ...
        if (outcard_count == 0 && table._logic.is_magic_card(_send_card_data)) {
            ...
            table.operate_effect_action(_seat_index, GameConstants.Effect_Action_Other, dacard_count, effect_indexs, 1,
    ...
    public boolean handler_operate_card(MJTable_JiangSu_JH table, int seat_index, int operate_code, int operate_card) {
        ...
        case GameConstants.WIK_ZI_MO: // 自摸
        {
            ...
            if (!table.has_rule(GameConstants.GAME_RULE_JIANGSU_PEI_CHONG)) {
                ...
                for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                    if (table._peng_palyer_count[_seat_index][i] >= 3) {
    ...
    public void other_deal(MJTable_JiangSu_JH table) {
        PlayerStatus curPlayerStatus = table._playerStatus[_seat_index];
        curPlayerStatus.reset();
