...
public class WSKHandlerOutCardOperate_ShangRaoDaZha extends WSKHandlerOutCardOperate<WSKTable_ShangRaoDaZha> {
	...
	public void exe(WSKTable_ShangRaoDaZha table) {
		...
		if (card_type >= GameConstants.WSK_GF_CT_BOMB_4) {
		...
		// 罚王检测
		int total_wang_count = table._logic.Get_Wang_Count(table.GRR._cards_data[_out_card_player], table.GRR._cards_data[_out_card_player].length);
		...
		if (table.GRR._card_count[_out_card_player] == 0) {
			...
			if (table._is_yi_da_san) {
				...
				GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._chuwan_shunxu[0], GameConstants.Game_End_NORMAL), delay, TimeUnit.SECONDS);
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					if (!table._logic.have_card_num(table.GRR._cards_data[i], table.GRR._card_count[i], 4) && table.fa_wang[i] == 0) {
						for (int j = 0; j < table.GRR._card_count[i]; j++) {
							if (table._logic.GetCardColor(table.GRR._cards_data[i][j]) == 0x40) {
				...
				if (!table.checkChunTian(_out_card_player)) {
					if (table.has_rule(GameConstants.GAME_RULE_WSK_GF_NEI_XIAO) && _out_card_player == table.GRR._banker_player) {
						for (int i = 0; i < table.getTablePlayerNumber(); i++) {
							int score = table._logic.GetHandCardXianScore(table.GRR._cards_data[i], table.GRR._card_count[i], table.sheng_dang_biaozhi);
					...
					} else if (!table.has_rule(GameConstants.GAME_RULE_WSK_GF_NEI_XIAO) && _out_card_player != table.GRR._banker_player) {
						for (int i = 0; i < table.getTablePlayerNumber(); i++) {
							int score = table._logic.GetHandCardXianScore(table.GRR._cards_data[i], table.GRR._card_count[i], table.sheng_dang_biaozhi);
							for (int j = 0; j < table.getTablePlayerNumber(); j++) {
				...
				// 一二游
				if ((table._chuwan_shunxu[0] != GameConstants.INVALID_SEAT && table._chuwan_shunxu[1] != GameConstants.INVALID_SEAT && table._chuwan_shunxu[1] == table._friend_seat[table._chuwan_shunxu[0]])
						|| (table._chuwan_shunxu[0] != GameConstants.INVALID_SEAT && table._chuwan_shunxu[1] != GameConstants.INVALID_SEAT && table._chuwan_shunxu[2] != GameConstants.INVALID_SEAT)) {
					for (int j = 0; j < table.getTablePlayerNumber(); j++) {
		...
		// 显示出牌
		table.operate_out_card(table._out_card_player, table._turn_out_card_count, table._turn_out_card_data, table._turn_out_card_type, GameConstants.INVALID_SEAT, false);
	...
	public void table_pai_socre(WSKTable_ShangRaoDaZha table) {
		int pai_score = 0;
	...
	public void user_pass_card(WSKTable_ShangRaoDaZha table) {
		...
		// 显示出牌
		table.operate_out_card(_out_card_player, 0, null, GameConstants.WSK_GF_CT_PASS, GameConstants.INVALID_SEAT, false);
	...
	public int adjust_out_card_right(WSKTable_ShangRaoDaZha table) {
		int card_type = GameConstants.WSK_GF_CT_ERROR;
		if (table.GRR._card_count[_out_card_player] == _out_card_count) {
			card_type = table._logic.GetCardType(_out_cards_data, _out_card_count, true);
		} else {
			card_type = table._logic.GetCardType(_out_cards_data, _out_card_count, false);
		}
		if (card_type == GameConstants.WSK_GF_CT_ERROR) {
			return card_type;
		}
