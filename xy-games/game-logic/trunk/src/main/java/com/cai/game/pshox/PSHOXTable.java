...
import protobuf.clazz.pshox.pshoxRsp.ButtonOperatePshox;
import protobuf.clazz.pshox.pshoxRsp.GameStartPshox;
import protobuf.clazz.pshox.pshoxRsp.JettonResultPshox;
import protobuf.clazz.pshox.pshoxRsp.OpenCardPshox;
import protobuf.clazz.pshox.pshoxRsp.Opreate_Pshox_Request;
import protobuf.clazz.pshox.pshoxRsp.PlayerResultPshox;
import protobuf.clazz.pshox.pshoxRsp.PukeGameEndPshox;
import protobuf.clazz.pshox.pshoxRsp.SelectdBankerPshox;
import protobuf.clazz.pshox.pshoxRsp.SelectdBankerResultPshox;
import protobuf.clazz.pshox.pshoxRsp.SendCardPshox;
import protobuf.clazz.pshox.pshoxRsp.TableResponsePshox;

///////////////////////////////////////////////////////////////////////////////////////////////
public class PSHOXTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	private static Logger logger = Logger.getLogger(PSHOXTable.class);
	...
	public PSHOXGameLogic _logic = null;
	...
	protected boolean on_handler_game_start() {
		...
		else if (this.getRuleValue(GameConstants.GAME_RULE_TWENTY_TUI_ZHU) == 1)
		...
		return game_start_Pshox();
	}

	// 斗板牛开始
	public boolean game_start_Pshox() {
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			...
			roomResponse.setType(MsgConstants.RESPONSE_PSHOX_GAME_START);
			roomResponse.setGameStatus(this._game_status);
			// 发送数据
			GameStartPshox.Builder gamestart = GameStartPshox.newBuilder();
	...
	public boolean update_button(int opr_type, int seat_index, int display_time, boolean is_grr) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PSHOX_UPDATE_BTN);
		ButtonOperatePshox.Builder button_operate = ButtonOperatePshox.newBuilder();
	...
	public boolean send_call_banker_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PSHOX_ROBOT_BANKER);
		SelectdBankerPshox.Builder robot_banker = SelectdBankerPshox.newBuilder();
	...
	public boolean set_banker_to_room() {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PSHOX_ROBOT_BANKER_RESULT);
		SelectdBankerResultPshox.Builder robot_banker = SelectdBankerResultPshox.newBuilder();
	...
	public void send_jetton_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PSHOX_JETTON);
		JettonResultPshox.Builder jetton_result = JettonResultPshox.newBuilder();
	...
	public boolean send_card(int card_count, int opr_type, boolean is_display) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PSHOX_SEND_CARD);
		SendCardPshox.Builder send_card = SendCardPshox.newBuilder();
			...
			for (int k = 0; k < this.getTablePlayerNumber(); k++) {
				...
				if (this._player_status[k] == false) {
					...
					if (card_count == GameConstants.OX_MAX_CARD_COUNT) {
						if (getRuleValue(GameConstants.GAME_RULE_SELECT_OX_VALUSE_ONE) == 1)
							this._player_times[i] = _logic.get_times_two(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
						if (getRuleValue(GameConstants.GAME_RULE_SELECT_OX_VALUSE_TWO) == 1)
					...
					if (this.getRuleValue(GameConstants.GAME_RULE_MSZQZ_OX) == 1) {
						for (int j = 0; j < 3; j++) {
							cards.addItem(GRR._cards_data[k][j]);
	...
	public boolean open_card(int seat_index) {
		...
		if (flag == false) {
			if (this.getRuleValue(GameConstants.GAME_RULE_TB_OX) == 1) {
				int max_win = process_tbox_calulate_end();
				process_ox_calulate_end(max_win);
			} else {
				process_ox_calulate_end(_cur_banker);
			}
	...
	public void process_ox_calulate_end(int seat_index) {

		if (this.getRuleValue(GameConstants.GAME_RULE_NNSZ_OX) == 1) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this._player_status[i] != true)
				continue;
			boolean first_ox = _logic.get_ox_card(GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
			boolean next_ox = _logic.get_ox_card(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
			int action = _logic.compare_card(GRR._cards_data[seat_index], GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, first_ox, next_ox,
					this.ruleMap);
			// 获取点数
			...
			// 获取点数
			if (action == 1) {
				if (this.getRuleValue(GameConstants.GAME_RULE_SELECT_OX_VALUSE_ONE) == 1)
					calculate_score = _logic.get_times_two(GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
				if (getRuleValue(GameConstants.GAME_RULE_SELECT_OX_VALUSE_TWO) == 1)
					calculate_score = _logic.get_times_one(GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
	...
	public boolean send_open_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PSHOX_OPEN_CARD);
		OpenCardPshox.Builder open_card = OpenCardPshox.newBuilder();
	...
	public RoomPlayerResponse.Builder load_player_info_data(int seat_index) {
	...
	public void load_player_info_data_game_start(GameStartPshox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_game_end(PukeGameEndPshox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_reconnect(TableResponsePshox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		ret = this.handler_game_finish_Pshox(seat_index, reason);

		return ret;
	}

	public boolean handler_game_finish_Pshox(int seat_index, int reason) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PSHOX_GAME_END);
		GameEndResponse.Builder game_end = GameEndResponse.newBuilder();
		PukeGameEndPshox.Builder game_end_Pshox = PukeGameEndPshox.newBuilder();
		RoomInfo.Builder room_info = getRoomInfo();
		game_end_Pshox.setRoomInfo(room_info);
		...
		this.load_player_info_data_game_end(game_end_Pshox);
		game_end_Pshox.setGameRound(_game_round);
		game_end_Pshox.setCurRound(_cur_round);
		game_end_Pshox.setDisplayTime(this._idi_ready_card_time);
		if (GRR != null) {
			GameRoundRecordOx record_info = new GameRoundRecordOx(this.getTablePlayerNumber(), GameConstants.OX_MAX_CARD_COUNT,
					this.getTablePlayerNumber());
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				game_end_Pshox.addEndScore((int) GRR._game_score[i]);
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
				for (int j = 0; j < GameConstants.OX_MAX_CARD_COUNT; j++) {
					cards_card.addItem(GRR._cards_data[i][j]);
				}
				game_end_Pshox.addCardsData(cards_card);
				game_end_Pshox.addStrOxValue(this._str_ox_value[i]);
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if (_cur_round >= _game_round) {// 局数到了
				...
				game_end_Pshox.setPlayerResult(this.process_player_result_Pshox(reason));
		...
		game_end_Pshox.setWinner(seat_index);
		game_end_Pshox.setReason(real_reason);
		////////////////////////////////////////////////////////////////////// 得分总的
		game_end.setEndType(real_reason);
		game_end.setGameRound(_game_round);
		game_end.setCurRound(_cur_round);
		game_end.setCommResponse(PBUtil.toByteString(game_end_Pshox));
	...
	public boolean handler_enter_room(Player player) {
		...
		if (player.getAccount_id() != this.getRoom_owner_account_id() && getRuleValue(GameConstants.GAME_RULE_START_FORBID_JOIN) == 1) {
	...
	public void is_game_start() {
		...
		if ((_player_count >= sit_max) && (_player_count == _cur_count)) {
			if (this._cur_round == 0 && has_rule(GameConstants.GAME_RULE_CONTORL_START)) {
				this.is_game_start = 1;
				control_game_start();
			} else
				handler_game_start();
		} else if ((this._cur_round == 0) && this.getRuleValue(GameConstants.GAME_RULE_CONTORL_START) == 1) {
			this.is_game_start = 0;
			control_game_start();
		}
	...
	public boolean table_response_observer(Player player) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_PSHOX_RECONNECT_DATA);

		TableResponsePshox.Builder tableResponse_Pshox = TableResponsePshox.newBuilder();
		load_player_info_data_reconnect(tableResponse_Pshox);
		RoomInfo.Builder room_info = getRoomInfo();
		tableResponse_Pshox.setRoomInfo(room_info);

		tableResponse_Pshox.setBankerPlayer(_cur_banker);

		int display_time = _cur_operate_time - ((int) (System.currentTimeMillis() / 1000L) - _operate_start_time);
		tableResponse_Pshox.setDisplayTime(display_time);

		for (int i = 0; i < getTablePlayerNumber(); i++) {

			tableResponse_Pshox.addCallButton(this._player_call_banker[i]);
			tableResponse_Pshox.addJettonScore(this._jetton_player[i]);
			tableResponse_Pshox.addOpenCard(this._open_card_player[i]);
			if (this._open_card_player[i] == true) {
				tableResponse_Pshox.addOxValue(this._ox_value[i]);
				tableResponse_Pshox.addTimes(this._player_times[i]);
			} else {
				tableResponse_Pshox.addOxValue(-1);
				tableResponse_Pshox.addTimes(0);
			}
		}
		tableResponse_Pshox.setSceneStatus(_game_status);
		if (_game_status == GameConstants.GS_OX_CALL_BANKER) {

			tableResponse_Pshox.setBankerPlayer(-1);
			int card_count = 4;
			...
			if (getRuleValue(GameConstants.GAME_RULE_KSZQZ_OX) == 1 || getRuleValue(GameConstants.GAME_RULE_ASZQZ_OX) == 1) {
				for (int k = 0; k < this.getTablePlayerNumber(); k++) {
					...
					tableResponse_Pshox.addCardsData(k, cards);
				}
			}
		...
		if (_game_status == GameConstants.GS_OX_OPEN_CARD) {
			...
			for (int k = 0; k < this.getTablePlayerNumber(); k++) {
				...
				if (this._player_status[k] == false) {
					...
					if (getRuleValue(GameConstants.GAME_RULE_MSZQZ_OX) == 1 && this._open_card_player[k] == false) {
						for (int j = 0; j < 3; j++) {
							cards.addItem(GRR._cards_data[k][j]);
					...
					tableResponse_Pshox.setCurOxValue(-1);
					tableResponse_Pshox.setCurTimes(0);
					tableResponse_Pshox.clearSortCard();
		...
		roomResponse.setCommResponse(PBUtil.toByteString(tableResponse_Pshox));
			...
			if (_game_status == GameConstants.GS_OX_OPEN_CARD) {
				...
				for (int k = 0; k < this.getTablePlayerNumber(); k++) {
					...
					if (this._player_status[k] == false) {
						...
						if (getRuleValue(GameConstants.GAME_RULE_MSZQZ_OX) == 1 && this._open_card_player[k] == false && k != seat_index) {
							for (int j = 0; j < 3; j++) {
								cards.addItem(GRR._cards_data[k][j]);
						...
						if (k == seat_index) {
							tableResponse_Pshox.setCurOxValue(this._ox_value[seat_index]);
							tableResponse_Pshox.setCurTimes(this._player_times[seat_index]);
						}
						int temp_cards_data[] = new int[GameConstants.OX_MAX_CARD_COUNT];
						for (int index = 0; index < GameConstants.OX_MAX_CARD_COUNT; index++) {
						...
						tableResponse_Pshox.clearSortCard();
						for (int index = 0; index < GameConstants.OX_MAX_CARD_COUNT; index++) {
							tableResponse_Pshox.addSortCard(temp_cards_data[index]);
						}
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		if (type == MsgConstants.REQUST_PSHOX_OPERATE) {

			Opreate_Pshox_Request req = PBUtil.toObject(room_rq, Opreate_Pshox_Request.class);
			if (req == null) {
	...
	private PlayerResultPshox.Builder process_player_result_Pshox(int reason) {
		int win_idx = 0;
		float max_score = 0;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			int winner = -1;
		...
		PlayerResultPshox.Builder player_result = PlayerResultPshox.newBuilder();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean animation_timer(int timer_id) {
		...
		case ID_TIMER_ANIMATION_START: {
			if (getRuleValue(GameConstants.GAME_RULE_ROOM_OX) == 1) {
			...
			} else if (getRuleValue(GameConstants.GAME_RULE_TB_OX) == 1) {
				this._cur_banker = -1;
				...
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					if (this._player_status[i] == false)
						continue;
					_jetton_player[i] = this._jetton_score[i][0];
				}
				send_card(5, 1, false);
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					if (this._player_status[i] == false)
			...
			} else if (getRuleValue(GameConstants.GAME_RULE_NNSZ_OX) == 1) // 牛牛上庄不是轮庄
			{
				if (this._cur_round == 1) {
	...
	public boolean trustee_timer(int operate_id, int seat_index) {
		if (operate_id == GameConstants.HJK_READY_TIMER) {
			...
			} else if (this.getRuleValue(GameConstants.GAME_RULE_FIVE_TO_TEN) == 1) {
