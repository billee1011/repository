...
public class PHZHandlerOutCardOperate_WangCheng extends HHHandlerOutCardOperate<WangChengHHTable> {
	@Override
	public void exe(WangChengHHTable table) {
		boolean shoot = false;
		if (table._logic.is_valid_card(_out_card_data - GameConstants.CARD_ESPECIAL_TYPE_CAN_SHOOT) == false)
			shoot = true;
		...
		if (shoot) {
			for (int c = 0; c < GameConstants.MAX_HH_INDEX; c++) {
				table._cannot_chi[_out_card_player][table._cannot_chi_count[_out_card_player]++] = table._logic.switch_to_card_data(c);
				table._cannot_peng[_out_card_player][table._cannot_peng_count[_out_card_player]++] = table._logic.switch_to_card_data(c);
			}
		}

		// 玩家出牌 响应判断,是否有吃,碰,胡 swe
		boolean bAroseAction = table.estimate_player_out_card_respond_chen_zhou(_out_card_player, _out_card_data, false);// ,

		// if (_type == GameConstants.OutCard_Type_Di_Hu) {
		// ChiHuRight chr = table.GRR._chi_hu_rights[next_player];
		// chr.set_empty();
		// int hu_xi_chi[] = new int[] { 0 };
		// playerStatus = table._playerStatus[next_player];
		// int hu_xi =
		// table.analyse_chi_hu_card(table.GRR._cards_index[next_player],
		// table.GRR._weave_items[next_player],
		// table.GRR._weave_count[next_player], next_player, _out_card_player,
		// _out_card_data, chr, _type, hu_xi_chi, true);// 自
		//
		// if (hu_xi != GameConstants.WIK_NULL) {
		// playerStatus.add_action(GameConstants.WIK_ZI_MO);
		// playerStatus.add_zi_mo(table._send_card_data, next_player);
		// bAroseAction = true;
		// }
		// }
	...
	public boolean handler_operate_card(WangChengHHTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		if (target_action == GameConstants.WIK_NULL) {
			// 过张的牌都不可以吃
			int next_player = (_out_card_player + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();
			table._cannot_chi[_out_card_player][table._cannot_chi_count[_out_card_player]++] = target_card;
