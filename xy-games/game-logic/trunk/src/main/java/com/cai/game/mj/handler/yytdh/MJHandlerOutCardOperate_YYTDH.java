...
public class MJHandlerOutCardOperate_YYTDH extends MJHandlerOutCardOperate<MJTable_YYTDH> {
    @Override
    public void exe(MJTable_YYTDH table) {
        ...
        // 玩家出牌 响应判断,是否有吃碰杠补胡
        boolean bAroseAction = table.estimate_player_out_card_respond_yytdh(_out_card_player, _out_card_data);// ,
    ...
    public boolean handler_operate_card(MJTable_YYTDH table, int seat_index, int operate_code, int operate_card) {
        // 效验状态
        // operate_code = table.get_real_card(operate_code);
        ...
        if (operate_code == GameConstants.WIK_CHI_HU) {
            ...
            // 效果
            table.process_chi_hu_player_operate_yytdh(seat_index, new int[] { operate_card }, 1, false);
        ...
        case GameConstants.WIK_CHI_HU: // 胡
        {
            ...
            if (jie_pao_count > 0) {
                if (jie_pao_count > 1) {
                    ...
                    table.set_niao_card_yytdh(_out_card_player, GameConstants.INVALID_VALUE, true, 0, true);
                } else if (jie_pao_count == 1) {
                    table._cur_banker = target_player;
                    // 结束后设置鸟牌
                    table.set_niao_card_yytdh(target_player, GameConstants.INVALID_VALUE, true, 0, false);
                }
                for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                    if ((i == _out_card_player) || (table.GRR._chi_hu_rights[i].is_valid() == false)) {
                        continue;
