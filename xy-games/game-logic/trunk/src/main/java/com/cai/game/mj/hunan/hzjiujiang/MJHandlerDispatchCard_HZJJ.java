...
public class MJHandlerDispatchCard_HZJJ extends MJHandlerDispatchCard<MJTable_HZJJ> {
    ...
    public void exe(MJTable_HZJJ table) {
        ...
        if (curPlayerStatus.has_zi_mo() && table.has_rule(GameConstants_HZJJ.GAME_RULE_QIANG_ZHI_HU)) {
			// 见炮胡
			table.exe_jian_pao_hu(_seat_index, GameConstants.WIK_ZI_MO, _send_card_data);
			return;
		}
        // 荒庄结束
	   if ( !curPlayerStatus.has_action() && table.GRR._left_card_count == llcard) {
	        for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	            table.GRR._chi_hu_card[i][0] = GameConstants.INVALID_VALUE;
	        }
	        table._cur_banker = table._last_dispatch_player;
...
//            }
        ...
        case GameConstants.WIK_ZI_MO: // 自摸
        {
            ...
            int add_niao = table.getAddNiao(_seat_index);
            // 下局胡牌的是庄家
            table.set_niao_card_hz(_seat_index, GameConstants.INVALID_VALUE, true, add_niao, operate_card,false);// 结束后设置鸟牌
