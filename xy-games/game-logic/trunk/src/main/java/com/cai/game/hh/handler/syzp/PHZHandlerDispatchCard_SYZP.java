 ...
 *
 */
public class PHZHandlerDispatchCard_SYZP extends HHHandlerDispatchCard<HHTable_SYZP> {
	...
	public void exe(HHTable_SYZP table) {
		...
		int wik_ti_pao = GameConstants.WIK_NULL;
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				wik_ti_pao = GameConstants.WIK_ZI_MO;
				bHupai = 1;
		...
		if(table.has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_YOU_PAO_BI_HU)){
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (table._playerStatus[i].has_chi_hu()) {
					...
					table.operate_out_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants.OUT_CARD_TYPE_MID, GameConstants.INVALID_SEAT);
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if ((action_pao[i] != GameConstants.WIK_NULL) && (bHupai == 0)) {
				wik_ti_pao = GameConstants.WIK_PAO;
				table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants.INVALID_SEAT, false);
		...
		if (curPlayerStatus.has_action()) {// 有动作
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				GameSchedule.put(new OutCardRunnable(table.getRoom_id(), _seat_index, _send_card_data), GameConstants.DELAY_AUTO_OUT_CARD,
						TimeUnit.MILLISECONDS);
			} else if (wik_ti_pao == GameConstants.WIK_NULL) {
	...
	public boolean handler_operate_card(HHTable_SYZP table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			table.operate_out_card(_seat_index, 1, new int[] { operate_card }, GameConstants.OUT_CARD_TYPE_MID, GameConstants.INVALID_SEAT);
