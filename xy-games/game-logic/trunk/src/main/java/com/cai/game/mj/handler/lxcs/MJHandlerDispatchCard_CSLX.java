 ...
 *
 */
public class MJHandlerDispatchCard_CSLX extends MJHandlerDispatchCard<MJTable> {
    ...
    public void exe(MJTable table) {
        ...
        // 小胡检测判断
        if (table.FIRST_DISPATHCARD) {
            // 判断是否有小胡
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                PlayerStatus playerStatus = table._playerStatus[i];
                // 胡牌判断
                int xiao_hu_action = table.analyse_chi_hu_card_cs_xiaohu_lx(table.GRR._cards_index[i],
                        table.GRR._start_hu_right[i]);

                // 小胡
                if (xiao_hu_action != GameConstants.WIK_NULL) {
                    playerStatus.add_action(GameConstants.WIK_XIAO_HU);
                    table._game_status = GameConstants.GS_MJ_XIAOHU;// 设置状态
                } else {
                    table.GRR._start_hu_right[i].set_empty();
                }
            }
            table.FIRST_DISPATHCARD = false;
        }
