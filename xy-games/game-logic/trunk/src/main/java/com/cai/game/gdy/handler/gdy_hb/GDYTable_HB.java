...
import protobuf.clazz.gdy.gdyRsp.QiePaiResult_GDY;
...
public class GDYTable_HB extends AbstractGDYTable {
	...
	protected void onInitTable() {
		...
		_handler_out_card_operate = new GDYHandlerOutCardOperate_HB();
	...
	protected boolean on_handler_game_start() {
		...
		if (_qie_pai_seat == GameConstants.INVALID_SEAT) {
			_qie_pai_seat = (_current_player + getTablePlayerNumber() - 1) % this.getTablePlayerNumber();
		}
		...
		if (this.has_rule(GameConstants.GAME_RULE_GDY_QIE_PAI)) {
			this._game_status = GameConstants.GS_MJ_QIE_PAI;// 设置状态
			this.set_timer(ID_TIMER_QIE_PAI_WAIT, 10);
			this.send_qie_pai_begin();
		} else {
			on_game_start();
		}
	...
	protected void test_cards() {
		...
		int cards[] = new int[] { 79, 78, 78, 59, 59, 61, 28, 1, 37, 9, 51, 3, 35, 29, 33, 23, 78, 52, 54, 20, 24, 58,
				6, 49, 39, 5, 53, 56, 57, 41, 44, 25, 55, 38, 45, 22, 17, 8, 10, 60, 26, 13, 7, 19, 4, 12, 27, 34, 36,
				40, 11, 42, 21, 50 };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			if (this.get_players()[i] == null) {
				continue;
	...
	protected boolean on_handler_game_finish(int seat_index, int reason) {
		...
		game_end_gdy.setRoomInfo(this.getRoomInfo());
	...
	public void send_qie_pai_begin() {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		QiePaiStart_GDY.Builder qie_pai_begin = QiePaiStart_GDY.newBuilder();
		qie_pai_begin.setQiepaiChair(this._qie_pai_seat);
		qie_pai_begin.setDisplayTime(10);
		qie_pai_begin.setRoomInfo(this.getRoomInfo());
		qie_pai_begin.setBankerPlayer(this.GRR._banker_player);
		load_player_info_data_qie_pai(qie_pai_begin);
		roomResponse.setType(MsgConstants.RESPONSE_GDEYE_QIE_PAI_BEGIN);// 201
		roomResponse.setCommResponse(PBUtil.toByteString(qie_pai_begin));
		this.send_response_to_room(roomResponse);
	}

	public void send_qie_pai_result(int seat_index, int card_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		QiePaiResult_GDY.Builder qie_pai_result = QiePaiResult_GDY.newBuilder();
		qie_pai_result.setQiepaiChair(seat_index);
		qie_pai_result.setCardIndex(card_index);
		roomResponse.setType(MsgConstants.RESPONSE_GDEYE_QIE_PAI_RESULT);// 201
		roomResponse.setCommResponse(PBUtil.toByteString(qie_pai_result));
		this.send_response_to_room(roomResponse);
	}
	...
	public void cal_score_gdy_jd(int end_score[], int win_seat_index) {
		...
		this._qie_pai_seat = (_win_player + 1) % getTablePlayerNumber();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			if (end_score[i] < end_score[_qie_pai_seat]) {
				_qie_pai_seat = i;
			}
			if (end_score[i] > 0) {
	...
	public boolean handler_player_be_in_room(int seat_index) {
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status)
			...
			if (_game_status == GameConstants.GS_MJ_QIE_PAI) {
				this.send_qie_pai_begin();
			}
		}
	...
	public boolean animation_timer(int timer_id) {
		...
		case ID_TIMER_QIE_PAI_WAIT: {
			deal_qie_pai(this._qie_pai_seat, 20);
			return true;
		}
		}
	...
	public void deal_qie_pai(int seat_index, int card_index) {
		...
		this.kill_timer();
		send_qie_pai_result(seat_index, card_index);
		this._game_status = GameConstants.GS_MJ_PLAY;// 设置状态
