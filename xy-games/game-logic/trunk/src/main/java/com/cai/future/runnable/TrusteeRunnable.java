...
public class TrusteeRunnable extends BaseFuture {
	private static Logger logger = LoggerFactory.getLogger(TrusteeRunnable.class);

	private int _room_id;
	private int _operate_id;
	private int _seat_index;

	public TrusteeRunnable(int room_id, int operate_id, int seat_index) {
		super(room_id);
		_room_id = room_id;
		_operate_id = operate_id;
	...
	public void execute() {
		...
		try {
			...
			try {
				roomLock.lock();
				table.trustee_timer(_operate_id, _seat_index);
			} finally {
				roomLock.unlock();
