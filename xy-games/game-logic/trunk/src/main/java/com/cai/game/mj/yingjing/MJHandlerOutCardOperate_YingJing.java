 ...
 */
public class MJHandlerOutCardOperate_YingJing extends MJHandlerOutCardOperate<MJTable_YingJing> {
	...
	public void exe(MJTable_YingJing table) {
		...
		table.operate_player_cards(_out_card_player, hand_card_count, cards, 0, null,false);
		...
		if(table.lang_qi[_out_card_player]){
			table.lang_qi_out_count[_out_card_player]++;
		}
		int type = GameConstants.INVALID_SEAT;
		if (_type == GameConstants_YINGJING.HU_CARD_TYPE_GANG_PAO) {
			type = GameConstants_YINGJING.HU_CARD_TYPE_GANG_PAO;
		}
		...
		if (bAroseAction == false) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if(playerStatus.has_chi_hu()  && table._player_result.is_qing_hu(i)){
					// 见炮胡
					table.exe_jian_pao_hu(i, GameConstants.WIK_CHI_HU, _out_card_data);
				} else if (playerStatus.has_action()) {
	...
	public boolean handler_operate_card(MJTable_YingJing table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			...
			if (isFrist) {
				...
				if (_type != GameConstants_YINGJING.HU_CARD_TYPE_GANG_PAO) {
					// 添加放炮动画
					table.operate_effect_action(_out_card_player, GameConstants.EFFECT_ACTION_TYPE_HU, 1,
							new long[] { GameConstants_YINGJING.CHR_FANG_PAO }, 1, GameConstants.INVALID_SEAT);
				}
			}
		...
		case GameConstants.WIK_CHI_HU: {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				jie_pao_count++;
				//包赔结束标识
				if( table._playerStatus[_out_card_player]._hu_card_count > 0 && !table._player_result.is_qing_hu(_out_card_player)){
					if(table._player_result.is_qing_hu(i)){
						table.needBaoPei[_out_card_player][table.needBaoPeiCount[_out_card_player]++] = i;
					}
				}
			}
			...
			if (jie_pao_count > 0) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					// int next_player =
					// table.get_banker_next_seat(_out_card_player + i);
					...
					// table.process_chi_hu_player_operate(i, _out_card_data,
					// false);
					table.process_chi_hu_player_score(i, _out_card_player, _out_card_data, false);

					if (_type == GameConstants_YINGJING.HU_CARD_TYPE_GANG_PAO && jie_pao_count == 1) {
						// 杠上炮了，记录杠上跑的接炮人
	...
	public boolean handler_player_be_in_room(MJTable_YingJing table, int seat_index) {
		...
		// 出牌
		/*
		 * table.operate_out_card(_out_card_player, 0, new int[] { },
