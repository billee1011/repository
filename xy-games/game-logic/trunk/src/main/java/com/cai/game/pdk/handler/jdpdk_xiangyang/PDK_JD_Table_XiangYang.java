...
import protobuf.clazz.pdk_xiangyang.pdk_xiangyangPdkRsp.GameStart_PDK_XiangYang;
import protobuf.clazz.pdk_xiangyang.pdk_xiangyangPdkRsp.Open_Less_Response_XiangYang;
import protobuf.clazz.pdk_xiangyang.pdk_xiangyangPdkRsp.OutCardDataPdk_XiangYang;
import protobuf.clazz.pdk_xiangyang.pdk_xiangyangPdkRsp.PukeGameEndPdk_XiangYang;
import protobuf.clazz.pdk_xiangyang.pdk_xiangyangPdkRsp.RefreshCardsPdk_XiangYang;
import protobuf.clazz.pdk_xiangyang.pdk_xiangyangPdkRsp.TableResponse_PDK_XiangYang;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PDK_JD_Table_XiangYang extends PDKTable {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_out_card_operate = new PDKHandlerOutCardOperate_JD_XiangYang();
	...
	private boolean game_start_pkd() {
		...
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			...
			// 发送数据
			GameStart_PDK_XiangYang.Builder gamestart_pdk = GameStart_PDK_XiangYang.newBuilder();
	...
	private void test_cards() {
		...
		int cards[] = new int[] { 58, 37, 39, 20, 56, 61, 55, 23, 6, 26, 57, 42, 38, 40, 53, 41, 29, 3, 25, 9, 19, 24,
				27, 7, 8, 2, 51, 44, 4, 60, 13, 36, 43, 28, 1, 45, 59, 11, 54, 52, 22, 35, 21, 12, 5, 10, 33, 17 };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this.GRR._card_count[i] > 1) {
				if (i == this.GRR._banker_player) {
					...
					if (this._out_card_times[i] == 0 && this.has_rule(GameConstants.GAME_RULE_CHUN_TIAN)) {
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			...
			if (reason == GameConstants.Game_End_NORMAL) {
				...
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					...
					game_end_pdk.addPiaoFen(this._piao_fen[i]);
				}
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status)
			...
			if (this._handler != null && !_is_in_qie) {
				...
				TableResponse_PDK_XiangYang.Builder tableResponse_pdk = TableResponse_PDK_XiangYang.newBuilder();
	...
	public void Refresh_Open_Less(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		Open_Less_Response_XiangYang.Builder open_less = Open_Less_Response_XiangYang.newBuilder();
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player) {
		...
		//// 发送玩家出牌 201 =玩家出牌(CMD_OutCard: operate_player,operate_card)
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		OutCardDataPdk_XiangYang.Builder outcarddata = OutCardDataPdk_XiangYang.newBuilder();
	...
	public void load_player_info_data_game_start(GameStart_PDK_XiangYang.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public void load_player_info_data_reconnect(TableResponse_PDK_XiangYang.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public boolean operate_player_cards() {
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			...
			// 刷新玩家手牌数量
			RefreshCardsPdk_XiangYang.Builder refreshcards = RefreshCardsPdk_XiangYang.newBuilder();
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				if (this.has_rule(GameConstants.GAME_RULE_DISPLAY_CARD)) {
