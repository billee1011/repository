 ...
 *
 */
public class DispatchTouCardRunnable extends BaseFuture{
	
	private static Logger logger = LoggerFactory.getLogger(DispatchTouCardRunnable.class);
	...
	private int _type;
	public DispatchTouCardRunnable(int room_id,int seat_index,int type,boolean tail){
		super(room_id);
		_room_id = room_id;
	...
	public void execute() {
		try {
			...
			try{
				roomLock.lock();
				table.runnable_Dispatch_tou_card_data(_seat_index,_type, _tail);
			}finally{
				roomLock.unlock();
