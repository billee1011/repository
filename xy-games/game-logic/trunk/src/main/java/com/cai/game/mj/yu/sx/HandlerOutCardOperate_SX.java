...
public class HandlerOutCardOperate_SX extends MJHandlerOutCardOperate<Table_SX> {
	...
	public void exe(Table_SX table) {
		...
		for (int i = 0; i < GameConstants.MAX_COUNT; i++) {
			if (cards[i] == GameConstants_SX.HZ_MAGIC_CARD) {
				cards[i] += GameConstants_SX.CARD_ESPECIAL_TYPE_LAI_ZI;
			}
		}
		table.operate_player_cards(_out_card_player, hand_card_count, cards, 0, null);

		if (table._logic.is_magic_card(_out_card_data)) {
			table.operate_out_card(_out_card_player, 1, new int[] { _out_card_data + GameConstants_SX.CARD_ESPECIAL_TYPE_LAI_ZI },
	...
	@Override
	public boolean handler_operate_card(Table_SX table, int seat_index, int operate_code, int operate_card) {
		...
		if (table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_CHI_HU) && operate_code != GameConstants.WIK_CHI_HU) {
			...
			table._playerStatus[seat_index].chi_hu_round_invalid();
		} else if (operate_code == GameConstants.WIK_PENG && table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_GANG)) {
		...
		case GameConstants.WIK_CHI_HU: {
			...
			if (jie_pao_count > 0) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					if (_type == GameConstants_SX.OutCard_Type_Di_Hu && table.has_rule(GameConstants_SX.GAME_RULE_DIAN_PAO)) {
						table.GRR._chi_hu_rights[i].opr_or(GameConstants_SX.CHR_DI_HU);
					}
	...
	public boolean handler_player_be_in_room(Table_SX table, int seat_index) {
		...
		for (int i = 0; i < GameConstants.MAX_COUNT; i++) {
			if (table._logic.is_magic_card(hand_cards[i])) {
				tableResponse.addCardsData(hand_cards[i] + GameConstants_SX.CARD_ESPECIAL_TYPE_LAI_ZI);
			} else {
