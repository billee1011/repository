...
public class MJHandlerDispatchCard_PXZZ extends MJHandlerDispatchCard<MJTable_PXZZ> {
    ...
    public void exe(MJTable_PXZZ table) {
        ...
        if (table.GRR._left_card_count == llcard) {
            ...
            //红中金币场全部倒牌
            if(table.isCoinRoom()){
            	table.process_chi_hu_player_operate_all();
            }
            // 流局
        ...
        //WalkerGeek 中途改牌
       /* if (AbstractMJTable.DEBUG_CARDS_MODE || table.BACK_DEBUG_CARDS_MODE){
			table.testChangeCard();
			for(int k = 0; k < table.getTablePlayerNumber(); k++){
				int cards[] = new int[GameConstants.MAX_COUNT];
				// 刷新自己手牌
				int hand_card_count = table._logic.switch_to_cards_data(table.GRR._cards_index[k], cards);
				table.operate_player_cards(k, hand_card_count, cards, 0, null);
			}
        }*/
        ...
        boolean is_si_lai = table.GRR._cards_index[_seat_index][31] + (table._logic.is_magic_card(_send_card_data) ? 1 : 0) == 4;
        if (action != GameConstants.WIK_NULL || is_si_lai) {
            // 添加动作
     ...
     */
    @Override
    public boolean handler_operate_card(MJTable_PXZZ table, int seat_index, int operate_code, int operate_card) {
        ...
        case GameConstants.WIK_ZI_MO: // 自摸
        {
           ...
           /* if (table.GRR._cards_index[_seat_index][table._logic.get_magic_card_index(0)] == 0) {
            ...
            // 下局胡牌的是庄家
            table.set_niao_card_hz(_seat_index, GameConstants.INVALID_VALUE, true, add_niao, operate_card);// 结束后设置鸟牌
    ...
    @Override
    public boolean handler_be_set_trustee(MJTable_PXZZ table, int seat_index) {
        handler_check_auto_behaviour(table, seat_index, _send_card_data);
        return false;
    }
}
