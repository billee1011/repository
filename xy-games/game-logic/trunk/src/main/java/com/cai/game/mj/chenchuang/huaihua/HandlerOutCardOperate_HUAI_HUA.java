...
public class HandlerOutCardOperate_HUAI_HUA extends MJHandlerOutCardOperate<Table_HUAI_HUA> {
	...
	public void exe(Table_HUAI_HUA table) {
		...
		if(_out_card_data != Constants_HUAI_HUA.HONG_ZHONG_DATA)
        ...
        case GameConstants.WIK_CHI_HU: // 胡
        {
            ...
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                ...
                if(_type == Constants_HUAI_HUA.HU_CARD_TYPE_GANG_KAI_HUA)
            		table.GRR._chi_hu_rights[i].opr_or(Constants_HUAI_HUA.CHR_GANG_SHANG_PAO);
                jie_pao_count++;
            }

            if (jie_pao_count > 0) {
            	table.GRR._chi_hu_rights[_out_card_player].opr_or(Constants_HUAI_HUA.CHR_FANG_PAO);
            	if (jie_pao_count > 1) {
                    // 通炮
                    table._cur_banker = _out_card_player;
                    table.is_duo_xiang = true;
                    if(_type != Constants_HUAI_HUA.HU_CARD_TYPE_GANG_KAI_HUA)
                    	table.set_niao_card(seat_index, true);
