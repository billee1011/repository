...
public class HandlerDispatchCard_GXLZ extends MJHandlerDispatchCard<Table_GXLZ> {
	...
	public HandlerDispatchCard_GXLZ() {
	...
	public void exe(Table_GXLZ table) {
		...
		table.card_cannot_eat[_seat_index] = 0;
		...
		table.state_of_can_jie_pao[_seat_index] = false;

		int niao_count = table.get_niao_card_num();

		if (table.GRR._left_card_count == niao_count) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		if (_type == GameConstants.GANG_TYPE_JIE_GANG) { // TODO
															// 点杠杠开算杠上花，但是需要点杠人包给分。
		...
		int card_type = Constants_GXLZ.HU_CARD_TYPE_ZI_MO;
		if (_type == GameConstants.GANG_TYPE_JIE_GANG) { // TODO
															// 点杠杠开算杠上花，但是需要点杠人包给分。
			card_type = Constants_GXLZ.HU_CARD_TYPE_DIAN_GANG_GANG_KAI;
		} else if (_type == GameConstants.GANG_TYPE_AN_GANG || _type == GameConstants.GANG_TYPE_ADD_GANG) { // 暗杠碰杠按杠开计分
			card_type = Constants_GXLZ.HU_CARD_TYPE_GANG_KAI;
		}
		...
		if (action != GameConstants.WIK_NULL) {
			if (_type == GameConstants.GANG_TYPE_JIE_GANG) { // TODO
				...
				// curPlayerStatus.add_chi_hu(_send_card_data,
				// table._provide_player);
	...
	@Override
	public boolean handler_player_out_card(Table_GXLZ table, int seat_index, int card) {
	...
	public boolean handler_operate_card(Table_GXLZ table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_CHI_HU: {
			...
			table.set_niao_card_liu_zhou(_seat_index);
			table.GRR.liu_zhou_player_valid_niao_count[_seat_index] = table.get_pick_niao_count(_seat_index);
