...
public class MJHandlerGang_YiYang_QY extends MJHandlerGang<MJTable_YiYang_QY> {
	@Override
	...
	public boolean handler_operate_card(MJTable_YiYang_QY table, int seat_index, int operate_code, int operate_card) {
		...
		if (jie_pao_count > 0) {
			...
			// table.set_niao_card(_seat_index, GameConstants.INVALID_VALUE,
			// true, 0);// 抓鸟
			// table.set_niao_card_fei(_seat_index, GameConstants.INVALID_VALUE,
			// true, 0);// 飞鸟
			// }
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				// 0);// 抓鸟
				// table.set_niao_card_fei(i, GameConstants.INVALID_VALUE, true,
				// 0);// 飞鸟
				// }
	 ...
	 ***/
	@Override
	protected boolean exe_gang(MJTable_YiYang_QY table) {
