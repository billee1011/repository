...
public class DDZHandlerAddtimes_TXW extends DDZHandlerAddtimes<TXW_Table> {
	protected int _seat_index;
	protected int _game_status;
	// private int _current_player =MJGameConstants.INVALID_SEAT;

	public DDZHandlerAddtimes_TXW() {
	}
	...
	public boolean handler_call_banker(TXW_Table table, int seat_index, int addtimes) {
		if (table._add_times[seat_index] != -1) {
			if (!table.has_rule(DDZConstants.GAME_RULE_TXW_TI_TWO) && table._call_action[seat_index] == 1) {
				return false;
			}
			// if (!table.has_rule(DDZConstants.GAME_RULE_TXW_TI_TWO)
			// || table._add_times[table.GRR._banker_player] == -1) {
			// return false;
			// } else if (table._aggin_add_times[seat_index] != -1) {
			// return false;
			// }
		...
		if (addtimes > 3) {
			...
			if (seat_index == table.GRR._banker_player) {
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					} else if (i == table.GRR._banker_player) {
						table._user_times[i] *= 2;
						if (table._user_times[i] > table._boom_count_limit) {
							table._user_times[i] = table._boom_count_limit;
						}
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					if (table._add_times[prv_player] == 1 || table._call_action[prv_player] > 1) {
						break;
					}
					prv_player = (prv_player - 1 + table.getTablePlayerNumber()) % table.getTablePlayerNumber();
				}

				int prv_times = table._user_times[prv_player];
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					if (i != table.GRR._banker_player && table._add_times[i] == 1) {
						table._user_times[i] = 2 * prv_times;
						if (table._user_times[i] > table._boom_count_limit) {
							table._user_times[i] = table._boom_count_limit;
						}
					}
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i != table.GRR._banker_player) {
					if ((table._add_times[i] == 0) && table._add_times[table.GRR._banker_player] == -1) {
						table._is_out[i] = 1;
					} else if (table._call_action[i] == 1 && table._add_times[table.GRR._banker_player] == -1
							&& !table.has_rule(DDZConstants.GAME_RULE_TXW_TI_TWO)) {
						table._is_out[i] = 1;
					}
					if (table._is_out[i] == 0 && table._add_times[i] == 1) {
		...
		if (seat_index == table.GRR._banker_player) {
			if (table.has_rule(DDZConstants.GAME_RULE_TXW_DUI_TI) && table._add_times[seat_index] == 1) {
				int no_out_count = 0;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if (no_out_count == 2) {
					table._handler = table._handler_add_times_again;
					table._current_player = next_player;
					table._game_status = GameConstants.GS_DDZ_ADD_TIMES_AGAIN;// 设置状态
				} else {
		...
		} else if (seat_index == table.GRR._banker_player) {
			int cur_action[] = new int[2];
			cur_action[0] = 3;
			cur_action[1] = 5;
			table.call_banker_resopnse(2, seat_index, addtimes, table._current_player, cur_action,
