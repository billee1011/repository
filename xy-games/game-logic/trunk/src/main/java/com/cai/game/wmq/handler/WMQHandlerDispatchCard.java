package com.cai.game.wmq.handler;
...
public class WMQHandlerDispatchCard<T extends WMQTable> extends WMQHandler<T>{
...
//		}
	...
	public boolean handler_player_be_in_room(T table,int seat_index) {
		...
		{
			if(table.GRR._left_card_count == 22)
			{
				if(seat_index == _seat_index){
...
//		}
		...
		{
//			table.log_error("dispatch"+_seat_index +"cur" + seat_index+"card" +_send_card_data);
			table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data }, seat_index,false);
		}
