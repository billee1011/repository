...
public class MJHandlerGang_SanMenXia extends MJHandlerGang<MJTable_SanMenXia> {
	@Override
	public void exe(MJTable_SanMenXia table) {
		...
		if (table.has_rule(GameConstants.GAME_RULE_DIAN_PAO_HU_SMX)) {
	...
	@Override
	public boolean handler_operate_card(MJTable_SanMenXia table, int seat_index, int operate_code, int operate_card) {
	...
	@Override
	protected boolean exe_gang(MJTable_SanMenXia table) {
		...
		boolean gang_pao = table.has_rule(GameConstants.GAME_RULE_DAI_GANG_PAO_SMX);
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if(table.has_rule(GameConstants.GAME_RULE_AN_JIA_BEI_SMX)){
					score *= 2;
				}


				if (gang_pao) {
					int gangpaoscore = table._player_result.pao[i] + table._player_result.pao[_seat_index];
					table.GRR._score_of_gangpao[i] -= gangpaoscore;
					table.GRR._score_of_gangpao[_seat_index] += gangpaoscore;
				}
			...
			int score = GameConstants.CELL_SCORE*(table.getTablePlayerNumber() - 1);

			if (gang_pao) {
				int gangpaoscore = table._player_result.pao[_provide_player] + table._player_result.pao[_seat_index];
				table.GRR._score_of_gangpao[_provide_player] -= gangpaoscore;
				table.GRR._score_of_gangpao[_seat_index] += gangpaoscore;
			}
		...
		table.exe_gang_selectcard(_seat_index);
