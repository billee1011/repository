...
public class WuGangDealFirstCardRunnable extends BaseFuture {

	private static Logger logger = LoggerFactory.getLogger(WuGangDealFirstCardRunnable.class);

	private int _room_id;
	private int _seat_index;

	public WuGangDealFirstCardRunnable(int room_id, int seat_index) {
		super(room_id);
		_room_id = room_id;
	...
	public void execute() {
		try {
			Table_WuGang table = (Table_WuGang) PlayerServiceImpl.getInstance().getRoomMap().get(_room_id);
			if (table == null) {
