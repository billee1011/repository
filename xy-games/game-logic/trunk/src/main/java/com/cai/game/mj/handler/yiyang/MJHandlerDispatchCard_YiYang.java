...
public class MJHandlerDispatchCard_YiYang extends MJHandlerDispatchCard<MJTable_YiYang> {
	...
	public void exe(MJTable_YiYang table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			return;
		} else if (table.GRR._left_card_count <= table.getTablePlayerNumber()) {
			// 最后四张牌。看谁能自摸。不用打
		...
		if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants_YiYang.GAME_RULE_HUNAN_BAO_TING)
		...
		if (table.GRR._left_card_count > llcard && !table._playerStatus[_seat_index].is_bao_ting()) {
	...
	public boolean handler_operate_card(MJTable_YiYang table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			if(!table.is_mj_type(GameConstants.GAME_TYPE_MJ_YI_YANG_QIN_YOU))
				table.set_niao_card_fei(_seat_index, GameConstants.INVALID_VALUE, true, 0);// 飞鸟
