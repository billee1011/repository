 ...
 * 
 */
package com.cai.game.pdk.handler.fifteenpdk_yy;
...
import com.cai.game.pdk.PDKGameLogic_YY;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PDK_FIFTEENYY_Table extends PDKTable {

	public PDK_FIFTEENYY_Table() {
		_logic = new PDKGameLogic_YY();
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_out_card_operate = new PDKHandlerOutCardOperate_FIFTEENYY();
	...
	private boolean game_start_pkd() {
		...
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			...
			gamestart_pdk.setDisplayTime(this.getPlay_card_time());
	...
	private void test_cards() {
		int cards[] = new int[] { 0x03, 0x13, 0x23, 0x04, 0x14, 0x24, 0x25, 0x15, 0x05, 0x06, 0x28, 0x29, 0x2A, 0x2B,
				0x2C, 0x2D, 0x07, 0x17, 0x27, 0x08, 0x18, 0x28, 0x29, 0x19, 0x09, 0x06, 0x24, 0x25, 0x2A, 0x2B, 0x2C,
				0x2D };
		// int cards[] = new int[]
		// {0x0d,0x1d,0x2d,0x1c,0x2c,0x3b,0x1b,0x25,0x35,0x06,0x16,0x26,0x36,0x07,0x28};
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean handler_player_be_in_room(int seat_index) {
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status)
			...
			if (this._handler != null) {
				...
				tableResponse_pdk.setDisplayTime(this.getPlay_card_time());
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player) {
		...
		outcarddata.setDisplayTime(this.getPlay_card_time());
		if (_turn_out_card_count == 0) {
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		...
		// Enter_Back_Request req = PBUtil.toObject(room_rq,
		// Enter_Back_Request.class);
		// if (req.getIsBack() == 1) {
		// _enter_back[seat_index] = true;
		// this.get_players()[seat_index].setOnline(false);
		// this.operate_player_data();
		// } else {
		// _enter_back[seat_index] = false;
		// this.get_players()[seat_index].setOnline(true);
		// this.operate_player_data();
		// }
		//
		// if (_current_player == seat_index && !_enter_back[seat_index]) {
		// int can_out_card_data[] = new int[get_hand_card_count_max()];
		// if (_logic.Player_Can_out_card(GRR._cards_data[seat_index],
		...
		// _auto_out_card_scheduled = GameSchedule.put(new
		// PDKAutoOutCardRunnable(getRoom_id(),
		// seat_index, this, GameConstants.PDK_CT_PASS), 1, TimeUnit.SECONDS);
		// }
		// } else {
		// _auto_out_card_scheduled = GameSchedule.put(
		// new PDKAutoOutCardRunnable(getRoom_id(), seat_index, this,
		// GameConstants.PDK_CT_PASS),
		// 1, TimeUnit.SECONDS);
		// }
		// }
		// }
