 ...
 *
 */
public class HandCardUpdateRunnable extends BaseFuture{
	
	private static Logger logger = LoggerFactory.getLogger(HandCardUpdateRunnable.class);
	...
	private int _card;
	public HandCardUpdateRunnable(int room_id,int seat_index,int card,boolean tail){
		super(room_id);
		_room_id = room_id;
	...
	public void execute() {
		try {
			GXZPTable table = (GXZPTable)PlayerServiceImpl.getInstance().getRoomMap().get(_room_id);
			if(table==null){
			...
			try{
				roomLock.lock();
				table.hand_card_update_data(_seat_index,_card, _tail);
			}finally{
				roomLock.unlock();
