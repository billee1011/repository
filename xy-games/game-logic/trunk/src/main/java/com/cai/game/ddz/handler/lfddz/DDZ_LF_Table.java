...
import com.cai.game.ddz.DDZGameLogic_LF;
...
import protobuf.clazz.ddz_lf.ddz_lfRsp.CallBankerResult_DDZ_LF;
import protobuf.clazz.ddz_lf.ddz_lfRsp.GameStartDDZ_LF;
import protobuf.clazz.ddz_lf.ddz_lfRsp.Opreate_RequestDdz_LF;
import protobuf.clazz.ddz_lf.ddz_lfRsp.OutCardData_DDZ_LF;
import protobuf.clazz.ddz_lf.ddz_lfRsp.PlayerControl_DDZ_LF;
import protobuf.clazz.ddz_lf.ddz_lfRsp.PukeGameEnd_DDZ_LF;
import protobuf.clazz.ddz_lf.ddz_lfRsp.RefreshCards_DDZ_LF;
import protobuf.clazz.ddz_lf.ddz_lfRsp.Send_Card_DDZ_LF;
import protobuf.clazz.ddz_lf.ddz_lfRsp.Send_Last_Card_DDZ_LF;
import protobuf.clazz.ddz_lf.ddz_lfRsp.Send_SpecialCard_DDZ_LF;
import protobuf.clazz.ddz_lf.ddz_lfRsp.TableResponse_DDZ_LF;

///////////////////////////////////////////////////////////////////////////////////////////////
public class DDZ_LF_Table extends DDZTable {
	...
	protected static final int ID_TIMER_CALL_BANKER_TO_REDISTH = 3;// 叫庄到重新发牌
	protected static final int ID_TIMER_ADD_TIME_OUT_OPREATE = 4;// 加倍到出牌
	protected static final int ID_TIMER_AUTO_OUT_CARD = 5;// 自动出牌

	protected static final int GAME_OPREATE_TYPE_CALL_BANKER = 1;
	protected static final int GAME_OPREATE_TYPE_ADD_TIMES = 2;
	protected static final int GAME_OPREATE_TYPE_ADD_TIMES_BANKER = 3;
	DDZGameLogic_LF _logic;

	public DDZ_LF_Table() {

		_logic = new DDZGameLogic_LF();
		...
		_di_pai_card_count = GameConstants.JXDDZ_DI_PAI_COUNT_JD;
		_di_pai_card_data = new int[GameConstants.JXDDZ_DI_PAI_COUNT_JD];
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		// 初始化基础牌局handle
		_handler_out_card_operate = new DDZHandlerOutCardOperate_LF();
		_handler_call_banker = new DDZHandlerCallBanker_LF();
		_handler_add_times = new DDZHandlerAddtimes_LF();

		_handler_finish = new DDZHandlerFinish();
		this.setMinPlayerCount(3);

		if (this.has_rule(DDZConstants.GAME_RULE_LF_DDZ_BOOM_LIMIT_FIVE)) {
			_boom_count_limit = 5;
		} else if (this.has_rule(DDZConstants.GAME_RULE_LF_DDZ_BOOM_LIMIT_FOUR)) {
			_boom_count_limit = 4;
		} else {
			_boom_count_limit = 100;
		}
	}
	...
	protected boolean on_handler_game_start() {
		...
		_di_pai_card_count = DDZConstants.LFDDZ_DI_PAI_COUNT_JD;
		...
		_repertory_card = new int[DDZConstants.CARD_COUNT_DDZ_LF];
		shuffle(_repertory_card, DDZConstants.CARD_DATA_DDZ_LF);
	...
	public boolean game_start_ddzjd() {
		...
		roomResponse.setType(DDZMsgConstants.RESPONSE_DDZ_GAME_START_LF);
		GameStartDDZ_LF.Builder gameStartResponse = GameStartDDZ_LF.newBuilder();
		...
		Send_Card(0);
		Send_Special_Card(_fan_pai_data, 1, this._cur_banker);
		this.Send_di_Card(true, false, GameConstants.INVALID_SEAT);
	...
	public void Send_Card(int type) {
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			roomResponse.setType(DDZMsgConstants.RESPONSE_DDZ_SEND_CARD);
			Send_Card_DDZ_LF.Builder send_card = Send_Card_DDZ_LF.newBuilder();
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public void Send_Special_Card(int card_data, int count, int seat_index) {

		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(DDZMsgConstants.RESPONSE_DDZ_SEND_SPECIAL_CARD);
		Send_SpecialCard_DDZ_LF.Builder send_card = Send_SpecialCard_DDZ_LF.newBuilder();
		send_card.setActionChair(seat_index);
		send_card.setCardCount(count);
		send_card.addCardsData(card_data);
		send_card.setActionType(1);
		roomResponse.setCommResponse(PBUtil.toByteString(send_card));
		this.send_response_to_room(roomResponse);
		GRR.add_room_response(roomResponse);
	}

	public void Send_di_Card(boolean is_animation, boolean is_open, int to_player) {

		if (to_player == GameConstants.INVALID_SEAT) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			roomResponse.setType(DDZMsgConstants.RESPONSE_DDZ_SEND_DI_CARD);
			Send_Last_Card_DDZ_LF.Builder send_card = Send_Last_Card_DDZ_LF.newBuilder();
			for (int i = 0; i < this._di_pai_card_count; i++) {
				if (is_open) {
					send_card.addCardsData(this._di_pai_card_data[i]);
				} else {
					send_card.addCardsData(GameConstants.BLACK_CARD);
				}

			}
			send_card.setCardCount(_di_pai_card_count);
			send_card.setShowDipaiAction(is_animation);
			send_card.setCardsTimes(1);
			send_card.setGameTimes(1);
			send_card.setCardsType(DDZConstants.DDZ_LF_CT_ERROR);
			roomResponse.setCommResponse(PBUtil.toByteString(send_card));
			GRR.add_room_response(roomResponse);
		} else {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			...
			send_card.setShowDipaiAction(is_animation);
			send_card.setCardsTimes(1);
			send_card.setGameTimes(_user_times[to_player]);
			send_card.setCardsType(DDZConstants.DDZ_LF_CT_ERROR);
			roomResponse.setCommResponse(PBUtil.toByteString(send_card));
	...
	public int get_hand_card_count_max() {
		return DDZConstants.DDZ_MAX_COUNT_LF + DDZConstants.LFDDZ_DI_PAI_COUNT_JD;
	}

	public int get_no_banker_card_count() {
		return DDZConstants.DDZ_MAX_COUNT_LF;
	}
	...
	private void shuffle(int repertory_card[], int card_cards[]) {
		...
		_fan_pai_data = repertory_card[(int) (RandomUtil.getRandomNumber(Integer.MAX_VALUE)
				% (DDZConstants.CARD_COUNT_DDZ_LF - DDZConstants.LFDDZ_DI_PAI_COUNT_JD))];
		...
		for (int i = 0; i < count; i++) {
			for (int j = 0; j < get_no_banker_card_count(); j++) {
				...
				if (GRR._cards_data[i][j] == _fan_pai_data) {
					this._cur_banker = i;
				}
			}
	...
	private void test_cards() {
		int cards[] = new int[] { 0x03, 0x13, 0x23, 0x04, 0x14, 0x24, 0x05, 0x15, 0x25, 0x06, 0x16, 0x26, 0x3a, 0x3b,
				0x3c, 0x3d, 0x31, 0x07, 0x17, 0x27, 0x08, 0x18, 0x28, 0x09, 0x19, 0x29, 0x0a, 0x1a, 0x2a, 0x35, 0x36,
				0x37, 0x38, 0x39, 0x0b, 0x1b, 0x2b, 0x0c, 0x1c, 0x2c, 0x0d, 0x1d, 0x2d, 0x01, 0x11, 0x21, 0x12, 0x22,
				0x33, 0x34, 0x4E, 0x4F };
		...
		int di_cards[] = new int[] { 0x33, 0x34, 0x4E, 0x4F };
		for (int i = 0; i < _di_pai_card_count; i++) {
	...
	public void load_player_info_data_game_start(GameStartDDZ_LF.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public void load_player_info_data_game_end(PukeGameEnd_DDZ_LF.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_reconnect(TableResponse_DDZ_LF.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public boolean handler_game_finish_ddz(int seat_index, int reason) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(DDZMsgConstants.RESPONSE_DDZ_GAME_END);
		GameEndResponse.Builder game_end = GameEndResponse.newBuilder();
		PukeGameEnd_DDZ_LF.Builder game_end_ddz = PukeGameEnd_DDZ_LF.newBuilder();
		game_end_ddz.setRoomInfo(this.getRoomInfo());
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			game_end_ddz.addTimes(_user_times[i]);
			game_end_ddz.addCellScore((int) this.game_cell);
			if (chuntian[i] != -1) {
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		if (type == MsgConstants.REQUST_DDZLF_OPERATE) {
			Opreate_RequestDdz_LF req = PBUtil.toObject(room_rq, Opreate_RequestDdz_LF.class);
			// 逻辑处理
			return handler_requst_opreate(seat_index, req.getOpreateType(), req.getAction());
		}

		return true;
	}

	public boolean handler_requst_opreate(int seat_index, int opreate_type, int cation) {
		switch (opreate_type) {
		case GAME_OPREATE_TYPE_CALL_BANKER: {
			if (this._handler_call_banker != null) {
				this._handler_call_banker.handler_call_banker(this, seat_index, cation, -1);
			}
			break;
		}
		case GAME_OPREATE_TYPE_ADD_TIMES: {
			if (this._handler_add_times != null) {
				this._handler_add_times.handler_call_banker(this, seat_index, cation);
			}
			break;
		}
		case GAME_OPREATE_TYPE_ADD_TIMES_BANKER: {
			if (this._handler_add_times != null) {
				this._handler_add_times.handler_call_banker(this, seat_index, cation);
			}
			break;
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player) {
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			OutCardData_DDZ_LF.Builder outcarddata = OutCardData_DDZ_LF.newBuilder();
			Int32ArrayResponse.Builder cards = Int32ArrayResponse.newBuilder();
			roomResponse.setType(DDZMsgConstants.RESPONSE_DDZ_OUT_CARD);// 201
			...
			// 刷新玩家手牌数量
			outcarddata.setUserCardCount(GRR._card_count[index]);
			for (int j = 0; j < GRR._card_count[index]; j++) {
				outcarddata.addUserCardsData(GRR._cards_data[index][j]);
			}
	...
	public void send_callbaner_result() {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(DDZMsgConstants.RESPONSE_DDZ_CALL_BANKER_RESULT);
		CallBankerResult_DDZ_LF.Builder call_banker_ddz = CallBankerResult_DDZ_LF.newBuilder();
	...
	public void call_banker_resopnse(int type, int prv_type, int call_player, int call_action, int cur_player,
			int cur_action[], int to_player) {
		// type 1叫地主 2为题 3为回提
		if (to_player == GameConstants.INVALID_SEAT) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			roomResponse.setType(DDZMsgConstants.RESPONSE_DDZ_CALL_BANKER_RESPONSE);
			PlayerControl_DDZ_LF.Builder call_banker_ddz = PlayerControl_DDZ_LF.newBuilder();
			call_banker_ddz.setType(type);
			...
			if (type == 1) {
				...
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					call_banker_ddz.addAllCallAction(this._add_times[i]);
				}
			}
	...
	public void refresh_card_data(int type) {
		// 0正常刷牌 1：发底牌刷牌
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			roomResponse.setType(DDZMsgConstants.RESPONSE_DDZ_REFRESH_PLAYER_CARDS);
			RefreshCards_DDZ_LF.Builder refresh_card_data = RefreshCards_DDZ_LF.newBuilder();
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
				if (i == play_index) {
					for (int j = 0; j < this.GRR._card_count[i]; j++) {
						if (type == 1 && i == GRR._banker_player) {
							boolean is_di_pai = false;
							for (int x = 0; x < _di_pai_card_count; x++) {
								if (_di_pai_card_data[x] == GRR._cards_data[i][j]) {
									is_di_pai = true;
									break;
								}
							}
				...
				refresh_card_data.addHandCardCount(this.GRR._card_count[i]);
				refresh_card_data.addHandCardsData(cards_card);
			}
			roomResponse.setCommResponse(PBUtil.toByteString(refresh_card_data));
			this.send_response_to_player(play_index, roomResponse);
		}
	...
	protected void timerCallBack(SheduleArgs args) {
		...
		case ID_TIMER_CALL_BANKER_TO_REDISTH: {
			exe_dispath();
			return;
		}
		...
		case ID_TIMER_AUTO_OUT_CARD: {
			...
			auto_out_card(seat_index, card_data, card_count);
			return;
		}
		}
		return;
