...
public class WSKHandlerOutCardOperate_DY extends WSKHandlerOutCardOperate<WSKTable_DY> {

	@Override
	public void exe(WSKTable_DY table) {
		...
		_out_type = card_type;
		if (table.has_rule(GameConstants.GAME_RULE_HSDY_RECORD_OUT_CARD)) {
			table._logic.RemoveCard(_out_cards_data, _out_card_count, table._repertory_card, table._all_card_count);
		}

		table._all_card_count -= _out_card_count;
		// 桌面牌分
		...
		table._cur_out_car_type[_out_card_player] = _out_card_count;
		...
		table.Refresh_Record_Out_Card(GameConstants.INVALID_SEAT);
		table.Refresh_user_get_score(GameConstants.INVALID_SEAT);
	}

	public void table_pai_socre(WSKTable_DY table) {
		int pai_score = 0;
	...
	public void user_pass_card(WSKTable_DY table) {
		if (table._turn_out_card_count == 0) {
			return;
		}
		table._cur_out_car_type[_out_card_player] = GameConstants.HSDY_CT_PASS;
		// 清空接下去出牌玩家出牌数据
		int next_player = (_out_card_player + 1) % table.getTablePlayerNumber();
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		if (next_player == table._out_card_player) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				table._cur_out_car_type[i] = GameConstants.HSDY_CT_ERROR;
			...
			if (table.GRR._card_count[next_player] == 0) {
				...
				table._cur_out_car_type[table._current_player] = GameConstants.HSDY_CT_ERROR;
	...
	public int adjust_out_card_right(WSKTable_DY table) {

		table._logic.make_change_card(_out_change_cards_data, _out_card_count, _out_cards_data, _out_type);
		table._logic.SortCardList(_out_change_cards_data, _out_card_count, GameConstants.WSK_ST_VALUE);
	...
	@Override
	public boolean handler_player_be_in_room(WSKTable_DY table, int seat_index) {
		...
		// if (!table._is_yi_da_san && table._out_card_ming_ji !=
		// GameConstants.INVALID_CARD) {
		// if (table.GRR._card_count[seat_index] == 0) {
		// Int32ArrayResponse.Builder cards = Int32ArrayResponse.newBuilder();
		// tableResponse.setCardsData(table._friend_seat[seat_index], cards);
		// for (int j = 0; j <
		// table.GRR._card_count[table._friend_seat[seat_index]]; j++) {
		// cards.addItem(table.GRR._cards_data[table._friend_seat[seat_index]][j]);
		// }
		// tableResponse.setCardsData(seat_index, cards);
		...
		if (table._turn_out_card_count == 0 || table._logic.search_out_card(table.GRR._cards_data[seat_index],
				table.GRR._card_count[seat_index], table._turn_out_card_data, table._turn_out_card_count) > 0) {
			for (int j = 0; j < table.GRR._card_count[seat_index]; j++) {
				tableResponse.addUserCanOutData(table.GRR._cards_data[seat_index][j]);
			}
			tableResponse.setUserCanOutCount(table.GRR._card_count[seat_index]);
		} else {
			tableResponse.setUserCanOutCount(0);
		}
		if (table._out_card_ming_ji == GameConstants.INVALID_CARD) {
