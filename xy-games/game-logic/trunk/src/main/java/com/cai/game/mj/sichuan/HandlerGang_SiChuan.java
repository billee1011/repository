...
public class HandlerGang_SiChuan extends MJHandlerGang<AbstractSiChuanMjTable> {
	@Override
	public void exe(AbstractSiChuanMjTable table) {
		...
		if (table.is_mj_type(GameConstants.GAME_TYPE_GUANG_AN)) {
			tmp_type = GameConstants.WIK_GANG;
		} else if (table.is_mj_type(GameConstants.GAME_TYPE_LE_SHAN_YAO_JI)) {
			if (table.has_rule(Constants_SiChuan.GAME_RULE_JI_SHI_YU)) {
				if (GameConstants.GANG_TYPE_ADD_GANG == _type) {
					...
					if (table.has_rule(Constants_SiChuan.GAME_RULE_JI_SHI_YU)) {
						if ((cbWeaveKind == GameConstants.WIK_PENG && _action == GameConstants.WIK_SUO_GANG_1)
								|| (cbWeaveKind == GameConstants.WIK_SUO_PENG_1 && _action == GameConstants.WIK_SUO_GANG_2)
								|| (cbWeaveKind == GameConstants.WIK_SUO_PENG_2 && _action == GameConstants.WIK_SUO_GANG_3)) {
						} else if (_center_card != table._send_card_data && table._send_card_data != table.magicCard) {
							tmp_type = GameConstants.WIK_GANG;
						}
					}
				}
	...
	protected boolean exe_gang(AbstractSiChuanMjTable table) {
		...
		if (table.is_mj_type(GameConstants.GAME_TYPE_LE_SHAN_YAO_JI) && table.GRR._weave_items[_seat_index][cbWeaveIndex].is_vavild) {
			if (_type == GameConstants.GANG_TYPE_AN_GANG) {
				int[] row = new int[table.SCORE_DETAIL_COLUMN];
				if (_action == GameConstants.WIK_GANG) {
					row[0] = ScoreRowType.AN_GANG_WU_JI.getType();
				} else {
					row[0] = ScoreRowType.AN_GANG_RUAN_GANG.getType();
				}

				for (int o_player = 0; o_player < table.getTablePlayerNumber(); o_player++) {
					if (_seat_index == o_player)
						continue;

					if (table.GRR._weave_items[_seat_index][cbWeaveIndex].gang_gei_fen_valid[o_player] == false)
						continue;

					// 有幺鸡2分 无幺鸡4分
					int score = 2 * GameConstants.CELL_SCORE;
					if (_action == GameConstants.WIK_GANG && table.hasRuleWuJiGangDouble)
						score *= 2;

					row[o_player + 1] -= score;
			...
			if (_type == GameConstants.GANG_TYPE_JIE_GANG) {
				int[] row = new int[table.SCORE_DETAIL_COLUMN];
				if (_action == GameConstants.WIK_GANG) {
					row[0] = ScoreRowType.MING_GANG_WU_JI.getType();
				} else {
					row[0] = ScoreRowType.MING_GANG_RUAN_GANG.getType();
				}
			...
			if (_type == GameConstants.GANG_TYPE_ADD_GANG) {
				int[] row = new int[table.SCORE_DETAIL_COLUMN];
				if (_action == GameConstants.WIK_GANG) {
					row[0] = ScoreRowType.BA_GANG_WU_JI.getType();
				} else {
					row[0] = ScoreRowType.BA_GANG_RUAN_GANG.getType();
				}
					...
					if (jie_pao_count == 3) {
						...
						if (table.left_player_count + jie_pao_count == table.getTablePlayerNumber()) {
							for (int i = 0; i < table.getTablePlayerNumber(); i++) {
								int next_player = table.get_banker_next_seat(_seat_index + i);
								if (table.GRR._chi_hu_rights[next_player].is_valid() && !table.had_hu_pai[next_player]) {
									table.next_banker_player = next_player;
									break;
								}
							}
						}
