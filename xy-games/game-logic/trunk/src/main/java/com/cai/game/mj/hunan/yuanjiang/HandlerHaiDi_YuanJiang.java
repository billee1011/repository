...
public class HandlerHaiDi_YuanJiang extends MJHandlerDispatchCard<Table_YuanJiang> {
	...
	public void exe(Table_YuanJiang table) {
		...
		table.has_dispatch_hai_di[_seat_index] = true;
		table.hai_di_card[_seat_index] = _send_card_data;
		...
		if (0 != cbActionMask) {
			for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
				if ((m_gangCardResult.isPublic[i] == 0)
						|| (m_gangCardResult.type[i] == GameConstants.GANG_TYPE_ADD_GANG && m_gangCardResult.cbCardData[i] == _send_card_data)) {
	...
	public boolean handler_operate_card(Table_YuanJiang table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: {
			table.has_hai_di_zi_mo[seat_index] = true;
	...
	public boolean handler_player_be_in_room(Table_YuanJiang table, int seat_index) {
		...
		if (table.has_dispatch_hai_di[seat_index] && !table.has_hai_di_gang[seat_index]) {
			table.operate_player_get_card(seat_index, 1, new int[] { table.hai_di_card[seat_index] }, seat_index);
		}
