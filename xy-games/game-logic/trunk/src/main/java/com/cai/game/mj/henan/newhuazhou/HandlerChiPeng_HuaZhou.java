...
public class HandlerChiPeng_HuaZhou extends MJHandlerChiPeng<Table_HuaZhou> {
	protected GangCardResult m_gangCardResult;

	public HandlerChiPeng_HuaZhou() {
	...
	public void exe(Table_HuaZhou table) {
		...
		// 清理晒张
		if (table.is_shai_zhang_card(_seat_index, _card)) {
			table.have_shaizhang[_seat_index] = false;
		}
...
//		if (table.is_shai_zhang_card(_seat_index, _card)) {
//			table.GRR._weave_items[_seat_index][wIndex].center_card = _card + GameConstants.CARD_ESPECIAL_TYPE_LAI_GEN;
//		}
		...
		if (ting_count > 0) {
			...
			// 刷新晒张
			table.refresh_shai_zhang_card(_seat_index, tmp_hand_card_count, tmp_cards);

			for (int i = 0; i < tmp_hand_card_count; i++) {
				for (int j = 0; j < ting_count; j++) {
					if (tmp_cards[i] == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
						if (tmp_cards[i] < GameConstants.CARD_ESPECIAL_TYPE_LAI_GEN) {
