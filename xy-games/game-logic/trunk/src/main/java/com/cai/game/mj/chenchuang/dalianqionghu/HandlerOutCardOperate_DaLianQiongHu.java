...
public class HandlerOutCardOperate_DaLianQiongHu extends MJHandlerOutCardOperate<Table_DaLianQiongHu> {
	...
	public void exe(Table_DaLianQiongHu table) {
		...
		int data = table._logic.is_magic_card(_out_card_data)?_out_card_data+GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI:_out_card_data;
		table.operate_out_card(_out_card_player, 1, new int[] { data }, GameConstants.OUT_CARD_TYPE_MID,
		...
		//////报听看宝
		if(_type == Constants_MJ_DA_LIAN_QH.HU_CARD_TYPE_BAO_TING && !table.has_rule(Constants_MJ_DA_LIAN_QH.GAME_RULE_AFTER_OUT_BAO)){
			if(table.is_chong_bao(_out_card_player))
				return;
		}
	...
	private void handler_operate_bu_tong_pao(Table_DaLianQiongHu table, int seat_index,
		...
		case GameConstants.WIK_CHI_HU: {
			...
			table.GRR._chi_hu_rights[_out_card_player].opr_or(Constants_MJ_DA_LIAN_QH.CHR_FANG_PAO);
		...
		case GameConstants.WIK_ZI_MO: {
			...
			table.GRR._win_order[_out_card_player] = 1;
			table.process_chi_hu_player_operate(seat_index, operate_card, false);
			...
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), _out_card_player, GameConstants.Game_End_NORMAL), GameConstants.GAME_FINISH_DELAY,
					TimeUnit.SECONDS);
