...
public class HandlerGang_Yong_Zhou extends MJHandlerGang<Table_Yong_Zhou> {
	...
	@Override
	public void exe(Table_Yong_Zhou table) {
	...
	@Override
	public boolean exe_gang(Table_Yong_Zhou table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			int score = table.has_rule(Constants_MJ_YONG_ZHOU.GAME_RULE_NO_OPEN) ? 1 : 2;
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
				table.GRR._gang_score[_seat_index].scores[cbGangIndex][i] -= score;// 其他玩家扣分
			...
			int score = table.has_rule(Constants_MJ_YONG_ZHOU.GAME_RULE_NO_OPEN) ? 1 : 3;
			
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_provide_player] -= score;// 其他玩家扣分
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += score;// 一共加分
		...
		// 从后面发一张牌给玩家
		table.exe_dispatch_card(_seat_index, Constants_MJ_YONG_ZHOU.HU_CARD_TYPE_GSKH, 0);

		return true;
	}

	@Override
	public boolean handler_operate_card(Table_Yong_Zhou table, int seat_index, int operate_code, int operate_card) {
	...
	private void handler_operate_tong_pao(Table_Yong_Zhou table, int seat_index,
        ...
        if (jie_pao_count > 0) {
            ...
            table.GRR._chi_hu_rights[_seat_index].opr_or(Constants_MJ_YONG_ZHOU.CHR_BEI_QIANG_GANG);
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if ((table.GRR._chi_hu_rights[i].is_valid() == false)) {
                    continue;
                }
	...
	private void handler_operate_bu_tong_pao(Table_Yong_Zhou table, int seat_index,
			int operate_code, int operate_card) {
		int target_player = seat_index;
	...
	@Override
	public boolean handler_player_be_in_room(Table_Yong_Zhou table, int seat_index) {
