...
public class MJHandlerDispatchCard_TT extends MJHandlerDispatchCard<MJTable_TT> {
	...
	public void exe(MJTable_TT table) {
		...
		// 荒庄结束
		if (table.GRR._left_card_count == table.liuPiaCount) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		if (table.GRR._left_card_count > llcard) {
			int cbActionMask = table._logic.analyse_gang_by_card_ll(table.GRR._cards_index[_seat_index], _send_card_data,
	...
	public boolean handler_operate_card(MJTable_TT table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			int add_niao = table.getMoreZhuaNiao(_seat_index);
