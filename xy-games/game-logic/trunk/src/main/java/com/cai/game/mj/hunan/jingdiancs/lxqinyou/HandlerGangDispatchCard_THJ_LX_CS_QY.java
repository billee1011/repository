...
public class HandlerGangDispatchCard_THJ_LX_CS_QY extends AbstractMJHandler<Table_LX_CS_QY> {
	...
	public HandlerGangDispatchCard_THJ_LX_CS_QY() {
			...
			for (int k = 0; k < table.getTablePlayerNumber(); k++) {
				if (k == _seat_index) {
					...
					int action = table.analyse_chi_hu_card(table.GRR._cards_index[k], table.GRR._weave_items[k], table.GRR._weave_count[k], bu_card,
							chr, Constants_LX_CS_QY.HU_CARD_TYPE_GANG_KAI, k);
	...
	public boolean handler_operate_card(Table_LX_CS_QY table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: {
			...
			if (table.GRR._chi_hu_rights[_seat_index].is_mul_long(Constants_LX_CS_QY.CHR_ZI_MO)) {
		...
		case GameConstants.WIK_CHI_HU: {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if (table.GRR._chi_hu_rights[i].is_mul_long(Constants_LX_CS_QY.CHR_JIE_PAO)) {
	...
	private void exe_chi_peng(Table_LX_CS_QY table, int target_player, int target_action, int target_card) {
		table.operate_out_card(this._seat_index, 0, null, GameConstants.OUT_CARD_TYPE_LEFT, GameConstants.INVALID_SEAT);
