 ...
 *
 */
public class MJHandlerDispatchCard_CS extends MJHandlerDispatchCard<MJTable> {
	...
	public void exe(MJTable table) {
		...
		// 海底
		if ((table.is_mj_type(GameConstants.GAME_TYPE_CS) || table.is_mj_type(GameConstants.GAME_TYPE_HALL_CHANG_SHA_MJ)
				|| table.is_mj_type(GameConstants.GAME_TYPE_THJ_CS)) && table.GRR._left_card_count == 1) {
				...
				for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
					...
					if (table.GRR._left_card_count > 2) {
						...
						if (is_ting == true) {
							...
							can_gang = table.is_cs_ting_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
