...
public class MJHandlerDispatchCard_KLDS extends MJHandlerDispatchCard<MJTable_KLDS> {
    ...
    public void exe(MJTable_KLDS table) {
        ...
        int can_strat = 0;
        for (int i = 0; i < table.getTablePlayerNumber(); i++) {
            if (table.GRR._discard_count[i] > 0) {
            	can_strat ++;
            }
        }

        if (!table.ding_sheng && table._cur_banker == _seat_index && can_strat == table.getTablePlayerNumber()) {
            boolean flag = false; // 不能定神
            for (int j = 0; j < table.getTablePlayerNumber(); j++) {
            	if(table.GRR._discard_count[j] == 0){
            		break;
            	}
                int discard = table.GRR._discard_cards[j][table.GRR._discard_count[j] - 1];
                if (discard == GameConstants.INVALID_SEAT) {
                    continue;
                }

                // 有一个不等于中发白
                if (table._logic.switch_to_card_index(discard) < table._logic.switch_to_card_index(0x35)) {
                    flag = true;
                }
            }
            if (flag) {
                table.exe_ding_sheng(_seat_index, 0, table);
                return;
            }
        }
        ...
        // 报听强制胡牌
        if (action != GameConstants.WIK_NULL && table._playerStatus[_seat_index].is_bao_ting()) {
        ...
        // 加到手牌
        table.GRR._cards_index[_seat_index][send_card_index]++;
        if (!table._playerStatus[_seat_index].is_bao_ting() && table._logic.get_magic_card_count() > 0
                && table.GRR._cards_index[_seat_index][table._logic.get_magic_card_index(0)] >= 2) {
            // 减去神牌
            table.GRR._cards_index[_seat_index][table._logic.get_magic_card_index(0)] -= 2;
            ...
            for (int i = 0; i < GameConstants.MAX_INDEX - GameConstants.CARD_HUA_COUNT; i++) {
                ...
                if (count > 0) {
                    ...
                    // 检查打出哪一张牌后可以听牌
                    table._playerStatus[_seat_index]._hu_out_card_ting_count[ting_count] = table.get_bao_fei_card(
            ...
            if (ting_count > 0) {
                ...
                for (int i = 0; i < hand_card_count; i++) {
                    for (int j = 0; j < ting_count; j++) {
                            if (cards[i] == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
                            	if (table._logic.is_magic_card(cards[i])) {
                            		cards[i] += GameConstants.CARD_ESPECIAL_TYPE_SHENGPAI_TING;
                            	} else {
                    ...
                    if (table._logic.is_magic_card(cards[i])) {
                    	cards[i] += GameConstants.CARD_ESPECIAL_TYPE_SHENGPAI;
                    }
                }
        ...
        if(!can_bao_ting){
            ...
            table.GRR._cards_index[_seat_index][send_card_index]--;  //减掉
            //刷新手牌
            if (ting_count > 0) {
        ...
        if(table._playerStatus[_seat_index]._hu_out_card_count > 0){
        	for (int j = 0; j < table._playerStatus[_seat_index]._hu_out_card_count; j++) {
        		if (_send_card_data == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
        			if (table._logic.is_magic_card(_send_card_data)) {
        				real_card += GameConstants.CARD_ESPECIAL_TYPE_SHENGPAI_TING;
        			}else{
    	...
    	if (table._logic.is_magic_card(real_card)) {
    		real_card += GameConstants.CARD_ESPECIAL_TYPE_SHENGPAI;
    	}
    ...
    public boolean handler_operate_card(MJTable_KLDS table, int seat_index, int operate_code, int operate_card) {
        ...
        case GameConstants.WIK_BAO_TING: // 报听
        {
            ...
            table.exe_out_card_bao_ting(_seat_index, operate_card,
                    _type == GameConstants.DispatchCard_Type_Tian_Hu ? GameConstants.OutCard_Type_Di_Hu
    ...
    public boolean handler_player_be_in_room(MJTable_KLDS table, int seat_index) {
                ...
                for (int k = 0; k < out_ting_count; k++) {
                    if (hand_cards[j] == table._playerStatus[seat_index]._hu_out_card_ting[k]) {
                        if(table._logic.is_magic_card(hand_cards[j])){
                        	 hand_cards[j] += GameConstants.CARD_ESPECIAL_TYPE_SHENGPAI_TING;
                        }else{
    	...
    	for (int j = 0; j < hand_card_count; j++) {
            if (table._logic.is_magic_card(hand_cards[j])) {
                hand_cards[j] += GameConstants.CARD_ESPECIAL_TYPE_SHENGPAI;
            }
        }
        ...
        for (int i = 0; i < out_ting_count; i++) {
            ...
            if(table._logic.is_magic_card(table._playerStatus[seat_index]._hu_out_card_ting[i])){
            	roomResponse.addOutCardTing(
            			table._playerStatus[seat_index]._hu_out_card_ting[i] + GameConstants.CARD_ESPECIAL_TYPE_SHENGPAI_TING);
            }else{
            	roomResponse.addOutCardTing(
