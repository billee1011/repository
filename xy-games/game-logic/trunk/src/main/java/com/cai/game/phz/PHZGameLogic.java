...
class can_chi_index {
	public int _one_index;
	public int _two_index;
	public boolean _is_pass;

	public can_chi_index(int one_index, int two_index) {
		this._one_index = one_index;
		this._two_index = two_index;
		_is_pass = true;
	}

	public void set_is_pass(boolean is_pass) {
		_is_pass = is_pass;
	}
}
...
public class PHZGameLogic {

	private static Logger logger = Logger.getLogger(PHZGameLogic.class);
	...
	public int check_chi(int cards_index[], int cur_card, int type_count[], int type_eat_count[], List<can_chi_index> can_not_chi) {
		...
		if ((card_value == 2) || (card_value == 7) || (card_value == 10)) {
			...
			if (!is_can_not_chi) {
				...
				for (; i < excursion.length; i++) {
					int index = acceptIndex + excursion[i];
					if ((index != cur_card_index) && ((cards_index[index] == 0)))
						break;
				}
				if (i == excursion.length) {
	...
	public int check_liu_wai_yywhz(int card_index[], WeaveItem WeaveItem[], int cbWeaveCount, int cur_card, GangCardResult gangCardResult,
			boolean is_dispath) {
		int cbActionMask = GameConstants.WIK_NULL;
		// 手上杠牌
	...
	public int check_qing_wai_yiyangwhz(int card_index[], WeaveItem WeaveItem[], int cbWeaveCount, int cur_card, GangCardResult gangCardResult,
			boolean is_dispath) {
		int cbActionMask = GameConstants.WIK_NULL;
		// 手上杠牌
		for (int i = 0; i < cbWeaveCount; i++) {
			...
			if ((card_value == 2) || (card_value == 7) || (card_value == 10)) {
				...
				if (can_not_chi != null) {
					for (int i = 0; i < can_not_chi.size(); i++) {
						can_chi_index object = can_not_chi.get(i);
						int number = 0;
						if ((object._one_index == one_index || object._two_index == two_index)) {
							number++;
						}
						if (number == 2) {
							return true;
	...
	public int get_weave_hu_xi_yjghz(WeaveItem weave_item) {
		switch (weave_item.weave_kind) {
		case GameConstants_YJGHZ.WIK_QING_WAI:
		case GameConstants_YJGHZ.WIK_QING_NEI:
		case GameConstants_YJGHZ.WIK_PIAO:
		case GameConstants_YJGHZ.WIK_WAI:
		case GameConstants_YJGHZ.WIK_KAN:
		case GameConstants.WIK_PENG:
		case GameConstants.WIK_EQS:
			return 1;
		}
	...
	public int calculate_all_pai_count_yjghz(AnalyseItem analyseItem) {
		int count = 0;
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			...
			case GameConstants_YJGHZ.WIK_WAI:
			case GameConstants_YJGHZ.WIK_KAN:
			case GameConstants_YJGHZ.WIK_SHUNZI:
			case GameConstants.WIK_PENG:
	...
	public int calculate_hei_dui_zi_pai_count_yjghz(AnalyseItem analyseItem) {
		int count = 0;
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			switch (analyseItem.cbWeaveKind[i]) {
			case GameConstants_YJGHZ.WIK_QING_WAI:
	...
	public int calculate_hei_pai_count_yjghz(AnalyseItem analyseItem) {
		int count = 0;
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			switch (analyseItem.cbWeaveKind[i]) {
			case GameConstants_YJGHZ.WIK_QING_WAI:
	...
	public int calculate_dui_zi_pai_count_yjghz(AnalyseItem analyseItem) {
		int count = 0;
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			switch (analyseItem.cbWeaveKind[i]) {
			case GameConstants_YJGHZ.WIK_QING_WAI:
	...
	public int calculate_hong_pai_count_yjghz(AnalyseItem analyseItem) {
		int count = 0;
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			switch (analyseItem.cbWeaveKind[i]) {
			case GameConstants_YJGHZ.WIK_QING_NEI:
	...
	public int calculate_da_pai_count_qlhf(AnalyseItem analyseItem) {
		int count = 0;
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			count += get_da_card_qlhf(analyseItem.cbWeaveKind[i], analyseItem.cbCenterCard[i]);
		}
	...
	public int calculate_xiao_pai_count_qlhf(AnalyseItem analyseItem) {
		int count = 0;
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			count += get_xiao_card_qlhf(analyseItem.cbWeaveKind[i], analyseItem.cbCenterCard[i]);
		}
	...
	public int get_da_card_qlhf(int weave_kind, int center_card) {
		int count = 0;
		if (center_card > GameConstants.CARD_ESPECIAL_TYPE_NIAO) {
			center_card -= GameConstants.CARD_ESPECIAL_TYPE_NIAO;
		}
	...
	public int get_xiao_card_qlhf(int weave_kind, int center_card) {
		int count = 0;
		if (center_card > GameConstants.CARD_ESPECIAL_TYPE_NIAO) {
			center_card -= GameConstants.CARD_ESPECIAL_TYPE_NIAO;
		}
	...
	public boolean is_hanghangxi(AnalyseItem analyseItem) {
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			if (analyseItem.cbWeaveKind[i] == GameConstants.WIK_LEFT || analyseItem.cbWeaveKind[i] == GameConstants.WIK_RIGHT
					|| analyseItem.cbWeaveKind[i] == GameConstants.WIK_CENTER || analyseItem.cbWeaveKind[i] == GameConstants.WIK_YYWHZ_SHUNZI) {
				return false;
			}
		}
	...
	public boolean is_duizixi(AnalyseItem analyseItem) {
		for (int i = 0; i < analyseItem.cbWeaveKind.length; i++) {
			if (analyseItem.cbWeaveKind[i] == GameConstants.WIK_LEFT || analyseItem.cbWeaveKind[i] == GameConstants.WIK_RIGHT
					|| analyseItem.cbWeaveKind[i] == GameConstants.WIK_CENTER || analyseItem.cbWeaveKind[i] == GameConstants.WIK_YYWHZ_SHUNZI
					|| analyseItem.cbWeaveKind[i] == GameConstants.WIK_YYWHZ_SHUNZI_EQS || analyseItem.cbWeaveKind[i] == GameConstants.WIK_EQS) {
				return false;
			}
		}
		if (analyseItem.cbMenEye[0] != 0) {
	...
	public boolean is_yin(AnalyseItem analyseItem) {
		...
		if (cbMenEye[0] != 0) {
			if (color_hei(cbMenEye[0]) == false && color_hei(cbMenEye[1]) == false) {
				hong_count++;
			} else if (color_hei(cbMenEye[1]) == false || color_hei(cbMenEye[1]) == false) {
				ott_count++;
			}
		}
	...
	public boolean is_hua_huzi(AnalyseItem analyseItem) {
		...
		if (analyseItem.cbMenEye[0] != 0) {
			if (color_hei(analyseItem.cbMenEye[0]) && color_hei(analyseItem.cbMenEye[1])) {
				return false;
			}
		}
		return true;
	...
	public boolean analyse_card_yywhz(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index, int cur_card,
		...
		for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
			...
			if ((i < (GameConstants.MAX_HH_INDEX - 2)) && (cards_index[i] > 0) && ((i % 10) <= 7)) {
				for (int j = 1; j <= cards_index[i]; j++) {
					if ((cards_index[i + 1] >= j) && (cards_index[i + 2] >= j) && (cur_card == switch_to_card_data(i)
							|| cur_card == switch_to_card_data(i + 1) || cur_card == switch_to_card_data(i + 2))) {
						kindItem[cbKindItemCount].cbCardIndex[0] = i;
			...
			if ((card_date & GameConstants.LOGIC_MASK_VALUE) == 0x02) {
				for (int j = 1; j <= cards_index[i]; j++) {
					if (cards_index[i + 5] >= j && cards_index[i + 8] >= j && (cur_card == switch_to_card_data(i)
							|| cur_card == switch_to_card_data(i + 5) || cur_card == switch_to_card_data(i + 8))) {
						kindItem[cbKindItemCount].cbCardIndex[0] = i;
				...
				if (bEnoughCard == true) {
					...
					if ((bNeedCardEye == false) || (cbCardEye != 0) || cbMenEye[0] != 0) {
						...
						for (int i = 0; i < cbWeaveCount; i++) {
							...
							get_weave_card_yywhz(weaveItem[i].weave_kind, weaveItem[i].center_card, analyseItem.cbCardData[i]);
							count++;
						}
						// 设置牌型
						for (int i = 0; i < cbLessKindItem; i++) {
	...
	// 分析扑克
	public boolean analyse_card_yjghz(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index, int cur_card,
			List<AnalyseItem> analyseItemArray, boolean has_feng, int hu_xi[], boolean yws_type) {
		// 计算数目
		int cbCardCount = get_card_count_by_index(cards_index);
		hu_xi[0] = 0;
			...
			for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
				if (cards_index[i] == 2) {
					...
					for (int j = 0; j < cbWeaveCount; j++) {
						...
						analyseItem.hu_xi[j] = weaveItem[j].hu_xi;
						hu_xi[0] += analyseItem.hu_xi[j];
						get_weave_card_yjghz(weaveItem[j].weave_kind, weaveItem[j].center_card, analyseItem.cbCardData[j]);
						count++;
					}
					...
					analyseItem.cbMenEye[0] = cur_card;
					analyseItem.cbMenEye[1] = cur_card;
					// 插入结果
					analyseItemArray.add(analyseItem);
				...
				if ((cur_card == switch_to_card_data(i))) {
					if (seat_index != provider_index) {
						if (cards_index[i] >= 4) {
							...
							kindItem[cbKindItemCount].cbValidIndex[1] = i;
							kindItem[cbKindItemCount].cbValidIndex[2] = i;
							kindItem[cbKindItemCount].cbWeaveKind = GameConstants_YJGHZ.WIK_KAN;
							kindItem[cbKindItemCount].cbCenterCard = switch_to_card_data(i);
							cbKindItemCount++;
						} else {
							kindItem[cbKindItemCount].cbCardIndex[0] = i;
				...
				for (int j = 1; j <= cards_index[i]; j++) {
					if ((cards_index[i + 1] >= j) && (cards_index[i + 2] >= j) && (cur_card == switch_to_card_data(i)
						...
						if (cards_index[switch_to_card_index(cur_card)] >= 2) {
							...
							kindItem[cbKindItemCount].cbValidIndex[1] = i + 1;
							kindItem[cbKindItemCount].cbValidIndex[2] = i + 2;
							kindItem[cbKindItemCount].cbWeaveKind = GameConstants_YJGHZ.WIK_SHUNZI;
							kindItem[cbKindItemCount].cbCenterCard = switch_to_card_data(i);
							cbKindItemCount++;
						}
					} else if ((cards_index[i + 1] >= j) && (cards_index[i + 2] >= j)) {
				...
				if (bEnoughCard == true) {
					...
					if ((bNeedCardEye == false) || (cbCardEye != 0) || cbMenEye[0] != 0) {
						...
						for (int i = 0; i < cbWeaveCount; i++) {
							...
							analyseItem.cbCenterCard[i] = weaveItem[i].center_card;
							analyseItem.hu_xi[i] = weaveItem[i].hu_xi;
							get_weave_card_yjghz(weaveItem[i].weave_kind, weaveItem[i].center_card, analyseItem.cbCardData[i]);
							count++;
						}
						// 设置牌型
						for (int i = 0; i < cbLessKindItem; i++) {
						...
						if (cbCardEye != 0) {
							analyseItem.curCardEye = true;
							analyseItem.cbCardEye = cbCardEye;
							analyseItem.cbMenEye[0] = cbCardEye;
							analyseItem.cbMenEye[1] = cbCardEye;
						}
							...
							for (int i = 0; i < 7; i++) {
								if (analyseItem.cbCenterCard[i] > GameConstants.CARD_ESPECIAL_TYPE_NIAO) {
									if (GameConstants.WIK_LEFT == analyseItem.cbWeaveKind[i]) {
										analyseItem.cbWeaveKind[i] = GameConstants_YJGHZ.WIK_SHUNZI;
									}
									analyseItem.cbCenterCard[i] -= GameConstants.CARD_ESPECIAL_TYPE_NIAO;
								}
							}
						}
						if (cbMenEye[0] > GameConstants.CARD_ESPECIAL_TYPE_NIAO || cbMenEye[1] > GameConstants.CARD_ESPECIAL_TYPE_NIAO) {
							for (int i = 0; i < 7; i++) {
								if (analyseItem.cbCenterCard[i] > GameConstants.CARD_ESPECIAL_TYPE_NIAO) {
									if (GameConstants.WIK_LEFT == analyseItem.cbWeaveKind[i]) {
										analyseItem.cbWeaveKind[i] = GameConstants_YJGHZ.WIK_SHUNZI;
	...
	// 分析扑克
	public boolean analyse_card_yiyangwhz(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index,
			int cur_card, List<AnalyseItem> analyseItemArray, boolean has_feng, int hu_xi[], boolean yws_type) {
		// 计算数目
		int cbCardCount = get_card_count_by_index(cards_index);
		hu_xi[0] = 0;
				...
				if (bEnoughCard == true) {
					...
					if ((bNeedCardEye == false) || (cbCardEye != 0) || cbMenEye[0] != 0) {
						...
						for (int i = 0; i < cbWeaveCount; i++) {
							...
							analyseItem.cbCenterCard[i] = weaveItem[i].center_card;
							analyseItem.hu_xi[i] = weaveItem[i].hu_xi;
							get_weave_card_yiyangwhz(weaveItem[i].weave_kind, weaveItem[i].center_card, analyseItem.cbCardData[i]);
							count++;
						}
						// 设置牌型
						for (int i = 0; i < cbLessKindItem; i++) {
						...
						for (int i = 0; i < 7; i++) {
							if (analyseItem.cbCenterCard[i] > GameConstants.CARD_ESPECIAL_TYPE_NIAO
									&& analyseItem.cbWeaveKind[i] == GameConstants.WIK_YYWHZ_WAI) {
								for (int j = 0; j < 7; j++) {
									if (i == j) {
										continue;
									}
	...
	// 获取组合
	public int get_weave_card_yjghz(int cbWeaveKind, int cbCenterCard, int cbCardBuffer[]) {
		// 组合扑克
		switch (cbWeaveKind) {
		case GameConstants.WIK_LEFT: // 左吃类型
		{
	...
	public static void main(String[] args) {
		// 插入扑克
		PHZGameLogic logic = new PHZGameLogic();
