package com.cai.game.shengji.handler.xp240;
...
import com.cai.game.shengji.SJGameLogic_Xpsj;
...
import protobuf.clazz.Xpsj.XpsjRsp.CallBankerResponse_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.Card_Arrary_xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.DiPaiCard_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.GameStart_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.LiShiCard_xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.MaiDiOpreate_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.Opreate_RequestWsk_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.OutCardDataXpsj;
import protobuf.clazz.Xpsj.XpsjRsp.PaiFenData_xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.PukeGameEndXpsj;
import protobuf.clazz.Xpsj.XpsjRsp.RefreshCardData_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.SendCardEnd_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.SendCard_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.TableResponse_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.TableScore_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.Zhu_Count_Xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.Zhu_card_Data_xpsj;
import protobuf.clazz.Xpsj.XpsjRsp.effect_type_xpsj;

public class SJTable_XP_240 extends SJTable {
	...
	public SJGameLogic_Xpsj _logic;

	public SJTable_XP_240() {
		_logic = new SJGameLogic_Xpsj();
	}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_out_card_operate = new SJHandlerOutCardOperate_XP_240();
	...
	public void Reset() {
		...
		_pai_score_count = 28;
		_pai_score = 240;
		_repertory_card = new int[SJConstants.CARD_COUNT_XP_SJ];
		shuffle(_repertory_card, GameConstants.CARD_DATA_XP_240);
	...
	protected void test_cards() {
		...
		int cards[] = new int[] { 27, 60, 5, 25, 78, 7, 12, 18, 13, 41, 28, 55, 21, 29, 61, 29, 34, 24, 9, 26, 53, 49, 18, 56, 9, 55, 17, 78, 33, 59,
				2, 79, 17, 13, 12, 42, 37, 25, 27, 26, 37, 43, 8, 10, 57, 40, 61, 56, 40, 34, 1, 41, 50, 33, 8, 28, 45, 5, 7, 10, 23, 60, 49, 57, 45,
				11, 23, 24, 42, 58, 39, 43, 39, 44, 11, 79, 53, 1, 58, 44, 2, 21, 50, 59 };
	...
	public int get_hand_card_count_max() {
		if (this.has_rule(GameConstants.GAME_RULE_XPSJ_THREE_PEOPLE)) {
			return SJConstants.XP_SJ_MAX_COUT;
		} else {
			return SJConstants.XP_SJ_MAX_COUT - 7;
		}
	...
	protected boolean on_game_start() {
		...
		// 发送数据
		GameStart_Xpsj.Builder gamestart = GameStart_Xpsj.newBuilder();
	...
	public void send_history(int seat_index) {
		...
		LiShiCard_xpsj.Builder history_card = LiShiCard_xpsj.newBuilder();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			Card_Arrary_xpsj.Builder card_array = Card_Arrary_xpsj.newBuilder();
			for (int j = 0; j < 2; j++) {
	...
	void send_card(int type) {
		...
		for (int index = 0; index < getTablePlayerNumber(); index++) {
			...
			roomResponse.setGameStatus(this._game_status);
			roomResponse.setRoomInfo(getRoomInfo());
			SendCard_Xpsj.Builder send_card = SendCard_Xpsj.newBuilder();
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
	...
	public void send_di_pai_data(int seat_index) {
		...
		DiPaiCard_Xpsj.Builder di_pai_data = DiPaiCard_Xpsj.newBuilder();
		for (int i = 0; i < _di_pai_count; i++) {
	...
	public void send_zhu_data(int to_player) {
		...
		Zhu_card_Data_xpsj.Builder zhu_data = Zhu_card_Data_xpsj.newBuilder();
	...
	public void Refresh_Color_count(int seat_index) {
		...
		roomResponse.setGameStatus(this._game_status);
		roomResponse.setRoomInfo(getRoomInfo());
		Zhu_Count_Xpsj.Builder color_count = Zhu_Count_Xpsj.newBuilder();
	...
	protected void timerCallBack(SheduleArgs args) {
		...
		case ID_TIMER_SEND_CARD_SEND: {
			...
			if (GRR._card_count[0] >= this.get_hand_card_count_max()) {
				...
				SendCardEnd_Xpsj.Builder send_card_end = SendCardEnd_Xpsj.newBuilder();
	...
	public void load_player_info_data_game_start(GameStart_Xpsj.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_reconnect(TableResponse_Xpsj.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_game_end(PukeGameEndXpsj.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void Refresh_pai_score(int to_player) {
		...
		roomResponse.setRoomInfo(getRoomInfo());
		// 发送数据
		PaiFenData_xpsj.Builder pai_score_data = PaiFenData_xpsj.newBuilder();
	...
	protected boolean on_handler_game_finish(int seat_index, int reason) {
		...
		PukeGameEndXpsj.Builder gameend_sj = PukeGameEndXpsj.newBuilder();
	...
	public void cal_score(int end_score[], int seat_index) {
		int base_score = 240 / this.getTablePlayerNumber();
		int other_score = 0;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player) {
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			OutCardDataXpsj.Builder outcarddata = OutCardDataXpsj.newBuilder();
			Int32ArrayResponse.Builder cards = Int32ArrayResponse.newBuilder();
			...
			if (index == _current_player) {
				...
				if (_out_card_times[index] == 0 && _current_player == GRR._banker_player && _logic._zhu_type == 4) {
					can_out_count = _logic.player_can_out_card_first(GRR._cards_data[_current_player], GRR._card_count[_current_player],
					...
					can_out_count = _logic.Player_Can_out_card(GRR._cards_data[_current_player], GRR._card_count[_current_player],
	...
	public void deal_get_di_pai(int seat_index) {
		...
		MaiDiOpreate_Xpsj.Builder mai_di_opetate = MaiDiOpreate_Xpsj.newBuilder();
	...
	public void send_call_data(int seat_index, int to_player) {
		if (to_player == GameConstants.INVALID_SEAT) {
			...
			CallBankerResponse_Xpsj.Builder call_bnaker_response = CallBankerResponse_Xpsj.newBuilder();
	...
	public void RefreshCard(int to_player) {
		if (to_player == GameConstants.INVALID_SEAT) {
			...
			// 发送数据
			RefreshCardData_Xpsj.Builder refresh_card = RefreshCardData_Xpsj.newBuilder();
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
				for (int j = 0; j < this.GRR._card_count[i]; j++) {
					cards_card.addItem(this.GRR._cards_data[i][j]);
	...
	public void RefreshScore(int to_player, int is_delay) {
		...
		// 发送数据
		TableScore_Xpsj.Builder refresh_score = TableScore_Xpsj.newBuilder();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
