...
import com.cai.game.phu.handler.PHHandler;
import com.cai.game.phu.handler.PHHandlerChiPeng;
import com.cai.game.phu.handler.PHHandlerDispatchCard;
import com.cai.game.phu.handler.PHHandlerFinish;
import com.cai.game.phu.handler.PHHandlerGang;
import com.cai.game.phu.handler.PHHandlerOutCardOperate;
import com.cai.game.phu.handler.phuyx.PHHandleAddFirstCard_YX;
import com.cai.game.phu.handler.phuyx.PHHandlerChiPeng_YX;
import com.cai.game.phu.handler.phuyx.PHHandlerChuLiFirstCard_YX;
import com.cai.game.phu.handler.phuyx.PHHandlerDispatchCard_YX;
import com.cai.game.phu.handler.phuyx.PHHandlerDispatchFirstCard_YX;
import com.cai.game.phu.handler.phuyx.PHHandlerGang_YX;
import com.cai.game.phu.handler.phuyx.PHHandlerOutCardOperate_YX;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PHTable extends AbstractRoom {
	/**
	 * 
	 */
	private static final long serialVersionUID = 7060061356475703643L;

	private static Logger logger = Logger.getLogger(PHTable.class);
	...
	public PHGameLogic _logic = null;
	...
	public int _handler_status_index[]; // 操作handler
	public long _g_sys_timer;
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_cur_round = 0;
		_handler_status_index = new int[this.getTablePlayerNumber()];
		Arrays.fill(_handler_status_index, 0);
		// 上次抓的牌
		_last_card = 0;
		_long_count = new int[this.getTablePlayerNumber()]; // 每个用户有几条龙
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	private void shuffle(int repertory_card[], int mj_cards[]) {
		...
		int count = getMaxTablePlayerNumber();
	...
	private void test_cards() {
		...
		// 0x01,0x02,0x03,0x04,0x01,0x02,0x03,0x04,0x01,0x02,0x03,0x04,0x05,0x05,
		// 0x06,0x07,0x08,0x09,0x0a,0x06,0x07,0x08,0x09,0x0a,0x05,0x06,0x07,0x08,
		// 0x09,0x0a,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x11,0x12,
		// 0x15,0x14,0x16,0x17,0x18,0x19,0x1a,0x11,0x12,0x13,0x14,0x13,0x16,0x17,
		// 0x15,0x18,0x19,0x1a,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1a,
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		if (!(reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW)) {
			for (int j = 0; j < this.getTablePlayerNumber(); j++) {
				...
				if (GRR != null) {
					int temp_match_times = 1;
					if (this.matchId != 0)
						temp_match_times = this.matchBase.getBase() * this.matchBase.getBaseScore() * this.matchBase.getTimes();

					GRR._game_score[j] = _game_mid_score[j] * temp_match_times;
	...
	public boolean handler_game_finish_phu(int seat_index, int reason) {
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL ||
			...
			for (int i = 0; i < this.getMaxTablePlayerNumber(); i++) {
	...
	public boolean sao_ti_operate(int _seat_index, int _provide_player, int _center_card, int _action, boolean _p, int _score[], boolean _depatch) {
		...
		_logic.calculate_game_weave_score(_game_mid_score, _game_weave_score, _score, getTablePlayerNumber(), false, this.get_match_times());
	...
	public int analyse_chi_hu_card_phu(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index, int cur_card,
		...
		if (timer.get() > 300) {
			String str = "phz ting card cost time = " + timer.duration() + "  and cards is =" + Arrays.toString(cbCardIndexTemp) + "Arrays weaveItem"
					+ Arrays.toString(weaveItems);
			this.log_warn(str);
			MongoDBServiceImpl.getInstance().server_error_log(this.getRoom_id(), ELogType.roomLogicSlow,
					SysGameTypeDict.getInstance().getGameDescByTypeIndex(_game_type_index) + "游戏类型:" + _game_type_index + "_cur_round" + _cur_round,
					0L, str);
		}
	...
	@Override
	public boolean handler_status_operate_card(int seat_index, int operate_code, int operate_card, int luoCode, int handler_status) {

		if (this._handler != null) {
			this._handler.handler_status_operate_card(this, seat_index, operate_code, operate_card, luoCode, handler_status);
		}
	...
	public int get_match_times() {
		int match_times = 1;
		// if(this.matchId != 0)
		// match_times = this.matchBase.getBase() *
		// this.matchBase.getBaseScore() * this.matchBase.getTimes();
		return match_times;
	}
	...
	public void process_chi_hu_player_score_ph(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		} else if (has_rule(GameConstants.GAME_RULE_FOUR_FIRST_MUL)) {
			calculate_score = _logic.get_chi_hu_action_rank_ph(seat_index, chr, 1, false);
			int times = this._banker_count[seat_index];
			if (this._banker_count[seat_index] > 4) {
				times = 4;
			}
			calculate_score *= times;
		}
		int temp_match_times = 1;
		if (this.matchId != 0)
			temp_match_times = this.matchBase.getBase() * this.matchBase.getBaseScore() * this.matchBase.getTimes();
		if ((chr.opr_and(GameConstants.CHR_TIAN_HU_PH)).is_empty()) {
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				GRR._game_score[i] = this._game_mid_score[i] * temp_match_times;
			}
		}

		float lChiHuScore = calculate_score * this.get_match_times();
		...
		int match_times = get_match_times();
		...
		if (zimo) {
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				// 胡牌分
				GRR._game_score[i] -= lChiHuScore * temp_match_times;
				GRR._game_score[seat_index] += lChiHuScore * temp_match_times;
				score[i] -= lChiHuScore;
				score[seat_index] += lChiHuScore;
			...
			GRR._game_score[provide_index] -= lChiHuScore * temp_match_times;
			GRR._game_score[seat_index] += lChiHuScore * temp_match_times;
			score[provide_index] -= lChiHuScore;
			score[seat_index] += lChiHuScore;
		}

		_logic.calculate_game_weave_score(_game_mid_score, _game_weave_score, score, getTablePlayerNumber(), false, 1);
		if (_game_weave_score[seat_index] != 0) {
			operate_game_mid_score(true);
		}
	...
	// 添加牌到牌队
	public boolean operate_minus_discard(int seat_index, int count, int cards[], boolean is_grr) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		this.load_common_status(roomResponse);
	...
	// return true;
	// }
	public boolean operate_player_timer(boolean is_grr) {
		// if(this.is_sys() == false)
		// return true;
		// long delay = 10000;
		...
		// if(is_grr == true)
		// {
		// _g_sys_timer = System.currentTimeMillis();
		// }
		// else{
		// if(System.currentTimeMillis() - _g_sys_timer < delay)
		// delay = delay-(System.currentTimeMillis() - _g_sys_timer);
		// }
		//
		// RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		// roomResponse.setType(MsgConstants.RESPONSE_GSYS_TIMER);
		// roomResponse.setInsertTime((int)delay); //到计时
		// if(is_grr == true)
		// GRR.add_room_response(roomResponse);
		// this.send_response_to_room( roomResponse);
		return true;
	...
	public boolean operate_player_action(int seat_index, boolean close) {
		...
		if (delay > 40000)
			delay = 20000;
		roomResponse.setInsertTime(delay); // 到计时
		if (close == true) {
			GRR.add_room_response(roomResponse);
			// 通知玩家关闭
			this.send_response_to_player(seat_index, roomResponse);
	...
	public void load_common_status(RoomResponse.Builder roomResponse) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			roomResponse.addHandlerStatus(_playerStatus[i].get_handler_status());
		}
	}
	...
	public boolean set_timer(int timer_type, int time, boolean makeDBtimer) {
		...
		// _game_scheduled = GameSchedule.put(new
		// AddJettonRunnable(getRoom_id()), sysParamModel6.getVal1(),
		// TimeUnit.SECONDS);
		...
		// _game_scheduled = GameSchedule.put(new
		// OpenCardRunnable(getRoom_id()), sysParamModel6.getVal2(),
		// TimeUnit.SECONDS);
		// _operate_start_time = (int) (System.currentTimeMillis() / 1000L);
		// this._cur_operate_time = sysParamModel6.getVal2();
		// } else if (timer_type == GameConstants.PH_WARING) {
		// _game_scheduled = GameSchedule.put(new
		// RobotBankerRunnable(getRoom_id()), sysParamModel6.getVal3(),
		// TimeUnit.SECONDS);
		// _operate_start_time = (int) (System.currentTimeMillis() / 1000L);
		// this._cur_operate_time = sysParamModel6.getVal3();
		// } else if (timer_type == GameConstants.PH_READY) {
		// _game_scheduled = GameSchedule.put(new ReadyRunnable(getRoom_id()),
		// sysParamModel6.getVal4(),
		// TimeUnit.SECONDS);
		// _operate_start_time = (int) (System.currentTimeMillis() / 1000L);
	...
	public void runnable_minus_discard(int seat_index, int card_count, int card_data[], boolean send_client) {
		...
		if (send_client == true) {
			this.operate_minus_discard(seat_index, GRR._discard_count[seat_index], GRR._discard_cards[seat_index], true);
		}
	}
	...
	public int getTablePlayerNumber() {
		if (this.getRuleValue(GameConstants.GAME_RULE_TWO_PLAYER_PH) == 1)
			return 2;
		if (this.getRuleValue(GameConstants.GAME_RULE_THREE_PLAYER_PH) == 1)
			return 3;
		return GameConstants.GAME_PLAYER_FPHZ;
	}

	public int getMaxTablePlayerNumber() {
		return GameConstants.GAME_PLAYER_FPHZ;
	}
	...
	public boolean hu_pai_timer(int seat_index, int operate_card, int wik_kind) {
		handler_status_operate_card(seat_index, wik_kind, operate_card, -1, this._playerStatus[seat_index].get_handler_status());
		return false;
	}
