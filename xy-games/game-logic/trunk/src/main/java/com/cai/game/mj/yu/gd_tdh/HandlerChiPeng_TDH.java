...
public class HandlerChiPeng_TDH extends MJHandlerChiPeng<Table_TDH> {
	...
	public void exe(Table_TDH table) {
		...
		int card_type_count = GameConstants_TDH.MAX_ZI;
		...
		if (ting_count > 0) {
			int tmp_cards[] = new int[GameConstants_TDH.MAX_COUNT];
			...
			for (int i = 0; i < tmp_hand_card_count; i++) {
				for (int j = 0; j < ting_count; j++) {
					if (tmp_cards[i] == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
						tmp_cards[i] += GameConstants_TDH.CARD_ESPECIAL_TYPE_TING;
						break;
					}
				}
			}
	...
	public boolean handler_operate_card(Table_TDH table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants_TDH.WIK_NULL) {
			...
			table.change_player_status(_seat_index, GameConstants_TDH.INVALID_VALUE);
			table.change_player_status(_seat_index, GameConstants_TDH.Player_Status_OUT_CARD);
			table.operate_player_status();
	...
	@Override
	public boolean handler_player_out_card(Table_TDH table, int seat_index, int card) {
