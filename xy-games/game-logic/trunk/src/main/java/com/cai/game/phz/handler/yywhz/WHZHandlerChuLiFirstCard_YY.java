...
public class WHZHandlerChuLiFirstCard_YY extends PHZHandlerDispatchCard<YYWHZTable> {
	...
	public void exe(YYWHZTable table) {
		...
		if (is_tianhu == 2) {
			...
			for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
				if (cbCardIndexTemp[i] == 3) {
					...
					for (int x = 0; x < table._hu_weave_count[_seat_index]; x++) {
						if (table._logic.switch_to_card_index(table._hu_weave_items[_seat_index][x].center_card) == i) {
							for (int j = 0; j < 4; j++) {
								table._hu_weave_items[_seat_index][x].weave_card[j] = table._logic
										.switch_to_card_data(i);
							}
							table._hu_weave_items[_seat_index][x].weave_kind = GameConstants.WIK_YYWHZ_KAN;
							table._hu_weave_items[_seat_index][x].hu_xi = table._logic
									.get_weave_hu_xi_yywhz_yywzh(table._hu_weave_items[_seat_index][x]);
							is_have = true;
							break;
						}
					}
					if (!is_have) {
						for (int j = 0; j < 4; j++) {
							table._hu_weave_items[_seat_index][table._hu_weave_count[_seat_index]].weave_card[j] = table._logic
									.switch_to_card_data(i);
						}
						table._hu_weave_items[_seat_index][table._hu_weave_count[_seat_index]].weave_kind = GameConstants.WIK_YYWHZ_KAN;
						table._hu_weave_items[_seat_index][table._hu_weave_count[_seat_index]].hu_xi = table._logic
								.get_weave_hu_xi_yywhz_yywzh(
										table._hu_weave_items[_seat_index][table._hu_weave_count[_seat_index]]);
				...
				if (cbCardIndexTemp[i] == 4) {
					...
					for (int x = 0; x < table._hu_weave_count[_seat_index]; x++) {
						if (table._logic.switch_to_card_index(table._hu_weave_items[_seat_index][x].center_card) == i) {
							...
							table._hu_weave_items[_seat_index][x].weave_kind = GameConstants.WIK_YYWHZ_LIU_NEI;
							table._hu_weave_items[_seat_index][x].hu_xi = table._logic
									.get_weave_hu_xi_yywhz_yywzh(table._hu_weave_items[_seat_index][x]);
							is_have = true;
							break;
		...
		int cbActionMask = table.estimate_player_liu_nei_respond_yywhz(table.GRR._cards_index[_seat_index],
		...
		if (cbActionMask != GameConstants.WIK_NULL) {// 有溜
			for (int i = 0; i < gangCardResult.cbCardCount; i++) {
				...
				if (is_tianhu == 1) {
					...
					chr.opr_or(GameConstants.CHR_TIAN_HU_WHZ);
				} else if (is_tianhu == 2) {
					// 添加动作
					table._playerStatus[_seat_index].add_action(GameConstants.WIK_ZI_MO);
					table._playerStatus[_seat_index].add_zi_mo(_send_card_data, _seat_index);
		...
		boolean is_xian_jia_tian_hu = false;
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if (i == this._seat_index) {
				continue;
			}
			...
			int cbCardIndexTemp[] = new int[GameConstants.MAX_HH_INDEX];
			for (int j = 0; j < GameConstants.MAX_HH_INDEX; j++) {
				cbCardIndexTemp[j] = table.GRR._cards_index[i][j];
			}
			boolean wudui = table._logic.wudui(cbCardIndexTemp);
			boolean shidui = table._logic.shidui(cbCardIndexTemp);
			boolean yidui = table._logic.yidui(cbCardIndexTemp);
			boolean jiudui = table._logic.jiudui(cbCardIndexTemp);

			if (wudui) {
				chiHuRight.opr_or(GameConstants.CHR_WU_DUI_WHZ);
				table._playerStatus[i].add_action(GameConstants.WIK_ZI_MO);
				table._playerStatus[i].add_zi_mo(0, i);
				chr.opr_or(GameConstants.CHR_ZI_MO);
			}
			if (shidui) {
			...
			for (int j = 0; j < GameConstants.MAX_HH_INDEX; j++) {
				if (cbCardIndexTemp[j] == 3) {
					for (int x = 0; x < 4; x++) {
						table._hu_weave_items[i][table._hu_weave_count[i]].weave_card[x] = table._logic
								.switch_to_card_data(j);
					}
					table._hu_weave_items[i][table._hu_weave_count[i]].weave_kind = GameConstants.WIK_YYWHZ_KAN;
					table._hu_weave_items[i][table._hu_weave_count[i]].hu_xi = table._logic
							.get_weave_hu_xi_yywhz_yywzh(table._hu_weave_items[i][table._hu_weave_count[i]]);
					table._hu_weave_count[i]++;
				}
				if (cbCardIndexTemp[j] == 4) {
					...
					table._hu_weave_items[i][table._hu_weave_count[i]].weave_kind = GameConstants.WIK_YYWHZ_LIU_NEI;
					table._hu_weave_items[i][table._hu_weave_count[i]].hu_xi = table._logic
							.get_weave_hu_xi_yywhz_yywzh(table._hu_weave_items[i][table._hu_weave_count[i]]);
					table._hu_weave_count[i]++;
				}
		...
		if (is_tianhu == 0 && !is_xian_jia_tian_hu && cbActionMask == GameConstants.WIK_NULL) {
			chr.set_empty();
			// 加到手牌
	 ...
	 */
	@Override
	public boolean handler_player_out_card(YYWHZTable table, int seat_index, int card) {
		// 错误断言
	...
	public boolean handler_operate_card(YYWHZTable table, int seat_index, int operate_code, int operate_card,
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			if (table.has_rule(GameConstants.GAME_RULE_DI_ERZI_LIANG_PAI) == false)
				table.operate_player_get_card(target_player, 1, new int[] { table._send_card_data },
			...
			if (!(table.GRR._chi_hu_rights[target_player].opr_and(GameConstants.CHR_WU_DUI_WHZ)).is_empty()
				...
				table.GRR._chi_hu_card[target_player][0] = cards[hand_card_count - 1];
		...
		{
			// if(table.GRR._cards_index[target_player][table._logic.switch_to_card_index(_liu_card_data[target_player])]
			// != 4){
			// // 用户状态
			// int next_player = (target_player + table.getTablePlayerNumber() +
			...
			// return true;
			// }else{
			// table.GRR._cards_index[target_player][table._logic.switch_to_card_index(_liu_card_data[target_player])]=0;
			// // 组合扑克
			...
			// _liu_card_data[target_player];
			// table.GRR._weave_items[target_player][wIndex].weave_kind =
			// target_action;
			//
			// table.GRR._weave_items[target_player][wIndex].provide_player =
			// target_player;
			// table.GRR._weave_items[target_player][wIndex].hu_xi =
			// table._logic.get_weave_hu_xi_yywhz_yywzh(table.GRR._weave_items[target_player][wIndex]);
			// }
			// table._user_out_card_count[target_player]++;
			// int _action = GameConstants.WIK_YYWHZ_LIU_NEI;
			// // 效果
			// table.operate_effect_action(target_player,
