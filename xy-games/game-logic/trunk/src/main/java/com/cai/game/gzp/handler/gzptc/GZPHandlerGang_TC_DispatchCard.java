...
public class GZPHandlerGang_TC_DispatchCard extends GZPHandler {
	...
	public GZPHandlerGang_TC_DispatchCard() {
		_gang_card_data = new CardsData(GameConstants.GZP_MAX_COUNT);
			...
			for (int k = 0; k < table.getTablePlayerNumber(); k++) {
				...
				if (k == _seat_index) {
					...
					int action = table.analyse_chi_hu_card(table.GRR._cards_index[k], table.GRR._weave_items[k],
							table.GRR._weave_count[k],table._sheng_guan_index[_seat_index],table._pick_up_index[_seat_index], bu_card, chr, GameConstants.HU_CARD_TYPE_GANG_KAI,k,k);// 自摸
...
//						chr.opr_or(GameConstants.CHR_FLS_GANGSHANGHUA);
	...
	public boolean handler_operate_card(GZPTable table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			table.process_chi_hu_player_score_gzp(_seat_index, _seat_index, target_card, true);
...
//			table._player_result.da_hu_zi_mo[_seat_index]++;
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				table.process_chi_hu_player_score_gzp(i, _seat_index, target_card, false);
...
//				table._player_result.da_hu_jie_pao[i]++;
	...
	private void exe_chi_peng(GZPTable table, int target_player, int target_action, int target_card) {
		// 删掉出来的那两张张牌
