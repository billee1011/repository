...
public class HandlerChiPeng_YangZhong extends MJHandlerChiPeng<Table_YangZhong> {
	...
	public HandlerChiPeng_YangZhong() {
	...
	public void exe(Table_YangZhong table) {
		...
		int card_color = table._logic.get_card_color(_card);
		int card_value = table._logic.get_card_value(_card);
		...
		if (_action != GameConstants.WIK_PENG && table._card_can_not_out_after_chi[_seat_index] != 0) {
			for (int i = 0; i < hand_card_count; i++) {
				if (cards[i] == table._card_can_not_out_after_chi[_seat_index]) {
	...
	public boolean handler_player_be_in_room(Table_YangZhong table, int seat_index) {
		...
		if (seat_index == _seat_index && _action != GameConstants.WIK_PENG) {
			if (table._card_can_not_out_after_chi[_seat_index] != 0) {
				for (int i = 0; i < GameConstants.MAX_COUNT; i++) {
					if (hand_cards[i] == table._card_can_not_out_after_chi[_seat_index]) {
