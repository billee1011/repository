...
public class MJHandlerDispatchCard_HeNan_py extends MJHandlerDispatchCard<MJTable_PY> {
    ...
    public void exe(MJTable_PY table) {
        ...
        if (_type == GameConstants.HU_CARD_TYPE_GANG_DI) {
            _type = GameConstants.HU_CARD_TYPE_GANG_KAI;
        }
        ...
        if (table.has_rule(GameConstants.GAME_RULE_PUYANG_BAOTING)) {
            if (table._playerStatus[_seat_index]._hu_card_count > 0
                    && table._playerStatus[_seat_index].is_bao_ting() == true) {
                int action = table.analyse_chi_hu_card_henan_py(table.GRR._cards_index[_seat_index],
        ...
        if (table.has_rule(GameConstants.GAME_RULE_PUYANG_YAOJIUPU)) {
            if (table._logic.switch_to_card_index(_send_card_data) % 9 == 0
                    && table._logic.switch_to_card_index(_send_card_data) < 27) {
                real_card += GameConstants.CARD_ESPECIAL_TYPE_YAOJIU;
            } else if (table._logic.switch_to_card_index(_send_card_data) % 9 == 8
                    && table._logic.switch_to_card_index(_send_card_data) < 27) {
                real_card += GameConstants.CARD_ESPECIAL_TYPE_YAOJIU;
            }
        }
                ...
                if (table._playerStatus[_seat_index].is_bao_ting()) {
                    for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
                        ...
                        if (table._logic.get_card_count_by_index(table.GRR._cards_index[_seat_index]) == 1) {
                            ting_count = table.get_ting_card(ting_cards, cbCardIndexTemp,
    ...
    public boolean handler_operate_card(MJTable_PY table, int seat_index, int operate_code, int operate_card) {
        ...
        case GameConstants.WIK_ZI_MO: // 自摸
        {
            ...
            table.process_chi_hu_player_score_py(_seat_index, _seat_index, operate_card, true);
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if (i == _seat_index) {
                    continue;
                }
