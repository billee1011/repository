...
public class HandlerOutCardOperate_XinYang extends MJHandlerOutCardOperate<Table_XinYang> {
	...
	public void exe(Table_XinYang table) {
		if(_out_card_data == table._player_result.pao[_out_card_player])
			table._player_result.pao[_out_card_player] = 0;
		else if(_out_card_data == table._player_result.qiang[_out_card_player])
			table._player_result.qiang[_out_card_player] = 0;
		...
		int hand_card_count = table.switch_to_cards_data(_out_card_player, table.GRR._cards_index[_out_card_player], cards);
		//刷新手牌
	...
	private void handler_operate_bu_tong_pao(Table_XinYang table, int seat_index,
		...
		case GameConstants.WIK_CHI_HU: {
			...
			if(table._cur_banker != target_player || table.continueBankerCount == 2){
				table._cur_banker = (table._cur_banker + 1) % table.getTablePlayerNumber();
				table.continueBankerCount = 0;
			}else{
				table.continueBankerCount++;
				if(table.has_rule(Constants_MJ_XIN_YANG.GAME_RULE_SHU_SAN_JIA))
					table.isLastGameDianPao3 = true;
			}
			table.GRR._chi_hu_rights[target_player].set_valid(true);
			table.GRR._chi_hu_rights[_out_card_player].opr_or(Constants_MJ_XIN_YANG.CHR_FANG_PAO);
	...
	public boolean handler_player_be_in_room(Table_XinYang table, int seat_index) {
		...
		table.switch_to_cards_data(seat_index, table.GRR._cards_index[seat_index], hand_cards);
