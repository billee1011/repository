...
public class MJHandlerChiPeng_YongZhou extends MJHandlerChiPeng<MJTable_YongZhou> {
	private GangCardResult m_gangCardResult;

	public MJHandlerChiPeng_YongZhou() {
	...
	public void exe(MJTable_YongZhou table) {
		...
		// 刷新手牌包括组合
		Tuple<Integer, List<Integer>> t = table.switch_to_cards_data(_seat_index);
		...
		for (int i = 0; i < weave_count; i++) {
			...
			if (table._logic.is_magic_card(weaves[i].center_card)) {
				weaves[i].center_card += GameConstants.CARD_ESPECIAL_TYPE_WANG_BA;
			}
		}
		// 刷新手牌
		...
		if (table.GRR._left_card_count > llcard) {
			int cbActionMask = table.analyse_gang_card_all(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
			...
			if (cbActionMask != 0) {
				for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
					...
					if (m_gangCardResult.cbCardData[i] == _card) {
						continue;
					}
					// 加上刚
	 ...
	 */
	@Override
	public boolean handler_player_out_card(MJTable_YongZhou table, int seat_index, int card) {
