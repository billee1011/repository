package com.cai.game.mj.yu.kwx;
...
public class HandlerDispatchCard_KWX extends MJHandlerDispatchCard<Table_KWX> {
	...
	public HandlerDispatchCard_KWX() {
	...
	@Override
	public void exe(Table_KWX table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			if (liang_count != table.getTablePlayerNumber() && liang_count != 0) {
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			// TODO:流局谁摸最后一张牌谁做庄
			table._cur_banker = (_seat_index + table.getTablePlayerNumber() - 1) % table.getTablePlayerNumber();
		...
		if (ting_count > 0) {
			...
			if (table.player_liang[_seat_index] != 1 && table.filterLiang(_seat_index)) {
				...
				int liang_count = table.checkLiangAddWeave(_seat_index, liang_cards_index);
				if (liang_count > 0) {
		...
		if (curPlayerStatus.has_action()) {
			if (table.player_liang[_seat_index] == 1 && table._playerStatus[_seat_index].has_action_by_code(GameConstants_KWX.WIK_ZI_MO)) {
				...
				if (table.GRR._left_card_count == 0) {
					table.GRR._chi_hu_rights[_seat_index].opr_or(GameConstants_KWX.CHR_HAI_DI_HU);
				}
		...
		if (table.has_rule(GameConstants_KWX.GAME_RULE_AUTO_10)) {
			...
			table.future = GameSchedule.put(new Runnable() {
				...
				public void run() {
					...
					handler_check_auto_behaviour_not_gold(table, _seat_index, _send_card_data);
				}
	...
	@Override
	public boolean handler_player_out_card(Table_KWX table, int seat_index, int card) {
		...
		// } else {
		// 出牌
		table.exe_out_card(seat_index, card, _type);
		// }
	...
	@Override
	public boolean handler_operate_card(Table_KWX table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants_KWX.WIK_NULL) {
			if (table._playerStatus[seat_index].has_action_by_code(GameConstants_KWX.WIK_ZI_MO)) {
				table._playerStatus[seat_index].add_cards_abandoned_hu(_send_card_data);
				table.pass_hu_fan[seat_index] = table.pass_hu_fan[seat_index] > table.get_fan(seat_index, seat_index) ? table.pass_hu_fan[seat_index]
						: table.get_fan(seat_index, seat_index);
	...
	@Override
	public boolean handler_player_be_in_room(Table_KWX table, int seat_index) {
		...
		if (_seat_index == seat_index || table._game_type_index == GameConstants.GAME_TYPE_KWX_2D) {
			int show_send_card = _send_card_data;
