...
public class WHZHandlerOutCardOperate_YiYang extends PHZHandlerOutCardOperate<YiYangWHZTable> {
	@Override
	public void exe(YiYangWHZTable table) {
		...
		if (table.GRR._cannot_out_index[_out_card_player][table._logic.switch_to_card_index(_out_card_data)] >= 1) {
			// table.log_error(_out_card_player+"出牌出错
			// WHZHandlerOutCardOperate_YY "+_out_card_data);
			//
			// // 刷新手牌
			// int cards[] = new int[GameConstants.MAX_HH_COUNT];
			// // 刷新自己手牌
			// int hand_card_count =
			// table._logic.switch_to_cards_data(table.GRR._cards_index[_out_card_player],
			// cards);
			// table.operate_player_cards(_out_card_player, hand_card_count,
			// cards,
			// table.GRR._weave_count[_out_card_player],table.GRR._weave_items[_out_card_player]);
			//
			// table._playerStatus[_out_card_player].set_status(GameConstants.Player_Status_OUT_CARD);//
			// 出牌状态
			// table.operate_player_status();
			//
			// table.send_error_notify(_out_card_player, 2, "这张牌不能出");
			// return ;
		...
		// 删除扑克
		if (table._logic.remove_card_by_index(table.GRR._cards_index[_out_card_player], _out_card_data) == false) {
			table.log_error("出牌删除出错");
			return;
		}
		//
		table._playerStatus[_out_card_player].reset();
		// 出牌记录
		...
		table._cannot_peng[_out_card_player][table._cannot_peng_count[_out_card_player]++] = _out_card_data;
		table._hand_card_index[_out_card_player][table._logic.switch_to_card_index(_out_card_data)]--;
		table._user_out_card_count[_out_card_player]++;

		table.cannot_chi_card_YIYANG(_out_card_player, _out_card_data, table.GRR._cards_index[_out_card_player]);
		// 用户切换
		...
		table._playerStatus[_out_card_player]._hu_card_count = table.get_yywhz_ting_card(
		...
		bAroseAction = table.estimate_player_out_card_respond_yiyangwhz(_out_card_player, _out_card_data,
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (table._is_xiang_gong[i]) {
				continue;
			}
			GangCardResult gangCardResult = new GangCardResult();
			gangCardResult.cbCardCount = 0;
			int cbActionMask = table.estimate_player_piao_respond_yywhz(table.GRR._cards_index[i],
					table.GRR._weave_items[i], table.GRR._weave_count[i], _out_card_data, gangCardResult, i,
					_out_card_player, false);
			if (cbActionMask != GameConstants.WIK_NULL) {// 有飘
				for (int j = 0; j < gangCardResult.cbCardCount; j++) {
					table._playerStatus[i].add_liu(gangCardResult.cbCardData[j], i, gangCardResult.isPublic[j],
							cbActionMask);
					table._playerStatus[i].add_action(cbActionMask);// 飘牌
					table._playerStatus[i].add_action(GameConstants.WIK_NULL);
					table._playerStatus[i].add_pass(_out_card_data, i);
		...
		if (bAroseAction == false) {
			...
			table.cannot_chi_card_YIYANG(next_player, _out_card_data, table.GRR._cards_index[next_player]);
			//
	...
	public boolean handler_operate_card(YiYangWHZTable table, int seat_index, int operate_code, int operate_card,
			int luoCode) {
		// 效验状态
		...
		for (int p = 0; p < table.getTablePlayerNumber(); p++) {
			...
			if (table._playerStatus[i].has_action()) {
				if (table._playerStatus[i].is_respone()) {
					// 获取已经执行的动作的优先级
					cbUserActionRank = table._logic.get_action_rank_yiyang(table._playerStatus[i].get_perform(), i,
							_out_card_player) + table.getTablePlayerNumber() - p;
					cbActionRank[i] = cbUserActionRank;
				} else {
					// 获取最大的动作的优先级
					cbUserActionRank = table._logic.get_action_list_rank_yiyang(table._playerStatus[i]._action_count,
							table._playerStatus[i]._action, i, _out_card_player) + table.getTablePlayerNumber() - p;
				}

				if (table._playerStatus[target_player].is_respone()) {
					// 获取已经执行的动作的优先级
					cbTargetActionRank = table._logic.get_action_rank_yiyang(
							table._playerStatus[target_player].get_perform(), target_player, _out_card_player)
							+ target_p;
					cbActionRank[i] = cbUserActionRank;
				} else {
					// 获取最大的动作的优先级
					cbTargetActionRank = table._logic.get_action_list_rank_yiyang(
							...
							table._playerStatus[target_player]._action, target_player, _out_card_player) + target_p;
				}
				...
				for (int j = 0; j < table._playerStatus[i]._action_count; j++) {
					...
					case GameConstants.WIK_EQS:
						if (target_action == GameConstants.WIK_NULL && table._playerStatus[i].get_exe_pass() == true) {
							table.cannot_chi_card_YIYANG(i, operate_card, table.GRR._cards_index[i]);
						}

						break;
					case GameConstants.WIK_PENG:
						if (table._logic.get_action_rank_yiyang(target_action, target_player,
								_out_card_player) < table._logic.get_action_rank_yiyang(GameConstants.WIK_PENG,
										target_player, _out_card_player)) {
		...
		case GameConstants.WIK_YIYANGWHZ_PIAO: {
			table.exe_piao(target_player, _out_card_player, target_action, operate_card,
