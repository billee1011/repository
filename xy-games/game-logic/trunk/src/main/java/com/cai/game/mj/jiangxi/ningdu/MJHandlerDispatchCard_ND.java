 ...
 *
 */
public class MJHandlerDispatchCard_ND extends MJHandlerDispatchCard<MJTable_ND> {
	boolean ting_send_card = false;

	@Override
	public void exe(MJTable_ND table) {
		// 用户状态
		...
		if (table.GRR._left_card_count > 0) {
			...
			int cbActionMask = table._logic.analyse_gang_by_card_hong_zhong(table.GRR._cards_index[_seat_index],
	 ...
	 */
	@Override
	public boolean handler_operate_card(MJTable_ND table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			/*for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				for (int j = 0; j < table.GRR._player_niao_count[i]; j++) {
						...
						table.GRR._player_niao_cards[i][j] = table.set_ding_niao_valid(table.GRR._player_niao_cards[i][j],flag);// 胡牌的鸟生效
				}
			}*/
	...
	@Override
	public boolean handler_player_be_in_room(MJTable_ND table, int seat_index) {
