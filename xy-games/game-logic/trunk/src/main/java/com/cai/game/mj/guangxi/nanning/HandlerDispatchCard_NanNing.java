...
public class HandlerDispatchCard_NanNing extends MJHandlerDispatchCard<Table_NanNing> {
	...
	public HandlerDispatchCard_NanNing() {
	...
	public void exe(Table_NanNing table) {
		...
		int card_type = GameConstants_NanNing.HU_CARD_TYPE_ZI_MO;
		if (_type == GameConstants.GANG_TYPE_JIE_GANG || _type == GameConstants.GANG_TYPE_ADD_GANG) { // 点杠杠上花按点炮计分
			card_type = GameConstants_NanNing.HU_CARD_QUAN_BAO | GameConstants_NanNing.HU_CARD_TYPE_GANG_KAI;
		} else if (_type == GameConstants.GANG_TYPE_AN_GANG) { // 暗杠碰杠按杠开计分
			card_type = GameConstants_NanNing.HU_CARD_TYPE_GANG_KAI;
			table.quan_bao[_seat_index] = -1;
		} else {
			table.quan_bao[_seat_index] = -1;
		}

		int action = GameConstants.WIK_NULL;
		if (!table.feng_pai[_seat_index])
		...
		if (action == GameConstants.WIK_NULL && table.GRR._left_card_count == table.getCsDingNiaoNum()) {
			table.exe_out_card(_seat_index, _send_card_data, _type);
			return;
		}
	...
	public boolean handler_player_out_card(Table_NanNing table, int seat_index, int card) {
		...
		if (_type == GameConstants_NanNing.DispatchCard_Type_Tian_Hu) {
			// 出牌
			table.exe_out_card(_seat_index, card, GameConstants_NanNing.OutCard_Type_Di_Hu);
		} else {
			// 出牌
	...
	public boolean handler_operate_card(Table_NanNing table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: {
			...
			if (_type == GameConstants.GANG_TYPE_JIE_GANG) {
				int provide_player = table.GRR._weave_items[_seat_index][table.GRR._weave_count[_seat_index] - 1].provide_player;
				table.process_chi_hu_player_score(_seat_index, provide_player, operate_card, false);
			} else {
