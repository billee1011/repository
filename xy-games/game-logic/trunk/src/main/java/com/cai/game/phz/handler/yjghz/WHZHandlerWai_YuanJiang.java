...
public class WHZHandlerWai_YuanJiang extends PHZHandlerWai<YuanJiangGHZTable> {

	private GangCardResult m_gangCardResult;

	public WHZHandlerWai_YuanJiang() {
	...
	public void exe(YuanJiangGHZTable table) {
		...
		table.estimate_player_chipeng_qing_piao_respond(_seat_index, _seat_index, _card);
		// 歪后判断胡牌
		...
		int action_hu = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
				table.GRR._weave_count[_seat_index], _seat_index, _seat_index, _card, chr, card_type, false);// 自摸
		...
		for (int p = 0; p < table.getTablePlayerNumber(); p++) {
			...
			if (table._playerStatus[i].has_action()) {
				if (table._playerStatus[i].is_respone()) {
					...
					// 获取最大的动作的优先级
					cbUserActionRank = table._logic.get_action_list_rank_yiyang(table._playerStatus[i]._action_count, table._playerStatus[i]._action,
							i, _seat_index) + table.getTablePlayerNumber() - p;
				}

				if (table._playerStatus[target_player].is_respone()) {
					// 获取已经执行的动作的优先级
					cbTargetActionRank = table._logic.get_action_rank_yiyang(table._playerStatus[target_player].get_perform(), target_player,
							_seat_index) + target_p;
					cbActionRank[i] = cbUserActionRank;
				} else {
					// 获取最大的动作的优先级
					cbTargetActionRank = table._logic.get_action_list_rank_yiyang(table._playerStatus[target_player]._action_count,
