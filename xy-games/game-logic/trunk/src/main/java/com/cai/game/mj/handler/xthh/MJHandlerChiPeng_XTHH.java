...
public class MJHandlerChiPeng_XTHH extends MJHandlerChiPeng<MJTable> {
	...
	public void exe(MJTable table) {
		//
		if (_type != GameConstants.CHI_PENG_TYPE_CHAO_TIAN) {
			// 组合扑克
		...
		if (table.GRR._left_card_count > 0) {
			// 只检测手上的牌能不能杠
			int cbActionMask = table._logic.analyse_gang_card_all_xthh(table, _seat_index, table.GRR._cards_index[_seat_index],
		...
		for (int i = 0; i < GameConstants.MAX_INDEX - GameConstants.CARD_FENG_COUNT - GameConstants.CARD_HUA_COUNT; i++) {
			...
			if (count > 0) {
				...
				// 检查打出哪一张牌后可以听牌
				table._playerStatus[_seat_index]._hu_out_card_ting_count[ting_count] = table.get_xthh_ting_card(
		...
		table.operate_chi_hu_cards(_seat_index, 1, new int[] { 0 });
