...
public class WHZHandlerQing_YuanJiang extends PHZHandlerLiu<YuanJiangGHZTable> {

	private GangCardResult m_gangCardResult;

	public WHZHandlerQing_YuanJiang() {
	...
	public void exe(YuanJiangGHZTable table) {
		...
		if (_action == GameConstants_YJGHZ.WIK_QING_NEI) {
			// 组合扑克
			...
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				if (table.GRR._weave_items[_seat_index][i].weave_kind == GameConstants_YJGHZ.WIK_WAI
			...
			if (wIndex == -1) {
				table.log_player_error(_seat_index, "请牌出错");
				return;
			}
			table.GRR._weave_items[_seat_index][wIndex].public_card = 1;
			table.GRR._weave_items[_seat_index][wIndex].center_card = _card;
