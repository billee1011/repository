...
public class MJHandlerGang_YongZhou extends MJHandlerGang<MJTable_YongZhou> {
    @Override
    public void exe(MJTable_YongZhou table) {
        ...
        // 检查对这个杠有没有 胡
        boolean bAroseAction = bAroseAction = table.estimate_gang_respond(_seat_index, _center_card);
     ...
     */
    @Override
    public boolean handler_operate_card(MJTable_YongZhou table, int seat_index, int operate_code, int operate_card) {
        // 抢杠胡
        ...
        if (jie_pao_count > 0) {
            ...
            for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
                ...
                if (jie_pao_count == 1) {
                    table.set_niao_card(i, GameConstants.INVALID_VALUE, true, 0);// 抓鸟
                }
     ...
     ***/
    @Override
    protected boolean exe_gang(MJTable_YongZhou table) {
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HUNAN_GANG_SUI_HU)) {
                for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                    if (i == _seat_index)
                        continue;
