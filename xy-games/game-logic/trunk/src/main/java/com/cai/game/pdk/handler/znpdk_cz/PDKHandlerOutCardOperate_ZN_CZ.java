...
public class PDKHandlerOutCardOperate_ZN_CZ extends PDKHandlerOutCardOperate {
	@Override
	public void exe(PDKTable table) {
		...
		if (next_player == table._zha_niao_palyer) {
		...
		if (0 == table.GRR._card_count[_out_card_player]) {
			...
			if (cbCardType == GameConstants.PDK_CT_BOMB_CARD) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					if (table._out_card_player == table._hong_tao_palyer) {
						if (i == table._zha_niao_palyer) {
							continue;
						}
						table._out_bomb_score[i] -= table._bomb_cell_score * 2;
						table._out_bomb_score[table._zha_niao_palyer] += table._bomb_cell_score;
						table._out_bomb_score[table._out_card_player] += table._bomb_cell_score;
						if (table.matchId == 0) {
							table._player_result.game_score[i] -= table._bomb_cell_score * 2;
							table._player_result.game_score[table._zha_niao_palyer] += table._bomb_cell_score;
	...
	public boolean handler_player_be_in_room(PDKTable table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (table.has_rule(GameConstants.GAME_RULE_DISPLAY_CARD)) {
				if (table._zha_niao_palyer == i) {
					tableResponse_pdk.addCardCount(table.GRR._card_count[table._hong_tao_palyer]);
				} else {
			...
			if (i == seat_index) {
				if (table._zha_niao_palyer == i) {
					for (int j = 0; j < table.GRR._card_count[table._hong_tao_palyer]; j++) {
						cards_card.addItem(table.GRR._cards_data[table._hong_tao_palyer][j]);
					}
				} else {
		...
		tableResponse_pdk.setZhaNiaoPlayer(table._zha_niao_palyer);
		tableResponse_pdk.setMagicCard(GameConstants.INVALID_CARD);
		tableResponse_pdk.setHongTaoPlayer(table._hong_tao_palyer);
	...
	public void no_out_card(PDKTable table) {
		...
		if (table._current_player == table._out_card_player) {
			...
			if (!table._logic.SearchOutCard(table.GRR._cards_data[next_player], table.GRR._card_count[next_player],
					table._turn_out_card_data, table._turn_out_card_count)) {
