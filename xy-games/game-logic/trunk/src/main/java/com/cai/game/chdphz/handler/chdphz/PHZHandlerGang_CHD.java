package com.cai.game.chdphz.handler.chdphz;
...
public class PHZHandlerGang_CHD extends PHZHandlerGang<CHDPHZTable> {
...
//			}
			...
			table.process_chi_hu_player_score_phz_chd(_seat_index, _provide_player, operate_card, true);
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_phz_chd(target_player,_provide_player, _center_card, false);
	        ...
	        if(pai_count == 0)
	        {
	        	...
	        	if(all_hu_xi>=15)
	        	{
					...
					if (hong_pai_count >= 10) {
						...
						chr.opr_or(GameConstants.CHR_TEN_HONG_PAI_CHD);
					}
					if(table.has_rule(GameConstants.GAME_RULE_SI_QI_HONG_CHD))
						if(hong_pai_count == 4 || hong_pai_count == 7){
							chr.opr_or(GameConstants.CHR_SI_QI_CHD);
						}
					if (hong_pai_count == 1) {
						chr.opr_or(GameConstants.CHR_ONE_HONG_CHD);
					}
					if (hei_pai_count == all_cards_count) {
						chr.opr_or(GameConstants.CHR_ALL_HEI_CHD);
					}
					if (table.GRR._left_card_count == 0) {
						chr.opr_or(GameConstants.CHR_HAI_HU_CHD);
					}
					if (table._da_pai_count[_seat_index] >= 18) {
						chr.opr_or(GameConstants.CHR_DA_HU_CHD);
					}
					if (table._xiao_pai_count[_seat_index] >= 16) {
						chr.opr_or(GameConstants.CHR_XIAO_HU_CHD);
					}
					if (table._logic.calculate_pengpeng_count(table._hu_weave_items[_seat_index], table._hu_weave_count[_seat_index])) {
						chr.opr_or(GameConstants.CHR_DUI_ZI_HU_CHD);
					}
					if(table.has_rule(GameConstants.GAME_RULE_MAN_YUAN_HUA))
					{
						if(is_man_yuan_hua == true)
						{
							chr.opr_or(GameConstants.CHR_MAN_YUAN_HUA_CHD);
						}
					}
		
					if (table.has_rule(GameConstants.GAME_RULE_HHX_CHD)) {
						if (hang_hang_xi_count == 7) {
							chr.opr_or(GameConstants.CHR_HANG_HANG_XI_CHD);
						}
					}
					if (table.has_rule(GameConstants.GAME_RULE_TUAN_CHD)) {
						...
						if (table._tuan_yuan_count[_seat_index] > 0) {
							chr.opr_or(GameConstants.CHR_TUAN_CHD);
						}
					}
					if(table.has_rule(GameConstants.GAME_RULE_TUAN_YUAN_CHD)&&table._logic.is_calculate_tuan_yuan(table._hu_weave_items[_seat_index], table._hu_weave_count[_seat_index]))
					{
						chr.opr_or(GameConstants.CHR_TUAN_YUAN_CHD);
					}
					if(table.has_rule(GameConstants.GAME_RULE_HFT_CHD))
					{
						int count = table._logic.is_hong(table._hu_weave_items[_seat_index], table._hu_weave_count[_seat_index]); 
						if(count== 3){
							chr.opr_or(GameConstants.CHR_HONG_FAN_TIAN_CHD);
						}
						else if(count == 1){
							chr.opr_or(GameConstants.CHR_MTH_DA_CHD);
						}
						else if(count == 2){
							chr.opr_or(GameConstants.CHR_MTH_XIAO_CHD);
						}
					}
					else if(table.has_rule(GameConstants.GAME_RULE_MTH_CHD))
					{
						int count = table._logic.is_hong(table._hu_weave_items[_seat_index], table._hu_weave_count[_seat_index]); 
						if(count == 1){
							chr.opr_or(GameConstants.CHR_MTH_DA_CHD);
					...
					if(table.has_rule(GameConstants.GAME_RULE_DIAN_DENG_CHD))
					{
						if(table._dian_deng_count >=1)
						{
							chr.opr_or_long(GameConstants.CHR_HUANG_FAN_CHD);
						}
					}
					else 
					...
					if(table.has_rule(GameConstants.GAME_RULE_GAI_CHD))
					{
						if (table.GRR._left_card_count == 18 ) {
							...
							chr.opr_or(GameConstants.CHR_GAI_CHD);
