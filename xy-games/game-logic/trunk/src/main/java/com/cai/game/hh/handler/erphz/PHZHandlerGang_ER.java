...
public class PHZHandlerGang_ER extends HHHandlerGang<HHTable_ER> {
	...
	public boolean handler_operate_card(HHTable_ER table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		if (playerStatus.has_action() == false) {
			table.log_player_error(seat_index, "HHHandlerGang_CS出牌,玩家操作已失效");
			return false;
		}

		// 是否已经响应
		if (playerStatus.is_respone()) {
			table.log_player_error(seat_index, "HHHandlerGang_CS出牌,玩家已操作");
			return false;
		}

		if ((operate_code != GameConstants.WIK_NULL) && (operate_code != GameConstants.WIK_ZI_MO))// 没有这个操作动作
		{
			table.log_player_error(seat_index, "HHHandlerGang_CS出牌操作,没有动作");
			return false;
		}

		if ((operate_code != GameConstants.WIK_NULL) && (operate_card != _center_card)) {
			table.log_player_error(seat_index, "HHHandlerGang_CS出牌操作,操作牌对象出错");
			return false;
		}
		...
		case GameConstants.WIK_NULL: {
			...
			if ((table._is_xiang_gong[_seat_index] == false) && (table._long_count[_seat_index] == 1 || GameConstants.SAO_TYPE_MINE_SAO == _type)) {
				...
				if (pai_count == 0) {
					...
					table.log_info(_seat_index +  "扫和提龙出牌状态");
				}
	 ...
	 * 
	 ***/
	protected boolean exe_gang(HHTable_ER table) {
		...
		if (GameConstants.PAO_TYPE_AN_LONG == _type) {
			...
			if (cbWeaveIndex == -1) {
				table.log_info(_seat_index+ "杠牌出错");
				return false;
			}
