...
public class MJHandlerChiPeng_JangSu_ZZ extends MJHandlerChiPeng<MJTable_JangSu_ZZ> {
	private GangCardResult m_gangCardResult;

	public MJHandlerChiPeng_JangSu_ZZ() {
	...
	public void exe(MJTable_JangSu_ZZ table) {
		...
		if (table._logic.is_magic_card(_card)) {
			...
			table.operate_effect_action(_seat_index, GameConstants.Effect_Action_Other, 0, effect_indexs, 1, GameConstants.INVALID_SEAT);
		}
	...
	public void PengtoGang(MJTable_JangSu_ZZ table) {
	 ...
	 */
	@Override
	public boolean handler_player_out_card(MJTable_JangSu_ZZ table, int seat_index, int card) {
		...
		for (int index = 0; index < table.getTablePlayerNumber(); index++) {
			...
			if (dacard_count > 0) {
				...
				if (outcard_da_count == 0) {
					...
					table.operate_effect_action(index, GameConstants.Effect_Action_Other, 0, effect_indexs, 1, seat_index);
				}
