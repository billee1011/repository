...
public class WSKGameLogic_DCTS extends WSKGameLogic {

	public WSKGameLogic_DCTS() {
	...
	public int GetCardType(int cbCardData[], int cbCardCount) {
		...
		if (cbCardCount == 4 && card_index.card_index[13] + card_index.card_index[14] == cbCardCount) {
			return WSKConstants.DCTS_CT_KING_BOMB;
		}

		if (card_index.card_index[this.switch_card_to_idnex(cbCardData_temp[cbCardCount - 1])] == cbCardCount) {
			// 所有牌都为同一种牌
			if (cbCardCount == 1) {
				return WSKConstants.DCTS_CT_SINGLE;
			}
			if (cbCardCount == 2) {
				return WSKConstants.DCTS_CT_DOUBLE;
			}
			if (cbCardCount == 3) {
				return WSKConstants.DCTS_CT_THREE;
			}
			return WSKConstants.DCTS_CT_BOMB;
		}

		// 顺子
		if (cbCardCount >= 5 && this.has_rule(WSKConstants.GAME_RULE_SING_LINK_DCTS)) {
			if (this.is_link(card_index, 1, cbCardCount) && wang_count == 0) {
				return WSKConstants.DCTS_CT_SINGLE_LINK;
			}
		}
		...
		if (cbCardCount % 2 == 0) {
			...
			if (this.is_link(card_index, 2, cbCardCount / 2) && wang_count == 0) {
				return WSKConstants.DCTS_CT_DOUBLE_LINK;
			}
		}
