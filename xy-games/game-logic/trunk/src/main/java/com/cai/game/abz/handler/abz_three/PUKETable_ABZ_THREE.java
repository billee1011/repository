package com.cai.game.abz.handler.abz_three;
...
public class PUKETable_ABZ_THREE extends PUKETable {
	...
	public PUKETable_ABZ_THREE() {
		super(PUKEType.GAME_TYPE_PUKE_ABZ_THREE);
	}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			_select_bao[i] = -1;
			_change_action[i] = -1;
			_cur_out_card_type[i] = GameConstants.ABZ_CT_ERROR;
			_chang_card[i] = GameConstants.INVALID_CARD;
		}
		_cur_banker = GameConstants.INVALID_SEAT;
		_handler_call_banker = new PUKEHandlerCallBanker_ABZ_THREE();
		_handler_out_card_operate = new PUKEHandlerOutCardOperate_ABZ_THREE();
		_handler_change_card = new PUKEHandlerChangeCard_ABZ_THREE();
	...
	protected void test_cards() {
		...
		int cards[] = new int[] { 78, 2, 21, 79, 34, 22, 17, 35, 61, 43, 5, 26, 2, 10, 40, 40, 20, 61, 1, 11, 45, 52,
				21, 6, 57, 33, 43, 50, 38, 45, 38, 36, 44, 28, 51, 60, 10, 78, 13, 11, 18, 39, 26, 20, 53, 23, 27, 6,
				42, 29, 39, 24, 41, 55, 3, 25, 55, 51, 59, 58, 19, 12, 56, 12, 29, 1, 24, 34, 37, 44, 23, 54, 8, 50, 57,
				54, 19, 27, 25, 4, 3, 13, 42, 8, 4, 58, 59, 60, 7, 35, 28, 49, 56, 53, 79, 9, 9, 22, 33, 37, 49, 5, 52,
				7, 41, 17, 18, 36 };
	...
	public int get_hand_card_count_max() {
		return GameConstants.ABZ_MAX_COUT;
	}
	...
	protected boolean on_game_start() {
		...
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				} else if (has_rule(GameConstants.GAME_RULE_ABZ_YU_PAI_XIANSHI_NO)) {
	...
	public boolean handler_operate_out_card_mul(int get_seat_index, List<Integer> list, int card_count,
			int b_out_card) {
		if (this._handler_out_card_operate != null && this.GRR._banker_player != GameConstants.INVALID_SEAT) {
