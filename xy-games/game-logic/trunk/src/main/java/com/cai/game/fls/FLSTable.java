...
import com.cai.game.fls.handler.FLSHandlerXiaoHu;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class FLSTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	public FLSHandlerXiaoHu _handler_xiao_hu;
	...
	public int send_count_when_started = 0;
	public boolean dispatch_after_xiao_hu = false;
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_FLS_LX) || is_mj_type(GameConstants.GAME_TYPE_FLS_LX_THREE)
				|| is_mj_type(GameConstants.GAME_TYPE_FLS_LX_TWO)) {
			// 初始化基础牌局handler
		...
		_handler_xiao_hu = new FLSHandlerXiaoHu();
	...
	public boolean reset_init_data() {
		...
		send_count_when_started = 0;
		dispatch_after_xiao_hu = false;

		if (is_mj_type(GameConstants.GAME_TYPE_FLS_LX_DP)) {
			GRR = new GameRoundRecord(GameConstants.MAX_WEAVE_FLS, GameConstants.MAX_FLS_COUNT, getMaxIndex());
		} else {
			GRR = new GameRoundRecord(GameConstants.MAX_WEAVE_FLS, GameConstants.MAX_FLS_COUNT, getMaxIndex());
		}
		GRR.setRoom(this);
	...
	protected boolean on_handler_game_start() {
		...
		} else if (is_mj_type(GameConstants.GAME_TYPE_FLS_LX_TWO)) {
			_repertory_card = new int[GameConstants.CARD_COUNT_FLS_LX];
			shuffle(_repertory_card, GameConstants.CARD_DATA_FLS_LX);
		}
		...
		try {
			if (!is_sys() && GRR != null) {
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					...
					if (number == 2) {
						MongoDBServiceImpl.getInstance().card_log(this.get_players()[i], ECardType.anLong2, "", 0, 0l, this.getRoom_id());
					}
				}
			}
	...
	public void runnable_xiao_hu() {
		...
		dispatch_after_xiao_hu = true;
		exe_dispatch_card(GRR._banker_player, GameConstants.WIK_NULL, 0);
	}
	...
	public void game_start_real_twenty() {
		...
		// 检测听牌
		for (int i = 0; i < playerCount; i++) {
			this._playerStatus[i]._hu_card_count = this.get_fls_ting_card_twenty(this._playerStatus[i]._hu_cards, this.GRR._cards_index[i],
					this.GRR._weave_items[i], this.GRR._weave_count[i], i);
	...
	public void game_start_real() {
		...
		// 检测听牌
		for (int i = 0; i < playerCount; i++) {
			this._playerStatus[i]._hu_card_count = this.get_fls_ting_card(this._playerStatus[i]._hu_cards, this.GRR._cards_index[i],
	...
	public void game_start_real_dp() {
		...
		// 检测听牌
		for (int i = 0; i < playerCount; i++) {
			this._playerStatus[i]._hu_card_count = this.get_fls_ting_card_dp(this._playerStatus[i]._hu_cards, this.GRR._cards_index[i],
					this.GRR._weave_items[i], this.GRR._weave_count[i]);
			if (this._playerStatus[i]._hu_card_count > 0) {
				this.operate_chi_hu_cards(i, this._playerStatus[i]._hu_card_count, this._playerStatus[i]._hu_cards);
			}
	...
	private void shuffle_two(int repertory_card[], int mj_cards[]) {
		_all_card_len = repertory_card.length;
		GRR._left_card_count = _all_card_len;
	 ...
	 * @return
	 */
	public boolean isZhuangDui(int i) {
	...
	private void test_cards() {
		int cards[] = new int[] { 0x01, 0x02, 0x03, 0x011, 0x12, 0x13, 0x21, 0x22, 0x23, 0x31, 0x32, 0x33, 0x71, 0x72, 0x73, 0x51, 0x52, 0x53, 0x53,
				0x01 };
	...
	public boolean handler_game_finish_fls_dp(int seat_index, int reason) {
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL ||
			...
			for (int i = 0; i < count; i++) {
				...
				game_end.addGameScore(GRR._game_score[i] + GRR._start_hu_score[i]);// 放炮的人？
	...
	public RoomInfo.Builder getRoomInfo() {
		...
		int number = getTablePlayerNumber();
		if (number == GameConstants.GAME_PLAYER - 1) {
			beginLeftCard = GameConstants.CARD_COUNT_FLS_LX - 55;
		} else if (number == GameConstants.GAME_PLAYER - 2) {
			beginLeftCard = GameConstants.CARD_COUNT_FLS_LX - 37;
		}
	...
	public int analyse_chi_hu_card(int cards_index[], WeaveItem weaveItems[], int weave_count, int cur_card, ChiHuRight chiHuRight, int card_type,
		...
		if (GameConstants.GAME_TYPE_FLS_LX == _game_type_index || GameConstants.GAME_TYPE_FLS_LX_THREE == _game_type_index
				|| GameConstants.GAME_TYPE_FLS_LX_TWO == _game_type_index) {
			...
			return analyse_chi_hu_card_fls_twenty(cards_index, weaveItems, weave_count, cur_card, chiHuRight, card_type, seat_index);
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_fls_dp(int cards_index[], WeaveItem weaveItems[], int weaveCount, int cur_card, ChiHuRight chiHuRight,
			int card_type) {
		// cbCurrentCard一定不为0 !!!!!!!!!
		...
		// 分析扑克
		boolean bValue = _logic.analyse_card(cbCardIndexTemp, weaveItems, weaveCount, analyseItemArray, false, cur_card);
		if (!bValue) {
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_fls(int cards_index[], WeaveItem weaveItems[], int weaveCount, int cur_card, ChiHuRight chiHuRight,
			int card_type) {
		// cbCurrentCard一定不为0 !!!!!!!!!
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_fls_twenty(int cards_index[], WeaveItem weaveItems[], int weaveCount, int cur_card, ChiHuRight chiHuRight,
		...
		// 分析扑克
		boolean bValue = _logic.analyse_card_twenty(cbCardIndexTemp, weaveItems, weaveCount, analyseItemArray, false, cur_card);
		if (!bValue) {
		...
		for (int i = 0; i < analyseItemArray.size(); i++) {
			...
			if (_logic.is_ku_hu(analyseItem) && _logic.is_pengpeng_hu_down(weaveItems, weaveCount) && analyseItem.totalScore > 0) {
	...
	private boolean isScoreEnough(List<AnalyseItem> analyseItemArray, int downScore, int cur_card, int card_type) {
		...
		boolean isZimo = card_type == GameConstants.HU_CARD_TYPE_ZIMO || card_type == GameConstants.HU_CARD_TYPE_GANG_KAI;
		int bestScore = 0;
			...
			for (int i = 0; i < length; i++) {
				...
				case GameConstants.WIK_PENG:
					if (analyseItem.cbCenterCard[i] == 0x01 || analyseItem.cbCenterCard[i] == 0x71) {
						...
						if (analyseItem.cbCenterCard[i] == cur_card && !isZimo && !_logic.isInCardEye(analyseItem, cur_card) && !isInchi) {
	 ...
	 * @return
	 */
	private boolean isScoreEnoughtwenty(List<AnalyseItemTwenty> analyseItemArray, int downScore, int cur_card, int card_type) {
	...
	private int getEyeScoreTwenty(AnalyseItemTwenty analyseItem, int cur_card) {
		if (analyseItem.eyeDui) {
			...
			for (int i = 0; i < analyseItem.cbCardEye.length; i++) {
				if (analyseItem.cbCardEye[i] == 0x01 || analyseItem.cbCardEye[i] == 0x71 || analyseItem.cbCardEye[i] == 0x01
						|| analyseItem.cbCardEye[i] == 0x71) {
					return 4;
				}
			}
		}
	 ...
	 * @return
	 */
	private boolean isEyeScoreFull(AnalyseItem analyseItem, boolean hasJin, boolean hasKan, int handScore, int downScore, int cur_card,
			int card_type) {
	...
	public int get_fls_ting_card_dp(int[] cards, int cards_index[], WeaveItem weaveItem[], int cbWeaveCount) {
		...
		for (int i = 0; i < mj_count; i++) {
			...
			chr.set_empty();
			if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card_fls_dp(cbCardIndexTemp, weaveItem, cbWeaveCount, cbCurrentCard, chr,
	...
	public int get_fls_ting_card(int[] cards, int cards_index[], WeaveItem weaveItem[], int cbWeaveCount) {
		...
		for (int i = 0; i < mj_count; i++) {
			...
			cbCurrentCard = _logic.switch_to_card_data(i);
			chr.set_empty();
			if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card_fls(cbCardIndexTemp, weaveItem, cbWeaveCount, cbCurrentCard, chr,
					GameConstants.HU_CARD_TYPE_ZIMO)) {
				cards[count] = cbCurrentCard;
				count++;
			}
	 ...
	 * @return
	 */
	public int get_fls_ting_card_twenty(int[] cards, int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index) {
		...
		for (int i = 0; i < mj_count; i++) {
			...
			cbCurrentCard = _logic.switch_to_card_data(i);
			chr.set_empty();
			if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card_fls_twenty(cbCardIndexTemp, weaveItem, cbWeaveCount, cbCurrentCard, chr,
					...
					if (action != 0) {
						...
						if (GRR._left_card_count > left) {
							...
							if (is_ting == true) {
								...
								can_gang = this.is_fls_ting_card(GRR._cards_index[i], GRR._weave_items[i], GRR._weave_count[i]);
					...
					if (action != 0) {
						...
						if (GRR._left_card_count > left) {
							...
							if (is_ting == true) {
								...
								can_gang = this.is_fls_ting_card_dp(GRR._cards_index[i], GRR._weave_items[i], GRR._weave_count[i]);
	...
	public boolean estimate_gang_fls_respond(int seat_index, int provider, int card, boolean d, boolean check_chi) {
		...
		action = analyse_chi_hu_card(GRR._cards_index[seat_index], GRR._weave_items[seat_index], cbWeaveCount, card, chr,
				GameConstants.HU_CARD_TYPE_GANG_PAO, seat_index);
	...
	public boolean handler_enter_room(Player player) {
		...
		if (super.handler_enter_room(player)) {
			...
			// 俱乐部需要知道准备状态
			if (player.get_seat_index() >= 0 && player.get_seat_index() < _player_ready.length) {
				_player_ready[player.get_seat_index()] = 1;
			}
			return true;
		}
		return false;
	...
	public boolean handler_player_ready(int seat_index, boolean is_cancel) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_FLS_LX_TWENTY) || is_mj_type(GameConstants.GAME_TYPE_FLS_LX_THREE)) {
			number = playerNumber == 0 ? GameConstants.GAME_PLAYER - 1 : playerNumber;
		}

		if (is_mj_type(GameConstants.GAME_TYPE_FLS_LX_TWO)) {
			number = 2;
		}
		...
		if (is_mj_type(GameConstants.GAME_TYPE_FLS_LX_TWO)) {
			playerNumber = GameConstants.GAME_PLAYER - 2;
		}
	...
	public void process_chi_hu_player_operate(int seat_index, int operate_card, boolean rm) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_FLS_LX) || this.is_mj_type(GameConstants.GAME_TYPE_FLS_LX_DP)
				|| is_mj_type(GameConstants.GAME_TYPE_FLS_LX_THREE) || is_mj_type(GameConstants.GAME_TYPE_FLS_LX_TWO)) {
			// 效果
	...
	private void countCardType(ChiHuRight chiHuRight, int seat_index) {
		try {
			...
			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_QING_HU)).is_empty()) {
				wFanShu += 1;
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.qing, "", 0, 0l, this.getRoom_id());
			}
			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_KA_HU)).is_empty()) {
				wFanShu += 1;
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.ka, "", 0, 0l, this.getRoom_id());
			}
			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_TAI_HU)).is_empty()) {
				wFanShu += 1;
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.tai, "", 0, 0l, this.getRoom_id());
			}
			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_KU_HU)).is_empty()) {
				wFanShu += 5;
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.ku, "", 0, 0l, this.getRoom_id());
			}
			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_ZHONG_HU)).is_empty()) {
				wFanShu += 6;
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.zhong, "", 0, 0l, this.getRoom_id());
			}

			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_HEI_HU)).is_empty()) {
				wFanShu += 4;
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.hei, "", 0, 0l, this.getRoom_id());
			}

			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_SHI_DUI)).is_empty()) {
				wFanShu += 10;
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.shidui, "", 0, 0l, this.getRoom_id());
			}
			...
			if (hasHong)
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.hong, "", 0, 0l, this.getRoom_id());
		} catch (Exception e) {
	...
	private void countCardTypefls(ChiHuRight chiHuRight, int seat_index) {
		try {
			int wFanShu = 0;
			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_MENQING)).is_empty() && !(chiHuRight.opr_and(GameConstants.CHR_ZI_MO)).is_empty()) {
				// 门清自摸3分（接炮算小胡，放炮者出1分）；
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.zimomq, "", 0, 0l, this.getRoom_id());
			}

			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_PENGPENGHU)).is_empty()) {
				wFanShu = 3;
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.pengpenghu, "", 0, 0l, this.getRoom_id());
			}

			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_MANTIAN_FEI)).is_empty()) {
				wFanShu = 3;
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.mantianfei, "", 0, 0l, this.getRoom_id());
			}

			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_GANGSHANGHUA)).is_empty()) {
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.gangshanghua, "", 0, 0l, this.getRoom_id());
			}
			...
			if (!(chiHuRight.opr_and(GameConstants.CHR_FLS_HAIDI)).is_empty() && !(chiHuRight.opr_and(GameConstants.CHR_ZI_MO)).is_empty()) {
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.haidi, "", 0, 0l, this.getRoom_id());
			}
	 ...
	 * @param zimo
	 */
	public void process_chi_hu_player_score_fls_twenty(int seat_index, int provide_index, int operate_card, boolean zimo) {
		GRR._chi_hu_card[seat_index][0] = operate_card;
	 ...
	 */
	//
	public boolean operate_out_card(int seat_index, int count, int cards[], int type, int to_player, boolean has_action) {
		if (seat_index == GameConstants.INVALID_SEAT) {
			return false;
		}
		//// 发送玩家出牌 201 =玩家出牌(CMD_OutCard: operate_player,operate_card)
		...
		if (is_mj_type(GameConstants.GAME_TYPE_HENAN_AY)) {
			...
			SysParamModel sysParamModel104 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(gameId).get(1104);
	...
	public boolean operate_effect_action(int seat_index, int effect_type, int effect_count, long effect_indexs[], int time, int to_player,
			int center_card, boolean is_public, boolean add_to_record) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		...
		if (effect_type == GameConstants.EFFECT_ACTION_TYPE_HU) {
			if (is_mj_type(GameConstants.GAME_TYPE_FLS_LX_TWENTY)) {
				...
				for (int i = 0; i < effect_count; i++) {
					if (effect_indexs[i] == GameConstants.CHR_FLS_HONG_HU4 || effect_indexs[i] == GameConstants.CHR_FLS_HONG_HU5
							|| effect_indexs[i] == GameConstants.CHR_FLS_HONG_HU6 || effect_indexs[i] == GameConstants.CHR_FLS_HONG_HU7) {
						continue;
					}
					realEffect++;
	...
	public int get_chi_hu_action_rank(ChiHuRight chiHuRight) {
		int score = 0;
		if (chiHuRight.opr_and(GameConstants.CHR_FLS_QUE_YI_SE).is_empty() == false)
			score += 2;
		if (chiHuRight.opr_and(GameConstants.CHR_FLS_BAN_BAN_HU).is_empty() == false)
			score += 2;
		if (chiHuRight.opr_and(GameConstants.CHR_FLS_LIU_LIU_SHUN).is_empty() == false)
			score += 2;
		if (chiHuRight.opr_and(GameConstants.CHR_FLS_DA_HONG_SI_XI).is_empty() == false)
			score += 2;
		return score;
	}
	 ...
	 */
	public boolean operate_effect_action(int seat_index, int effect_type, int effect_count, long effect_indexs[], int time, int to_player,
			int center_card, boolean is_public) {
		// 记录回放
		if (_handler == _handler_xiao_hu) {
			record_effect_action(seat_index, effect_type, effect_count, effect_indexs, 1);
		} else if (effect_type == GameConstants.EFFECT_ACTION_TYPE_HU) {
			record_effect_action(seat_index, GameConstants.EFFECT_ACTION_TYPE_HU, 1, new long[] { GameConstants.WIK_ZI_MO }, 1);
		} else {
			record_effect_action(seat_index, effect_type, effect_count, effect_indexs, 1);
		}
		return operate_effect_action(seat_index, effect_type, effect_count, effect_indexs, time, to_player, center_card, is_public, true);
	}
	...
	public boolean operate_player_get_card(int seat_index, int count, int cards[], int to_player) {
		...
		int flashTime = 150;
		int gameId = this.getGame_id() == 0 ? 5 : this.getGame_id();
		SysParamModel sysParamModel108 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(gameId).get(1108);
	...
	public boolean operate_player_cards(int seat_index, int card_count, int cards[], int weave_count, WeaveItem weaveitems[]) {
		...
		if (weave_count > 0) {
			for (int j = 0; j < weave_count; j++) {
				...
				if ((weaveitems[j].weave_kind == GameConstants.WIK_GANG || weaveitems[j].weave_kind == GameConstants.WIK_ZHAO)
	...
	private void progress_banker_select() {
		...
		if (is_sys() || (isClubRoom() && _cur_round == 1)) {// 金币场 随机庄家
			Random random = new Random();//
			int rand = random.nextInt(6) + 1 + random.nextInt(6) + 1;
			_banker_select = rand % getTablePlayerNumber();//
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < l; j++) {
				...
				if (GRR._chi_hu_rights[i].is_valid()) {
					...
					if (type == GameConstants.CHR_FLS_MENQING && !(GRR._chi_hu_rights[i].opr_and(GameConstants.CHR_ZI_MO).is_empty())) {
						des += " 门清";
					}
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < l; j++) {
				...
				if (GRR._chi_hu_rights[i].is_valid()) {
					...
					if (type == GameConstants.CHR_FLS_HONG_HU7 || type == GameConstants.CHR_FLS_HONG_HU6 || type == GameConstants.CHR_FLS_HONG_HU5
							|| type == GameConstants.CHR_FLS_HONG_HU4) {
	...
	private void set_result_describe() {
		if (this.is_mj_type(GameConstants.GAME_TYPE_FLS_LX) || is_mj_type(GameConstants.GAME_TYPE_FLS_LX_TWO)
				|| this.is_mj_type(GameConstants.GAME_TYPE_FLS_LX_DP) || is_mj_type(GameConstants.GAME_TYPE_FLS_LX_THREE)) {
	...
	public boolean exe_xiao_hu() {
		change_handler(_handler_xiao_hu);
		this._handler_xiao_hu.reset_status(GRR._banker_player);
		this._handler.exe(this);
	...
	public boolean handler_request_trustee(int get_seat_index, boolean isTrustee, int Trustee_type) {
		...
		if (is_match() || isCoinRoom() || isClubMatch()) {
			...
			int status = _playerStatus[get_seat_index].get_status();

			if (istrustee[get_seat_index] && this._current_player == get_seat_index) {
				if (status == GameConstants.Player_Status_OUT_CARD) {
					int card = AiGameLogic.get_card(this, get_seat_index);
					if (card != 0) {
						handler_player_out_card(get_seat_index, card);
					}
				} else if (status == GameConstants.Player_Status_OPR_CARD) {
					AiGameLogic.ai_operate_card(this, get_seat_index);
				}
	...
	public int getTablePlayerNumber() {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_FLS_LX_THREE)) {
			return GameConstants.GAME_PLAYER_HH;
		}
		if (is_mj_type(GameConstants.GAME_TYPE_FLS_LX_TWO)) {
			return 2;
	...
	public void be_in_room_trustee(int get_seat_index) {
		if (is_match() || isCoinRoom() || isClubMatch()) {
			...
			send_response_to_other(get_seat_index, roomResponse);
		} else {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			roomResponse.setType(MsgConstants.RESPONSE_IS_TRUSTEE);
			roomResponse.setOperatePlayer(get_seat_index);
	...
	public int analyse_chi_hu_card_xiaohu(int cards_index[], ChiHuRight chiHuRight, int seat_index) {
		...
		boolean has_da_hong_si_xi = false;

		if (has_rule(GameConstants.GAME_RULE_LIXIANG_FLS_QUE_YI_SE))
			has_que_yi_se = check_que_yi_se(cbCardIndexTemp, chiHuRight);
		if (has_rule(GameConstants.GAME_RULE_LIXIANG_FLS_BAN_BAN_HU))
			has_ban_ban_hu = check_ban_ban_hu(cbCardIndexTemp, chiHuRight);
		if (has_rule(GameConstants.GAME_RULE_LIXIANG_FLS_LIU_LIU_SHUN))
			has_liu_liu_shun = check_liu_liu_shun(cbCardIndexTemp, chiHuRight);
		if (has_rule(GameConstants.GAME_RULE_LIXIANG_FLS_DA_HONG_SI_XI))
			has_da_hong_si_xi = check_da_hong_si_xi(cbCardIndexTemp, chiHuRight);

		if (has_que_yi_se || has_ban_ban_hu || has_liu_liu_shun || has_da_hong_si_xi)
			cbChiHuKind = GameConstants.WIK_CHI_HU;
	...
	public boolean check_que_yi_se(int cards_index[], ChiHuRight chr) {
		...
		if (count < 3) {
			chr._show_all = true;
			chr.opr_or(GameConstants.CHR_FLS_QUE_YI_SE);
			return true;
		} else {
			return false;
		}
	...
	public boolean check_ban_ban_hu(int cards_index[], ChiHuRight chr) {
		...
		if (result == true) {
			chr._show_all = true;
			chr.opr_or(GameConstants.CHR_FLS_BAN_BAN_HU);
		}

		return result;
	}

	public boolean check_liu_liu_shun(int cards_index[], ChiHuRight chr) {
		int index_a = -1;
		int index_b = -1;
		int index_c = -1;
		for (int i = 0; i < cards_index.length; i++) {
			...
			if (index_a != -1 && index_b != -1 && index_c != -1) {
				...
				chr._index_zt_lls_1 = index_c;

				chr.opr_or(GameConstants.CHR_FLS_LIU_LIU_SHUN);
				return true;
			}
		}
	...
	public boolean check_da_hong_si_xi(int cards_index[], ChiHuRight chr) {
		for (int i = 0; i < cards_index.length; i++) {
			if (cards_index[i] == 4) {
				int card_value = _logic.get_card_value(_logic.switch_to_card_data(i));
				if (card_value == 1) {
					chr._index_da_si_xi = i;
					chr.opr_or(GameConstants.CHR_FLS_DA_HONG_SI_XI);
					return true;
				}
			}
		}
	...
	public boolean handler_create_room(Player player, int type, int maxNumber) {
		boolean r = super.handler_create_room(player, type, maxNumber);

		// 俱乐部需要知道准备状态
		if (player.get_seat_index() < 0 || player.get_seat_index() >= _player_ready.length) {
			int seatIndex = getPlayerIndex(player.getAccount_id());
			if (seatIndex != GameConstants.INVALID_SEAT) {
				_player_ready[seatIndex] = 1;
			}
		} else {
			_player_ready[player.get_seat_index()] = 1;
		}
