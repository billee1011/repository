...
public class HandlerChiPeng_QuanZhou extends MJHandlerChiPeng<Table_QuanZhou> {
	...
	public HandlerChiPeng_QuanZhou() {
...
//			table.GRR._weave_items[_seat_index][wIndex].is_vavild = false;
		...
		if (ting_count > 0) {
			...
			int[] copy_tmp_cards = Arrays.copyOf(tmp_cards, Constants_MJ_QUAN_ZHOU.MAX_HAND_CARD_COUNT);
		...
		if(table.GRR._cards_index[_seat_index][table._logic.get_magic_card_index(0)] == 3){
			curPlayerStatus.add_action(Constants_MJ_QUAN_ZHOU.WIK_SAN_JIN_DAO);
			curPlayerStatus.add_action_card(1, 0, Constants_MJ_QUAN_ZHOU.WIK_SAN_JIN_DAO, _seat_index);
		}
	...
	public boolean handler_operate_card(Table_QuanZhou table, int seat_index, int operate_code, int operate_card) {
		...
		case Constants_MJ_QUAN_ZHOU.WIK_SAN_JIN_DAO: {
			...
			table.GRR._chi_hu_rights[_seat_index].opr_or(Constants_MJ_QUAN_ZHOU.CHR_THREE_GOLD_INVERTED);
			...
			table.process_chi_hu_player_score(_seat_index, _seat_index, operate_card, false);
