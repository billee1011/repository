...
public class PDKHandlerOutCardOperate_FIFTEENYUEY extends PDKHandlerOutCardOperate<PDK_FIFTEENYUEY_Table> {
	@Override
	public void exe(PDK_FIFTEENYUEY_Table table) {
			...
			if (cbCardType == GameConstants.PDK_CT_BOMB_CARD) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					if (table._hong_tao_palyer == i && table.has_rule(GameConstants.GAME_RULE_ZHADAN_ZHAIAO)
						...
						table._out_bomb_score_zhaniao[i] -= 10;
						table._out_bomb_score_zhaniao[table._out_card_player] += 10;
					} else {
					...
					table._out_bomb_score[table._out_card_player] += 10;

					if (table.matchId == 0) {
						table._player_result.game_score[i] -= 10;
						table._player_result.game_score[table._out_card_player] += 10;
					}
				}
	...
	@Override
	public boolean handler_player_be_in_room(PDK_FIFTEENYUEY_Table table, int seat_index) {
	...
	// 玩家不出
	public void no_out_card(PDK_FIFTEENYUEY_Table table) {
		int can_out_card_data[] = new int[table.get_hand_card_count_max()];
	...
	// 判断玩家出牌合法
	public int adjust_out_card_right(PDK_FIFTEENYUEY_Table table) {
		...
		if (table.has_rule(GameConstants.GAME_RULE_ZHADAN_BUKECHAI)) {
			for (int i = 0; i < _out_card_count; i++) {
				...
				if (table.has_rule(GameConstants.GAME_RULE_KKK_BOOM)
						...
						&& cbCardType != GameConstants.PDK_CT_BOMB_CARD) {
					table.send_error_notify(_out_card_player, 2, "请选择正确的牌型!");
					return GameConstants.PDK_CT_ERROR;
				}
	...
	// 放走包赔
	public boolean fang_zou_bao_pei(PDK_FIFTEENYUEY_Table table) {
	...
	// 必须出最大牌
	public boolean mustmax(PDK_FIFTEENYUEY_Table table) {
	...
	// 自动出牌
	public void adjustAutoOutCard(PDK_FIFTEENYUEY_Table table, int seat_index) {
