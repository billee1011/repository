...
public class MJHandlerChiPeng_HaiNan extends MJHandlerChiPeng<MJTable_HaiNan> {
	...
	public void exe(MJTable_HaiNan table) {
		...
		if (table.getWeaveItemCount(_seat_index,3) == 3) {
			table.operate_effect_action(_seat_index, GameConstants.Effect_Action_Other, 1,
					new long[] { GameConstants_HaiNan.EFFECT_ACTION_OTHER_SAN_DAO_GANG }, 1000,
					GameConstants.INVALID_SEAT);
		} else if (table.getWeaveItemCount(_seat_index,4) == 4) {
			table.operate_effect_action(_seat_index, GameConstants.Effect_Action_Other, 1,
					new long[] { GameConstants_HaiNan.EFFECT_ACTION_OTHER_SI_DAO_GANG }, 1000,
					GameConstants.INVALID_SEAT);
		}
		 ...
		 * 
		 * m_gangCardResult.cbCardCount = 0; // 如果牌堆还有牌，判断能不能杠 if
		 * (table.GRR._left_card_count > 0) { // 只检测手上的牌能不能杠 int cbActionMask =
		 * table._logic.analyse_gang_card_all_new_zz(table.GRR._cards_index[
		 * _seat_index], table.GRR._weave_items[_seat_index],
		 * table.GRR._weave_count[_seat_index], m_gangCardResult, false, -1);
		 * 
		 * if (cbActionMask != 0) {
		 * curPlayerStatus.add_action(GameConstants.WIK_GANG);// 转转就是杠 for (int
		 * i = 0; i < m_gangCardResult.cbCardCount; i++) { // 加上杠
		 * curPlayerStatus.add_gang(m_gangCardResult.cbCardData[i], _seat_index,
		 * m_gangCardResult.isPublic[i]); } } }
		 */

		// 检索手中花牌
//		int[] hua_cards = new int[8];
//		int hua_count = 0;
//		for (int i = 0; i < 8; i++) {
//			int index = table._logic.switch_to_card_index(GameConstants.CARD_CAI_SHENG) + i;
//			if (table.GRR._cards_index[_seat_index][index] > 0) {
//				hua_cards[hua_count++] = table._logic.switch_to_card_data(index);
//			}
//		}
//
//		if (hua_count > 0) {
...
//				int index = table._logic.switch_to_card_index(GameConstants.CARD_CAI_SHENG) + i;
//				if (table.GRR._cards_index[_seat_index][index] > 0) {
//					table.GRR._cards_index[_seat_index][index] = 0;
//				}
//			}
//
//			// 刷新自己手牌
//			int cards1[] = new int[GameConstants.MAX_COUNT];
//			int hand_card_count1 = table._logic.switch_to_cards_data(table.GRR._cards_index[_seat_index], cards1);
//			table.operate_player_cards(_seat_index, hand_card_count1, cards1, 0, null);
//
//			// 显示出牌
//			table.operate_out_card(_seat_index, hua_count, hua_cards, GameConstants.OUT_CARD_TYPE_MID,
//					GameConstants.INVALID_SEAT);
//			table.exe_add_discard(_seat_index, hua_count, hua_cards, false, GameConstants.DELAY_SEND_CARD_DELAY);
//
//			if(table.GRR._left_card_count <= (15+ table.getTablePlayerNumber())){
//				table.liu_ju_count +=hua_count;
//			}
//			// 效果
...
//
//			table.GRR._chi_hu_rights[_seat_index].hua_count += hua_count;// 花牌加一
//			// 发牌
//			table.exe_dispatch_card(_seat_index, GameConstants_HaiNan.HU_CARD_TYPE_BU_HUA,
//					GameConstants.DELAY_AUTO_OUT_CARD_TRUTESS, hua_count);
//			return;
//		}
	 ...
	 */
	@Override
	public boolean handler_player_out_card(MJTable_HaiNan table, int seat_index, int card) {
