 ...
 */
public class Table_HYJD extends AbstractMJTable {

	public HandlerSelectMagic_HYJD handler_select_magic;
	/**
	 * 
	 */
	...
	protected void onInitTable() {
		_handler_chi_peng = new HandlerChiPeng_HYJD();
		_handler_dispath_card = new HandlerDispatchCard_HYJD();
		_handler_gang = new HandlerGang_HYJD();
		_handler_out_card_operate = new HandlerOutCardOperate_HYJD();
		handler_select_magic = new HandlerSelectMagic_HYJD();
	...
	protected void init_shuffle() {
		...
		if (has_rule(GameConstants_HYJD.GAME_RULE_SI_HUA_GUI)) {
			cards_list = Lists.newArrayList();
			_repertory_card = new int[GameConstants_HYJD.CARD_DATA_SI_HUA.length];
			shuffle(_repertory_card, GameConstants_HYJD.CARD_DATA_SI_HUA);

		} else if (has_rule(GameConstants_HYJD.GAME_RULE_BA_HUA_GUI)) {
			cards_list = Lists.newArrayList();
			_repertory_card = new int[GameConstants_HYJD.CARD_DATA_BA_HUA.length];
			shuffle(_repertory_card, GameConstants_HYJD.CARD_DATA_BA_HUA);
		} else {
			cards_list = Lists.newArrayList();
			_repertory_card = new int[GameConstants_HYJD.CARD_DATA_NOT_HUA.length];
			shuffle(_repertory_card, GameConstants_HYJD.CARD_DATA_NOT_HUA);
		}
	}
	...
	public int analyse_chi_hu_card(int[] cards_index, WeaveItem[] weaveItems, int weave_count, int cur_card,
		...
		// 鬼牌信息
		int[] magic_cards_index = new int[GameConstants_HYJD.MAX_MAGIC_CARD];
		...
		if (has_rule(GameConstants_HYJD.GAME_RULE_HU_QIDUI)) {
			is_qi_xiao_dui = is_qi_xiao_dui(cards_index, weaveItems, weave_count, cur_card) != GameConstants.WIK_NULL;
		}
		if (is_qi_xiao_dui) {
			chiHuRight.opr_or(GameConstants_HYJD.CHR_QI_XIAO_DUI);
		}

		if (!can_win && !is_qi_xiao_dui) {
			chiHuRight.set_empty();
			return GameConstants.WIK_NULL;
		}

		// 花吊
		boolean is_hua_diao = false;
		if (_logic.is_valid_card(cur_card)) {
			int count = 0;
			for (int i = 0; i < GameConstants.MAX_INDEX; i++) {
			...
			if (count == GameConstants.MAX_INDEX) {
				// 花吊花
				if (_logic.is_magic_card(cur_card)) {
					chiHuRight.opr_or(GameConstants_HYJD.CHR_HUA_DIAO_HUA);
				} else {
					chiHuRight.opr_or(GameConstants_HYJD.CHR_HUA_DAN_DIAO);
				}
				is_hua_diao = true;
			}
		}

		if (!is_qi_xiao_dui && !is_hua_diao) {
			chiHuRight.opr_or(GameConstants_HYJD.CHR_PING_HU);
		}
		if (magic_count == 0 && has_rule(GameConstants_HYJD.GAME_RULE_WUGUI_DOUBLE)) {
			chiHuRight.opr_or(GameConstants_HYJD.CHR_WU_GUI_JIA_BEI);
		}
	...
	public void process_chi_hu_player_score(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		di_fen = get_hu_pai_xing_fen(chr, seat_index) * GameConstants_HYJD.CELL_SCORE;
		...
		// // 买马
		// if (has_rule(GameConstants_HYJD.GAME_RULE_MAI_MA)) {
		// // 中马数
		// int maCount = GRR._player_niao_count[seat_index];
		// // 马跟底分
		// if (has_rule(GameConstants_HYJD.GAME_RULE_MAI_MA)) {
		// for (int i = 0; i < getTablePlayerNumber(); i++) {
		// if (i == seat_index) {
		// continue;
		// }
		// GRR._game_score[i] -= total_socre * maCount;
		// GRR._game_score[seat_index] += total_socre * maCount;
		// }
		// }
		// // 马跟杠分
		// float scores[][][] = new
		// float[GameConstants.GAME_PLAYER][GameConstants.MAX_WEAVE][GameConstants.GAME_PLAYER];
		// if (has_rule(GameConstants_HYJD.GAME_RULE_MAI_MA)) {
		// for (int i = 0; i < getTablePlayerNumber(); i++) {
		// for (int j = 0; j < GRR._gang_score[i].gang_count; j++) {
		// for (int k = 0; k < getTablePlayerNumber(); k++) {
		// scores[i][j][k] = GRR._gang_score[i].scores[j][k] * maCount;
		// GRR._game_score[seat_index] += scores[i][j][k];
		// }
		// }
		// }
		// }
	...
	private void cal_ma_gen_di_fen_and_gang_fen(int seat_index, int provide_player, boolean is_bao, int num) {
			...
			for (int j = 0; j < GRR._gang_score[i].gang_count; j++) {
				for (int k = 0; k < this.getTablePlayerNumber(); k++) {
					if (i == seat_index) {
						...
						if (has_rule(GameConstants_HYJD.GAME_RULE_MA_GEN_GANG)) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			// 马跟底分
			if (has_rule(GameConstants_HYJD.GAME_RULE_MA_GEN_DIFEN)) {
				only_ma = false;
				if (i == seat_index) {
	...
	private int get_hu_pai_xing_fen(ChiHuRight chr, int seat_index) {
		...
		if (!chr.opr_and(GameConstants_HYJD.CHR_HUA_DIAO_HUA).is_empty()) {
			di_fen = 4;
		} else if (!chr.opr_and(GameConstants_HYJD.CHR_HUA_DAN_DIAO).is_empty()) {
			di_fen = 2;
		} else if (!chr.opr_and(GameConstants_HYJD.CHR_QI_XIAO_DUI).is_empty()) {
			di_fen = 1;
		} else if (!chr.opr_and(GameConstants_HYJD.CHR_PING_HU).is_empty()) {
			di_fen = 1;
		}
		// 无鬼加倍
		if (has_rule(GameConstants_HYJD.GAME_RULE_WUGUI_DOUBLE)) {
			if (!chr.opr_and(GameConstants_HYJD.CHR_WU_GUI_JIA_BEI).is_empty()) {
				di_fen = di_fen * 2;
			}
		}
		return di_fen;
		...
		for (int player = 0; player < this.getTablePlayerNumber(); player++) {
			...
			for (int typeIndex = 0; typeIndex < chrTypes; typeIndex++) {
				...
				if (GRR._chi_hu_rights[player].is_valid()) {
					...
					// result.append(" 自摸");
					// break;
					if (type == GameConstants_HYJD.CHR_HUA_DIAO_HUA) {
						result.append(" 花吊花");
					} else if (type == GameConstants_HYJD.CHR_HUA_DAN_DIAO) {
						result.append(" 花单吊");
					} else if (type == GameConstants_HYJD.CHR_QI_XIAO_DUI) {
						result.append(" 七对");
					} else if (type == GameConstants_HYJD.CHR_PING_HU) {
						result.append(" 平胡");
					} else if (type == GameConstants_HYJD.CHR_WU_GUI_JIA_BEI) {
						result.append(" 无鬼加倍");
					}
	...
	public int get_ting_card(int[] cards, int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index) {
		...
		// if (has_rule(GameConstants_HYJD.GAME_RULE_HZ_GUI)) {
		// real_max_ting_count = GameConstants.MAX_ZI_FENG;
		// } else if (has_rule(GameConstants_HYJD.GAME_RULE_SI_HUA_GUI)) {
		// real_max_ting_count = GameConstants.MAX_ZI_FENG + 4;
		// } else if (has_rule(GameConstants_HYJD.GAME_RULE_BA_HUA_GUI)) {
		// real_max_ting_count = GameConstants.MAX_INDEX;
		// }
		...
		for (int i = 0; i < max_ting_count; i++) {
			...
			if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card(cbCardIndexTemp, weaveItem, cbWeaveCount, cbCurrentCard,
					chr, GameConstants.CHR_ZI_MO, seat_index)) {
	...
	public int getNIaoNum() {
		...
		if (has_rule(GameConstants_HYJD.GAME_RULE_MA_2)) {
			nNum = GameConstants.ZHANIAO_2;
		}
		if (has_rule(GameConstants_HYJD.GAME_RULE_MA_4)) {
			nNum = GameConstants.ZHANIAO_4;
		}
		if (has_rule(GameConstants_HYJD.GAME_RULE_MA_6)) {
			nNum = GameConstants.ZHANIAO_6;
		}
		if (has_rule(GameConstants_HYJD.GAME_RULE_MA_8)) {
	...
	protected boolean on_handler_game_finish(int seat_index, int reason) {
		...
		if (GRR != null) {
			...
			game_end.setCellScore(GameConstants_HYJD.CELL_SCORE);
			...
			for (int i = 0; i < niao_num && i < GRR._count_niao; i++) {
				boolean have_niao = false;
				for (int j = 0; j < GRR._player_niao_count[seat_index]; j++) {
					if (GRR._cards_data_niao[i] == GRR._player_niao_cards[seat_index][j]) {
						have_niao = true;
					}
				}
				if (have_niao) {
					game_end.addCardsDataNiao(GRR._cards_data_niao[i] + GameConstants.CARD_ESPECIAL_TYPE_NIAO);
				} else {
	...
	private int getCardValue(int card) {
		if (card < GameConstants_HYJD.CHUN_MAGIC_CARD) {
			return _logic.get_card_value(card);
		}

		// 花牌
		switch (card) {
		case GameConstants_HYJD.CHUN_MAGIC_CARD:
			return 1;
		case GameConstants_HYJD.XIA_MAGIC_CARD:
			return 2;
		case GameConstants_HYJD.QIU_MAGIC_CARD:
			return 3;
		case GameConstants_HYJD.DONG_MAGIC_CARD:
			return 4;
		case GameConstants_HYJD.MEI_MAGIC_CARD:
			return 5;
		case GameConstants_HYJD.LAN_MAGIC_CARD:
			return 6;
		case GameConstants_HYJD.ZHU_MAGIC_CARD:
			return 7;
		case GameConstants_HYJD.JU_MAGIC_CARD:
			return 8;
		}
		return 0;
	}
	...
	public String getGameRuleDes() {
		...
		if (has_rule(GameConstants_HYJD.GAME_RULE_GUI_CARD)) {
			stringBuilder.append("鬼牌");
		}
		if (has_rule(GameConstants_HYJD.GAME_RULE_HZ_GUI)) {
			stringBuilder.append("红中牌");
		}
		if (has_rule(GameConstants_HYJD.GAME_RULE_SI_HUA_GUI)) {
			stringBuilder.append("4花鬼");
		}
		if (has_rule(GameConstants_HYJD.GAME_RULE_BA_HUA_GUI)) {
			stringBuilder.append("8花鬼");
		}

		if (has_rule(GameConstants_HYJD.GAME_RULE_WUGUI_DOUBLE)) {
			stringBuilder.append("无鬼加倍");
		}
		if (has_rule(GameConstants_HYJD.GAME_RULE_MAI_MA)) {
	...
	public boolean estimate_gang_respond(int seat_index, int card) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (playerStatus.is_chi_hu_round()) {
				...
				// GameConstants_HYJD.HU_CARD_TYPE_QIANG_GANG, i);
	...
	public void test_cards() {
		int[] cards_of_player0 = new int[] { 0x03, 0x04, 0x05, 0x16, 0x17, 0x17, 0x17, 0x17, 0x19, 0x31, 0x31, 0x31,
				0x01 };
		int[] cards_of_player1 = new int[] { 0x03, 0x04, 0x05, 0x16, 0x17, 0x17, 0x17, 0x17, 0x19, 0x31, 0x31, 0x31,
				0x01 };
		int[] cards_of_player2 = new int[] { 0x03, 0x04, 0x11, 0x13, 0x17, 0x18, 0x19, 0x21, 0x28, 0x31, 0x33, 0x33,
				0x37 };
		int[] cards_of_player3 = new int[] { 0x01, 0x01, 0x01, 0x05, 0x06, 0x07, 0x14, 0x15, 0x16, 0x31, 0x38, 0x39,
				0x31 };
	...
	public void showSpecialCard(int seat_index) {
		if (has_rule(GameConstants_HYJD.GAME_RULE_HZ_GUI) || has_rule(GameConstants_HYJD.GAME_RULE_SI_HUA_GUI)
				|| has_rule(GameConstants_HYJD.GAME_RULE_BA_HUA_GUI)) {
