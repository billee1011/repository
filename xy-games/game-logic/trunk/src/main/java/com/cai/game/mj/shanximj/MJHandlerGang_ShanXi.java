...
public class MJHandlerGang_ShanXi extends MJHandlerGang<Table_ShanXi> {
    @Override
    public void exe(Table_ShanXi table) {
        ...
        table._playerStatus[_seat_index].clear_cards_abandoned_hu();;// 可以胡了
...
//        }
        ...
        // 抢杠胡判断
        if (table.has_rule(Constants_ShanXi.GAME_RULE_ZHI_ZHA_BU_HU)) {
            this.exe_gang(table);
            return;
        }

        // 检查对这个杠有没有 胡
        boolean bAroseAction = table.estimate_gang_respond_hnhz(_seat_index, _center_card);
        ...
        if (bAroseAction == false) {
            ...
            for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
                ...
                handler_check_auto_behaviour(table, i, _center_card);
            }
        }
    ...
    public boolean handler_operate_card(Table_ShanXi table, int seat_index, int operate_code, int operate_card) {
        ...
        if (operate_code == GameConstants.WIK_NULL) {
        	...
        	if (table._playerStatus[seat_index].has_chi_hu() && !table.has_rule(Constants_ShanXi.GAME_RULE_ZHI_ZHA_BU_HU)) {
            ...
            table.process_chi_hu_player_operate_hnhz(seat_index, _center_card, false);// 效果
...
//            int add_niao = 0;
        	...
        	if(table.GRR._cards_index[_provide_player][table._logic.switch_to_card_index(operate_card)] > 0){
...
//                    add_niao = 1;
//                }
//                table.set_niao_card(target_player, GameConstants.INVALID_VALUE, true, add_niao);// 结束后设置鸟牌
            }
...
//                table._player_result.zi_mo_count[i]++;
     ...
     ***/
    @Override
    protected boolean exe_gang(Table_ShanXi table) {
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            ...
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][_provide_player] -=  GameConstants.CELL_SCORE;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
        ...
        if (table.has_rule(Constants_ShanXi.GAME_RULE_DAI_FENG_PAI)) {
            has_feng = true;
        }
        // 红中麻将,检查听牌
        table._playerStatus[_seat_index]._hu_card_count = table.get_hnhz_ting_card_new(
                table._playerStatus[_seat_index]._hu_cards, table.GRR._cards_index[_seat_index],
                table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], has_feng);
