...
public class HandlerOutCardOperate_GY extends MJHandlerOutCardOperate<Table_GY> {
	...
	public boolean handler_operate_card(Table_GY table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			...
			// if
			// (table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_PENG))
			// {
		...
		if (operate_code == GameConstants.WIK_PENG && table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_GANG)) {
		...
		case GameConstants.WIK_CHI_HU: {
			...
			if (jie_pao_count > 0) {
				...
				// 连庄玩法 加连庄数
				// if
				// (table.has_rule(GameConstants_GY.GAME_RULE_CONTINUE_BANKER)
