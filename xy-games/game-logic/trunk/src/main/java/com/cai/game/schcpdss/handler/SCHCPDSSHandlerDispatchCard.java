package com.cai.game.schcpdss.handler;
...
public class SCHCPDSSHandlerDispatchCard<T extends SCHCPDSSTable> extends SCHCPDSSHandler<T> {
	...
	public SCHCPDSSHandlerDispatchCard() {
	...
	public void reset_status(int seat_index, int send_card, int type) {
		_seat_index = seat_index;
		_type = type;
		_send_card_data = send_card;
	...
	public boolean handler_player_be_in_room(T table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < GameConstants.MAX_CP_WEAVE; j++) {
				...
				if (seat_index != i) {
					if ((table.GRR._weave_items[i][j].weave_kind == GameConstants.DSS_WIK_PENG||table.GRR._weave_items[i][j].weave_kind == GameConstants.DSS_WIK_TOU)
		...
		if (table._is_ting_pai[seat_index] == -1)
			table.operate_bao_ting(seat_index, false);
