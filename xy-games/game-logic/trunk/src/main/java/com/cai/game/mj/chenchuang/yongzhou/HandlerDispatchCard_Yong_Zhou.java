...
public class HandlerDispatchCard_Yong_Zhou extends MJHandlerDispatchCard<Table_Yong_Zhou> {
	...
	public HandlerDispatchCard_Yong_Zhou() {
	...
	public void exe(Table_Yong_Zhou table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			table._cur_banker = (_seat_index - 1) % table.getTablePlayerNumber();
		...
		// 判断是否是杠后抓牌,杠开花只算接杠
		int card_type = Constants_MJ_YONG_ZHOU.HU_CARD_TYPE_ZI_MO;
		if(_type == Constants_MJ_YONG_ZHOU.HU_CARD_TYPE_GSKH)
			card_type = Constants_MJ_YONG_ZHOU.HU_CARD_TYPE_GSKH;
		// 检查牌型,听牌
	...
	@Override
	public boolean handler_player_out_card(Table_Yong_Zhou table, int seat_index, int card) {
