...
public class MJHandlerGang_TT extends MJHandlerGang<MJTable_TT> {
    @Override
    public void exe(MJTable_TT table) {
        ...
        // 抢杠胡判断
        if (!table.has_rule(GameConstants_TIAOTIAO.GAME_RULE_KE_QIANG_GANG_HU)) {
            this.exe_gang(table);
            return;
        }
     ...
     */
    @Override
    public boolean handler_operate_card(MJTable_TT table, int seat_index, int operate_code, int operate_card) {
        // 抢杠胡
        ...
        if (jie_pao_count > 0) {
            ...
            if (jie_pao_count > 1) {
                ...
                // 红中通炮就不加鸟了
                table.set_niao_card_hz(_provide_player, GameConstants.INVALID_VALUE, true, add_niao, _center_card);// 结束后设置鸟牌
            } else if (jie_pao_count == 1) {
                table._cur_banker = target_player;
                add_niao = table.getMoreZhuaNiao(_seat_index);;
                table.set_niao_card_hz(target_player, GameConstants.INVALID_VALUE, true, add_niao, _center_card);// 结束后设置鸟牌
            }
     ...
     ***/
    @Override
    protected boolean exe_gang(MJTable_TT table) {
