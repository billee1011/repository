...
public class HandlerDispatchCard_PuCheng extends MJHandlerDispatchCard<Table_PuCheng> {
	...
	public HandlerDispatchCard_PuCheng() {
	...
	public void exe(Table_PuCheng table) {
		//烧庄处理
		if(table.is_chase_vaild && table.banker_out_card > 0 && _seat_index == table.GRR._banker_player)
			table.chase_count++; 
		...
		// 判断是否是杠后抓牌,杠开花只算接杠
		int card_type = Constants_MJ_PC.HU_CARD_TYPE_ZI_MO;
		// 检查牌型,听牌
		...
		table.execute_trustee(_seat_index);
	...
	@Override
	public boolean handler_player_out_card(Table_PuCheng table, int seat_index, int card) {
