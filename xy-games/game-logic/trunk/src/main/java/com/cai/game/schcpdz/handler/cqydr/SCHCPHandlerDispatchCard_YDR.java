 ...
 *
 */
public class SCHCPHandlerDispatchCard_YDR extends SCHCPDZHandlerDispatchCard<SCHCPDZTable> {
...
//		 _send_card_data = 0x42;
		...
		while(loop < table.getTablePlayerNumber()){
			...
			action_hu[i] = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i],
					table.GRR._weave_count[i],i,_seat_index,_send_card_data, chr[i], card_type,false);// 自摸
...
//				action_hu[i] = GameConstants.WIK_NULL;
		...
		if(action ==  GameConstants.DZ_WIK_NULL)
			bAroseAction = table.estimate_player_chi_ydr(_seat_index, _send_card_data,true);
		for(int i = 0; i<table.getTablePlayerNumber();i++){
