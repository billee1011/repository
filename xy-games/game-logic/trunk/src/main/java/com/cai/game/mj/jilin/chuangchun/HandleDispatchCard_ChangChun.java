...
public class HandleDispatchCard_ChangChun extends AbstractMJHandler<MjTable_ChangChun> {
	...
	public HandleDispatchCard_ChangChun() {
	...
	public void exe(MjTable_ChangChun table) {
		...
		if(table.has_rule(GameConstants.GAME_RULE_ZHUA_WAN_HUANG_CC)){
		...
		else if(table.has_rule(GameConstants.GAME_RULE_LIU_QI_DUN_CC)){
		...
		//胡牌上听
		if(table.has_rule(GameConstants.GAME_RULE_HU_XU_TING_CC) && !table._playerStatus[_seat_index].is_bao_ting() ){
					...
					for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
	                    ...
	                    if (table._logic.get_card_count_by_index(table.GRR._cards_index[_seat_index]) == 1) {
	                    	iting_count = table.get_ting_card(ting_cards, cbCardIndexTemp,
	                                table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index] + 1,_seat_index);
	                    } else {
	                    	iting_count = table.get_ting_card(ting_cards, cbCardIndexTemp,
	                                table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index],_seat_index);
	                    }
