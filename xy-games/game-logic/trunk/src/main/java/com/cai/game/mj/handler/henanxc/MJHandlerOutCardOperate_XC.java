...
public class MJHandlerOutCardOperate_XC extends MJHandlerOutCardOperate<MJTable> {
	@Override
	public void exe(MJTable table) {
		...
		if (table._playerStatus[_out_card_player].is_bao_ting()) {
			table.operate_chi_hu_henan_xc_cards(_out_card_player, ting_count, ting_cards);
		} else if (table.getRuleValue(GameConstants.GAME_RULE_HENAN_BAO_TING) != 1) {
			table.xc_analyse_type = table.XC_ANALYSE_TING;

			table._playerStatus[_out_card_player]._hu_card_count = table.get_xc_ting_card(_out_card_player,
		...
		boolean bAroseAction = table.estimate_player_out_card_respond_henan_xc(_out_card_player, _out_card_data);
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		for (int p = 0; p < table.getTablePlayerNumber(); p++) {
			...
			if (table._playerStatus[i].has_action()) {
				if (table._playerStatus[i].is_respone()) {
					cbUserActionRank = table._logic.get_action_rank_xc(table._playerStatus[i].get_perform()) + table.getTablePlayerNumber() - p;
				} else {
					cbUserActionRank = table._logic.get_action_list_rank_xc(table._playerStatus[i]._action_count, table._playerStatus[i]._action)
							+ table.getTablePlayerNumber() - p;
				}

				if (table._playerStatus[target_player].is_respone()) {
					cbTargetActionRank = table._logic.get_action_rank_xc(table._playerStatus[target_player].get_perform()) + target_p;
				} else {
					cbTargetActionRank = table._logic.get_action_list_rank_xc(table._playerStatus[target_player]._action_count,
		...
		case GameConstants.WIK_CHI_HU: {
			...
			table.process_chi_hu_player_score_henan_xc(target_player, _out_card_player, _out_card_data, false);
