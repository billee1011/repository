...
public class HandlerGang_PING_XIANG extends MJHandlerGang<Table_PING_XIANG> {
	...
	public void exe(Table_PING_XIANG table) {
		...
		if(table.has_rule(Constants_MJ_PING_XIANG_258.GAME_RULE_KE_QIANG_GANG_HU)){
		...
		if (bAroseAction == false) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if (playerStatus.has_chi_hu()) {
					if(table.has_rule(Constants_MJ_PING_XIANG_258.GAME_RULE_QIANG_ZHI_HU_PAI)){
	...
	@Override
	public boolean exe_gang(Table_PING_XIANG table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_provide_player] -= 1;
		...
		boolean is_gang_yao = _action == Constants_MJ_PING_XIANG_258.WIK_GANG_YAO;
		if(is_gang_yao)
			table.is_gang_yao[_seat_index] = true;
		// 从后面发一张牌给玩家
		table.exe_dispatch_card(_seat_index, is_gang_yao ? Constants_MJ_PING_XIANG_258.HU_CARD_TYPE_GANG_KAI_HUA : 0, 0);
	...
	private void handler_operate_tong_pao(Table_PING_XIANG table, int seat_index,
        ...
        if (jie_pao_count > 0) {
            ...
            table.GRR._chi_hu_rights[_seat_index].opr_or(Constants_MJ_PING_XIANG_258.CHR_FANG_PAO);
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if ((table.GRR._chi_hu_rights[i].is_valid() == false)) {
                    continue;
                }
	...
	private void handler_operate_bu_tong_pao(Table_PING_XIANG table, int seat_index,
        ...
        if (target_action == GameConstants.WIK_CHI_HU) {
            ...
            table.process_chi_hu_player_operate(target_player, _center_card, false);// 效果
        }
