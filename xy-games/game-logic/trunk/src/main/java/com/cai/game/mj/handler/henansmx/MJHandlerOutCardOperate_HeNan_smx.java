...
public class MJHandlerOutCardOperate_HeNan_smx extends MJHandlerOutCardOperate<MJTable_SMX> {
    ...
    public void exe(MJTable_SMX table) {
        ...
        // 河南麻将,检查听牌
        table._playerStatus[_out_card_player]._hu_card_count = table.get_henan_smx_ting_card(
        ...
        // 判断玩家是否表演
        if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HENAN_SMX_BIAO_YAN)) {
            if (ting_count != 1 || ting_cards[0] != -1) {
                table._biaoyan_count[_out_card_player] = 0;
            }
            if (ting_count == 1 && ting_cards[0] == -1 && table._biaoyan_count[_out_card_player] == 0) {
                table._biaoyan_count[_out_card_player] = 1;
            } else if (ting_count == 1 && ting_cards[0] == -1 && table._logic.is_magic_card(_out_card_data)) {
                table._biaoyan_count[_out_card_player]++;
            }
        }
        ...
        // 玩家出牌 响应判断,是否有吃碰杠补胡
        boolean bAroseAction = table.estimate_player_out_card_respond_henan_smx(_out_card_player, _out_card_data);// ,
     ...
     */
    @Override
    public boolean handler_operate_card(MJTable_SMX table, int seat_index, int operate_code, int operate_card) {
        // 效验状态
        ...
        case GameConstants.WIK_CHI_HU: // 胡
        {
            ...
            // table.process_chi_hu_player_score_henan_smx(target_player,_out_card_player,
            ...
            //
            // GameSchedule.put(new GameFinishRunnable(table.getRoom_id(),
            // table._banker_select, GameConstants.Game_End_NORMAL),
