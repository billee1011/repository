...
public class MJHandlerDispatchCard_HuNan_ChangDe extends MJHandlerDispatchCard<MJTable_HuNan_ChangDe> {

	protected GangCardResult m_gangCardResult;

	public MJHandlerDispatchCard_HuNan_ChangDe() {
	...
	public void exe(MJTable_HuNan_ChangDe table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			table._cur_banker = table._last_dispatch_player; // 最后摸牌的是下一局的庄家
		...
		if (!curPlayerStatus.isAbandoned()) {
			// 胡牌检测
	...
	public boolean handler_operate_card(MJTable_HuNan_ChangDe table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			// 设置奖码
			if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HUNAN_MO_JI_JIANG_JI)) {
				table.set_niao_card(_seat_index, operate_card, true, add_niao);
			} else {
				table.set_niao_card(_seat_index, GameConstants.INVALID_VALUE, true, add_niao);
			}

			table.process_chi_hu_player_operate(_seat_index, operate_card, true);
			table.process_chi_hu_player_score(_seat_index, _seat_index, operate_card, 0); // 自摸胡算分
