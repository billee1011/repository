...
public class SCHCPHandlerOutCardOperate_YanZhong extends SCHCPHandlerOutCardOperate<SCHCPTable_YanZhong> {
	@Override
	public void exe(SCHCPTable_YanZhong table) {
		...
		for(int i = 0; i< table.getTablePlayerNumber();i++)
		{
			Arrays.fill(table._temp_peng_index[i],0);
		}
		if(table.GRR._cards_index[_out_card_player][table._logic.switch_to_card_index(_out_card_data)] >= 3)
		{
			table.log_error(_out_card_player+"出牌出错 SCHCPHandlerOutCardOperate_YX "+_out_card_data);
			return ;
		}
		
		if(table.GRR._cannot_out_index[_out_card_player][table._logic.switch_to_card_index(_out_card_data)]>=1&&table._zhao_card[_out_card_player][table._logic.switch_to_card_index(_out_card_data)]==0)
		{
			table.log_error(_out_card_player+"出牌出错  schcpHandlerOutCardOperate_YX "+_out_card_data);
			return ;
		}
		Arrays.fill(table._temp_peng_index[_out_card_player], 0);
		//
		// 出牌记录
		...
		if(table._zhao_card[_out_card_player][table._logic.switch_to_card_index(_out_card_data)]>0)
			table._zhao_card[_out_card_player][table._logic.switch_to_card_index(_out_card_data)]--;
		int next_player = (_out_card_player + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();
		table._current_player = next_player;
		table.must_out_card();
		table.add_must_zhao(_out_card_player, _out_card_data);
		table.add_zhao(_out_card_player, _out_card_data);
		...
		while(loop < table.getTablePlayerNumber()){
			...
			if(i == _out_card_player)
				continue;
			if(table._guo_hu_pai_cards[i][table._logic.switch_to_card_index(_out_card_data)]>0)
				continue;
			...
			action_hu[i] = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i],
					table.GRR._weave_count[i],i,_out_card_player,_out_card_data, chr[i], card_type,false,false);// 自摸
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				tempPlayerStatus.add_action(GameConstants.CP_WIK_NULL);
...
//				return ;
		...
		if(table._playerStatus[_out_card_player]._hu_card_count == 0)
		...
		for(int i = 0; i < table.getTablePlayerNumber();i++)
		{
			if(i == table._xiao_jia)
				continue;
			table._cannot_peng_index[i][table._logic.switch_to_card_index(_out_card_data)]++;
		}																									// EstimatKind.EstimatKind_OutCar
		
		// 如果没有需要操作的玩家，派发扑克
		
		if (bAroseAction == true|| hu_pai == 1){
			// 等待别人操作这张牌
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	...
	public boolean handler_operate_card(SCHCPTable_YanZhong table, int seat_index, int operate_code, int operate_card,int luoCode) {
		...
		if(operate_code != GameConstants.CP_WIK_CHI_HU){
...
//		}
		...
		if(target_action == GameConstants.WIK_NULL)
		{
			//过张的牌都不可以吃
			int next_player = (_out_card_player + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();
			table.cannot_chicard(next_player, target_card);
			for(int i = 0; i<table.getTablePlayerNumber();i++)
			{
				 for(int j = 0; j< table._playerStatus[i]._action_count;j++)
				 {
					if(table._playerStatus[i]._action[j] == GameConstants.CP_WIK_CHE)
					{
						table._cannot_peng_index[next_player][table._logic.switch_to_card_index(target_card)]=1;
					}
				}
			}
		}
		if(table._logic.get_action_rank(target_action)<table._logic.get_action_rank(GameConstants.CP_WIK_CHE))
		{
			for(int i = 0; i<table.getTablePlayerNumber();i++)
			{
				 for(int j = 0; j< table._playerStatus[i]._action_count;j++)
				 {
					if(table._playerStatus[i]._action[j] == GameConstants.CP_WIK_CHE)
					{
						table._cannot_peng_index[i][table._logic.switch_to_card_index(target_card)]=1;
					}
				}
			}
		}
		...
		case GameConstants.CP_WIK_LEFT: // 上牌操作
		{
			...
			if(table._zhao_card[target_player][table._logic.switch_to_card_index(cbRemoveCard[0] )]>0)
				table._zhao_card[target_player][table._logic.switch_to_card_index(cbRemoveCard[0] )]--;
		...
		case GameConstants.CP_WIK_CHE: // 碰牌操作
		{
			...
			table.exe_gang(target_player, this._out_card_player, operate_card, GameConstants.CP_WIK_CHE,GameConstants.CP_TYPE_OUT_CHE,true, true, false,1000);
...
//					GameConstants.CHI_PENG_TYPE_DISPATCH,target_lou_code);
	...
	public boolean handler_player_be_in_room(SCHCPTable_YanZhong table, int seat_index) {
		...
		for (int i = 0; i < hand_card_count; i++) {
			if(cards_index[table._logic.switch_to_card_index(cards[i])]>0)
			{
				cards_index[table._logic.switch_to_card_index(cards[i])]--;
				cards[i]|=0x100;
			}
