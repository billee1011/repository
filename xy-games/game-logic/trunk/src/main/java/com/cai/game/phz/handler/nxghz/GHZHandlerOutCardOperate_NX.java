...
public class GHZHandlerOutCardOperate_NX extends PHZHandlerOutCardOperate<NXGHZTable> {
	@Override
	...
	boolean bhuAction = false;
	
	bAroseAction = table.estimate_player_out_card_respond_yywhz(_out_card_player,_out_card_data,table.GRR._weave_items[_out_card_player],table.GRR._weave_count[_out_card_player],false);// ,
	
	bhuAction = table.estimate_player_out_card_hu_yywhz(_out_card_player,_out_card_data,table.GRR._weave_items[_out_card_player],table.GRR._weave_count[_out_card_player],false);																											// EstimatKind.EstimatKind_OutCar
		
	// 如果没有需要操作的玩家，派发扑克
	if (bAroseAction == false && bhuAction == false) {
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			table._playerStatus[i].clean_action();
			table._playerStatus[i].clean_status();
		}
