...
public class MJHandlerYingBao extends AbstractMJHandler<Table_GY> {
	...
	public boolean handler_operate_card(Table_GY table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants_MYGY.WIK_ZI_MO: {
			...
			// table.GRR._chi_hu_rights[table._current_player].opr_or(GameConstants_MYGY.CHR_TIAN_HU);
			table.player_ying_bao[table._current_player] = true;
			table._playerStatus[table._current_player].set_card_status(GameConstants_MYGY.CARD_STATUS_BAO_TING);
			...
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._current_player, GameConstants_MYGY.Game_End_NORMAL),
					GameConstants_MYGY.GAME_FINISH_DELAY, TimeUnit.SECONDS);

			return true;
		}
		case GameConstants_MYGY.WIK_YING_BAO_XIAN: // 报听
		{
			table.player_ying_bao[seat_index] = true;
			table._playerStatus[seat_index].set_card_status(GameConstants_MYGY.CARD_STATUS_BAO_TING);

			// 效果
			table.operate_effect_action(seat_index, GameConstants_MYGY.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { GameConstants_MYGY.WIK_BAO_TING },
		...
		for (int p = 0; p < table.getTablePlayerNumber(); p++) {
			...
			if (table._playerStatus[p].has_action_by_code(GameConstants_MYGY.WIK_YING_BAO_XIAN)) {
				return true;
			}
		}

		if (table._playerStatus[table._current_player].has_action()) {
			table.change_player_status(table._current_player, GameConstants_MYGY.Player_Status_OPR_CARD);
			table.operate_player_action(table._current_player, false);
		} else {
			table.player_mo_first[table._current_player] = false;
			table.change_player_status(table._current_player, GameConstants_MYGY.Player_Status_OUT_CARD);
	...
	public boolean handler_player_out_card(Table_GY table, int seat_index, int card) {
		...
		if (_type == GameConstants_MYGY.DispatchCard_Type_Tian_Hu) {
			// 出牌
			table.exe_out_card(table._current_player, card, GameConstants_MYGY.OutCard_Type_Di_Hu);
		} else {
			// 出牌
		...
		if (seat_index == table._current_player) {
			if (table.player_duan[table._current_player] != -1) {
				if (table.player_duan[table._current_player] == table._logic.get_card_color(table._send_card_data)) {
					...
					if (add_flag) {
						table._logic.remove_card_by_data(hand_cards, table._send_card_data + GameConstants_MYGY.CARD_ESPECIAL_TYPE_LIANG_ZHANG);
					} else {
						table._logic.remove_card_by_data(hand_cards, table._send_card_data);
					}
				}
		...
		if (hand_have_duan_card) {
			if (table.player_duan[table._current_player] == table._logic.get_card_color(table._send_card_data)) {
				...
				table.operate_player_get_card(table._current_player, 1,
						new int[] { show_send_card + GameConstants_MYGY.CARD_ESPECIAL_TYPE_LIANG_ZHANG }, seat_index);
			}
		} else {
			table.operate_player_get_card(table._current_player, 1, new int[] { show_send_card }, seat_index);
		}

		if (seat_index != table._current_player && table._playerStatus[seat_index].has_action_by_code(GameConstants_MYGY.WIK_BAO_TING)) {
