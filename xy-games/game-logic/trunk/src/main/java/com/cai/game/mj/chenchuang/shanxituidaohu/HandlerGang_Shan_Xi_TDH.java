...
public class HandlerGang_Shan_Xi_TDH extends MJHandlerGang<Table_Shan_Xi_TDH> {
	...
	public HandlerGang_Shan_Xi_TDH() {
	}
	...
	@Override
	public boolean exe_gang(Table_Shan_Xi_TDH table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
				int score = (_seat_index == table.GRR._banker_player || i == table.GRR._banker_player) && table.has_rule(Constants_MJ_SAN_XI_TDH.GAME_RULE_PING_HU) ? 4 : 2;
				table.GRR._game_score[i] -= score;
				table.GRR._game_score[_seat_index] += score;
			...
			int score = (_seat_index == table.GRR._banker_player || _provide_player == table.GRR._banker_player) && table.has_rule(Constants_MJ_SAN_XI_TDH.GAME_RULE_PING_HU) ? 6 : 3;
			if(table.is_bao_ting[_provide_player]){
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					if (i == _seat_index)
						continue;
					score = (_seat_index == table.GRR._banker_player || i == table.GRR._banker_player) && table.has_rule(Constants_MJ_SAN_XI_TDH.GAME_RULE_PING_HU) ? 2 : 1;
					table.GRR._game_score[i] -= score;
					table.GRR._game_score[_seat_index] += score;
				...
				table.GRR._game_score[_provide_player] -= score;
				table.GRR._game_score[_seat_index] += score;
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
				int score = (_seat_index == table.GRR._banker_player || i == table.GRR._banker_player) && table.has_rule(Constants_MJ_SAN_XI_TDH.GAME_RULE_PING_HU) ? 2 : 1;
				table.GRR._game_score[i] -= score;
				table.GRR._game_score[_seat_index] += score;
	...
	public boolean handler_operate_card(Table_Shan_Xi_TDH table, int seat_index, int operate_code, int operate_card) {
		...
		if (table.has_rule(Constants_MJ_SAN_XI_TDH.GAME_RULE_YI_PAO_DUO_XIANG)) {
	...
	private void handler_operate_tong_pao(Table_Shan_Xi_TDH table, int seat_index,
        ...
        if (jie_pao_count > 0) {
            ...
            table.GRR._chi_hu_rights[_seat_index].opr_or(Constants_MJ_SAN_XI_TDH.CHR_FANG_PAO);
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if ((table.GRR._chi_hu_rights[i].is_valid() == false)) {
                    continue;
                }
	...
	private void handler_operate_bu_tong_pao(Table_Shan_Xi_TDH table, int seat_index,
			int operate_code, int operate_card) {
		int target_player = seat_index;
