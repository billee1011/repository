...
public class THKHandlerOutCardOperate_HY extends HHHandlerOutCardOperate<HHTable> {
	@Override
	public void exe(HHTable table) {
		...
		while(loop < table.getTablePlayerNumber()){
			...
			action_pao[i] = table.estimate_player_respond_xiao(i,_out_card_player ,_out_card_data, pao_type[i],false);
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				if(all_hu_xi < table._guo_hu_hu_xi[i][table._logic.switch_to_card_index(_out_card_data)] +3)
				{
					chr[i].set_empty();
					continue;
				}
...
//		}
		...
		case GameConstants.WIK_PAO: // 杠牌操作
		{
			...
			int pao_type[] =  new int[1];
			int action = table.estimate_player_respond_xiao(target_player,this._out_card_player ,operate_card, pao_type,false);	
