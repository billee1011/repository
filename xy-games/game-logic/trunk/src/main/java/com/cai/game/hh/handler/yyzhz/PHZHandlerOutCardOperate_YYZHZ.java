...
public class PHZHandlerOutCardOperate_YYZHZ extends HHHandlerOutCardOperate<HHTable_YYZHZ> {
	@Override
	public void exe(HHTable_YYZHZ table) {
		...
		// 刷新自己手牌
		int hand_card_count = table._logic.switch_to_cards_data_lai(table.GRR._cards_index[_out_card_player], cards);
		...
		//听牌操作
		int count = playerStatus._hu_card_count = table.get_hh_ting_card_twenty(playerStatus._hu_cards, table.GRR._cards_index[_out_card_player],
				table.GRR._weave_items[_out_card_player], table.GRR._weave_count[_out_card_player], _out_card_player, _out_card_player);

		int ting_cards[] =count == 0 ? new int[]{0} : playerStatus._hu_cards ;
		...
		if(!table._logic.is_magic_card(_out_card_data)){
			bAroseAction = table.estimate_player_out_card_respond_yyzhz(_out_card_player, _out_card_data, false);
		}
		...
		if (bAroseAction == false) {
			...
			int discard_time  = HHManager.getInstance().getSysParamValue(table.getGame_id(), HHManager.DEFAULT_2000, HHManager.VAL1, 1105);
			...
			int dispatch_time = HHManager.getInstance().getSysParamValue(table.getGame_id(), HHManager.DEFAULT_3000, HHManager.VAL5, 1105);
	...
	public boolean handler_player_be_in_room(HHTable_YYZHZ table, int seat_index) {
		...
		int hand_card_count = table._logic.switch_to_cards_data_lai(table.GRR._cards_index[seat_index], cards);
