...
public class HandlerOutCardOperate_HuoJia extends MJHandlerOutCardOperate<Table_HuoJia> {
    ...
    public void exe(Table_HuoJia table) {
        ...
        int hu_out_card_count = table._playerStatus[_out_card_player]._hu_out_card_count;
        if (table.has_bao_ting_and_clicked[_out_card_player]) {
            // 如果有报听并点击了报听，判断出的牌是不是之前存好的牌数据
            for (int i = 0; i < hu_out_card_count; i++) {
                if (_out_card_data == table._playerStatus[_out_card_player]._hu_out_card_ting[i]) {
                    table.is_bao_ting[_out_card_player] = true;

                    // 存储报听的牌是哪张废牌，暗听时才使用
                    table.GRR._chi_hu_rights[_out_card_player].bao_ting_index = table.GRR._discard_count[_out_card_player];
        ...
        if (table.has_bao_ting_and_clicked[_out_card_player] && table.is_bao_ting[_out_card_player]
                && table.has_rule(Constants_HuoJia.GAME_RULE_AN_TING)) {
            table.operate_out_card_bao_ting(_out_card_player, 1,
        ...
        if (ting_count > 0 && table.is_bao_ting[_out_card_player]) {
        ...
        table.has_bao_ting_and_clicked[_out_card_player] = false;
    ...
    public boolean handler_operate_card(Table_HuoJia table, int seat_index, int operate_code, int operate_card) {
        ...
        case GameConstants.WIK_CHI_HU: {
            ...
            table.GRR._chi_hu_rights[_out_card_player].opr_or(Constants_HuoJia.CHR_FANG_PAO);
