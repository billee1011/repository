...
public class MJHandlerOutCardOperate_TT extends MJHandlerOutCardOperate<MJTable_TT> {
    ...
    public void exe(MJTable_TT table) {
        ...
        table._playerStatus[_out_card_player]._hu_card_count = table.get_ting_card(
                ...
                table.GRR._weave_items[_out_card_player], table.GRR._weave_count[_out_card_player], false,_out_card_player);
        ...
        case GameConstants.WIK_CHI_HU: // 胡
        {
            ...
            if (jie_pao_count > 0) {
                ...
                if (jie_pao_count > 1) {
                    // 结束后设置鸟牌
                    // 红中通炮就不加鸟了
                    table.set_niao_card_hz(_out_card_player, GameConstants.INVALID_VALUE, true, add_niao, target_card);
                    // 结束
                    table._cur_banker = _out_card_player;
                } else if (jie_pao_count == 1) {
                    table._cur_banker = target_player;
                    add_niao = table.getMoreZhuaNiao(target_player);
                    // 结束后设置鸟牌
                    table.set_niao_card_hz(target_player, GameConstants.INVALID_VALUE, true, add_niao, target_card);
                }
                for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                    if ((i == _out_card_player) || (table.GRR._chi_hu_rights[i].is_valid() == false)) {
                        continue;
    ...
    @Override
    public boolean handler_be_set_trustee(MJTable_TT table, int seat_index) {
