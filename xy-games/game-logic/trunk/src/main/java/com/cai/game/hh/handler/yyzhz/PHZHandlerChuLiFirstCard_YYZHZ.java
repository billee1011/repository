...
public class PHZHandlerChuLiFirstCard_YYZHZ extends HHHandlerDispatchCard<HHTable_YYZHZ> {
	...
	public void exe(HHTable_YYZHZ table) {
		...
		// 刷新手牌包括组合
		table.GRR._cards_index[_seat_index][table._logic.switch_to_card_index_yyzhz(table._send_card_data)]++;
		...
		if (table.GRR._cards_index[_seat_index][table._logic.switch_to_card_index_yyzhz(table._send_card_data)] != 0) {
			int hu_xi[] = new int[1];
			action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index],
					table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], _seat_index,
					_seat_index, 0, chr, card_type, hu_xi, true);// 自摸
		} else {
			int hu_xi[] = new int[1];
		...
		if (action != GameConstants.WIK_NULL) {
			...
			chr.set_empty();
			// 加到手牌
			hand_card_count = table._logic.switch_to_cards_data_lai(table.GRR._cards_index[_seat_index], cards);
