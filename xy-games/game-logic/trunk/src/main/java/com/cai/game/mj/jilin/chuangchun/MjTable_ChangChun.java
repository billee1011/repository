...
public class MjTable_ChangChun extends AbstractMJTable {
	private static final long serialVersionUID = -2456323602522819218L;

	protected HandleChiPeng_ChangChun handle_chi_peng;
	protected HandleDispatchCard_ChangChun handle_dispatch_card;
	protected HandleGang_ChangChun handle_gang;
	protected HandleOutCard_ChangChun handle_out_card;
	protected HandlerBaoTing_ChangChun _handler_out_card_bao_ting;
	protected HandlerFenZhang_ChangChun handler_fen_zhang;
	protected HandleLiang_ChangChun handler_liang;
	protected HandleBuCard_ChangChun handler_bu_card;
	public boolean[] has_fen_zhang = new boolean[getTablePlayerNumber()];
	public int[] fen_zhang_card = new int[getTablePlayerNumber()];
	protected LiangCardResult m_liangCardResult;
	protected BuCardResult m_BuCardResult;

	public MjTable_ChangChun() {
		super(MJType.GAME_TYPE_CHANG_CHUN);
	}
	...
	public boolean exe_liang(int seat_index, int provide_player, int center_card, int action, int type, int index, boolean d) {
		set_handler(handler_liang);
		handler_liang.reset_status(seat_index, provide_player, center_card, action, type, index, d);
		handler_liang.exe(this);
	...
	public boolean exe_bu_card(int seat_index, int provider, int action, int card, int type) {

		set_handler(handler_bu_card);
		handler_bu_card.reset_status(seat_index, provider, action, card, type);
		handler_bu_card.exe(this);
	...
	public boolean handler_requst_pao_qiang(Player player, int pao, int qiang) {
		// pao :表示补的那张牌， qiang：表示补的类型
		exe_bu_card(player.get_seat_index(), player.get_seat_index(), GameConstants.WIK_BU_ZHNAG, pao, qiang);
		return true;
	}

	@Override
	protected void onInitTable() {
		handle_chi_peng = new HandleChiPeng_ChangChun();
		handle_dispatch_card = new HandleDispatchCard_ChangChun();
		handle_gang = new HandleGang_ChangChun();
		handle_out_card = new HandleOutCard_ChangChun();
		handler_liang = new HandleLiang_ChangChun();
		handler_bu_card = new HandleBuCard_ChangChun();
		m_liangCardResult = new LiangCardResult(30);
		m_BuCardResult = new BuCardResult(5);
	}

	@Override
	public int getTablePlayerNumber() {
		if (this.getRuleValue(GameConstants.GAME_RULE_FOUR_CC) == 1)
			return 4;
		if (this.getRuleValue(GameConstants.GAME_RULE_THREE_CC) == 1)
			return 3;
		if (this.getRuleValue(GameConstants.GAME_RULE_TWO_CC) == 1)
			return 2;
	...
	public int analyse_chi_hu_card(int[] cards_index, WeaveItem[] weaveItems, int weave_count, int cur_card, ChiHuRight chiHuRight, int card_type,
		...
		// 飘胡七对不限三色和幺九
		if (this.has_rule(GameConstants.GAME_RULE_BU_XIAN_CC)) {

		}

		// 幺九
		if (!_logic.is_have_yaojiu_jilincc(cbCardIndexTemp, weaveItems, weave_count)) {
			return GameConstants.WIK_NULL;
		}
		// 清一色
		if (_logic.is_qing_yi_se_sy(cards_index, weaveItems, weave_count, cur_card)) {
			if (!this.has_rule(GameConstants.GAME_RULE_DAI_QUE_MEN_CC)) {
				return GameConstants.WIK_NULL;
			} else {
				b_da_hu = true;
				chiHuRight.opr_or(GameConstants.CHR_QING_YI_SE_JL);
			}
		}

		// 特殊杠判断
		if (_logic.ping_hu_tiao_jian_cc(cards_index, weaveItems, weave_count, cur_card)) {
			return GameConstants.WIK_NULL;
		}
		...
		// 门清
		if (_logic.is_men_qing_jilin_cc(cards_index, weaveItems, weave_count,
				this.has_rule(GameConstants.GAME_RULE_XIA_DAN_ZHAN_CC)) != GameConstants.WIK_NULL) {
			b_da_hu = true;
	...
	public void process_chi_hu_player_score(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		if (zimo) {
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				if (this.has_rule(GameConstants.GAME_RULE_BAO_PEI_CC) && _playerStatus[provide_index].is_bao_ting()) {
	...
	public boolean estimate_player_out_card_respond(int seat_index, int card, int type) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (GRR._left_card_count > 0 /*
				...
				if ((seat_index + 1) % getTablePlayerNumber() == i && can_chi(i)) {
				...
				if (action != 0 && can_peng_gang(i)) {
					if (_playerStatus[i].is_bao_ting()) {
						...
						int ting_count = get_ting_card(ting_cards, cbCardIndexTemp, GRR._weave_items[i], GRR._weave_count[i] + 1, i);
						if (ting_count > 0) {
			...
			if (_playerStatus[i].is_bao_ting() == true || !this.has_rule(GameConstants.GAME_RULE_HU_XU_TING_CC)) {
	...
	public boolean can_chi(int seat_index) {
		if (GRR._weave_count[seat_index] < 3)
			return true;
		return false;
	}

	public boolean can_peng_gang(int seat_index) {
		if (GRR._weave_count[seat_index] < 3)
			return true;
		for (int i = 0; i < GRR._weave_count[seat_index]; i++) {
			if (GRR._weave_items[seat_index][i].weave_kind == GameConstants.WIK_LEFT
					|| GRR._weave_items[seat_index][i].weave_kind == GameConstants.WIK_CENTER
					|| GRR._weave_items[seat_index][i].weave_kind == GameConstants.WIK_RIGHT) {
				return false;
			}
		}
	...
	public boolean estimate_player_bu_card_respond(int seat_index, int card, int type) {
		boolean bAroseAction = false;
	...
	protected void test_cards() {
		int cards[] = new int[] { 0x31, 0x32, 0x33, 0x34, 0x05, 0x05, 0x05, 0x06, 0x06, 0x06, 0x11, 0x09, 0x01 };
	...
	public boolean operate_player_action(int seat_index, boolean close) {
		...
		for (int i = 0; i < curPlayerStatus._weave_count; i++) {
			...
			weaveItem_item.setHuXi(curPlayerStatus._action_weaves[i].type);
	...
	public boolean operate_player_cards(int seat_index, int card_count, int cards[], int weave_count, WeaveItem weaveitems[]) {
		...
		if (weave_count > 0) {
			for (int j = 0; j < weave_count; j++) {
				...
				for (int n = 0; n < weaveitems[j].weave_card.length; n++) {
					if (weaveitems[j].weave_card[n] == 0) {
						continue;
					}
					weaveItem_item.addWeaveCard(weaveitems[j].weave_card[n]);
				}
				weaveItem_item.setHuXi(weaveitems[j].type);
				roomResponse.addWeaveItems(weaveItem_item);
			}
		}
