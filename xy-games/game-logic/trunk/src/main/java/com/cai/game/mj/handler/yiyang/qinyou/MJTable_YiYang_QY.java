...
public class MJTable_YiYang_QY extends AbstractMJTable {
	...
	protected MJHandlerOutCardBaoTing_YiYang_QY _handler_out_card_bao_ting; // 报听
	...
	public MJTable_YiYang_QY() {
		super(MJType.GAME_TYPE_MJ_YI_YANG_QIN_YOU);
	}

	@Override
	protected void onInitTable() {
		_handler_dispath_card = new MJHandlerDispatchCard_YiYang_QY();
		_handler_out_card_operate = new MJHandlerOutCardOperate_YiYang_QY();
		_handler_gang = new MJHandlerGang_YiYang_QY();
		_handler_chi_peng = new MJHandlerChiPeng_YiYang_QY();
		_handler_out_card_bao_ting = new MJHandlerOutCardBaoTing_YiYang_QY();
	}
	...
	public int analyse_chi_hu_card(int[] cards_index, WeaveItem[] weaveItems, int weaveCount, int cur_card, ChiHuRight chiHuRight, int card_type,
		...
		if(is_mj_type(GameConstants.GAME_TYPE_MJ_YI_YANG_QIN_YOU)){
			// 一条龙
			if (has_rule(GameConstants_YiYang_QY.GAME_YI_TIAO_LONG) && is_yi_tiao_long(cbCardIndexTemp)) {
				chiHuRight.opr_or(GameConstants_YiYang_QY.CHR_YI_TIAO_LONG);
				chiHuRight.da_hu_count++;
				isWin = true;
			}
		}else{
			if (GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_M_Q_J_J_HU)) {
		...
		if(is_mj_type(GameConstants.GAME_TYPE_MJ_YI_YANG_QIN_YOU)){
			if (card_type != GameConstants.HU_CARD_TYPE_QIANGGANG && GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_MEN_QING)) {
				// 门清
				if (_logic.is_men_qing(weaveItems, weaveCount) == GameConstants.CHR_HUNAN_MEN_QING) {
					chiHuRight.opr_or(GameConstants.CHR_HUNAN_MEN_QING);
				}
			...
			if (GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_MEN_QING)
	...
	private float getScore(ChiHuRight chr, float lChiHuScore) {
		...
		if (da_hu_count > 0) {
			...
			if (!(chr.opr_and(GameConstants_YiYang_QY.CHR_YI_TIAO_LONG)).is_empty()) {
				count++;
			}
		...
		for (int player = 0; player < getTablePlayerNumber(); player++) {
			...
			for (int typeIndex = 0; typeIndex < chrTypes; typeIndex++) {
				...
				if (GRR._chi_hu_rights[player].is_valid()) {
					...
					if (type == GameConstants_YiYang_QY.CHR_YI_TIAO_LONG) {
						gameDesc.append(" 一条龙");
					}
	...
	public boolean estimate_player_out_card_respond(int seat_index, int card) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (GRR._left_card_count > 1 && !_playerStatus[i].is_bao_ting()) {
				// 杠牌判断
	...
	public int getFeiNiaoNum() {
		int num = 0;
		if (GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_CS_FEI_NIAO1)) {
			return GameConstants.FEINIAO_1;
		} else if (GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_CS_FEI_NIAO2)) {
			return GameConstants.FEINIAO_2;
		} else if (GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_CS_FEI_NIAO3)) {
	...
	public void set_niao_card_fei(int seat_index, int card, boolean show, int add_niao) {
		if (!GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_FEI_NIAO)) {
			return;
		}
	...
	public int getCsDingNiaoNum() {
		...
		if (GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_ZHANIAO1)) {
			return GameConstants.ZHANIAO_1;
		} else if (GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_ZHANIAO2)) {
			return GameConstants.ZHANIAO_2;
		} else if (GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_ZHANIAO3)) {
	...
	public void set_niao_card(int seat_index, int card, boolean show, int add_niao) {
		if(GRR._left_card_count == 0){
			if(getCsDingNiaoNum() == 1)//3.海底胡牌抓鸟规则调整（仅限抓1鸟）：海底胡牌后，胡的那张为鸟牌
				GRR._left_card_count = 1;
			else
				return;
		}
		if(is_mj_type(GameConstants.GAME_TYPE_MJ_YI_YANG_QIN_YOU)){
			if(has_rule(GameConstants_YiYang_QY.GAME_BU_ZHUA_NIAO))
				return;
		}else{
			if (!GameDescUtil.has_rule(gameRuleIndexEx, GameConstants_YiYang_QY.GAME_RULE_HUNAN_ZHANIAO)) {
				return;
			}
		}
		...
		GRR._show_bird_effect = show;
		GRR._count_niao = getCsDingNiaoNum() > GRR._left_card_count ? GRR._left_card_count : getCsDingNiaoNum();
	...
	protected boolean on_handler_game_finish(int seat_index, int reason) {
		...
		if (GRR != null) {
			//一字撬奖喜
			if(is_mj_type(GameConstants.GAME_TYPE_MJ_YI_YANG_QIN_YOU) && has_rule(GameConstants_YiYang_QY.GAME_YI_ZI_QIAO)){
			...
			if(reason == GameConstants.Game_End_DRAW && GRR._left_card_count == 0){
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					if(_playerStatus[i]._hu_card_count > 0){
						for (int j = 0; j < getTablePlayerNumber(); j++) {
							if(j == i)
								continue;
							if(_playerStatus[j]._hu_card_count > 0)
								continue;
							GRR._game_score[i] += 2;
							GRR._game_score[j] -= 2;
						}
