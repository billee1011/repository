 ...
 *
 */
public class THKHandlerDispatchCard_HY extends HHHandlerDispatchCard<HHTable> {
	...
	public void exe(HHTable table) {
		...
		int ti_sao = table.estimate_player_ti_xiao_respond_phz_chd(_seat_index, _send_card_data);
		if (ti_sao != GameConstants.WIK_NULL) {
			if (ti_sao != GameConstants.WIK_CHOU_XIAO)
		...
		while (loop < table.getTablePlayerNumber()) {
			...
			action_pao[i] = table.estimate_player_respond_xiao(i, _seat_index, _send_card_data, pao_type[i], true);
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				if (all_hu_xi < table._guo_hu_hu_xi[i][table._logic.switch_to_card_index(_send_card_data)] + 3) {
					chr[i].set_empty();
					continue;
				}
				table._guo_hu_hu_xi[i][table._logic.switch_to_card_index(_send_card_data)] = all_hu_xi;
	...
	public boolean handler_operate_card(HHTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		if ((operate_code != GameConstants.WIK_NULL) && (playerStatus.has_action_by_code(operate_code) == false)) {
			table.log_info("DispatchCard has_action_by_code:" + operate_code);
			return false;
		}
		...
		if (table._playerStatus[target_player].is_respone() == false) {
			table.log_info("max target_player");
			return true;
		}
		...
		case GameConstants.WIK_NULL: {
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					int action = table.estimate_player_respond_xiao(i, _seat_index, _send_card_data, pao_type, true);
		...
		case GameConstants.WIK_PAO: // 杠牌操作
		{
			...
			int pao_type[] = new int[1];
			int action = table.estimate_player_respond_xiao(target_player, _seat_index, _send_card_data, pao_type, true);
			if (action != GameConstants.WIK_NULL) {
	...
	public boolean handler_player_out_card(HHTable table, int seat_index, int card) {
		...
		if (table.has_rule(GameConstants.GAME_RULE_DI_MING_WEI)
				&& table.GRR._cannot_out_index[seat_index][table._logic.switch_to_card_index(card)] >= 1) {
