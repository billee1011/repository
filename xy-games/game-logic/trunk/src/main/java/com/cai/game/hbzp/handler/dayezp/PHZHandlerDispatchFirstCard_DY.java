...
public class PHZHandlerDispatchFirstCard_DY extends PHZHandlerDispatchCard<HBPHZTable> {
	        	...
	        	if(all_hu_xi>=max_hu_xi)
	        	{
					...
					for(int j = 0; j< GameConstants.MAX_HH_INDEX;j++)
					{
						...
						if(table.GRR._cards_index[i][j] == 4)
						{
							...
							table._hu_weave_items[i][weave_count].weave_kind = GameConstants.WIK_HBZP_LONG;
							table._hu_weave_items[i][weave_count].hu_xi = table._logic.get_weave_hu_xi(table._hu_weave_items[i][weave_count],table._fan_jiang_card);
					...
					{
						table.handler_operate_card(i, GameConstants.WIK_ZI_MO,hu_card, -1);
						return ;
					}
