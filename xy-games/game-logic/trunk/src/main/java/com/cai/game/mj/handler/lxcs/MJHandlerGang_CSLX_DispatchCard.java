		...
		for (int i = 0; i < GameConstants.CS_GANG_DRAW_COUNT; i++) {
			...
			for (int k = 0; k < table.getTablePlayerNumber(); k++) {
				...
				if (k == _seat_index) {
							...
							if (cbActionMask != GameConstants.WIK_NULL) {// 有杠
								...
								for (int gc = 0; gc < m_gangCardResult.cbCardCount; gc++) {
									...
									if (is_ting == true) {
										...
										// 把可以杠的这张牌去掉。看是不是听牌
										int bu_index = table._logic
												.switch_to_card_index(m_gangCardResult.cbCardData[gc]);
					...
					if (k == chi_seat_index) {
						bAroseAction = table.estimate_gang_cs_lx_respond(k, _seat_index, bu_card, _double, true);// ,
																													// EstimatKind.EstimatKind_OutCard
					} else {
						bAroseAction = table.estimate_gang_cs_lx_respond(k, _seat_index, bu_card, _double, false);// ,
																													// EstimatKind.EstimatKind_OutCard
					}
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if (table.GRR._chi_hu_rights[i].is_mul(GameConstants.CHR_SHU_FAN)) {
					...
					table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i],
							...
							GameConstants.HU_CARD_TYPE_ZIMO, i);// 自摸
					ChiHuRight chrtemp2 = new ChiHuRight();
					table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i],
					...
					table.process_chi_hu_player_operate_cs(i, new int[] { table._playerStatus[i]._operate_card }, 1,
							false);
					table.process_chi_hu_player_score_cs_lx(i, _seat_index, false);
				}
