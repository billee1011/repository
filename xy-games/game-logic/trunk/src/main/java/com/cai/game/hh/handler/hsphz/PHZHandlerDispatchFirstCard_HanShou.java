...
public class PHZHandlerDispatchFirstCard_HanShou extends HHHandlerDispatchCard<HanShouHHTable> {
	...
	public void exe(HanShouHHTable table) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if ((ti_count >= 3) || (sao_count >= 5) || (ti_count + sao_count) >= 5) {
				...
				if (all_hu_xi >= 15) {
					...
					// if (table.chu_hei_dui(table._hu_weave_items[i])) {
					// chr.opr_or(GameConstants.CHR_CHUN_HEI_DUI);
					// }
					// if (table.chu_hong_dui(table._hu_weave_items[i]) >=
					// 13) {
					// chr.opr_or(GameConstants.CHR_HONG_WU_DUI);
					// }
					// if (table.chu_hong_dui(table._hu_weave_items[i]) >=
					// 10 && table.chu_hong_dui(table._hu_weave_items[i]) <
					// 13) {
					// chr.opr_or(GameConstants.CHR_HONG_DUI);
					// }
					// if
					// (table._logic.calculate_pengpeng_count(table._hu_weave_items[i],
					// table._hu_weave_count[i])) {
					// }
					// if ((ti_count + sao_count) == 14 || (ti_count == 4 &&
					// sao_count == 8)) {
		...
		if (is_hu == false) {
			...
			int huSeat = -1;
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				int seatIndex = (_seat_index + table.getTablePlayerNumber() + i) % table.getTablePlayerNumber();
				PlayerStatus curPlayerStatus = table._playerStatus[seatIndex];
				if (!curPlayerStatus.has_action()) {
					continue;
				}
				...
				table.process_chi_hu_player_operate(seatIndex, 0, true);
				table.process_chi_hu_player_score_phz(seatIndex, _seat_index, 0, true);

				// 记录
				if (table.GRR._chi_hu_rights[seatIndex].da_hu_count > 0) {
					table._player_result.da_hu_zi_mo[seatIndex]++;
				} else {
					table._player_result.xiao_hu_zi_mo[seatIndex]++;
				}
				table.countChiHuTimes(seatIndex, true);
				break;
			}
