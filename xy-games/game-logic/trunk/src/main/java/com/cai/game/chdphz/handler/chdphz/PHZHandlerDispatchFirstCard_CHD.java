...
public class PHZHandlerDispatchFirstCard_CHD extends PHZHandlerDispatchCard<CHDPHZTable> {
			...
			if((ti_count >= 3)||(sao_count >= 5))
			{
	        	...
	        	if(all_hu_xi>=15)
	        	{
					...
					if (card_type == GameConstants.HU_CARD_TYPE_ZIMO ) {
						if(_seat_index == i)
							chr.opr_or(GameConstants.CHR_TIAN_HU_CHD);
						else 
							chr.opr_or(GameConstants.CHR_DI_HU_CHD);
						chr.opr_or(GameConstants.CHR_TING_HU_CHD);
...
//			}
			...
			table.process_chi_hu_player_score_phz_chd(target_player, _seat_index, operate_card, true);
