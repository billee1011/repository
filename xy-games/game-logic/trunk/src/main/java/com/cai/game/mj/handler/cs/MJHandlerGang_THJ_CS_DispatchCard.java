...
public class MJHandlerGang_THJ_CS_DispatchCard extends MJHandler {
	...
	int[][] special_player_cards;

	public MJHandlerGang_THJ_CS_DispatchCard() {
	...
	public void exe(MJTable table) {
		...
		special_player_cards = new int[table.getTablePlayerNumber()][GameConstants.CS_GANG_DRAW_COUNT];
		...
		for (int i = 0; i < GameConstants.CS_GANG_DRAW_COUNT; i++) {
			...
			for (int k = 0; k < table.getTablePlayerNumber(); k++) {
				...
				if (k == _seat_index) {
					...
					int action = table.analyse_chi_hu_card(table.GRR._cards_index[k], table.GRR._weave_items[k], table.GRR._weave_count[k], bu_card,
							chr, GameConstants.HU_CARD_TYPE_ZIMO, k);// 自摸
					if (action != GameConstants.WIK_NULL) {
						special_player_cards[k][i] += GameConstants.CARD_ESPECIAL_TYPE_CAN_WIN;
					...
					if (table.GRR._left_card_count > 2) {
						int cbActionMask = table._logic.analyse_gang_card_cs(table.GRR._cards_index[k], bu_card, table.GRR._weave_items[k],
								table.GRR._weave_count[k], m_gangCardResult);
						...
						if (cbActionMask != GameConstants.WIK_NULL) {
							...
							for (int gc = 0; gc < m_gangCardResult.cbCardCount; gc++) {
								...
								if (is_ting == true) {
									...
									can_gang = table.is_cs_ting_card(table.GRR._cards_index[k], table.GRR._weave_items[k], table.GRR._weave_count[k],
											k);
					...
					if (k == chi_seat_index) {
						bAroseAction = table.estimate_gang_thj_cs_respond(k, _seat_index, bu_card, _double, true, special_player_cards, i);
					} else {
						bAroseAction = table.estimate_gang_thj_cs_respond(k, _seat_index, bu_card, _double, false, special_player_cards, i);
					}
				}
		...
		GameSchedule.put(new Runnable() {
			@Override
			public void run() {
				// 和长沙麻将不一样，土豪金长沙麻将，如果杠翻出来的牌，玩家能胡，需要有特效，并且需要分别对待
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					table.operate_out_card(_seat_index, GameConstants.CS_GANG_DRAW_COUNT, special_player_cards[i], GameConstants.OUT_CARD_TYPE_LEFT,
							i);
				}
				...
				if (has_action == false) {
					...
					GameSchedule.put(new AddDiscardRunnable(table.getRoom_id(), _seat_index, _gang_card_data.get_card_count(),
							_gang_card_data.get_cards(), true, table.getMaxCount()), GameConstants.GANG_CARD_CS_DELAY, TimeUnit.MILLISECONDS);
		...
		}, table.time_for_tou_zi_animation + table.time_for_tou_zi_fade + table.time_for_run_delay, TimeUnit.MILLISECONDS);
	}
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		MahjongUtils.recordActionWhenSomeoneClickWin(table);
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			// 下局胡牌的是庄家
			table.set_niao_card_cs(table.GRR._banker_player, GameConstants.INVALID_VALUE, true, 0, false);// 结束后设置鸟牌MJGameConstants.INVALID_VALUE
			...
			// table.GRR._chi_hu_rights[_seat_index],MJGameConstants.HU_CARD_TYPE_ZIMO);
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			if (jie_pao_count > 1) {
				table._cur_banker = _seat_index;
				table.set_niao_card_cs(table.GRR._banker_player, GameConstants.INVALID_VALUE, true, 0, true);
			} else {
				table._cur_banker = target_player;
				table.set_niao_card_cs(table.GRR._banker_player, GameConstants.INVALID_VALUE, true, 0, false);
			}
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				// table.GRR._chi_hu_rights[i],MJGameConstants.HU_CARD_TYPE_PAOHU);
	...
	public boolean handler_player_be_in_room(MJTable table, int seat_index) {
		...
		table.operate_out_card(_seat_index, GameConstants.CS_GANG_DRAW_COUNT, special_player_cards[seat_index], GameConstants.OUT_CARD_TYPE_LEFT,
				seat_index);
