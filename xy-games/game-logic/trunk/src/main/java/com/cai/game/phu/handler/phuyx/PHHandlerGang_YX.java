...
public class PHHandlerGang_YX extends PHHandlerGang<PHTable> {
...
//			table.operate_minus_discard(seat_index, table.GRR._discard_count[seat_index], table.GRR._discard_cards[seat_index] ,false);
	...
	public boolean handler_status_operate_card(PHTable table, int seat_index, int operate_code, int operate_card,int luoCode,int handler_status) {
		...
		if (playerStatus.has_action() == false) {
			table.log_player_error(seat_index, "phHandlerGang_YX出牌,玩家操作已失效");
			return false;
		}
//		assert handler_status == GameConstants.PH_STATUS_GANG;
		if(handler_status != GameConstants.PH_STATUS_GANG){
			table.log_error("phHandlerGang_YX handler_status = "+ handler_status+"!= "+GameConstants.PH_STATUS_GANG+"seat_index = " + seat_index);
//			return false;
		}
...
//				table.operate_player_get_card(this._provide_player, 1, new int[] { _center_card }, GameConstants.INVALID_SEAT,false,1);
	 ...
	 * 
	 ***/
	protected boolean exe_gang(PHTable table) {
...
//			table.GRR._weave_count[_seat_index]++;
		...
		if((_depatch == true)&&(table._ti_two_long[_seat_index] == false)&&(table._is_xiang_gong[_seat_index] == false)&&(_action != GameConstants.WIK_PAO)){
			...
			if(action_hu != GameConstants.WIK_NULL||pai_count == 0){
				...
				if(table.has_rule(GameConstants.GAME_RULE_QZ_HU_PAI_ON)||pai_count == 0||table._warning[_seat_index] == 2)
				{
					table._playerStatus[_seat_index].set_handler_status(GameConstants.PH_STATUS_GANG);
				...
				else if(pai_count != 0)
				{
				 ...
				 if (tempPlayerStatus.has_action()) {
					 ...
					 tempPlayerStatus.set_handler_status(GameConstants.PH_STATUS_GANG);
					 //操作状态
