...
public class PHZHandlerGang_DY extends PHZHandlerGang<HBPHZTable> {
...
//		}
		...
		case GameConstants.WIK_NULL:
		{
			...
			if((table._is_xiang_gong[_seat_index] == false )&&(table._long_count[_seat_index] <= 1 &&table._is_cannot_kai_zhao[_seat_index] == -1))
			{    
				...
				if(table._long_count[_seat_index] > 1 &&table._is_cannot_kai_zhao[_seat_index] == -1)
...
//			}
			...
			table.process_chi_hu_player_score_dy(target_player, this._provide_player, operate_card, true);

		
			table.countChiHuTimes(_seat_index, this._provide_player,true);
		...
		case GameConstants.WIK_HBZP_CHI_HU: // 自摸
		{
			table.GRR._chi_hu_rights[_seat_index].set_valid(true);
...
//			}
			...
			table.process_chi_hu_player_score_dy(target_player, this._provide_player, operate_card, false);
			table.process_chi_hu_player_operate(target_player, operate_card, false);
			table.countChiHuTimes(target_player, this._provide_player,false);
	 ...
	 * 
	 ***/
	protected boolean exe_gang(HBPHZTable table) {
		int cbCardIndex = table._logic.switch_to_card_index(_center_card);
		int cbWeaveIndex = -1;

		if (GameConstants.PAO_TYPE_OHTER_PAO  == _type) {
			// 别人打的牌
		...
		int hu_xi_count = table._logic.get_weave_hu_xi(table.GRR._weave_items[_seat_index][cbWeaveIndex],table._fan_jiang_card);
		table.operate_player_cards(_seat_index, hand_card_count, cards, table.GRR._weave_count[_seat_index],
				table.GRR._weave_items[_seat_index]);
		int card_count = table._logic.get_card_count_by_index(table.GRR._cards_index[_seat_index]);
		if(card_count == 0 )
		{
		...
		if(table._long_count[_seat_index] == 5)
			table._is_cannot_kai_zhao[_seat_index] = 2;
		...
		if((_depatch == false)&&(table._ti_two_long[_seat_index] == false)&&(table._is_xiang_gong[_seat_index] == false)){
			// 变量定义
			ChiHuRight chr = new ChiHuRight();
		...
		if((table._is_xiang_gong[_seat_index] == false )&&(table._long_count[_seat_index] == 1))
		{    
			//要出牌，但是没有牌出设置成相公  下家用户发牌
			if(table._ti_mul_long[_seat_index] > 1 && table._is_cannot_kai_zhao[_seat_index] == -1&&is_out_card==true){
			...
			else if(table._ti_mul_long[_seat_index] == 1 && table._is_cannot_kai_zhao[_seat_index] == -1&&is_out_card==true){
				if(table._ti_mul_long[_seat_index]>0)
					table._ti_mul_long[_seat_index]--;
				table._playerStatus[_seat_index].add_action(GameConstants.WIK_HBZP_OUT_CARD);
				table._playerStatus[_seat_index].add_out_card(_center_card,GameConstants.WIK_HBZP_OUT_CARD, _seat_index);
				table._playerStatus[_seat_index].add_action(GameConstants.WIK_NULL);
				table._playerStatus[_seat_index].add_pass(_center_card, _seat_index);
				table._playerStatus[_seat_index].set_status(GameConstants.Player_Status_OUT_CARD);// 出牌状态
				table.operate_player_action(_seat_index, false);
				table.operate_player_status();
			}
			else if(table._ti_mul_long[_seat_index] == 0)
			{
				//胡牌了不执行
				table._playerStatus[_seat_index].set_status(GameConstants.Player_Status_OUT_CARD);// 出牌状态
				table.operate_player_status();
			...
			if(table._long_count[_seat_index] > 2 && table._is_cannot_kai_zhao[_seat_index] == -1){
				if(table._ti_mul_long[_seat_index]>0)
					table._ti_mul_long[_seat_index]--;
				table._is_cannot_kai_zhao[_seat_index] = 2;
				table._playerStatus[_seat_index].set_status(GameConstants.Player_Status_OUT_CARD);// 出牌状态
			...
			else if(table._long_count[_seat_index] == 2 && table._is_cannot_kai_zhao[_seat_index] == -1&&is_out_card == true){
				if(table._ti_mul_long[_seat_index]>0)
					table._ti_mul_long[_seat_index]--;
				table._playerStatus[_seat_index].add_action(GameConstants.WIK_HBZP_OUT_CARD);
				table._playerStatus[_seat_index].add_out_card(_center_card,GameConstants.WIK_HBZP_OUT_CARD, _seat_index);
