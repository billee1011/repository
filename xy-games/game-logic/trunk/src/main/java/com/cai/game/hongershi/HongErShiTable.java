...
public class HongErShiTable extends HHTable {
	...
	public boolean estimate_player_mo_card_response(int seat_index, int card, int operate_card, boolean dipatch) {
		...
		// if (player_bao_ting[seat_index] && check_si_dui(seat_index)) {
		// return false;
		// }
	...
	public void refresh_game_status(boolean is_record) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_DEH_REFRESH_GAME_STATUS);
		roomResponse.setGameStatus(GameConstants.GS_MJ_PIAO);
		this.send_response_to_room(roomResponse);

		if (is_record && GRR != null) {
	...
	public int analyse_chi_hu_card(int seatIndex, int provider_index, int card, int type, ChiHuRight chr, boolean dispatch) {
		...
		if (type == HongErShiConstants.WIK_TIAN_HU) {
			...
			if (!can_jie_pao && !is_si_dui(cardsIndex) && GRR._weave_count[seatIndex] == weave_count_chi
			...
			if (!can_jie_pao && !is_si_dui(cardsIndex) && type != HongErShiConstants.WIK_QIANG_GANG && GRR._weave_count[seatIndex] == weave_count_chi
					&& has_rule(HongErShiConstants.RULE_YI_FAN_QI_HU)) {
				chr.set_empty();
				return GameConstants.WIK_NULL;
			}
		...
		if (is_si_dui(cardsIndex)) {
			chr.opr_or(HongErShiConstants.WIK_SI_DUI);
		...
		// }
		// }
		if (red > 0 && red < 20 && check_14_or_red && !check_magic_hu) {
			chr.set_empty();
			return GameConstants.WIK_NULL;
		}
	...
	public int analyse_chi_hu_card_bao_ting(int seatIndex, int provider_index, int card, int type, ChiHuRight chr, boolean dispatch) {
		...
		if (type == HongErShiConstants.WIK_TIAN_HU) {
			...
			if (!is_si_dui(cardsIndex) && type != HongErShiConstants.WIK_QIANG_GANG && GRR._weave_count[seatIndex] == weave_count_chi
					&& has_rule(HongErShiConstants.RULE_YI_FAN_QI_HU)) {
				chr.set_empty();
				return GameConstants.WIK_NULL;
			}
	...
	public boolean is_si_dui(int[] cards_value_index) {
	...
	public void test_cards() {
		int[] cards_of_player0 = new int[] { 0x0c, 0x1c, 0x2c, 0x04, 0x14, 0x05, 0x06 };
		int[] cards_of_player1 = new int[] { 0x0c, 0x1c, 0x2c, 0x04, 0x14, 0x05, 0x06 };
		int[] cards_of_player2 = new int[] { 0x0a, 0x2a, 0x04, 0x06, 0x08, 0x14, 0x09 };
