...
public class MJHandlerPiCi_lygc extends MJHandler {
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		table.process_chi_hu_player_operate_pi_ci(_seat_index, table._logic.switch_to_card_data(table._logic.get_ci_card_index()), false);
		...
		GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._cur_banker, GameConstants.Game_End_NORMAL), table.game_finish_delay_lygc,
				TimeUnit.SECONDS);
	...
	public boolean handler_player_be_in_room(MJTable table, int seat_index) {
		...
		for (int j = 0; j < hand_card_count; j++) {
			if (table._logic.is_ci_card(table._logic.switch_to_card_index(hand_cards[j]))) {
				hand_cards[j] += GameConstants.CARD_ESPECIAL_TYPE_CI;
			}
		}
