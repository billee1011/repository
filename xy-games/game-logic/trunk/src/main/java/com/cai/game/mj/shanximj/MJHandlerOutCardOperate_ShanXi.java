...
public class MJHandlerOutCardOperate_ShanXi extends MJHandlerOutCardOperate<Table_ShanXi> {
	...
	public void exe(Table_ShanXi table) {
		...
		// 红中麻将,检查听牌
		table._playerStatus[_out_card_player]._hu_card_count = table.get_hnhz_ting_card_new(table._playerStatus[_out_card_player]._hu_cards, table.GRR._cards_index[_out_card_player], table.GRR._weave_items[_out_card_player], table.GRR._weave_count[_out_card_player], has_feng);
		...
		// 玩家出牌 响应判断,是否有吃碰杠补胡
		boolean bAroseAction = table.estimate_player_out_card_respond_hnhz(_out_card_player, _out_card_data);// ,
...
//			table.exe_add_discard(_out_card_player, 1, new int[] { _out_card_data }, false, GameConstants.DELAY_SEND_CARD_DELAY);
	...
	public boolean handler_operate_card(Table_ShanXi table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			...
			// table.process_chi_hu_player_operate(seat_index, operate_card,
			// false);
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._cur_banker, GameConstants.Game_End_NORMAL), GameConstants.GAME_FINISH_DELAY, TimeUnit.SECONDS);
