...
public class HandlerGang_ZPTDH extends MJHandlerGang<Table_ZPTDH> {
	...
	public void exe(Table_ZPTDH table) {
		...
		if(table.hasRule(Constants_MJ_ZPTDH.GAME_RULE_12_LD_CB)){
			...
			if(wIndex == 2 && GameConstants.GANG_TYPE_ADD_GANG != _type){
			...
			if(wIndex == 3 && GameConstants.GANG_TYPE_JIE_GANG == _type){
	...
	@Override
	public boolean exe_gang(Table_ZPTDH table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			if(table.GRR._weave_count[_seat_index] == 4){
				table.cBMap.put(_seat_index, _provide_player);
			}
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_provide_player] -= 3;
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += 3;
		...
		// 从后面发一张牌给玩家
		int type = 1;
		if(table.hasRule(Constants_MJ_ZPTDH.GAME_RULE_FG_GK_CB) && GameConstants.GANG_TYPE_JIE_GANG == _type)
			type = Constants_MJ_ZPTDH.HU_CARD_TYPE_FANG_GANG_KAI_HU;
		
		table.exe_dispatch_card(_seat_index, type, 0);
		return true;
	}
	...
	private void handler_operate_tong_pao(Table_ZPTDH table, int seat_index,
        ...
        for (int i = 0; i < table.getTablePlayerNumber(); i++) {
            ...
            if(!table.hasRule(Constants_MJ_ZPTDH.GAME_RULE_ZI_MO_MAI_MA))
            	table.set_niao_card(i);
            jie_pao_count++;
        }
        ...
        if (jie_pao_count > 0) {
            ...
            table.GRR._chi_hu_rights[_seat_index].opr_or(Constants_MJ_ZPTDH.CHR_BEI_QIANG_GANG);
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if ((table.GRR._chi_hu_rights[i].is_valid() == false)) {
                    continue;
                }
	...
	private void handler_operate_bu_tong_pao(Table_ZPTDH table, int seat_index,
			int operate_code, int operate_card) {
		int target_player = seat_index;
        ...
        case GameConstants.WIK_CHI_HU: {
            ...
            table.GRR._chi_hu_rights[_seat_index].opr_or(Constants_MJ_ZPTDH.CHR_FANG_PAO);
