...
public class HandlerGang_GY extends MJHandlerGang<Table_GY> {
	...
	public boolean exe_gang(Table_GY table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			table.GRR._weave_count[_seat_index]++;
			if (table.is_ji_card(_center_card)) {
				if (_center_card == GameConstants_GY.BA_TONG_CARD) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			int score = 3;
			// table.player_show_desc_11[_seat_index][4] += score;
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
	...
	public boolean handler_operate_card(Table_GY table, int seat_index, int operate_code, int operate_card) {
		...
		if (jie_pao_count > 0) {
			...
			// 连庄玩法 加连庄数
			// if (table.has_rule(GameConstants_GY.GAME_RULE_CONTINUE_BANKER) ||
			// table._game_type_index == GameConstants.GAME_TYPE_GY_EDG
			// || table._game_type_index == GameConstants.GAME_TYPE_GT_SDG) {
