...
public class BYZPHandlerOutCardOperate_GL extends GXZPHandlerOutCardOperate<GXZPTable> {
	@Override
	public void exe(GXZPTable table) {
		...
		while(loop < table.getTablePlayerNumber()){
			...
			loop ++;
			action_pao[i] = table.estimate_player_respond_glzp(i,_out_card_player ,_out_card_data, pao_type[i],false);
			
			if(table.has_rule(GameConstants.GAME_RULE_NO_DIAN_PAO))
			{
				continue;
			}
		...
		{
			PlayerStatus tempPlayerStatus = table._playerStatus[i]; 
			if(tempPlayerStatus.has_chi_hu() == true)
			{
			...
			if(i==_out_card_player) continue;
			 if((action_pao[i] != GameConstants.WIK_NULL)&& (bHupai == 0)){
				 int pao_type_out[] = new int[1];
					ti_pao = table.estimate_player_respond_glzp(i,_out_card_player ,_out_card_data, pao_type_out,false);
	...
	public boolean handler_operate_card(GXZPTable table, int seat_index, int operate_code, int operate_card,int luoCode) {
        ...
        for (int i = 0; i < table.getTablePlayerNumber(); i++) {
            if ((table._playerStatus[i].is_respone() == false) && (table._playerStatus[i].has_chi_hu())&&table.getRuleValue(GameConstants.GAME_RULE_YPSHX)==1)
                return false;
        }
...
//			}
			...
			if(table.getRuleValue(GameConstants.GAME_RULE_YPSHX)==1)
			{
				for(int i = 0; i < table.getTablePlayerNumber();i++)
				{
			...
			if(table.getRuleValue(GameConstants.GAME_RULE_YPSHX)==1)
			{
			    ...
			    for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	        		...
	        		table.process_chi_hu_player_score_bayi(i, this._out_card_player, operate_card, false);
	        		// 效果
	        	
	            	table.GRR._chi_hu_card[i][0] = _out_card_data;

	            	table.countChiHuTimes_bayi(i, false);
	            }
			}
			else
			{
				table.process_chi_hu_player_operate(target_player, operate_card, false);
				table.process_chi_hu_player_score_bayi(target_player, this._out_card_player, operate_card, false);
            	table.GRR._chi_hu_card[target_player][0] = _out_card_data;

            	table.countChiHuTimes_bayi(target_player, false);
			}
	...
	public boolean handler_player_be_in_room(GXZPTable table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < GameConstants.MAX_WEAVE_HH; j++) {
				...
				if (seat_index != i) {
					if ((table.GRR._weave_items[i][j].weave_kind == GameConstants.WIK_AN_LONG
							|| table.GRR._weave_items[i][j].weave_kind == GameConstants.WIK_TI_LONG
							|| table.GRR._weave_items[i][j].weave_kind == GameConstants.WIK_XIAO
							)
