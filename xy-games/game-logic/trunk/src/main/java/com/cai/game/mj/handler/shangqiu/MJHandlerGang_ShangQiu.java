...
public class MJHandlerGang_ShangQiu extends MJHandlerGang<MJTable_ShangQiu> {
    ...
    public void exe(MJTable_ShangQiu table) {
        ...
        if ((GameConstants.GANG_TYPE_AN_GANG == _type) || (GameConstants.GANG_TYPE_JIE_GANG == _type)
                || table.has_rule(GameConstants_SQ.GAME_RULE_SQ_YING_KOU) || table._player_result.nao[_seat_index] > 0 // 暗杆锁死状态直接执行杠，不检测抢杠胡
        ) {
            this.exe_gang(table);
            return;
        }
     ...
     ***/
    @Override
    protected boolean exe_gang(MJTable_ShangQiu table) {
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            ...
            if (table.has_rule(GameConstants_SQ.GAME_RULE_SQ_AN_GANG_SUO_SI)) {
                table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1,
                        new long[] { GameConstants.WIK_AN_GANG_SHUO_SI }, 1, GameConstants.INVALID_SEAT);
                for (int i = 0; i < table.getTablePlayerNumber(); i++) {
        ...
        if (GameConstants.GANG_TYPE_AN_GANG != _type) {
            ...
            for (int i = 0; i < table.GRR.get_liang_card_count(_seat_index); i++) {
                if (liang_card_hand[i] == _center_card) {
                    int num = table.GRR._weave_items[_seat_index][cbWeaveIndex].client_special_count;
                    table.GRR._weave_items[_seat_index][cbWeaveIndex].client_special_card[num] = _center_card;
                    table.GRR._weave_items[_seat_index][cbWeaveIndex].client_special_count++;
                }
            }
        }
        ...
        if (table.has_rule(GameConstants_SQ.GAME_RULE_SQ_LIANG_SI_DA_YI)) {
            if (table.GRR.remove_liang_pai(_center_card, _seat_index)) {
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            ...
            // 放杠1分
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] = GameConstants.CELL_SCORE;// m_pGameServiceOption->lCellScore*2;//配置参数乘
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][table.GRR._weave_items[_seat_index][cbWeaveIndex].provide_player] = -1
                    * GameConstants.CELL_SCORE;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
            table._player_result.ming_gang_count[_seat_index]++;
        }

        // 开杠次数
        table.kai_gang_count++;
