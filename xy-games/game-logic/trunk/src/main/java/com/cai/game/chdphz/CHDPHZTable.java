...
import com.cai.game.chdphz.CHDPHZGameLogic.AnalyseItem;
import com.cai.game.chdphz.handler.PHZHandler;
import com.cai.game.chdphz.handler.PHZHandlerChiPeng;
import com.cai.game.chdphz.handler.PHZHandlerDispatchCard;
import com.cai.game.chdphz.handler.PHZHandlerFinish;
import com.cai.game.chdphz.handler.PHZHandlerGang;
import com.cai.game.chdphz.handler.PHZHandlerOutCardOperate;
import com.cai.game.chdphz.handler.chdphz.PHZHandlerChiPeng_CHD;
import com.cai.game.chdphz.handler.chdphz.PHZHandlerChuLiFirstCard_CHD;
import com.cai.game.chdphz.handler.chdphz.PHZHandlerDispatchCard_CHD;
import com.cai.game.chdphz.handler.chdphz.PHZHandlerDispatchFirstCard_CHD;
import com.cai.game.chdphz.handler.chdphz.PHZHandlerGang_CHD;
import com.cai.game.chdphz.handler.chdphz.PHZHandlerOutCardOperate_CHD;
import com.cai.game.chdphz.handler.dhd.DHDHandlerChiPeng_CHD;
import com.cai.game.chdphz.handler.dhd.DHDHandlerChuLiFirstCard_CHD;
import com.cai.game.chdphz.handler.dhd.DHDHandlerDispatchCard_CHD;
import com.cai.game.chdphz.handler.dhd.DHDHandlerDispatchFirstCard_CHD;
import com.cai.game.chdphz.handler.dhd.DHDHandlerGang_CHD;
import com.cai.game.chdphz.handler.dhd.DHDHandlerOutCardOperate_CHD;
import com.cai.game.chdphz.handler.hhd.HHHandlerChiPeng_CHD;
import com.cai.game.chdphz.handler.hhd.HHHandlerChuLiFirstCard_CHD;
import com.cai.game.chdphz.handler.hhd.HHHandlerDispatchCard_CHD;
import com.cai.game.chdphz.handler.hhd.HHHandlerDispatchFirstCard_CHD;
import com.cai.game.chdphz.handler.hhd.HHHandlerGang_CHD;
import com.cai.game.chdphz.handler.hhd.HHHandlerOutCardOperate_CHD;
import com.cai.game.chdphz.handler.lbaphz.PHZHandlerChiPeng_LBA;
import com.cai.game.chdphz.handler.lbaphz.PHZHandlerChuLiFirstCard_LBA;
import com.cai.game.chdphz.handler.lbaphz.PHZHandlerDispatchCard_LBA;
import com.cai.game.chdphz.handler.lbaphz.PHZHandlerDispatchFirstCard_LBA;
import com.cai.game.chdphz.handler.lbaphz.PHZHandlerGang_LBA;
import com.cai.game.chdphz.handler.lbaphz.PHZHandlerOutCardOperate_LBA;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class CHDPHZTable extends AbstractRoom {
	/**
	 * 
	 */
	private static final long serialVersionUID = 7060061356475703643L;

	private static Logger logger = Logger.getLogger(CHDPHZTable.class);
	...
	public CHDPHZGameLogic _logic = null;
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_PHZ_CHD) || is_mj_type(GameConstants.GAME_TYPE_PHZ_CHD_DT)) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_HH_CHD) || is_mj_type(GameConstants.GAME_TYPE_HH_CHD_DT)) {
			_handler_dispath_card = new HHHandlerDispatchCard_CHD();
			_handler_out_card_operate = new HHHandlerOutCardOperate_CHD();
			_handler_gang = new HHHandlerGang_CHD();
			_handler_chi_peng = new HHHandlerChiPeng_CHD();

			_handler_chuli_firstcards = new HHHandlerChuLiFirstCard_CHD();
			_handler_dispath_firstcards = new HHHandlerDispatchFirstCard_CHD();
		}
		if (is_mj_type(GameConstants.GAME_TYPE_DHD_CHD) || is_mj_type(GameConstants.GAME_TYPE_DHD_CHD_DT)) {
			_handler_dispath_card = new DHDHandlerDispatchCard_CHD();
			_handler_out_card_operate = new DHDHandlerOutCardOperate_CHD();
			_handler_gang = new DHDHandlerGang_CHD();
			_handler_chi_peng = new DHDHandlerChiPeng_CHD();

			_handler_chuli_firstcards = new DHDHandlerChuLiFirstCard_CHD();
			_handler_dispath_firstcards = new DHDHandlerDispatchFirstCard_CHD();
		}
		if (is_mj_type(GameConstants.GAME_TYPE_LBA_CHD) || is_mj_type(GameConstants.GAME_TYPE_LBA_CHD_DT)) {
			_handler_dispath_card = new PHZHandlerDispatchCard_LBA();
			_handler_out_card_operate = new PHZHandlerOutCardOperate_LBA();
			_handler_gang = new PHZHandlerGang_LBA();
			_handler_chi_peng = new PHZHandlerChiPeng_LBA();

			_handler_chuli_firstcards = new PHZHandlerChuLiFirstCard_LBA();
			_handler_dispath_firstcards = new PHZHandlerDispatchFirstCard_LBA();
		}
	...
	protected boolean on_handler_game_start() {
		...
		if(this.getRuleValue(GameConstants.GAME_RULE_CAN_LESS) == 1)
		{
			...
			if (this.getRuleValue(GameConstants.GAME_RULE_HGW_MINUE_20_CARD) == 1)
				GRR._left_card_count -= 20;
...
//				 24, 2, 23, 22, 1, 1, 2, 5, 10, 3, 21, 19, 6, 21, 18, 6, 25, 22, 3, 23, 20, 5, 7, 19, 25,
//				 8, 3, 1, 26, 7, 2, 7, 8, 5, 25, 7, 21, 8, 10, 9, 24, 9, 5, 9, 9, 10, 1, 25, 18, 6, 18, 19, 
//				 24, 26, 23, 21, 10, 17, 3, 4, 20, 26, 19, 18,
//		
//		 };
//		 this._cur_banker = 0;
...
//		 this.DEBUG_CARDS_MODE = true;
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_PHZ_CHD) || is_mj_type(GameConstants.GAME_TYPE_DHD_CHD) || is_mj_type(GameConstants.GAME_TYPE_LBA_CHD)
				|| is_mj_type(GameConstants.GAME_TYPE_PHZ_CHD_DT) || is_mj_type(GameConstants.GAME_TYPE_DHD_CHD_DT)
				|| is_mj_type(GameConstants.GAME_TYPE_LBA_CHD_DT)) {
	...
	public boolean handler_game_finish_phz_chd(int seat_index, int reason) {
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL ||
			...
			} else if (is_mj_type(GameConstants.GAME_TYPE_PHZ_CHD) || is_mj_type(GameConstants.GAME_TYPE_PHZ_CHD_DT)) {
				if (reason == GameConstants.Game_End_DRAW) {
					this._huang_zhang_count++;
				} else {
					this._huang_zhang_count = 0;
	...
	public int analyse_chi_hu_card(int cards_index[], WeaveItem weaveItems[], int weave_count, int seat_index, int provider_index, int cur_card,
		...
		if (GameConstants.GAME_TYPE_HH_CHD == _game_type_index || GameConstants.GAME_TYPE_HH_CHD_DT == _game_type_index) {
		...
		if (GameConstants.GAME_TYPE_PHZ_CHD == _game_type_index || GameConstants.GAME_TYPE_PHZ_CHD_DT == _game_type_index) {
		...
		if (GameConstants.GAME_TYPE_LBA_CHD == _game_type_index || GameConstants.GAME_TYPE_LBA_CHD_DT == _game_type_index) {
			return analyse_chi_hu_card_phz_lba(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type,
					hu_xi);
		}
		if (GameConstants.GAME_TYPE_DHD_CHD == _game_type_index || GameConstants.GAME_TYPE_DHD_CHD_DT == _game_type_index) {
			return analyse_chi_hu_card_dhd(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type, hu_xi);
		}
		return 0;
	}
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_phz_lba(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index,
		...
		int hu_xi[] = new int[1];
		hu_xi[0] = 0;
		boolean yws = has_rule(GameConstants.GAME_RULE_YWS_CHD);
			...
			for (int i = 0; i < weaveCount; i++) {
				if ((cur_card == weaveItems[i].center_card)
					...
					boolean temp_bValue = _logic.analyse_card_phz(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index, cur_card,
							analyseItemArray, false, hu_xi, yws);
		...
		for (int j = 0; j < 7; j++) {
			...
			if (this.getRuleValue(GameConstants.GAME_RULE_YING_CHD) == 1) {
				this._ying_hu_count[seat_index] += _logic.get_ying(_hu_weave_items[seat_index][j].weave_kind,
						_hu_weave_items[seat_index][j].center_card);
			}
			if (this.getRuleValue(GameConstants.GAME_RULE_PIAO_CHD) == 1 && is_piao != 2) {
				int piao = _logic.get_piao(_hu_weave_items[seat_index][j].weave_kind, _hu_weave_items[seat_index][j].center_card);
				if (piao == 2)
					is_piao = 2;
				if (piao == 1)
					piao_count++;
		...
		if (card_type == GameConstants.HU_CARD_TYPE_ZIMO && (seat_index == provider_index)) {
			...
			chiHuRight.opr_or_long(GameConstants.CHR_JEI_PAO_HU_CHD);
		}
		if (hong_pai_count >= 10) {
			_hong_pai_count[seat_index] = hong_pai_count;
			chiHuRight.opr_or_long(GameConstants.CHR_TEN_HONG_PAI_CHD);
		}
		if (has_rule(GameConstants.GAME_RULE_SI_QI_HONG_CHD))
			if (hong_pai_count == 4 || hong_pai_count == 7) {
				chiHuRight.opr_or_long(GameConstants.CHR_SI_QI_CHD);
			}
		if (hong_pai_count == 1) {
			chiHuRight.opr_or_long(GameConstants.CHR_ONE_HONG_CHD);
		}
		...
		if (this._da_pai_count[seat_index] >= 18) {
			chiHuRight.opr_or_long(GameConstants.CHR_DA_HU_CHD);
		}
		if (this._xiao_pai_count[seat_index] >= 16) {
			chiHuRight.opr_or_long(GameConstants.CHR_XIAO_HU_CHD);
		}
		if (GRR._left_card_count == 0) {
			chiHuRight.opr_or_long(GameConstants.CHR_HAI_HU_CHD);
		}
		if (_logic.calculate_pengpeng_count(_hu_weave_items[seat_index], _hu_weave_count[seat_index])) {
			chiHuRight.opr_or_long(GameConstants.CHR_DUI_ZI_HU_CHD);
		}
		if (this.has_rule(GameConstants.GAME_RULE_HHX_CHD)) {
			if (hang_hang_xi_count == 7) {
				chiHuRight.opr_or_long(GameConstants.CHR_HANG_HANG_XI_CHD);
			}
		}
		if (this.has_rule(GameConstants.GAME_RULE_JIA_HHX_CHD)) {
			if (hang_hang_xi_count == 6) {
				chiHuRight.opr_or_long(GameConstants.CHR_HANG_HANG_XI_lIU_CHD);
			}
		}
		if (this.has_rule(GameConstants.GAME_RULE_TUAN_CHD)) {
			_tuan_yuan_count[seat_index] = _logic.calculate_tuan_count(_hu_weave_items[seat_index], _hu_weave_count[seat_index]);

			if (_tuan_yuan_count[seat_index] > 0) {
				chiHuRight.opr_or_long(GameConstants.CHR_TUAN_CHD);
			}
		}
		if (this.getRuleValue(GameConstants.GAME_RULE_SHUN_CHD) == 1) {
			if (_logic.calculate_shun(_hu_weave_items[seat_index], _hu_weave_count[seat_index])) {
				chiHuRight.opr_or_long(GameConstants.CHR_SHUN_CHD);
			}
		}
		if (this.getRuleValue(GameConstants.GAME_RULE_PIAO_CHD) == 1 && is_piao != 2) {
			if (piao_count == 1) {
				chiHuRight.opr_or_long(GameConstants.CHR_DAN_PIAO_CHD);
			}
			if (piao_count == 2) {
				chiHuRight.opr_or_long(GameConstants.CHR_SHUANG_PIAO_CHD);
			}
		}
		if (this.getRuleValue(GameConstants.GAME_RULE_YING_CHD) == 1) {
			if (this._ying_hu_count[seat_index] > 0)
				chiHuRight.opr_or_long(GameConstants.CHR_YING_CHD);
		}
		if (has_rule(GameConstants.GAME_RULE_BEI_CHD)) {
			if (_logic.is_bei_kao_bei(_playerStatus[seat_index]._hu_cards, _playerStatus[seat_index]._hu_card_count, cur_card, analyseItem,
					weaveCount) == true) {
				chiHuRight.opr_or_long(GameConstants.CHR_BEI_CHD);
			}
		}
		if (has_rule(GameConstants.GAME_RULE_GAI_CHD)) {
			if (this.GRR._left_card_count == 18) {

				chiHuRight.opr_or_long(GameConstants.CHR_GAI_CHD);
		...
		if (card_count == 1) {
			chiHuRight.opr_or_long(GameConstants.CHR_SHUA_HOU_CHD);
		}

		cbChiHuKind = GameConstants.WIK_CHI_HU;

		if ((this._is_di_hu == true) && (GRR._banker_player != seat_index)) {
			chiHuRight.opr_or_long(GameConstants.CHR_DI_HU_CHD);
			return cbChiHuKind;
		}
		if ((this.GRR._left_card_count == 19) && (GRR._banker_player == seat_index)) {
			if (card_type == GameConstants.HU_CARD_TYPE_ZIMO) {
				chiHuRight.opr_or_long(GameConstants.CHR_TIAN_HU_CHD);
			}
			return cbChiHuKind;
		}
	...
	public int analyse_chi_hu_card_phz_chd(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index,
		...
		boolean is_man_yuan_hua = _logic.is_man_yuan_hua(_hu_weave_items[seat_index], _hu_weave_count[seat_index]);
		if (card_type == GameConstants.HU_CARD_TYPE_ZIMO && (seat_index == provider_index)) {
			chiHuRight.opr_or_long(GameConstants.CHR_ZI_MO);
		} else if (card_type == GameConstants.HU_CARD_TYPE_ZIMO && (seat_index != provider_index)) {
			chiHuRight.opr_or_long(GameConstants.CHR_JEI_PAO_HU_CHD);
		...
		if (is_man_yuan_hua == true && has_rule(GameConstants.GAME_RULE_MAN_YUAN_HUA)) {
			chiHuRight.opr_or_long(GameConstants.CHR_MAN_YUAN_HUA_CHD);
		}
		if (this.has_rule(GameConstants.GAME_RULE_HHX_CHD)) {
			if (hang_hang_xi_count == 7) {
				chiHuRight.opr_or_long(GameConstants.CHR_HANG_HANG_XI_CHD);
		...
		if (this.has_rule(GameConstants.GAME_RULE_TUAN_YUAN_CHD)
				&& _logic.is_calculate_tuan_yuan(_hu_weave_items[seat_index], _hu_weave_count[seat_index])) {
			chiHuRight.opr_or_long(GameConstants.CHR_TUAN_YUAN_CHD);
		}
		if (this.has_rule(GameConstants.GAME_RULE_HFT_CHD)) {
			int count = _logic.is_hong(_hu_weave_items[seat_index], _hu_weave_count[seat_index]);
			if (count == 3) {
				chiHuRight.opr_or_long(GameConstants.CHR_HONG_FAN_TIAN_CHD);
				chiHuRight.opr_or_long(GameConstants.CHR_MTH_DA_CHD);
				chiHuRight.opr_or_long(GameConstants.CHR_MTH_XIAO_CHD);
			} else if (count == 2) {
				chiHuRight.opr_or_long(GameConstants.CHR_MTH_DA_CHD);
			} else if (count == 1) {
				chiHuRight.opr_or_long(GameConstants.CHR_MTH_XIAO_CHD);
			}
		} else if (this.has_rule(GameConstants.GAME_RULE_MTH_CHD)) {
			int count = _logic.is_hong(_hu_weave_items[seat_index], _hu_weave_count[seat_index]);
			if (count == 2) {
				chiHuRight.opr_or_long(GameConstants.CHR_MTH_DA_CHD);
			} else if (count == 1) {
		...
		if (this.has_rule(GameConstants.GAME_RULE_DIAN_DENG_CHD)) {
			if (this._dian_deng_count >= 1) {
				chiHuRight.opr_or_long(GameConstants.CHR_HUANG_FAN_CHD);
			}
		} else if (this._huang_zhang_count >= 1) {
			chiHuRight.opr_or_long(GameConstants.CHR_HUANG_FAN_CHD);
		}
		if (has_rule(GameConstants.GAME_RULE_BEI_CHD)) {
			if (_logic.is_bei_kao_bei(_playerStatus[seat_index]._hu_cards, _playerStatus[seat_index]._hu_card_count, cur_card, analyseItem,
		...
		if (flag == 0) {
			chiHuRight.opr_or_long(GameConstants.CHR_TING_HU_CHD);
		}
		cbChiHuKind = GameConstants.WIK_CHI_HU;
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_dhd(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index, int cur_card,
			ChiHuRight chiHuRight, int card_type, int hu_xi_hh[]) {
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if (this._cur_round > 0||this.getRuleValue(GameConstants.GAME_RULE_CAN_LESS)==1)
	...
	public void countChiHuTimes(int _seat_index, boolean isZimo) {
		...
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_HUANG_FAN_CHD)).is_empty()) {
			_player_result.ming_tang_count[_seat_index]++;
		}
	...
	public void set_result_describe(int seat_index) {
		if (this.is_mj_type(GameConstants.GAME_TYPE_HH_CHD) || this.is_mj_type(GameConstants.GAME_TYPE_HH_CHD_DT)) {
			set_result_describe_hh();
		}
		if (this.is_mj_type(GameConstants.GAME_TYPE_PHZ_CHD) || this.is_mj_type(GameConstants.GAME_TYPE_PHZ_CHD_DT)) {
			set_result_describe_phz_chd(seat_index);
		}
		if (this.is_mj_type(GameConstants.GAME_TYPE_DHD_CHD) || this.is_mj_type(GameConstants.GAME_TYPE_DHD_CHD_DT)) {
			set_result_describe_dhd_chd(seat_index);
		}
		if (this.is_mj_type(GameConstants.GAME_TYPE_LBA_CHD) || this.is_mj_type(GameConstants.GAME_TYPE_LBA_CHD_DT)) {
			set_result_describe_lba(seat_index);
		}
