...
public class PHZHandlerDispatchFirstCard_YJ extends PHZHandlerDispatchCard<SCPHZTable> {
...
//		_send_card_data = 0x18;
		...
		{		
			...
			chr.set_empty();
			if(((ti_count + sao_count >= 5))||(ti_count >= 3)||(sao_count >= 5)||(hei_pai_count == 0)||(hong_pai_count == 0))
			{	  	
        		...
        		if(ti_count >= 3)
        		{
					...
					chr.opr_or(GameConstants.CHR_YJ_SHEHU);
        		}
        		if(((ti_count+sao_count>=5))||(sao_count >= 5))
        		{

					chr.opr_or(GameConstants.CHR_YJ_WANHU);
        		}
				if(hei_pai_count == 0)
				{
					chr.opr_or(GameConstants.CHR_YJ_HHU);
				}
				if(hong_pai_count == 0)
				{
					chr.opr_or(GameConstants.CHR_YJ_XHEIHU);
				}
