...
public class HandlerGang_TDH_3D extends MJHandlerGang<Table_TDH_3D> {
	...
	public void reset_status(int seat_index, int provide_player, int center_card, int action, int type, boolean self, boolean d) {
		...
		_action = action;
		_type = type;
		if (GameConstants_TDH.GANG_TYPE_AN_GANG == _type) {
			_p = false;
		} else {
			_p = true;
		}
	...
	public void exe(Table_TDH_3D table) {
		...
		table.operate_effect_action(_seat_index, GameConstants_TDH.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { _action }, 1,
				GameConstants_TDH.INVALID_SEAT);

		if ((GameConstants_TDH.GANG_TYPE_AN_GANG == _type) || (GameConstants_TDH.GANG_TYPE_JIE_GANG == _type)) {
			this.exe_gang(table);
			return;
		}
	...
	@Override
	public boolean exe_gang(Table_TDH_3D table) {
		...
		} else if (GameConstants_TDH.GANG_TYPE_JIE_GANG == _type) {
			// 别人打的牌

			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
			table.operate_remove_discard(this._provide_player, table.GRR._discard_count[_provide_player]);
		} else if (GameConstants_TDH.GANG_TYPE_ADD_GANG == _type) {
			// 看看是不是有碰的牌，明杠
			// 寻找组合
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				int cbWeaveKind = table.GRR._weave_items[_seat_index][i].weave_kind;
				int cbCenterCard = table.GRR._weave_items[_seat_index][i].center_card;
				if ((cbCenterCard == _center_card) && (cbWeaveKind == GameConstants_TDH.WIK_PENG)) {
		...
		WeaveItem weaves[] = new WeaveItem[GameConstants_TDH.MAX_WEAVE];
		...
		for (int i = 0; i < weave_count; i++) {
			...
			weaves[i].provide_player = table.GRR._weave_items[_seat_index][i].provide_player + GameConstants_TDH.WEAVE_SHOW_DIRECT;
		}
		...
		if (GameConstants_TDH.GANG_TYPE_AN_GANG == _type) {
			if (_double) {
				int score = GameConstants_TDH.CELL_SCORE;
				score = 2 * table.get_di_fen();
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					if (i == _seat_index)
						continue;
			...
			if (_double) {
				int score = GameConstants_TDH.CELL_SCORE + GameConstants_TDH.CELL_SCORE + GameConstants_TDH.CELL_SCORE;
		...
		} else if (GameConstants_TDH.GANG_TYPE_ADD_GANG == _type) {// 放碰的人给分
			if (_double) {
				int score = GameConstants_TDH.CELL_SCORE;
				score *= table.get_di_fen();
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	...
	public boolean handler_operate_card(Table_TDH_3D table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants_TDH.WIK_CHI_HU) {
			...
			// 客户端播放放炮动画
			table.GRR._chi_hu_rights[_seat_index].opr_or(GameConstants_TDH.CHR_FANG_PAO);
		...
		if (jie_pao_count > 0) {
			...
			if (jie_pao_count == 1) {
				table.set_niao_card(target_player, GameConstants_TDH.INVALID_VALUE, true, 0);
			} else {
				table.continue_banker = 0;
				table.niao_num = 0;
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				int seat = (_seat_index + i) % table.getTablePlayerNumber();
				if ((seat == _seat_index) || (table.GRR._chi_hu_rights[seat].is_valid() == false)) {
					continue;
				}

				table.GRR._chi_hu_card[seat][0] = operate_card;

				table.process_chi_hu_player_score(seat, _seat_index, operate_card, false, Table_TDH_3D.HU_TYPE_QIANG_GANG_HU);

				table._player_result.jie_pao_count[seat]++;
				table._player_result.dian_pao_count[_seat_index]++;
			}
			if (jie_pao_count == 1) {
				table.set_banker(target_player, true);
			} else {
				table.set_banker(_seat_index, false);
			}
	...
	public boolean handler_player_be_in_room(Table_TDH_3D table, int seat_index) {
		...
		if (table.GRR._chi_hu_rights[seat_index].is_valid()) {
			...
			table.operate_effect_action(_seat_index, GameConstants_TDH.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { _action }, 1, seat_index);
