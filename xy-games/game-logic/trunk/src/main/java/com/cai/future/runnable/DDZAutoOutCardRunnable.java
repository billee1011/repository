...
public class DDZAutoOutCardRunnable extends BaseFuture{
	
	private static Logger logger = LoggerFactory.getLogger(DDZAutoOutCardRunnable.class);


	private int _room_id;
	private int _seat_index;
	private DDZTable _table;
	public DDZAutoOutCardRunnable(int room_id,int seat_index,DDZTable table){
		super(room_id);
		_room_id = room_id;
	...
	public void execute() {
		try {
			...
			try{
				roomLock.lock();
				_table.out_card_time_finish(_seat_index);
			}finally{
				roomLock.unlock();
