...
public class HandlerOutCardOperate_HuaZhou extends MJHandlerOutCardOperate<Table_HuaZhou> {
	@Override
	public void exe(Table_HuaZhou table) {
		...
		// 刷新晒张
		table.refresh_shai_zhang_card(_out_card_player, hand_card_count, cards);
		...
		case GameConstants.WIK_CHI_HU: {
			...
			if (jie_pao_count > 0) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					if (jie_pao_count > 1) {
						table.process_chi_hu_player_score_duopao(i, _out_card_player, _out_card_data, true);
					} else {
						table.process_chi_hu_player_score_duopao(i, _out_card_player, _out_card_data, false);
					}
