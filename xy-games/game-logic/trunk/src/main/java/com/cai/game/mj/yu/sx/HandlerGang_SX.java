...
public class HandlerGang_SX extends MJHandlerGang<Table_SX> {
	...
	@Override
	public boolean exe_gang(Table_SX table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			if (table.has_rule(GameConstants_SX.GAME_RULE_FAN_BEI_AN_GANG)) {
				score *= 2;
			}
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
			...
			int score = GameConstants.CELL_SCORE;
			if (table.has_rule(GameConstants_SX.GAME_RULE_FAN_BEI_DIAN_GANG)) {
				score *= 2;
			}
			...
			table.player_ming_gang_all_count[_seat_index]++;
	...
	public boolean handler_player_be_in_room(Table_SX table, int seat_index) {
		...
		for (int i = 0; i < GameConstants.MAX_COUNT; i++) {
			if (table._logic.is_magic_card(hand_cards[i])) {
				hand_cards[i] += GameConstants_SX.CARD_ESPECIAL_TYPE_LAI_ZI;
			}
