...
public class HandlerGang_WanZai extends MJHandlerGang<Table_WanZai> {
	...
	public void exe(Table_WanZai table) {
		...
		if ((GameConstants.GANG_TYPE_AN_GANG == _type) || (GameConstants.GANG_TYPE_ADD_GANG == _type)) {
			this.exe_gang(table);
			return;
		}
		//判断抢杠胡
	...
	private void exe_tong_pao_hu(Table_WanZai table) {
		...
		table.GRR._chi_hu_rights[_seat_index].opr_or(Constants_WanZai.CHR_BEI_QIANG_GANG);	
        table._cur_banker = _seat_index;
        for (int i = 0; i < table.getTablePlayerNumber(); i++) {
            if ((i == _seat_index) || (table.GRR._chi_hu_rights[i].is_valid() == false)) {
                continue;
            }
            table.process_chi_hu_player_score(i, _seat_index, _seat_index, false);
	...
	@Override
	public boolean exe_gang(Table_WanZai table) {
		...
		float[] scores = new float[table.getTablePlayerNumber()];
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
				int score = 2 * table.getFireScore(i, _seat_index);
			...
			int score = 2 * table.getFireScore(_provide_player, _seat_index);
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
				int score = table.getFireScore(i, _seat_index);
		...
		table.operate_player_score(_seat_index, scores);
