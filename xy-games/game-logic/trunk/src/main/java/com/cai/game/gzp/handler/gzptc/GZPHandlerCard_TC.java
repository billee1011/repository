 ...
 *
 */
public class GZPHandlerCard_TC extends GZPHandlerDispatchCard {
...
//		table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants.INVALID_SEAT);
		...
		if (table.GRR._left_card_count >= left && curPlayerStatus.lock_huan_zhang() == false) {
			...
			if (cbActionMask != GameConstants.WIK_NULL) {// 有杠
				...
				for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
					// 加上补张
					if (!curPlayerStatus.lock_huan_zhang()) {
						curPlayerStatus.add_zhao(m_gangCardResult.cbCardData[i], _seat_index,
	...
	public boolean handler_operate_card(GZPTable table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.GZP_WIK_ZHAO: // 杠牌操作
		{
			for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			table.process_chi_hu_player_score_gzp(_seat_index, _seat_index, operate_card, true);
