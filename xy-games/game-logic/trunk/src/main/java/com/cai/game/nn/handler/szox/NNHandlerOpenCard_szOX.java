...
public class NNHandlerOpenCard_szOX extends NNHandlerOpenCard {
	...
	public boolean handler_open_cards(NNTable table, int seat_index, boolean open_flag) {
		...
		table._logic.get_ox_card(table.GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT, table._game_rule_index,table._game_type_index);
		table._card_type_ox[seat_index] = table._logic.get_card_type(table.GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT,
		...
		if (flag == true) {
			...
			int card_type = table._logic.get_card_type(table.GRR._cards_data[table._win_player_oxtb], GameConstants.OX_MAX_CARD_COUNT,
					table._game_rule_index,table._game_type_index);
			if (card_type >= 10)
				table._next_banker = table._win_player_oxtb;
			else if(GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_JDOX_MBOX))
			{
				table._next_banker = table._win_player_oxtb;
			}
				...
				if (k == seat_index) {
					...
					if(table.is_mj_type(GameConstants.GAME_TYPE_JDOX_YY))
					{
						if (GameDescUtil.has_rule(table.getGameRuleIndexEx(),GameConstants.GAME_RULE_JDOX_FAN_BEN))
							times = table._logic.get_times_mul(table.GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, table._game_rule_index,table._game_type_index);
						if (GameDescUtil.has_rule(table.getGameRuleIndexEx(),GameConstants.GAME_RULE_JDOX_PING_BEN))
							times = table._logic.get_times_ping(table.GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, table._game_rule_index,table._game_type_index);

					}
					else{
						if (table.has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_ONE))
							times = table._logic.get_times_two(table.GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, table._game_rule_index,table._game_type_index);
						if (table.has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_TWO))
							times = table._logic.get_times_one(table.GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, table._game_rule_index,table._game_type_index);
					}
				} else {
					for (int j = 0; j < GameConstants.OX_MAX_CARD_COUNT; j++) {
