...
public class WHZHandlerChiPeng_YiYang extends PHZHandlerChiPeng<YiYangWHZTable> {
	
	private GangCardResult m_gangCardResult;
	
	public WHZHandlerChiPeng_YiYang(){
	...
	public void exe(YiYangWHZTable table) {
		...
		is_liu = table.estimate_player_chipeng_qing_piao_respond_yywhz(_seat_index,_seat_index,_card);
		...
		if(table._logic.check_si_shou(_action,_seat_index,_card,table)){
			for(int i=0;i<GameConstants.MAX_HH_INDEX;i++){
				table.GRR._cannot_out_index[_seat_index][i]++;
			}
		}
