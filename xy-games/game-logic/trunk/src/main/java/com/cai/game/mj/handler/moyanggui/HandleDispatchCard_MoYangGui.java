...
public class HandleDispatchCard_MoYangGui extends AbstractMJHandler<MjTableMoYangGui> {
	...
	public HandleDispatchCard_MoYangGui() {
	...
	public void exe(MjTableMoYangGui table) {
		...
		if (table.GRR._left_card_count > 0) {
			...
			int cbActionMask = table._logic.analyse_gang_card_myg(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
					table.GRR._weave_count[_seat_index], m_gangCardResult, true, -1, table.GRR._cards_abandoned_gang[_seat_index]);
	...
	public boolean handler_operate_card(MjTableMoYangGui table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_NULL) {
			if (table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_GANG)) {
				for (int i = 0; i < table._playerStatus[seat_index]._weave_count; i++) {
					if (table._playerStatus[seat_index]._action_weaves[i].weave_kind == GameConstants.WIK_GANG) {
						int card = table._playerStatus[seat_index]._action_weaves[i].center_card;
						table._playerStatus[seat_index].add_cards_abandoned_gang(table._logic.switch_to_card_index(card));
						table.GRR._cards_abandoned_gang[seat_index][table._logic.switch_to_card_index(card)]++;
					}
				}
			}
			/*
			 * boolean bhasGang = false; for (int i = 0; i <
			 * table._playerStatus[seat_index]._action_count; i++) { if
			 * (table._playerStatus[seat_index]._action[i] ==
			 * GameConstants.WIK_GANG){ bhasGang = true; } } if(bhasGang){
			 * table._playerStatus[seat_index].add_cards_abandoned_gang(table.
			 * _logic.switch_to_card_index(_out_card_data));
			 * table.GRR._cards_abandoned_gang[seat_index][table._logic.
			 * switch_to_card_index(_out_card_data)]++; }
			 */
		}
		...
		case GameConstants.WIK_ZI_MO: {
			...
			// table.FanPaiSuanFen(operate_card);
	...
	@Override
	public boolean handler_player_out_card(MjTableMoYangGui table, int seat_index, int card) {
	...
	public boolean handler_player_be_in_room(MjTableMoYangGui table, int seat_index) {
		...
		// 癞子标记
		/*
		 * for (int j = 0; j < hand_card_count; j++) { if
		 * (table._logic.is_magic_card(hand_cards[j])) { hand_cards[j] +=
		 * GameConstants.CARD_ESPECIAL_TYPE_GUI; } } for (int i = 0; i <
		 * hand_card_count; i++) { tableResponse.addCardsData(hand_cards[i]); }
		 * roomResponse.setTable(tableResponse);
		 * table.send_response_to_player(seat_index, roomResponse);
		 */
		...
		if (table._logic.is_magic_card(_send_card_data)) {
			isendcard += GameConstants.CARD_ESPECIAL_TYPE_GUI;
		}
