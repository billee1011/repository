...
public class HandlerDispatchCard_HuaZhou extends MJHandlerDispatchCard<Table_HuaZhou> {
	boolean ting_send_card = false;
	protected GangCardResult m_gangCardResult;
	int pre_dispatch_player_seat;

	public HandlerDispatchCard_HuaZhou() {
		m_gangCardResult = new GangCardResult();
		pre_dispatch_player_seat = 0;
	}
	...
	public void exe(Table_HuaZhou table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			table._cur_banker = pre_dispatch_player_seat;
		...
		pre_dispatch_player_seat = _seat_index;
		...
		int card_type = Constants_HuaZhou.HU_CARD_TYPE_ZI_MO;
		...
		for (int i = 0; i < card_type_count; i++) {
			// // 晒张过滤
			// if (i == table.get_shai_zhang_index(_seat_index)) {
			// continue;
			// }
		...
		if (ting_count > 0) {
			...
			// 刷新晒张
			table.refresh_shai_zhang_card(_seat_index, hand_card_count, cards);
			...
			for (int i = 0; i < hand_card_count; i++) {
				for (int j = 0; j < ting_count; j++) {
					if (cards[i] == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
						if (cards[i] < GameConstants.CARD_ESPECIAL_TYPE_LAI_GEN) {
		...
		// if (table.is_shai_zhang_card(_seat_index, _send_card_data)) {
		...
		if (table.GRR._left_card_count > 0) {
			...
			//组合牌过滤晒张标志
//			WeaveItem tmpWeaveItem[]=new WeaveItem[GameConstants.MAX_WEAVE];
//			for (int j = 0; j < GameConstants.MAX_WEAVE; j++) {
//				tmpWeaveItem[j] = new WeaveItem();
//			}
//			table.restore_shai_zhang_weave(_seat_index,table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index],tmpWeaveItem);
			
			int cbActionMask = table._logic.analyse_gang_hong_zhong_all(table.GRR._cards_index[_seat_index],
					table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], m_gangCardResult, true,
					table.GRR._cards_abandoned_gang[_seat_index]);
	...
	@Override
	public boolean handler_player_out_card(Table_HuaZhou table, int seat_index, int card) {
	...
	@Override
	public boolean handler_operate_card(Table_HuaZhou table, int seat_index, int operate_code, int operate_card) {
	...
	public boolean handler_player_be_in_room(Table_HuaZhou table, int seat_index) {
		...
		//刷新晒张
		table.refresh_shai_zhang_card(seat_index, hand_card_count, hand_cards);
			...
			for (int j = 0; j < hand_card_count; j++) {
				for (int k = 0; k < out_ting_count; k++) {
					if (hand_cards[j] == table._playerStatus[seat_index]._hu_out_card_ting[k]) {
						if (hand_cards[j] < GameConstants.CARD_ESPECIAL_TYPE_LAI_GEN) {
