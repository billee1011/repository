...
public class HandlerChiPeng_ZhuZhouWang extends MJHandlerChiPeng<Table_ZhuZhouWang> {
		...
		if (SysParamServerUtil.is_new_algorithm(3000, 3000, 1)) {
			...
			if (ting_count > 0) {
				...
				for (int i = 0; i < tmp_hand_card_count; i++) {
					...
					if (table.get_real_card(tmp_cards[i]) == table.joker_card_1
							|| table.get_real_card(tmp_cards[i]) == table.joker_card_2) {
						tmp_cards[i] += GameConstants.CARD_ESPECIAL_TYPE_WANG_BA;
					} else if (table.get_real_card(tmp_cards[i]) == table.ding_wang_card) {
