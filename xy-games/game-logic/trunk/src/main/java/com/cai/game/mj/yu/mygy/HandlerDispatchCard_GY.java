...
public class HandlerDispatchCard_GY extends MJHandlerDispatchCard<Table_GY> {
	...
	public void exe(Table_GY table) {
		...
		if (_type == GameConstants_MYGY.DispatchCard_Type_Tian_Hu && !table.has_rule(GameConstants_MYGY.GAMR_RULE_2_REN_JU)
				&& !table.has_rule(GameConstants_MYGY.GAMR_RULE_3_REN_JU)
				&& (!table.has_rule(GameConstants_MYGY.GAME_RULE_ER_FANG_PAI) && table.getTablePlayerNumber() != 4)) {

			for (int player = 0; player < table.getTablePlayerNumber(); player++) {
				table._playerStatus[player].add_action(GameConstants_MYGY.WIK_DING_WANG);
				table._playerStatus[player].add_action(GameConstants_MYGY.WIK_DING_TONG);
				table._playerStatus[player].add_action(GameConstants_MYGY.WIK_DING_TIAO);
		...
		if (table.player_duan[_seat_index] != -1 && !table._logic.is_magic_card(_send_card_data)) {
		...
		if (hand_have_duan_card) {
			...
			for (int i = 0; i < table.GRR._cards_index[_seat_index][table._logic.get_magic_card_index(0)]
			...
			for (int i = 0; i < GameConstants_MYGY.MAX_INDEX; i++) {
				if (table.GRR._cards_index[_seat_index][i] == 0 || table._logic.is_magic_index(i))
					continue;
				...
				for (int j = 0; j < table.GRR._cards_index[_seat_index][i]; j++) {
					if (table._logic.get_card_color(table._logic.switch_to_card_data(i)) != table.player_duan[_seat_index]) {
						cards[hand_card_count++] = table._logic.switch_to_card_data(i) + GameConstants_MYGY.CARD_ESPECIAL_TYPE_LIANG_ZHANG;
					}
				}
			}
			for (int i = 0; i < GameConstants_MYGY.MAX_INDEX; i++) {
			...
			if (table.player_duan[_seat_index] == table._logic.get_card_color(_send_card_data)) {
				table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants_MYGY.INVALID_SEAT);
			} else {
				table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data + GameConstants_MYGY.CARD_ESPECIAL_TYPE_LIANG_ZHANG },
	...
	public boolean handler_operate_card(Table_GY table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants_MYGY.WIK_DING_WANG:
		case GameConstants_MYGY.WIK_DING_TONG:
		case GameConstants_MYGY.WIK_DING_TIAO: {
