 ...
 *
 */
public class PHZHandlerDispatchCard_MS extends PHZHandlerDispatchCard<SCPHZTable> {
	...
	public void exe(SCPHZTable table) {
		...
		while (loop < table.getTablePlayerNumber()) {
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				if (table.has_rule(GameConstants.GAME_RULE_QIANG_ZHI_HU_PAI) || table.isTrutess(i)) {
					...
					GameSchedule.put(new HuPaiRunnable(table.getRoom_id(), i, GameConstants.WIK_ZI_MO, _send_card_data), hucard_time,
		...
		if ((bAroseAction == false) && (ti_sao == GameConstants.WIK_NULL)) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if (table._playerStatus[i].has_action() && table.isTrutess(i)) {
					GameSchedule.put(new OperateCardRunnable(table.getRoom_id(), i, _send_card_data), GameConstants.DELAY_AUTO_OUT_CARD_TRUTESS,
							TimeUnit.MILLISECONDS);
				}
			}
		}
		if (curPlayerStatus.has_action()) { // 有动作
			...
			// GameSchedule.put(new OperateCardRunnable(table.getRoom_id(),
			// _seat_index, _send_card_data), 1, TimeUnit.MILLISECONDS);
			// return;
			// }
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				if (bAroseAction == false) {
					...
					if (table.has_rule(GameConstants.GAME_RULE_ADD_SPEED) && sysParamModel1104 != null && sysParamModel1104.getVal4() > 0
							&& sysParamModel1104.getVal4() < 10000) {
						discard_time = sysParamModel1104.getVal4();
					}
					if (table.has_rule(GameConstants.GAME_RULE_ADD_SPEED) == false && sysParamModel1110 != null && sysParamModel1110.getVal4() > 0
					...
					if (table.has_rule(GameConstants.GAME_RULE_ADD_SPEED) && sysParamModel1104 != null && sysParamModel1104.getVal5() > 0
							&& sysParamModel1104.getVal5() < 10000) {
						dispatch_time = sysParamModel1104.getVal5();
					}
					if (table.has_rule(GameConstants.GAME_RULE_ADD_SPEED) == false && sysParamModel1110 != null && sysParamModel1110.getVal5() > 0
							&& sysParamModel1110.getVal5() < 10000) {
	...
	public boolean handler_operate_card(SCPHZTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		case GameConstants.WIK_PAO: // 杠牌操作
		{
			...
			if (action != GameConstants.WIK_NULL) {
				table.exe_gang(target_player, _seat_index, _send_card_data, action, pao_type[0], true, true, false, 300);
			}
