...
public class MJHandlerGang_HuNan_ChenZhou extends MJHandlerGang<MJTable_HuNan_ChenZhou> {
	@Override
	public void exe(MJTable_HuNan_ChenZhou table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { GameConstants.WIK_AN_GANG }, 1,
					GameConstants.INVALID_SEAT);
		} else {
			table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { GameConstants.WIK_MING_GANG }, 1,
					GameConstants.INVALID_SEAT);
		}
		...
		if (GameDescUtil.has_rule(table._game_rule_index, GameConstants.GAME_RULE_HUNAN_QIANGGANGHU)) {
	...
	@Override
	protected boolean exe_gang(MJTable_HuNan_ChenZhou table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				// table.GRR._game_score[i] -= 2 * GameConstants.CELL_SCORE;
				table._player_result.game_score[i] -= 2 * GameConstants.CELL_SCORE;
				// table.GRR._game_score[_seat_index] += 2 *
			...
			// table.GRR._game_score[_provide_player] -= gang_fen *
			// GameConstants.CELL_SCORE;
			table._player_result.game_score[_provide_player] -= gang_fen * GameConstants.CELL_SCORE;
			// table.GRR._game_score[_seat_index] += gang_fen *
			// GameConstants.CELL_SCORE;
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				// table.GRR._game_score[i] -= GameConstants.CELL_SCORE;
				// table.GRR._game_score[_seat_index] +=
				// GameConstants.CELL_SCORE;
			}
	...
	@Override
	public boolean handler_operate_card(MJTable_HuNan_ChenZhou table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_CHI_HU && table.has_rule(GameConstants.GAME_RULE_DUO_PAO)) {
		...
		for (int i = 0; i < table.getTablePlayerNumber() && table.has_rule(GameConstants.GAME_RULE_DUO_PAO); i++) {
		...
		if (jie_pao_count > 0) {
			...
			if (jie_pao_count > 1) {
				...
				niao_seat = _seat_index;
				;
			} else {
				table._cur_banker = target_player;
			}

			table.set_niao_card(niao_seat);
			...
			for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
				...
				/*if (!table.has_rule(GameConstants.GAME_RULE_DUO_PAO))
					table.process_chi_hu_player_operate(i, operate_card, false);*/
				table.process_chi_hu_player_score(i, _seat_index, operate_card, 1);
