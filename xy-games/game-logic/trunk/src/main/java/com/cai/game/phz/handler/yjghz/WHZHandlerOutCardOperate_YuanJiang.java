...
public class WHZHandlerOutCardOperate_YuanJiang extends PHZHandlerOutCardOperate<YuanJiangGHZTable> {
	@Override
	public void exe(YuanJiangGHZTable table) {
		...
		table.cannot_chi_card(_out_card_player, _out_card_data, table.GRR._cards_index[_out_card_player]);
		// 用户切换
		...
		bAroseAction = table.estimate_player_out_card_respond_yywhz(_out_card_player, _out_card_data, table.GRR._weave_items[_out_card_player],
				table.GRR._weave_count[_out_card_player], false);
		...
		// int cbActionMask = table.check_piao(table.GRR._cards_index[i],
		// table.GRR._weave_items[i], table.GRR._weave_count[i], _out_card_data,
		// gangCardResult, i, _out_card_player, false);
		// if (cbActionMask != GameConstants.WIK_NULL) {// 有飘
		// for (int j = 0; j < gangCardResult.cbCardCount; j++) {
		// table._playerStatus[i].add_action_card(gangCardResult.isPublic[i],
		// gangCardResult.cbCardData[j], GameConstants_YJGHZ.WIK_PIAO, i);
	...
	public boolean handler_operate_card(YuanJiangGHZTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		for (int p = 0; p < table.getTablePlayerNumber(); p++) {
			...
			if (table._playerStatus[i].has_action()) {
				if (table._playerStatus[i].is_respone()) {
					// 获取已经执行的动作的优先级
					cbUserActionRank = table.get_action_rank(table._playerStatus[i].get_perform(), i, _out_card_player) + table.getTablePlayerNumber()
							- p;
				...
				if (table._playerStatus[target_player].is_respone()) {
					// 获取已经执行的动作的优先级
					cbTargetActionRank = table.get_action_rank(table._playerStatus[target_player].get_perform(), target_player, _out_card_player)
							+ target_p;
					cbActionRank[i] = cbUserActionRank;
				} else {
					// 获取最大的动作的优先级
			...
			if (table._playerStatus[i].has_action()) {
				for (int j = 0; j < table._playerStatus[i]._action_count; j++) {
					...
					case GameConstants.WIK_EQS:
						...
						break;
					case GameConstants.WIK_PENG:
						if (table.get_action_rank(target_action, target_player, _out_card_player) < table._logic
								.get_action_rank_yiyang(GameConstants.WIK_PENG, target_player, _out_card_player)) {
		...
		case GameConstants_YJGHZ.WIK_PIAO: {
			table.exe_piao(target_player, _out_card_player, target_action, operate_card, GameConstants.CHI_PENG_TYPE_DISPATCH, 0);
			return true;
		}
		default:
			return false;
