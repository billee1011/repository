...
public class SCPHZGameLogic {

	private static Logger logger = Logger.getLogger(SCPHZGameLogic.class);
	...
	public int is_weave_hu(WeaveItem weave_item[], int weave_count, int type)
	{
		int count = 0;
		for(int i = 0; i< weave_count ;i++)
		{
			if(weave_item[i].weave_kind == type)
				count ++;
		}
		return count ;
	}
	public int get_weave_hu_xi(WeaveItem weave_item) {
		...
		case GameConstants.WIK_PENG:
		{
			if(this.color_hei(weave_item.center_card)==false)
				return (get_card_color(weave_item.center_card) != 0) ? 6 : 3;
			else
	...
	public int get_kai_count(WeaveItem weaveItems[], int weaveCount){
		int count = 0;
		for(int i = 0; i < weaveCount; i++ ){
			switch (weaveItems[i].weave_kind) {
			case GameConstants.WIK_TI_LONG:
	...
	public boolean is_man_chuan_hua(WeaveItem weave_item[],int weave_count)
	{
		for(int i = 0; i < weave_count; i++ ){
			switch (weave_item[i].weave_kind) {
			case GameConstants.WIK_TI_LONG:
			case GameConstants.WIK_AN_LONG:
			case GameConstants.WIK_PAO:
				if (color_hei(weave_item[i].center_card) == true)
					return false;
			case GameConstants.WIK_SAO:
			case GameConstants.WIK_PENG:
			case GameConstants.WIK_XXD:
			...
			{
				boolean flag = false;
				if (this.color_hei(weave_item[i].center_card) == false&&flag == false)
					flag = true;
				if (color_hei(weave_item[i].center_card + 1) == false&&flag == false)
					flag = true;
				if (color_hei(weave_item[i].center_card + 2) == false&&flag == false)
					flag = true;
				if(flag == false)
					return false;
				break;
	...
	// 分析扑克
	public boolean analyse_card_yj(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index, int cur_card,
			List<AnalyseItem> analyseItemArray, boolean has_feng, int hu_xi[]) {
		// 计算数目
		int cbCardCount = get_card_count_by_index(cards_index);
		hu_xi[0] = 0;
		// 跑胡判断
		WeaveItem pao_WeaveItem[] = new WeaveItem[5];
		for(int i = 0; i< 5; i++){
			pao_WeaveItem[i] = new WeaveItem();
		}
		int pao_count = 0;
		...
		for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
			if (cards_index[i] == 4) {
				cards_index[i] = 0;
				pao_WeaveItem[pao_count].center_card = switch_to_card_data(i);
				if ((switch_to_card_index(cur_card) != i))
				{
					pao_WeaveItem[pao_count].weave_kind = GameConstants.WIK_AN_LONG;
					an_long_count++;
				}
				else
					pao_WeaveItem[pao_count].weave_kind = GameConstants.WIK_PAO;
		...
		{
			for(int i = 0; i<cbWeaveCount;i++){
				if(weaveItem[i].weave_kind == GameConstants.WIK_AN_LONG)
					an_long_count++;
			}
			if(an_long_count > 0)
				return false;
		...
		if ((cbLessKindItem == 0) && (bNeedCardEye == true)) {
			for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
				if (cards_index[i] == 2) {
					...
					if (pao_count > 0) {
						for(int j = 0; j< pao_count;j++){
							analyseItem.cbWeaveKind[count] = pao_WeaveItem[j].weave_kind;
							analyseItem.cbCenterCard[count] = pao_WeaveItem[j].center_card;
							analyseItem.hu_xi[count] = get_weave_hu_xi(pao_WeaveItem[j]);
							hu_xi[0] += analyseItem.hu_xi[count];
							get_weave_card(pao_WeaveItem[j].weave_kind, pao_WeaveItem[j].center_card, analyseItem.cbCardData[count]);
							count++;
						}
		...
		for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
			...
			if ((i < (GameConstants.MAX_HH_INDEX - 2)) && (cards_index[i] > 0) && ((i % 10) <= 7)) {
				for (int j = 1; j <= cards_index[i]; j++) {
					...
					if ((cards_index[i] < 3) 
							&& (cards_index[i + 1] >= j && (cards_index[i + 1] < 3 ))
							&& (cards_index[i + 2] >= j && (cards_index[i + 2] < 3 ))) {
						kindItem[cbKindItemCount].cbCardIndex[0] = i;
						kindItem[cbKindItemCount].cbCardIndex[1] = i + 1;
						kindItem[cbKindItemCount].cbCardIndex[2] = i + 2;
						kindItem[cbKindItemCount].cbValidIndex[0] = i;
	...
	// 分析扑克
	public boolean analyse_card_sy(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index, int cur_card,
			List<AnalyseItem> analyseItemArray, boolean has_feng, int hu_xi[],boolean is_tian_hu) {
		// 计算数目
		int cbCardCount = get_card_count_by_index(cards_index);
		hu_xi[0] = 0;
		// 跑胡判断
		...
		for (int i = 0; i < GameConstants.MAX_HH_INDEX; i++) {
			if (cards_index[i] == 4) {
				...
				else if(switch_to_card_index(cur_card) != i)
				{
					pao_WeaveItem[pao_count].weave_kind = GameConstants.WIK_AN_LONG;
					an_long_count++;
				}
	...
	public int get_chi_hu_action_rank_yj(ChiHuRight chiHuRight) {
		int wFanShu = 0;
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_QUANFENG)).is_empty()) {
			wFanShu += 1;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_ESS_HU)).is_empty()) {
			wFanShu += 1;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_XHHU)).is_empty()) {
			wFanShu += 1;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_HDHU)).is_empty()) {
			wFanShu += 1;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_ZIAN)).is_empty()) {
			wFanShu += 2;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_MCH)).is_empty()) {
			wFanShu += 2;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_XPHU)).is_empty()) {
			wFanShu += 2;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_EQSHU)).is_empty()) {
			wFanShu += 2;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_THU)).is_empty()) {
			wFanShu += 2;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_DIHU)).is_empty()) {
			wFanShu += 2;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_ZHHU)).is_empty()) {
			wFanShu += 2;
		}
		
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_XIAOHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_DHEIHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_XHEIHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_XIAOHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_HMCH)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_DPHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_DHEIHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_HHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_DHHHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_SSHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_SHEHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_SKHU)).is_empty()) {
			wFanShu += 3;
		}
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_YJ_WANHU)).is_empty()) {
			wFanShu += 3;
		}
	...
	public int get_chi_hu_action_rank_sy(ChiHuRight chiHuRight) {
		int wFanShu = 0;
		...
		if (!(chiHuRight.opr_and_long(GameConstants.CHR_SY_0_HU)).is_empty()) {
			wFanShu += 2;
		}	
	...
	public static void main(String[] args) {
		// 插入扑克
		SCPHZGameLogic logic = new SCPHZGameLogic();
