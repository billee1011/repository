...
public class MJHandlerChiPeng_SG extends MJHandlerChiPeng<MJTable> {
	...
	public void exe(MJTable table) {
		...
		// null,GameConstants.OUT_CARD_TYPE_MID,GameConstants.INVALID_SEAT);
		...
		if (SysParamServerUtil.is_new_algorithm(3000, 3000, 1)) {
			...
			for (int i = 0; i < card_type_count; i++) {
				...
				if (count > 0) {
					...
					table._playerStatus[_seat_index]._hu_out_card_ting_count[ting_count] = MahjongTingPaiUtil.getInstance().get_sg_ting_card(
							table._playerStatus[_seat_index]._hu_out_cards[ting_count], table.GRR._cards_index[_seat_index],
							table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], table);
				...
				for (int i = 0; i < tmp_hand_card_count; i++) {
					...
					if (tmp_cards[i] < GameConstants.CARD_ESPECIAL_TYPE_TING) {
						if (table._logic.is_magic_card(tmp_cards[i])) {
							tmp_cards[i] += GameConstants.CARD_ESPECIAL_TYPE_GUI;
						}
					}
				}
