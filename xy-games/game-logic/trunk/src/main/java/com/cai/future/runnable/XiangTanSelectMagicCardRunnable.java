...
public class XiangTanSelectMagicCardRunnable extends BaseFuture {

	private static Logger logger = LoggerFactory.getLogger(XiangTanSelectMagicCardRunnable.class);

	private int _room_id;
	private int _seat_index;

	public XiangTanSelectMagicCardRunnable(int room_id, int seat_index) {
		super(room_id);
		_room_id = room_id;
	...
	public void execute() {
		try {
			...
			try {
				roomLock.lock();
				table.runnable_remove_da_dian_card(_seat_index);
			} finally {
				roomLock.unlock();
			}
		} catch (Exception e) {
