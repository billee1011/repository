...
public class HandlerGang_DaLianQiongHu extends MJHandlerGang<Table_DaLianQiongHu> {
	...
	public HandlerGang_DaLianQiongHu() {
	}
	...
	@Override
	public boolean exe_gang(Table_DaLianQiongHu table) {
		table.gang_count++;
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			table.is_kai_kou[_seat_index] = true;
			// 别人打的牌
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
		...
		if(!table.is_bao_ting[_seat_index]){
			table._playerStatus[_seat_index]._hu_card_count = table.get_ting_card(table._playerStatus[_seat_index]._hu_cards, table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], _seat_index);
			if (table._playerStatus[_seat_index]._hu_card_count > 0) {
				if(table.has_hand_card19(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index])){
					table.operate_chi_hu_cards(_seat_index, table._playerStatus[_seat_index]._hu_card_count, table._playerStatus[_seat_index]._hu_cards);
					table._playerStatus[_seat_index].add_action(Constants_MJ_DA_LIAN_QH.WIK_GANG_BAO_TING);
					table._playerStatus[_seat_index]._response = false;
		...
		// 从后面发一张牌给玩家
		table.exe_dispatch_card(_seat_index, Constants_MJ_DA_LIAN_QH.HU_CARD_TYPE_GKSH, 0);
	...
	private void handler_operate_tong_pao(Table_DaLianQiongHu table, int seat_index,
        ...
        if (jie_pao_count > 0) {
            ...
            table.GRR._chi_hu_rights[_seat_index].opr_or(Constants_MJ_DA_LIAN_QH.CHR_FANG_PAO);
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if ((table.GRR._chi_hu_rights[i].is_valid() == false)) {
                    continue;
                }
	...
	private void handler_operate_bu_tong_pao(Table_DaLianQiongHu table, int seat_index,
			int operate_code, int operate_card) {
		int target_player = seat_index;
        ...
        boolean has_bao_ting = table._playerStatus[_seat_index].has_action_by_code(Constants_MJ_DA_LIAN_QH.WIK_GANG_BAO_TING) ||
        		!table.GRR._chi_hu_rights[_seat_index].opr_and(Constants_MJ_DA_LIAN_QH.CHR_CHONG_BAO).is_empty();
        // 用户状态
        for (int i = 0; i < table.getTablePlayerNumber(); i++) {
            table._playerStatus[i].clean_action();
            table._playerStatus[i].clean_status();
        ...
        case Constants_MJ_DA_LIAN_QH.WIK_GANG_BAO_TING:{
			...
			if(table.has_rule(Constants_MJ_DA_LIAN_QH.GAME_RULE_AFTER_OUT_BAO) || !table.is_chong_bao(_seat_index)){
				if(is_change_bao){
					GameSchedule.put(() -> {
						table.exe_dispatch_card(_seat_index, Constants_MJ_DA_LIAN_QH.HU_CARD_TYPE_GKSH_BT, 0);
					}, 2, TimeUnit.SECONDS);
				}else{
					table.exe_dispatch_card(_seat_index, Constants_MJ_DA_LIAN_QH.HU_CARD_TYPE_GKSH_BT, 0);
				}
        ...
        case GameConstants.WIK_CHI_HU: {
            ...
            table.GRR._chi_hu_rights[_seat_index].opr_or(Constants_MJ_DA_LIAN_QH.CHR_BEI_QIANG_GANG);
