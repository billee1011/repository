...
public class HHTable_CS extends HHTable {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_dispath_card = new PHZHandlerDispatchCard_CS();
		_handler_out_card_operate = new PHZHandlerOutCardOperate_CS();
		_handler_gang = new PHZHandlerGang_CS();
		_handler_chi_peng = new PHZHandlerChiPeng_CS();
		_handler_chuli_firstcards = new PHZHandlerChuLiFirstCard_CS();
		_handler_dispath_firstcards = new PHZHandlerDispatchFirstCard_CS();
	}
	...
	// if (has_rule(GameConstants_CS.GAME_RULE_PLAYER_FAN5)) {
	// buffer.append(" 5番上限");
	// }
	// if (has_rule(GameConstants_CS.GAME_RULE_PLAYER_FAN10)) {
	// buffer.append(" 10番上限");
	// }
	// if (has_rule(GameConstants_CS.GAME_RULE_PLAYER_QING_ZHI_HU_PAI)) {
	// buffer.append(" 自摸翻倍");
	// }
	// if (has_rule(GameConstants_CS.GAME_RULE_PLAYER_FAN_NONE)) {
	// buffer.append(" 番数无上限");
	// }
	...
	public void test_cards() {
		 ...
		 * 0x18, 0x08 }; for (int i = 0; i < getTablePlayerNumber(); i++) { for
		 * (int j = 0; j < GameConstants.MAX_HH_INDEX; j++) {
		 * GRR._cards_index[i][j] = 0; } } for (int i = 0; i <
		 * getTablePlayerNumber(); i++) { for (int j = 0; j < cards.length; j++)
