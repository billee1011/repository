...
public class SCHCPHandlerGang_DSSMS extends SCHCPDSSHandlerGang<SCHCPDSSTable_MS> {
	...
	public boolean handler_operate_card(SCHCPDSSTable_MS table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_schcp(target_player, this._provide_player, operate_card, false);
	 ...
	 * 
	 ***/
	protected boolean exe_gang(SCHCPDSSTable_MS table) {
		int cbCardIndex = table._logic.switch_to_card_index(_center_card);
		int cbWeaveIndex = -1;
		if (GameConstants.CHI_PENG_TYPE_ADD == _type || GameConstants.CHI_PENG_TYPE_DISPATCH == _type
				|| GameConstants.CHI_PENG_TYPE_OUT_CARD == _type) {
			// 暗龙
			// 设置变量
		...
		} else if (GameConstants.CHR_DING_FU_TOU_TYPE_DISPATCH == _type || GameConstants.CHR_DING_FU_TOU_TYPE_ADD == _type) {
			// 提龙
			// 设置变量
		...
		if (GameConstants.CHI_PENG_TYPE_OUT_CARD == _type) {
			table.operate_out_card(_provide_player, 0, null, GameConstants.OUT_CARD_TYPE_MID, GameConstants.INVALID_SEAT);
		}
		if (GameConstants.CHI_PENG_TYPE_ADD == _type || GameConstants.CHI_PENG_TYPE_DISPATCH == _type
				|| GameConstants.CHR_DING_FU_TOU_TYPE_DISPATCH == _type || GameConstants.CHR_DING_FU_TOU_TYPE_ADD == _type) {
		...
		if (GameConstants.CHR_DING_FU_TOU_TYPE_DISPATCH == _type || GameConstants.CHR_DING_FU_TOU_TYPE_ADD == _type) {
			table.GRR._weave_items[_seat_index][cbWeaveIndex].hu_xi = 0;
		} else
			table.GRR._weave_items[_seat_index][cbWeaveIndex].hu_xi = table._logic.get_analyse_tuo_shu(_action, _center_card);
		// 设置用户
		table._current_player = _seat_index;
		if (_type == GameConstants.CHI_PENG_TYPE_DISPATCH || _type == GameConstants.CHI_PENG_TYPE_OUT_CARD) {
			if (table.GRR._cards_index[_seat_index][cbCardIndex] > 2)
		...
		// if (GameConstants.CHR_DING_FU_TOU_TYPE_DISPATCH != _type ) {
		// curPlayerStatus.add_action(GameConstants.DSS_WIK_TOU);
		// curPlayerStatus.add_tou(_center_card, GameConstants.DSS_WIK_TOU,
		// _seat_index);
		...
		if (table._is_ting_pai[_seat_index] == 1) {
		...
		// int gameId = table.getGame_id() == 0 ? 5 : table.getGame_id();
		// int dispatch_time = 3000;
		// SysParamModel sysParamModel1105 =
		// SysParamDict.getInstance().getSysParamModelDictionaryByGameId(gameId)
		// .get(1105);
		// if (sysParamModel1105 != null && sysParamModel1105.getVal2() > 0 &&
		// sysParamModel1105.getVal2() < 10000) {
		// dispatch_time = sysParamModel1105.getVal2();
		// }
		// table.exe_dispatch_card(_seat_index, GameConstants.WIK_NULL,
		// dispatch_time);
		// }
