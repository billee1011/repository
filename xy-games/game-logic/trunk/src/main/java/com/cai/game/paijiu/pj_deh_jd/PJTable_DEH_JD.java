 ...
 * 
 */
package com.cai.game.paijiu.pj_deh_jd;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PJTable_DEH_JD extends PJTable {
	...
	protected boolean on_handler_game_start() {
		...
		if (has_rule(GameConstants.GAME_RULE_DEH_PI_ZI_TWO)) {
			game_cell = 2;
		} else if (has_rule(GameConstants.GAME_RULE_DEH_PI_ZI_THREE)) {
			game_cell = 3;
		} else if (has_rule(GameConstants.GAME_RULE_DEH_PI_ZI_FOUR)) {
			game_cell = 4;
		} else if (has_rule(GameConstants.GAME_RULE_DEH_PI_ZI_FIVE)) {
			game_cell = 5;
		} else {
			game_cell = 1;
		}
	...
	private void test_cards() {
		...
		// 0x1d, 0x3d, 0x31, 0x32};
		int cards[] = new int[] { 7, 21, 6, 59, 26, 12, 44, 58, 42, 35, 38, 25, 23, 34, 53, 36, 4, 40, 39, 54, 57, 79,
				52, 22, 2, 27, 56, 8, 24, 55, 20, 10, 0x1a, 0x38, 0x28, 0x19, 0x1a, 0x38, 0x28, 0x19, 0x34, 0x14, 0x08,
				0x15, 0x26, 0x02, 0x34, 0x04, 0x19, 0x15, 0x23, 0x37, 0x3b, 0x1b, 0x0a, 0x22 };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean handler_game_finish_dehjd(int seat_index, int reason) {
		...
		if (win_sort[0] != GameConstants.INVALID_SEAT) {
		...
		if (GRR != null) {
			...
			game_end_deh.setStartTime(GRR._start_time);
		}
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
		...
		if (reason == GameConstants.Game_End_NORMAL) {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				if (win_sort[i] != GameConstants.INVALID_SEAT
						&& (end_score[win_sort[i]] == 0 || end_score[win_sort[i]] < 0)
	...
	// 结算分数
	public void cal_score_deh_jd(int end_score[], int win_sort[], int mang_pi[], int end_score_except_mangpi[],
			int xian_qian_score[]) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			for (int j = i + 1; j < getTablePlayerNumber(); j++) {
				...
				if (this._player_ready[weidao_sort[i]] == 1 && this._player_ready[weidao_sort[j]] == 1
						&& !this._is_give_up[weidao_sort[i]] && !this._is_give_up[weidao_sort[j]]) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			for (int j = i + 1; j < this.getTablePlayerNumber(); j++) {
				if (end_score[win_sort[i]] < end_score[win_sort[j]] || this._player_ready[win_sort[i]] == 0
						|| this._is_give_up[win_sort[i]]) {
					int temp = win_sort[i];
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			for (int j = i + 1; j < this.getTablePlayerNumber(); j++) {
				...
				if (end_score[win_sort[j]] < 0 || (end_score[win_sort[j]] == 0 && end_score[win_sort[i]] > 0)) {
					continue;
				}
				int card_one_one[] = new int[this.get_hand_card_count_max()];
				int card_one_two[] = new int[this.get_hand_card_count_max()];
				...
				if (_is_give_up[win_sort[i]] && !_is_give_up[win_sort[j]]) {
					...
					if (ncompare > 0) {
						...
						if (this.has_rule(GameConstants.GAME_RULE_DEH_DI_ZHUANG_FIRST)) {
							if ((_cur_banker + getTablePlayerNumber())
									- win_sort[i] > (_cur_banker + getTablePlayerNumber()) - win_sort[j]) {
								win_num++;
							} else if ((_cur_banker + getTablePlayerNumber())
									- win_sort[i] < (_cur_banker + getTablePlayerNumber()) - win_sort[j]) {
								win_num--;
							}
						}
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status)
			...
			if (!this._is_bobo[seat_index]) {
				...
				tableResponse_deh
						.setMaxBoboScore((int) (_zi_chan_score[seat_index] + _player_result.game_score[seat_index]));
				...
				if ((int) this._player_result.game_score[seat_index] == 0) {
					if (this._player_result.game_score[seat_index] + _zi_chan_score[seat_index] >= _min_bobo_score) {
						...
						tableResponse_deh.setMinBoboScore(
								(int) this._player_result.game_score[seat_index] + _zi_chan_score[seat_index]);
					}
			...
			if (_deh_game_staus == GAME_CHE_PAI_BEGIN && !_is_open_card[seat_index]
					&& this._player_ready[seat_index] == 1) {
	...
	public boolean deal_gen_jetton(int seat_index) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			if (this._player_ready[next_player] == 0 || _is_give_up[next_player]
					|| this._player_result.game_score[next_player] == 0) {
				next_player = (next_player + 1) % this.getTablePlayerNumber();
			} else {
				break;
			}
		...
		if (count == 0) {
			...
			if (_end_index == 4) {
				...
				refresh_game_status(true);
				set_timer(ID_TIMER_CHE_PAI_WAIT, CHE_PAI_WAIT_TIME);
					...
					if (_end_index >= 2) {
						...
						if (this._player_result.game_score[_cur_round_banker] == 0) {
							for (int i = 0; i < getTablePlayerNumber(); i++) {
								...
								if (this._is_give_up[index] || this._player_result.game_score[index] == 0
										|| this._player_ready[index] == 0) {
									continue;
								}
								_cur_round_banker = index;
								break;
	...
	public boolean deal_give_up(int seat_index) {
		...
		if (has_rule(GameConstants.GAME_RULE_DEH_HAN_QIAN_BI_GUN)) {
			...
			this._jetton_current_add_min = max_jetton_score * 2 - _jetton_score[_current_player]
					+ _cur_round_jetton_score[_current_player];
		} else if (has_rule(GameConstants.GAME_RULE_DEH_MIN_HAN_5)) {
			if (_jetton_current_add_min < _jetton_current + 5) {
				_jetton_current_add_min = _jetton_current + 5;
			}

		} else if (has_rule(GameConstants.GAME_RULE_DEH_MIN_HAN_10)) {
			if (_jetton_current_add_min < _jetton_current + 10) {
				_jetton_current_add_min = _jetton_current + 10;
			}
		} else {
		...
		if (count <= 1) {
			...
			Refresh_User_Operate(this._current_player);
			GameSchedule.put(new GameFinishRunnable(getRoom_id(), win_seat_index, GameConstants.Game_End_NORMAL), 3,
					TimeUnit.SECONDS);
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (cur_jetton_max == this._cur_round_jetton_score[i] && this._is_jetton[i]) {
				continue;
			}
			if (this._is_pass[i]) {
				continue;
		...
		if (count == 0) {
			...
			if (count == 0) {
				...
				this._current_player = GameConstants.INVALID_SEAT;
				GameSchedule.put(
						new GameFinishRunnable(getRoom_id(), GameConstants.INVALID_SEAT, GameConstants.Game_End_DRAW),
						3, TimeUnit.SECONDS);
				Operate_Result(seat_index, 1);
				return true;
			} else {
				...
				if (count == 0) {
					...
					if (count == 1 && has_rule(GameConstants.GAME_RULE_DEH_PEI_SI_PI)) {
						...
						Refresh_User_Operate(this._current_player);
						GameSchedule.put(
								new GameFinishRunnable(getRoom_id(), win_seat_index, GameConstants.Game_End_NORMAL), 3,
								TimeUnit.SECONDS);
						return true;
					} else {
						if (count > 1 && has_rule(GameConstants.GAME_RULE_DEH_PEI_SI_PI)) {
	...
	public boolean deal_pass(int seat_index) {
		...
		if (_is_jetton[seat_index] || this._player_ready[seat_index] == 0 || _is_give_up[seat_index]
				|| _is_pass[seat_index]) {
			return true;
		}
		if (this._player_ready[seat_index] == 0) {
	...
	public boolean deal_shuo_hand(int seat_index) {
		...
		if (_is_give_up[seat_index] || this._player_ready[seat_index] == 0
	...
	public boolean deal_add_score(int seat_index, int jetton_score) {
		...
		if (_jetton_current_add_min == 0) {
			if (_jetton_current_add_min >= jetton_score || this._player_ready[seat_index] == 0
					|| _is_give_up[seat_index]) {
				return true;
			}
		} else {
			if (_jetton_current_add_min > jetton_score || this._player_ready[seat_index] == 0
		...
		if (_jetton_current == 0) {
			_first_jetton_count = jetton_score;
		}
	...
	public boolean deal_bobo_score(int seat_index, int jetton_score) {
		if (_is_bobo[seat_index] || jetton_score < _player_result.game_score[seat_index]
				|| _deh_game_staus != GAME_BOBO) {
			return true;
		}
	...
	public boolean deal_bu_score_query(int seat_index) {
		...
		if (this._zi_chan_score[seat_index] + this._player_result.game_score[seat_index] <= BU_FEN_MIN
				&& !_is_bu_zi_chan[seat_index]) {
	...
	public void Refresh_User_Operate(int seat_index) {
		...
		if (seat_index != GameConstants.INVALID_SEAT) {
			...
			if (_jetton_current > _player_result.game_score[seat_index] + _cur_round_jetton_score[seat_index]) {
				...
				if (_jetton_current_add_min > _player_result.game_score[seat_index]
						+ _cur_round_jetton_score[seat_index]) {
					user_can_operate.setAddOperate(0);
				} else {
					user_can_operate.setAddOperate(1);
				}
				user_can_operate.setAddScoreMin(_jetton_current_add_min);
				user_can_operate.setAddScoreMax(
						(int) _player_result.game_score[seat_index] + _cur_round_jetton_score[seat_index]);
	...
	public boolean handler_open_card(int get_seat_index, List<Integer> list, int card_count) {
		...
		if (!this._logic.check_data(GRR._cards_data[get_seat_index], GRR._card_count[get_seat_index], card_date,
				list.size())) {
			this.send_error_notify(get_seat_index, 2, "摆牌不正确！");
			return false;
		}
		logger.info("房间[" + this.getRoom_id() + "]" + "局数[" + this._cur_round + "]" + "_out_cards_data:"
				+ Arrays.toString(card_date) + "_out_card_count:" + card_count);
		for (int i = 0; i < list.size(); i++) {
	...
	public boolean animation_timer(int timer_id) {
		...
		case ID_TIMER_TOU_TO_WEI: {
			...
			GameSchedule.put(
					new GameFinishRunnable(getRoom_id(), GameConstants.INVALID_SEAT, GameConstants.Game_End_NORMAL), 3,
					TimeUnit.SECONDS);
			return true;
		}
