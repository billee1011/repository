package com.cai.game.wsk.handler.gf_bawang;
...
public class WSKHandlerCallBnakerGF_BaWang extends WSKHandlerCallBnaker<WSKTable_GF_BaWang> {

	@Override
	public void exe(WSKTable_GF_BaWang table) {
	}

	public boolean handler_call_banker(WSKTable_GF_BaWang table, int seat_index, int call_action) {
				...
				if (table.has_rule(GameConstants.GAME_RULE_WSK_GF_CALL_HONGTAO_SAN)) {
					...
					for (int i = 0; i < table.getTablePlayerNumber(); i++) {
						for (int j = 0; j < table.GRR._card_count[i]; j++) {
							if (table._jiao_pai_card == table.GRR._cards_data[i][j] % 0x100
									&& table._jiao_pai_seat == GameConstants.INVALID_SEAT) {
								table._jiao_pai_seat = i;
								if (table._jiao_pai_card == table.GRR._cards_data[i][j + 1] % 0x100) {
									table._friend_seat[table._jiao_pai_seat] = i;
									table._friend_seat[i] = table._jiao_pai_seat;
								}
								break;
							} else if (table._jiao_pai_card == table.GRR._cards_data[i][j] % 0x100) {
								table._friend_seat[table._jiao_pai_seat] = i;
								table._friend_seat[i] = table._jiao_pai_seat;
							}
						}
					}
					if (table._friend_seat[table._jiao_pai_seat] == table._jiao_pai_seat) {
						table._friend_seat[table._jiao_pai_seat] = (table._jiao_pai_seat + 2)
								% table.getTablePlayerNumber();
						table._friend_seat[(table._jiao_pai_seat + 2)
								% table.getTablePlayerNumber()] = table._jiao_pai_seat;
					}

					for (int i = 0; i < table.getTablePlayerNumber(); i++) {
						if (i != table._jiao_pai_seat && i != table._friend_seat[table._jiao_pai_seat]) {
							for (int j = 0; j < table.getTablePlayerNumber(); j++) {
								if (j != table._jiao_pai_seat && j != table._friend_seat[table._jiao_pai_seat]
										&& i != j) {
					...
					liang_pai_result.addSeatIndex(table._jiao_pai_seat);
					liang_pai_result.addSeatIndex(table._friend_seat[table._jiao_pai_seat]);
					...
					if (table.has_rule(GameConstants.GAME_RULE_WSK_GF_TOU_XIANG)) {
						...
						table.set_timer(WSKTable_GF_BaWang.TIME_OUT_NO_TOU_XIANG, 20);
					} else {
						table._game_status = GameConstants.GS_GFWSK_PLAY;
					}
					table._cur_banker = table.GRR._banker_player;
					// table._game_status = GameConstants.GS_GFWSK_PLAY;
					// table.operate_out_card(GameConstants.INVALID_SEAT, 0,
					// null, GameConstants.WSK_GF_CT_ERROR,
		...
		} else if (table._game_status == GameConstants.GS_GFWSK_PLAY) {
	...
	@Override
	public boolean handler_player_be_in_room(WSKTable_GF_BaWang table, int seat_index) {
		if (table.GRR == null) {
			return false;
		}
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		...
		if (table._game_status == GameConstants.GS_GFWSK_PLAY) {
			table.operate_out_card(GameConstants.INVALID_SEAT, 0, null, GameConstants.WSK_GF_CT_ERROR, seat_index,
					false);
		}
		return true;
	}
