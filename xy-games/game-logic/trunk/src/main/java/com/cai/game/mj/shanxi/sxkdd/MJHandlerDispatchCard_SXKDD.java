 ...
 */
public class MJHandlerDispatchCard_SXKDD extends MJHandlerDispatchCard<MJTable_SXKDD> {
	...
	public void exe(MJTable_SXKDD table) {
		...
		if (table._playerStatus[_seat_index].is_bao_ting()) {
			...
			if (action != GameConstants.WIK_NULL && table._playerStatus[_seat_index].is_chi_hu_round()) {
				// 添加动作
		...
		if (table.GRR._left_card_count > 1) {
			...
			if (cbActionMask != GameConstants.WIK_NULL) {
				...
				for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
					if (((m_gangCardResult.type[i] == GameConstants.GANG_TYPE_AN_GANG) && m_gangCardResult.cbCardData[i] != _send_card_data)
...
//					}
		...
		/*
		 * boolean isTrue = false; if ((out_ting_count > 0) && (seat_index ==
		 * _seat_index)) { for (int j = 0; j < hand_card_count; j++) { for (int
		 * k = 0; k < out_ting_count; k++) { if (hand_cards[j] ==
		 * table._playerStatus[seat_index]._hu_out_card_ting[k]) {
		 * if(!table._logic.is_magic_card(table._playerStatus[seat_index].
		 * _hu_out_card_ting[k])){ isTrue = true; } hand_cards[j] +=
		 * GameConstants.CARD_ESPECIAL_TYPE_TING; break; } } } } if(isTrue){
