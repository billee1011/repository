package com.cai.game.nn.handler.mfzox;
...
public class NNHandlerOpenCard_mfzOX extends NNHandlerOpenCard {
		...
		for (int k = 0; k < GameConstants.GAME_PLAYER_OX; k++) {
			...
			if (table._player_status[k] != true) {
				...
				if (k == seat_index) {
					...
					for (int j = 0; j < GameConstants.OX_MAX_CARD_COUNT; j++) {
						if(table._last_card[k] == table.GRR._cards_data[k][j] )
							cards.addItem(table.GRR._cards_data[k][j]|0x100);
						else
