 ...
 * 
 */
package com.cai.game.pdk.handler.fifteenpdk_xiangyang;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PDK_FIFTEEN_Table_XiangYang extends PDKTable {
	...
	public PDK_FIFTEEN_Table_XiangYang() {
	}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_out_card_operate = new PDKHandlerOutCardOperate_FIFTEEN_XiangYang();
	...
	private void test_cards() {
		if (DEBUG_CARDS_MODE) {
			int cards[] = new int[] { 3, 19, 35, 51, 4, 20, 36, 52, 5, 21, 37, 53, 6, 22, 38, 54, 7, 23, 39, 55, 8, 24,
					40, 56, 9, 25, 41, 57, 10, 26, 42, 58, 11, 27, 43, 59, 12, 28, 44, 60, 61, 29, 45, 49, 50 };
			// int cards[] = new int[]
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this.GRR._card_count[i] > 1) {
				if (i == this.GRR._banker_player) {
					...
					if (this._out_card_times[i] == 0 && has_rule(GameConstants.GAME_RULE_CHUN_TIAN)) {
			...
			if (reason == GameConstants.Game_End_NORMAL) {
				...
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					...
					if (GRR != null && i != GRR._banker_player && this._out_card_times[i] == 0
							&& has_rule(GameConstants.GAME_RULE_CHUN_TIAN)) {
						if (this._bao_pei_palyer == GameConstants.INVALID_SEAT) {
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status)
			...
			if (this._handler != null && !_is_in_qie) {
				...
				tableResponse_pdk.setRoomInfo(this.getRoomInfo());
				if (GRR != null) {
