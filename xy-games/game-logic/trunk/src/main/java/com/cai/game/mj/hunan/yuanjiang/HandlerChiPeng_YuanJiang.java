...
public class HandlerChiPeng_YuanJiang extends MJHandlerChiPeng<Table_YuanJiang> {
	...
	public HandlerChiPeng_YuanJiang() {
		_type = 0;
			...
			for (int i = 0; i < tmp_hand_card_count; i++) {
				for (int j = 0; j < ting_count; j++) {
					if (tmp_cards[i] == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
						tmp_cards[i] = tmp_cards[i] + GameConstants.CARD_ESPECIAL_TYPE_TING;
					}
				}
			}
