...
public class BYZPHandlerChuLiFirstCard_GL extends GXZPHandlerDispatchCard<GXZPTable> {
...
//		_send_card_data = table._send_card_data;
		...
		while(loop < table.getTablePlayerNumber()){
			int i  = (_seat_index+loop + table.getTablePlayerNumber() + 1)
					% table.getTablePlayerNumber();
			loop++;
...
//		else
	...
	public boolean handler_operate_card(GXZPTable table, int seat_index, int operate_code, int operate_card,int luoCode) {
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_bayi(target_player, _seat_index, operate_card, false);
