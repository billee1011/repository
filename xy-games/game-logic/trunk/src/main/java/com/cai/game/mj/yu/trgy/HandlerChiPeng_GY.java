...
public class HandlerChiPeng_GY extends MJHandlerChiPeng<Table_GY> {
	...
	public void exe(Table_GY table) {
		...
		if (table.is_ji_card(_card)) {
			if (_card == GameConstants_MYGY.BA_TONG_CARD && table.chong_feng_ji_seat_bt == -2) {
				table.responsibility_ji[table.responsibility_ji_count][1].setLeft(_provider);
				table.responsibility_ji[table.responsibility_ji_count++][1].setRight(_seat_index);
				table._player_result.duanmen[_provider] = 1;
			...
			} else if (_card == GameConstants_MYGY.YJ_CARD && table.chong_feng_ji_seat_yj == -2) {
				table.responsibility_ji[table.responsibility_ji_count][0].setLeft(_provider);
				table.responsibility_ji[table.responsibility_ji_count++][0].setRight(_seat_index);
				table._player_result.haspiao[_provider] = 1;
				table.operate_effect_action(_seat_index, GameConstants_MYGY.EFFECT_ACTION_TYPE_ACTION, 1,
						new long[] { GameConstants_MYGY.WIK_ZE_REN_JI }, 1, GameConstants_MYGY.INVALID_SEAT);
			}
			for (int i = 0; i < 3; i++)
				table.out_ji_pai[_seat_index][table.out_ji_pai_count[_seat_index]++] = _card;
		...
		if (table.GRR._left_card_count > 0) {
			...
			int cbActionMask = table.analyse_gang_hong_zhong_all(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
					table.GRR._weave_count[_seat_index], m_gangCardResult, false, table._playerStatus[_seat_index].get_cards_abandoned_gang(),
					_seat_index, 0);
		...
		int card_type_count = GameConstants_MYGY.MAX_ZI;
		...
		if (ting_count > 0) {
			int tmp_cards[] = new int[GameConstants_MYGY.MAX_COUNT];
			int tmp_hand_card_count = table.switch_to_cards_data(table.GRR._cards_index[_seat_index], tmp_cards, _seat_index);

			for (int i = 0; i < tmp_hand_card_count; i++) {
				for (int j = 0; j < ting_count; j++) {
					if (tmp_cards[i] == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
						tmp_cards[i] += GameConstants_MYGY.CARD_ESPECIAL_TYPE_TING;
						break;
					}
				}
			}
	...
	public boolean handler_operate_card(Table_GY table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants_MYGY.WIK_NULL) {
			...
			table.change_player_status(_seat_index, GameConstants_MYGY.INVALID_VALUE);
			table.change_player_status(_seat_index, GameConstants_MYGY.Player_Status_OUT_CARD);
			table.operate_player_status();
