...
public class WMQHandlerOutCardOperate_AX_S extends WMQHandlerOutCardOperate<WMQTable> {
	@Override
	...
	{
		table.log_error(_out_card_player+"出牌出错  WMQHandlerOutCardOperate_AX "+_out_card_data);
		return ;
	}
	//
	// 出牌记录
	...
	if(table.GRR._cards_index[_out_card_player][table._logic.switch_to_card_index(_out_card_data)]==3)
		table.cannot_outcard(table._out_card_player, 1, _out_card_data, true);
	// 用户切换
	...
	bAroseAction = table.estimate_player_out_card_respond_wmq(_out_card_player,_out_card_data,false);// ,
	...
	if (bAroseAction == false ) {
		...
		SysParamModel sysParamModel1105 = SysParamServerDict.getInstance()
				.getSysParamModelDictionaryByGameId(SysGameTypeDict.getInstance().getGameIDByTypeIndex(table._game_type_index))
				.get(1105);
		...
		table.exe_add_discard(_out_card_player, 1, new int[] { _out_card_data|GameConstants.WMQ_OUT_CARD }, true,discard_time);
	...
	public boolean handler_operate_card(WMQTable table, int seat_index, int operate_code, int operate_card,int luoCode) {
		...
		if(target_action == GameConstants.WIK_NULL)
		{
			//过张的牌都不可以吃
			int next_player = (_out_card_player + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();
			table._cannot_chi[next_player][table._logic.switch_to_card_index(target_card)]++;
			...
			for(int i = 0; i<table.getTablePlayerNumber();i++)
			{
				 for(int j = 0; j< table._playerStatus[i]._action_count;j++)
				 {
					...
					{
						table._cannot_peng[i][table._logic.switch_to_card_index(target_card)]++;
					}
				}
			}
		}
		...
		case GameConstants.WIK_NULL: {
			...
			table.operate_out_card(_out_card_player, 0,null, GameConstants.OUT_CARD_TYPE_MID,
					GameConstants.INVALID_SEAT);
			table.exe_add_discard(this._out_card_player, 1, new int[] { this._out_card_data|GameConstants.WMQ_OUT_CARD }, true, 0);
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_wmq_s(target_player, _out_card_player, _out_card_data, false);
...
//			table.countChiHuTimes_wmq(target_player, false);
	...
	public boolean handler_player_be_in_room(WMQTable table, int seat_index) {
		...
		if(table._zhe_zhe_count[seat_index]>0)
			table.operate_zhe_zhe_count(seat_index, false);
		return true;
	}
}
