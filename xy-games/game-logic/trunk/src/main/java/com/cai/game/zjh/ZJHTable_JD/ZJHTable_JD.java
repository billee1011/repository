 ...
 * 
 */
package com.cai.game.zjh.ZJHTable_JD;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class ZJHTable_JD extends ZJHTable {
	...
	protected boolean on_handler_game_start() {
		...
		if (has_rule(GameConstants.GAME_RULE_ZJH_ROUND_10)) {
			_jetton_round = getRuleValue(GameConstants.GAME_RULE_ZJH_ROUND_10);
		...
		if (has_rule(GameConstants.GAME_RULE_ZJH_JETTON_5)) {
			_jetton_max = 5;

			_can_add_jetton_score[0] = 1;
			_can_add_jetton_score[1] = 2;
			_can_add_jetton_score[2] = 5;
			...
			_can_add_jetton_score[3] = 15;
			_can_add_jetton_score[4] = 20;
		...
		_jetton_current = (int) _can_add_jetton_score[0];
	...
	public boolean animation_timer(int timer_id) {
		...
		case ID_TIMER_SEND_TO_OPREATE: {
			...
			this._jetton_current = _can_add_jetton_score[0];
		...
		case ID_AUTO_OPERATE: {
			if (this.has_rule(GameConstants.GAME_RULE_ZJH_TRUSTEE_JETTON)) {
				...
				if (playernum <= 2 && _cur_jetton_round >= _compare_current) {
					this.deal_compare(_current_player, GameConstants.INVALID_SEAT);
				} else {
	...
	private void test_cards() {
		...
		// int cards[] = new int[]
		// {0x01,0x22,0x13,0x1B,0x2D,0x1C,0x22,0x32,0x23,0x24,0x25,0x37,0x05,0x05,0x05};
		int cards[] = new int[] { 0x09, 0x19, 0x27, 0x29, 0x39, 0x17, 54, 45, 5, 60, 57, 52, 35, 36, 10, 51, 56, 9, 4, 41, 11, 33, 50, 59, 55, 3, 61,
				21, 1, 17, 29, 27, 28, 38, 24, 40, 19, 34, 20, 23, 42, 26, 18, 12, 8, 2, 39, 58, 43, 37, 7, 49 };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			for (int j = 0; j < get_hand_card_count_max(); j++) {
	...
	public boolean handler_requst_opreate(int seat_index, int opreate_type, int compareyplayer, int jetton_score) {
		if (this._current_player == GameConstants.INVALID_SEAT && opreate_type != 6) {
			return true;
		}
		switch (opreate_type) {
		case 1: {
		...
		case 5: {
			// 加注
			_pre_opreate_type = opreate_type;
			return deal_add_jetton(seat_index, jetton_score);
		}
		case 6: {
			// 两牌
	...
	public boolean deal_compare(int seat_index, int compareyplayer) {
		...
		this.kill_timer();
		compare_ing_seat_index = GameConstants.INVALID_SEAT;
		_user_jetton_score[seat_index] += _user_gen_score[seat_index] * 2;
		_jetton_total_score += _user_gen_score[seat_index] * 2;
		...
		score_result.setAddJettonScore(_user_gen_score[seat_index] * 2);
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
		...
		compare_result.setAddJettonScore(_user_gen_score[seat_index] * 2);
		...
		// 被比牌玩家
		Arrays.fill(_user_can_add_jetton_score[compareyplayer], 0);
		index = 0;
		for (int x = 0; x < 6; x++) {
			if (_can_add_jetton_score[x] > this._jetton_current && _can_add_jetton_score[x] <= this._jetton_max) {
				if (this.isLookCard[compareyplayer]) {
					_user_can_add_jetton_score[compareyplayer][index++] = _can_add_jetton_score[x] * 2;
				} else {
					_user_can_add_jetton_score[compareyplayer][index++] = _can_add_jetton_score[x];
				}
			}
		}
