...
public class QJQFNotOutCardRunnable extends BaseFuture {

	private static Logger logger = LoggerFactory.getLogger(QJQFNotOutCardRunnable.class);

	private int _room_id;
	private int _seat_index;
	private QJQFTable _table;

	public QJQFNotOutCardRunnable(int room_id, int seat_index, QJQFTable table) {
		super(room_id);
		_room_id = room_id;
	...
	@Override
	public void execute() {
		ReentrantLock roomLock = _table.getRoomLock();
		try {
			...
			_table.handler_operate_out_card_mul(_seat_index, Collections.emptyList(), 0, 0, "");
