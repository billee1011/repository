...
public class PHZHandlerChuLiFirstCard_AnHua extends HHHandlerDispatchCard<AnHuaHHTable> {
	...
	public void exe(AnHuaHHTable table) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			int hu_xi = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i], table.GRR._weave_count[i], i, _seat_index,
					table._send_card_data, chrs[i], GameConstants.HU_CARD_TYPE_TIAN_HU, hu_xi_chi, true);// 自
	...
	private void bankerOperaterCard(AnHuaHHTable table) {
		// 刷新手牌包括组合
