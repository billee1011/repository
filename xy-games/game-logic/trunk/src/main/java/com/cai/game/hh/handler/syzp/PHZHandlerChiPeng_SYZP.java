...
public class PHZHandlerChiPeng_SYZP extends HHHandlerChiPeng<HHTable_SYZP> {

	@SuppressWarnings("unused")
	private GangCardResult m_gangCardResult;

	public PHZHandlerChiPeng_SYZP() {
	 ...
	 */
	@Override
	public boolean handler_operate_card(HHTable_SYZP table, int seat_index, int operate_code, int operate_card, int lou_pai) {
		PlayerStatus playerStatus = table._playerStatus[seat_index];

		// 效验操作
		if ((operate_code != GameConstants.WIK_NULL) && (playerStatus.has_action_by_code(operate_code) == false)) {
			table.log_error("HHHandlerChiPeng_syzp 没有这个操作:" + operate_code);
			return false;
		}
		...
		if (seat_index != _seat_index) {
			table.log_error("HHHandlerChiPeng_syzp 不是当前玩家操作" );
			return false;
		}
