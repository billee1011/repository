...
import com.cai.future.runnable.BTZOpenCardRunnable;
import com.cai.future.runnable.BTZReadyRunnable;
 ...
 */
public class BTZTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	public BTZHandler<? super BTZTable> _handler;

	public BTZHandlerDispatchCard<? super BTZTable> _handler_dispath_card;
	public BTZHandlerCallBanker<? super BTZTable> _handler_Call_banker;
	public BTZHandlerAddJetton<? super BTZTable> _handler_add_jetton;
	public BTZHandlerOpenCard<? super BTZTable> _handler_open_card;
	...
	public int ownRoomSeat; // 房主的位置
	public boolean isGameStart = false;

	public BTZTable(int game_rule_index) {
		super(RoomType.OX, GameConstants.GAME_PLAYER_BTZ);
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		this.setMinPlayerCount(2); // 扳砣子最小开局人数为2
	}
	...
	public void send_card_date_ox() {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			if (this.isTrutess(i) && !this._open_card[i]) {
				GameSchedule.put(new BTZOpenCardRunnable(this.getRoom_id(), i), 7, TimeUnit.SECONDS);
			}
		}
	...
	public void process_openCard() {
		...
		} else if (has_rule(BTZConstants.BTZ_RULE_12_LEI)) {
			tui_zhu_max = 12;
		}
	...
	protected void shuffle(int repertory_card[], int mj_cards[]) {
		...
		if (_game_type_index == GameConstants.GAME_TYPE_BTZ_YY) {
			int playerCount = this.getPlayerCount();
			int doubleCount = 0; // 豹子数量
			for (int i = 0; i < playerCount; i++) {
				if (repertory_card[i] == repertory_card[i + 1]) {
					doubleCount++;
				}
			}
			playerCount = playerCount % 2 == 1 ? playerCount + 1 : playerCount;
			if (playerCount / 2 <= doubleCount) {
				BTZUtils.random_card_data(repertory_card, mj_cards);
			}
		}
	...
	public boolean handler_game_finish_tbox(int seat_index, int reason) {
		...
		if (end) { // 删除
			...
			if (_game_type_index == GameConstants.GAME_TYPE_BTZ_YY) {
				...
				if ((win == 1 && GRR._game_score[this._cur_banker] > 0) || (lose == 1 && GRR._game_score[this._cur_banker] < 0)
						|| (win == 0 && lose == 0)) {
					delay = 2000;
				}
				GameSchedule.put(new BTZReadyRunnable(this.getRoom_id()), delay, TimeUnit.MILLISECONDS);
			} else {
	...
	public boolean handler_release_room(Player player, int opr_code) {
		...
		case GameConstants.Release_Room_Type_QUIT: {// 玩家未开始游戏 退出
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				if ((this.get_players() != null && this.get_players()[i] != null) && (_player_ready[i] == 1)) {
					player_count += 1;
				}
			}
	...
	public boolean handler_enter_room(Player player) {
		if (getRuleValue(GameConstants.GAME_RULE_IP) > 0) {
			for (Player tarplayer : get_players()) {
				if (tarplayer == null || player.getAccount_id() == tarplayer.getAccount_id()) {
					continue;
				}
		...
		if (_game_type_index == GameConstants.GAME_TYPE_BTZ_TB || _game_type_index == GameConstants.GAME_TYPE_BTZ_YY
				|| (GameConstants.GAME_TYPE_BTZ_TB_BEGIN <= _game_type_index && GameConstants.GAME_TYPE_BTZ_TB_END >= _game_type_index)) {
			if (player.getAccount_id() != this.getRoom_owner_account_id() && has_rule(BTZConstants.TB_RULE_NOT_TO_ENTER)) {
				boolean flag = false;
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					if (this._player_status[i] == true) {
						flag = true;
