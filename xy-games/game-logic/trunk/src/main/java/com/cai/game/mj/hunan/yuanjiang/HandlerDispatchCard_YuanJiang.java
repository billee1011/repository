...
public class HandlerDispatchCard_YuanJiang extends MJHandlerDispatchCard<Table_YuanJiang> {
	...
	public HandlerDispatchCard_YuanJiang() {
	...
	public void exe(Table_YuanJiang table) {
		...
		if (table.GRR._left_card_count <= table.getTablePlayerNumber()) {
			table.is_hai_di_state = true;
			...
			GameSchedule.put(new Runnable() {
				@Override
				public void run() {
					table.exe_hai_di(_seat_index, GameConstants.WIK_NULL, 0);
				}
		...
		if (_type == GameConstants.DISPATCH_CARD_TYPE_GANG) {
			hu_card_type = Constants_YuanJiang.HU_CARD_TYPE_GANG_SHANG_HUA;
		}
		...
		if (table.GRR._left_card_count > 0) {
			...
			if (table.is_bao_ting[_seat_index]) {
				...
				if (0 != cbActionMask) {
					for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
						if ((m_gangCardResult.isPublic[i] == 0) || (m_gangCardResult.type[i] == GameConstants.GANG_TYPE_ADD_GANG
								&& m_gangCardResult.cbCardData[i] == _send_card_data)) {
							currentPlayerStatus.add_action(GameConstants.WIK_GANG);
							currentPlayerStatus.add_gang(m_gangCardResult.cbCardData[i], _seat_index, m_gangCardResult.isPublic[i]);
						}
					}
