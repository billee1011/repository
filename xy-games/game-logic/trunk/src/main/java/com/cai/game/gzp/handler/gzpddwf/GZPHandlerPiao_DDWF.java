...
public class GZPHandlerPiao_DDWF extends GZPHandler {
	
	private static Logger logger = Logger.getLogger(GZPHandlerPiao_DDWF.class);
...
//			table.istrustee[i]=false;
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			roomResponse.setPaoMax(GameConstants.PAO_MAX_COUNT_GZP);
			roomResponse.setPaoDes("最多飘3个");
			table.send_response_to_player(i, roomResponse);
		}
	}

	public boolean handler_pao_qiang(GZPTable table, int seat_index, int pao, int qiang) {
		if(table._game_status != GameConstants.GS_MJ_PIAO)
			return true;
		table._playerStatus[seat_index]._is_pao_qiang = true;
		
		if (pao < 0 || pao > GameConstants.PAO_MAX_COUNT_GZP) {
			pao=0;
		}
	...
	private void player_reconnect(GZPTable table, int seat_index) {
