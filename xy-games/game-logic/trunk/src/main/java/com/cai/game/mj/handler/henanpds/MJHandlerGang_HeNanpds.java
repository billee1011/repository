...
public class MJHandlerGang_HeNanpds extends MJHandlerGang<MJTable> {
    @Override
    public void exe(MJTable table) {
        ...
        if (table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU)) {// 可炮胡的时候 才能抢杆
    ...
    public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
        ...
        if (jie_pao_count > 0) {
            ...
            table.process_chi_hu_player_score_henan_pds(target_player, _seat_index, _center_card, false);
    ...
    protected boolean exe_gang(MJTable table) {
        ...
        if ((GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HENAN_CHU_FENG_BAO_TING)
                ...
                || (!GameDescUtil.has_rule(table.getGameRuleIndexEx(),
                        GameConstants.GAME_RULE_HENAN_CHU_FENG_BAO_TING))) {
            // 河南麻将,检查听牌
            table._playerStatus[_seat_index]._hu_card_count = table.get_henan_ting_card(
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                ...
                if (jia_di) {// 庄家加底
                    if (zhuang_gang) {
                        score += 1;// 庄家杆 别人多输一分
                    } else if (table.GRR._banker_player == i) {// 别人杆 庄家多输一分
                        score += 1;
                    }
                }

                if (gang_pao) {// 杆跑--双方下跑分数+底分
                    // 勾选杠跑玩法，开牌前根据跑头和跑扣最大的分数来决定杠跑分。
                    int xiapaoF = 0;
                    int xiaopaoS = 0;
                    xiapaoF = (table._player_result.pao[i] > table._player_result.qiang[i])
                            ? table._player_result.pao[i]
                            : table._player_result.qiang[i];
                    xiaopaoS = (table._player_result.pao[_seat_index] > table._player_result.qiang[_seat_index])
                            ? table._player_result.pao[_seat_index]
                            : table._player_result.qiang[_seat_index];
                    score += xiapaoF + xiaopaoS;
            ...
            if (jia_di) {// 庄家加底
                if (zhuang_gang) {
                    score += 1;// 庄家杆 别人多输一分
                } else if (zhuang_fang_gang) {// 庄家放杆 庄家多输一分
                    score += 1;
                }
            }
            ...
            if (gang_pao) {// 杆跑--双方下跑分数+底分
                ...
                int xiapaoF = 0;
                int xiaopaoS = 0;
                xiapaoF = (table._player_result.pao[_provide_player] > table._player_result.qiang[_provide_player])
                        ? table._player_result.pao[_provide_player]
                        : table._player_result.qiang[_provide_player];
                xiaopaoS = (table._player_result.pao[_seat_index] > table._player_result.qiang[_seat_index])
                        ? table._player_result.pao[_seat_index]
                        : table._player_result.qiang[_seat_index];
                score += xiapaoF + xiaopaoS;
                // score +=
                // table._player_result.pao[_provide_player]+table._player_result.pao[_seat_index];
            }
            ...
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] = score;// m_pGameServiceOption->lCellScore*2;//配置参数乘
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][table.GRR._weave_items[_seat_index][cbWeaveIndex].provide_player] = -score;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
