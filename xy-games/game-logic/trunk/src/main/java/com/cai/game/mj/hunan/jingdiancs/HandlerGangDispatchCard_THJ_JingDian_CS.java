...
public class HandlerGangDispatchCard_THJ_JingDian_CS extends AbstractMJHandler<Table_JingDian_CS> {
	...
	public HandlerGangDispatchCard_THJ_JingDian_CS() {
	...
	public void exe(Table_JingDian_CS table) {
			...
			for (int k = 0; k < table.getTablePlayerNumber(); k++) {
				if (k == _seat_index) {
					...
					int action = table.analyse_chi_hu_card(table.GRR._cards_index[k], table.GRR._weave_items[k], table.GRR._weave_count[k], bu_card,
							chr, Constants_JingDian_CS.HU_CARD_TYPE_GANG_KAI, k);
						...
						if (cbActionMask != GameConstants.WIK_NULL) {
							for (int gc = 0; gc < m_gangCardResult.cbCardCount; gc++) {
								...
								if (curPlayerStatus.lock_huan_zhang()) {
									boolean has_huan_zhang = table.check_gang_huan_zhang(k, m_gangCardResult.cbCardData[gc]);
					...
					bAroseAction = table.estimate_player_fan_pai_response_thj(k, _seat_index, bu_card, special_player_cards, i);
				}
		...
		GameSchedule.put(new Runnable() {
			@Override
			public void run() {
				// 和经典长沙麻将不一样，土豪金经典长沙麻将，如果杠翻出来的牌，玩家能胡，需要有特效，并且需要分别对待
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	...
	public boolean handler_operate_card(Table_JingDian_CS table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: {
			...
			if (table.GRR._chi_hu_rights[_seat_index].is_mul_long(Constants_JingDian_CS.CHR_ZI_MO)) {
		...
		case GameConstants.WIK_CHI_HU: {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if (table.GRR._chi_hu_rights[i].is_mul_long(Constants_JingDian_CS.CHR_JIE_PAO)) {
					...
					table.process_chi_hu_player_operate(i, new int[] { table._playerStatus[i]._operate_card }, 1, false);
	...
	private void exe_chi_peng(Table_JingDian_CS table, int target_player, int target_action, int target_card) {
		table.operate_out_card(this._seat_index, 0, null, GameConstants.OUT_CARD_TYPE_LEFT, GameConstants.INVALID_SEAT);
