package com.cai.game.wsk.handler.zzshA;
...
import com.cai.game.wsk.WSKGameLogic_ZZSHA;
...
import protobuf.clazz.zzshA.zzshARsp.GameStart_zzshA;
import protobuf.clazz.zzshA.zzshARsp.OutCardDataWsk_zzshA;
import protobuf.clazz.zzshA.zzshARsp.PukeGameEndWsk_zzshA;
import protobuf.clazz.zzshA.zzshARsp.RefreshScore_zzshA;
import protobuf.clazz.zzshA.zzshARsp.Refresh_Pai_zzshA;
import protobuf.clazz.zzshA.zzshARsp.SendCard_zzshA;
import protobuf.clazz.zzshA.zzshARsp.TableResponse_zzshA;
import protobuf.clazz.zzshA.zzshARsp.effect_type_zzshA;

public class WSKTable_ZzshA extends AbstractWSKTable {
	...
	public int _out_sign_data[][];
	public int _out_sign_count[];
	public int _hong_tao_chair = GameConstants.INVALID_SEAT;
	public int _fang_kuai_chair = GameConstants.INVALID_SEAT;

	protected static final int ID_TIMER_SEND_CARD = 1;// 发牌时间
	public int sort[];// 排序标志
	public WSKGameLogic_ZZSHA _logic;

	public WSKTable_ZzshA() {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_out_sign_data = new int[getTablePlayerNumber()][this.get_hand_card_count_max()];
		_out_sign_count = new int[getTablePlayerNumber()];
		...
		_handler_out_card_operate = new WSKHandlerOutCardOperate_ZzshA();
	...
	public void Refresh_user_get_score(int to_player) {
		...
		RefreshScore_zzshA.Builder refresh_user_getscore = RefreshScore_zzshA.newBuilder();
		roomResponse.setType(WSKMsgConstants.RESPONSE_ZZASHA_USER_GET_SCORE);
		Player rplayer;
	...
	protected boolean on_handler_game_start() {
		...
		_fang_kuai_chair = GameConstants.INVALID_SEAT;
		_hong_tao_chair = GameConstants.INVALID_SEAT;
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			_out_sign_count[i] = 0;
			_seat_team[i] = 0;
			Arrays.fill(_out_sign_data[i], GameConstants.INVALID_CARD);
		...
		_repertory_card = new int[WSKConstants.CARD_COUNT_ZZSHA];
		shuffle(_repertory_card, WSKConstants.CARD_DATA_ZZSHA);
	...
	public void shuffle(int repertory_card[], int card_cards[]) {
		...
		for (int i = 0; i < count; i++) {
			for (int j = 0; j < this.get_hand_card_count_max(); j++) {
				...
				if (GRR._cards_data[i][j] == 0x01 || GRR._cards_data[i][j] == 0x21) {
					_seat_team[i] = 1;
					if (GRR._cards_data[i][j] == 0x21) {
						_hong_tao_chair = i;
					}
					if (GRR._cards_data[i][j] == 0x01) {
						_fang_kuai_chair = i;
					}
				}
	...
	protected boolean on_game_start() {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(WSKMsgConstants.RESPONSE_ZZSHA_GAME_START);
		roomResponse.setGameStatus(this._game_status);
		roomResponse.setRoomInfo(getRoomInfo());
		// 发送数据
		GameStart_zzshA.Builder gamestart = GameStart_zzshA.newBuilder();
		...
		this.send_effect_type(_cur_banker, 1, null, 1, GameConstants.INVALID_SEAT);
		return true;
	}
	...
	public void send_effect_type(int seat_index, int type, int data[], int is_animation, int to_player) {
		//
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(WSKMsgConstants.RESPONSE_ZZSHA_EFFECT);
		effect_type_zzshA.Builder effect = effect_type_zzshA.newBuilder();
	...
	public void Send_card() {
		...
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			roomResponse.setType(WSKMsgConstants.RESPONSE_ZZSHA_SEND_CARD);
			roomResponse.setGameStatus(this._game_status);
			roomResponse.setRoomInfo(getRoomInfo());
			// 发送数据
			SendCard_zzshA.Builder send_card = SendCard_zzshA.newBuilder();
	...
	protected boolean on_handler_game_finish(int seat_index, int reason) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(WSKMsgConstants.RESPONSE_ZZSHA_GAME_END);
		GameEndResponse.Builder game_end = GameEndResponse.newBuilder();
		PukeGameEndWsk_zzshA.Builder game_end_wsk = PukeGameEndWsk_zzshA.newBuilder();
		...
		if (GRR != null) {
			...
			game_end_wsk.setFangKuaiAChair(_fang_kuai_chair);
			game_end_wsk.setHongTaoAChair(_hong_tao_chair);
		}
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			game_end_wsk.addXianQianScore(jia_fa_socre[i]);
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if (_cur_round >= _game_round) {// 局数到了
				...
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					...
					game_end_wsk.addGuanMenTimes(this._player_info[i]._guan_men_times);
					game_end_wsk.addShuangATimes(this._player_info[i]._banker_timer);
				}
	...
	public void load_player_info_data_game_start(GameStart_zzshA.Builder roomResponse) {
		Player rplayer;
	...
	public void load_player_info_data_reconnect(TableResponse_zzshA.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_game_end(PukeGameEndWsk_zzshA.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player,
		...
		if (to_player == GameConstants.INVALID_SEAT) {
			for (int index = 0; index < this.getTablePlayerNumber(); index++) {
				RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
				OutCardDataWsk_zzshA.Builder outcarddata = OutCardDataWsk_zzshA.newBuilder();
				// Int32ArrayResponse.Builder cards =
				// Int32ArrayResponse.newBuilder();
				roomResponse.setType(WSKMsgConstants.RESPONSE_ZZSHA_OUT_CARD);// 201
