...
public class SCHCPHandlerOutCardOperate_YDR extends SCHCPDZHandlerOutCardOperate<SCHCPDZTable> {
	@Override
	public void exe(SCHCPDZTable table) {
		...
		int next_player = table.get_cur_index(_out_card_player);
		...
		if( table.is_bao_zi(_out_card_player) ){
			table._is_bao_zi[_out_card_player] = true;
			table.operate_player_xiang_gong_flag(_out_card_player, true);
			table.operate_effect_action(_out_card_player, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { GameConstants.DZ_BAO_ZI }, 1,
					GameConstants.INVALID_SEAT);
		}
		...
		while(loop < table.getTablePlayerNumber()){
			int i = table.get_cur_index(_out_card_player+loop) ; 
			loop ++;
			...
			action_hu[i] = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i],
					table.GRR._weave_count[i],i,_out_card_player,_out_card_data, chr[i], card_type,false);// 自摸
...
//				action_hu[i] = GameConstants.WIK_NULL;
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				tempPlayerStatus.add_action(GameConstants.DZ_WIK_NULL);
...
//				return ;
		...
		for(int i = 0; i< table.getTablePlayerNumber();i++){
			...
			if(i == _out_card_player)
				continue;
			action |= table.estimate_player_gang(i, _out_card_data,false);
			action |= table.estimate_player_tu_huo_xiang(i, _out_card_data,false);
		}
		if(action == GameConstants.DZ_WIK_NULL)
			bAroseAction = table.estimate_player_chi_ydr(_out_card_player,_out_card_data,false);// ,
		for(int i = 0; i<table.getTablePlayerNumber();i++){
			table._cannot_peng_index[i][table._logic.switch_to_card_index(_out_card_data)]++;
		}
		table._table_cards[table._logic.switch_to_card_index(_out_card_data)]++;																											// EstimatKind.EstimatKind_OutCar
		
		// 如果没有需要操作的玩家，派发扑克
		
		if (bAroseAction != false||action != GameConstants.DZ_WIK_NULL||hu_pai != 0){
			// 等待别人操作这张牌
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			// 用户切换
			_current_player = table._current_player = table.get_cur_index(_out_card_player) ; 
	...
	public boolean handler_ask_player( SCHCPDZTable table, int seat_index){
	...
	public boolean handler_operate_card(SCHCPDZTable table, int seat_index, int operate_code, int operate_card,int luoCode) {
		...
		if(operate_card != table._out_card_data&& operate_code !=  GameConstants.CP_WIK_NULL)
		{
...
//		}
		...
		// 用户状态
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if(i != target_player&&table._playerStatus[i].has_action_by_bh_code(GameConstants.DZ_WIK_PENG)&&target_card!=0)
				table._cannot_peng_index[i][table._logic.switch_to_card_index(target_card)]++;
			table._playerStatus[i].clean_action();
			table._playerStatus[i].clean_status();
		...
		{
			table.exe_gang(target_player, this._out_card_player, operate_card, target_action,GameConstants.CHR_DZ_OUT_CARD,true, true, false,1000);
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_chq_ydr(target_player, this._out_card_player, operate_card, false);
