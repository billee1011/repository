...
public class MJHandlerGang_JZ extends MJHandlerGang<MJTable_JZ> {
    @Override
...
//                        % table.getTablePlayerNumber();
     ...
     ***/
    @Override
    protected boolean exe_gang(MJTable_JZ table) {
        ...
        //
        int di_fen = 1;
        if (table.has_rule(GameConstants_JZ.GAME_RULE_JZ_515)) {
            di_fen = 5;
        }
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if (i == _seat_index)
                    continue;
                int score = di_fen;
                if(table.has_rule(GameConstants_JZ.GAME_RULE_JZ_GANG_PAO)){
                	score += ((table._player_result.pao[i] < 0 ? 0 : table._player_result.pao[i])
                			+ (table._player_result.pao[_seat_index] < 0 ? 0 :  table._player_result.pao[_seat_index]));// 跑分
                }
                // 暗杠每人1分
            ...
            table._player_result.an_gang_count[_seat_index]++;
        	 ...
        	 int score = di_fen;
             if(table.has_rule(GameConstants_JZ.GAME_RULE_JZ_GANG_PAO)){
             	score += ((table._player_result.pao[_provide_player] < 0 ? 0 : table._player_result.pao[_provide_player])
             			+ (table._player_result.pao[_seat_index] < 0 ? 0 :  table._player_result.pao[_seat_index]));// 跑分
             }
            // 放杠1分
            ...
            table._player_result.ming_gang_count[_seat_index]++;
        	 ...
        	 int score = di_fen;
        	 int index = table.GRR._weave_items[_seat_index][cbWeaveIndex].provide_player;
             if(table.has_rule(GameConstants_JZ.GAME_RULE_JZ_GANG_PAO)){
             	score += ((table._player_result.pao[index] < 0 ? 0 : table._player_result.pao[index])
             			+ (table._player_result.pao[_seat_index] < 0 ? 0 :  table._player_result.pao[_seat_index]));// 跑分
             }
            // 放杠1分
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] = score * GameConstants.CELL_SCORE;// m_pGameServiceOption->lCellScore*2;//配置参数乘
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][index] = -score * GameConstants.CELL_SCORE;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
