...
public class MJHandlerChiPeng_KLDS extends MJHandlerChiPeng<MJTable_KLDS> {
	private GangCardResult m_gangCardResult;

	public MJHandlerChiPeng_KLDS() {
	...
	public void exe(MJTable_KLDS table) {
		...
		if (ting_count > 0) {
			...
			for (int i = 0; i < tmp_hand_card_count; i++) {
				for (int j = 0; j < ting_count; j++) {
					if (tmp_cards[i] == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
						if (table._logic.is_magic_card(tmp_cards[i])) {
							tmp_cards[i] += GameConstants.CARD_ESPECIAL_TYPE_SHENGPAI_TING;
						} else {
				...
				if (table._logic.is_magic_card(tmp_cards[i])) {
					tmp_cards[i] += GameConstants.CARD_ESPECIAL_TYPE_SHENGPAI;
				}
			}
	 ...
	 */
	@Override
	public boolean handler_player_out_card(MJTable_KLDS table, int seat_index, int card) {
	...
	public boolean handler_player_be_in_room(MJTable_KLDS table, int seat_index) {
		...
		for (int i = 0; i < out_ting_count; i++) {
			...
			if (table._logic.is_magic_card(table._playerStatus[seat_index]._hu_out_card_ting[i])) {
				roomResponse.addOutCardTing(table._playerStatus[seat_index]._hu_out_card_ting[i] + GameConstants.CARD_ESPECIAL_TYPE_SHENGPAI_TING);
			} else {
