...
public class WSKGameLogic_PC extends WSKGameLogic {

	public WSKGameLogic_PC() {
	...
	public int GetCardType(int cbCardData[], int cbCardCount, boolean isLast) {
		...
		if (wang_count == 4 && cbCardCount == 4) {
			return GameConstants.WSK_PC_CT_KING_FOUR;
		}
		// 王加4炸
		if (wang_count > 0) {
			if (card_index.card_index[this.switch_card_to_idnex(cbCardData[0])] >= 4) {
				...
				if (count + wang_count == cbCardCount) {
					if (cbCardCount == 4) {
						return GameConstants.WSK_PC_CT_BOMB_4;
					}
					if (cbCardCount == 5) {
						return GameConstants.WSK_PC_CT_BOMB_5;
					}
					if (cbCardCount == 6) {
						return GameConstants.WSK_PC_CT_BOMB_6;
					}
					if (cbCardCount == 7) {
						return GameConstants.WSK_PC_CT_BOMB_7;
					}
					if (cbCardCount == 8) {
						return GameConstants.WSK_PC_CT_BOMB_8;
					}
					if (cbCardCount == 9) {
						return GameConstants.WSK_PC_CT_BOMB_9;
					}
					if (cbCardCount == 10) {
						return GameConstants.WSK_PC_CT_BOMB_10;
					}
					if (cbCardCount == 11) {
						return GameConstants.WSK_PC_CT_BOMB_11;
					}
					if (cbCardCount == 12) {
						return GameConstants.WSK_PC_CT_BOMB_12;
					}
				}
			}
		}
		if (card_index.card_index[this.switch_card_to_idnex(cbCardData[cbCardCount - 1])] == cbCardCount
			...
			// 所有牌都为同一种牌
			if (cbCardCount == 1) {
				return GameConstants.WSK_PC_CT_SINGLE;
			}
			if (cbCardCount == 2) {
				return GameConstants.WSK_PC_CT_DOUBLE;
			}
			// if (cbCardCount == 3) {
			// return GameConstants.WSK_PC_CT_THREE;
			// }
			if (cbCardCount == 4) {
				return GameConstants.WSK_PC_CT_BOMB_4;
			}
		...
		if (cbCardCount == 5) {
			// 三带二
			// if (has_rule(GameConstants.GAME_RULE_WSK_PC_SAN_DAI_ER)) {
			for (int i = 0; i < cbCardCount; i++) {
				if (card_index.card_index[this.switch_card_to_idnex(cbCardData[i])] >= 3
						|| card_index.card_index[this.switch_card_to_idnex(cbCardData[i])] >= 3) {
					return GameConstants.WSK_PC_CT_THREE_TAKE_TWO;
				}
			}
			// }
		}
		...
		if (cbCardCount >= 5) {
			if (this.is_link(card_index, 1, cbCardCount)) {
				return GameConstants.WSK_PC_CT_SINGLE_LINK;
			}
		}
		...
		if (cbCardCount % 2 == 0) {
			...
			if (this.is_link(card_index, 2, cbCardCount / 2) && wang_count == 0 && er_count == 0) {
				return GameConstants.WSK_PC_CT_DOUBLE_LINK;
			}
		}
		...
		if (nPlane == 0) {
			return GameConstants.WSK_PC_CT_PLANE_LOST;
		} else if (nPlane == 1) {
			return GameConstants.WSK_PC_CT_PLANE;
		}
		// 三连
		if (cbCardCount % 3 == 0) {
	...
	public boolean CompareCard_WSK(int cbFirstCard[], int cbNextCard[], int cbFirstCount, int cbNextCount,
		...
		// 炸弹以上一定大于单牌、对子和单龙
		if (cbNextType >= GameConstants.WSK_PC_CT_BOMB_4) {
		...
		switch (cbFirstType) {
		case GameConstants.WSK_PC_CT_SINGLE:
		case GameConstants.WSK_PC_CT_DOUBLE: {
			if (cbFirstType != cbNextType) {
				return false;
			}
			return GetCardLogicValue(cbNextCard[0]) > GetCardLogicValue(cbFirstCard[0]);
		}
		case GameConstants.WSK_PC_CT_SINGLE_LINK:
		case GameConstants.WSK_PC_CT_DOUBLE_LINK: {
			if (cbFirstType != cbNextType || cbFirstCount != cbNextCount) {
				return false;
			}
			return GetCardLogicValue(cbNextCard[0]) > GetCardLogicValue(cbFirstCard[0]);
		}
		case GameConstants.WSK_PC_CT_PLANE:
		case GameConstants.WSK_PC_CT_PLANE_LOST: {
			if (cbNextType != GameConstants.WSK_PC_CT_PLANE && cbNextType != GameConstants.WSK_PC_CT_PLANE_LOST) {
				return false;
			}
		...
		case GameConstants.WSK_PC_CT_THREE_TAKE_TWO:
		case GameConstants.WSK_PC_CT_THREE: {
			if (cbNextType != GameConstants.WSK_PC_CT_THREE_TAKE_TWO && cbNextType != GameConstants.WSK_PC_CT_THREE) {
				return false;
			}
			int next_index = -1;
			int first_index = -1;
	...
	// 获得同类型
	public int get_cards(int cbCardData[], int cbCardCount, int card[], int value) {
		int count = 0;
		for (int i = 0; i < cbCardCount; i++) {
			if (value == this.GetCardLogicValue(cbCardData[i]))
				card[count++] = cbCardData[i];
		}
		return count;
	}
	...
	public void SortCardList(int cbCardData[], int cbCardCount, int cbSortType) {
		...
		if (cbSortType == GameConstants.WSK_ST_COUNT) {
			...
			int sort_num = 0;
			for (int i = GameConstants.WSK_MAX_INDEX - 1; i > GameConstants.WSK_MAX_INDEX - 1; i--) {
			...
			for (int i = cbCardCount - 1; i >= 0; i--) {
				if (cbCardData[i] > 0x100) {
					...
					for (int j = i; j < cbCardCount - 1; j++) {
						if ((cbCardData[j + 1] & 0xF00) < (cbCardData[j] & 0xF00)) {
	...
	public int GetCardXianScore(int cbCardData[], int cbCardCount, int card_type) {
		...
		if (this.GetCardValue(cbCardData[0]) == 2) {
			if (card_type == GameConstants.WSK_PC_CT_BOMB_12) {
				return 256;
			}
		...
		if (card_type == GameConstants.WSK_PC_CT_BOMB_5) {
			score = 1;
		}
		if (card_type == GameConstants.WSK_PC_CT_BOMB_6) {
			score = 2;
		}
		if (card_type == GameConstants.WSK_PC_CT_BOMB_7) {
			score = 4;
		}
		if (card_type == GameConstants.WSK_PC_CT_BOMB_8) {
			score = 8;
		}
		if (card_type == GameConstants.WSK_PC_CT_BOMB_9) {
			score = 16;
		}
		if (card_type == GameConstants.WSK_PC_CT_BOMB_10) {
			score = 32;
		}
		if (card_type == GameConstants.WSK_PC_CT_BOMB_11) {
			score = 64;
		}
		if (card_type == GameConstants.WSK_PC_CT_BOMB_12) {
			score = 128;
		}
		if (card_type == GameConstants.WSK_PC_CT_KING_FOUR) {
			score = 16;
		}
	...
	public int Ai_Out_Card(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count,
			int three_link_count, int card_data[]) {

		int card_count = 0;
		int turn_card_type = this.GetCardType(turn_card_data, turn_card_count);
		int tip_out_card[][] = new int[cbCardCount * 2][cbCardCount];
		int tip_out_count[] = new int[cbCardCount * 2];
		int tip_type_count = 0;
		switch (turn_card_type) {
		case GameConstants.WSK_PC_CT_KING_FOUR:
		case GameConstants.WSK_PC_CT_BOMB_12:
		case GameConstants.WSK_PC_CT_BOMB_11: {
			break;
		}
		case GameConstants.WSK_PC_CT_BOMB_10:
		case GameConstants.WSK_PC_CT_BOMB_9:
		case GameConstants.WSK_PC_CT_BOMB_8:
		case GameConstants.WSK_PC_CT_BOMB_7:
		case GameConstants.WSK_PC_CT_BOMB_6:
		case GameConstants.WSK_PC_CT_BOMB_5:
		case GameConstants.WSK_PC_CT_BOMB_4: {
			tip_type_count += search_out_card_boom(cbCardData, cbCardCount, turn_card_data, turn_card_count,
					tip_out_card, tip_out_count, tip_type_count, false);
			break;
		}
		case GameConstants.WSK_PC_CT_PLANE:
		case GameConstants.WSK_PC_CT_PLANE_LOST: {
			tip_type_count += search_out_card_plane(cbCardData, cbCardCount, turn_card_data, turn_card_count,
					three_link_count, tip_out_card, tip_out_count, tip_type_count, false);
			break;
		}
		case GameConstants.WSK_PC_CT_DOUBLE_LINK: {
		...
		case GameConstants.WSK_PC_CT_ERROR: {
			tip_type_count += search_out_error(cbCardData, cbCardCount, turn_card_data, turn_card_count, tip_out_card,
					tip_out_count, tip_type_count, false);
			break;
		}
		}
		if (tip_type_count > 0) {
			for (int i = 0; i < tip_out_count[0]; i++)
				card_data[i] = tip_out_card[0][i];
			card_count = tip_out_count[0];
		}

		return card_count;
	}

	public boolean SearchOutCard(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count,
			int three_link_count) {
	...
	public int search_out_card(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count,
		...
		case GameConstants.WSK_PC_CT_BOMB_4: {
			tip_type_count += search_out_card_boom(cbCardData, cbCardCount, turn_card_data, turn_card_count,
					tip_out_card, tip_out_count, tip_type_count, true);
			return tip_type_count;
		}
		case GameConstants.WSK_PC_CT_PLANE:
		case GameConstants.WSK_PC_CT_PLANE_LOST: {
			tip_type_count += search_out_card_plane(cbCardData, cbCardCount, turn_card_data, turn_card_count,
					three_link_count, tip_out_card, tip_out_count, tip_type_count, true);
			return tip_type_count;
		}
		case GameConstants.WSK_PC_CT_DOUBLE_LINK: {
			tip_type_count += search_out_card_double_link(cbCardData, cbCardCount, turn_card_data, turn_card_count,
		...
		case GameConstants.WSK_PC_CT_ERROR: {
			tip_type_count += search_out_error(cbCardData, cbCardCount, turn_card_data, turn_card_count, tip_out_card,
					tip_out_count, tip_type_count, true);
			return tip_type_count;
		}
		}
	...
	public int search_out_error(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count,
			int tip_out_card[][], int tip_out_count[], int all_tip_count, boolean is_tip) {
				...
				if (hand_card_idnex.card_index[index] == count) {
					...
					if (count == 3) {
						...
						for (int j = 0; j < GameConstants.WSK_MAX_INDEX; j++) {
							if (j == index)
								continue;
							for (int k = 0; k < hand_card_idnex.card_index[j]; k++) {
								tip_out_card[all_tip_count][tip_out_count[all_tip_count]++] = hand_card_idnex.card_data[j][k];
								take_count++;
								if (take_count == 2)
									break;
							}
					...
					if (count == 4) {
						for (int y = 0; y < hand_card_idnex.card_index[13]; y++) {
							tip_out_card[all_tip_count][tip_out_count[all_tip_count]++] = hand_card_idnex.card_data[13][y];
						}
						for (int y = 0; y < hand_card_idnex.card_index[14]; y++) {
							tip_out_card[all_tip_count][tip_out_count[all_tip_count]++] = hand_card_idnex.card_data[14][y];
						}
					}
					all_tip_count++;
					if (is_tip == false && all_tip_count == 1)
		...
		// 搜索炸弹
		all_tip_count += search_boom(cbCardData, cbCardCount, tip_out_card, tip_out_count, all_tip_count, is_tip);
		return all_tip_count;
	}
	...
	public int search_out_single_double(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count,
		...
		for (int j = turn_card_count; j < 4; j++) {
			for (int i = 0; i < cbCardCount;) {
				int index = this.switch_card_to_idnex(cbCardData[i]);
				if (is_boom == true && (this.GetCardColor(cbCardData[i]) == 4)) {
					if (hand_card_idnex.card_index[index] > 0) {
						i += hand_card_idnex.card_index[index];
					} else {
						i++;
				...
				if (hand_card_idnex.card_index[index] == j && index > turn_index) {
	...
	public int search_out_card_three(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count,
		...
		for (int i = 0; i < cbCardCount;) {
			...
			if (hand_card_idnex.card_index[index] == 3 && index > turn_index) {
				int take_count = 0;
				for (int index_num = 1; index_num < 4; index_num++) {
					for (int take_index = 0; take_index < GameConstants.WSK_MAX_INDEX; take_index++) {
						if (is_boom == true && take_index > 10) {
							continue;
						}
						if (take_count >= 2) {
							break;
						}
						if (take_index != index && hand_card_idnex.card_index[take_index] == index_num) {
	...
	public int search_out_card_single_link(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count,
		...
		for (int i = 0; i < cbCardCount;) {
			int index = this.switch_card_to_idnex(cbCardData[i]);
			if (hand_card_idnex.card_index[index] >= 1 && hand_card_idnex.card_index[index] < 4 && index > turn_index
					&& index != 12) {
				int next_index = index;
				for (int j = i + hand_card_idnex.card_index[index]; j < cbCardCount;) {
					int other_index = this.switch_card_to_idnex(cbCardData[j]);
					if (hand_card_idnex.card_index[other_index] >= 1 && hand_card_idnex.card_index[other_index] < 4
							&& next_index == other_index - 1 && other_index != 12) {
						next_index = other_index;
						if ((next_index - index) + 1 == turn_card_count) {
							for (int x = next_index; x >= index; x--) {
								for (int y = 0; y < 1; y++) {
	...
	public int search_out_card_double_link(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count,
		...
		for (int i = 0; i < cbCardCount;) {
			int index = this.switch_card_to_idnex(cbCardData[i]);
			if (hand_card_idnex.card_index[index] >= 2 && hand_card_idnex.card_index[index] < 4 && index > turn_index
					&& index != 12) {
				int next_index = index;
				for (int j = i + hand_card_idnex.card_index[index]; j < cbCardCount;) {
					int other_index = this.switch_card_to_idnex(cbCardData[j]);
					...
					if (hand_card_idnex.card_index[other_index] >= 2 && hand_card_idnex.card_index[other_index] < 4
							&& next_index == other_index - 1 && other_index != 12) {
						next_index = other_index;
						if ((next_index - index) + 1 == turn_card_count / 2) {
							for (int x = next_index; x >= index; x--) {
								for (int y = 0; y < 2; y++) {
									tip_out_card[all_tip_count][tip_out_count[all_tip_count]++] = hand_card_idnex.card_data[x][y];
								}
	...
	public int search_out_card_plane(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count,
			int three_link_count, int tip_out_card[][], int tip_out_count[], int all_tip_count, boolean is_tip) {
		tagAnalyseIndexResult_WSK hand_card_idnex = new tagAnalyseIndexResult_WSK();
		tagAnalyseIndexResult_WSK card_index = new tagAnalyseIndexResult_WSK();
		AnalysebCardDataToIndex(turn_card_data, turn_card_count, card_index);
		AnalysebCardDataToIndex(cbCardData, cbCardCount, hand_card_idnex);
		...
		for (int i = GameConstants.WSK_MAX_INDEX - 4; i >= 0; i--) {
			...
			if (cbCardCount > three_link_count * 5) {
					...
					for (int j = i - 1; j >= 0; j--) {
						if (hand_card_idnex.card_index[j] == 3) {
							if ((i - j) + 1 == three_link_count) {
								int take_count = 0;
								for (int index_num = 1; index_num < 4; index_num++) {
									for (int take_index = 0; take_index < GameConstants.WSK_MAX_INDEX
											- 2; take_index++) {
										if (is_boom == true && take_index > 13) {
											continue;
										}
				...
				// 如果牌还够的情况下，先不考虑拆炸弹
				if (hand_card_idnex.card_index[i] == 3 && i > turn_index && i < 12) {
					for (int j = i - 1; j >= 0; j--) {
						if (hand_card_idnex.card_index[j] == 3 && j < 12) {
							if ((i - j) + 1 == three_link_count) {
								for (int take_index = 0; take_index < GameConstants.WSK_MAX_INDEX; take_index++) {
									if (is_boom == true && take_index > 13) {
										continue;
	...
	public int search_boom(int cbCardData[], int cbCardCount, int tip_out_card[][], int tip_out_count[],
			int all_tip_count, boolean is_tip) {
		tagAnalyseIndexResult_WSK hand_card_idnex = new tagAnalyseIndexResult_WSK();
		AnalysebCardDataToIndex(cbCardData, cbCardCount, hand_card_idnex);
		int wang_count = hand_card_idnex.card_index[13] + hand_card_idnex.card_index[14];
		// 不带王的炸弹
		...
		for (int add_boom_count = 0; add_boom_count <= 8 - 4; add_boom_count++) {
			for (int i = 0; i < cbCardCount;) {
				int index = this.switch_card_to_idnex(cbCardData[i]);
				if (hand_card_idnex.card_index[index] == 4 + add_boom_count) {
		...
		if ((hand_card_idnex.card_index[14] + hand_card_idnex.card_index[13] != 4) && boom_count == 1) {
			for (int j = 0; j < hand_card_idnex.card_index[13]; j++) {
				tip_out_card[all_tip_count - 1][tip_out_count[all_tip_count - 1]++] = hand_card_idnex.card_data[13][j];
			}
			for (int j = 0; j < hand_card_idnex.card_index[14]; j++) {
				tip_out_card[all_tip_count - 1][tip_out_count[all_tip_count - 1]++] = hand_card_idnex.card_data[14][j];
			}
		...
		for (int count = 1; count <= wang_count; count++) {
			for (int add_boom_count = 0; add_boom_count <= 4; add_boom_count++) {
				for (int i = 0; i < cbCardCount;) {
					int index = this.switch_card_to_idnex(cbCardData[i]);
					if (hand_card_idnex.card_index[index] >= 4) {
						if (hand_card_idnex.card_index[index] + count == 4 + add_boom_count) {
							for (int j = 0; j < hand_card_idnex.card_index[index]; j++) {
								tip_out_card[all_tip_count][tip_out_count[all_tip_count]++] = hand_card_idnex.card_data[index][j];
							}
							int use_wang = 0;
	...
	public int search_out_card_boom(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count,
		...
		if ((hand_card_idnex.card_index[14] + hand_card_idnex.card_index[13] != 4) && all_tip_count == 1) {
			for (int j = 0; j < hand_card_idnex.card_index[13]; j++) {
				tip_out_card[all_tip_count - 1][tip_out_count[all_tip_count - 1]++] = hand_card_idnex.card_data[13][j];
			}
			for (int j = 0; j < hand_card_idnex.card_index[14]; j++) {
	...
	public int get_three_link_count(int cbCardData[], int cbCardCount, int type) {
		...
		if (type == GameConstants.WSK_PC_CT_PLANE_LOST) {
			for (int i = GameConstants.WSK_MAX_INDEX - 4; i >= 0; i--) {
				// 三个2不能当做飞机
				if (card_data_index.card_index[i] >= 3) {
					int link_num = 1;
			...
			for (int i = GameConstants.WSK_MAX_INDEX - 4; i >= 0; i--) {
				...
				if (card_data_index.card_index[i] >= 3) {
					...
					for (int j = i - 1; j >= 0; j--) {
						...
						if (j == GameConstants.WSK_MAX_INDEX - 4) {
							if (link_num > 1 && link_num * 5 == cbCardCount) {
								return link_num;
							}
						}
