 ...
 * @author WalkerGeek 
 */
public class PHZHandlerDispatchFirstCard_YYZHZ extends HHHandlerDispatchCard<HHTable_YYZHZ> {
	...
	public void exe(HHTable_YYZHZ table) {
		...
		//板板胡
		int[] tem_card = new int[GameConstants_YYZHZ.MAX_YYZHZ_INDEX];
		for (int i = 0; i < GameConstants_YYZHZ.MAX_YYZHZ_INDEX; i++) {
			tem_card[i] = table.GRR._cards_index[_seat_index][i];
		}
		int send_index  = table._logic.switch_to_card_index_yyzhz(_send_card_data);
		tem_card[send_index]++;
		
		
		boolean ban_ban_hu = table._logic.checkBanBanHU(tem_card);
		ChiHuRight chr = table.GRR._chi_hu_rights[_seat_index];
		if(ban_ban_hu){
			chr.opr_or(GameConstants_YYZHZ.CHR_HU_BAN_BAN_HU);
	...
	@Override
	public boolean handler_operate_card(HHTable_YYZHZ table,int seat_index, int operate_code, int operate_card,int luoCode){
