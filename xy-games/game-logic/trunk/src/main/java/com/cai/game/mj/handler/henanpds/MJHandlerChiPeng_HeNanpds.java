...
public class MJHandlerChiPeng_HeNanpds extends MJHandlerChiPeng<MJTable> {
	boolean can_bao_ting = false;

	private GangCardResult m_gangCardResult;

	public MJHandlerChiPeng_HeNanpds() {
	...
	public void exe(MJTable table) {
			...
			if (ting_count > 0) {
				...
				for (int j = 0; j < tmp_hand_card_count; j++) {
					for (int k = 0; k < ting_count; k++) {
						if (tmp_cards[j] == table._playerStatus[_seat_index]._hu_out_card_ting[k]) {
							tmp_cards[j] += GameConstants.CARD_ESPECIAL_TYPE_TING;
							break;
						}
					}

					if (table.has_rule(GameConstants.GAME_RULE_HENAN_DAI_HUN) && tmp_cards[j] < GameConstants.CARD_ESPECIAL_TYPE_TING
							&& table._logic.is_magic_card(tmp_cards[j])) {
						tmp_cards[j] += GameConstants.CARD_ESPECIAL_TYPE_HUN;
					}
				}
		...
		// 剩余20张 结算
		// llcard = GameConstants.CARD_COUNT_LEFT_PDSHUANGZHUANG;
		// }
