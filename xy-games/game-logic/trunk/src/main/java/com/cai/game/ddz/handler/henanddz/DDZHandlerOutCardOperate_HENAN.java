...
public class DDZHandlerOutCardOperate_HENAN extends DDZHandlerOutCardOperate {
	@Override
	public void exe(DDZTable table) {
		...
		if (cbCardType == GameConstants.DDZ_CT_BOMB_CARD || cbCardType == GameConstants.DDZ_CT_MISSILE_CARD) {
			if (table.has_rule(GameConstants.GAME_RULE_LIMIT_FOUR_BOMB) && table._boom_count < 2) {
				table._times *= 2;
				table._boom_count++;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					table._user_times[i] *= 2;
				}
			} else if (table.has_rule(GameConstants.GAME_RULE_LIMIT_FIVE_BOMB) && table._boom_count < 3) {
				table._times *= 2;
				table._boom_count++;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					table._user_times[i] *= 2;
				}
			} else if (table.has_rule(GameConstants.GAME_RULE_LIMIT_SIX_BOMB) && table._boom_count < 4) {
				table._times *= 2;
				table._boom_count++;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					table._user_times[i] *= 2;
