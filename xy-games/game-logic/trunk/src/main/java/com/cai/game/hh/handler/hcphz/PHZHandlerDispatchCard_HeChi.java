 ...
 *
 */
public class PHZHandlerDispatchCard_HeChi extends HHHandlerDispatchCard<HeChiHHTable> {
	...
	public void exe(HeChiHHTable table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				table.operate_show_card(i, GameConstants_HeChi.Show_Card_HU, hand_card_count, cards, table.GRR._weave_items[i],
						table.GRR._weave_count[i], GameConstants_HeChi.INVALID_SEAT);
		...
		if (saoChuanCount > 0) {
			...
			for (int i = 0; i < saoChuanCount; i++) {
				int cbWeaveIndex = table.GRR._weave_count[_seat_index];
				table.GRR._weave_items[_seat_index][cbWeaveIndex].public_card = 1;
				table.GRR._weave_items[_seat_index][cbWeaveIndex].center_card = table._logic.switch_to_card_data(saoChuan[i]);
				table.GRR._weave_items[_seat_index][cbWeaveIndex].weave_kind = GameConstants_HeChi.WIK_TI_LONG;
				...
				// 删除手上的牌
				table.GRR._cards_index[_seat_index][saoChuan[i]] = 0;
		...
		int sao = table.estimate_player_ti_wei_respond_phz(_seat_index, _send_card_data);
		if (sao != GameConstants_HeChi.WIK_NULL) {
			table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants_HeChi.INVALID_SEAT, false);
			return;
		}
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if ((action_pao[i] != GameConstants_HeChi.WIK_NULL) && (bHupai == 0)) {
				sao = GameConstants_HeChi.WIK_PAO;
				table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants_HeChi.INVALID_SEAT, false);
				table.exe_gang(i, _seat_index, _send_card_data, action_pao[i], pao_type[i][0], true, true, false, 1000);
				return;
			} else if (action_pao[i] != GameConstants_HeChi.WIK_NULL) {
				sao = GameConstants_HeChi.WIK_PAO;
			}
		}

		// 玩家出牌 响应判断,是否有吃,碰,胡 swe
		table.log_info(_seat_index + "发牌 " + _send_card_data + "ti_sao = " + sao);
		boolean bAroseAction = false;
		if (sao != GameConstants_HeChi.WIK_PAO) {
			bAroseAction = table.estimate_player_out_card_respond_PHZ(_seat_index, _send_card_data, false);
			if (bHupai == 0)
				table.operate_player_get_card(_seat_index, 1, new int[] { _send_card_data }, GameConstants_HeChi.INVALID_SEAT, false);
		}

		if ((bAroseAction == false) && (sao == GameConstants_HeChi.WIK_NULL)) {
		...
		if (curPlayerStatus.has_action()) {// 有动作
			if (table.isTrutess(_seat_index)) {
				GameSchedule.put(new OutCardRunnable(table.getRoom_id(), _seat_index, _send_card_data),
						GameConstants_HeChi.DELAY_AUTO_OUT_CARD_TRUTESS, TimeUnit.MILLISECONDS);
				return;
			}
			curPlayerStatus.set_status(GameConstants_HeChi.Player_Status_OPR_CARD);// 操作状态
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				GameSchedule.put(new OutCardRunnable(table.getRoom_id(), _seat_index, _send_card_data), GameConstants_HeChi.DELAY_AUTO_OUT_CARD,
						TimeUnit.MILLISECONDS);
			} else if (sao == GameConstants_HeChi.WIK_NULL) {
	...
	public boolean handler_operate_card(HeChiHHTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		// 判断可不可以吃的上家用户
		int last_player = (target_player + table.getPlayerCount() + 1) % table.getPlayerCount();
		boolean flag = false;
		...
		if (target_action == GameConstants.WIK_NULL) {
		...
		} else if ((target_action & eat_type) != GameConstants_HeChi.WIK_NULL) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				for (int j = 0; j < table._playerStatus[i]._action_count; j++) {
					if (table._playerStatus[i]._action[j] == GameConstants.WIK_PENG) {
						table._cannot_peng[i][table._cannot_peng_count[i]++] = _send_card_data;
			...
			// table._cannot_chi[_seat_index][table._cannot_chi_count[_seat_index]++]
			// = _send_card_data;
		}
		// 用户状态
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		case GameConstants_HeChi.WIK_NULL: {
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					if (action != GameConstants_HeChi.WIK_NULL) {
				...
				table._last_player = next_player;
				// 没有人要就加入到牌堆
				table.exe_dispatch_card(next_player, GameConstants_HeChi.WIK_NULL, 0);
				table._last_card = _send_card_data;
		...
		case GameConstants_HeChi.WIK_LEFT: // 上牌操作
		{
			...
			table.exe_chi_peng(target_player, _seat_index, target_action, target_card, GameConstants_HeChi.CHI_PENG_TYPE_DISPATCH, target_lou_code);
			return true;
		}
		...
		case GameConstants_HeChi.WIK_PAO: // 杠牌操作
		{
			int pao_type[] = new int[1];
