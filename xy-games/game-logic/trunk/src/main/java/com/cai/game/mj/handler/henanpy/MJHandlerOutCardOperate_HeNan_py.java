...
public class MJHandlerOutCardOperate_HeNan_py extends MJHandlerOutCardOperate<MJTable_PY> {
    ...
    public void exe(MJTable_PY table) {
        ...
        // real_card += MJGameConstants.CARD_ESPECIAL_TYPE_HUN;
        // }
        // 显示出牌
        ...
        // 玩家出牌 响应判断,是否有吃碰杠补胡
        boolean bAroseAction = table.estimate_player_out_card_respond_henan_py(_out_card_player, _out_card_data);// ,
    ...
    public boolean handler_operate_card(MJTable_PY table, int seat_index, int operate_code, int operate_card) {
        ...
        if (table._playerStatus[_out_card_player].is_respone() == false
                && table._playerStatus[_out_card_player].has_action_by_code(GameConstants.WIK_BAO_TING)) {
            if (operate_code != GameConstants.WIK_CHI_HU) {
                return false;
            } else {
                table._playerStatus[_out_card_player].clean_action();
            }

        } else {
            if (table._playerStatus[_out_card_player].is_respone()
                    && table._playerStatus[_out_card_player].has_action_by_code(GameConstants.WIK_BAO_TING)) {
                if (_out_card_player == seat_index && operate_code == GameConstants.WIK_BAO_TING) {
        ...
        {
            if (this._type == GameConstants.HU_CARD_TYPE_GANG_KAI
                    && table.has_rule(GameConstants.GAME_RULE_PUYANG_HUANGHUNAG)) {
                table.GRR._gang_score[_out_card_player].gang_count--;
                for (int w = table.GRR._weave_count[_out_card_player] - 1; w >= 0; w--) {
                    if (table.GRR._weave_items[_out_card_player][w].weave_kind != GameConstants.WIK_GANG
                            || !table.GRR._weave_items[_out_card_player][w].is_vavild) {
                        continue;
                    }
                    table.GRR._weave_items[_out_card_player][w].is_vavild = false;
                    break;
                }
            ...
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                ...
                if (this._type == GameConstants.HU_CARD_TYPE_GANG_KAI
                        && table.has_rule(GameConstants.GAME_RULE_PUYANG_HUANGHUNAG)) {
                    int cbGangIndex = 0;
                    int cbtargerGangIndex = table.GRR._gang_score[_out_card_player].gang_count;

                    table.gang_shang_pao_score[i] += table.GRR._gang_score[_out_card_player].scores[cbtargerGangIndex][_out_card_player];
                    table.gang_shang_pao_score[_out_card_player] -= table.GRR._gang_score[_out_card_player].scores[cbtargerGangIndex][_out_card_player];
                }

                table.process_chi_hu_player_score_py(i, _out_card_player, _out_card_data, false);
            ...
            if (this._type == GameConstants.HU_CARD_TYPE_GANG_KAI
                    && table.has_rule(GameConstants.GAME_RULE_PUYANG_HUANGHUNAG)) {
                int cbGangIndex = table.GRR._gang_score[_out_card_player].gang_count;
                table.GRR._gang_score[_out_card_player].scores[cbGangIndex][_out_card_player] = 0;
                table.GRR._chi_hu_rights[target_player].opr_or(GameConstants.CHR_HUNAN_GANG_SHANG_PAO);
        ...
        case GameConstants.WIK_BAO_TING: //
        {
            ...
            // 加到牌堆 没有人要
            table.exe_add_discard(this._out_card_player, 1,
                    new int[] { _out_card_data + GameConstants.CARD_ESPECIAL_TYPE_BAO_TING }, false, 0);

            table.exe_dispatch_card(next_player, GameConstants.WIK_NULL, GameConstants.DELAY_SEND_CARD_DELAY);

            // 报听
            // table.exe_out_card_bao_ting(_out_card_player, operate_card,
