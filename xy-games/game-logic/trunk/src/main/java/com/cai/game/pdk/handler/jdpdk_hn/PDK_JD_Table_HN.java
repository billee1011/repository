 ...
 * 
 */
package com.cai.game.pdk.handler.jdpdk_hn;
...
import com.cai.game.pdk.PDKGameLogic_HN;
...
import protobuf.clazz.pdk_xiangyang.pdk_xiangyangPdkRsp.PukeGameEndPdk_XiangYang;
import protobuf.clazz.pdk_xiangyang.pdk_xiangyangPdkRsp.card_arry_xiangyang;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PDK_JD_Table_HN extends PDKTable {
	...
	public PDKGameLogic_HN _logic;

	public PDK_JD_Table_HN() {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_logic = new PDKGameLogic_HN();
		_handler_out_card_operate = new PDKHandlerOutCardOperate_JD_HN();
	...
	protected boolean on_handler_game_start() {
		...
		//
		if (this.has_rule(GameConstants.GAME_RULE_BOOM_FOUR_AAAA)) {
			_repertory_card = new int[GameConstants.CARD_COUNT_PDK_JD];
			shuffle(_repertory_card, GameConstants.CARD_DATA_PDK_JD_AAAA);
		} else {
			_repertory_card = new int[GameConstants.CARD_COUNT_PDK_JD];
	...
	public void update_card_type() {
		if (!this.isCoinRoom()) {
			return;
		}
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			Player player = get_players()[i];
			if (this._logic.SearchThreeLineCard(GRR._cards_data[i], GRR._card_count[i])) {
				// 上报牌型

				if (player != null) {
					triggerTypeEvent(ETriggerType.START, player.getAccount_id(), GameConstants.PDK_CT_PLANE);
				}
			}
			if (this._logic.SearchBoomTen(GRR._cards_data[i], GRR._card_count[i])) {
				// 上报牌型
				if (player != null) {
					triggerTypeEvent(ETriggerType.START, player.getAccount_id(), GameConstants.PDK_CT_BOMB_CARD);
				}
			}
			if (this._logic.SearchSingleLink(GRR._cards_data[i], GRR._card_count[i])) {
				// 上报牌型
				if (player != null) {
					triggerTypeEvent(ETriggerType.START, player.getAccount_id(), GameConstants.PDK_CT_SINGLE_LINE);
				}
			}
			if (this._logic.SearchDoubleLink(GRR._cards_data[i], GRR._card_count[i])) {
				// 上报牌型
				if (player != null) {
					triggerTypeEvent(ETriggerType.START, player.getAccount_id(), GameConstants.PDK_CT_DOUBLE_LINE);
				}
			}
			if (this._logic.SearchSingleLinkColor(GRR._cards_data[i], GRR._card_count[i])) {
				// 上报牌型
				if (player != null) {
					triggerTypeEvent(ETriggerType.START, player.getAccount_id(), GameConstants.PDK_CT_HONG_HUA_SHUN);
				}
			}
			if (player != null) {
				triggerEventOver(ETriggerType.START, player.getAccount_id());
			}
	...
	private boolean game_start_pkd() {
		...
		} else if (has_rule(GameConstants.GAME_RULE_SHOU_JU_HEITAO_SAN)) {
			GRR._banker_player = this.find_banker();
		}
	...
	private void test_cards() {
		...
		int cards[] = new int[] { 26, 24, 25, 6, 58, 19, 22, 37, 60, 39, 57, 20, 43, 1, 27, 38, 12, 61, 41, 7, 45, 4, 3,
				42, 8, 44, 33, 52, 29, 55, 11, 40, 17, 35, 36, 9, 59, 53, 28, 54, 56, 21, 23, 51, 5, 13, 2, 10 };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public void cal_score_pdk_jd(int end_score[]) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			if (this.GRR._card_count[i] == 0) {
				...
				if (this.GRR._card_count[i] > max_count) {
					max_count = this.GRR._card_count[i];
				}
			}
		}
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this.GRR._card_count[i] > 1) {
				if (i == this.GRR._banker_player) {
					if (this.has_rule(GameConstants.GAME_RULE_CAL_SCORE_TWO)) {
						if (this.GRR._card_count[i] == max_count) {
							end_score[i] -= 2;
						} else {
							end_score[i] -= 1;
						...
						end_score[i] -= this.GRR._card_count[i] * this._boom_times;
					}
					...
					if (this.has_rule(GameConstants.GAME_RULE_CAL_SCORE_TWO)) {
						...
						if (this._out_card_times[i] == 0) {
							end_score[i] -= this.GRR._card_count[i] * 2 * this._boom_times;
						} else {
							end_score[i] -= this.GRR._card_count[i] * this._boom_times;
						}
	...
	public boolean handler_game_finish_pdk_jd(int seat_index, int reason) {
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			...
			PukeGameEndPdk_XiangYang.Builder game_end_pdk = PukeGameEndPdk_XiangYang.newBuilder();
			game_end_pdk.setRoomInfo(this.getRoomInfo());
			...
			if (GRR != null) {
				...
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					...
					Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
					card_arry_xiangyang.Builder out_card_array = card_arry_xiangyang.newBuilder();
					for (int j = 0; j < GRR._card_count[i]; j++) {
						cards_card.addItem(GRR._cards_data[i][j]);
					}
		...
		if (godViewObservers().count() > 0) {
			send_response_to_all_god(roomResponse);
		}
	...
	public boolean handler_player_ready(int seat_index, boolean is_cancel) {
		if (is_cancel) {//
			...
			if (this._cur_round > 0) {
				...
				send_response_to_all_god(roomResponse2);
			}
			return false;
		}
		if (this.get_players()[seat_index] == null) {
	...
	public boolean handler_god_observer_be_in_room(Player player) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status)) {
			...
			if (this._handler != null && !_is_in_qie) {
				this._handler.handler_god_observer_be_in_room(this, player);
			} else if (!_is_in_qie) {
				...
				if (GRR != null) {
					...
					for (int i = 0; i < getTablePlayerNumber(); i++) {
						...
						tableResponse_pdk.addCardsData(cards_card);
					}
				...
				roomResponse.setCommResponse(PBUtil.toByteString(tableResponse_pdk));
				send_response_to_god_player(player, roomResponse);
				// operate_player_cards();
			}
			if (_is_in_qie) {
				QiePaiStart.Builder qie_pai = QiePaiStart.newBuilder();
	...
	public void handler_pause_room() {
		if (_auto_out_card_scheduled != null) {
			_auto_out_card_scheduled.cancel(false);
		}
		return;
	}
	...
	public void handler_continue_room() {
		...
		if (GRR != null) {
			if (this._turn_out_card_count != 0) {
				...
				if (has_rule(GameConstants.GAME_RULE_KUAI_SU)) {
					auto_out_card_dely = 500;
				}
				if (matchId != 0) {
					auto_out_card_dely = 2000;
				}
				if (_logic.Player_Can_out_card(GRR._cards_data[this._current_player], GRR._card_count[_current_player],
						_turn_out_card_data, _turn_out_card_count, can_out_card_data) == 0) {

					_auto_out_card_scheduled = GameSchedule.put(
							new PDKAutoOutCardRunnable(getRoom_id(), _current_player, this, GameConstants.PDK_CT_PASS),
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player) {
		...
		if (this._turn_out_card_type == GameConstants.PDK_CT_BOMB_CARD
				&& this.has_rule(GameConstants.GAME_RULE_BOOM_FOUR_TAKE_ONE)) {
			if (_turn_out_card_count == 4) {
				outcarddata.setPrOutCardType(GameConstants.PDK_CT_THREE_TAKE_ONE);
			} else {
				outcarddata.setPrOutCardType(GameConstants.PDK_CT_FOUR_LINE_TAKE_ONE);
			}
	...
	public void load_player_info_data_game_end(PukeGameEndPdk_XiangYang.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
