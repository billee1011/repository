...
public class MJHandlerGang_LILINGZZ extends MJHandlerGang<MJTable_LILINGZZ> {

    // 执行杆操作
    @Override
    public void exe(MJTable_LILINGZZ table) {
        // 用户状态
     ...
     */
    @Override
    public boolean handler_operate_card(MJTable_LILINGZZ table, int seat_index, int operate_code, int operate_card) {
        if (GameDescUtil.has_rule(table.gameRuleIndexEx, GameConstants.GAME_RULE_HUNAN_BU_TONG_PAO)) {
                ...
                for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
                    for (int j = 0; j < table.GRR._player_niao_count[i]; j++) {
                        if (i == target_player) {
                            ...
                            table.GRR._player_niao_cards[i][j] = table
                                    .set_ding_niao_valid(table.GRR._player_niao_cards[i][j], false);// 胡牌的鸟失效
                        }
                    }
                }
            ...
            if (operate_code == GameConstants.WIK_NULL) {
                ...
                //WalkerGeek 有一家胡了其他不需要操作
                for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                	if(i == seat_index || i == _provide_player){
                		continue;
                	}
                	if(table._playerStatus[i].has_chi_hu() && !table.GRR._chi_hu_rights[i].is_valid()){
                          ...
                          table._playerStatus[i].operate(operate_code, operate_card);
                          //关闭操作
                          //table.operate_player_action(i, false);
                	}
                }
           ...
           //WalkerGeek 需求修改: 通炮情况下有一个玩家点了胡其他有胡的玩家也必须跟着胡
           /* boolean hasHu = false;  //是否有人胡牌
            for(int i = 0; i < table.getTablePlayerNumber(); i++){
     ...
     ***/
    @Override
    protected boolean exe_gang(MJTable_LILINGZZ table) {
