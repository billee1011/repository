...
public class PHZHandlerChuLiFirstCard_XiangXiang extends HHHandlerDispatchCard<XiangXiangHHTable> {
	...
	public boolean handler_operate_card(XiangXiangHHTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		case GameConstants_XiangXiang.WIK_NULL: {
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				GameSchedule.put(new OutCardRunnable(table.getRoom_id(), _seat_index, table._send_card_data),
						GameConstants_XiangXiang.DELAY_AUTO_OUT_CARD, TimeUnit.MILLISECONDS);
			} else {
		...
		case GameConstants_XiangXiang.WIK_ZI_MO: // 自摸
		{
			...
			// }
			// }
			table.GRR._chi_hu_rights[target_player].opr_or(GameConstants_XiangXiang.CHR_TIAN_HU);

			if (table.has_rule(GameConstants_XiangXiang.GAME_RULE_DI_ERZI_LIANG_PAI) == false)
				table.operate_player_get_card(_seat_index, 1, new int[] { table._send_card_data }, GameConstants_XiangXiang.INVALID_SEAT, false);
	 ...
	 */
	@Override
	public boolean handler_player_out_card(XiangXiangHHTable table, int seat_index, int card) {
		// 错误断言
		...
		// 出牌
		table.exe_out_card(_seat_index, card, GameConstants.DispatchCard_Type_Tian_Hu);
