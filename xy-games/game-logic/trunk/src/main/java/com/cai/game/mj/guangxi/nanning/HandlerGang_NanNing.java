...
public class HandlerGang_NanNing extends MJHandlerGang<Table_NanNing> {
	...
	@Override
	public boolean exe_gang(Table_NanNing table) {

		int cbCardIndex = table._logic.switch_to_card_index(_center_card);
		int cbWeaveIndex = -1;

		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			table._chi_pai_count[_seat_index][_provide_player]++; // 吃三比，不用在意提供者是谁
		...
		for (int i = 0; i < weave_count; i++) {
			...
			if (_provide_player == table.GRR._weave_items[_seat_index][i].provide_player)
				direct_count++;
		}

		if (table.feng_pai_4_orther_seat[_seat_index] == -1 && direct_count >= 3 && _seat_index != _provide_player) {
			table.feng_pai[_provide_player] = true;
			table.feng_pai_4_orther_seat[_seat_index] = _provide_player;
		}
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			table._player_result.ming_gang_count[_seat_index]++;
			table.quan_bao[_seat_index] = provide_index;
		}
