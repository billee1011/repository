...
public class PHZHandlerGang_XiangXiang extends HHHandlerGang<XiangXiangHHTable> {
	...
	public void exe(XiangXiangHHTable table) {
		...
		if (_depatch == false)
			table.operate_out_card(this._provide_player, 0, null, GameConstants_XiangXiang.OUT_CARD_TYPE_MID, GameConstants_XiangXiang.INVALID_SEAT);
		else
			table.operate_player_get_card(this._provide_player, 0, null, GameConstants_XiangXiang.INVALID_SEAT, false);

		// 效果
		table.operate_effect_action(_seat_index, GameConstants_XiangXiang.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { _action }, 5,
				GameConstants_XiangXiang.INVALID_SEAT);

		// if ((GameConstants_XiangXiang.GANG_TYPE_AN_GANG == _type) ||
		// (GameConstants_XiangXiang.GANG_TYPE_JIE_GANG == _type)) {
		// this.exe_gang(table);
		// return;
		// }
		this.exe_gang(table);
		...
		//// }
		//// if(playerStatus.has_chi_hu()){
		//// if(table.has_rule(GameConstants_XiangXiang.GAME_RULE_LIXIANG_FLS_JIANPAOHU)
		// ||
		// table.isTrutess(i)){
		//// //见炮胡
		//// table.exe_jian_pao_hu(i,GameConstants_XiangXiang.WIK_CHI_HU,_center_card);
		//// }else{
		//// table._playerStatus[i].set_status(GameConstants_XiangXiang.Player_Status_OPR_CARD);//操作状态
		//// table.operate_player_action(i, false);
		//// }
		//// }
		// }
	...
	public boolean handler_operate_card(XiangXiangHHTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		if ((operate_code != GameConstants_XiangXiang.WIK_NULL) && (operate_code != GameConstants_XiangXiang.WIK_ZI_MO))// 没有这个操作动作
		{
			table.log_player_error(seat_index, "HHHandlerGang_YX出牌操作,没有动作");
			return false;
		}

		if ((operate_code != GameConstants_XiangXiang.WIK_NULL) && (operate_card != _center_card)) {
		...
		// 这一轮就不能吃胡了没过牌之前都不能胡
		// } else if (operate_code == GameConstants_XiangXiang.WIK_CHI_HU) {
		...
		case GameConstants_XiangXiang.WIK_NULL: {
			...
			table.is_first_mo[target_player] = false;
			if ((table._is_xiang_gong[_seat_index] == false)
					&& (table._long_count[_seat_index] == 1 || GameConstants_XiangXiang.SAO_TYPE_MINE_SAO == _type)) {
				...
				if (pai_count == 0) {
					...
					table._playerStatus[_seat_index].set_status(GameConstants_XiangXiang.Player_Status_OUT_CARD);// 出牌状态
				...
				table.exe_dispatch_card(next_player, GameConstants_XiangXiang.WIK_NULL, 1500);
				table.log_info(next_player + "提 扫 跑 发牌" + _seat_index);
		...
		case GameConstants_XiangXiang.WIK_ZI_MO: // 自摸
		{
			...
			if (_depatch == true)
				table.operate_player_get_card(this._provide_player, 1, new int[] { _center_card }, GameConstants_XiangXiang.INVALID_SEAT, false);
			...
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), _seat_index, GameConstants_XiangXiang.Game_End_NORMAL), delay,
					TimeUnit.SECONDS);
	 ...
	 * 
	 ***/
	protected boolean exe_gang(XiangXiangHHTable table) {
		int cbCardIndex = table._logic.switch_to_card_index(_center_card);
		int cbWeaveIndex = -1;
		boolean is_ting_hu = true;
		if (GameConstants_XiangXiang.PAO_TYPE_AN_LONG == _type) {
			// 暗龙
			// 设置变量
		...
		} else if (GameConstants_XiangXiang.PAO_TYPE_TI_MINE_LONG == _type) {
			// 提龙
			// 设置变量
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
			table._long_count[_seat_index]++;
		} else if (GameConstants_XiangXiang.PAO_TYPE_MINE_SAO_LONG == _type) {
			// 提龙
			// 设置变量
			// 看看是不是有碰的牌，明杠
			// 寻找组合
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				...
				if (cbWeaveKind != GameConstants_XiangXiang.WIK_AN_LONG)
					is_ting_hu = false;
				if ((cbCenterCard == _center_card) && (cbWeaveKind == GameConstants_XiangXiang.WIK_WEI)) {
		...
		} else if (GameConstants_XiangXiang.PAO_TYPE_OTHER_SAO_PAO == _type) {
			// 别人打的牌
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				int cbWeaveKind = table.GRR._weave_items[_seat_index][i].weave_kind;
				int cbCenterCard = table.GRR._weave_items[_seat_index][i].center_card;
		...
		} else if (GameConstants_XiangXiang.PAO_TYPE_OHTER_PAO == _type) {
			// 别人打的牌
			table._long_count[_seat_index]++;
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
		...
		} else if (GameConstants_XiangXiang.PAO_TYPE_MINE_PENG_PAO == _type) {
			// 看看是不是有碰的牌，明杠
			// 寻找组合
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				int cbWeaveKind = table.GRR._weave_items[_seat_index][i].weave_kind;
				int cbCenterCard = table.GRR._weave_items[_seat_index][i].center_card;
				if ((cbCenterCard == _center_card) && (cbWeaveKind == GameConstants_XiangXiang.WIK_PENG)) {
					cbWeaveIndex = i;// 第几个组合可以碰
					table._long_count[_seat_index]++;
					break;
				}
		...
		} else if (GameConstants_XiangXiang.SAO_TYPE_MINE_SAO == _type) {
			// 扫牌
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
				table.cannot_outcard(i, 1, _center_card, true);
			}
		}
		...
		if (an_long_count > 0) {
			...
			for (int i = 0; i < an_long_count; i++) {
				...
				table.GRR._weave_items[_seat_index][cbWeaveIndex].weave_kind = GameConstants_XiangXiang.WIK_AN_LONG;
			...
			// 刷新手牌包括组合
			cards = new int[GameConstants_XiangXiang.MAX_HH_COUNT];
		...
		int pai_count = 0;
		int action_hu = GameConstants_XiangXiang.WIK_NULL;
		for (int i = 0; i < GameConstants_XiangXiang.MAX_HH_INDEX; i++) {
			if (table.GRR._cards_index[_seat_index][i] < 3)
				pai_count += table.GRR._cards_index[_seat_index][i];
		}
		if ((_depatch == true) && (table._ti_two_long[_seat_index] == false) && (table._is_xiang_gong[_seat_index] == false)) {
			...
			if (is_ting_hu == true) {
				for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
					int cbWeaveKind = table.GRR._weave_items[_seat_index][i].weave_kind;
					if (!((cbWeaveKind == GameConstants_XiangXiang.WIK_TI_LONG) || (cbWeaveKind == GameConstants_XiangXiang.WIK_AN_LONG)
							|| (cbWeaveKind == GameConstants_XiangXiang.WIK_WEI))) {
						is_ting_hu = false;
					}
				}
			}
			if (action_hu != GameConstants_XiangXiang.WIK_NULL) {
				...
				if (tempPlayerStatus.has_action()) {
					tempPlayerStatus.set_status(GameConstants_XiangXiang.Player_Status_OPR_CARD);//
					// 操作状态
					table.operate_player_action(_seat_index, false);
					if (is_ting_hu == true) {
						if ((chr.opr_and(GameConstants_XiangXiang.CHR_TING_HU)).is_empty())
							chr.opr_or(GameConstants_XiangXiang.CHR_TING_HU);
					}
					return true;
				}
			} else {
				chr.set_empty();
				table.is_first_mo[_seat_index] = false;
			}

			// if (pai_count == 0) {
			// PlayerStatus curPlayerStatus = table._playerStatus[_seat_index];
			// curPlayerStatus.reset();
			// curPlayerStatus.add_action(GameConstants_XiangXiang.WIK_ZI_MO);
			...
			// }
			// return handler_operate_card(table, _seat_index,
			// GameConstants_XiangXiang.WIK_ZI_MO, _center_card, 0);
			// }
			if (pai_count == 0) {
				int all_hu_xi = 0;
				for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				...
				if ((card_type == GameConstants_XiangXiang.HU_CARD_TYPE_ZIMO) && (_seat_index == _provide_player)) {
					chr.opr_or(GameConstants_XiangXiang.CHR_ZI_MO);
				} else if ((card_type == GameConstants_XiangXiang.HU_CARD_TYPE_ZIMO) && (_seat_index != _provide_player)) {
					chr.opr_or(GameConstants_XiangXiang.CHR_JEI_PAO_HU);
				}
				// if (hong_pai_count >= 10) {
				// table._hong_pai_count[_seat_index] = hong_pai_count;
				// chr.opr_or(GameConstants_XiangXiang.CHR_TEN_HONG_PAI);
				// }
				if (hong_pai_count >= 10 && hong_pai_count < 13) {
					chr.opr_or(GameConstants_XiangXiang.CHR_TEN_HONG_PAI);
				}
				if (hong_pai_count >= 13) {
					chr.opr_or(GameConstants_XiangXiang.CHR_THIRTEEN_HONG_PAI);
				}
				if (hong_pai_count == 1) {
					chr.opr_or(GameConstants_XiangXiang.CHR_ONE_HONG);
				}
				if (hei_pai_count == all_cards_count) {
					chr.opr_or(GameConstants_XiangXiang.CHR_ALL_HEI);
				}
				PlayerStatus curPlayerStatus = table._playerStatus[_seat_index];
				curPlayerStatus.reset();
				...
				curPlayerStatus.add_action(GameConstants_XiangXiang.WIK_ZI_MO);
				curPlayerStatus.add_zi_mo(_center_card, _seat_index);
				return handler_operate_card(table, _seat_index, GameConstants_XiangXiang.WIK_ZI_MO, _center_card, 0);
				// curPlayerStatus.add_action(GameConstants_XiangXiang.WIK_NULL);
				// curPlayerStatus.add_pass(_center_card, _seat_index);
				// if (curPlayerStatus.has_action()) {
				// curPlayerStatus.set_status(GameConstants_XiangXiang.Player_Status_OPR_CARD);//
				// // 操作状态
				// table.operate_player_action(_seat_index, false);
				// return true;
				// }
	...
	public boolean handler_player_be_in_room(XiangXiangHHTable table, int seat_index) {
		...
		// _player_action[seat_index] : MJGameConstants_XiangXiang.WIK_NULL);
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < 55; j++) {
				if (table._logic.is_magic_card(table.GRR._discard_cards[i][j])) {
					// 癞子
					int_array.addItem(table.GRR._discard_cards[i][j] + GameConstants_XiangXiang.CARD_ESPECIAL_TYPE_HUN);
				} else {
			...
			for (int j = 0; j < GameConstants_XiangXiang.MAX_WEAVE_HH; j++) {
				WeaveItemResponse.Builder weaveItem_item = WeaveItemResponse.newBuilder();
				if (seat_index != i) {
					if ((table.GRR._weave_items[i][j].weave_kind == GameConstants_XiangXiang.WIK_GANG
							|| table.GRR._weave_items[i][j].weave_kind == GameConstants_XiangXiang.WIK_ZHAO)
		...
		for (int i = 0; i < GameConstants_XiangXiang.MAX_HH_COUNT; i++) {
		...
		// 效果
		table.operate_effect_action(_seat_index, GameConstants_XiangXiang.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { _action }, 1, seat_index);

		if (table._playerStatus[seat_index].has_action() && table._playerStatus[seat_index].is_respone() == false) {
			table.operate_player_action(seat_index, false);
		}

		if (!table._xian_ming_zhao[seat_index]) {
			table.cannot_outcard(seat_index, 0, 0, true);
		}
