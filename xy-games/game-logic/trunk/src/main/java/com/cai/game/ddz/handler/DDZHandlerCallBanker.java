...
public class DDZHandlerCallBanker<T extends DDZTable> extends DDZHandler<T> {
	...
	public boolean handler_call_banker(T table, int seat_index, int call_banker, int qiang_bangker) {
		...
		if (table.has_rule(GameConstants.GAME_RULE_JIAO_DI_ZHU)) {
			...
			if (call_banker > 0) {
				...
				// 出牌操作延迟
				GameSchedule.put(new DDZOutCardHandleRunnable(table.getRoom_id(), table.GRR._banker_player, table,
						qiang_player, call_player, qiang_action, call_action, true, true), 1000, TimeUnit.MILLISECONDS);
				return true;
			}
			call_action = 0;
			// 判断是否还有人没有轮到叫地主操作
			...
			if (call_banker == 3) {
				...
				if (!bhave_no_call) {
					...
					if (table._banker_select == GameConstants.INVALID_SEAT) {
						// 没有玩家叫地主
						GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._banker_select,
