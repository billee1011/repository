...
public abstract class GZPHandler {

	public abstract void exe(GZPTable table);

	// 准备
	public boolean handler_player_ready(GZPTable table, int seat_index) {
	...
	public boolean handler_release_room(GZPTable table, Player player, int opr_code) {
		return table.handler_release_room(player, opr_code);
	}

	public boolean handler_audio_chat(GZPTable table, Player player, com.google.protobuf.ByteString chat, int l,
			float audio_len) {
		return false;
	}
