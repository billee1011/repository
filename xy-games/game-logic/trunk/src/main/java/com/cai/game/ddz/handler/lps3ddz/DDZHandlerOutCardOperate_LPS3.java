...
import protobuf.clazz.lpsddz.lpsDdzRsp.Effect_Action_lps;
import protobuf.clazz.lpsddz.lpsDdzRsp.TableResponseDDZ_lps;

public class DDZHandlerOutCardOperate_LPS3 extends DDZHandlerOutCardOperate<DDZ_LPS3_Table> {
	@Override
	public void exe(DDZ_LPS3_Table table) {
		...
		if (cbCardType == GameConstants.DDZ_CT_BOMB_CARD || cbCardType == GameConstants.DDZ_CT_MISSILE_CARD) {
			table._boom_count++;
			table.m_player_boom[_out_card_player]++;
			//table._boom_num[_out_card_player]++;
			if (table.has_rule(GameConstants.GAME_RUL_LIMIT_24_LPS3)) {
				int game_times = table._times * 2;
				table._times = game_times >= 24 ? 24 : game_times;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					int usertimes = table._user_times[i] * 2 ;
					table._user_times[i] = usertimes >= 24 ? 24 : usertimes;
				}
			} else if (table.has_rule(GameConstants.GAME_RUL_LIMIT_48_LPS3)) {
				int game_times = table._times * 2;
				table._times = game_times >= 48 ? 48 : game_times;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					int usertimes = table._user_times[i] *= 2;
					table._user_times[i] = usertimes >= 48 ? 48 : usertimes;
				}
			} else if (table.has_rule(GameConstants.GAME_RUL_LIMIT_96_LPS3)) {
				int game_times = table._times * 2;
				table._times = game_times >= 96 ? 96 : game_times;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {				
					int usertimes = table._user_times[i] *= 2;
					table._user_times[i] = usertimes >= 96 ? 96 : usertimes;
				}
			}else if (table.has_rule(GameConstants.GAME_RUL_LIMIT_192_LPS3)) {
				int game_times = table._times * 2;
				table._times = game_times >= 192 ? 192 : game_times;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					int usertimes = table._user_times[i] *= 2;
					table._user_times[i] = usertimes >= 192 ? 192 : usertimes;
				}
			}else if(table.has_rule(GameConstants.GAME_RUL_LIMIT_NOT_LPS3)){
				table._times *= 2;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					table._user_times[i] *= 2;
				}
			}
			table.send_last_card(false);
			// 比赛场更新数据
		...
		int countindex = table.m_discard_count;
		for (int i = 0; i < this._out_card_count; i++) {
			...
			table.m_discard[countindex + i] = this._out_cards_data[i];
		}
		table.m_discard_count +=  _out_card_count;
		...
		if (pre_turn_out_type > GameConstants.DDZ_CT_THREE
			...
			Effect_Action_lps.Builder effect_action = Effect_Action_lps.newBuilder();
		...
		table._auto_out_card_scheduled = GameSchedule.put(
				new DDZAutoOutCardRunnable(table.getRoom_id(), table._current_player, table), delay, TimeUnit.SECONDS);
	...
	public int adjust_out_card_right(DDZ_LPS3_Table table) {
		...
		//不能四带二，四带两对
		if(table.has_rule(GameConstants.GAME_RULE_NOT_FOUR_TO_TWO_LPS3) && 
				(cbCardType == GameConstants.DDZ_CT_FOUR_TAKE_ONE || cbCardType == GameConstants.DDZ_CT_FOUR_TAKE_TWO) ){
			table.send_error_notify(_out_card_player, 2, "请选择正确的牌型!");
	...
	// 玩家不出
	public void no_out_card(DDZ_LPS3_Table table) {
