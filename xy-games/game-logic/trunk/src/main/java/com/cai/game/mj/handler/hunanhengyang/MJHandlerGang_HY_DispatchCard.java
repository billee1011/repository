...
public class MJHandlerGang_HY_DispatchCard extends AbstractMJHandler<MJTable_HY> {
    ...
    public MJHandlerGang_HY_DispatchCard() {
    ...
    public void exe(MJTable_HY table) {
        ...
        // 补张是牌型报听出骰子动画
        if (table.is_hy_ting_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
        ...
        for (int i = 0; i < GameConstants.CS_GANG_DRAW_COUNT; i++) {
            ...
            /*
             * if(table.DEBUG_CARDS_MODE){ if(i==0)bu_card=0x03; if(i==1)bu_card=0x35; }
             */
            ...
            for (int k = 0; k < table.getTablePlayerNumber(); k++) {
                ...
                if (k == _seat_index) {
                    ...
                    int action = table.analyse_chi_hu_card(table.GRR._cards_index[k], table.GRR._weave_items[k],
                            table.GRR._weave_count[k], bu_card, chr, GameConstants.HU_CARD_TYPE_ZIMO, k, false);// 自摸
                    ...
                    // 如果牌堆还有牌，判断能不能杠
                    if (table.GRR._left_card_count > (table.get_liu_niao() + 1)) {
                        ...
                        if (cbActionMask != GameConstants.WIK_NULL) {// 有杠
                            for (int gc = 0; gc < m_gangCardResult.cbCardCount; gc++) {
                                ...
                                can_gang = table.is_hy_ting_card(table.GRR._cards_index[k], table.GRR._weave_items[k],
                                        table.GRR._weave_count[k], k);
    ...
    public boolean handler_operate_card(MJTable_HY table, int seat_index, int operate_code, int operate_card) {
        ...
        case GameConstants.WIK_ZI_MO: // 自摸
        {
            ...
            // 结束后设置鸟牌MJGameConstants.INVALID_VALUE
            ...
            // table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], pWeaveItem,
        ...
        case GameConstants.WIK_CHI_HU: // 胡
        {
            ...
            if (jie_pao_count > 1) {
                table._cur_banker = _seat_index;
                // table.set_niao_card_hy(table.GRR._banker_player,GameConstants.INVALID_VALUE,true,0,true);
            } else {
    ...
    private void show_shai_zi(MJTable_HY table, int seat_index) {
    ...
    private void exe_chi_peng(MJTable_HY table, int target_player, int target_action, int target_card) {
