...
import protobuf.clazz.jxklox.jxklRsp.ButtonOperateJxklox;
import protobuf.clazz.jxklox.jxklRsp.BuyJettonResultJxklox;
import protobuf.clazz.jxklox.jxklRsp.GameStartJxklox;
import protobuf.clazz.jxklox.jxklRsp.JettonResultJxklox;
import protobuf.clazz.jxklox.jxklRsp.OpenCardJxklox;
import protobuf.clazz.jxklox.jxklRsp.Opreate_Jxklox_Request;
import protobuf.clazz.jxklox.jxklRsp.PlayerResultJxklox;
import protobuf.clazz.jxklox.jxklRsp.PukeGameEndJxklox;
import protobuf.clazz.jxklox.jxklRsp.RoomPlayerResponseJxklox;
import protobuf.clazz.jxklox.jxklRsp.SelectdBankerJxklox;
import protobuf.clazz.jxklox.jxklRsp.SelectdBankerResultJxklox;
import protobuf.clazz.jxklox.jxklRsp.SendCardGPJxklox;
import protobuf.clazz.jxklox.jxklRsp.SendCardJxklox;
import protobuf.clazz.jxklox.jxklRsp.TableResponseJxklox;

///////////////////////////////////////////////////////////////////////////////////////////////
public class JXKLOXTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	private static Logger logger = Logger.getLogger(JXKLOXTable.class);
	...
	public boolean _is_tui_zhu[]; // 推注操作
	...
	public JXKLOXGameLogic _logic = null;
	...
	public int _buy_jetton_max; // 最大买码个数
	public int _buy_jetton_count[]; // 买码筹码
	public int _buy_jetton_area[]; // 买码区域
	public int _is_buy_jetton[]; // 是否操作买码
	...
	public int _banker_max_times; // 最大庄家倍数
	...
	public int _gp_card; // 公牌
	public int _next_banker; // 下一局的庄家
	public int _banker_count; // 做庄次数
	public boolean is_trun_banker;// 是否轮庄
	public int _buy_all_jetton[]; // 买码总注
	...
	public JXKLOXTable() {
		...
		_gp_card = GameConstants.INVALID_CARD;
		_is_bu_banker = new boolean[getTablePlayerNumber()];
		_jetton_score = new int[getTablePlayerNumber()][GameConstants.KLOX_MAX_JETTON_COUNT];
		_jetton_count = new int[GameConstants.KLOX_MAX_JETTON_COUNT];
		...
		_is_tui_zhu = new boolean[getTablePlayerNumber()];
		Arrays.fill(_is_tui_zhu, false);
		Arrays.fill(_can_tuizhu_player, -1);
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			_str_ox_value[i] = "";
			_jetton_score[i] = new int[GameConstants.KLOX_MAX_JETTON_COUNT];
		}
		...
		_buy_all_jetton = new int[getTablePlayerNumber()];
		Arrays.fill(_buy_all_jetton, 0);
		_owner_seat = -1;
	}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_qie_card = 0; // 切的牌
		_all_card_len = 0; // 总牌数
		_buy_jetton_max = 0; // 最大买码个数
		_buy_jetton_count = new int[this.getTablePlayerNumber()]; // 买码筹码
		_buy_jetton_area = new int[this.getTablePlayerNumber()]; // 买码区域
		_is_buy_jetton = new int[this.getTablePlayerNumber()]; // 是否操作买码
		Arrays.fill(_buy_jetton_count, 0);
		Arrays.fill(_is_buy_jetton, -2);
		Arrays.fill(_buy_jetton_area, -2);
		_player_ready = new int[this.getTablePlayerNumber()];
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			_player_ready[i] = 0;
		}
		...
		if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_HAND_START) == 1)
			this.setMinPlayerCount(2);
		if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_FOUR_START) == 1)
			this.setMinPlayerCount(4);
		if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_FIVE_START) == 1)
			this.setMinPlayerCount(5);
		if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_SIX_START) == 1)
			this.setMinPlayerCount(6);
		if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_SEVEN_START) == 1)
			this.setMinPlayerCount(7);
		if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_EIGHT_START) == 1)
	...
	protected boolean on_handler_game_start() {
		...
		_cur_jetton_round = 0;
		//
		if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_LAI_ZI) == 1) {
		...
		if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_ROB_FOUR) == 1)
			this._banker_max_times = 4;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_ROB_THREE) == 1)
			this._banker_max_times = 3;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_ROB_TWO) == 1)
			this._banker_max_times = 2;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_ROB_ONE) == 1)
			this._banker_max_times = 1;
		else
			this._banker_max_times = 1;

		if (has_rule(GameConstants.GAME_RULE_KLJX_ONE_TWO)) {
			_jetton_count_max = 2;
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			this._buy_jetton_max = 2;
			this._buy_jetton_count[0] = 1;
			this._buy_jetton_count[1] = 2;

		}
		if (has_rule(GameConstants.GAME_RULE_KLJX_TWO_FOUR)) {
			_jetton_count_max = 2;
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				Arrays.fill(this._jetton_score[i], 0);
				this._jetton_count[i] = _jetton_count_max;
			...
			this._buy_jetton_max = 2;
			this._buy_jetton_count[0] = 2;
			this._buy_jetton_count[1] = 4;
		}
		if (has_rule(GameConstants.GAME_RULE_KLJX_FOUR_EIGHT)) {
			_jetton_count_max = 2;
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				Arrays.fill(this._jetton_score[i], 0);
				this._jetton_count[i] = _jetton_count_max;
			...
			this._buy_jetton_max = 2;
			this._buy_jetton_count[0] = 4;
			this._buy_jetton_count[1] = 8;
		}
		if (has_rule(GameConstants.GAME_RULE_KLJX_THREE_SIX)) {
			_jetton_count_max = 2;
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				Arrays.fill(this._jetton_score[i], 0);
				this._jetton_count[i] = _jetton_count_max;
				for (int j = 0; j < this._jetton_count[i]; j++) {
					this._jetton_score[i][j] = (1 + j) * 3;
				}
			}
			this._buy_jetton_max = 2;
			this._buy_jetton_count[0] = 3;
			this._buy_jetton_count[1] = 6;
		}
		if (has_rule(GameConstants.GAME_RULE_KLJX_FIVE_TEN)) {
			_jetton_count_max = 2;
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				Arrays.fill(this._jetton_score[i], 0);
				this._jetton_count[i] = _jetton_count_max;
			...
			this._buy_jetton_max = 2;
			this._buy_jetton_count[0] = 5;
			this._buy_jetton_count[1] = 10;
		}

		if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_TUI_ZHU_FIVE) == 1)
			this._tui_zhu_times = 20;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_TUI_ZHU_FOUR) == 1)
			this._tui_zhu_times = 15;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_TUI_ZHU_THREE) == 1)
			this._tui_zhu_times = 10;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_TUI_ZHU_TWO) == 1)
			this._tui_zhu_times = 5;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_TUI_ZHU) == 1)
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this._can_tuizhu_player[i] > 0) {
				...
				this._is_tui_zhu[i] = true;
			} else
				this._is_tui_zhu[i] = false;
		}
		return game_start_Jxklox();
	}

	public void set_jetton_button() {
		this._buy_jetton_max = 2;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (i == this._cur_banker)
				continue;
			if (this._player_call_banker[i] == this._player_call_banker[this._cur_banker]) {
				Arrays.fill(this._jetton_score[i], 0);
				this._jetton_count[i] = 0;
				this._jetton_score[i][this._jetton_count[i]++] = this._buy_jetton_count[1];
				if (this._can_tuizhu_player[i] > 0) {
					this._jetton_score[i][this._jetton_count[i]++] = this._can_tuizhu_player[i];
					this._is_tui_zhu[i] = true;
				} else
				...
				Arrays.fill(this._jetton_score[i], 0);
				this._jetton_count[i] = _buy_jetton_max;
				this._jetton_score[i][0] = this._buy_jetton_count[0];
				this._jetton_score[i][1] = this._buy_jetton_count[1];
				this._is_tui_zhu[i] = false;
			}
	...
	// 斗板牛开始
	public boolean game_start_Jxklox() {
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			...
			roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_GAME_START);
			roomResponse.setGameStatus(this._game_status);
			// 发送数据
			GameStartJxklox.Builder gamestart = GameStartJxklox.newBuilder();
	...
	public boolean update_button(int opr_type, int seat_index, int display_time, boolean is_grr) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_UPDATE_BTN);
		ButtonOperateJxklox.Builder button_operate = ButtonOperateJxklox.newBuilder();
		...
		if (opr_type == 1 && seat_index != _cur_banker) {
			...
			if (this._is_tui_zhu[seat_index] == true)
				button_operate.setIsHaveTuiZhu(true);
			else
				button_operate.setIsHaveTuiZhu(false);
			if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_BUY_JETTON) == 1 && this._is_buy_jetton[seat_index] == -2) {
				Int32ArrayResponse.Builder buy_jetton = Int32ArrayResponse.newBuilder();
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					buy_jetton.clear();
					if (this._player_status[i] == false) {
						button_operate.addBuyJetton(i, buy_jetton);
						continue;
					}
					...
					if (i == this._cur_banker || seat_index == i) {
						button_operate.addBuyJetton(i, buy_jetton);
						continue;
					}
					if (this._buy_jetton_area[seat_index] == -2) {
						for (int j = 0; j < this._buy_jetton_max; j++) {
							buy_jetton.addItem(this._buy_jetton_count[j]);
						}
					}
		...
		if (opr_type == 2) {
			if (this.is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX) && seat_index == this._cur_banker) {
				for (int j = 0; j < 2; j++) {
					button_operate.addButton(j);
				}
			} else if (this.is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX) == false) {
				for (int j = 0; j < 2; j++) {
					button_operate.addButton(j);
				}
			}
			...
			if (is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX)) {
				button_operate.setGpCard(GameConstants.BLACK_CARD);
			}
	...
	public boolean send_call_banker_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_ROBOT_BANKER);
		SelectdBankerJxklox.Builder robot_banker = SelectdBankerJxklox.newBuilder();
	...
	public boolean set_banker_to_room() {
		...
		this._player_call_banker[this._cur_banker] = this._banker_times;
		this._can_tuizhu_player[_cur_banker] = 0;
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_ROBOT_BANKER_RESULT);
		SelectdBankerResultJxklox.Builder robot_banker = SelectdBankerResultJxklox.newBuilder();
	...
	public boolean call_banker(int seat_index, int sub_index) {
		...
		if (sub_index < 0 || sub_index > this._banker_max_times) {
			log_error("叫庄下标不对" + this._jetton_count);
			return true;
		}
		...
		if (flag == false) {
			...
			set_banker_to_room();
			if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_LIMIT_LOW_JETTON) == 1)
				set_jetton_button();
	...
	public boolean result_buy(int seat_index, int sub_index, int buy_index) {
		if (this._game_status != GameConstants.GS_OX_ADD_JETTON)
			return true;
		if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_BUY_JETTON) != 1)
			return true;
		if (sub_index != -1 && buy_index != -1) {
			if (this._player_status[buy_index] == false || buy_index == seat_index || buy_index == this._cur_banker) {
				log_error("买区域不对" + buy_index);
				return true;
			}
			if (this._is_buy_jetton[seat_index] != -2) {
				log_error("您已经买码了" + this._current_player);
				return true;
			}
			this._buy_jetton_area[seat_index] = buy_index;
			this._is_buy_jetton[seat_index] = this._buy_jetton_count[sub_index];
			this._buy_all_jetton[buy_index] += this._is_buy_jetton[seat_index];
		} else {
			this._buy_jetton_area[seat_index] = -1;
			this._is_buy_jetton[seat_index] = 0;
		}

		send_buy_jetton_to_room(seat_index);
	...
	public boolean result_jetton(int seat_index, int sub_index) {
		...
		if (this._is_tui_zhu[seat_index] == true && this._jetton_score[seat_index][sub_index] > this._buy_jetton_count[1]) {
	...
	public void send_jetton_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_JETTON);
		JettonResultJxklox.Builder jetton_result = JettonResultJxklox.newBuilder();
	...
	public void send_buy_jetton_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_BUY_JETTON);
		BuyJettonResultJxklox.Builder jetton_result = BuyJettonResultJxklox.newBuilder();
		jetton_result.setBuyJettonSeat(seat_index);
		jetton_result.setBuyjettonScore(this._is_buy_jetton[seat_index]);
		jetton_result.setBuySeatIndex(this._buy_jetton_area[seat_index]);
		if (this._buy_jetton_area[seat_index] != -1) {
			jetton_result.setBuyAllJetton(this._buy_all_jetton[this._buy_jetton_area[seat_index]]);
		}
		roomResponse.setCommResponse(PBUtil.toByteString(jetton_result));
		GRR.add_room_response(roomResponse);
		this.send_response_to_room(roomResponse);
	...
	public boolean send_card_gp(boolean is_display) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_SEND_GP);
		SendCardGPJxklox.Builder send_card_gp = SendCardGPJxklox.newBuilder();
		if (is_display == false)
			send_card_gp.setGpCard(GameConstants.BLACK_CARD);
		else
			send_card_gp.setGpCard(this._gp_card);
		roomResponse.setCommResponse(PBUtil.toByteString(send_card_gp));
		GRR.add_room_response(roomResponse);
		this.send_response_to_room(roomResponse);
		this.observers().sendAll(roomResponse);
		return true;
	...
	public boolean send_card(int card_count, int opr_type) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_SEND_CARD);
		SendCardJxklox.Builder send_card = SendCardJxklox.newBuilder();
			...
			for (int k = 0; k < this.getTablePlayerNumber(); k++) {
				...
				if (this._player_status[k] == false) {
					...
					if (card_count == GameConstants.OX_MAX_CARD_COUNT) {
						if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_JETTON_ONE_TIME) == 1)
							this._player_times[k] = _logic.get_times_one(GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
						else
							this._player_times[k] = _logic.get_times_two(GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);

						this._ox_value[k] = _logic.get_card_type(GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
						...
						_logic.get_ox_card(temp_cards_data, GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
	...
	public boolean open_card(int seat_index) {
		...
		if (this.is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX) && seat_index != this._cur_banker) {
			log_error("公牌牛牛，只有庄家可以开牌" + seat_index);
			return true;
		}
		this._open_card_player[seat_index] = true;
		if (this.is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX)) {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				int loop = (seat_index + i) % this.getTablePlayerNumber();
				if (this._player_status[loop] == false)
					continue;
				this._open_card_player[loop] = true;
				_logic.get_ox_card(GRR._cards_data[loop], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
				if (this.is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX)) {
					if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_JETTON_ONE_TIME) == 1)
						this._player_times[loop] = _logic.get_times_one(GRR._cards_data[loop], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
					else
						this._player_times[loop] = _logic.get_times_two(GRR._cards_data[loop], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);

					this._ox_value[loop] = _logic.get_card_type(GRR._cards_data[loop], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);

				}
				send_open_to_room(loop);
			}
		} else {
			_logic.get_ox_card(GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
			if (this.is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX)) {
				if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_JETTON_ONE_TIME) == 1)
					this._player_times[seat_index] = _logic.get_times_one(GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
				else
					this._player_times[seat_index] = _logic.get_times_two(GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);

				this._ox_value[seat_index] = _logic.get_card_type(GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
		...
		if (flag == false) {
			if (is_mj_type(GameConstants.GAME_TYPE_KLJX_NNSZ)) {
				int max_seat_index = process_tbox_calulate_end();
				int card_type = _logic.get_card_type(this.GRR._cards_data[max_seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
				if (card_type >= GameConstants.KLOX_JX_XIAO_TEN && card_type <= GameConstants.KLOX_JX_TEN)
					this._next_banker = max_seat_index;
			}

			process_ox_calulate_end(this._cur_banker);
			all_game_end_record();
			int delay = 2;
	...
	public void process_ox_calulate_end(int max_seat_index) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this._player_status[i] != true)
				continue;
			boolean first_ox = _logic.get_ox_card(GRR._cards_data[max_seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
			boolean next_ox = _logic.get_ox_card(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
			boolean action = _logic.compare_card(GRR._cards_data[max_seat_index], GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, first_ox,
					next_ox, this.ruleMap);
			// 获取点数
			...
			if (action == true) {
				if (this.getRuleValue(GameConstants.GAME_RULE_KLJX_JETTON_ONE_TIME) == 1)
					calculate_score = _logic.get_times_one(GRR._cards_data[max_seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
				else
					calculate_score = _logic.get_times_two(GRR._cards_data[max_seat_index], GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);

				lChiHuScore = calculate_score * _jetton_player[i] * this._banker_times;
				for (int j = 0; j < this.getTablePlayerNumber(); j++) {
					if (this._is_buy_jetton[j] == 0)
						continue;
					if (this._buy_jetton_area[j] != i)
						continue;

					buy_score = calculate_score * this._is_buy_jetton[j] * this._banker_times;
					GRR._game_score[j] -= buy_score;
					GRR._game_score[max_seat_index] += buy_score;
			...
			GRR._game_score[max_seat_index] += lChiHuScore;
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				if (this._can_tuizhu_player[i] == 0) {
					if (GRR._game_score[i] > 0) {
						int temp = 0;
						if ((int) GRR._game_score[i] + this._jetton_player[i] > this._buy_jetton_count[0] * this._tui_zhu_times)
							temp = this._buy_jetton_count[0] * this._tui_zhu_times;
						else
							temp = (int) GRR._game_score[i] + this._jetton_player[i];
						if (this._buy_jetton_count[1] >= temp)
	...
	public boolean send_open_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_OPEN_CARD);
		OpenCardJxklox.Builder open_card = OpenCardJxklox.newBuilder();
		open_card.setOpenCard(_open_card_player[seat_index]);
		open_card.setSeatIndex(seat_index);
		int count = GameConstants.OX_MAX_CARD_COUNT;
		// if(is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX))
		// count = 4;
		...
		if (is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX))
			open_card.setGpCard(this._gp_card);
	...
	private void shuffle(int repertory_card[], int card_cards[]) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX)) {
			this._gp_card = repertory_card[GRR._left_card_count++];
			for (int i = 0; i < this.getTablePlayerNumber(); i++)
				GRR._cards_data[i][count] = this._gp_card;
	...
	private void test_cards() {
		...
		// 0x01,0x11,0x03,0x02,0x21,0x17,0x08,0x29,0x3D,0x2C
		// };
	...
	public RoomPlayerResponseJxklox.Builder load_player_info_data(int seat_index) {
		Player rplayer = this.get_players()[seat_index];
		if (rplayer == null)
			return null;
		RoomPlayerResponseJxklox.Builder room_player = RoomPlayerResponseJxklox.newBuilder();
		room_player.setAccountId(rplayer.getAccount_id());
		room_player.setHeadImgUrl(rplayer.getAccount_icon());
		room_player.setIp(rplayer.getAccount_ip());
		room_player.setUserName(rplayer.getNick_name());
	...
	public void load_player_info_data_game_start(GameStartJxklox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_game_end(PukeGameEndJxklox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_reconnect(TableResponseJxklox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void testRealyCard(int[] realyCards) {
		...
		int card_count = GameConstants.OX_MAX_CARD_COUNT;
		for (int i = 0; i < count; i++) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX)) {
			this._gp_card = realyCards[k];
			for (int i = 0; i < this.getTablePlayerNumber(); i++)
				GRR._cards_data[i][card_count - 1] = this._gp_card;
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		ret = this.handler_game_finish_Jxklox(seat_index, reason);

		return ret;
	}

	public boolean handler_game_finish_Jxklox(int seat_index, int reason) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_GAME_END);
		GameEndResponse.Builder game_end = GameEndResponse.newBuilder();
		PukeGameEndJxklox.Builder game_end_Jxklox = PukeGameEndJxklox.newBuilder();
		RoomInfo.Builder room_info = getRoomInfo();
		game_end_Jxklox.setRoomInfo(room_info);
		game_end.setRoomInfo(getRoomInfo());

		if (reason == GameConstants.Game_End_NORMAL) {
		}
		this.load_player_info_data_game_end(game_end_Jxklox);
		game_end_Jxklox.setGameRound(_game_round);
		game_end_Jxklox.setCurRound(_cur_round);
		game_end_Jxklox.setDisplayTime(this._idi_ready_card_time);
		if (GRR != null) {
			this.operate_player_data();
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				game_end.addGameScore(GRR._game_score[i]);
				game_end_Jxklox.addEndScore((int) GRR._game_score[i]);
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
				for (int j = 0; j < GameConstants.OX_MAX_CARD_COUNT; j++) {
					cards_card.addItem(GRR._cards_data[i][j]);
				}
				game_end_Jxklox.addCardsData(cards_card);
				game_end_Jxklox.addStrOxValue(this._str_ox_value[i]);
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if (_cur_round >= _game_round) {// 局数到了
				...
				game_end_Jxklox.setPlayerResult(this.process_player_result_Jxklox(reason));
		...
		game_end_Jxklox.setWinner(seat_index);
		game_end_Jxklox.setReason(real_reason);
		////////////////////////////////////////////////////////////////////// 得分总的
		game_end.setEndType(real_reason);
		game_end.setGameRound(_game_round);
		game_end.setCurRound(_cur_round);
		game_end.setCommResponse(PBUtil.toByteString(game_end_Jxklox));
	...
	public boolean handler_enter_room(Player player) {
		...
		if (player.getAccount_id() != this.getRoom_owner_account_id() && has_rule(GameConstants.GAME_RULE_KLJX_MIN_FORBID_JOIN)) {
		...
		if (has_rule(GameConstants.GAME_RULE_KLJX_HAND_START)) {
	...
	public void is_game_start() {
		...
		if ((_player_count >= sit_max) && (_player_count == _cur_count)) {
			if (this._cur_round == 0 && this.getRuleValue(GameConstants.GAME_RULE_KLJX_HAND_START) == 1) {
				this.is_game_start = 1;
				control_game_start();
			} else
				handler_game_start();
		} else if ((this._cur_round == 0) && this.getRuleValue(GameConstants.GAME_RULE_KLJX_HAND_START) == 1) {
			this.is_game_start = 0;
			control_game_start();
		}
		// if((this._cur_round == 0 )&&(_player_count >=
		// sit_max)&&(_player_count ==
		// _cur_count)&&this.getRuleValue(GameConstants.GAME_RULE_KLJX_HAND_START)==1)
		// {
		// this.is_game_start = 1;
		// control_game_start();
		// }
		// else if((this._cur_round == 0
		// )&&this.getRuleValue(GameConstants.GAME_RULE_KLJX_HAND_START)==1)
		// {
		// this.is_game_start = 0;
		// control_game_start();
		// }
		// else if ((_player_count >=sit_max)&&(_player_count ==
		// _cur_count)&&(this.is_game_start ==
		// 2||this.getRuleValue(GameConstants.GAME_RULE_KLJX_HAND_START)!=1))
		// {
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status) && this.get_players()[seat_index] != null) {
			...
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			roomResponse.setType(MsgConstants.RESPONSE_KLOX_JX_RECONNECT_DATA);

			TableResponseJxklox.Builder tableResponse_Jxklox = TableResponseJxklox.newBuilder();
			load_player_info_data_reconnect(tableResponse_Jxklox);
			RoomInfo.Builder room_info = getRoomInfo();
			tableResponse_Jxklox.setRoomInfo(room_info);

			tableResponse_Jxklox.setBankerPlayer(_cur_banker);

			tableResponse_Jxklox.setPlayerStatus(this._player_status[seat_index]);
			int display_time = _cur_operate_time - ((int) (System.currentTimeMillis() / 1000L) - _operate_start_time);
			tableResponse_Jxklox.setDisplayTime(display_time);
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				tableResponse_Jxklox.addCallButton(this._player_call_banker[i]);

				tableResponse_Jxklox.addJettonScore(this._jetton_player[i]);

				tableResponse_Jxklox.addOpenCard(this._open_card_player[i]);

				tableResponse_Jxklox.addBuyAllJetton(this._buy_all_jetton[i]);
				if (this._open_card_player[i] == true || i == seat_index) {
					tableResponse_Jxklox.addOxValue(this._ox_value[i]);
					tableResponse_Jxklox.addTimes(this._player_times[i]);
					if (i == seat_index && this._open_card_player[i] == true) {
						if (is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX))
							tableResponse_Jxklox.setGpCard(this._gp_card);
					} else if (_game_status == GameConstants.GS_OX_OPEN_CARD)
						tableResponse_Jxklox.setGpCard(GameConstants.BLACK_CARD);
					...
					_logic.get_ox_card(temp_cards_data, GameConstants.OX_MAX_CARD_COUNT, this.ruleMap);
					tableResponse_Jxklox.clearSortCard();
					for (int index = 0; index < GameConstants.OX_MAX_CARD_COUNT; index++) {
						tableResponse_Jxklox.addSortCard(temp_cards_data[index]);
					}
				} else {
					tableResponse_Jxklox.addOxValue(-1);
					tableResponse_Jxklox.addTimes(0);
				}

			}
			if (seat_index == this._cur_banker) {
				tableResponse_Jxklox.setCurBuyArea(-1);
				tableResponse_Jxklox.setCurBuyScore(-1);
			} else {
				tableResponse_Jxklox.setCurBuyArea(this._buy_jetton_area[seat_index]);
				tableResponse_Jxklox.setCurBuyScore(this._is_buy_jetton[seat_index]);
			}

			tableResponse_Jxklox.setSceneStatus(_game_status);
			if (_game_status == GameConstants.GS_OX_CALL_BANKER) {

				tableResponse_Jxklox.setBankerPlayer(-1);
				int card_count = 4;
				if (is_mj_type(GameConstants.GAME_TYPE_KLJX_SIX_MPQZ) || is_mj_type(GameConstants.GAME_TYPE_KLJX_EIGHT_MPQZ)
						|| is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX)) {
					for (int k = 0; k < this.getTablePlayerNumber(); k++) {
						Int32ArrayResponse.Builder cards = Int32ArrayResponse.newBuilder();
						if (this._player_status[k] == false) {
							for (int j = 0; j < card_count; j++) {
						...
						tableResponse_Jxklox.addCardsData(k, cards);
					}
				}
				...
				if (is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX)) {
					...
					for (int k = 0; k < this.getTablePlayerNumber(); k++) {
						...
						if (this._player_status[k] == false) {
							...
							if (k == seat_index && this._open_card_player[k] == false) {
								for (int j = 0; j < card_count; j++) {
									cards.addItem(GRR._cards_data[k][j]);
			...
			roomResponse.setCommResponse(PBUtil.toByteString(tableResponse_Jxklox));
			this.send_response_to_player(seat_index, roomResponse);
	...
	public boolean table_response_observer(Player player) {
		...
		RoomInfo.Builder room_info = getRoomInfo();
		tableResponse_Jxklox.setRoomInfo(room_info);
		tableResponse_Jxklox.setPlayerStatus(false);
		tableResponse_Jxklox.setBankerPlayer(_cur_banker);
		int display_time = _cur_operate_time - ((int) (System.currentTimeMillis() / 1000L) - _operate_start_time);
		tableResponse_Jxklox.setDisplayTime(display_time);
		tableResponse_Jxklox.setCurBuyArea(-2);
		tableResponse_Jxklox.setCurBuyScore(-2);
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			tableResponse_Jxklox.addCallButton(this._player_call_banker[i]);
			tableResponse_Jxklox.addJettonScore(this._jetton_player[i]);
			tableResponse_Jxklox.addOpenCard(this._open_card_player[i]);
			...
			if (is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX)) {
				...
				for (int k = 0; k < this.getTablePlayerNumber(); k++) {
					...
					if (this._player_status[k] == false) {
						...
						if (this._open_card_player[k] == false) {
							for (int j = 0; j < card_count; j++) {
								cards.addItem(GRR._cards_data[k][j]);
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		if (type == MsgConstants.REQUST_KLOXJX_OPERATE) {

			Opreate_Jxklox_Request req = PBUtil.toObject(room_rq, Opreate_Jxklox_Request.class);
			// 逻辑处理
			return handler_requst_opreate(seat_index, req.getOpreateType(), req.getAddJettonScore(), req.getBuyIndex());
		}
		return true;
	}

	// 1:r抢庄 2：下注 3：开牌
	public boolean handler_requst_opreate(int seat_index, int operate_type, int sub_index, int buy_index) {
		if (operate_type < 1 || operate_type > 5) {
		...
		case 4: {
			return send_card_gp(true);
		}
		case 5:
			return result_buy(seat_index, sub_index, buy_index);
		}
		return true;
	}
	...
	private PlayerResultJxklox.Builder process_player_result_Jxklox(int reason) {
		int win_idx = 0;
		float max_score = 0;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			int winner = -1;
		...
		PlayerResultJxklox.Builder player_result = PlayerResultJxklox.newBuilder();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean handler_request_trustee(int get_seat_index, boolean isTrustee, int Trustee_type) {
		...
		// this.set_trustee_timer(this._cur_game_timer,
		// GameConstants.OX_TRUESTEE_OPERATE_TIME, false);
		// else
		// {
	...
	public int getTablePlayerNumber() {
		if (is_mj_type(GameConstants.GAME_TYPE_KLJX_SIX_MPQZ) || is_mj_type(GameConstants.GAME_TYPE_KLJX_NNSZ)
				|| is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX) || is_mj_type(GameConstants.GAME_TYPE_KLJX_ZYQZ))
			return GameConstants.GAME_PLAYER_OX;
		else if (is_mj_type(GameConstants.GAME_TYPE_KLJX_EIGHT_MPQZ))
			return GameConstants.GAME_PLYER_EIGHT_OX;
		return GameConstants.GAME_PLYER_EIGHT_OX;
	}
	...
	public boolean animation_timer(int timer_id) {
		...
		case ID_TIMER_ANIMATION_START: {
			if (is_mj_type(GameConstants.GAME_TYPE_KLJX_NNSZ)) {
				this._banker_times = this._banker_max_times;
			...
			if (is_mj_type(GameConstants.GAME_TYPE_KLJX_ZYQZ)) {
				set_call_banker();

			} else if (is_mj_type(GameConstants.GAME_TYPE_KLJX_SIX_MPQZ) || is_mj_type(GameConstants.GAME_TYPE_KLJX_EIGHT_MPQZ)
					|| is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX)) {
				send_card(4, 0);
				set_call_banker();
		...
		case ID_TIMER_ANIMATION_ROBOT: {
			if (is_mj_type(GameConstants.GAME_TYPE_KLJX_DGOX) == false) {
				...
				// if(i == this._cur_banker)
				// continue;
				// if(this._banker_max_times >1
				// &&this._jetton_count_max>1&&this._banker_times>1)
				// {
				// if(this._player_call_banker[i] == this._banker_times)
				// for(int j = 1; j<this._jetton_count[i];j++)
				// {
				// this._jetton_score[i][j-1] = this._jetton_score[i][j];
				// }
				// this._jetton_count[i]--;
				// }
				// }
				//
				// }
