...
public class HandlerDispatchCard_HLKDD extends MJHandlerDispatchCard<Table_HLKDD> {
	...
	public HandlerDispatchCard_HLKDD() {
	...
	public void exe(Table_HLKDD table) {
		...
		// 判断是否是杠后抓牌,杠开花只算接杠
		int card_type = Constants_MJ_HLKDD.HU_CARD_TYPE_ZI_MO;
		// 检查牌型,听牌
		...
		for (int i = 0; i < card_type_count; i++) {
			...
			if (count > 0) {
				...
				if (table._playerStatus[_seat_index]._hu_out_card_ting_count[ting_count] > 0) {
					...
					// 没有报听检查打出的牌能否报听
					table.check_bao_ting(_seat_index, ting_count, i);
					///
					ting_count++;
		...
		if (ting_count > 0) {
			...
			if(!table.is_bao_ting[_seat_index] && table.baoTingOutCards.size() > 0){
		...
		if (curPlayerStatus.has_action()) {
			...
			if(table.is_bao_ting[_seat_index] || table.GRR._left_card_count == 0){
				GameSchedule.put(()->{
	...
	@Override
	public boolean handler_player_out_card(Table_HLKDD table, int seat_index, int card) {
	...
	public boolean handler_operate_card(Table_HLKDD table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_NULL) {
			/*if(table.has_rule(Constants_MJ_HLKDD.GAME_RULE_GUO_HU_ONLY_ZI_MO) && playerStatus.has_chi_hu()){
		...
		case GameConstants.WIK_BAO_TING:{
			...
			table.baoTingDiscard[seat_index] = table.GRR._discard_count[seat_index];
			_type = Constants_MJ_HLKDD.HU_CARD_TYPE_BAO_TING;
