...
public class HandlerYaoHaiDi_LX_CS_QY extends MJHandlerYaoHaiDi<Table_LX_CS_QY> {
	private static Logger logger = Logger.getLogger(HandlerYaoHaiDi_LX_CS_QY.class);
	...
	public void exe(Table_LX_CS_QY table) {
		...
		int action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
				table.GRR._weave_count[_seat_index], _send_card_data, chr, Constants_LX_CS_QY.HU_CARD_TYPE_ZI_MO, _seat_index);

		if (action != GameConstants.WIK_NULL) {
			curPlayerStatus.add_action(GameConstants.WIK_ZI_MO);
			curPlayerStatus.add_zi_mo(_send_card_data, _seat_index);

			chr.opr_or_jd_cs(Constants_LX_CS_QY.CHR_HAI_DI_LAO_YUE);
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (action != GameConstants.WIK_NULL) {
				...
				chr.opr_or_jd_cs(Constants_LX_CS_QY.CHR_HAI_DI_PAO);
