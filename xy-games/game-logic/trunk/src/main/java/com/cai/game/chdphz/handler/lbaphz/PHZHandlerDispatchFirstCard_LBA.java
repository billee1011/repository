...
public class PHZHandlerDispatchFirstCard_LBA extends PHZHandlerDispatchCard<CHDPHZTable> {
...
//		_send_card_data = 0x18;
		...
		for(int i = 0;i<table.getTablePlayerNumber();i++)
		{
			...
			table._da_pai_count[i]  = 0;
			table._xiao_pai_count[i] = 0;
			table._hong_pai_count[i] = 0;
			table._tuan_yuan_count[i] = 0; 
			for(int j = 0; j< GameConstants.MAX_HH_INDEX;j++)
			{
				...
				if(table._logic.color_hei(table._logic.switch_to_card_data(j))==true){
        			...
        			table._hong_pai_count[i] += table.GRR._cards_index[i][j];
        		}
				if(table._logic.get_card_color(table._logic.switch_to_card_data(j))>0)
				{
					table._da_pai_count[i] +=table.GRR._cards_index[i][j];
				}
				else 
				{
					table._xiao_pai_count[i] += table.GRR._cards_index[i][j];
				}
			...
			if((ti_count >= 3)||(sao_count >= 5))
			{
	        	...
	        	if(all_hu_xi>=15)
	        	{
	        		...
	        		all_cards_count = table._hong_pai_count[i] +hei_pai_count;
					if (card_type == GameConstants.HU_CARD_TYPE_ZIMO ) {
						if(_seat_index == i)
							chr.opr_or_long(GameConstants.CHR_TIAN_HU_CHD);
						else 
							chr.opr_or_long(GameConstants.CHR_DI_HU_CHD);
						chr.opr_or_long(GameConstants.CHR_TING_HU_CHD);
						chr.opr_or_long(GameConstants.CHR_ZI_MO);
					}

					if(table._hong_pai_count[i] == 1)
					{
					...
					{  
						table._tuan_yuan_count[i] =table._logic.calculate_tuan_count(table._hu_weave_items[i],table._hu_weave_count[i]);
						
						if(table._tuan_yuan_count[i] >0)
						{
					...
					{
						chr.opr_or_long(GameConstants.CHR_TUAN_YUAN_CHD);
					}

					if(table._huang_zhang_count >= 1){
						chr.opr_or_long(GameConstants.CHR_HUANG_FAN_CHD);
					}
//					if (table._logic.calculate_pengpeng_count(table._hu_weave_items[i], table._hu_weave_count[i])) {
//						chr.opr_or_long(GameConstants.CHR_DUI_ZI_HU_CHD);
//					}
					if(table.has_rule(GameConstants.GAME_RULE_HFT_CHD))
					{
						int count = table._logic.is_hong(table._hu_weave_items[i], table._hu_weave_count[i]); 
						if(count== 3){
							chr.opr_or_long(GameConstants.CHR_HONG_FAN_TIAN_CHD);
							chr.opr_or_long(GameConstants.CHR_MTH_DA_CHD);
							chr.opr_or_long(GameConstants.CHR_MTH_XIAO_CHD);
						}
						else if(count == 2){
							chr.opr_or_long(GameConstants.CHR_MTH_DA_CHD);
						}
...
//			}
			...
			table.process_chi_hu_player_score_lba(target_player, _seat_index, operate_card, true);
