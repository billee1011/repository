...
public class HandlerOutCardOperate_JiYuan extends MJHandlerOutCardOperate<Table_JiYuan> {
    ...
    public void exe(Table_JiYuan table) {
        ...
        table.canNotHu[_out_card_player] = _out_card_data; // 过手胡记录
    }
    ...
    public boolean handler_operate_card(Table_JiYuan table, int seat_index, int operate_code, int operate_card) {
        ...
        if (operate_code == GameConstants.WIK_CHI_HU) {
            ...
            if (table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_CHI_HU)) {
                table.canNotHu[seat_index] = _out_card_data;
            }

            if (table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_GANG)) {
                table.canNotGang[seat_index] = _out_card_data;
            }
        }
        ...
        boolean hasGang = table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_GANG);
        ...
        case GameConstants.WIK_PENG: {
            ...
            // 该玩家可接杠而选择碰牌不杠 则要过一轮之后才能补杠
            if (hasGang) {
                table.canNotGang[seat_index] = operate_card;
                table.outCardRound[seat_index] = 2; // 这里设置为2、玩家出两张牌后表示过一轮(出一张牌是当前杠牌、再补一张抓一张才是下一轮)
            }
        ...
        case GameConstants.WIK_CHI_HU:
            ...
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                ...
                jie_pao_count++;
                table.jiePao[i]++;
                table.chr[i].opr_or(Constants_MJ_JiYuan.CHR_JIE_PAO); // 接炮
            }

            if (jie_pao_count > 0) {
                table.dianPao[_out_card_player] += jie_pao_count;
                table.chr[_out_card_player].opr_or(Constants_MJ_JiYuan.CHR_FANG_PAO); // 放炮
                if (jie_pao_count == 1) {
