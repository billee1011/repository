...
public class MJHandlerDispatchCard_SanMenXia extends MJHandlerDispatchCard<MJTable_SanMenXia> {
	...
	public void exe(MJTable_SanMenXia table) {
		...
		if(_type == GameConstants.HU_CARD_TYPE_GANG_KAI){
			_send_card_data = table.select_gang_card;
		}
		else{
		...
		if (table.has_rule(GameConstants.GAME_RULE_DAI_FENG_SMX))
		...
		if (ting_count > 0) {
			...
			for (int i = 0; i < hand_card_count; i++) {
				...
				if (table._logic.is_magic_card(cards[i])) {
					iTempcard += GameConstants.CARD_ESPECIAL_TYPE_HUN;
				} 
				cards[i] = iTempcard;
			}
	 ...
	 */
	@Override
	public boolean handler_player_out_card(MJTable_SanMenXia table, int seat_index, int card) {
		// 错误断言
	...
	public boolean handler_operate_card(MJTable_SanMenXia table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: {
			...
			if(table.has_rule(GameConstants.GAME_RULE_GANG_SUI_HU_SMX)){
			...
			if(table._player_result.biaoyan[_seat_index] != 0){
				table.process_player_biaoyan(_seat_index,GameConstants.Effect_Action_Other,GameConstants.INVALID_SEAT);
			}
