...
public class MJHandlerGang_NingXia_HuaShui extends MJHandlerGang<MJTable_NingXia_HuaShui> {
	...
	@Override
	protected boolean exe_gang(MJTable_NingXia_HuaShui table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_provide_player] -= gang_fen
		...
		if (table.GRR._left_card_count > 0) {
			table.exe_dispatch_card(_seat_index, Constants_MJ_NXHS.HU_GANG_SHANG_KAI_HUA, 0);
		} else {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
