...
public class SCHCPHandlerGang_YanZhong extends SCHCPHandlerGang<SCHCPTable_YanZhong> {
	...
	public boolean handler_operate_card(SCHCPTable_YanZhong table, int seat_index, int operate_code, int operate_card,
		...
		if ((operate_code != GameConstants.WIK_NULL) && (operate_code != GameConstants.CP_WIK_TOU))// 没有这个操作动作
		{
	 ...
	 * 
	 ***/
	protected boolean exe_gang(SCHCPTable_YanZhong table) {
		int cbCardIndex = table._logic.switch_to_card_index(_center_card);
		int cbWeaveIndex = -1;
		Arrays.fill(table._temp_peng_index[_seat_index], 0);
		if (GameConstants.CP_TYPE_HUA == _type || GameConstants.CP_TYPE_DISPATCH_CHE == _type) {
			// 暗龙
			// 设置变量
		...
		} else if (GameConstants.CP_TYPE_OUT_CHE == _type) {
			// 提龙
			// 设置变量
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
		...
		else if(GameConstants.CP_TYPE_HUA_LONG == _type){
			for(int i = 0; i< table.GRR._weave_count[_seat_index];i++){
				if((table.GRR._weave_items[_seat_index][i].weave_kind==GameConstants.DZ_WIK_PENG
						||table.GRR._weave_items[_seat_index][i].weave_kind==GameConstants.CP_WIK_HUA
						||table.GRR._weave_items[_seat_index][i].weave_kind==GameConstants.CP_WIK_CHE) 
		...
		if (_type == GameConstants.CP_TYPE_OUT_CHE) {
			// 删掉出来的那张牌
		...
		if(GameConstants.CP_TYPE_HUA_LONG != _type)
		{
			table._temp_peng_index[_seat_index][table._logic.switch_to_card_index(_center_card)]++;
		}
		...
		table.GRR._weave_items[_seat_index][cbWeaveIndex].hu_xi = table._logic.get_analyse_tuo_shu(_action,
				_center_card);
		table.set_must_out_card(table._logic.switch_to_card_index(_center_card));
		table.must_out_card();
		table.add_must_zhao(_seat_index, _center_card);
		// 设置用户
		...
		if(table._logic.get_dot(_center_card) != 7)
...
//			curPlayerStatus.add_tou(_center_card, GameConstants.CP_WIK_TOU, _seat_index);
...
//				GameSchedule.put(new OutCardRunnable(table.getRoom_id(), _seat_index, _center_card),
