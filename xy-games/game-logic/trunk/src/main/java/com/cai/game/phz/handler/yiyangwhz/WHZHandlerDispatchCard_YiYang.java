 ...
 *
 */
public class WHZHandlerDispatchCard_YiYang extends PHZHandlerDispatchCard<YiYangWHZTable> {
	...
	public void exe(YiYangWHZTable table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			// 流局
			table.operate_effect_action(table._cur_banker, GameConstants.EFFECT_ACTION_DRAW, 1,
					new long[] { GameConstants.ACT_DRAW }, 1, GameConstants.INVALID_SEAT);
		...
		bAroseAction = table.estimate_player_out_card_respond_yiyangwhz(_seat_index, _send_card_data,
				table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], true);

		// 判断玩家清飘
		if (table.estimate_player_dispatch_qing_piao_respond_yywhz(_seat_index, _send_card_data)) {
			bAroseAction = true;
		}
	...
	public boolean handler_operate_card(YiYangWHZTable table, int seat_index, int operate_code, int operate_card,
			...
			if (table._playerStatus[i].has_action()) {
				...
				if (table._playerStatus[target_player].is_respone()) {
					...
					cbTargetActionRank = table._logic.get_action_rank_yiyang(
							table._playerStatus[target_player].get_perform(), target_player, _seat_index) + target_p;
					cbActionRank[i] = cbUserActionRank;
				} else {
					// 获取最大的动作的优先级
			...
			if (_seat_index != target_player) {
				...
				for (int j = 0; j < table._playerStatus[last_player]._action_count; j++) {
					...
					case GameConstants.WIK_EQS:
						...
						table.cannot_chi_card_YIYANG(last_player, operate_card, table.GRR._cards_index[last_player]);
						if (flag == false)
				...
				for (int j = 0; j < table._playerStatus[i]._action_count; j++) {
					...
					case GameConstants.WIK_YYWHZ_WAI:
						if (table._logic.get_action_rank_yiyang(target_action, target_player,
								_seat_index) < table._logic.get_action_rank_yiyang(GameConstants.WIK_YYWHZ_WAI,
										target_player, _seat_index)) {
							table._cannot_peng[i][table._cannot_peng_count[i]++] = operate_card;
						}
						break;
					case GameConstants.WIK_PENG:
						if (table._logic.get_action_rank_yiyang(target_action, target_player,
								_seat_index) < table._logic.get_action_rank_yiyang(GameConstants.WIK_PENG,
										target_player, _seat_index)) {
							table._cannot_peng[i][table._cannot_peng_count[i]++] = operate_card;
						}
						break;
		...
		case GameConstants.WIK_YIYANGWHZ_QING_WAI:// 溜牌
		{
