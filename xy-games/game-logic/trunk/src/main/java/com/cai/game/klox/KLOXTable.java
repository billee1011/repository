...
import protobuf.clazz.klox.klRsp.ButtonOperateKlox;
import protobuf.clazz.klox.klRsp.GameStartKlox;
import protobuf.clazz.klox.klRsp.JettonResultKlox;
import protobuf.clazz.klox.klRsp.OpenCardKlox;
import protobuf.clazz.klox.klRsp.Opreate_Klox_Request;
import protobuf.clazz.klox.klRsp.PlayerResultKlox;
import protobuf.clazz.klox.klRsp.PukeGameEndKlox;
import protobuf.clazz.klox.klRsp.RoomPlayerResponseKlox;
import protobuf.clazz.klox.klRsp.SelectdBankerKlox;
import protobuf.clazz.klox.klRsp.SelectdBankerResultKlox;
import protobuf.clazz.klox.klRsp.SendCardGPKlox;
import protobuf.clazz.klox.klRsp.SendCardKlox;
import protobuf.clazz.klox.klRsp.TableResponseKlox;

///////////////////////////////////////////////////////////////////////////////////////////////
public class KLOXTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	private static Logger logger = Logger.getLogger(KLOXTable.class);
	...
	public KLOXGameLogic _logic = null;
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_HAND_START) == 1)
			this.setMinPlayerCount(2);
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_FOUR_START) == 1)
			this.setMinPlayerCount(4);
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_FIVE_START) == 1)
			this.setMinPlayerCount(5);
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_SIX_START) == 1)
			this.setMinPlayerCount(6);
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_SEVEN_START) == 1)
			this.setMinPlayerCount(7);
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_EIGHT_START) == 1)
	...
	protected boolean on_handler_game_start() {
		...
		_cur_jetton_round = 0;
		//
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_LAI_ZI) == 1) {
		...
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_ROB_THREE) == 1)
			this._banker_max_times = 3;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KL_ROB_TWO) == 1)
			this._banker_max_times = 2;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KL_ROB_ONE) == 1)
			this._banker_max_times = 1;
		else
			this._banker_max_times = 1;

		if (this.getRuleValue(GameConstants.GAME_RULE_KL_DI_THREE) == 1)
			_jetton_count_max = 3;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KL_DI_TWO) == 1)
			_jetton_count_max = 2;
		else
			_jetton_count_max = 1;

		if (this.getRuleValue(GameConstants.GAME_RULE_KL_TUI_ZHU_FIVE) == 1)
			this._tui_zhu_times = 20;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KL_TUI_ZHU_FOUR) == 1)
			this._tui_zhu_times = 15;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KL_TUI_ZHU_THREE) == 1)
			this._tui_zhu_times = 10;
		else if (this.getRuleValue(GameConstants.GAME_RULE_KL_TUI_ZHU_TWO) == 1)
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this._jetton_count[i] == 1) {
				this._jetton_score[i][0] = 1;
			}
			if (this._jetton_count[i] == 2) {
				this._jetton_score[i][0] = 1;
				this._jetton_score[i][1] = 5;
			}
			if (this._jetton_count[i] == 3) {
				this._jetton_score[i][0] = 1;
				this._jetton_score[i][1] = 5;
				this._jetton_score[i][2] = 10;
			}
		...
		int di_score = 1;
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_DI_ONE) == 1)
			di_score = 1;
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_DI_TWO) == 1)
			di_score = 5;
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_DI_THREE) == 1)
			di_score = 10;
		if (this._jetton_count_max == 1 && this.is_mj_type(GameConstants.GAME_TYPE_KLDGOX) == false) {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				if (this._player_status[i] == false)
					continue;
				this._jetton_player[i] = di_score;
			}
		}
		return game_start_Klox();
	}

	// 斗板牛开始
	public boolean game_start_Klox() {
		...
		for (int index = 0; index < this.getTablePlayerNumber(); index++) {
			...
			roomResponse.setType(MsgConstants.RESPONSE_KLOX_GAME_START);
			roomResponse.setGameStatus(this._game_status);
			// 发送数据
			GameStartKlox.Builder gamestart = GameStartKlox.newBuilder();
	...
	public boolean update_button(int opr_type, int seat_index, int display_time, boolean is_grr) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_UPDATE_BTN);
		ButtonOperateKlox.Builder button_operate = ButtonOperateKlox.newBuilder();
		...
		if (opr_type == 2) {
			if (this.is_mj_type(GameConstants.GAME_TYPE_KLDGOX) && seat_index == this._cur_banker) {
				for (int j = 0; j < 2; j++) {
					button_operate.addButton(j);
				}
			} else if (this.is_mj_type(GameConstants.GAME_TYPE_KLDGOX) == false) {
				for (int j = 0; j < 2; j++) {
					button_operate.addButton(j);
				}
			}
			...
			if (is_mj_type(GameConstants.GAME_TYPE_KLDGOX)) {
	...
	public boolean send_call_banker_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_ROBOT_BANKER);
		SelectdBankerKlox.Builder robot_banker = SelectdBankerKlox.newBuilder();
	...
	public boolean set_banker_to_room() {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_ROBOT_BANKER_RESULT);
		SelectdBankerResultKlox.Builder robot_banker = SelectdBankerResultKlox.newBuilder();
	...
	public void send_jetton_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_JETTON);
		JettonResultKlox.Builder jetton_result = JettonResultKlox.newBuilder();
	...
	public boolean send_card_gp(boolean is_display) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_SEND_GP);
		SendCardGPKlox.Builder send_card_gp = SendCardGPKlox.newBuilder();
		if (is_display == false)
	...
	public boolean send_card(int card_count, int opr_type) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_SEND_CARD);
		SendCardKlox.Builder send_card = SendCardKlox.newBuilder();
			...
			for (int k = 0; k < this.getTablePlayerNumber(); k++) {
				...
				if (this._player_status[k] == false) {
					...
					if (card_count == GameConstants.OX_MAX_CARD_COUNT) {
						if (is_mj_type(GameConstants.GAME_TYPE_KLJDOX) || is_mj_type(GameConstants.GAME_TYPE_KLDGOX)
								|| (is_mj_type(GameConstants.GAME_TYPE_KLTBOX) && this.getRuleValue(GameConstants.GAME_RULE_KL_MODE) == 1))
							this._player_times[k] = _logic.get_times_one(GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
						else if ((is_mj_type(GameConstants.GAME_TYPE_KLTBOX) && this.getRuleValue(GameConstants.GAME_RULE_KL_NO_TIMES) == 1))
							this._player_times[k] = 1;
						if (is_mj_type(GameConstants.GAME_TYPE_KLFKOX))
							this._player_times[k] = _logic.get_times_two(GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);

						this._ox_value[k] = _logic.get_card_type(GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
	...
	public boolean open_card(int seat_index) {
		...
		if (this.is_mj_type(GameConstants.GAME_TYPE_KLDGOX)) {
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				int loop = (seat_index + i) % this.getTablePlayerNumber();
				if (this._player_status[loop] == false)
					continue;
				this._open_card_player[loop] = true;
				_logic.get_ox_card(GRR._cards_data[loop], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
				if (this.is_mj_type(GameConstants.GAME_TYPE_KLDGOX)) {
					this._player_times[loop] = _logic.get_times_one(GRR._cards_data[loop], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);

					this._ox_value[loop] = _logic.get_card_type(GRR._cards_data[loop], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
		...
		if (flag == false) {
			if (is_mj_type(GameConstants.GAME_TYPE_KLTBOX)) {
				int max_seat_index = process_tbox_calulate_end();
				process_ox_calulate_end(max_seat_index);
			} else
	...
	public void process_ox_calulate_end(int max_seat_index) {
		if (this.getRuleValue(GameConstants.GAME_RULE_KL_TURN_BANKER) == 1) {
			is_trun_banker = false;
			if (this.getRuleValue(GameConstants.GAME_RULE_KL_NO_OX_OFF) == 1) {
				int card_type = _logic.get_card_type(GRR._cards_data[_cur_banker], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
				if (card_type == 0)
					is_trun_banker = true;
			}
			if (this.getRuleValue(GameConstants.GAME_RULE_KL_BANKER_OFF) == 1) {
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					if (this._player_status[i] == false)
						continue;
					if (i == _cur_banker)
					...
					if (card_type >= GameConstants.KLOX_XIAO_TEN)
						is_trun_banker = true;

				}
			}
			if (this.getRuleValue(GameConstants.GAME_RULE_KL_TWO_OFF) == 1) {
				if (_banker_count == 2)
					is_trun_banker = true;
			}
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this._player_status[i] != true)
				continue;
			boolean first_ox = _logic.get_ox_card(GRR._cards_data[max_seat_index], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
			boolean next_ox = _logic.get_ox_card(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
			...
			if (action == true) {
				if (is_mj_type(GameConstants.GAME_TYPE_KLJDOX) || is_mj_type(GameConstants.GAME_TYPE_KLDGOX)
						|| (is_mj_type(GameConstants.GAME_TYPE_KLTBOX) && this.getRuleValue(GameConstants.GAME_RULE_KL_MODE) == 1))
					calculate_score = _logic.get_times_one(GRR._cards_data[max_seat_index], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
				else if ((is_mj_type(GameConstants.GAME_TYPE_KLTBOX) && this.getRuleValue(GameConstants.GAME_RULE_KL_NO_TIMES) == 1))
					calculate_score = 1;
				if (is_mj_type(GameConstants.GAME_TYPE_KLFKOX))
					calculate_score = _logic.get_times_two(GRR._cards_data[max_seat_index], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
	...
	public boolean send_open_to_room(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_OPEN_CARD);
		OpenCardKlox.Builder open_card = OpenCardKlox.newBuilder();
	...
	private void test_cards() {
		...
		// 0x1a,0x11,0x0B,0x4f,0x1D,0x09,0x18,0x27,0x36,0x4e
		// };
	...
	public RoomPlayerResponseKlox.Builder load_player_info_data(int seat_index) {
		Player rplayer = this.get_players()[seat_index];
		if (rplayer == null)
			return null;
		RoomPlayerResponseKlox.Builder room_player = RoomPlayerResponseKlox.newBuilder();
		room_player.setAccountId(rplayer.getAccount_id());
		room_player.setHeadImgUrl(rplayer.getAccount_icon());
		room_player.setIp(rplayer.getAccount_ip());
		room_player.setUserName(rplayer.getNick_name());
	...
	public void load_player_info_data_game_start(GameStartKlox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_game_end(PukeGameEndKlox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_reconnect(TableResponseKlox.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		ret = this.handler_game_finish_Klox(seat_index, reason);

		return ret;
	}

	public boolean handler_game_finish_Klox(int seat_index, int reason) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_KLOX_GAME_END);
		GameEndResponse.Builder game_end = GameEndResponse.newBuilder();
		PukeGameEndKlox.Builder game_end_Klox = PukeGameEndKlox.newBuilder();
		RoomInfo.Builder room_info = getRoomInfo();
		game_end_Klox.setRoomInfo(room_info);
		game_end.setRoomInfo(getRoomInfo());

		if (reason == GameConstants.Game_End_NORMAL) {
		}
		this.load_player_info_data_game_end(game_end_Klox);
		game_end_Klox.setGameRound(_game_round);
		game_end_Klox.setCurRound(_cur_round);
		game_end_Klox.setDisplayTime(this._idi_ready_card_time);
		if (GRR != null) {
			this.operate_player_data();
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				game_end.addGameScore(GRR._game_score[i]);
				game_end_Klox.addEndScore((int) GRR._game_score[i]);
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
				for (int j = 0; j < GameConstants.OX_MAX_CARD_COUNT; j++) {
					cards_card.addItem(GRR._cards_data[i][j]);
				}
				game_end_Klox.addCardsData(cards_card);
				game_end_Klox.addStrOxValue(this._str_ox_value[i]);
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if (_cur_round >= _game_round) {// 局数到了
				...
				game_end_Klox.setPlayerResult(this.process_player_result_Klox(reason));
		...
		game_end_Klox.setWinner(seat_index);
		game_end_Klox.setReason(real_reason);
		////////////////////////////////////////////////////////////////////// 得分总的
		game_end.setEndType(real_reason);
		game_end.setGameRound(_game_round);
		game_end.setCurRound(_cur_round);
		game_end.setCommResponse(PBUtil.toByteString(game_end_Klox));
	...
	public boolean handler_enter_room(Player player) {
		...
		if (player.getAccount_id() != this.getRoom_owner_account_id() && has_rule(GameConstants.GAME_RULE_KL_MIN_FORBID_JOIN)) {
		...
		if (has_rule(GameConstants.GAME_RULE_KL_HAND_START)) {
	...
	public void is_game_start() {
		...
		if ((_player_count >= sit_max) && (_player_count == _cur_count)) {
			if (this._cur_round == 0 && this.getRuleValue(GameConstants.GAME_RULE_KL_HAND_START) == 1) {
				this.is_game_start = 1;
				control_game_start();
			} else
				handler_game_start();
		} else if ((this._cur_round == 0) && this.getRuleValue(GameConstants.GAME_RULE_KL_HAND_START) == 1) {
			this.is_game_start = 0;
			control_game_start();
		}
		// if((this._cur_round == 0 )&&(_player_count >=
		// sit_max)&&(_player_count ==
		// _cur_count)&&this.getRuleValue(GameConstants.GAME_RULE_KL_HAND_START)==1)
		// {
		// this.is_game_start = 1;
		// control_game_start();
		// }
		// else if((this._cur_round == 0
		// )&&this.getRuleValue(GameConstants.GAME_RULE_KL_HAND_START)==1)
		// {
		// this.is_game_start = 0;
		// control_game_start();
		// }
		...
		// 2||this.getRuleValue(GameConstants.GAME_RULE_KL_HAND_START)!=1))
		// {
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status) && this.get_players()[seat_index] != null) {
			...
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			roomResponse.setType(MsgConstants.RESPONSE_KLOX_RECONNECT_DATA);

			TableResponseKlox.Builder tableResponse_Klox = TableResponseKlox.newBuilder();
			load_player_info_data_reconnect(tableResponse_Klox);
			RoomInfo.Builder room_info = getRoomInfo();
			tableResponse_Klox.setRoomInfo(room_info);

			tableResponse_Klox.setBankerPlayer(_cur_banker);

			int display_time = _cur_operate_time - ((int) (System.currentTimeMillis() / 1000L) - _operate_start_time);
			tableResponse_Klox.setDisplayTime(display_time);
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				tableResponse_Klox.addCallButton(this._player_call_banker[i]);
				if (this.is_mj_type(GameConstants.GAME_TYPE_KLDGOX) == true)
					tableResponse_Klox.addJettonScore(this._jetton_player[i]);
				else
					tableResponse_Klox.addJettonScore(-1);
				tableResponse_Klox.addOpenCard(this._open_card_player[i]);
				if (this._open_card_player[i] == true || i == seat_index) {
					tableResponse_Klox.addOxValue(this._ox_value[i]);
					tableResponse_Klox.addTimes(this._player_times[i]);
					if (i == seat_index && this._open_card_player[i] == true) {
						if (is_mj_type(GameConstants.GAME_TYPE_KLDGOX))
							tableResponse_Klox.setGpCard(this._gp_card);
					} else if (_game_status == GameConstants.GS_OX_OPEN_CARD)
						tableResponse_Klox.setGpCard(GameConstants.BLACK_CARD);
					...
					_logic.get_ox_card(temp_cards_data, GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
					tableResponse_Klox.clearSortCard();
					for (int index = 0; index < GameConstants.OX_MAX_CARD_COUNT; index++) {
						tableResponse_Klox.addSortCard(temp_cards_data[index]);
					}
				} else {
					tableResponse_Klox.addOxValue(-1);
					tableResponse_Klox.addTimes(0);
				}

			}
			tableResponse_Klox.setSceneStatus(_game_status);
			if (_game_status == GameConstants.GS_OX_CALL_BANKER) {

				tableResponse_Klox.setBankerPlayer(-1);
				int card_count = 4;
				if (has_rule(GameConstants.GAME_RULE_KL_BANKER) || is_mj_type(GameConstants.GAME_TYPE_KLDGOX)) {
					for (int k = 0; k < this.getTablePlayerNumber(); k++) {
						Int32ArrayResponse.Builder cards = Int32ArrayResponse.newBuilder();
						if (this._player_status[k] == false) {
							for (int j = 0; j < card_count; j++) {
						...
						tableResponse_Klox.addCardsData(k, cards);
					}
				}
			...
			roomResponse.setCommResponse(PBUtil.toByteString(tableResponse_Klox));
			this.send_response_to_player(seat_index, roomResponse);
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		if (type == MsgConstants.REQUST_KLOX_OPERATE) {

			Opreate_Klox_Request req = PBUtil.toObject(room_rq, Opreate_Klox_Request.class);
			// 逻辑处理
	...
	private PlayerResultKlox.Builder process_player_result_Klox(int reason) {
		int win_idx = 0;
		float max_score = 0;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			int winner = -1;
		...
		PlayerResultKlox.Builder player_result = PlayerResultKlox.newBuilder();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean animation_timer(int timer_id) {
		...
		case ID_TIMER_ANIMATION_START: {
			if (has_rule(GameConstants.GAME_RULE_KL_TURN_BANKER)) {
				...
				if (this._cur_round == 1) {
					...
					if (is_trun_banker == true) {
						is_trun_banker = false;
						this._banker_count = 0;
						for (int i = 0; i < this.getTablePlayerNumber(); i++) {
							int seat_index = (this._next_banker + i) % this.getTablePlayerNumber();
							if (seat_index == this._next_banker)
								continue;
			...
			} else if (has_rule(GameConstants.GAME_RULE_KL_FIXED)) {
				this._banker_times = this._banker_max_times;
				if (this._cur_round == 1) {
					int chairID[] = new int[this.getTablePlayerNumber()];
					int chair_count = 0;
				...
				set_open_card();
			} else if (has_rule(GameConstants.GAME_RULE_KL_BANKER) || is_mj_type(GameConstants.GAME_TYPE_KLDGOX)) {
				send_card(4, 0);
				set_call_banker();

			} else if (is_mj_type(GameConstants.GAME_TYPE_KLTBOX)) {
				int di_score = 1;
				if (this.getRuleValue(GameConstants.GAME_RULE_KL_DI_ONE) == 1)
					di_score = 1;
				if (this.getRuleValue(GameConstants.GAME_RULE_KL_DI_TWO) == 1)
		...
		case ID_TIMER_ANIMATION_ROBOT: {
			if (this._jetton_count_max == 1 && is_mj_type(GameConstants.GAME_TYPE_KLDGOX) == false) {
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					if (this._player_status[i] == false)
						continue;
					if (i == this._cur_banker)
				...
				if (is_mj_type(GameConstants.GAME_TYPE_KLDGOX) && this.getRuleValue(GameConstants.GAME_RULE_KL_LIMIT_LOW_JETTON) == 1) {
					for (int i = 0; i < this.getTablePlayerNumber(); i++) {
						if (this._player_status[i] == false)
							continue;
						if (i == this._cur_banker)
							continue;
						if (this._banker_max_times > 1 && this._jetton_count_max > 1 && this._banker_times > 1) {
