...
public class TDZGameLogic {

	// private static Logger logger = Logger.getLogger(TDZGameLogic.class);
	...
	public int get_times_one(int card_date[], int card_count, int game_rule_index) {
		...
		if (times < 7)
			...
			times = 4;
		else if (times == GameConstants.OX_FIVE_KING)
			times = 5;
		else if (times == GameConstants.OX_BOOM)
			times = 6;
		else if (times == GameConstants.OX_WUXIAONIU)
			times = 8;
		return times;
	}
