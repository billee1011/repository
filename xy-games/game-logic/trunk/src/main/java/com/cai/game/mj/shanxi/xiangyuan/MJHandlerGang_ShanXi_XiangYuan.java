...
public class MJHandlerGang_ShanXi_XiangYuan extends MJHandlerGang<MJTable_ShanXi_XiangYuan> {
	...
	@Override
	protected boolean exe_gang(MJTable_ShanXi_XiangYuan table) {
		int cbCardIndex = table._logic.switch_to_card_index(_center_card);
		int cbWeaveIndex = -1;
		table.gang_count++;
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			table.gang_change_state[i] = true;
		}
		...
		if (table.GRR._left_card_count > table.huang_zhuang_count + table.gang_count * 2) {
			table.exe_dispatch_card(_seat_index, Constants_MJ_XIANG_YUAN.HU_CARD_TYPE_ZI_MO, 0);
		} else {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	...
	public boolean handler_operate_card(MJTable_ShanXi_XiangYuan table, int seat_index, int operate_code,
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			...
			table.push_card[seat_index] = true;
			// table.process_chi_hu_player_operate(seat_index, operate_card,
			// false);
			table.hu_pai_people++;
			table.is_zm_or_dp[seat_index] = 2;// 接炮的玩家
			table.dian_pao = _seat_index;// 点炮的玩家
			// 客户端播放放炮动画
		...
		// // 获取已经执行的动作的优先级
		// cbUserActionRank =
		// table._logic.get_action_rank(table._playerStatus[i].get_perform());
		// } else {
		// // 获取最大的动作的优先级
		// cbUserActionRank =
		// table._logic.get_action_list_rank(table._playerStatus[i]._action_count,
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			if (table.hu_pai_people == 3) {// 一炮三响的时候直接算流局
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					table.GRR._chi_hu_card[i][0] = GameConstants.INVALID_VALUE;
				}
