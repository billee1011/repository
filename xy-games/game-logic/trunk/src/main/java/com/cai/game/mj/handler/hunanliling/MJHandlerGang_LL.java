...
public class MJHandlerGang_LL extends MJHandlerGang<MJTable_LL> {
    @Override
    public void exe(MJTable_LL table) {
        ...
        // 杠红中,不能抢杠
        if (table._logic.get_magic_card_count() > 0 && table._logic.switch_to_card_index(_center_card) == table._logic.get_magic_card_index(0)) {
            this.exe_gang(table);
            return;
        }

        // 抢杠胡判断
        if (!table.has_rule(GameConstants.GAME_RULE_HUNAN_GANG_HU)) {
            this.exe_gang(table);
            return;
        }
     ...
     ***/
    @Override
    protected boolean exe_gang(MJTable_LL table) {
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            ...
            // WalkerGeek 放杠算分规则：放杠人包其他两人分数,（牌桌人数-1） *杠底分
        ...
        //添加杠分动画
        long changScore[] = new long[4];
        for (int i = 0; i < table.getTablePlayerNumber(); i++) {
        	changScore[i] = (long)table.GRR._gang_score[_seat_index].scores[cbGangIndex][i];
        }
        table.operate_effect_action(_seat_index, GameConstants.EFFECT_GNAG_SCORE, 4, changScore, 1,
				GameConstants.INVALID_SEAT);
