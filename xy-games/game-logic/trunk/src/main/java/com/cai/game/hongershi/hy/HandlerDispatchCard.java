...
public class HandlerDispatchCard extends HHHandlerDispatchCard<HongErShiTable_HY> {
	...
	public void exe(HongErShiTable_HY table) {
		...
		if (GameDescUtil.has_rule(table.gameRuleIndexEx, HongErShiConstants.RULE_7_IS_KING)
				&& table.logic.get_card_value(_send_card_data) == HongErShiConstants.MAGIC_SEVEN_VALUE) {
			table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { HongErShiConstants.WIK_TOU }, 5,
					GameConstants.INVALID_SEAT);
		}
		...
		if (_type == GameConstants.WIK_NULL) {
			...
			while (_send_card_data == HongErShiConstants.MAGIC_CARD_KING
				...
				if (!table.has_king_tou[_seat_index]) {
					...
					table.GRR._weave_items[_seat_index][cbWeaveIndex].center_card = _send_card_data;
					...
					table.GRR._weave_items[_seat_index][cbWeaveIndex].weave_card[0] = _send_card_data;

					table.has_king_tou[_seat_index] = true;
				} else {
					int cbIndex = -1;
					for (int wc = 0; wc < table.GRR._weave_count[_seat_index]; wc++) {
						if (table.GRR._weave_items[_seat_index][wc].weave_kind != HongErShiConstants.WIK_TOU) {
							continue;
						}

						if (table.GRR._weave_items[_seat_index][wc].center_card == _send_card_data) {
							cbIndex = wc;
							break;
						}
						if (table.logic.get_card_value(_send_card_data) == table.logic
								.get_card_value(table.GRR._weave_items[_seat_index][wc].center_card)) {
							cbIndex = wc;
							break;
						}
					...
					if (cbIndex == -1) {
						...
						for (int c = 0; c < table.GRR._weave_items[_seat_index][cbIndex].weave_card.length; c++) {
							if (table.GRR._weave_items[_seat_index][cbIndex].weave_card[c] == 0) {
								table.GRR._weave_items[_seat_index][cbIndex].weave_card[c] = _send_card_data;
								break;
							}
						}
					}
