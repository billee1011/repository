...
public class MJTable_JiangSu_JH extends AbstractMJTable {
	...
	public MJHandlerHun_JiangSu_JH _handler_hun;// 癞子牌
	...
	public MJTable_JiangSu_JH() {
		super(MJType.GAME_TYPE_JIANGSU_ZZ);
	}

	@Override
	protected void onInitTable() {
		_handler_dispath_card = new MJHandlerDispatchCard_MJHandlerChiPeng_JiangSu_JH();
		_handler_out_card_operate = new MJHandlerOutCardOperate_JiangSu_JH();
		_handler_gang = new MJHandlerGang_JiangSu_JH();
		_handler_chi_peng = new MJHandlerChiPeng_JiangSu_JH();
		_handler_hun = new MJHandlerHun_JiangSu_JH();
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				for (int j = 0; j < GRR._cards_index[i].length; j++) {
					if (GRR._cards_index[i][j] == 4) {
						MongoDBServiceImpl.getInstance().card_log(this.get_players()[i], ECardType.anLong, "", GRR._cards_index[i][j], 0l, this.getRoom_id());
					}
				}
			}
		} catch (Exception e) {
	...
	protected void test_cards() {
		...
		int cards[] = new int[] { 0x15, 0x15, 0x15, 0x08, 0x08, 0x08, 0x03, 0x04, 0x05, 0x18, 0x18, 0x22, 0x22 };
	...
	protected boolean on_game_start() {
		...
		if (this.has_rule(GameConstants.GAME_RULE_JIANGSU_DOUBLE)) {
			...
			if (this._lian_zhuang_player == GameConstants.INVALID_SEAT) {
				GameSchedule.put(new LengTuoZiRunnable(getRoom_id(), GRR._banker_player, this), GameConstants.GAME_FINISH_DELAY, TimeUnit.SECONDS);
			} else {
				_lian_zhuang_player = GameConstants.INVALID_SEAT;
	...
	public int analyse_chi_hu_card(int cards_index[], WeaveItem weaveItems[], int weaveCount, int cur_card, ChiHuRight chiHuRight, int card_type) {
		...
		if (card_type == GameConstants.HU_CARD_TYPE_PAOHU && _logic.is_laizi_hu(analyseItemArray, cbCardIndexTemp, weaveItems, weaveCount)) {
			chiHuRight.set_empty();
			return GameConstants.WIK_NULL;
		}
	...
	public int get_ting_card(int[] cards, int cards_index[], WeaveItem weaveItem[], int cbWeaveCount) {
		...
		for (int i = 0; i < mj_count; i++) {
			...
			if (GameConstants.WIK_CHI_HU == analyse_chi_hu_card(cbCardIndexTemp, weaveItem, cbWeaveCount, cbCurrentCard, chr,
				...
				if (this._logic.is_magic_index(i)) {
					if (chr.opr_and(GameConstants.CHR_HENAN_QI_XIAO_DUI).is_empty() || chr.opr_and(GameConstants.CHR_HENAN_HH_QI_XIAO_DUI).is_empty()) {
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL || reason
			...
			for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
				...
				for (int j = 0; j < _da_tou_da[i] / 6; j++) {
					cs.addItem(_logic.switch_to_card_data(_logic.get_magic_card_index(0)) + GameConstants.CARD_ESPECIAL_TYPE_TOU_DA);
				}
				if (this._da_tou_da[i] > 0) {
					GRR._card_count[i] += _da_tou_da[i] / 6;
				}
				for (int j = 0; j < _da_er_da[i] / 4; j++) {
					cs.addItem(_logic.switch_to_card_data(_logic.get_magic_card_index(0)) + GameConstants.CARD_ESPECIAL_TYPE_ER_DA);
				}
				if (this._da_er_da[i] > 0) {
					GRR._card_count[i] += _da_er_da[i] / 4;
				}
				for (int j = 0; j < _da_peng_da[i] / 2; j++) {
					cs.addItem(_logic.switch_to_card_data(_logic.get_magic_card_index(0)) + GameConstants.CARD_ESPECIAL_TYPE_BEIDONG_DA);
				}
	 ...
	 */
	//
	public boolean operate_out_card(int seat_index, int count, int cards[], int type, int to_player, int discard_count) {
	 ...
	 * @param zimo
	 */
	public void process_chi_hu_player_score_jszz(int seat_index, int provide_index, int operate_card, boolean zimo, boolean yipaosanxiang, boolean is_beopei,
			boolean gang_fangpao) {
		if (this.has_rule(GameConstants.GAME_RULE_JIANGSU_DOUBLE)) {
			if (this._cur_banker == seat_index) {
				this._lian_zhuang_player = seat_index;
			} else {
				this._cur_banker = (_cur_banker + 1) % getTablePlayerNumber();
			}
		} else {
			this._cur_banker = (_cur_banker + 1) % getTablePlayerNumber();
		}
		...
		if (zimo) {
			// 自摸
			int score_time = (_user_times[seat_index] + _da_tou_da[seat_index] + _da_er_da[seat_index] + _da_peng_da[seat_index]);
			...
			if (has_rule(GameConstants.GAME_RULE_JIANGSU_PEI_CHONG)) {
				if (has_rule(GameConstants.GAME_RULE_JIANGSU_YUAN_ZI)) {
					for (int i = 0; i < getTablePlayerNumber(); i++) {
						...
						if (i == provide_index) {
							if (_player_result.game_score[i] - (score_time + _user_times[seat_index] * 4) * _di_fen >= -_yuanzi_fen) {
							...
							if (_player_result.game_score[i] - (score_time + _user_times[seat_index] * 2) * _di_fen >= -_yuanzi_fen) {
	...
	public void load_game_end_detail(GameEndResponse.Builder gameendResponse) {
		...
		for (int player = 0; player < GameConstants.GAME_PLAYER; player++) {
			...
			if (chr.is_valid()) {
				String hu_detail = "";
				if (!(chr.opr_and(GameConstants.CHR_ZI_MO)).is_empty() || !(chr.opr_and(GameConstants.CHR_JIANGSU_ZZ_AN_GANG_KAI)).is_empty()
						|| !(chr.opr_and(GameConstants.HU_CARD_TYPE_GANG_KAI)).is_empty()) {
					if (!(chr.opr_and(GameConstants.CHR_JIANGSU_ZZ_AN_GANG_KAI)).is_empty()
					...
					for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
						...
						if (this._is_bao_pei) {
							...
							detail.setSeatIndex(GRR._weave_items[player][GRR._weave_count[player] - 1].provide_player);
						...
						if (has_rule(GameConstants.GAME_RULE_JIANGSU_DOUBLE) && this._b_double) {
							detail.setScore((_user_times[player] + _da_tou_da[player] + _da_er_da[player] + _da_peng_da[player]) * 2);
						} else {
							detail.setScore(_user_times[player] + _da_tou_da[player] + _da_er_da[player] + _da_peng_da[player]);
						}
					...
					if (this._is_bao_pei || (GRR._chi_hu_rights[fangpao_player].opr_and(GameConstants.CHR_JIANGSU_ZZ_YI_PAO_SAN_XIANG)).is_empty()) {
						for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
							if (i == player) {
								continue;
							}
							...
							if (!this._is_bao_pei || hu_count == 1) {
								if (!(otherchr.opr_and(GameConstants.CHR_FANG_PAO)).is_empty() || !(otherchr.opr_and(GameConstants.CHR_TONG_PAO)).is_empty()) {
									hu_detail += "放冲 +4 ";
									...
									detail.setSeatIndex(i);
									if (has_rule(GameConstants.GAME_RULE_JIANGSU_DOUBLE) && this._b_double) {
										detail.setScore((_user_times[player] * 4 + _da_tou_da[player] + _da_er_da[player] + _da_peng_da[player]) * 2);
									} else {
										detail.setScore(_user_times[player] * 4 + _da_tou_da[player] + _da_er_da[player] + _da_peng_da[player]);
									}
									...
									detail.setScoreType(SCORE_TYPE_HU_TYPE);// 类型
									if (this.has_rule(GameConstants.GAME_RULE_JIANGSU_BAO_CHONG) || this.has_rule(GameConstants.GAME_RULE_JIANGSU_DOUBLE)) {
									...
									if (has_rule(GameConstants.GAME_RULE_JIANGSU_DOUBLE) && this._b_double) {
										detail.setScore((_user_times[player] * 2 + _da_tou_da[player] + _da_er_da[player] + _da_peng_da[player]) * 2);
									} else {
										detail.setScore(_user_times[player] * 2 + _da_tou_da[player] + _da_er_da[player] + _da_peng_da[player]);
									}
			...
			if (fangpao_player != GameConstants.INVALID_SEAT
					&& !(GRR._chi_hu_rights[fangpao_player].opr_and(GameConstants.CHR_JIANGSU_ZZ_YI_PAO_SAN_XIANG)).is_empty() && fangpao_player == player) {
		...
		for (int player = 0; player < GameConstants.GAME_PLAYER; player++) {
			...
			if (chr.is_valid()) {
				if (!(chr.opr_and(GameConstants.CHR_ZI_MO)).is_empty() || !(chr.opr_and(GameConstants.CHR_JIANGSU_ZZ_AN_GANG_KAI)).is_empty()) {
					...
					if (type == GameConstants.CHR_JIANGSU_ZZ_AN_GANG_KAI || type == GameConstants.CHR_JIANGSU_ZZ_MING_GANG_KAI) {
				...
				gameDesc.append(" 放冲");
			} else if (!(chr.opr_and(GameConstants.CHR_JIANGSU_ZZ_CHU_CHONG)).is_empty() && has_rule(GameConstants.GAME_RULE_JIANGSU_PEI_CHONG)) {
				gameDesc.append(" 出冲");
	...
	public void runnable_remove_hun_middle_cards(int seat_index) {
		...
		// 检测听牌
		for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
			this._playerStatus[i]._hu_card_count = this.get_ting_card(this._playerStatus[i]._hu_cards, this.GRR._cards_index[i], this.GRR._weave_items[i],
	...
	public int get_real_card(int card) {
		...
		} else if (card > GameConstants.CARD_ESPECIAL_TYPE_ER_DA && card < GameConstants.CARD_ESPECIAL_TYPE_BEIDONG_DA) {
			card -= GameConstants.CARD_ESPECIAL_TYPE_ER_DA;
		} else if (card > GameConstants.CARD_ESPECIAL_TYPE_BEIDONG_DA && card < GameConstants.CARD_ESPECIAL_TYPE_PEN_CHANGER) {
