...
import com.cai.future.runnable.OperateCardRunnable;
...
public class PHZHandlerOutCardOperate_MS extends PHZHandlerOutCardOperate<SCPHZTable> {
	@Override
	public void exe(SCPHZTable table) {
		...
		while (loop < table.getTablePlayerNumber()) {
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				if (table.has_rule(GameConstants.GAME_RULE_QIANG_ZHI_HU_PAI) || table.isTrutess(i)) {
					int hucard_time = 600;
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if ((action_pao[i] != GameConstants.WIK_NULL) && (bHupai == 0)) {
				...
				ti_pao = table.estimate_player_respond_phz_chd(i, _out_card_player, _out_card_data, pao_type_out, false);
		...
		if (bAroseAction == false && ti_pao == GameConstants.WIK_NULL && bHupai == 0) {
			...
			int gameId = table.getGame_id() == 0 ? 5 : table.getGame_id();
			SysParamModel sysParamModel1105 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(gameId).get(1105);
			SysParamModel sysParamModel1111 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(gameId).get(1111);
			if (table.has_rule(GameConstants.GAME_RULE_ADD_SPEED) && sysParamModel1105 != null && sysParamModel1105.getVal1() > 0
					&& sysParamModel1105.getVal1() < 10000) {
				discard_time = sysParamModel1105.getVal1();
			}
			if (table.has_rule(GameConstants.GAME_RULE_ADD_SPEED) == false && sysParamModel1111 != null && sysParamModel1111.getVal1() > 0
					&& sysParamModel1111.getVal1() < 10000) {
			...
			if (table.has_rule(GameConstants.GAME_RULE_ADD_SPEED) && sysParamModel1105 != null && sysParamModel1105.getVal2() > 0
					&& sysParamModel1105.getVal2() < 10000) {
				dispatch_time = sysParamModel1105.getVal2();
			}
			if (table.has_rule(GameConstants.GAME_RULE_ADD_SPEED) == false && sysParamModel1111 != null && sysParamModel1111.getVal2() > 0
					&& sysParamModel1111.getVal2() < 10000) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if (table._playerStatus[i].has_action() && !table.isTrutess(i)) {
					table._playerStatus[i].set_status(GameConstants.Player_Status_OPR_CARD);// 操作状态
					table.operate_player_action(i, false);
				}
				if (table._playerStatus[i].has_action() && table.isTrutess(i)) {
					GameSchedule.put(new OperateCardRunnable(table.getRoom_id(), i, _out_card_data), GameConstants.DELAY_AUTO_OUT_CARD_TRUTESS,
	...
	public boolean handler_operate_card(SCPHZTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		case GameConstants.WIK_NULL: {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				int pao_type[] = new int[1];
				int action = table.estimate_player_respond_phz_chd(i, _out_card_player, _out_card_data, pao_type, false);
		...
		case GameConstants.WIK_PAO: // 杠牌操作
		{
			...
			int pao_type[] = new int[1];
			int action = table.estimate_player_respond_phz_chd(target_player, this._out_card_player, operate_card, pao_type, false);
			if (action != GameConstants.WIK_NULL) {
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table._shang_zhuang_player = target_player;
			table.process_chi_hu_player_score_hh(target_player, this._out_card_player, operate_card, false);
