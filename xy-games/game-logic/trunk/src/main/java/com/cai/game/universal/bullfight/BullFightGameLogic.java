...
public class BullFightGameLogic {
     ...
     * @return
     */
    public int get_card_type(int card_data[], int card_count, int game_rule_index, int game_type_index) {
        ...
        if (game_type_index == GameConstants.GAME_TYPE_JDOX_YY) {
            ...
            if (king_count == GameConstants.OX_MAX_CARD_COUNT)
                return GameConstants.OX_KING_NIU;
            else if (king_count == GameConstants.OX_MAX_CARD_COUNT - 1 && ten_count == 1)
                return GameConstants.OX_YING_NIU;
        } else {
        ...
        if ((same_color == true) && FvMask.has_any(game_rule_index, FvMask.mask(GameConstants.GAME_RULE_TONG_HUA_NIU)))
            return GameConstants.OX_TONG_HUA_VALUE;

        if ((line_card == true) && FvMask.has_any(game_rule_index, FvMask.mask(GameConstants.GAME_RULE_SHUN_ZI_NIU)))
            return GameConstants.OX_SHUN_ZI_VALUE;

        if (game_type_index != GameConstants.GAME_TYPE_JDOX_YY) {
            if (FvMask.has_any(game_rule_index, FvMask.mask(GameConstants.GAME_RULE_OX_WUHUANIU))) {
                if (king_count == GameConstants.OX_MAX_CARD_COUNT)
                    return GameConstants.OX_FIVE_KING_VALUE;
            }
        }
     ...
     * @return
     */
    public int get_times_one(int card_data[], int card_count, int game_rule_index, int game_type_index) {
        int times = 0;

        if (card_count != GameConstants.OX_MAX_CARD_COUNT)
            return 0;

        times = get_card_type(card_data, card_count, game_rule_index, game_type_index);
        ...
        if (times < 7)
            ...
            times = 4;
        else if (times == GameConstants.OX_SHUN_ZI_VALUE)
            times = 5;
        else if (times == GameConstants.OX_TONG_HUA_VALUE)
            times = 6;
        else if (times == GameConstants.OX_HU_LU_VALUE)
            times = 7;
        else if (times == GameConstants.OX_FIVE_KING_VALUE)
            times = 5;
        else if (times == GameConstants.OX_BOOM_VALUE)
            times = 8;
        else if (times == GameConstants.OX_WUXIAONIU_VALUE)
            times = 8;
        else if (times == GameConstants.OX_TONG_HUA_XHUN_VALUE)
            times = 10;
     ...
     * @return
     */
    public int get_times_ping(int card_data[], int card_count, int game_rule_index, int game_type_index) {
        int times = 1;
        return times;
    }

    public int get_times_two(int card_data[], int card_count, int game_rule_index, int game_type_index) {
        int times = 0;
     ...
     * @return
     */
    public boolean get_ox_card(int card_data[], int card_count, int game_rule_index, int game_type_index) {
        if (card_count != GameConstants.OX_MAX_CARD_COUNT)
            return false;
    ...
    public boolean compare_card(int first_data[], int next_data[], int card_count, boolean first_ox, boolean next_ox,
        ...
        // 比较牛大小
        // 获取点数
        int next_type = get_card_type(next_data, card_count, game_rule_index, game_type_index);
