...
public class SCHCPDZHandlerGang<T extends SCHCPDZTable> extends SCHCPDZHandler<T> {
...
//		
	...
	public boolean handler_player_out_card(T table, int seat_index, int card) {
		...
		if (table._logic.is_valid_card(card) == false) {
			table.log_error("SCHCPHandlerGang出牌,牌型出错");
			return false;
		}

		// 效验参数
		if (seat_index != _seat_index) {
			table.log_error("SCHCPHandlerGang出牌,没到出牌");
			return false;
		}
		...
		{
			table.log_error("SCHCPHandlerGang状态不对不能出牌");
			return false;
		}
		if(table.GRR._cards_index[_seat_index][table._logic.switch_to_card_index(card)] >= 3)
		{
			...
			table.log_error(_seat_index+"出牌出错 SCHCPHandlerGang "+_seat_index);
			return true;
		}
		// 删除扑克
		if (table._logic.remove_card_by_index(table.GRR._cards_index[_seat_index], card) == false) {
			table.log_error("SCHCPHandlerGang出牌删除出错");
			return false;
		}
