...
public class HandlerDispatchCard_ZPTDH extends MJHandlerDispatchCard<Table_ZPTDH> {
	...
	public HandlerDispatchCard_ZPTDH() {
	...
	public void exe(Table_ZPTDH table) {
		if(table.is_gen_zhuang_valid && _type == 0 && _seat_index == table._cur_banker && !table.is_gen_zhuang){
			table.is_gen_zhuang = true;
		}
		table.send_card_count[_seat_index]++;
		...
		// 流局处理
		if (table.GRR._left_card_count == table.getHorseCount()) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		// 判断是否是杠后抓牌,杠开花只算接杠
		int card_type = Constants_MJ_ZPTDH.HU_CARD_TYPE_ZI_MO;
		if(_type == Constants_MJ_ZPTDH.HU_CARD_TYPE_FANG_GANG_KAI_HU)
			card_type = Constants_MJ_ZPTDH.HU_CARD_TYPE_FANG_GANG_KAI_HU;
		// 检查牌型,听牌
	...
	public boolean handler_player_out_card(Table_ZPTDH table, int seat_index, int card) {
		...
		//跟庄处理
		if(table.is_gen_zhuang_valid){
			if(_type == GameConstants.GANG_TYPE_HONG_ZHONG && _seat_index == table._cur_banker)
				table.banker_out_first_card = card;
			else if(table.banker_out_first_card != card)
	...
	public boolean handler_operate_card(Table_ZPTDH table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if(_seat_index == i)
					table.hu_dec_type[i] = 1;
				else
					table.hu_dec_type[i] = 5;
			}
			ChiHuRight chiHuRight = table.GRR._chi_hu_rights[_seat_index];
			if(table.send_card_count[_seat_index] == 1){
				chiHuRight.set_empty();
				chiHuRight.opr_or(Constants_MJ_ZPTDH.CHR_TIAN_HU);
			}
			if(!chiHuRight.opr_and(Constants_MJ_ZPTDH.CHR_FANG_GANG_KAI).is_empty())
				table.hu_dec_type[_seat_index] = 4;
			chiHuRight.set_valid(true);
