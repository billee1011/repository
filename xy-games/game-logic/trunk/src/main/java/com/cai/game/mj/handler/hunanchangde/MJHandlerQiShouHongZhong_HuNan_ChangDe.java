...
public class MJHandlerQiShouHongZhong_HuNan_ChangDe extends AbstractMJHandler<MJTable_HuNan_ChangDe> {
	...
	public boolean handler_operate_card(MJTable_HuNan_ChangDe table, int seat_index, int operate_code, int operate_card) {
		...
		if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HUNAN_MO_JI_JIANG_JI)) {
			// 起手红中奖码按红中算
			int tmpCard = table._logic.switch_to_card_data(table._logic.get_magic_card_index(0));
			table.set_niao_card(_seat_index, tmpCard, true, 0);
		} else {
