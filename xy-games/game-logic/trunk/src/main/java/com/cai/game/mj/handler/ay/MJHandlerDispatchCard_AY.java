...
public class MJHandlerDispatchCard_AY extends MJHandlerDispatchCard<MJTable> {
    ...
    public void exe(MJTable table) {
        ...
        if (table.GRR._left_card_count == 0) {
            ...
            // 连庄
            table._cur_banker = table.GRR._banker_player;// (table._banker_select + 1) % MJGameConstants.GAME_PLAYER;
        ...
        if ((_send_card_data == GameConstants.CARD_CAI_SHENG) || table.GRR._cards_index[_seat_index][34] > 0) {
            ...
            // 显示出牌
            table.operate_out_card(_seat_index, 1, new int[] { GameConstants.CARD_CAI_SHENG },
                    GameConstants.OUT_CARD_TYPE_MID, GameConstants.INVALID_SEAT);
            table.exe_add_discard(_seat_index, 1, new int[] { GameConstants.CARD_CAI_SHENG }, false,
                    GameConstants.DELAY_SEND_CARD_DELAY);

            // 效果
            table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1,
                    new long[] { GameConstants.WIK_BU_HUA }, 1, GameConstants.INVALID_SEAT);
        ...
        for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
            ...
            if (table.GRR._cards_index[i][34] > 0) {
                ...
                // 显示出牌
                table.operate_out_card(i, 1, new int[] { GameConstants.CARD_CAI_SHENG },
                        GameConstants.OUT_CARD_TYPE_MID, GameConstants.INVALID_SEAT);
                table.exe_add_discard(i, 1, new int[] { GameConstants.CARD_CAI_SHENG }, false, 0);

                // 效果
                table.operate_effect_action(i, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1,
                        new long[] { GameConstants.WIK_BU_HUA }, 1, GameConstants.INVALID_SEAT);
            }
        }
        ...
        if (table._playerStatus[_seat_index].is_bao_ting() == true) {
            ...
            for (int i = 0; i < table._playerStatus[_seat_index]._hu_card_count; i++) {
                if (table._playerStatus[_seat_index]._hu_cards[i] == _send_card_data) {
                    action = table.analyse_chi_hu_card_ay(_seat_index, table.GRR._cards_index[_seat_index],
                            ...
                            chr, GameConstants.HU_CARD_TYPE_ZIMO);// 自摸
            ...
            for (int i = 0; i < GameConstants.MAX_INDEX - GameConstants.CARD_HUA_COUNT; i++) {
                ...
                if (count > 0) {
                    ...
                    // 检查打出哪一张牌后可以听牌
                    table._playerStatus[_seat_index]._hu_out_card_ting_count[ting_count] = table.get_ay_ting_card(
                            _seat_index, table._playerStatus[_seat_index]._hu_out_cards[ting_count],
            ...
            if (cbActionMask != GameConstants.WIK_NULL) {// 有杠
                ...
                for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
                    if ((m_gangCardResult.type[i] != GameConstants.GANG_TYPE_AN_GANG)
                            ...
                            || (table.check_gang_huan_zhang(GameConstants.GAME_TYPE_HENAN_AY, _seat_index,
    ...
    public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
        ...
        case GameConstants.WIK_GANG: // 杠牌操作
        {
            if (table._playerStatus[_seat_index].is_bao_ting() == false) {
                // TODO: 发牌时，有杠和听牌，点了牌但是没出牌，然后点了杠，先清空显示的听牌
                int[] tmp_ting_cards = new int[1];
                tmp_ting_cards[0] = 0;
                table.operate_chi_hu_cards(_seat_index, 1, tmp_ting_cards);
            }
        ...
        case GameConstants.WIK_ZI_MO: // 自摸
        {
            ...
            if (table.GRR._banker_player != seat_index) {
                ...
                // 庄家切换
                table._cur_banker = (table.GRR._banker_player + GameConstants.GAME_PLAYER + 1)
