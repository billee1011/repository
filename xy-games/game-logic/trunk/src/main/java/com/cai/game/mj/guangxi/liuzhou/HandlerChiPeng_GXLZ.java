...
public class HandlerChiPeng_GXLZ extends MJHandlerChiPeng<Table_GXLZ> {
	...
	public void exe(Table_GXLZ table) {
		...
		if (table.state_of_can_jie_pao[_seat_index] == false) {
			// 如果从有接炮胡的地方，点了吃碰杠，不算过圈
		...
		if (_action != GameConstants.WIK_PENG) {
			...
			table.get_cards_cant_out(table._card_can_not_out_after_chi[_seat_index], _action, cards_cant_out);
			for (int i = 0; i < hand_card_count; i++) {
				if (cards[i] == cards_cant_out[0] || cards[i] == cards_cant_out[1]) {
	...
	public boolean handler_player_be_in_room(Table_GXLZ table, int seat_index) {
		...
		if (seat_index == _seat_index && _action != GameConstants.WIK_PENG) {
			int[] cards_cant_out = new int[2];
			table.get_cards_cant_out(table._card_can_not_out_after_chi[_seat_index], _action, cards_cant_out);

			for (int i = 0; i < GameConstants.MAX_COUNT; i++) {
				if (hand_cards[i] == cards_cant_out[0] || hand_cards[i] == cards_cant_out[1]) {
