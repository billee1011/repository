...
public class PHZHandlerGang_MS extends PHZHandlerGang<SCPHZTable> {
...
//		}
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table._shang_zhuang_player = target_player;
			table.process_chi_hu_player_score_hh(target_player, this._provide_player, operate_card, false);
...
//			}
	 ...
	 * 
	 ***/
	protected boolean exe_gang(SCPHZTable table) {
	        ...
	        if(pai_count == 0)
	        {
	        	...
	        	if(all_hu_xi>=max_hu_xi)
	        	{
					...
					if (table.has_rule(GameConstants.GAME_RULE_HONG_HEI_DIAN)) {
						...
						if (table.is_mj_type(GameConstants.GAME_TYPE_LHQ_QD)) {
							if (hong_pai_count >= 8) {
