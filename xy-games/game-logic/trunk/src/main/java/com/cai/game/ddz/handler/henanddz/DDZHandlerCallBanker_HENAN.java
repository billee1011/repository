...
public class DDZHandlerCallBanker_HENAN extends DDZHandlerCallBanker{
	protected int _seat_index;
	protected int _game_status;
	//private int _current_player =MJGameConstants.INVALID_SEAT; 
	
	
	public DDZHandlerCallBanker_HENAN(){
	}
	...
	public  boolean handler_call_banker(DDZTable table,int seat_index,int call_banker,int qiang_bangker)
	{
		...
		if(table.has_rule(GameConstants.GAME_RULE_JIAO_DI_ZHU)){
			...
			if(call_banker > 0){
				...
				//出牌操作延迟
				table._call_banker_finish_scheduled=GameSchedule.put(new DDZOutCardHandleRunnable(table.getRoom_id(), table.GRR._banker_player,table,qiang_player,
			...
			if(!bhave_no_call){
				...
				//出牌操作延迟
				GameSchedule.put(new DDZOutCardHandleRunnable(table.getRoom_id(), GameConstants.INVALID_SEAT,table,qiang_player,
						call_player,qiang_action,call_action,true,true),
						1000, TimeUnit.MILLISECONDS);
				return true;
			}
...
//			table._auto_call_banker_scheduled=GameSchedule.put(new DDZAutoCallbankerRunnable(table.getRoom_id(), table,cur_qiang_player,0),
//					dealy, TimeUnit.SECONDS);
//		}else{
//			table._auto_call_banker_scheduled=GameSchedule.put(new DDZAutoCallbankerRunnable(table.getRoom_id(), table,cur_call_player,1),
//					dealy, TimeUnit.SECONDS);
//		}
