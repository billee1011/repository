...
public class Table_TDH_3D extends AbstractMJTable {
	...
	public HandlerSelectMagic_TDH_3D handler_select_magic;
	public MJHandlerOutCardBaoTing_TDH_3D _handler_out_card_bao_ting;
	...
	List<Integer> cards_list;

	public Table_TDH_3D() {
		super(MJType.GAME_TYPE_MJ_TDH_3D);
	}

	@Override
	protected void onInitTable() {
		_handler_chi_peng = new HandlerChiPeng_TDH_3D();
		_handler_dispath_card = new HandlerDispatchCard_TDH_3D();
		_handler_gang = new HandlerGang_TDH_3D();
		_handler_out_card_operate = new HandlerOutCardOperate_TDH_3D();
		_handler_out_card_bao_ting = new MJHandlerOutCardBaoTing_TDH_3D();
		handler_select_magic = new HandlerSelectMagic_TDH_3D();
	...
	public int get_ting_card(int[] cards, int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index) {
		...
		for (int i = 0; i < GameConstants_TDH.MAX_INDEX; i++) {
			...
			if(cards_list.contains(cbCurrentCard))
				max_ting_count ++;
			else
				continue;
			chr.set_empty();
	...
	protected void test_cards() {
		...
		// 七对
		int cards[] = new int[] { 0x38, 0x39, 0x42, 0x41, 0x14, 0x14, 0x25, 0x16, 0x16, 0x16, 0x17, 0x17, 0x17 };
