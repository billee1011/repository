...
class GenerateFengTable_FengChi_magic {
	...
	private static boolean put(int[] cards, int magic_count, boolean has_eye, boolean check_peng_hu, boolean check_258) {
		...
		TableManager.getInstance().put_feng_chi_hd(key, magic_count, has_eye);

		return true;
	}

	private static void generate_key(int[] cards, boolean has_eye, boolean check_peng_hu, boolean check_258, int eye_index) {
		if (false == put(cards, 0, has_eye, check_peng_hu, check_258))
			return;

		generate_sub_key(cards, 1, has_eye, check_peng_hu, check_258, eye_index);
	}

	private static void generate_sub_key(int[] cards, int magic_count, boolean has_eye, boolean check_peng_hu, boolean check_258, int eye_index) {
		for (int i = 0; i < GameUtilConstants.MAX_FENG_CARD_VALUE; i++) {
			if (0 == cards[i] || cards[i] == 1 || (cards[i] == 2 && i != eye_index))
				continue;
			...
			if (magic_count < GameUtilConstants.MAX_MAGIC_COUNT) {
				generate_sub_key(cards, magic_count + 1, has_eye, check_peng_hu, check_258, eye_index);
			}
	...
	private static void generate_table(boolean has_eye, boolean check_peng_hu, boolean check_258) {
		...
		generate_sub_table(cards, 1, false, check_peng_hu, check_258, -1);
	}
	...
	private static void generate_eye_table(boolean has_eye, boolean check_peng_hu, boolean check_258) {
		...
		for (int i = 0; i < GameUtilConstants.MAX_FENG_CARD_VALUE; i++) {
			cards[i] = 2;
			generate_key(cards, true, check_peng_hu, check_258, i);
			generate_sub_table(cards, 1, true, check_peng_hu, check_258, i);
			cards[i] = 0;
		}
	}

	private static void generate_sub_table(int[] cards, int weave_count, boolean has_eye, boolean check_peng_hu, boolean check_258, int eye_index) {
		for (int i = 0; i < GameUtilConstants.MAX_FENG_LOOPS; i++) {
			...
			generate_key(cards, has_eye, check_peng_hu, check_258, eye_index);

			if (weave_count < GameUtilConstants.MAX_WEAVE_COUNT)
				generate_sub_table(cards, weave_count + 1, has_eye, check_peng_hu, check_258, eye_index);
	...
	private static void generate() {
		...
		TableManager.getInstance().dump_feng_chi_hd();
