...
import com.cai.game.zhadan.handler.ncgz.Constants_NCGZ;

public class ZhaDanGameLogic_NCGZ extends ZhaDanGameLogic {

	public ZhaDanGameLogic_NCGZ() {
	...
	public int GetCardType(int cbCardData[], int cbCardCount) {
		...
		if (out_card_idnex.card_index[13] + out_card_idnex.card_index[14] == cbCardCount && cbCardCount >= 4) {
			return Constants_NCGZ.NCGZ_CT_BOMB;
		}
		if (out_card_idnex.card_index[one_idnex] == cbCardCount) {
			if (cbCardCount == 1) {
				return Constants_NCGZ.NCGZ_CT_SINGLE;
			} else if (cbCardCount == 2) {
				return Constants_NCGZ.NCGZ_CT_DOUBLE;
			} else if (cbCardCount == 3) {
				return Constants_NCGZ.NCGZ_CT_THREE;
			} else {
				return Constants_NCGZ.NCGZ_CT_BOMB;
			}
		}
	...
	public boolean CompareCard(int cbFirstCard[], int cbNextCard[], int cbFirstCount, int cbNextCount) {
		...
		if (cbFirstType == Constants_NCGZ.NCGZ_CT_BOMB && cbNextType < Constants_NCGZ.NCGZ_CT_BOMB) {
			return false;
		}
		if (cbNextType == Constants_NCGZ.NCGZ_CT_BOMB && cbFirstType < Constants_NCGZ.NCGZ_CT_BOMB) {
			return true;
		}
		if (cbNextType == cbFirstType) {
			if (cbNextType == Constants_NCGZ.NCGZ_CT_BOMB) {
	...
	public int search_out_card(int cbCardData[], int cbCardCount, int turn_card_data[], int turn_card_count) {
		...
		case Constants_NCGZ.NCGZ_CT_SINGLE:
		case Constants_NCGZ.NCGZ_CT_DOUBLE:
		case Constants_NCGZ.NCGZ_CT_THREE: {
			return search_out_single(cbCardData, cbCardCount, turn_card_data, turn_card_count);
		}
		case Constants_NCGZ.NCGZ_CT_BOMB: {
