...
public class MJHandlerQiShouHongZhong_HZJJ extends AbstractMJHandler<MJTable_HZJJ> {
    ...
    public void exe(MJTable_HZJJ table) {
    	 ...
    	 if ( table.has_rule(GameConstants_HZJJ.GAME_RULE_QIANG_ZHI_HU)) {
 			// 见炮胡
 			table.exe_jian_pao_hu(_seat_index, GameConstants.WIK_ZI_MO, 0x35);
 		}else{
    ...
    public boolean handler_operate_card(MJTable_HZJJ table, int seat_index, int operate_code, int operate_card) {
        ...
        // 下局胡牌的是庄家
        table.set_niao_card_hz(_seat_index, GameConstants.INVALID_VALUE, true, 0, 0x35,false); // 结束后设置鸟牌
        // 吃牌权位
