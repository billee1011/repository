 ...
 *
 */
public class Handler_first_Operate extends HHHandler<HongErShiTable_HY> {
	...
	public void reset(HongErShiTable_HY table, int start_check_tou_seat_index) {
	...
	public void exe(HongErShiTable_HY table) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (table.touCards[p]) {
				if (table.logic.countKingNumber(table.GRR._cards_data[p], table.GRR._card_count[p]) > 0) {
				...
				} else if (GameDescUtil.has_rule(table.gameRuleIndexEx, HongErShiConstants.RULE_7_IS_KING)
						&& table.logic.countSevenNumber(table.GRR._cards_data[p], table.GRR._card_count[p]) > 0) {
					table._playerStatus[p].add_action(HongErShiConstants.WIK_TOU);
					table._playerStatus[p].add_tou(7, HongErShiConstants.WIK_TOU, 1);
				}
	...
	public boolean handler_operate_card(HongErShiTable_HY table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		if (operate_code == HongErShiConstants.WIK_TOU) {
			...
			int seven_count = table.logic.countSevenNumber(table.GRR._cards_data[seat_index], table.GRR._card_count[seat_index]);
			if (!GameDescUtil.has_rule(table.gameRuleIndexEx, HongErShiConstants.RULE_7_IS_KING)) {
				seven_count = 0;
			}
			int[] send_card = new int[king_count + seven_count];
			int send_card_count = 0;
			...
			for (int i = 0; i < king_count; i++) {
				...
				if (!table.has_king_tou[seat_index]) {
					...
					table.GRR._weave_items[seat_index][cbWeaveIndex].weave_card[0] = HongErShiConstants.MAGIC_CARD_KING;

					table.has_king_tou[seat_index] = true;
				} else {
					int cbIndex = -1;
					for (int wc = 0; wc < table.GRR._weave_count[seat_index]; wc++) {
						if (table.GRR._weave_items[seat_index][wc].weave_kind != HongErShiConstants.WIK_TOU) {
							continue;
						}
					...
					if (cbIndex == -1) {
						...
						for (int c = 0; c < table.GRR._weave_items[seat_index][cbIndex].weave_card.length; c++) {
							if (table.GRR._weave_items[seat_index][cbIndex].weave_card[c] == 0) {
								table.GRR._weave_items[seat_index][cbIndex].weave_card[c] = HongErShiConstants.MAGIC_CARD_KING;
								break;
							}
						}
					}
				...
				send_card[send_card_count++] = send_card_data;
			}

			for (int ic = 0; ic < 53 && seven_count > 0; ic++) {
				int card = table.logic.switch_to_card_data(ic);
				if (table.logic.get_card_value(card) != HongErShiConstants.MAGIC_SEVEN_VALUE) {
					continue;
				}

				for (int icc = 0; icc < cardsIndex[ic]; icc++) {
					if (!table.has_king_tou[seat_index]) {
						int cbWeaveIndex = table.GRR._weave_count[seat_index];
						table.GRR._weave_count[seat_index]++;
						table.GRR._weave_items[seat_index][cbWeaveIndex].public_card = 0;
						table.GRR._weave_items[seat_index][cbWeaveIndex].center_card = card;
						table.GRR._weave_items[seat_index][cbWeaveIndex].weave_kind = HongErShiConstants.WIK_TOU;
						table.GRR._weave_items[seat_index][cbWeaveIndex].provide_player = seat_index;
						table.GRR._weave_items[seat_index][cbWeaveIndex].weave_card[0] = card;
						...
						for (int wc = 0; wc < table.GRR._weave_count[seat_index]; wc++) {
							...
							if (table.logic
									.get_card_value(table.GRR._weave_items[seat_index][wc].center_card) != HongErShiConstants.MAGIC_SEVEN_VALUE) {
								continue;
							}
							cbIndex = wc;
							break;
						...
						if (cbIndex == -1) {
							...
							for (int c = 0; c < table.GRR._weave_items[seat_index][cbIndex].weave_card.length; c++) {
								if (table.GRR._weave_items[seat_index][cbIndex].weave_card[c] == 0) {
									table.GRR._weave_items[seat_index][cbIndex].weave_card[c] = table.logic.switch_to_card_data(ic);
									break;
								}
							}
						}
			...
			table.operate_player_mo_card(seat_index, king_count + seven_count, send_card, hand_card_count, GameConstants.INVALID_SEAT, true);
			...
			if (table.logic.countKingNumber(table.GRR._cards_data[seat_index], table.GRR._card_count[seat_index]) > 0
					|| (GameDescUtil.has_rule(table.gameRuleIndexEx, HongErShiConstants.RULE_7_IS_KING)
							&& table.logic.countSevenNumber(table.GRR._cards_data[seat_index], table.GRR._card_count[seat_index]) > 0)) {
			...
			if (is_add_gang) {
				...
				table.GRR._weave_items[seat_index][add_gang_weave_index].weave_kind = GameConstants.GANG_TYPE_AN_GANG;
