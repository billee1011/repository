 ...
 */
public class MJHandlerDispatchCard_XTDGK extends MJHandlerDispatchCard<MJTable_XTDGK> {
	...
	public void exe(MJTable_XTDGK table) {
		...
		if (_type == GameConstants.HU_CARD_TYPE_GANG_KAI) {
			type = GameConstants_XTDGK.HU_CARD_TYPE_GANG_KAI;
		}
		// 胡牌检测
		...
		if (!table.getISHU(_seat_index)) {
			action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
					table.GRR._weave_count[_seat_index], _send_card_data, chr, type, _seat_index, GameConstants.INVALID_SEAT);// 自摸
		}

		if (action != GameConstants.WIK_NULL) {
			//if (table.fan_shu_when_zi_mo[_seat_index] > table.fan_shu_when_abandoned_zi_mo[_seat_index]) {
				// 添加动作
		...
		if (table.GRR._left_card_count > 0) {
			...
			int cbActionMask = table.analyse_gang_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
					table.GRR._weave_count[_seat_index], m_gangCardResult, true, _seat_index);
			if (cbActionMask != GameConstants.WIK_NULL) {

				boolean has_gang = false;

				for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
					if ((m_gangCardResult.type[i] != GameConstants.GANG_TYPE_AN_GANG) || (!table._player_result.is_bao_hu(_seat_index))
		...
		// 请胡
		if (table.checkQingHua(table.GRR._cards_index[_seat_index]) && !table._player_result.is_qing_hu(_seat_index)) {
			curPlayerStatus.add_action(GameConstants_XTDGK.WIK_QING_HU);
		}

		if (curPlayerStatus.has_action()) {// 有动作
			if (table._player_result.is_qing_hu(_seat_index)) {
				GameSchedule.put(new Runnable() {
					@Override
					public void run() {
						table.exe_out_card_qing_hu(_seat_index, _send_card_data, GameConstants.WIK_NULL);
					}
			...
			if (table._player_result.is_bao_hu(_seat_index) || table._player_result.is_qing_hu(_seat_index)) {
				if (table._player_result.is_qing_hu(_seat_index)) {
					...
					/*if (table.out_card_num[_seat_index] == 0) {
						...
						GameSchedule.put(new Runnable() {
							@Override
							public void run() {
								if (table._logic.remove_card_by_index(table.GRR._cards_index[_seat_index], _send_card_data) == false) {
									table.log_error("出牌删除出错");
									return;
								}
								table.exe_out_card_bao_ting(_seat_index, _send_card_data, GameConstants.WIK_NULL);
							}
						}, 500, TimeUnit.MILLISECONDS);
					//}
				}
	...
	public boolean handler_operate_card(MJTable_XTDGK table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_NULL) {
			...
			if (table._playerStatus[seat_index].has_zi_mo()
					&& (table._player_result.is_bao_hu(_seat_index) || table._player_result.is_qing_hu(_seat_index))) {
				table._playerStatus[seat_index].chi_hu_round_invalid();
				table.fan_shu_when_abandoned_zi_mo[seat_index] = table.fan_shu_when_zi_mo[seat_index];
		...
		case GameConstants_XTDGK.WIK_QING_HU: {
			...
			// 请胡
			table.exe_out_card_qing_hu(seat_index, operate_card, GameConstants.WIK_NULL);
			return true;
		}
		}
	...
	public boolean handler_player_out_card(MJTable_XTDGK table, int seat_index, int card) {
		...
		if (table._playerStatus[seat_index].get_status() == GameConstants.Player_Status_Bao_Hu) {
			...
			// 出牌
			table.exe_out_card(_seat_index, card, GameConstants_XTDGK.HU_CARD_TYPE_GANG_PAO);
		} else {
			// 出牌
