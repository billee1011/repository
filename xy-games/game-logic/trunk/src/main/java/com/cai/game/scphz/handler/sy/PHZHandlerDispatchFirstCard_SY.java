...
public class PHZHandlerDispatchFirstCard_SY extends PHZHandlerDispatchCard<SCPHZTable> {
...
//		_send_card_data = 0x18;
		...
		{		
			...
			chr.set_empty();
			int action_type =  GameConstants.WIK_NULL;
			if(i == _seat_index){
				int hu_xi[] = new int[1];
				action_type = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i],
						table.GRR._weave_count[i],i,_seat_index,_send_card_data, chr, card_type,hu_xi,true);// 自摸
	
				if (action_type != GameConstants.WIK_NULL) {
					// 添加动作
			...
			if(((ti_count + sao_count >= 5))||(ti_count >= 3)||(sao_count >= 5))
			{	  	
        		...
        		table.setChiHuRight(i,chr);
        		chr.opr_or(GameConstants.CHR_SY_TIAN_HU);
