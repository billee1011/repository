...
public class HandlerOutCardOperate_ZYZJ extends MJHandlerOutCardOperate<Table_ZYZJ> {
	...
	public void exe(Table_ZYZJ table) {
		...
		for(int i = 0;i < hand_card_count;i++){
			int temp = cards[i];
			if(temp > GameConstants.CARD_ESPECIAL_TYPE_LIANG_ZHANG)
				temp -= GameConstants.CARD_ESPECIAL_TYPE_LIANG_ZHANG;
		...
		if (table.is_ji_index(table._logic.switch_to_card_index(_out_card_data))) {
			if (table.chong_feng_ji_seat_yj == -1 && _out_card_data == GameConstants_ZYZJ.YJ_CARD) {
				...
				ioutcard += GameConstants_ZYZJ.CHONG_FENG_JI_BIAO_ZHI;
			}
		...
		if(_out_card_data == GameConstants_ZYZJ.YI_TONG_CARD && table.has_rule(GameConstants_ZYZJ.GAME_RULE_LAIZI_ZYZJ)){
			ioutcard += GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI;
		}
		
		table.operate_out_card(_out_card_player, 1, new int[] { ioutcard }, GameConstants.OUT_CARD_TYPE_MID, GameConstants.INVALID_SEAT);
		...
		if (bAroseAction == false) {
			...
			if (table.is_ji_index(table._logic.switch_to_card_index(_out_card_data))) {
				...
				if (table.chong_feng_ji_seat_yj == -1 && _out_card_data == GameConstants_ZYZJ.YJ_CARD) {
					...
					table._player_result.qiang[_out_card_player] = 1;
				}
				if (table.chong_feng_ji_seat_bt == -1 && _out_card_data == GameConstants_ZYZJ.BA_TONG_CARD) {
	...
	public boolean handler_operate_card(Table_ZYZJ table, int seat_index, int operate_code, int operate_card) {
		...
		//case GameConstants.WIK_SUO_PENG_2:{
			...
			if(target_action == GameConstants.WIK_PENG){
				cbRemoveCard = new int[] { target_card, target_card };
			} /*else if (target_action == GameConstants.WIK_SUO_PENG_1) {
				cbRemoveCard = new int[] { GameConstants_ZYZJ.YI_TONG_CARD, target_card };
		...
		case GameConstants.WIK_CHI_HU: {
			...
			if (jie_pao_count > 0) {
				...
				if(has_shao){
					table.process_reset_gang_score(_out_card_player);
				}
				
				if(table._playerStatus[_out_card_player]._hu_card_count > 0){
					table.process_ji_fen(-1);
				}else{
					table.process_ji_fen(_out_card_player);
				}
				
				//table.process_reponsibility_ji_fen();
				
				table.hutype = 2;
				table.fangpaoplayer[_out_card_player] = 1;
				if (jie_pao_count > 1) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					if (_type == GameConstants_ZYZJ.OutCard_Type_Di_Hu)
						table.GRR._chi_hu_rights[i].opr_or(GameConstants_ZYZJ.CHR_DI_HU);
