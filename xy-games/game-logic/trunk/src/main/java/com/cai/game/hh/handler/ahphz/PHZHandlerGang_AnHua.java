...
public class PHZHandlerGang_AnHua extends HHHandlerGang<AnHuaHHTable> {
	 ...
	 ***/
	@Override
	protected boolean exe_gang(AnHuaHHTable table) {
		...
		if ((_depatch == true) && (table._ti_two_long[_seat_index] == false) && (table._is_xiang_gong[_seat_index] == false)) {
			...
			if (pai_count == 0) {
				...
				if (all_hu_xi >= 15) {
					...
					if (table._xiao_pai_count[_seat_index] >= 18) {
						chr.opr_or(GameConstants.CHR_SHI_BA_XIAO);
					}
					...
					if (table.has_rule(GameConstants.GAME_RULE_DI_TUAN_YUAN)) {
						table._tuan_yuan_count[_seat_index] = table._logic.calculate_tuan_yuan_count(table._hu_weave_items[_seat_index],
