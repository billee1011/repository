...
public class MJTable_ZHUZHOUZZ extends AbstractMJTable {
	...
	*/
	...
	public MJTable_ZHUZHOUZZ() {
		super(MJType.GAME_TYPE_MJ_ZHUZHOU_ZZ);
	}
	...
	protected void onInitTable() {
		_handler_dispath_card = new MJHandlerDispatchCard_ZHUZHOUZZ();
		_handler_out_card_operate = new MJHandlerOutCardOperate_ZHUZHOUZZ();
		_handler_gang = new MJHandlerGang_ZHUZHOUZZ();
		_handler_chi_peng = new MJHandlerChiPeng_ZHUZHOUZZ();
				...
				if (GRR._chi_hu_rights[i].is_valid()) {
					...
					if (type == GameConstants.CHR_ZI_MO) {
						...
						if (GRR._count_pick_niao > 0
								&& !GameDescUtil.has_rule(gameRuleIndexEx, GameConstants.GAME_RULE_ZHUANG_NIAO)) {
	...
	public int getZuoPiaoScore() {
		...
		if (has_rule(GameConstants.GAME_RULE_HUNAN_CS_ZUOPIAO1)) {
			...
			return 3;
		} else if (has_rule(GameConstants.GAME_RULE_HUNAN_CS_DING_NIAO1)) {
			return 4;
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				for (int j = 0; j < GRR._gang_score[i].gang_count; j++) {
					for (int k = 0; k < getTablePlayerNumber(); k++) {
						lGangScore[k] += GRR._gang_score[i].scores[j][k]* get_bei_lv();;// 杠牌，每个人的分数
					}
				}
