...
import com.cai.future.runnable.RevomeHunMiddleCardRunnable_SMX;
 ...
 *
 */
public class MJHandlerHun_JiangSu_JH  extends AbstractMJHandler<MJTable_JiangSu_JH> {
	...
	public void exe(MJTable_JiangSu_JH table) {
		...
		if(table._b_double){
			if(table._touzi_count == 0){
				// 效果
				table.operate_effect_action(GameConstants.INVALID_SEAT, GameConstants.EFFECT_ACTION_TYPE_DOUBLE, 1, new long[] { 7 },
						1, GameConstants.INVALID_SEAT);
			}else{
				// 效果
				table.operate_effect_action(GameConstants.INVALID_SEAT, GameConstants.EFFECT_ACTION_TYPE_DOUBLE, 1, new long[] { table._touzi_count },
						1, GameConstants.INVALID_SEAT);
			}
		}
		...
		//延迟调度打点结束（发牌）
		GameSchedule.put(new RevomeHunMiddleCardRunnable_JIANGSU_ZZ(table.getRoom_id(), _banker),
				2, TimeUnit.SECONDS);
