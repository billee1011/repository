...
public class MJHandlerDispatchCard_ShanXi_JINGYUE extends MJHandlerDispatchCard<MJTable_ShanXi_JINGYUE> {
	...
	public void exe(MJTable_ShanXi_JINGYUE table) {
		...
		if (table._playerStatus[_seat_index].is_bao_ting()) {
			action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
					table.GRR._weave_count[_seat_index], _send_card_data, chr, GameConstants_SXJY.HU_CARD_TYPE_ZI_MO,
					_seat_index, false);
		}
		if (GameConstants.WIK_NULL != action && table._playerStatus[_seat_index].is_bao_ting()) {
			if (table.has_rule(GameConstants_SXJY.GAME_RULE_SHANXI_NENG_HU_BI_HU)) {
			...
			if (table._playerStatus[_seat_index].is_bao_ting()) {
				if (0 != cbActionMask) {
					for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
						...
						if (m_gangCardResult.cbCardData[i] > GameConstants_SXJY.CARD_ESPECIAL_TYPE_GANG_NAN_FENG) {
							m_gangCardResult.cbCardData[i] -= GameConstants_SXJY.CARD_ESPECIAL_TYPE_GANG_NAN_FENG;
						}
						...
						// 如果是风杠的牌型
						if (GameConstants_SXJY.GANG_TYPE_AN_FENG_GANG == m_gangCardResult.type[i]) {
							table.GRR._cards_index[_seat_index][tmp_card_index]--;
							table.GRR._cards_index[_seat_index][tmp_card_index + 1]--;
							table.GRR._cards_index[_seat_index][tmp_card_index + 2]--;
							table.GRR._cards_index[_seat_index][tmp_card_index + 3]--;
						} else {
						...
						if (is_ting_state_after_gang) {
							if (m_gangCardResult.type[i] == GameConstants_SXJY.GANG_TYPE_AN_FENG_GANG) {
								...
								curPlayerStatus.add_feng_gang(m_gangCardResult.cbCardData[i], _seat_index,
		...
		if (curPlayerStatus.has_action()) { // 有动作
			...
			SysParamModel sysParamModel = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(333).get(12000);
			int delay = 600;
			if (sysParamModel != null) {
				delay = sysParamModel.getVal1();
			}
			if (table._playerStatus[_seat_index].is_bao_ting()) {
				GameSchedule.put(new OutCardRunnable(table.getRoom_id(), _seat_index, _send_card_data), delay,
	...
	public boolean handler_operate_card(MJTable_ShanXi_JINGYUE table, int seat_index, int operate_code,
		...
		// if(operate_card==GameConstants_SXJY.DONG_FENG_CARD+GameConstants_SXJY.CARD_ESPECIAL_TYPE_GANG_NAN_FENG){
		// no_feng_operate_card=operate_card-GameConstants_SXJY.CARD_ESPECIAL_TYPE_GANG_NAN_FENG;
		// }
		...
		case GameConstants.WIK_FENG_GANG: {
			for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
				if (operate_card == m_gangCardResult.cbCardData[i]) {
					if (m_gangCardResult.cbCardData[i] != _send_card_data) {
						table.exe_gang(_seat_index, _seat_index, operate_card, operate_code, m_gangCardResult.type[i],
