...
public class MJHandlerGang_WEIHE extends MJHandlerGang<MJTable_WEIHE> {
	 ...
	 ***/
	@Override
	protected boolean exe_gang(MJTable_WEIHE table) {
		...
		int hand_card_count = table._logic.switch_to_cards_data_sichuan(table.GRR._cards_index[_seat_index], cards,table.ding_que_pai_se[_seat_index]);
		table.changCards(cards, _seat_index);
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				// 暗杠每人2分
				table.GRR._gang_score[_seat_index].scores[cbGangIndex][i] = - 1*table.getDiScore();// 暗杠，其他玩家扣分
				table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += 1*table.getDiScore();// 一共加分
			}

			table._player_result.an_gang_count[_seat_index]++;
		} else if (GameConstants.GANG_TYPE_JIE_GANG == _type) {
			// 放杠三分
			int score = 1*table.getDiScore();//table.getTablePlayerNumber() - 1;
