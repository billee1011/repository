...
import com.cai.game.pdk.PDKGameLogic_YUEY;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PDK_JDYUEY_Table extends PDKTable {
	public PDKGameLogic_YUEY _logic;

	public PDK_JDYUEY_Table() {
		_logic = new PDKGameLogic_YUEY();
	}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_enter_back = new boolean[getTablePlayerNumber()];
		...
		_handler_out_card_operate = new PDKHandlerOutCardOperate_JDYUEY();
		_bomb_cell_score = 10;
		if (this.matchId != 0) {
			_bomb_cell_score = 4;
		}
	...
	private void shuffle(int repertory_card[], int mj_cards[]) {
		...
		for (int i = 0; i < count; i++) {
			for (int j = 0; j < this.get_hand_card_count_max(); j++) {
				...
				if (!this.has_rule(GameConstants.GAME_RULE_NO_ZHA_NIAO) && GRR._cards_data[i][j] == 0x2A) {
					_hong_tao_palyer = i;
				}
			}
	...
	private void test_cards() {
		int cards[] = new int[] { 0x01, 0x11, 0x21, 0x1d, 0x0d, 0x2d, 0x03, 0x13, 0x23, 0x14, 0x24, 0x34, 0x16, 0x26,
				0x37, 0x28, 0x05, 0x07, 0x27, 0x17, 0x18, 0x08, 0x38, 0x19, 0x29, 0x39, 0x0a, 0x2a, 0x3b, 0x2b, 0x3b,
				0x0b, 0x01, 0x11, 0x21, 0x1d, 0x0d, 0x2d, 0x03, 0x13, 0x23, 0x14, 0x24, 0x34, 0x16, 0x26, 0x37, 0x28,
				0x05, 0x07, 0x27, 0x17, 0x18, 0x08, 0x38, 0x19, 0x29, 0x39, 0x0a, 0x2a, 0x3b, 0x2b, 0x3b, 0x0b };
	...
	public void cal_score_pdk_jd(int end_score[]) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			if (this.GRR._card_count[i] > 1) {
				...
				if (this.has_rule(GameConstants.GAME_RULE_HONGTAO10_ZANIAO_ADD_FIVE)) {
					if (_hong_tao_palyer == win_player) {
						end_score[i] -= 5;
					} else if (_hong_tao_palyer == i) {
						end_score[i] -= 5;
				...
				if (this.has_rule(GameConstants.GAME_RULE_HONGTAO10_ZANIAO_ADD_TEN)) {
					if (_hong_tao_palyer == win_player) {
						end_score[i] -= 10;
					} else if (_hong_tao_palyer == i) {
						end_score[i] -= 10;
	...
	public boolean handler_game_finish_pdk_jd(int seat_index, int reason) {
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if (_cur_round >= _game_round) {// 局数到了
				...
				for (int i = 0; i < this.getTablePlayerNumber(); i++) {
					this._player_result.game_score[i] = this._game_score[i];
				}
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((GameConstants.GS_MJ_FREE != _game_status && GameConstants.GS_MJ_WAIT != _game_status)
			...
			if (this._handler != null) {
				...
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					...
					if (has_rule(GameConstants.GAME_RULE_DISPLAY_CARD)) {
						...
						if (i == seat_index) {
							...
							tableResponse_pdk.addCardCount(0);
						}
					}
				...
				// 手牌--将自己的手牌数据发给自己
				Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
				if (!has_rule(GameConstants.GAME_RULE_FANG_ZUO_BI) || seat_index == this._current_player) {
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		...
		if (type == MsgConstants.REQUST_ENTER_BACK) {
			Enter_Back_Request req = PBUtil.toObject(room_rq, Enter_Back_Request.class);
			logger.info("房间[" + this.getRoom_id() + "]" + "_enter_back:" + req.getIsBack());
			if (req.getIsBack() == 1) {
				_enter_back[seat_index] = true;
			} else {
				_enter_back[seat_index] = false;
			}

			if (_current_player == seat_index && !_enter_back[seat_index]) {
				int can_out_card_data[] = new int[get_hand_card_count_max()];
				if (_logic.Player_Can_out_card(GRR._cards_data[seat_index], GRR._card_count[seat_index],
						_turn_out_card_data, _turn_out_card_count, can_out_card_data) == 0
						&& !_enter_back[seat_index]) {
					if (has_rule(GameConstants.GAME_RULE_OFFLINE_NO_AUTO)) {
						if (get_players()[seat_index].isOnline()) {
							_auto_out_card_scheduled = GameSchedule.put(new PDKAutoOutCardRunnable(getRoom_id(),
									seat_index, this, GameConstants.PDK_CT_PASS), 1, TimeUnit.SECONDS);
						}
					} else {
						_auto_out_card_scheduled = GameSchedule.put(
								new PDKAutoOutCardRunnable(getRoom_id(), seat_index, this, GameConstants.PDK_CT_PASS),
								1, TimeUnit.SECONDS);
					}
				}
			}
