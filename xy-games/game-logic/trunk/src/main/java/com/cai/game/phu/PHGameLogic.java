...
public class PHGameLogic {

	private static Logger logger = Logger.getLogger(PHGameLogic.class);
   ...
   public boolean check_lou_weave(int cards_index[],int cur_card,int action,int type_count[],int type_eat_count,LouWeaveItem lou_weave_item[],
    ...
    public void calculate_game_weave_score(int game_mid_score[],int game_weave_score[],int score[],int user_count,boolean add,int match_times)
    {
    	if(add == true)
    	{
    		for(int i = 0; i< user_count;i++)
	    	{
	    		game_weave_score[i] += score[i]*match_times;
	    	}
    	}
    	else
    	{
    	...
    	for(int i = 0; i< user_count; i++ )
    	{
    		game_mid_score[i] += score[i]*match_times;
    	}
    }
    public void calculate_game_mid_score(int seat_index,int provider,int user_count,boolean is_other ,int score[],int temp_score)
    {
    	if(is_other == false)
    	{
    		score[seat_index] = temp_score*(user_count -1);
    		score[provider] = - score[seat_index] ;
    		return ;
    	}
    	for(int i = 0; i<user_count; i++ )
    	{
    		if(i == seat_index)
    			continue;
    		score[i] = - temp_score;
    		score[seat_index] -=  score[i];
    	}
    	
    }
    public void  get_hu_weave_index(AnalyseItem analyseItem,int weave_count , int operate_card, ChiHuRight chiHuRight,int cannot_peng[],int cannot_peng_count,int warning)
    {
    	if(!(chiHuRight.opr_and(GameConstants.CHR_TIAN_HU_PH)).is_empty())
    		return ;
    	if(!(chiHuRight.opr_and(GameConstants.CHR_DI_HU_PH)).is_empty())
    		return ;
    	int hu_pai_wik = GameConstants.WIK_NULL;
    	int sao_peng = 0;
    	int dui_zi =0;
    	int long_count = 0;
    	for(int i = 0; i<7;i++)
    	{
    		...
    		{
    			if(analyseItem.cbCenterCard[i] == operate_card)
    			{
    				hu_pai_wik = analyseItem.cbWeaveKind[i];
    			}
    			sao_peng++;
    			break;
    		}
    		...
    		case GameConstants.WIK_PENG_OUT:
    		{
    			sao_peng++;
    			if(analyseItem.cbCenterCard[i] == operate_card)
    			{
    	...
    	case GameConstants.WIK_PAO:
    	{
    		chiHuRight.opr_or(GameConstants.CHR_PAO_HU_PH);
    		break;
    	}
    	case GameConstants.WIK_TI_LONG:
		case GameConstants.WIK_AN_LONG:
		{
			chiHuRight.opr_or(GameConstants.CHR_TI_LONG_HU_PH);
			break;
		}
		case GameConstants.WIK_PENG:
		case GameConstants.WIK_PENG_OUT:
		{
			if(sao_peng == 5)
			{
				chiHuRight.opr_or(GameConstants.CHR_WU_PFU_HU_PH);
			}
			else if(sao_peng == 4)
			{
				chiHuRight.opr_or(GameConstants.CHR_SI_QING_HU_PH);
			}
			else if(sao_peng == 3)
			{
				chiHuRight.opr_or(GameConstants.CHR_SAN_DA_HU_PH);
			}
			else 
			{
				chiHuRight.opr_or(GameConstants.CHR_PENG_HU_PH);
			}
			break;
		}
		case GameConstants.WIK_SAO:
		...
		case GameConstants.WIK_KAN:
		{
			if(sao_peng == 5)
			{
				chiHuRight.opr_or(GameConstants.CHR_WU_WFU_HU_PH);
			}
			else if(sao_peng == 4)
			{
				chiHuRight.opr_or(GameConstants.CHR_WEI_SI_HU_PH);
			}
			else if(sao_peng == 3)
			{
				chiHuRight.opr_or(GameConstants.CHR_W_SAN_DA_HU_PH);
			}
			else 
			{
				chiHuRight.opr_or(GameConstants.CHR_WEI_HU_PH);
			}
			break;
		} 
		default:
			chiHuRight.opr_or(GameConstants.CHR_PING_HU_PH);
    	}
    	return ;
    }
    public int get_weave_hu_fen(WeaveItem weave_items[],int weave_count ,int weave_kind,int peng_sao_count[])
    {
    	int score = 0;
    	peng_sao_count[0] = 0;
    	if(weave_kind == GameConstants.WIK_PAO)
    		return 4;
    	if(weave_kind == GameConstants.WIK_AN_LONG)
    		return 10;
    	if(weave_kind == GameConstants.WIK_TI_LONG)
    		return 8;
    	for(int i = 0; i < weave_count; i++)
    	{
    		switch(weave_items[i].weave_kind)
    		{
    		case GameConstants.WIK_PAO:
    		case GameConstants.WIK_AN_LONG:
    		case GameConstants.WIK_TI_LONG:
    	...
    	if(weave_kind == GameConstants.WIK_PENG||weave_kind == GameConstants.WIK_PENG_OUT)
    	{
    		switch(peng_sao_count[0])
    		{
    		case 0:
    		case 1:
      	...
      	if(weave_kind == GameConstants.WIK_SAO||weave_kind == GameConstants.WIK_CHOU_SAO)
    	{
    		switch(peng_sao_count[0])
    		{
    		case 0:
    ...
    public boolean is_si_qing(int cards_index[],WeaveItem weave_items[],int weave_count,int warning){
    	if(warning != 0)
    		return false;
    	int peng_sao_count = 0;
       	for(int i = 0; i < weave_count; i++)
    	{
    		switch(weave_items[i].weave_kind)
    		{
    ...
    public int get_anaylse_hu_fen(AnalyseItem analyseItem,int analyse_kind)
    {
    	int score = 0;
    	int peng_sao_count = 0;
    	if(analyse_kind == GameConstants.WIK_PAO)
    		return 4;
    	if(analyse_kind == GameConstants.WIK_AN_LONG)
    		return 10;
    	if(analyse_kind == GameConstants.WIK_TI_LONG)
    		return 8;
//    	for(int i = 0; i < 5; i++)
//    	{
//    		switch(analyseItem.cbWeaveKind[i])
...
//    		}
//    	}
    	if(analyse_kind == GameConstants.WIK_PENG||analyse_kind == GameConstants.WIK_PENG_OUT)
    	{
    		switch(peng_sao_count)
    		{
    		case 0:
    		case 1:
    		case 2: score = 1;break;
      	...
      	if(analyse_kind == GameConstants.WIK_SAO||analyse_kind == GameConstants.WIK_CHOU_SAO||analyse_kind == GameConstants.WIK_KAN)
    	{
    		switch(peng_sao_count)
    		{
    		case 0:
    ...
    public boolean is_sx_wu_qian_nian(AnalyseItem analyseItem,WeaveItem weaveItems[],
    		int weaveCount,int cbMingIndexTemp[] ){
    	int weave_hu_xi = 0;
	...
	public boolean is_card_to_weave(int card,WeaveItem weave_items[],int weave_count)
	{
		for(int i = 0; i<weave_count;i++ )
		{
			if(weave_items[i].center_card == card)
			{
				return false;
			}
		}
...
//		}
	...
	public boolean analyse_card_fphz(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index,int cur_card,
				List<AnalyseItem> analyseItemArray, boolean has_feng, int hu_xi[],boolean dispatch) {
			// 计算数目
			int cbCardCount = get_card_count_by_index(cards_index);
			hu_xi[0] = 0;
			//跑胡判断
			...
			WeaveItem kan_WeaveItem[] = new WeaveItem[7]; 
			for(int i = 0; i< 7; i++)
			{
				kan_WeaveItem[i] = new WeaveItem();
			}
			int pao_count = 0;
			int kan_count = 0;
			boolean  b_pao = false;
			...
			for(int i =0;i< GameConstants.MAX_HH_INDEX;i++){
				...
				if(cards_index[i] == 3&&seat_index == provider_index)
				{
					cards_index[i] =0;
					kan_WeaveItem[kan_count].center_card = switch_to_card_data(i);
					kan_WeaveItem[kan_count].weave_kind = GameConstants.WIK_KAN;
				...
				for(int i = 0; i< GameConstants.MAX_HH_INDEX;i++)
				{
					if(cards_index[i]==2)
					{
						...
						for (int j = 0; j < cbWeaveCount; j++) {
							...
							analyseItem.hu_xi[j] = get_anaylse_hu_fen(analyseItem,analyseItem.cbWeaveKind[j]);
						...
						if(b_pao == true){
							for(int k = 0; k<pao_count;k++)
							{
								...
								analyseItem.hu_xi[count] = get_anaylse_hu_fen(analyseItem,analyseItem.cbWeaveKind[count]);
						...
						if(b_kan == true){
							for(int k = 0; k<kan_count;k++)
							{
								analyseItem.cbWeaveKind[count] = kan_WeaveItem[k].weave_kind;
								analyseItem.cbCenterCard[count] = kan_WeaveItem[k].center_card;
								analyseItem.hu_xi[count] = get_anaylse_hu_fen(analyseItem,analyseItem.cbWeaveKind[count]);
								hu_xi[0] += analyseItem.hu_xi[count] ;
								get_weave_card(kan_WeaveItem[k].weave_kind, kan_WeaveItem[k].center_card, analyseItem.cbCardData[count]);
								count ++;
							}
						}
...
//								return false;
			...
			for(int i = 0; i<GameConstants.MAX_HH_INDEX; i++)
			{
				...
				if(cards_index[i] == 3)
				{
					...
					if((cur_card == switch_to_card_data(i)) && (seat_index != provider_index) && (dispatch == true))
						kindItem[cbKindItemCount].cbWeaveKind = GameConstants.WIK_PENG;
					else 	if((cur_card == switch_to_card_data(i)) && (seat_index != provider_index) && (dispatch == false))
						kindItem[cbKindItemCount].cbWeaveKind = GameConstants.WIK_PENG_OUT;
					else
				...
				//大小搭吃
				if((i<10)&&((cards_index[i] == 2)||((i == switch_to_card_index(cur_card))&&(cards_index[i] == 3) ))&&(cards_index[(i+10)%GameConstants.MAX_HH_INDEX]>=1)&&((cards_index[(i+10)%GameConstants.MAX_HH_INDEX]<3)||(((i+10)%GameConstants.MAX_HH_INDEX)==switch_to_card_index(cur_card)))){
					kindItem[cbKindItemCount].cbCardIndex[0] = i;
					kindItem[cbKindItemCount].cbCardIndex[1] = i;
				...
				//大小搭吃
				if((i<10)&&(cards_index[i] >=1)&&((cards_index[(i+10)%GameConstants.MAX_HH_INDEX]==2)||((((i+10)%GameConstants.MAX_HH_INDEX) == switch_to_card_index(cur_card))&&(cards_index[(i+10)%GameConstants.MAX_HH_INDEX] == 3)))
...
//							return false;
	...
	public int get_action_rank(int player_action) {
		...
		if (player_action == GameConstants.WIK_ZI_MO_SL) {
			return 80;
		}
		if (player_action == GameConstants.WIK_ZI_MO_QXD) {
			return 70;
		}
	
		if (player_action == GameConstants.WIK_ZI_MO_WH) {
			return 60;
		}
		// 自摸牌等级
	...
	public int get_chi_hu_action_rank_ph(int seat_index,ChiHuRight chiHuRight,int times,boolean is_zhuang_pao) {
		int fen_shu = 1;
		boolean is_times = true;
		if(!(chiHuRight.opr_and(GameConstants.CHR_PING_HU_PH)).is_empty()){
			fen_shu = 4;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_PENG_HU_PH)).is_empty()){
			fen_shu =5;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_WEI_HU_PH)).is_empty()){
			fen_shu =6;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_PAO_HU_PH)).is_empty()){
			fen_shu =8;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_TI_LONG_HU_PH)).is_empty()){
			fen_shu =12;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_SAN_DA_HU_PH)).is_empty()){
			fen_shu =9;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_W_SAN_DA_HU_PH)).is_empty()){
			fen_shu =10;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_SI_QING_HU_PH)).is_empty()){
			fen_shu =9;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_WEI_SI_HU_PH)).is_empty()){
			fen_shu =10;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_WU_PFU_HU_PH)).is_empty()){
			fen_shu = 40;
			is_times = false;
			is_zhuang_pao = false;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_WU_WFU_HU_PH)).is_empty()){
			fen_shu = 40;
			is_times = false;
			is_zhuang_pao = false;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_SHL_HU_PH)).is_empty()){
			fen_shu = 40;
			is_times = false;
			is_zhuang_pao = false;
		}
		if(!(chiHuRight.opr_and(GameConstants.CHR_XQD_HU_PH)).is_empty()){
			fen_shu = 40;
			is_times = false;
			is_zhuang_pao = false;
		}
	...
	public int get_chi_hu_lian_zhong_ph(int seat_index,ChiHuRight chiHuRight,int times)
	{
	    int fen_shu = 1;
	...
	public static void main(String[] args) {
		// 插入扑克
		PHGameLogic logic = new PHGameLogic();
