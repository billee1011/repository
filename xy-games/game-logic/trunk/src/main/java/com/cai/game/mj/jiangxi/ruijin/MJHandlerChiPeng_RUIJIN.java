...
public class MJHandlerChiPeng_RUIJIN extends MJHandlerChiPeng<MJTable_RUIJIN> {
	 ...
	 */
	@Override
	public boolean handler_player_out_card(MJTable_RUIJIN table, int seat_index, int card) {
	...
	public boolean handler_player_be_in_room(MJTable_RUIJIN table, int seat_index) {
		...
		/*
		 * int out_ting_count =
		 * table._playerStatus[seat_index]._hu_out_card_count;
		 * 
		 * if ((out_ting_count > 0) && (seat_index == _seat_index)) { for (int j
		 * = 0; j < hand_card_count; j++) { for (int k = 0; k < out_ting_count;
		 * k++) { if (hand_cards[j] ==
		 * table._playerStatus[seat_index]._hu_out_card_ting[k]) { hand_cards[j]
		 * += GameConstants.CARD_ESPECIAL_TYPE_TING; break; } } } }
		 */
		 ...
		 * 
		 * for (int i = 0; i < out_ting_count; i++) { int ting_card_cout =
		 * table._playerStatus[seat_index]._hu_out_card_ting_count[i];
		 * roomResponse.addOutCardTingCount(ting_card_cout);
		 * roomResponse.addOutCardTing(
		 * table._playerStatus[seat_index]._hu_out_card_ting[i] +
		 * GameConstants.CARD_ESPECIAL_TYPE_TING); Int32ArrayResponse.Builder
		 * int_array = Int32ArrayResponse.newBuilder(); for (int j = 0; j <
		 * ting_card_cout; j++) {
