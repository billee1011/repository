...
public class PHZHandlerOutCardOperate_LBA extends PHZHandlerOutCardOperate<CHDPHZTable> {
	@Override
	public void exe(CHDPHZTable table) {
		...
		while (loop < table.getTablePlayerNumber()&&table._is_di_hu ==true) {
...
//		}
		...
		case GameConstants.WIK_PAO: // 杠牌操作
		{
			...
			int pao_type[] =  new int[1];
			int action = table.estimate_player_respond_phz_chd(target_player,_out_card_player ,_out_card_data, pao_type,false);
			if(action != GameConstants.WIK_NULL){
				table.exe_gang(target_player, _out_card_player, _out_card_data, action,pao_type[0], false,true, false,1000);
			}
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_lba(target_player, _out_card_player, _out_card_data, false);
