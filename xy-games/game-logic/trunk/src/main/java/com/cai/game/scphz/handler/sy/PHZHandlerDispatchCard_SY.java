 ...
 *
 */
public class PHZHandlerDispatchCard_SY extends PHZHandlerDispatchCard<SCPHZTable> {
	...
	public void exe(SCPHZTable table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			for(int i = 0; i<table.getTablePlayerNumber();i++){
				...
				{
					table.GRR._game_score[i] = -(table.getTablePlayerNumber()-1);
				}
				else
					table.GRR._game_score[i] = 1;
				table._player_result.game_score[i] += table.GRR._game_score[i];
					
			}
			table._player_result.liu_zi_fen[table._cur_banker]++;
			// 流局
		...
		// 玩家出牌 响应判断,是否有提 暗龙
		 ...
		 if(an_long_count > 0)
		 {
			 for(int i = 0; i<table.GRR._weave_count[_seat_index];i++){
				 if(table.GRR._weave_items[_seat_index][i].weave_kind == GameConstants.WIK_AN_LONG||
						 table.GRR._weave_items[_seat_index][i].weave_kind == GameConstants.WIK_AN_LONG_LIANG)
					 end_long_count++;
			 }
			 end_long_count +=an_long_count; 
		 }
		 if(an_long_count > 0 )
		 {
...
//	
		 ...
		 if(end_long_count >= 2)
		 {
			 table._ti_mul_long[_seat_index] = end_long_count-1;
		 }
...
//		}
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if(table._guo_hu_pai_cards[i][table._logic.switch_to_card_index(_send_card_data)]>0)
				continue;
			...
			action_pao[i] = table.estimate_player_respond_yj(i, _seat_index, _send_card_data, pao_type[i], true);
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if ((action_pao[i] != GameConstants.WIK_NULL) && (bHupai == 0||action_hu[i] == GameConstants.WIK_NULL)) {
				...
				table._playerStatus[i].add_pao(_send_card_data, i);
		...
		if (ti_sao != GameConstants.WIK_PAO) {
			bAroseAction = table.estimate_player_out_card_respond_yj(_seat_index, _send_card_data, true);
		}
		for(int i = 0; i < table.getTablePlayerNumber();i++){
			if(table._playerStatus[i].has_action_by_code(GameConstants.WIK_PENG)){
				...
				{
					int cbRemoveCard[] = new int[] { _send_card_data, _send_card_data };
					if (!table._logic.remove_cards_by_index(table.GRR._cards_index[i], cbRemoveCard, 2)) {
						table.log_player_error(i, "碰牌删除出错");
						return ;
					}
					 table.exe_chi_peng(i, _seat_index, GameConstants.WIK_PENG, _send_card_data, GameConstants.CHI_PENG_TYPE_DISPATCH, -1);
					 return ;
				}
			}
		...
		if ((bAroseAction == false) && (ti_sao == GameConstants.WIK_NULL)&&bHupai == 0) {
	...
	public boolean handler_operate_card(SCPHZTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		case GameConstants.WIK_NULL: {
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					int action = table.estimate_player_respond_yj(i, _seat_index, _send_card_data, pao_type, true);
		...
		{
			int pao_type[] = new int[1];
			int action = table.estimate_player_respond_yj(target_player, _seat_index, _send_card_data, pao_type, true);
			if (action != GameConstants.WIK_NULL) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			table.process_chi_hu_player_score_sy(target_player, _seat_index, operate_card, true);
