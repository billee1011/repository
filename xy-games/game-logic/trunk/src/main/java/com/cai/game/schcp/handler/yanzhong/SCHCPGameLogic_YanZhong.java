...
public class SCHCPGameLogic_YanZhong {
	...
	public SCHCPGameLogic_YanZhong() {
		_magic_card_count = 0;
	...
	public int check_nei_hua(int card_index[], int cur_card) {
		...
		// 碰牌判断
		return (card_index[switch_to_card_index(cur_card)] == 2) ? GameConstants.CP_WIK_HUA : GameConstants.CP_WIK_NULL;
	}
	...
	public int check_che(int card_index[], int cur_card) {
		...
		// 碰牌判断
		return (card_index[switch_to_card_index(cur_card)] == 2) ? GameConstants.CP_WIK_CHE : GameConstants.CP_WIK_NULL;
	}
	...
	public boolean remove_cards_by_data(int cards[], int card_count, int remove_cards[], int remove_count) {
		...
		// 定义变量
		int cbDeleteCount = 0;
		int cbTempCardData[] = new int[GameConstants.MAX_CP_COUNT];
	...
	public int get_analyse_tuo_shu(int cbWeaveKind, int center_card) {
		switch (cbWeaveKind) {
		case GameConstants.CP_WIK_LONG:
			return (this.get_hong_dot(center_card) != 0) ? 10 : 6;
		case GameConstants.CP_WIK_KAN:
		case GameConstants.CP_WIK_CHE:
		case GameConstants.CP_WIK_HUA:
			return (get_hong_dot(center_card) != 0) ? 8 : 4;
		case GameConstants.CP_WIK_LEFT:
		case GameConstants.CP_WIK_CENTER:
		case GameConstants.CP_WIK_RIGHT: {
			int hong_tuo = 0;
	...
	public int get_four_count(WeaveItem weaveItem[], int cbWeaveCount, boolean is_jsg) {
		...
		for (int i = 0; i < cbWeaveCount; i++) {
			...
			case GameConstants.CP_WIK_CHE: 
			case GameConstants.CP_WIK_HUA:
			case GameConstants.CP_WIK_PENG:{
		...
		for (int i = 0; i < GameConstants.MAX_CP_INDEX; i++) {
			if (cards_index[i] == 4) {
				count++;
				if (get_times_cards(this.switch_to_card_data(i)) > 0  && is_jsg == true) {
					count++;
				}
			}
		}
	...
	public int get_action_rank(int player_action) {
		// 自摸牌等级
		if (player_action == GameConstants.CP_WIK_ZI_MO) {
			return 50;
		}
		// 吃胡牌等级
		if (player_action == GameConstants.CP_WIK_CHI_HU) {
			return 40;
		}

		// 偷牌等级
		if (player_action == GameConstants.CP_WIK_TOU) {
			return 30;
		}
		// 扯牌等级
		if (player_action == GameConstants.CP_WIK_CHE) {
			return 30;
		}
		...
		// 上牌等级
		if (player_action == GameConstants.CP_WIK_LEFT || player_action == GameConstants.CP_WIK_CENTER
				|| player_action == GameConstants.CP_WIK_RIGHT) {
			return 10;
		}
	...
	public int get_weave_card(int cbWeaveKind, int cbCenterCard, int cbCardBuffer[]) {
		...
		case GameConstants.CP_WIK_HUA:
		case GameConstants.CP_WIK_CHE:
		case GameConstants.CP_WIK_KAN: // 左吃类型
		{
			// 设置变量
	...
	public int get_chi_action(int card, int temp_card) {
		...
		case 0x12:
		case 0x66: {
			return GameConstants.CP_WIK_LEFT;
		...
		case 0x13: {
			if (temp_card == 0x0a)
				return GameConstants.CP_WIK_LEFT;
			if (temp_card == 0x46)
				return GameConstants.CP_WIK_CENTER;
		}
		case 0x05:
		case 0x50: {
			if (temp_card == 0x45)
		...
		case 0x42: {
			...
			if (temp_card == 0xf8)
				return GameConstants.CP_WIK_CENTER;
			if (temp_card == 0x80)
				return GameConstants.CP_WIK_RIGHT;
		}
		case 0x43:
		case 0x07:
		case 0x16: {
	...
	public int get_kind_card(int card, int type) {
		...
		case 0x04:
		case 0x13: {
			if (type == GameConstants.CP_WIK_LEFT)
				return 0x0a;
			if (type == GameConstants.CP_WIK_CENTER)
				return 0x46;
		}
		case 0x05:
		case 0x50: {
		...
		case 0x42: {
			...
			if (type == GameConstants.CP_WIK_CENTER)
				return 0xf8;
			if (type == GameConstants.CP_WIK_RIGHT)
				return 0x80;
		}
		case 0x43:
		case 0x07:
