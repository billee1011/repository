...
public class WSKHandlerOutCardOperate_ZzshA extends WSKHandlerOutCardOperate<WSKTable_ZzshA> {

	@Override
	public void exe(WSKTable_ZzshA table) {
		...
		if (card_type == WSKConstants.ZZSHA_CT_ERROR) {
			table.send_error_notify(_out_card_player, 2, "请选择正确的牌型!");
			return;
		}

		// 出A特效
		boolean is_A_effect = false;
		...
		for (int i = 0; i < _out_card_count; i++) {
			if (_out_cards_data[i] == 0x01 || _out_cards_data[i] == 0x21) {
				table._out_sign_data[_out_card_player][table._out_sign_count[_out_card_player]++] = _out_cards_data[i];
				is_A_effect = true;
			}
		...
		if (is_A_effect) {
			int data[] = new int[table._out_sign_count[_out_card_player]];
			for (int i = 0; i < table._out_sign_count[_out_card_player]; i++) {
				data[i] = table._out_sign_data[_out_card_player][i];
			}
	...
	public void table_pai_socre(WSKTable_ZzshA table) {
		int pai_score = 0;
	...
	public void user_pass_card(WSKTable_ZzshA table) {
		...
		if (next_player == table._out_card_player) {
			...
			if (table.GRR._card_count[next_player] == 0) {
				...
				int out_A_num = 0;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					if (table._out_sign_count[i] > 0) {
						out_A_num += table._out_sign_count[i];
					}
				}
				if (out_A_num >= 2) {
					for (int i = 0; i < table.getTablePlayerNumber(); i++) {
						...
						if (table._seat_team[next_player] == table._seat_team[table._out_card_player]
	...
	public int adjust_out_card_right(WSKTable_ZzshA table) {
		int card_type = WSKConstants.ZZSHA_CT_ERROR;
		card_type = table._logic.GetCardType(_out_cards_data, _out_card_count);

		if (card_type == WSKConstants.ZZSHA_CT_ERROR) {
			return card_type;
		}
		if (table._turn_out_card_count == 0 && card_type == WSKConstants.ZZSHA_CT_PLANE) {
	...
	@Override
	public boolean handler_player_be_in_room(WSKTable_ZzshA table, int seat_index) {

		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(WSKMsgConstants.RESPONSE_ZZSHA_RECONNECT_DATA);

		TableResponse_zzshA.Builder tableResponse = TableResponse_zzshA.newBuilder();
		...
		for (int index = 0; index < table.getTablePlayerNumber(); index++) {
			int data[] = new int[table._out_sign_count[index]];
			for (int i = 0; i < table._out_sign_count[index]; i++) {
				data[i] = table._out_sign_data[index][i];
			}
			table.send_effect_type(index, 2, data, 0, GameConstants.INVALID_SEAT);
		}
		return true;
	}
