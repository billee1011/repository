...
public class PDKHandlerOutCardOperate_FIFTEENYY extends PDKHandlerOutCardOperate {
	@Override
	public void exe(PDKTable table) {
		...
		if (table._logic.Player_Can_out_card(table.GRR._cards_data[next_player], table.GRR._card_count[next_player],
			...
			// if (table.get_players()[next_player].isOnline() &&
			// !table._enter_back[next_player]) {
			// table._auto_out_card_scheduled = GameSchedule.put(new
			// PDKAutoOutCardRunnable(table.getRoom_id(),
			// next_player, table, GameConstants.PDK_CT_PASS), delay,
	...
	public boolean handler_player_be_in_room(PDKTable table, int seat_index) {
		...
		// int can_out_card_data[] = new int[table.get_hand_card_count_max()];
		// if
		// (table._logic.Player_Can_out_card(table.GRR._cards_data[seat_index],
