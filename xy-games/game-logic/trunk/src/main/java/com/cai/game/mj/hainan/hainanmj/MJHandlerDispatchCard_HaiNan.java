 ...
 *
 */
public class MJHandlerDispatchCard_HaiNan extends MJHandlerDispatchCard<MJTable_HaiNan> {
	...
	public void exe(MJTable_HaiNan table) {
		...
		table._playerStatus[_seat_index].chi_peng_round_valid_ext();
		if (AbstractMJTable.DEBUG_CARDS_MODE) {
			//table.GRR._left_card_count = (15+ table.getTablePlayerNumber());
		}

		
		// 荒庄结束
		if (table.GRR._left_card_count == table.getLiuJuCount()) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		int hua_count = 0;
		if ((_send_card_data >= GameConstants.CARD_CAI_SHENG)) {
			hua_cards[hua_count++] = _send_card_data;
		}
		for (int i = 0; i < 8; i++) {
		...
		} else if (_type == GameConstants_HaiNan.HU_CARD_TYPE_BU_HUA) {
			card_type = GameConstants_HaiNan.HU_CARD_TYPE_BU_HUA;
		}
		// 胡牌检测
