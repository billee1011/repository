...
public class THKHandlerGang_HY extends HHHandlerGang<HHTable> {
...
//		}
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_thk(target_player, this._provide_player, operate_card, false);
...
//			}
	...
	protected boolean exe_gang(HHTable table) {
		...
		if (GameConstants.PAO_TYPE_AN_LONG == _type) {
			...
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				...
				if ((cbCenterCard == _center_card) && (cbWeaveKind == GameConstants.WIK_XIAO)) {
		...
		if(table._xian_ming_zhao[_seat_index] == true)
		{
			for(int i = 0 ;i < GameConstants.MAX_HH_INDEX;i++)
			{
				...
				{
					if(table.GRR._cannot_out_index[_seat_index][i] != 1)
					{
			...
			if(is_di_hu == true)
			{
				for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
					...
					if (!((cbWeaveKind == GameConstants.WIK_TI_LONG)||
							...
							(cbWeaveKind == GameConstants.WIK_XIAO))) {
						is_di_hu = false;
					}
		...
		if((_depatch == false)&&(table._ti_mul_long[_seat_index] == 0)&&(table._is_xiang_gong[_seat_index] == false)){
			// 变量定义
			ChiHuRight chr = new ChiHuRight();
	...
	public boolean handler_player_out_card(HHTable table, int seat_index, int card) {
		...
		else if(table.has_rule(GameConstants.GAME_RULE_DI_MING_WEI)&&table.GRR._cannot_out_index[seat_index][table._logic.switch_to_card_index(card)] >= 1 )
		{
