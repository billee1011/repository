package com.cai.game.xpbh.handler;
...
public class BHHandlerGang<T extends XPBHTable> extends BHHandler<T> {
	...
	public boolean handler_operate_card(T table,int seat_index, int operate_code, int operate_card,int lou_pai){
		...
		if(jie_pao_count>0){
			...
			for(int i=0; i<table.getTablePlayerNumber();i++){
				...
				table.process_chi_hu_player_score_bh(i,_seat_index, _center_card,false);
...
//		
	...
	public boolean handler_player_out_card(T table, int seat_index, int card) {
		...
		else if(table.GRR._cannot_out_index[seat_index][table._logic.switch_to_card_index(card)] >= 1 )
		{
			...
			{
				table._xian_ming_zhao[seat_index] = true;
			}
		}
...
//			}
		...
		int hand_card_count = table.switch_index_to_card(seat_index, hand_cards);
