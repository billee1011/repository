...
public class MJHandlerGang_HuNan_ChangDe extends MJHandlerGang<MJTable_HuNan_ChangDe> {
	...
	public boolean exe_gang(MJTable_HuNan_ChangDe table) {
	...
	public boolean handler_operate_card(MJTable_HuNan_ChangDe table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_CHI_HU) {
			...
			if (table._playerStatus[seat_index].has_chi_hu()) {
				table._playerStatus[seat_index].chi_hu_round_invalid(); // 这一轮就不能吃胡了
																		// 没过牌之前都不能胡
			}
		}
		...
		if (jie_pao_count > 0) {
			...
			if (jie_pao_count > 1) {
				if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HUNAN_QIANG_GANG_HU_JIANG_MA)) {
					// 设置奖码--通炮不加奖码，通炮时按照放炮者中的码算分
					if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HUNAN_MO_JI_JIANG_JI)) {
						table.set_niao_card(_provide_player, operate_card, true, add_niao);
					} else {
						table.set_niao_card(_provide_player, GameConstants.INVALID_VALUE, true, add_niao);
					}
				}

				table._cur_banker = _provide_player;
				...
				if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HUNAN_QIANG_GANG_HU_JIANG_MA)) {
					if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HUNAN_HONGZHONG)) {
						if (!GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HUNAN_MO_JI_JIANG_JI)) {
							if (table.GRR._cards_index[target_player][table._logic.get_magic_card_index(0)] == 0) {
								table.add_niao = 1; // 接炮者手中无红中，加一个奖码
							}
						}
					}
					...
					// 设置奖码--按接炮者中的码算分
					if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HUNAN_MO_JI_JIANG_JI)) {
						table.set_niao_card(target_player, operate_card, true, add_niao);
					} else {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				table.process_chi_hu_player_score(i, _seat_index, operate_card, 1); // 抢杠胡算分
