...
public class MJHandlerFinish_GY extends AbstractMJHandler<Table_GY> {
	...
	public boolean handler_player_show_card(Table_GY table, int seat_index) {
		if (table.GRR == null)
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < 55; j++) {
				int_array.addItem(table.game_end_GRR._discard_cards[i][j]);
			}
			...
			for (int j = 0; j < GameConstants_MYGY.MAX_WEAVE; j++) {
				...
				weaveItem_item.setProvidePlayer(table.game_end_GRR._weave_items[i][j].provide_player + GameConstants_MYGY.WEAVE_SHOW_DIRECT);
		...
		table.switch_to_cards_data(table.game_end_GRR._cards_index[seat_index], hand_cards, seat_index);
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			int hand_card_count = table.switch_to_cards_data(table.game_end_GRR._cards_index[i], cards, i);

			if (table.game_end_GRR._win_order[i] == 1) {
				cards[hand_card_count] = table.game_end_GRR._chi_hu_card[i][0] + GameConstants_MYGY.CARD_ESPECIAL_TYPE_HU;
				hand_card_count++;
			}

			operate_player_cards(table, i, 0, null, 0, null);
			table.operate_show_card(i, GameConstants_MYGY.Show_Card_HU, hand_card_count, cards, seat_index);
		}
		return true;
	}

	public boolean operate_player_cards(Table_GY table, int seat_index, int card_count, int cards[], int weave_count, WeaveItem weaveitems[]) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
