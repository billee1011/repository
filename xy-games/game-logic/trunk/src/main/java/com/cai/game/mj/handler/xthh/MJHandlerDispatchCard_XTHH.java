...
public class MJHandlerDispatchCard_XTHH extends MJHandlerDispatchCard<MJTable> {
	...
	public void exe(MJTable table) {
		...
		if (table._logic.is_magic_card(_send_card_data)) {
			table.GRR.mo_lai_count[_seat_index]++;// 摸赖次数
		}
		...
		if (table.GRR._left_card_count > 0) {
			...
			// 笑牌检测
			int cbActionMask = table._logic.analyse_xiao_by_dispacth_card(table, _seat_index, table.GRR._cards_index[_seat_index], _send_card_data,
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			// 下局胡牌的是庄家
			// table.set_niao_card(_seat_index,MJGameConstants.INVALID_VALUE,true,0);//
			// 结束后设置鸟牌
