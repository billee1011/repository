...
public class MJHandlerOutCardOperate_WEIHE extends MJHandlerOutCardOperate<MJTable_WEIHE> {
	...
	public void exe(MJTable_WEIHE table) {
		...
		table.changCards(cards, _out_card_player);
	...
	public boolean handler_operate_card(MJTable_WEIHE table, int seat_index, int operate_code, int operate_card) {
		...
		if (table._playerStatus[seat_index].has_chi_hu() && operate_code != GameConstants.WIK_CHI_HU) {
			if (table.has_rule(GameConstants_WEIHE.GAME_RULE_WEINAN_LOU_HU_ONE)) {
				table._playerStatus[seat_index].chi_hu_round_invalid_ext(_out_card_data);
			} else {
	...
	public boolean handler_player_be_in_room(MJTable_WEIHE table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (i == this._out_card_player) {
				tableResponse.addDiscardCount(table.GRR._discard_count[i]+1);
			}else{
		...
		table._logic.switch_to_cards_data_sichuan(table.GRR._cards_index[seat_index], hand_cards,
				table.ding_que_pai_se[seat_index]);
		table.changCards(hand_cards, seat_index);
