...
private static Logger logger = Logger.getLogger(NNGameLogic.class);
	...
	public int get_card_type(int card_date[],int card_count, int game_rule_index){
		...
		if(FvMask.has_any(game_rule_index, FvMask.mask(GameConstants.GAME_RULE_WU_HUA_NIU))){
			if(king_count==GameConstants.OX_MAX_CARD_COUNT) return GameConstants.DBN_FIVE_KING;
	...
	public int get_times_one(int card_date[],int card_count,int game_rule_index){
		...
		else if(times ==GameConstants.DBN_FIVE_KING)times = 5;
		else if(times ==GameConstants.DBN_BOOM)times = 6;
		else if(times ==GameConstants.DBN_WUXIAONIU)times = 8;
		return times;
	}
	/***
	 * 获取牛牛倍数
