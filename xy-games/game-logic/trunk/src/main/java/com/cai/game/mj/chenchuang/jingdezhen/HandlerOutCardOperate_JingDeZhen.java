...
public class HandlerOutCardOperate_JingDeZhen extends MJHandlerOutCardOperate<Table_JingDeZhen> {
	...
	public void exe(Table_JingDeZhen table) {
		...
		if(table._logic.is_magic_card(_out_card_data))
			table.out_bao_count[_out_card_player]++;
		table._out_card_count++;
		table._out_card_player = _out_card_player;
		table._out_card_data = _out_card_data;
		//得到下家
		int next_player = (_out_card_player + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();
		boolean is_ming_pai = table.is_bao_ding[_out_card_player] == 2 || table.player_bai_count[_out_card_player] > 0;
		//if(!is_ming_pai){
			int cards[] = new int[GameConstants.MAX_COUNT];
			int hand_card_count = table.switch_to_cards_data(table.GRR._cards_index[_out_card_player], cards);
			//刷新手牌
		...
		if (bAroseAction == false) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if(table.is_bao_ding[i] == 2 || table.player_bai_count[i] > 0){
					if (playerStatus.has_chi_hu() && table.numberOf1(table.setCanBaiPaiXing(table.GRR._chi_hu_rights[i], i)) >= table.player_bai_count[i]
	...
	public boolean handler_operate_card(Table_JingDeZhen table, int seat_index, int operate_code, int operate_card) {
		...
		if (table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_CHI_HU) && operate_code != GameConstants.WIK_CHI_HU) {
			...
			table.score_when_abandoned_jie_pao[seat_index] = table.getPaiXingScore(table.GRR._chi_hu_rights[seat_index], _out_card_player);
		}
	...
	private void handler_operate_tong_pao(Table_JingDeZhen table, int seat_index,
        ...
        case Constants_MJ_JING_DE_ZHEN.WIK_LL_CHI: {
        	int cbRemoveCard[] = {};
        	if(target_card == 0x31)
        		cbRemoveCard = new int[] { target_card + 1, target_card + 3 };
        	if(target_card == 0x32)
        		cbRemoveCard = new int[] { target_card - 1, target_card + 2 };
        	if(target_card == 0x34)
        		cbRemoveCard = new int[] { target_card - 2, target_card - 3 };
        	if (!table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2)) {
        		table.log_player_error(seat_index, "吃牌删除出错");
        		return;
        	}
        ...
        case Constants_MJ_JING_DE_ZHEN.WIK_RR_CHI: {
        	int cbRemoveCard[] = {};
        	if(target_card == 0x31)
        		cbRemoveCard = new int[] { target_card + 2, target_card + 3 };
        	if(target_card == 0x33)
        		cbRemoveCard = new int[] { target_card - 2, target_card + 1 };
        	if(target_card == 0x34)
        		cbRemoveCard = new int[] { target_card - 1, target_card - 3 };
        	if (!table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2)) {
        		table.log_player_error(seat_index, "吃牌删除出错");
		...
		case Constants_MJ_JING_DE_ZHEN.WIK_ZHI_GANG: {
			table.remove_discard_after_operate(_out_card_player, _out_card_data);
		...
		case GameConstants.WIK_BAO_TING:{
			table.operate_effect_action(seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { operate_code + 1000 }, 1, GameConstants.INVALID_SEAT);
			if(operate_card > 0)
				table.operate_effect_action(seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { Constants_MJ_JING_DE_ZHEN.WIK_BAI_PAI + 100 + operate_card }, 1, GameConstants.INVALID_SEAT);
			...
			table._player_result.biaoyan[seat_index] = operate_card;
			table.operate_player_info();
			table.is_bao_ding[seat_index] = 2;
			table.player_bai_count[seat_index] = operate_card;
			table._playerStatus[seat_index].clean_action(GameConstants.WIK_BAO_TING);
			int cards[] = new int[GameConstants.MAX_COUNT];
			int hand_card_count = table.switch_to_cards_data(table.GRR._cards_index[seat_index], cards);
			
			table.set_ting_card_bai(table.GRR._cards_index[seat_index], table.GRR._weave_items[seat_index], table.GRR._weave_count[seat_index],
					seat_index);
			...
			handler_player_out_card(table, seat_index, operate_card);
			table.exe_dispatch_card(seat_index, GameConstants.WIK_NULL, 0);
			return;
		}
        ...
        case GameConstants.WIK_CHI_HU: // 胡
        {
            ...
            if(jie_pao_count == 1){
            	...
            	for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                    ...
                    if(i == table._cur_banker)
                    	is_zhuang_hu = true;
                    int p = (i - _out_card_player + table.getTablePlayerNumber()) % table.getTablePlayerNumber();
                    if(p < z)
                    	z = p;
                }
            	if(is_zhuang_hu){
            ...
            if (jie_pao_count > 0) {
                ...
                table.GRR._chi_hu_rights[_out_card_player].opr_or_long(Constants_MJ_JING_DE_ZHEN.CHR_FANG_PAO);
                for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                    if ((i == _out_card_player) || (table.GRR._chi_hu_rights[i].is_valid() == false)) {
                        continue;
                    }
                    ...
                    if(_type == GameConstants.GANG_TYPE_HONG_ZHONG){
        				table.GRR._chi_hu_rights[i].opr_or_long(Constants_MJ_JING_DE_ZHEN.CHR_DI_HU);
        				//天胡所以玩家加收20分
        				for (int j = 0; j < table.getTablePlayerNumber(); j++) {
        					if(i == j)
        						continue;
        					int s = 10;
        					if((i == table.GRR._banker_player || j == table.GRR._banker_player) && is_fan_bei);
        						s = 20;
        					table.GRR._game_score[j] -= s;
        					table.GRR._game_score[i] += s;
        				}
        			}
                ...
                GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._cur_banker,
                        GameConstants.Game_End_NORMAL), 1, TimeUnit.SECONDS);
	...
	private boolean checkBaoTing(Table_JingDeZhen table) {
		boolean flag = false;
		int _current_player = (_out_card_player + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();
		if(table.is_bao_ding[_current_player] == 1 && table.GRR._weave_count[_current_player] == 0){
			for (int i = 0; i < 3; i++)table.can_bai_out_card[i] = new HashSet<Integer>();
			 table.get_ting_card(table._playerStatus[_current_player]._hu_cards, table.GRR._cards_index[_current_player],
					 table.GRR._weave_items[_current_player], table.GRR._weave_count[_current_player], _current_player, 0);
			 int baiPaiCount = table.getBaiPaiCount();
			 table._playerStatus[_current_player].add_action(GameConstants.WIK_BAO_TING);
			 table.add_action_weave(table._playerStatus[_current_player], 0, _current_player, baiPaiCount, GameConstants.WIK_BAO_TING);
			 table.change_player_status(_current_player, GameConstants.Player_Status_OPR_CARD);
	...
	private void handler_operate_bu_tong_pao(Table_JingDeZhen table, int seat_index,
			int operate_code, int operate_card) {
		int target_player = seat_index;
		int target_action = operate_code;
		int target_p = 0;
