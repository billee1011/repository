...
///////////////////////////////////////////////////////////////////////////////////////////////
public class EIGHTOXTable extends AbstractRoom {
	/**
	 * 
	 */
	...
	private static Logger logger = Logger.getLogger(EIGHTOXTable.class);
	...
	public EIGHTOXGameLogic _logic = null;
	...
	protected boolean on_handler_game_start() {
		...
		return game_start_Eightox();
	}

	// 斗板牛开始
	public boolean game_start_Eightox() {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			for (int k = 0; k < this.getTablePlayerNumber(); k++) {
				...
				if (this._player_status[k] == false) {
					...
					for (int j = 0; j < card_count; j++) {
						if (this.getRuleValue(GameConstants.GAME_RULE_KSZQZ_OX) == 1 && this._last_card[k] == GRR._cards_data[k][j])
	...
	public boolean send_open_to_room(int seat_index) {
		...
		for (int i = 0; i < GameConstants.OX_MAX_CARD_COUNT; i++) {
			if (_last_card[seat_index] == this.GRR._cards_data[seat_index][i] && this.getRuleValue(GameConstants.GAME_RULE_KSZQZ_OX) == 1)
				open_card.addCards(this.GRR._cards_data[seat_index][i] | 0x100);
			else
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		ret = this.handler_game_finish_Eightox(seat_index, reason);
	...
	public boolean handler_game_finish_Eightox(int seat_index, int reason) {
		...
		this.load_player_info_data_game_end(game_end_Eightox);
		game_end_Eightox.setGameRound(_game_round);
		game_end_Eightox.setCurRound(_cur_round);
		game_end_Eightox.setDisplayTime(this._idi_ready_card_time);
		if (GRR != null) {
			GameRoundRecordOx record_info = new GameRoundRecordOx(this.getTablePlayerNumber(), GameConstants.OX_MAX_CARD_COUNT,
					this.getTablePlayerNumber());
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				game_end_Eightox.addStrOxValue(this._str_ox_value[i]);
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if (_cur_round >= _game_round) {// 局数到了
				...
				game_end_Eightox.setPlayerResult(this.process_player_result_Eightox(reason));
	...
	public boolean table_response_observer(Player player) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			tableResponse_Eightox.addCallButton(this._player_call_banker[i]);
			tableResponse_Eightox.addJettonScore(this._jetton_player[i]);
			tableResponse_Eightox.addOpenCard(this._open_card_player[i]);
			if (this._open_card_player[i] == true) {
				tableResponse_Eightox.addOxValue(this._ox_value[i]);
				tableResponse_Eightox.addTimes(this._player_times[i]);
			} else {
				...
				for (int k = 0; k < this.getTablePlayerNumber(); k++) {
					...
					if (this._player_status[k] == false) {
						...
						for (int j = 0; j < card_count; j++) {
							...
							cards.addItem(this.GRR._cards_data[k][j]);
			...
			for (int k = 0; k < this.getTablePlayerNumber(); k++) {
				...
				if (this._player_status[k] == false) {
					...
					if (getRuleValue(GameConstants.GAME_RULE_MSZQZ_OX) == 1 && this._open_card_player[k] == false) {
						...
						for (int j = 0; j < card_count; j++) {
							if (_last_card[k] == this.GRR._cards_data[k][j] && this.getRuleValue(GameConstants.GAME_RULE_KSZQZ_OX) == 1)
								cards.addItem(GRR._cards_data[k][j] | 0x100);
							else
								cards.addItem(GRR._cards_data[k][j]);
			...
			if (_game_status == GameConstants.GS_OX_OPEN_CARD) {
				...
				for (int k = 0; k < this.getTablePlayerNumber(); k++) {
					...
					if (this._player_status[k] == false) {
						...
						if (k == seat_index) {
							tableResponse_Eightox.setCurOxValue(this._ox_value[seat_index]);
							tableResponse_Eightox.setCurTimes(this._player_times[seat_index]);
						}
						int temp_cards_data[] = new int[GameConstants.OX_MAX_CARD_COUNT];
						for (int index = 0; index < GameConstants.OX_MAX_CARD_COUNT; index++) {
