package com.cai.game.mj.handler.zhuzhouzz;
...
public class MJHandlerGang_ZHUZHOUZZ extends MJHandlerGang<MJTable_ZHUZHOUZZ> {

	// 执行杆操作
	@Override
	public void exe(MJTable_ZHUZHOUZZ table) {
		...
		// 修改湖南转转自摸胡没有抢杆胡 7.11
		if (table.has_rule(GameConstants.GAME_RULE_HUNAN_ZIMOHU)) {
			this.exe_gang(table);
			return;
		}
	 ...
	 */
	@Override
	public boolean handler_operate_card(MJTable_ZHUZHOUZZ table, int seat_index, int operate_code, int operate_card) {
				...
				for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
					for (int j = 0; j < table.GRR._player_niao_count[i]; j++) {
						if (i == target_player) {
							...
							table.GRR._player_niao_cards[i][j] = table.set_ding_niao_valid(table.GRR._player_niao_cards[i][j], true);// 胡牌的鸟生效
						} else {
							table.GRR._player_niao_cards[i][j] = table.set_ding_niao_valid(table.GRR._player_niao_cards[i][j], false);// 胡牌的鸟失效
						}
					}
				}
	 ...
	 ***/
	@Override
	protected boolean exe_gang(MJTable_ZHUZHOUZZ table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			int score = 2 *table.get_bei_lv();
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;

				// 暗杠每人2分
				table.GRR._gang_score[_seat_index].scores[cbGangIndex][i] = -score * GameConstants.CELL_SCORE;// 暗杠，其他玩家扣分
				table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += score * GameConstants.CELL_SCORE;// 一共加分
			...
			int score = (table.getTablePlayerNumber() - 1)*table.get_bei_lv();
			...
			table._player_result.game_score[_provide_player] -= score * GameConstants.CELL_SCORE;
			table._player_result.game_score[_seat_index] += score * GameConstants.CELL_SCORE;
			
			table._player_result.biaoyan[_provide_player] -= score * GameConstants.CELL_SCORE;
			table._player_result.biaoyan[_seat_index] += score * GameConstants.CELL_SCORE;

			table._player_result.ming_gang_count[_seat_index]++;
		} else if (GameConstants.GANG_TYPE_ADD_GANG == _type) {
			int score = GameConstants.CELL_SCORE*table.get_bei_lv();
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index)
					continue;
