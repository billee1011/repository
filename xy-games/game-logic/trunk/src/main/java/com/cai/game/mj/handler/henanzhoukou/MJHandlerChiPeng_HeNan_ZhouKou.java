...
public class MJHandlerChiPeng_HeNan_ZhouKou extends MJHandlerChiPeng<MJTable> {
	...
	public void exe(MJTable table) {
		...
		for (int j = 0; j < hand_card_count; j++) {
			if (table.has_rule(GameConstants.GAME_RULE_HENAN_DAI_HUN) && table._logic.is_magic_card(cards[j])) {
		...
		if (GameDescUtil.has_rule(table.getGameRuleIndexEx(), GameConstants.GAME_RULE_HENAN_CHU_FENG_BAO_TING)) {
			if (table._logic.get_card_count_by_index(table.GRR._cards_index[_seat_index]) <= 8) { // 手上牌的数量小于8，证明已经碰了2碰了
				...
				if (ting_count > 0) {
					...
					// 刷新自己手牌
					int tmp_hand_card_count = table._logic.switch_to_cards_data(table.GRR._cards_index[_seat_index], tmp_hand_cards);

					for (int i = 0; i < tmp_hand_card_count; i++) {
						for (int j = 0; j < ting_count; j++) {
							if (tmp_hand_cards[i] == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
								tmp_hand_cards[i] += GameConstants.CARD_ESPECIAL_TYPE_TING;
								break;
							}
						}

						if (table.has_rule(GameConstants.GAME_RULE_HENAN_DAI_HUN) && tmp_hand_cards[i] < GameConstants.CARD_ESPECIAL_TYPE_TING
								&& table._logic.is_magic_card(tmp_hand_cards[i])) {
							tmp_hand_cards[i] += GameConstants.CARD_ESPECIAL_TYPE_HUN;
						}
					}

					table.operate_player_cards_with_ting(_seat_index, tmp_hand_card_count, tmp_hand_cards, weave_count, weaves);
			...
			if (ting_count > 0) {
				...
				for (int i = 0; i < tmp_hand_card_count; i++) {
					...
					if (table.has_rule(GameConstants.GAME_RULE_HENAN_DAI_HUN) && tmp_cards[i] < GameConstants.CARD_ESPECIAL_TYPE_TING
							&& table._logic.is_magic_card(tmp_cards[i])) {
						tmp_cards[i] += GameConstants.CARD_ESPECIAL_TYPE_HUN;
					}
				}
