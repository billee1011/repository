...
public class MJHandlerOutCardOperate_XTHH extends MJHandlerOutCardOperate<MJTable> {
	@Override
	public void exe(MJTable table) {
		...
		// table.get_xthh_ting_card(table._playerStatus[_out_card_player]._hu_cards,
		...
		if (table._logic.is_magic_card(_out_card_data) == true) {
			...
			// if(heijiahei){
			// table.GRR._especial_txt = "飘赖*"+table.GRR._piao_lai_count+"(黑加黑)
			// 倍数:"+table.GRR._bei_shu;
			// }else{
			// table.GRR._especial_txt ="飘赖*"+table.GRR._piao_lai_count+"
			// 倍数:"+table.GRR._bei_shu;
			// }
			table.GRR._especial_txt = "飘赖*" + table.GRR._piao_lai_count;// +"
																		// 倍数:"+table.GRR._bei_shu;
			...
			// 效果
			table.operate_effect_action(_out_card_player, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { GameConstants.WIK_PIAO_LAI }, 1,
					GameConstants.INVALID_SEAT);

			if (table.has_rule(GameConstants.GAME_RULE_HEBEI_PIAO_LAI_YOU_JIANG)) {
				// 飘赖有奖
				int lai_count[] = new int[table.getTablePlayerNumber()];
				float score[] = new float[table.getTablePlayerNumber()];
			...
			if (_type == GameConstants.WIK_MENG_XIAO || _type == GameConstants.WIK_DIAN_XIAO || _type == GameConstants.WIK_HUI_TOU_XIAO
					|| _type == GameConstants.WIK_XIAO_CHAO_TIAN || _type == GameConstants.WIK_DA_CHAO_TIAN) {
				// 玩家出牌 响应判断,是否有吃碰杠补胡
				bAroseAction = table.estimate_player_out_card_respond_xthh(_out_card_player, _out_card_data, GameConstants.HU_CARD_TYPE_RE_CHONG);// ,
																																					// EstimatKind.EstimatKind_OutCard
			} else {
				bAroseAction = table.estimate_player_out_card_respond_xthh(_out_card_player, _out_card_data, GameConstants.HU_CARD_TYPE_PAOHU);// ,
																																				// EstimatKind.EstimatKind_OutCard
			}
	...
	// @Override
	// protected void exe_chi_peng(MJTable table,int target_player,int
	// target_action,int target_card){
	// // 组合扑克
	...
	// //table.operate_out_card(this._out_card_player, 0,
	// null,MJGameConstants.OUT_CARD_TYPE_MID,MJGameConstants.INVALID_SEAT);
	// table.operate_remove_discard(this._out_card_player,
	// table.GRR._discard_count[_out_card_player]);
	// // 设置用户
	...
	// weaves[i].weave_kind =
	// table.GRR._weave_items[target_player][i].weave_kind;
	// weaves[i].center_card =
	// table.GRR._weave_items[target_player][i].center_card;
	// weaves[i].public_card =
	// table.GRR._weave_items[target_player][i].public_card;
	// weaves[i].provide_player =
	// table.GRR._weave_items[target_player][i].provide_player+MJGameConstants.WEAVE_SHOW_DIRECT;
	// //癞子
