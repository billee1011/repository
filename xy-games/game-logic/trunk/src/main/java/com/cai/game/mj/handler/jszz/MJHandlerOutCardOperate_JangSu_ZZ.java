...
public class MJHandlerOutCardOperate_JangSu_ZZ extends MJHandlerOutCardOperate<MJTable_JangSu_ZZ> {
	...
	public void exe(MJTable_JangSu_ZZ table) {
		...
		if (table._logic.is_magic_card(_out_card_data)) {
			...
			if (outcard_count == 0 && table._xian_chu_count[_out_card_player] > 0) {
				return;
			}
		...
		if (!table.has_rule(GameConstants.GAME_RULE_JIANGSU_YUAN_ZI)) {
			...
			} else if (table._out_card_index[_out_card_player][table._logic.switch_to_card_index(_out_card_data)] == 3
					&& table._logic.is_da_gen_card(_out_card_data)) {
				int cell_score = 2;
				if (table._b_double && table.has_rule(GameConstants.GAME_RULE_JIANGSU_DOUBLE)) {
					cell_score *= 2;
				}
		...
		if (da_outcard_count == 0) {
			table.operate_effect_action(_out_card_player, GameConstants.Effect_Action_Other, dacard_count,
					effect_indexs, 1, GameConstants.INVALID_SEAT);
		} else {
			table.operate_effect_action(_out_card_player, GameConstants.Effect_Action_Other, 0, effect_indexs, 1,
		...
		if (table._logic.is_magic_card(_out_card_data)) {
			if (bAroseAction) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					int user_no_da_outcard_count = 0;
					for (int j = 0; j < GameConstants.MAX_INDEX; j++) {
						if (table._out_card_index[i][j] > 0 && !table._logic.is_magic_index(j)) {
							user_no_da_outcard_count += table._out_card_index[i][j];
						}
					}
					if (user_no_da_outcard_count != 0 && table._playerStatus[i].has_action()) {
	...
	public boolean handler_operate_card(MJTable_JangSu_ZZ table, int seat_index, int operate_code, int operate_card) {
		...
		if (table.has_rule(GameConstants.GAME_RULE_JIANGSU_BAO_CHONG)
			...
			if (operate_code == GameConstants.WIK_CHI_HU) {
				...
				int can_hu_count = 0;
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					if (table._playerStatus[i].has_chi_hu() && table.GRR._chi_hu_rights[i].is_valid()) {
						hu_count++;
					}
				...
				if (hu_count == 2 && can_hu_count == 3) {
					for (int i = 0; i < table.getTablePlayerNumber(); i++) {
						if (!table.GRR._chi_hu_rights[i].is_valid() && (table._playerStatus[i].has_chi_hu())) {
							table.GRR._chi_hu_rights[i].set_valid(true);// 胡牌生效
							table.process_chi_hu_player_operate(i, new int[] { operate_card }, 1, false);
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			if (table.has_rule(GameConstants.GAME_RULE_JIANGSU_PEI_CHONG)) {
				...
				for (int hu_player = 0; hu_player < table.getTablePlayerNumber(); hu_player++) {
					...
					for (int i = 0; i < table.getTablePlayerNumber(); i++) {
						if (table._peng_palyer_count[hu_player][i] + table._gang_palyer_count[hu_player][i] >= 3) {
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					if (hu_count == 3) {
						table.process_chi_hu_player_score_jszz(i, _out_card_player, _out_card_data, false, true, false,
								false);
						table._player_result.dian_pao_count[i]++;
						table._player_result.jie_pao_count[_out_card_player]++;
					} else {
						if (table.bao_pei_palyer[i] == GameConstants.INVALID_SEAT) {
							...
							table.process_chi_hu_player_score_jszz(i, table.bao_pei_palyer[i], _out_card_data, false,
									false, true, false);
							table._player_result.dian_pao_count[table.bao_pei_palyer[i]]++;
							table._player_result.jie_pao_count[i]++;
						}
			...
			// 记录
			GameSchedule.put(
					new GameFinishRunnable(table.getRoom_id(), table._current_banker, GameConstants.Game_End_NORMAL),
