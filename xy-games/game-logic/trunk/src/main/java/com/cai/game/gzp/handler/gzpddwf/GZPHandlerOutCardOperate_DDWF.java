...
public class GZPHandlerOutCardOperate_DDWF extends GZPHandlerOutCardOperate {
	@Override
	public void exe(GZPTable table) {
		...
		table.estimate_player_hua_ddwf(_out_card_player, _out_card_data,true);
		table.estimate_player_zhao(_out_card_player,_out_card_data);
		table.estimate_player_peng_ddwf(_out_card_player,_out_card_data);
		// 显示出牌
		...
		//出牌后，删除不能出牌的捡牌
		table.cannot_pickup_card_ddwf(_out_card_player, 0, 0, true);
...
//		}
	...
	public boolean handler_operate_card(GZPTable table, int seat_index, int operate_code, int operate_card) {
		...
		table.cannot_hu_zhao_peng_ddwf_card(seat_index,operate_card);
		// 玩家的操作
...
//			}
			...
			if(table.getRuleValue(GameConstants.GAME_RULE_TC_ONE_PAO_DUO_HU)==1)
			{
			    ...
			    for (int i = 0; i < table.getTablePlayerNumber(); i++) {
	            	...
	            	table.operate_effect_action(i, GameConstants.EFFECT_ACTION_TYPE_HU, 1, chr.type_list,
	        				1, GameConstants.INVALID_SEAT);
	            	table.process_chi_hu_player_score_gzp_ddwf(i, _out_card_player, _out_card_data, false);
			...
			else
			{
				...
				table.process_chi_hu_player_score_gzp_ddwf(target_player, _out_card_player, _out_card_data, false);
