package com.cai.game.chdphz.handler.lbaphz;
...
public class PHZHandlerGang_LBA extends PHZHandlerGang<CHDPHZTable> {
...
//		}
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_lba(target_player,_provide_player, _center_card, false);
...
//			}
			...
			table.process_chi_hu_player_score_lba(_seat_index, _provide_player, operate_card, true);
	 ...
	 * 
	 ***/
	protected boolean exe_gang(CHDPHZTable table) {
		...
		if((_depatch == false )&&(table._is_di_hu == true))
		{
		...
		if((_depatch == true)&&(table._ti_two_long[_seat_index] == false)&&(table._is_xiang_gong[_seat_index] == false)){
	        ...
	        if(pai_count == 0)
	        {
	        	...
	        	if(all_hu_xi>=15)
	        	{
					...
					table._da_pai_count[_seat_index]  = 0;
					table._xiao_pai_count[_seat_index] = 0;
					table._hong_pai_count[_seat_index] = 0;
					table._tuan_yuan_count[_seat_index] = 0; 
					table._ying_hu_count[_seat_index] = 0;
					int  is_piao = 0;
					int piao_count = 0;
	        		for(int i = 0; i<table.GRR._weave_count[_seat_index];i++ )
	            	{
	            		...
	            		table._da_pai_count[_seat_index] += table._logic.get_da_card(table._hu_weave_items[_seat_index][i].weave_kind, table._hu_weave_items[_seat_index][i].center_card);
	        			table._xiao_pai_count[_seat_index] += table._logic.get_xiao_card(table._hu_weave_items[_seat_index][i].weave_kind,
	        					table._hu_weave_items[_seat_index][i].center_card);
	            		if(table.getRuleValue(GameConstants.GAME_RULE_YING_CHD) == 1)
	    				{
	            			table._ying_hu_count[_seat_index] += table._logic.get_ying(table._hu_weave_items[_seat_index][i].weave_kind,
	            					table._hu_weave_items[_seat_index][i].center_card);
	    				}
	    				if(table.getRuleValue(GameConstants.GAME_RULE_PIAO_CHD)==1&&is_piao!=2)
	    				{
	    					int piao =  table._logic.get_piao(table._hu_weave_items[_seat_index][i].weave_kind,
	    							table._hu_weave_items[_seat_index][i].center_card);
	    					if(piao == 2)
	    						is_piao = 2;
	    					if(piao == 1)
	        		...
	        		boolean is_man_yuan_hua = table._logic.is_man_yuan_hua(table._hu_weave_items[_seat_index], table._hu_weave_count[_seat_index]);
	        		all_cards_count = hong_pai_count +hei_pai_count;
	        		
					if ((card_type == GameConstants.HU_CARD_TYPE_ZIMO)&&(_seat_index == _provide_player)) {
						chr.opr_or_long(GameConstants.CHR_ZI_MO);
					}
					else if ((card_type == GameConstants.HU_CARD_TYPE_ZIMO)&&(_seat_index != _provide_player)) {
						chr.opr_or_long(GameConstants.CHR_JEI_PAO_HU);
					}
					if (hong_pai_count >= 10) {
						table._hong_pai_count[_seat_index] = hong_pai_count;
						chr.opr_or_long(GameConstants.CHR_TEN_HONG_PAI_CHD);
					}
					if(table.has_rule(GameConstants.GAME_RULE_SI_QI_HONG_CHD))
						if(hong_pai_count == 4 || hong_pai_count == 7){
							chr.opr_or_long(GameConstants.CHR_SI_QI_CHD);
						}
					if (hong_pai_count == 1) {
						chr.opr_or_long(GameConstants.CHR_ONE_HONG_CHD);
					}
					if (hei_pai_count == all_cards_count) {
						chr.opr_or_long(GameConstants.CHR_ALL_HEI_CHD);
					}
					if (table.GRR._left_card_count == 0) {
						chr.opr_or_long(GameConstants.CHR_HAI_HU_CHD);
					}
					if (table._da_pai_count[_seat_index] >= 18) {
						chr.opr_or_long(GameConstants.CHR_DA_HU_CHD);
					}
					if (table._xiao_pai_count[_seat_index] >= 16) {
						chr.opr_or_long(GameConstants.CHR_XIAO_HU_CHD);
					}
					if (table._logic.calculate_pengpeng_count(table._hu_weave_items[_seat_index], table._hu_weave_count[_seat_index])) {
						chr.opr_or_long(GameConstants.CHR_DUI_ZI_HU_CHD);
					}
					
		
					if (table.has_rule(GameConstants.GAME_RULE_HHX_CHD)) {
						if (hang_hang_xi_count == 7) {
							chr.opr_or_long(GameConstants.CHR_HANG_HANG_XI_CHD);
						}
					}
					if (table.has_rule(GameConstants.GAME_RULE_TUAN_CHD)) {
						table._tuan_yuan_count[_seat_index] = table._logic.calculate_tuan_count(table._hu_weave_items[_seat_index], table._hu_weave_count[_seat_index]);

						if (table._tuan_yuan_count[_seat_index] > 0) {
							chr.opr_or_long(GameConstants.CHR_TUAN_CHD);
						}
					}
					if(table.has_rule(GameConstants.GAME_RULE_GAI_CHD))
					{
						if (table.GRR._left_card_count == 18 ) {
							
							chr.opr_or_long(GameConstants.CHR_GAI_CHD);

						}
					}
					if(table.getRuleValue(GameConstants.GAME_RULE_SHUN_CHD)==1){
						if(table._logic.calculate_shun(table._hu_weave_items[_seat_index], table._hu_weave_count[_seat_index]))
						{
							chr.opr_or_long(GameConstants.CHR_SHUN_CHD);
						}
					}
					if(table.getRuleValue(GameConstants.GAME_RULE_PIAO_CHD)==1&&is_piao!=2)
					{
						if(piao_count == 1)
						{
							chr.opr_or_long(GameConstants.CHR_DAN_PIAO_CHD);
						}
						if(piao_count == 2)
						{
							chr.opr_or_long(GameConstants.CHR_SHUANG_PIAO_CHD);
						}
					}
					if(table.getRuleValue(GameConstants.GAME_RULE_YING_CHD) == 1)
					{
						if(table._ying_hu_count[_seat_index]>0)
							chr.opr_or_long(GameConstants.CHR_YING_CHD);
					}
