...
public class PHZHandlerChuLiFirstCard_XT extends HHHandlerDispatchCard<HHTable> {
		...
		//// 玩家出牌 响应判断,是否有提 暗龙
			...
			if(table.has_rule(GameConstants.GAME_RULE_DI_ERZI_LIANG_PAI)){
				...
				if(table.has_rule(GameConstants.GAME_RULE_DI_MING_WEI))
				{
					...
					int an_long_Index_array[][] = new int[table.getTablePlayerNumber()][5];
					int an_long_count_array[] = new int[table.getTablePlayerNumber()];
					Arrays.fill(an_long_count_array,0);
					int k = 0;
					while (k++<table.getTablePlayerNumber())
					{	
						int j = (table.GRR._banker_player+k) % table.getTablePlayerNumber();
						Arrays.fill(an_long_Index_array[j],0);
						for(int i = 0; i< GameConstants.MAX_HH_INDEX;i++)
						{
							if(table.GRR._cards_index[j][i] == 4)
							{
								an_long_Index_array[j][an_long_count_array[j]++] = i;	
								if(table._logic.switch_to_card_index(table._send_card_data) == i)
								{
									ti_send_card = true;
								}
						...
						if(an_long_count_array[j] > 0 )
						{
							int _action = GameConstants.WIK_AN_LONG;
							//效果
							table.operate_effect_action(j,GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[]{_action}, 1,GameConstants.INVALID_SEAT);
						
							for(int i = 0; i< an_long_count_array[j];i++)
							{
								int cbWeaveIndex = table.GRR._weave_count[j];
								table.GRR._weave_items[j][cbWeaveIndex].public_card = 1;
								table.GRR._weave_items[j][cbWeaveIndex].center_card = table._logic.switch_to_card_data(an_long_Index_array[j][i]);
								if(j == _seat_index)
									table.GRR._weave_items[j][cbWeaveIndex].weave_kind = GameConstants.WIK_AN_LONG;
								else
									table.GRR._weave_items[j][cbWeaveIndex].weave_kind = GameConstants.WIK_AN_LONG_LIANG;
								table.GRR._weave_items[j][cbWeaveIndex].provide_player = j;
								table.GRR._weave_items[j][cbWeaveIndex].hu_xi = table._logic.get_weave_hu_xi(table.GRR._weave_items[j][cbWeaveIndex]);
								table.GRR._weave_count[j]++;
								table._long_count[j]++;
								// 删除手上的牌
								table.GRR._cards_index[j][an_long_Index_array[j][i]] = 0;
							...
							// 刷新手牌包括组合
							if(j != _seat_index)
								table._xt_display_an_long[j] = true;
							hand_card_count = table._logic.switch_to_cards_data(table.GRR._cards_index[j], cards);
							table.operate_player_cards(j, hand_card_count, cards, table.GRR._weave_count[j],
									table.GRR._weave_items[j]);
				
						}
					}
					for(int i = 0; i< table.getTablePlayerNumber();i++)
					{
						if(an_long_count_array[i] >= 2)
						{
							table._ti_mul_long[i] = an_long_count_array[i] -1; 
						}
					}
					...
					for(int i = 0; i< table.getTablePlayerNumber();i++){
						...
						int hu_xi[] = new int[1];
						if(table._guo_hu_xt[i]!=-1)
						{
							if(i == _seat_index)
								table._guo_hu_xt[i] = -1;
							else
								continue;
						}
						if(table._guo_hu_xt[i] == -1)
						...
						if (action_hu[i] != GameConstants.WIK_NULL) {
							// 添加动作
							table._guo_hu_xt[i] = _seat_index;
				...
				else if(table.has_rule(GameConstants.GAME_RULE_DI_AN_WEI))
				{
					int an_long_Index_array[][] = new int[table.getTablePlayerNumber()][5];
					int an_long_count_array[] = new int[table.getTablePlayerNumber()];
					Arrays.fill(an_long_count_array,0);
					...
					{	
						...
						if(an_long_count_array[j] > 0 )
						{
							...
							// 刷新手牌包括组合
							if(table.has_rule(GameConstants.GAME_RULE_DI_AN_WEI)&&j != _seat_index)
								table._xt_display_an_long[j] = true;
							hand_card_count = table._logic.switch_to_cards_data(table.GRR._cards_index[j], cards);
							table.operate_player_cards(j, hand_card_count, cards, table.GRR._weave_count[j],
									table.GRR._weave_items[j]);
					...
					if(bHupai == 0){
						...
						if(an_long_count_array[_seat_index]  >= 2)
						{
							table._ti_mul_long[_seat_index] = an_long_count_array[_seat_index]  - 1 ; 
						}
						// 加到手牌
						_send_card_data = 0;
					    hand_card_count = table._logic.switch_to_cards_data(table.GRR._cards_index[_seat_index], cards);
...
//			}
			...
			table.process_chi_hu_player_score_phz_xt(target_player, _seat_index, operate_card, true);
