...
public class MJHandlerOutCardOperate_HaiNan extends MJHandlerOutCardOperate<MJTable_HaiNan> {
	...
	public boolean handler_operate_card(MJTable_HaiNan table, int seat_index, int operate_code, int operate_card) {
		...
		if (table._playerStatus[seat_index].has_action_by_code(GameConstants.WIK_PENG)
			...
			table._playerStatus[seat_index].chi_peng_round_invalid_ext(_out_card_data);
		}
	...
	public boolean handler_player_be_in_room(MJTable_HaiNan table, int seat_index) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (seat_index ==_out_card_player  && i == this._out_card_player) {
