...
public class MJHandlerDispatchCard_ShanXi_KouDian extends MJHandlerDispatchCard<MJTable_KouDian> {
    ...
    public void exe(MJTable_KouDian table) {
        ...
        if (table._playerStatus[_seat_index]._hu_card_count > 0
            ...
            //补杠杠开算点炮，点炮不能胡单吊耗子
            //if(card_type == GameConstants.HU_CARD_TYPE_BU_GANG_KAI && table._playerStatus[_seat_index]._hu_card_count >= 28){
            //	action = GameConstants.WIK_NULL;
            //}
            
            //单吊耗子，不管摸到什么牌都可以自摸       或者报听了摸到癞子只能自动胡牌
            if(/*table._playerStatus[_seat_index]._hu_card_count >= 25 ||*/ table._logic.is_magic_card(_send_card_data)){
