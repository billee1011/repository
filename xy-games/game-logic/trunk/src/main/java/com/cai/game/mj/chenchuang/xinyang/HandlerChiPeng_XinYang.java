...
public class HandlerChiPeng_XinYang extends MJHandlerChiPeng<Table_XinYang> {
	...
	public void exe(Table_XinYang table) {
		// 删除禁锢的牌
		if (_card == table._player_result.pao[_seat_index])
			table._player_result.pao[_seat_index] = 0;
		if (_card == table._player_result.qiang[_seat_index])
		...
		if (ting_count > 0) {
			...
			int tmp_hand_card_count = table.switch_to_cards_data(_seat_index, table.GRR._cards_index[_seat_index], tmp_cards);
			...
			// 刷新手牌
			table.operate_player_cards(_seat_index, hand_card_count, cards, weave_count, weaves);
			table.operate_chi_hu_cards(_seat_index, 0, null);
		}
