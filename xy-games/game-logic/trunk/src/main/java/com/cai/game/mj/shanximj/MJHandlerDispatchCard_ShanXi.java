...
public class MJHandlerDispatchCard_ShanXi extends MJHandlerDispatchCard<Table_ShanXi> {
	...
	public void exe(Table_ShanXi table) {
		...
		int action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], _send_card_data, chr, Constants_ShanXi.HU_CARD_TYPE_ZI_MO, 0);// 自摸
		...
		boolean has_dai_feng = table.has_rule(Constants_ShanXi.GAME_RULE_DAI_FENG_PAI);
		int card_type_count = GameConstants.MAX_ZI_FENG;

		for (int x = 0; x < card_type_count; x++) {
			count = table.GRR._cards_index[_seat_index][x];

			if (count > 0) {
				table.GRR._cards_index[_seat_index][x]--;

				table._playerStatus[_seat_index]._hu_out_card_ting_count[ting_count] = table.get_hnhz_ting_card_new(table._playerStatus[_seat_index]._hu_out_cards[ting_count], table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index],
						has_dai_feng);

				if (table._playerStatus[_seat_index]._hu_out_card_ting_count[ting_count] > 0) {
					table._playerStatus[_seat_index]._hu_out_card_ting[ting_count] = table._logic.switch_to_card_data(x);

					ting_count++;

					if (send_card_index == x) {
						ting_send_card = true;
					}
				}
				table.GRR._cards_index[_seat_index][x]++;
				...
				for (int i = 0; i < hand_card_count; i++) {
					for (int j = 0; j < ting_count; j++) {
						if (cards[i] == table._playerStatus[_seat_index]._hu_out_card_ting[j]) {
							...
							break;
						} else if (table._logic.is_magic_card(cards[i])) {
		...
		if (table._logic.is_magic_card(_send_card_data) && !ting_send_card) {
			table.operate_player_get_card(_seat_index, 1, new int[] { real_card + GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI }, GameConstants.INVALID_SEAT);
		} else {
		...
		// if (table.GRR._left_card_count > llcard) {
		int cbActionMask = table._logic.analyse_gang_by_card(table.GRR._cards_index[_seat_index], _send_card_data, table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], m_gangCardResult);
		...
		if (table.GRR._left_card_count > llcard) {
			m_gangCardResult.cbCardCount = 0;
			int cbActionMask1 = table._logic.analyse_gang_card_all(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], m_gangCardResult, true);
	...
	public boolean handler_operate_card(Table_ShanXi table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_NULL) {
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				GameSchedule.put(new OutCardRunnable(table.getRoom_id(), _seat_index, _send_card_data), SysParamServerUtil.auto_out_card_time_mj_260(), TimeUnit.MILLISECONDS);
			} else {
			...
			for (int j = 0; j < hand_card_count; j++) {
				for (int k = 0; k < out_ting_count; k++) {
					if (hand_cards[j] == table._playerStatus[seat_index]._hu_out_card_ting[k]) {
						...
						break;
					} else if (table._logic.is_magic_card(hand_cards[j])) {
	...
	@Override
	public boolean handler_be_set_trustee(Table_ShanXi table, int seat_index) {
		handler_check_auto_behaviour(table, seat_index, _send_card_data);
		return false;
	}
}
