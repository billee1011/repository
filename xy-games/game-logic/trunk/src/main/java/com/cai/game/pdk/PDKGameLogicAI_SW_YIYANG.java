...
public class PDKGameLogicAI_SW_YIYANG {
	public Map<Integer, Integer> ruleMap = new HashMap<>();

	public PDKGameLogicAI_SW_YIYANG() {
			...
			for (int i = 0; i < GameConstants.PDK_MAX_INDEX; i++) {
				if (hand_index.card_index[i] > 0) {
					if (hand_index.card_index[i] != 3) {
						...
						if (card_count < 5) {
							card_count = 0;
							for (int j = hand_index.card_index[i] - 1; j >= 1; j--) {
								card_data[card_count++] = hand_index.card_data[i][j];
	...
	public int search_out_card_bomb(int cbHandCardData[], int cbHandCardCount, int cbOutCardData[], int out_card_count,
		...
		for (int i = cbHandCardCount - 1; i >= 0;) {
			...
			if (index > turn_index && hand_index.card_index[index] == 4) {
				...
				return card_count;
			} else if (this.has_rule(GameConstants.GAME_RULE_KKK_BOOM)) {
				if (hand_index.card_index[index] == 3 && index == 10) {
					for (int j = 0; j < hand_index.card_index[index]; j++) {
						card_data[card_count++] = hand_index.card_data[index][j];
					}
					return card_count;
	...
	public int search_out_card_plane(int cbHandCardData[], int cbHandCardCount, int cbOutCardData[], int out_card_count,
		...
		for (int index = 0; index < GameConstants.PDK_MAX_INDEX; index++) {
			if (this.has_rule(GameConstants.GAME_RULE_KKK_BOOM) && index == 10 && hand_index.card_index[index] >= 3
					&& this.has_rule(GameConstants.GAME_RULE_ZHADAN_BUKECHAI)) {
				continue;
			}
			if (hand_index.card_index[index] < max_num && hand_index.card_index[index] >= 3 && index > min_index) {
				for (int other_index = index + 1; other_index < GameConstants.PDK_MAX_INDEX; other_index++) {
					if (this.has_rule(GameConstants.GAME_RULE_KKK_BOOM) && index == 10
							&& hand_index.card_index[index] >= 3
							&& this.has_rule(GameConstants.GAME_RULE_ZHADAN_BUKECHAI)) {
						index = other_index;
						break;
					}
	...
	public int search_out_card_double_link(int cbHandCardData[], int cbHandCardCount, int cbOutCardData[],
		...
		for (int index = 0; index < GameConstants.PDK_MAX_INDEX; index++) {
			...
			if (this.has_rule(GameConstants.GAME_RULE_KKK_BOOM) && index == 10 && hand_index.card_index[index] >= 3
					&& this.has_rule(GameConstants.GAME_RULE_ZHADAN_BUKECHAI)) {
				continue;
			} else if (hand_index.card_index[index] < max_num && hand_index.card_index[index] >= 2
					&& index > min_index) {
				for (int other_index = index + 1; other_index < GameConstants.PDK_MAX_INDEX; other_index++) {
					if (this.has_rule(GameConstants.GAME_RULE_KKK_BOOM) && other_index == 10
							&& hand_index.card_index[other_index] >= 3
							&& this.has_rule(GameConstants.GAME_RULE_ZHADAN_BUKECHAI)) {
						break;
					} else if (hand_index.card_index[other_index] < max_num
							&& hand_index.card_index[other_index] >= 2) {
						if ((other_index - index) + 1 == out_card_count / 2) {
							for (int x = other_index; x >= index; x--) {
	...
	public int search_out_card_single_link(int cbHandCardData[], int cbHandCardCount, int cbOutCardData[],
		...
		for (int index = 0; index < GameConstants.PDK_MAX_INDEX; index++) {
			...
			if (this.has_rule(GameConstants.GAME_RULE_KKK_BOOM) && index == 10 && hand_index.card_index[index] >= 3
					&& this.has_rule(GameConstants.GAME_RULE_ZHADAN_BUKECHAI)) {
				continue;
			} else if (hand_index.card_index[index] < max_num && hand_index.card_index[index] > 0
					&& index > min_index) {
				for (int other_index = index + 1; other_index < GameConstants.PDK_MAX_INDEX; other_index++) {
					if (this.has_rule(GameConstants.GAME_RULE_KKK_BOOM) && other_index == 10
							&& hand_index.card_index[other_index] >= 3
							&& this.has_rule(GameConstants.GAME_RULE_ZHADAN_BUKECHAI)) {
						break;
					} else if (hand_index.card_index[other_index] < 4 && hand_index.card_index[other_index] > 0) {
						if ((other_index - index) + 1 == out_card_count) {
							for (int x = other_index; x >= index; x--) {
								card_data[card_count++] = hand_index.card_data[x][0];
							}
							return card_count;
	...
	public int search_out_card_three_take_two(int cbHandCardData[], int cbHandCardCount, int cbOutCardData[],
		...
		AnalysebCardDataToIndex(cbHandCardData, cbHandCardCount, hand_index);
		int turn_index = this.switch_card_to_idnex(cbOutCardData[0]);
		for (int index = GameConstants.PDK_MAX_INDEX - 1; index >= 0; index--) {
			if (this.has_rule(GameConstants.GAME_RULE_KKK_BOOM) && index == 10 && hand_index.card_index[index] >= 3
					&& this.has_rule(GameConstants.GAME_RULE_ZHADAN_BUKECHAI)) {
				continue;
			} else if (index > turn_index && hand_index.card_index[index] >= 3
					&& hand_index.card_index[index] < max_num) {
				if (cbHandCardCount < out_card_count) {
	...
	public int search_out_card_double(int cbHandCardData[], int cbHandCardCount, int cbOutCardData[],
		...
		for (int count = 2; count < max_num; count++) {
			for (int i = GameConstants.PDK_MAX_INDEX - 1; i >= 0; i--) {
				if (this.has_rule(GameConstants.GAME_RULE_KKK_BOOM) && i == 10 && hand_index.card_index[i] >= 3
						&& this.has_rule(GameConstants.GAME_RULE_ZHADAN_BUKECHAI)) {
					continue;
				} else if (i > turn_index && hand_index.card_index[i] == count) {
					card_data[card_count++] = hand_index.card_data[i][0];
					card_data[card_count++] = hand_index.card_data[i][1];
					return card_count;
				}
			}
		}
		...
		for (int i = 0; i < GameConstants.PDK_MAX_INDEX; i++) {
			if (hand_index.card_index[i] == 4) {
				...
				if (hand_index.card_index[i] == 3 && i == 10) {
					for (int j = 0; j < hand_index.card_index[i]; j++) {
						card_data[card_count++] = hand_index.card_data[i][j];
					}
					return card_count;
		...
		if (!max_must) {
			...
			for (int i = GameConstants.PDK_MAX_INDEX - 1; i >= 0; i--) {
				if (this.has_rule(GameConstants.GAME_RULE_KKK_BOOM) && i == 10 && hand_index.card_index[i] >= 3
						&& this.has_rule(GameConstants.GAME_RULE_ZHADAN_BUKECHAI)) {
					continue;
				} else if (i > turn_index && hand_index.card_index[i] < max_num) {
					card_data[card_count++] = hand_index.card_data[i][0];
					return card_count;
				} else {
					break;
		...
		if (cbCardCount == 1) {
			...
			for (int i = GameConstants.PDK_MAX_INDEX - 1; i >= 0; i--) {
				...
				if (card_index.card_index[i] == 3) {
					if (has_rule(GameConstants.GAME_RULE_KKK_BOOM) && i == 10) {
