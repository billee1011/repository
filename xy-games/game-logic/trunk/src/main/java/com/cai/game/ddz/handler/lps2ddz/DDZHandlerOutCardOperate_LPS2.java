...
public class DDZHandlerOutCardOperate_LPS2 extends DDZHandlerOutCardOperate<DDZ_LPS2_Table> {
	@Override
	public void exe(DDZ_LPS2_Table table) {
		...
		if (cbCardType == GameConstants.DDZ_CT_BOMB_CARD || cbCardType == GameConstants.DDZ_CT_MISSILE_CARD) {
			...
			table.send_last_card(true,false,false);
			// 比赛场更新数据
		...
		if(_out_card_player != table.GRR._banker_player){
			if(table.GRR._card_count[_out_card_player] <= table.m_rang_pai_count){
				table._current_player = GameConstants.INVALID_SEAT;
			}
		}else{
		...
		if (pre_turn_out_type > GameConstants.DDZ_CT_THREE
			...
			roomResponse.setType(MsgConstants.RESPONSE_DDZ_EFFECT);
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		if(_out_card_player != table.GRR._banker_player){
			if(table.GRR._card_count[_out_card_player] <= table.m_rang_pai_count){
				...
				table.m_cur_winer = _out_card_player;
				GameSchedule.put(
	...
	public int adjust_out_card_right(DDZ_LPS2_Table table) {
	...
	// 玩家不出
	public void no_out_card(DDZ_LPS2_Table table) {
