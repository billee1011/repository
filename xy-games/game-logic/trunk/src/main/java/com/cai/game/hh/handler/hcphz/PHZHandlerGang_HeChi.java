...
public class PHZHandlerGang_HeChi extends HHHandlerGang<HeChiHHTable> {
	...
	public void exe(HeChiHHTable table) {
		...
		if (_depatch == false)
			table.operate_out_card(this._provide_player, 0, null, GameConstants_HeChi.OUT_CARD_TYPE_MID, GameConstants_HeChi.INVALID_SEAT);
		else
			table.operate_player_get_card(this._provide_player, 0, null, GameConstants_HeChi.INVALID_SEAT, false);

		// 效果
		table.operate_effect_action(_seat_index, GameConstants_HeChi.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { _action }, 5,
				GameConstants_HeChi.INVALID_SEAT);

		// if ((GameConstants_HeChi.GANG_TYPE_AN_GANG == _type) ||
		// (GameConstants_HeChi.GANG_TYPE_JIE_GANG == _type)) {
		// this.exe_gang(table);
		// return;
		// }
		this.exe_gang(table);
		...
		//// table._playerStatus[i].set_status(GameConstants_HeChi.Player_Status_OPR_CARD);//
		// 操作状态
		//// table.operate_player_action(i, false);
		//// }
		//// if(playerStatus.has_chi_hu()){
		//// if(table.has_rule(GameConstants_HeChi.GAME_RULE_LIXIANG_FLS_JIANPAOHU)
		// ||
		// table.isTrutess(i)){
		//// //见炮胡
		//// table.exe_jian_pao_hu(i,GameConstants_HeChi.WIK_CHI_HU,_center_card);
		//// }else{
		//// table._playerStatus[i].set_status(GameConstants_HeChi.Player_Status_OPR_CARD);//操作状态
		//// table.operate_player_action(i, false);
		//// }
		//// }
		// }
	...
	public boolean handler_operate_card(HeChiHHTable table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		if ((operate_code != GameConstants_HeChi.WIK_NULL) && (operate_code != GameConstants_HeChi.WIK_ZI_MO))// 没有这个操作动作
		{
			table.log_player_error(seat_index, "HHHandlerGang_YX出牌操作,没有动作");
			return false;
		}

		if ((operate_code != GameConstants_HeChi.WIK_NULL) && (operate_card != _center_card)) {
		...
		// 这一轮就不能吃胡了没过牌之前都不能胡
		// } else if (operate_code == GameConstants_HeChi.WIK_CHI_HU) {
		...
		case GameConstants_HeChi.WIK_NULL: {
			...
			if ((table._is_xiang_gong[_seat_index] == false)
					&& (table._long_count[_seat_index] == 1 || GameConstants_HeChi.SAO_TYPE_MINE_SAO == _type)) {
				...
				if (pai_count == 0) {
					...
					table._playerStatus[_seat_index].set_status(GameConstants_HeChi.Player_Status_OUT_CARD);// 出牌状态
				...
				table.exe_dispatch_card(next_player, GameConstants_HeChi.WIK_NULL, 1500);
				table.log_info(next_player + "提 扫 跑 发牌" + _seat_index);
		...
		case GameConstants_HeChi.WIK_ZI_MO: // 自摸
		{
			...
			// }
			if (_depatch == true)
				table.operate_player_get_card(this._provide_player, 1, new int[] { _center_card }, GameConstants_HeChi.INVALID_SEAT, false);
			...
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), _seat_index, GameConstants_HeChi.Game_End_NORMAL), delay, TimeUnit.SECONDS);
	 ...
	 * 
	 ***/
	protected boolean exe_gang(HeChiHHTable table) {
		int cbCardIndex = table._logic.switch_to_card_index(_center_card);
		int cbWeaveIndex = -1;
		boolean is_ting_hu = true;
		if (GameConstants_HeChi.PAO_TYPE_AN_LONG == _type) {
			// 暗龙
			// 设置变量
		...
		} else if (GameConstants_HeChi.PAO_TYPE_TI_MINE_LONG == _type) {
			// 提龙
			// 设置变量
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
			table._long_count[_seat_index]++;
		} else if (GameConstants_HeChi.PAO_TYPE_MINE_SAO_LONG == _type) {
			// 提龙
			// 设置变量
			// 看看是不是有碰的牌，明杠
			// 寻找组合
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				...
				if (cbWeaveKind != GameConstants_HeChi.WIK_AN_LONG)
					is_ting_hu = false;
				if ((cbCenterCard == _center_card) && (cbWeaveKind == GameConstants_HeChi.WIK_WEI)) {
		...
		} else if (GameConstants_HeChi.PAO_TYPE_OTHER_SAO_PAO == _type) {
			// 别人打的牌
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				int cbWeaveKind = table.GRR._weave_items[_seat_index][i].weave_kind;
				int cbCenterCard = table.GRR._weave_items[_seat_index][i].center_card;
		...
		} else if (GameConstants_HeChi.PAO_TYPE_OHTER_PAO == _type) {
			// 别人打的牌
			table._long_count[_seat_index]++;
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
		...
		} else if (GameConstants_HeChi.PAO_TYPE_MINE_PENG_PAO == _type) {
			// 看看是不是有碰的牌，明杠
			// 寻找组合
			for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
				int cbWeaveKind = table.GRR._weave_items[_seat_index][i].weave_kind;
				int cbCenterCard = table.GRR._weave_items[_seat_index][i].center_card;
				if ((cbCenterCard == _center_card) && (cbWeaveKind == GameConstants_HeChi.WIK_PENG)) {
					cbWeaveIndex = i;// 第几个组合可以碰
					table._long_count[_seat_index]++;
					break;
				}
		...
		} else if (GameConstants_HeChi.SAO_TYPE_MINE_SAO == _type) {
			// 扫牌
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
		}
		...
		int pai_count = 0;
		int action_hu = GameConstants_HeChi.WIK_NULL;
		for (int i = 0; i < GameConstants_HeChi.MAX_HH_INDEX; i++) {
			if (table.GRR._cards_index[_seat_index][i] < 3)
				pai_count += table.GRR._cards_index[_seat_index][i];
		}
		if ((_depatch == true) && (table._ti_two_long[_seat_index] == false) && (table._is_xiang_gong[_seat_index] == false)) {
			...
			int card_type = GameConstants_HeChi.HU_CARD_TYPE_ZIMO;
			int hu_xi[] = new int[1];
			...
			if (is_ting_hu == true) {
				for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
					int cbWeaveKind = table.GRR._weave_items[_seat_index][i].weave_kind;
					if (!((cbWeaveKind == GameConstants_HeChi.WIK_TI_LONG) || (cbWeaveKind == GameConstants_HeChi.WIK_AN_LONG)
							|| (cbWeaveKind == GameConstants_HeChi.WIK_WEI))) {
						is_ting_hu = false;
					}
				}
			}
			if (action_hu != GameConstants_HeChi.WIK_NULL) {
				PlayerStatus tempPlayerStatus = table._playerStatus[_seat_index];
				tempPlayerStatus.add_action(GameConstants_HeChi.WIK_ZI_MO);
				tempPlayerStatus.add_zi_mo(_center_card, _seat_index);
				tempPlayerStatus.add_action(GameConstants_HeChi.WIK_NULL);
				tempPlayerStatus.add_pass(0, _seat_index);
				if (tempPlayerStatus.has_action()) {
					tempPlayerStatus.set_status(GameConstants_HeChi.Player_Status_OPR_CARD);//
					// 操作状态
					table.operate_player_action(_seat_index, false);
					if (is_ting_hu == true) {
						if ((chr.opr_and(GameConstants_HeChi.CHR_TING_HU)).is_empty())
							chr.opr_or(GameConstants_HeChi.CHR_TING_HU);
					}
					return true;
				}
			} else {
			...
			//
			// for (int i = 0; i < table.GRR._weave_count[_seat_index]; i++) {
			// table._hu_weave_items[_seat_index][i].public_card =
			// table.GRR._weave_items[_seat_index][i].public_card;
			// table._hu_weave_items[_seat_index][i].center_card =
			// table.GRR._weave_items[_seat_index][i].center_card;
			// table._hu_weave_items[_seat_index][i].weave_kind =
			// table.GRR._weave_items[_seat_index][i].weave_kind;
			// table._hu_weave_items[_seat_index][i].provide_player =
			// table.GRR._weave_items[_seat_index][i].provide_player;
			// table._hu_weave_items[_seat_index][i].hu_xi =
			// table.GRR._weave_items[_seat_index][i].hu_xi;
			// if (table._hu_weave_items[_seat_index][i].hu_xi > 0)
			// hang_hang_xi_count++;
			// hong_pai_count +=
			// table._logic.calculate_weave_hong_pai(table._hu_weave_items[_seat_index][i]);
			// hei_pai_count +=
			// table._logic.calculate_weave_hei_pai(table._hu_weave_items[_seat_index][i]);
			//
			// }
			...
			//
			// if ((card_type == GameConstants_HeChi.HU_CARD_TYPE_ZIMO) &&
			// (_seat_index == _provide_player)) {
			// chr.opr_or(GameConstants_HeChi.CHR_ZI_MO);
			// } else if ((card_type ==
			// GameConstants_HeChi.HU_CARD_TYPE_ZIMO) &&
			// (_seat_index != _provide_player)) {
			// chr.opr_or(GameConstants_HeChi.CHR_JEI_PAO_HU);
			// }
			// if (hong_pai_count >= 10) {
			// table._hong_pai_count[_seat_index] = hong_pai_count;
			// chr.opr_or(GameConstants_HeChi.CHR_TEN_HONG_PAI);
			// }
			// if (hong_pai_count == 1) {
			// chr.opr_or(GameConstants_HeChi.CHR_ONE_HONG);
			// }
			// if (hei_pai_count == 1) {
			// chr.opr_or(GameConstants_HeChi.CHR_ONE_HEI);
			// }
			// if (hei_pai_count == all_cards_count) {
			// chr.opr_or(GameConstants_HeChi.CHR_ALL_HEI);
			// }
			// if (table.GRR._left_card_count == 0) {
			// chr.opr_or(GameConstants_HeChi.CHR_HAI_HU);
			// }
			// if (table._da_pai_count[_seat_index] >= 18) {
			// chr.opr_or(GameConstants_HeChi.CHR_DA_HU);
			// }
			// if (table._xiao_pai_count[_seat_index] >= 16) {
			// chr.opr_or(GameConstants_HeChi.CHR_XIAO_HU);
			// }
			// if
			// (table._logic.calculate_pengpeng_count(table._hu_weave_items[_seat_index],
			// table._hu_weave_count[_seat_index])) {
			// chr.opr_or(GameConstants_HeChi.CHR_DUI_ZI_HU);
			// }
			// if
			// (table.has_rule(GameConstants_HeChi.GAME_RULE_DI_HANG_HANG_XI))
			// {
			// if (hang_hang_xi_count == 7) {
			// chr.opr_or(GameConstants_HeChi.CHR_HANG_HANG_XI);
			// }
			// }
			// if
			// (table.has_rule(GameConstants_HeChi.GAME_RULE_DI_TUAN_YUAN))
			// {
			// table._tuan_yuan_count[_seat_index] =
			// table._logic.calculate_tuan_yuan_count(table._hu_weave_items[_seat_index],
			// table._hu_weave_count[_seat_index]);
			//
			// if (table._tuan_yuan_count[_seat_index] > 0) {
			// chr.opr_or(GameConstants_HeChi.CHR_TUAN_YUAN);
			// }
			// }
			// if
			// (table.has_rule(GameConstants_HeChi.GAME_RULE_DI_HUANG_FAN))
			// {
			// if (table._huang_zhang_count >= 1) {
			// chr.opr_or(GameConstants_HeChi.CHR_HUANG_FAN);
			// }
			// }
			...
			//
			// // 添加动作
			// curPlayerStatus.add_action(GameConstants_HeChi.WIK_ZI_MO);
			// curPlayerStatus.add_zi_mo(_center_card, _seat_index);
			// curPlayerStatus.add_action(GameConstants_HeChi.WIK_NULL);
			// curPlayerStatus.add_pass(_center_card, _seat_index);
			// if (curPlayerStatus.has_action()) {
			// curPlayerStatus.set_status(GameConstants_HeChi.Player_Status_OPR_CARD);//
			// // 操作状态
			// table.operate_player_action(_seat_index, false);
			// return true;
			// }
		...
		if ((table._is_xiang_gong[_seat_index] == false) && (table._long_count[_seat_index] == 1 || GameConstants_HeChi.SAO_TYPE_MINE_SAO == _type)) {
			// 要出牌，但是没有牌出设置成相公 下家用户发牌
			if (pai_count == 0) {
				table._is_xiang_gong[_seat_index] = true;
				table.operate_player_xiang_gong_flag(_seat_index, table._is_xiang_gong[_seat_index]);
	...
	public boolean handler_player_be_in_room(HeChiHHTable table, int seat_index) {
		...
		// _player_action[seat_index] : MJGameConstants_HeChi.WIK_NULL);
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < 55; j++) {
				if (table._logic.is_magic_card(table.GRR._discard_cards[i][j])) {
					// 癞子
					int_array.addItem(table.GRR._discard_cards[i][j] + GameConstants_HeChi.CARD_ESPECIAL_TYPE_HUN);
				} else {
			...
			for (int j = 0; j < GameConstants_HeChi.MAX_WEAVE_HH; j++) {
				WeaveItemResponse.Builder weaveItem_item = WeaveItemResponse.newBuilder();
				if (seat_index != i) {
					if ((table.GRR._weave_items[i][j].weave_kind == GameConstants_HeChi.WIK_GANG
							|| table.GRR._weave_items[i][j].weave_kind == GameConstants_HeChi.WIK_ZHAO)
		...
		for (int j = 0; j < hand_card_count; j++) {
			if (table._logic.is_magic_card(cards[j])) {
				cards[j] += GameConstants_HeChi.CARD_ESPECIAL_TYPE_HUN;// 将癞子转下
			}
		}

		for (int i = 0; i < GameConstants_HeChi.MAX_HH_COUNT; i++) {
		...
		// 效果
		table.operate_effect_action(_seat_index, GameConstants_HeChi.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { _action }, 1, seat_index);
