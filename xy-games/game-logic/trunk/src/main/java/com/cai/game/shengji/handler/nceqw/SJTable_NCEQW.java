package com.cai.game.shengji.handler.nceqw;
...
import com.cai.game.shengji.SJGameLogic_NCEQW;
...
import protobuf.clazz.nceqw.nceqwRsp.CallBankerResponse_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.Card_Arrary_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.DiPaiCard_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.GameStart_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.Get_Score_Effect_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.JiaoZhuBegin_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.Kou_Di_Score_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.LiShiCard_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.MaiDiBegin_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.Opreate_RequestWsk_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.OutCardDataNceqw;
import protobuf.clazz.nceqw.nceqwRsp.PaiFenData_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.PukeGameEndNceqw;
import protobuf.clazz.nceqw.nceqwRsp.RefreshCardData_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.SendCard_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.TableResponse_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.TableScore_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.TouXiangResult_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.Zhu_pai_type_Nceqw;
import protobuf.clazz.nceqw.nceqwRsp.effect_type_Nceqw;

public class SJTable_NCEQW extends SJTable {
	...
	public SJGameLogic_NCEQW _logic;

	public SJTable_NCEQW() {
	}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_call_banker = new SJHandlerCallBanker_NCEQW();
		_handler_out_card_operate = new SJHandlerOutCardOperate_NCEQW();
		_out_card_player = GameConstants.INVALID_SEAT;
		_origin_out_card_player = GameConstants.INVALID_SEAT;
		_logic = new SJGameLogic_NCEQW();
	...
	protected boolean on_game_start() {
		...
		// 初始化游戏变量
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_GAME_START);
		roomResponse.setGameStatus(this._game_status);
		roomResponse.setRoomInfo(getRoomInfo());
		// 发送数据
		GameStart_Nceqw.Builder gamestart = GameStart_Nceqw.newBuilder();
	...
	public void Send_get_score_effect(int card_data, int score, int type, int to_player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_SCORE_EFFECT);
		roomResponse.setGameStatus(this._game_status);
		roomResponse.setRoomInfo(getRoomInfo());
		// 发送数据
		Get_Score_Effect_Nceqw.Builder pai_score_data = Get_Score_Effect_Nceqw.newBuilder();
	...
	public void send_effect_type(int seat_index, int type, int data, int to_player) {
		// type=1：毙牌
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_EFFECT_TYPE);
		effect_type_Nceqw.Builder effect = effect_type_Nceqw.newBuilder();
	...
	public void send_kou_di_score() {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_KOU_DI_SCORE);
		roomResponse.setGameStatus(this._game_status);
		roomResponse.setRoomInfo(getRoomInfo());
		// 发送数据
		Kou_Di_Score_Nceqw.Builder kou_di_score = Kou_Di_Score_Nceqw.newBuilder();
	...
	public void Refresh_pai_score(int to_player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_PAI_SCORE);
		roomResponse.setGameStatus(this._game_status);
		roomResponse.setRoomInfo(getRoomInfo());
		// 发送数据
		PaiFenData_Nceqw.Builder pai_score_data = PaiFenData_Nceqw.newBuilder();
	...
	public void Send_card() {
		...
		for (int play_index = 0; play_index < this.getTablePlayerNumber(); play_index++) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_SEND_CARD);
			roomResponse.setGameStatus(this._game_status);
			roomResponse.setRoomInfo(getRoomInfo());
			// 发送数据
			SendCard_Nceqw.Builder send_card = SendCard_Nceqw.newBuilder();
	...
	public void load_player_info_data_game_start(GameStart_Nceqw.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_reconnect(TableResponse_Nceqw.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_game_end(PukeGameEndNceqw.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			rplayer = this.get_players()[i];
			if (rplayer == null)
	...
	protected boolean on_handler_game_finish(int seat_index, int reason) {
		...
		if (reason == GameConstants.Game_End_NORMAL && GRR != null) {
			end_times = cal_score(end_score, seat_index);
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_GAME_END);
		roomResponse.setRoomInfo(getRoomInfo());
		GameEndResponse.Builder game_end = GameEndResponse.newBuilder();
		game_end.setRoomInfo(getRoomInfo());

		load_player_info_data(roomResponse);
		PukeGameEndNceqw.Builder gameend_sj = PukeGameEndNceqw.newBuilder();
		...
		if (GRR != null) {
			...
			if (GRR._banker_player != GameConstants.INVALID_SEAT) {
				gameend_sj.setScoreReason(end_times);
				gameend_sj.setCallScore(_select_dang[GRR._banker_player]);
			} else {
				gameend_sj.setScoreReason(end_times);
				gameend_sj.setCallScore(0);
			}
			gameend_sj.setGetScore(xian_get_score);
			gameend_sj.setBankerPlayer(GRR._banker_player);
			gameend_sj.setDiFen(_kou_di_score);
			gameend_sj.setDiTimes(this._kou_di_fan_bei);
			gameend_sj.setCallTimes(end_times);
		}
		...
		if (GRR != null) {
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				...
				gameend_sj.addEndScore(end_score[i]);
			}
		}
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player) {
		if (to_player == GameConstants.INVALID_SEAT) {
			for (int index = 0; index < this.getTablePlayerNumber(); index++) {
				RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
				OutCardDataNceqw.Builder outcarddata = OutCardDataNceqw.newBuilder();
				Int32ArrayResponse.Builder cards = Int32ArrayResponse.newBuilder();
				roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_OUT_CARD);// 201
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		...
		if (type == MsgConstants.REQUST_NCEQW_OPERATE) {
			Opreate_RequestWsk_Nceqw req = PBUtil.toObject(room_rq, Opreate_RequestWsk_Nceqw.class);
			// 逻辑处理
	...
	public void send_tou_xiang_result(int seat_index) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_TOU_XIANG);
		TouXiangResult_Nceqw.Builder tou_xiang_result = TouXiangResult_Nceqw.newBuilder();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public void deal_dipai(int seat_index) {
		...
		// 发送数据
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_DI_PAI_CARD);
		DiPaiCard_Nceqw.Builder di_pai_card = DiPaiCard_Nceqw.newBuilder();
		for (int i = 0; i < this._di_pai_count; i++) {
	...
	public void send_zhu_pai_type(int seat_index, int type) {
		if (seat_index == GameConstants.INVALID_SEAT) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_ZHU_TYPE);
			Zhu_pai_type_Nceqw.Builder di_pai_card = Zhu_pai_type_Nceqw.newBuilder();
	...
	public void RefreshCard(int to_player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_REFRESH_CARD);
		// 发送数据
		RefreshCardData_Nceqw.Builder refresh_card = RefreshCardData_Nceqw.newBuilder();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
			if (to_player == i) {
				for (int j = 0; j < this.GRR._card_count[i]; j++) {
	...
	public void send_history(int seat_index) {
		// 发送数据
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_LI_SHI_CARD);
		LiShiCard_Nceqw.Builder history_card = LiShiCard_Nceqw.newBuilder();
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			Card_Arrary_Nceqw.Builder card_array = Card_Arrary_Nceqw.newBuilder();
	...
	public void send_call_banker_respnse(int call_player, int call_action, int to_player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_CALL_BANKER);
		roomResponse.setRoomInfo(getRoomInfo());
		// 发送数据
		CallBankerResponse_Nceqw.Builder callbanker_response = CallBankerResponse_Nceqw.newBuilder();
		...
		if (to_player == GameConstants.INVALID_SEAT) {
			for (int idnex = 0; idnex < this.getTablePlayerNumber(); idnex++) {
				if (idnex == GRR._banker_player) {
					// 发送数据
					RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
					roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_MAI_DI_BEGIN);
					roomResponse.setRoomInfo(getRoomInfo());
					MaiDiBegin_Nceqw.Builder mai_pai_begin = MaiDiBegin_Nceqw.newBuilder();
	...
	public void send_jiao_zhu_begin(int to_player) {
		// 发送数据
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_JIAO_ZHU_BEGIN);
		roomResponse.setRoomInfo(getRoomInfo());
		JiaoZhuBegin_Nceqw.Builder jiaozhu_begin = JiaoZhuBegin_Nceqw.newBuilder();
		for (int i = 3; i >= 0; i--) {
	...
	public void RefreshScore(int to_player) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(SJMsgConstants.RESPONSE_NCEQW_REFRESH_SCORE);
		// 发送数据
		TableScore_Nceqw.Builder refresh_score = TableScore_Nceqw.newBuilder();
		int get_score = 0;
		int yu_score = 200;
		int banker_score = 0;
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
