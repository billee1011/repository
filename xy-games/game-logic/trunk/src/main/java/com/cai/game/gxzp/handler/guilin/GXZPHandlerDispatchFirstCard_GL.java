...
public class GXZPHandlerDispatchFirstCard_GL extends GXZPHandlerDispatchCard<GXZPTable> {
	        	...
	        	if(all_hu_xi>=max_hu_xi)
	        	{
					...
					for(int j = 0; j< GameConstants.MAX_HH_INDEX;j++)
					{
						...
						if(table.GRR._cards_index[i][j] == 4)
						{
							...
							table._hu_weave_items[i][weave_count].hu_xi = table._logic.get_weave_hu_xi(table._hu_weave_items[i][weave_count],table.has_rule(GameConstants.GAME_RULE_HONG_PENG));
						...
						if(table.GRR._cards_index[i][j] == 3)
						{
							...
							table._hu_weave_items[i][weave_count].hu_xi = table._logic.get_weave_hu_xi(table._hu_weave_items[i][weave_count],table.has_rule(GameConstants.GAME_RULE_HONG_PENG));	
							table._hu_xing_count[_seat_index] = 3;
							weave_count++;
						}
						if((i == table._current_player)&&(j == send_index))
							table.GRR._cards_index[i][j]--;
