...
import com.cai.future.runnable.XPLPDelayZimoRunnable;
...
public class LPHandlerDispatchCard_XP extends LPHandlerDispatchCard<LPTable_XP> {
	...
	public void exe(LPTable_XP table) {
		...
		Arrays.fill(table._can_not_pen_index[_seat_index], 0);
		...
		// }
		if (table._player_result.nao[_seat_index] < 1) {
			int action = table.analyse_chi_hu_card_xp(table.GRR._cards_index[_seat_index],
		...
		if (table._send_card_count == 1 && !table.has_rule(GameConstants.GAME_RULE_XP_LP_GU_CHOU_NO)) {
			table.set_handler(table._handler_gu);
			table._handler_gu.SetSeatIndex(_seat_index, _send_card_data);
			table._handler_gu.exe(table);
			return;
		}
		...
		if (curPlayerStatus.has_action()) {// 有动作
			if (curPlayerStatus.has_zi_mo()) {
				...
				// 自摸音效延迟
				GameSchedule.put(new XPLPDelayZimoRunnable(table.getRoom_id(), table, _seat_index, _send_card_data), 1,
						TimeUnit.SECONDS);
	...
	public boolean handler_operate_card(LPTable_XP table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			table.process_chi_hu_player_score_xp(_seat_index, _seat_index, operate_card, true);
			// 记录
			table._player_result.zi_mo_count[_seat_index]++;
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), _seat_index, GameConstants.Game_End_NORMAL),
					GameConstants.GAME_FINISH_DELAY, TimeUnit.SECONDS);
