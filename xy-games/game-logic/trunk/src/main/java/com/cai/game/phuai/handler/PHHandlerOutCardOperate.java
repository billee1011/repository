package com.cai.game.phuai.handler;
...
public class PHHandlerOutCardOperate<T extends PHAITable> extends PHHandler<T> {

	public int _out_card_player = GameConstants.INVALID_SEAT; // 出牌用户
	public int _out_temp_card_data = GameConstants.INVALID_VALUE; //出牌扑克
	...
	public boolean handler_player_be_in_room(PHAITable table,int seat_index) {
		...
		{
			//出牌
			table.operate_out_card(_out_card_player, 1, new int[]{_out_card_data},GameConstants.OUT_CARD_TYPE_MID,seat_index,1);
		}
		else{
			//出牌
			table.operate_out_card(_out_card_player, 1, new int[]{_out_card_data},GameConstants.OUT_CARD_TYPE_MID,seat_index,0);
		}
