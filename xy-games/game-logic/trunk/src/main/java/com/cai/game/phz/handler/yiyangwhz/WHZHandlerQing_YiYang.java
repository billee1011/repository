...
public class WHZHandlerQing_YiYang extends PHZHandlerLiu<YiYangWHZTable> {
	
	private GangCardResult m_gangCardResult;
	
	public WHZHandlerQing_YiYang(){
	...
	public void exe(YiYangWHZTable table) {
		...
		if(_action == GameConstants.WIK_YIYANGWHZ_QING_WAI){
			table.GRR._cards_index[_seat_index][table._logic.switch_to_card_index(_card)]=0;
			// 组合扑克
			int wIndex = -1;
			for(int i=0;i<table.GRR._weave_count[_seat_index];i++){
				if(table.GRR._weave_items[_seat_index][i].weave_kind == GameConstants.WIK_YIYANGWHZ_WAI
			...
			if(table.GRR._cards_index[_seat_index][table._logic.switch_to_card_index(_card)] != 4){
				// 用户状态
