...
public class HandlerPao_JingDeZhen extends AbstractMJHandler<Table_JingDeZhen> {
	...
	public boolean handler_pao_qiang(Table_JingDeZhen table, int seat_index, int pao, int qiang) {
		if (table._is_pao_qiang[seat_index])
			return false;

		table._is_pao_qiang[seat_index] = true;
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if (table._is_pao_qiang[i] == false) {
				return true;
			}
		}
		// 都ok了
	...
	public boolean handler_player_be_in_room(Table_JingDeZhen table, int seat_index) {
		 ...
		 * table.switch_to_cards_data(table.GRR._cards_index[seat_index],
		 * hand_cards);
		 * 
		 * 
	...
	private void player_reconnect(Table_JingDeZhen table, int seat_index) {
		if (table._is_pao_qiang[seat_index] == true) {
			return;
		}
		...
		roomResponse.setPaoDes("当前可以撤炮,最多下炮4个");
		table.send_response_to_player(seat_index, roomResponse);
