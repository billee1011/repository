...
public class HandlerGang_WuYuan extends MJHandlerGang<Table_WuYuan> {
...
//		}
	...
	@Override
	protected boolean exe_gang(Table_WuYuan table) {
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			int score = (GameConstants.CELL_SCORE << 1) + 1;
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == _seat_index) {
					continue;
				}
			...
			// = -score;
			//
			// table._player_result.men_qing[_provide_player]++;
	...
	public boolean handler_operate_card(Table_WuYuan table, int seat_index, int operate_code, int operate_card) {
		...
		if (jie_pao_count > 0) {
			...
			table.hu_pai_player = target_player;
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if ((i == _seat_index) || (table.GRR._chi_hu_rights[i].is_valid() == false)) {
					continue;
				}
