...
public class HandlerOutCardOperate_GuangChang extends MJHandlerOutCardOperate<Table_GuangChang> {
	...
	public void exe(Table_GuangChang table) {
		table.player_out_card_count[_out_card_player]++;
		//清空出牌人的状态
		...
		}else if (table.first_round) {
			if(_out_card_data == table.gen_zhuang_card){
				table.operate_effect_action(_out_card_player, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1,
						new long[] { GameConstants_GD_HUNDRED.WIK_GEN_ZHUANG }, 1, GameConstants.INVALID_SEAT);
			}else{
				table.gen_zhuang_card = -1;
			}
		}
		// 跟庄啦
		if (table.first_round && table.gen_zhuang_card != -1 && _out_card_player == table.last_out_player_4_banker && table.getTablePlayerNumber() == 4) {
			table.gen_zhuang_card = -2;
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				if (i == table._cur_banker)
					continue;
				table.GRR._game_score[i] += table.getRoomScore();
				table.GRR._game_score[table._cur_banker] -= table.getRoomScore();
			}
		}
	...
	public boolean handler_operate_card(Table_GuangChang table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_CHI_HU: {
			...
			if(table.GRR._banker_player == _out_card_player && table.player_out_card_count[_out_card_player] == 1){
				table.GRR._chi_hu_rights[target_player].set_empty();
				table.GRR._chi_hu_rights[target_player].opr_or(Constants_GuangChang.CHR_DI_HU);
			}else if(_type == 3){
				table.GRR._chi_hu_rights[target_player].opr_or(Constants_GuangChang.CHR_GANG_SHANG_PAO);
			}
			table.GRR._chi_hu_rights[target_player].set_valid(true);
			table.GRR._chi_hu_rights[_out_card_player].opr_or(Constants_GuangChang.CHR_FANG_PAO);
