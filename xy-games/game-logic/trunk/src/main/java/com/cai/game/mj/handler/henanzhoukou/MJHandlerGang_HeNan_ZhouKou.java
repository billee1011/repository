...
public class MJHandlerGang_HeNan_ZhouKou extends MJHandlerGang<MJTable> {
    @Override
    public void exe(MJTable table) {
        ...
        /**
         * //抢杠胡判断 if(!table.has_rule(GameConstants.GAME_RULE_HENAN_QIANG_GANG_HU)) {
         * this.exe_gang(table); return; }
         **/

        // 检查对这个杠有没有 胡
        boolean bAroseAction = table.estimate_gang_respond_henan_zhou_kou(_seat_index, _center_card);
        ...
        if (bAroseAction == false) {
            ...
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                ...
                if (playerStatus.has_chi_hu()) {
                    /**
                     * if(table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU)){ //见炮胡
                     * table.exe_jian_pao_hu(i,GameConstants.WIK_CHI_HU,_center_card); }else{
    ...
    protected boolean exe_gang(MJTable table) {
        ...
        // 检查听牌，杠之后要检查有没有换章
        table._playerStatus[_seat_index]._hu_card_count = table.get_henan_zhou_kou_ting_card(
        ...
        if (GameConstants.GANG_TYPE_AN_GANG == _type) {
            ...
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += score;// m_pGameServiceOption->lCellScore*2;//配置参数乘
            table.GRR._gang_score[_seat_index].scores[cbGangIndex][provider] -= score;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
