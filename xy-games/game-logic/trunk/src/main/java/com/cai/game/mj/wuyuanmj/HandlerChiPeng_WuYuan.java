...
public class HandlerChiPeng_WuYuan extends MJHandlerChiPeng<Table_WuYuan> {
	...
	public void exe(Table_WuYuan table) {
		...
		for (int i = 0; i < card_type_count; i++) {
			...
			if (count > 0) {
				...
				int card_data = table._logic.switch_to_card_data(i);
				if (table._playerStatus[_seat_index]._hu_out_card_ting_count[ting_count] > 0) {
					table._playerStatus[_seat_index]._hu_out_card_ting[ting_count] = card_data;
		...
		if (ting_count > 0) {
			...
			for (int i = 0; i < tmp_hand_card_count; i++) {
				...
				if (tmp_cards[i] > GameConstants.CARD_ESPECIAL_TYPE_TING) {
					if (table._logic.is_magic_card(tmp_cards[i] - GameConstants.CARD_ESPECIAL_TYPE_TING)) {
						tmp_cards[i] += GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI;
					}
				} else if (tmp_cards[i] < GameConstants.CARD_ESPECIAL_TYPE_LAI_ZI) {
	...
	public boolean handler_player_be_in_room(Table_WuYuan table, int seat_index) {
		...
		// 打出去可以听牌的个数
		roomResponse.setOutCardCount(hand_card_count);
