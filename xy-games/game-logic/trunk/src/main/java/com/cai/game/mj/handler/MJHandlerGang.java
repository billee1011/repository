...
public abstract class MJHandlerGang<T extends AbstractMJTable> extends AbstractMJHandler<T> {
	...
	protected int _type;
	protected int _operate_card; // 锁杠时 时间操作的牌 配合中心牌使用
	...
	public void reset_suo_gang_status(int seat_index, int provide_player, int center_card, int operate_card, int action, int type) {
		_seat_index = seat_index;
		_provide_player = provide_player;
		_center_card = center_card;
	...
	//
	// //检查对这个杠有没有 胡
	// boolean bAroseAction = table.estimate_gang_respond(_seat_index,
	// _center_card);
	//
	...
	// */
	// @Override
	// public boolean handler_operate_card(T table,int seat_index, int
	// operate_code,
	// int operate_card){
	// //抢杠胡
	//
	...
	// }
	//
	// if ((operate_code != GameConstants.WIK_NULL) && (operate_code !=
	// GameConstants.WIK_CHI_HU))// 没有这个操作动作
	// {
	// table.log_player_error(seat_index, "出牌操作,没有动作");
	// return false;
	// }
	//
	// if((operate_code!=GameConstants.WIK_NULL) && (operate_card !=
	// _center_card)){
	...
	// if(jie_pao_count>1){
	// //红中通炮就不加鸟了
	// table.set_niao_card(_provide_player,GameConstants.INVALID_VALUE,true,add_niao);//
	// 结束后设置鸟牌
	//
	// //结束
	...
	// }
	// }
	// table.set_niao_card(seat_index,GameConstants.INVALID_VALUE,true,add_niao);//
	// 结束后设置鸟牌
	//
