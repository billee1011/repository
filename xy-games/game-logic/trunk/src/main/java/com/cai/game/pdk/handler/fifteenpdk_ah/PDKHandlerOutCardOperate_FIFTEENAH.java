...
public class PDKHandlerOutCardOperate_FIFTEENAH extends PDKHandlerOutCardOperate {
	@Override
	public void exe(PDKTable table) {
		...
		if (table._logic.Player_Can_out_card(table.GRR._cards_data[next_player], table.GRR._card_count[next_player],
			...
			if (table.has_rule(GameConstants.GAME_RULE_OFFLINE_NO_AUTO) && !table._enter_back[next_player]) {
				if (table.get_players()[next_player].isOnline()) {
					table._auto_out_card_scheduled = GameSchedule.put(new PDKAutoOutCardRunnable(table.getRoom_id(),
							next_player, table, GameConstants.PDK_CT_PASS), 1000, TimeUnit.MILLISECONDS);
				}
			} else {
