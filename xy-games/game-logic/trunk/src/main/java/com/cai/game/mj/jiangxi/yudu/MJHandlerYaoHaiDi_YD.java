...
public class MJHandlerYaoHaiDi_YD extends MJHandlerYaoHaiDi<MJTable_YD> {

    private static Logger logger = Logger.getLogger(MJHandlerYaoHaiDi_YD.class);
    ...
    public void exe(MJTable_YD table) {
        ...
        int action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
                table.GRR._weave_count[_seat_index], _send_card_data, chr, GameConstants_YD.HU_CARD_TYPE_ZI_MO, _seat_index);// 自摸

        if (action != GameConstants.WIK_NULL) {
            // 添加动作
            curPlayerStatus.add_action(GameConstants.WIK_ZI_MO);
            curPlayerStatus.add_zi_mo(_send_card_data, _seat_index);
            chr.opr_or(GameConstants_YD.CHR_ZI_MO);// 海底捞
            has_action = true;
        }
        ...
        for (int i = 0; i < table.getTablePlayerNumber(); i++) {
            ...
            action = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i],
                    table.GRR._weave_count[i], _send_card_data, chr, GameConstants_YD.CHR_JIE_PAO, i);// 自摸

            if (action != GameConstants.WIK_NULL) {
                // 添加动作
                table._playerStatus[i].add_action(GameConstants.WIK_CHI_HU);
                table._playerStatus[i].add_chi_hu(_send_card_data, _seat_index);
                table.GRR._chi_hu_rights[i].opr_or(GameConstants_YD.CHR_JIE_PAO);// 海底炮
