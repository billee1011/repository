...
public class HandlerOutCardOperate_LUHE extends MJHandlerOutCardOperate<Table_LUHE> {

	public int _out_card_player;
	public int _out_card_data;
	public int _type;

	public ScheduledFuture _trustee_schedule_action[]; // 托管定时器
	...
	public void exe(Table_LUHE table) {
		...
		int next = (_out_card_player + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();
		table._playerStatus[next].chi_hu_round_valid();// 下家可以胡了
		table._playerStatus[next].clear_cards_abandoned_hu();
		...
		// 包四改变
		table.changeBaoSi(_out_card_player);
			...
			for (int  i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if (playerStatus.has_action()) {
					...
					if (!table.istrustee[i]) {
						if (_trustee_schedule_action == null) {
							_trustee_schedule_action = new ScheduledFuture[GameConstants.GAME_PLAYER];
						}

						if (_trustee_schedule_action[i] != null) {
							_trustee_schedule_action[i].cancel(false);
							_trustee_schedule_action[i] = null;
						}
						final int index = i;
						_trustee_schedule_action[i] = GameSchedule.put(new Runnable() {
							@Override
							public void run() {
								table.handler_request_trustee_action(index, true);
								handler_operate_card(table, index,  GameConstants.WIK_NULL, 0);
							}
						}, GameConstants.TRUSTEE_TIME_OUT_SECONDS_30, TimeUnit.SECONDS);
					}
				}
				handler_check_auto_behaviour_not_gold(table, i, _out_card_player);
			}
		}
	}
	...
	public boolean handler_operate_card(Table_LUHE table, int seat_index, int operate_code, int operate_card) {
		...
		if (_trustee_schedule_action!=null && _trustee_schedule_action[seat_index] != null) {
			_trustee_schedule_action[seat_index].cancel(false);
			_trustee_schedule_action[seat_index] = null;
		}
		...
		case GameConstants.WIK_CHI_HU: {
			...
			if (jie_pao_count > 0) {
				...
				// if
				// (table.has_rule(GameConstants_LUHE.GAME_RULE_PAO_HU_JIANG_MA))
				table.set_niao_card(niao_seat, 0, true, 0);
				...
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					table.process_chi_hu_player_score_luhe(i, _out_card_player, _out_card_data, false, false);
	...
	/*
	 * protected boolean handler_check_auto_behaviour(Table_LUHE table, int
	 * seat_index, int card_data) { if (table.istrustee[seat_index])// 非金币场 无托管
	 * return false;
	 * 
	 * PlayerStatus curPlayerStatus = table._playerStatus[seat_index]; if
	 * (curPlayerStatus.has_action()) {// 有动作
	 * table.operate_player_action(seat_index, true); if
	 * (curPlayerStatus.has_zi_mo() && card_data != GameConstants.INVALID_VALUE)
	 * { table.exe_jian_pao_hu(seat_index, GameConstants.WIK_ZI_MO, card_data);
	 * } else { table.exe_jian_pao_hu(seat_index, GameConstants.WIK_NULL,
	 * card_data); } return true; } else if (curPlayerStatus.get_status() ==
	 * GameConstants.Player_Status_OUT_CARD) { // 出牌状态
	 * 
	 * int out_card = GameConstants.INVALID_VALUE; int card_index =
	 * table._logic.switch_to_card_index(card_data); if (card_index !=
	 * GameConstants.MAX_INDEX && table.GRR._cards_index[seat_index][card_index]
	 * > 0) {// 手牌有 out_card = card_data; } else { for (int i = 0; i <
	 * GameConstants.MAX_FLS_INDEX; i++) { if
	 * (table.GRR._cards_index[seat_index][i] > 0) {// 托管 随意出一张牌 out_card =
	 * table._logic.switch_to_card_data(i); } } }
	 * 
	 * if (out_card != GameConstants.INVALID_VALUE) { GameSchedule.put(new
	 * OutCardRunnable(table.getRoom_id(), seat_index, out_card), 30000,
