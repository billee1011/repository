 ...
 *
 */
public class ChiPengCardRunnable extends BaseFuture {
	...
	private int _action;
	private int _type;
	private int _depatch;


	public ChiPengCardRunnable(int room_id, int seat_index, int provide_player, int center_card, int action,
	        int depatch) {
		super(room_id);
		_room_id = room_id;
	...
	public void execute() {
		try {
			...
			try {
				roomLock.lock();
				table.runnable_chi_peng_card_data(_seat_index, _provide_player, _center_card, _action,  _depatch);
			} finally {
				roomLock.unlock();
