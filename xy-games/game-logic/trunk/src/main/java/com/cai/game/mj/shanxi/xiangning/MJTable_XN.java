...
public class MJTable_XN extends AbstractMJTable {

	/** 选金handler */
	private MJHandlerJing_XN handlerJing_XN;

	/** 报听handler */
	private MJHandlerOutCardBaoTing_XN baoTing_XN;
	...
	public int getTablePlayerNumber() {
		if (has_rule(GameConstants_XiangNing.GAME_RULE_MJ_XN_PLAYER_NUMBER_2)) {
			return GameConstants.GAME_PLAYER - 2;
		} else if (has_rule(GameConstants_XiangNing.GAME_RULE_MJ_XN_PLAYER_NUMBER_3)) {
	...
	protected void onInitTable() {
		_handler_dispath_card = new MJHandlerDispatchCard_XN();
		_handler_out_card_operate = new MJHandlerOutCardOperate_XN();
		_handler_gang = new MJHandlerGang_XN();
		_handler_chi_peng = new MJHandlerChiPeng_XN();
		handlerJing_XN = new MJHandlerJing_XN();
		baoTing_XN = new MJHandlerOutCardBaoTing_XN();
	...
	public int analyse_chi_hu_card(int[] cards_index, WeaveItem[] weaveItems, int weave_count, int cur_card,
		...
		// 手上有金没有摔过金不给胡
		if (cbCardIndexTemp[_logic.get_magic_card_index(0)] > 0 && _player_result.biaoyan[_seat_index] <= 0 && !outTingCardIsMagic) {
			return cbChiHuKind;
		}
		...
		if (can_win) {
			...
			if (tingCardCount == 1) {// 1.只胡一张；卡边吊：两嘴
		...
		} else if (qxd != GameConstants.WIK_NULL) {
			if (tingCardCount == 1) {
				chiHuRight.opr_or(GameConstants.CHR_HENAN_DAN_DIAO);
				win_one = true;
			}
		...
		// 接炮不是赢一张
		if (!zimo && !win_one && cbCardIndexTemp[_logic.get_magic_card_index(0)] == 0
				&& _player_result.biaoyan[_seat_index] == 0) {
	...
	public void process_chi_hu_player_score(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		if (zimo) {
			...
			if (has_rule(GameConstants_XiangNing.GAME_RULE_MJ_XN_BAO_TING_BAO_HU)
					&& !_playerStatus[provide_index].is_bao_ting()) {
	...
	public int get_chi_hu_fen(ChiHuRight chiHuRight, int seat_index, boolean zimo) {
		int wFanShu = 0;
		if (_player_result.biaoyan[seat_index] == 3) {
			if (isWinOne(chiHuRight)) {
				if (zimo) {
					if (!has_rule(GameConstants_XiangNing.GAME_RULE_MJ_XN_BU_FAN)) {
						wFanShu = 34;
					} else {
						wFanShu = 18;
					}
		...
		} else if (_player_result.biaoyan[seat_index] == 2) {
			if (isWinOne(chiHuRight)) {
				if (zimo) {
					if (!has_rule(GameConstants_XiangNing.GAME_RULE_MJ_XN_BU_FAN)) {
						wFanShu = 18;
		...
		} else if (_player_result.biaoyan[seat_index] == 1) {
			if (isWinOne(chiHuRight)) {
				if (zimo) {
					wFanShu = 10;
	...
	public boolean isWinOne(ChiHuRight chiHuRight) {
		if (!(chiHuRight.opr_and(GameConstants.CHR_HENAN_BIAN_ZHANG)).is_empty()
				|| !(chiHuRight.opr_and(GameConstants.CHR_HENAN_KA_ZHANG)).is_empty()
				|| !(chiHuRight.opr_and(GameConstants.CHR_HENAN_DAN_DIAO)).is_empty()) {
			return true;
		}
		return false;
	}
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < l; j++) {
				...
				if (GRR._chi_hu_rights[i].is_valid()) {
					...
					if (!winOne) {
						if (type == GameConstants.CHR_HENAN_DAN_DIAO || type == GameConstants.CHR_HENAN_BIAN_ZHANG
								|| type == GameConstants.CHR_HENAN_KA_ZHANG) {
							des += " 一张赢 ";
							winOne = true;
						}
					}
	...
	public int get_ting_card(int[] cards, int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, boolean dai_feng,
		...
		if (count == 0) {
			/*
			 * cards[count] =
			 * _logic.switch_to_card_data(_logic.get_magic_card_index(0))+
			 * GameConstants.CARD_ESPECIAL_TYPE_JING; count++;
			 */
			...
			if(_player_result.biaoyan[seat_index] > 0){
				cards[count] = _logic.switch_to_card_data(_logic.get_magic_card_index(0))
						+ GameConstants.CARD_ESPECIAL_TYPE_JING;
				count++;
			}
		} else if (count == l) {
			count = 1;
	...
	public int get_ting_card_hu(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, boolean dai_feng,
		...
		if(AnalyseCardUtil.analyse_win_by_cards_index_taojiang(cbCardIndexTemp, cbCurrentCardIndex,
				magic_cards_index, magic_card_count)){
			count++;
		}
		return count;
	...
	public boolean estimate_player_out_card_respond(int seat_index, int card, int type) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (has_rule(GameConstants_XiangNing.GAME_RULE_MJ_XN_BAO_TING_BU_BAO_HU)
					|| (has_rule(GameConstants_XiangNing.GAME_RULE_MJ_XN_BAO_TING_BAO_HU)
				...
				if (_playerStatus[i].is_chi_hu_round()) {
					...
					tingCardCount = getTingCardCount(i);
	...
	public boolean estimate_gang_respond(int seat_index, int card) {
		...
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			...
			if (playerStatus.is_chi_hu_round()) {
				...
				chr.set_empty();
				int cbWeaveCount = GRR._weave_count[i];
				tingCardCount = getTingCardCount(seat_index);
				action = analyse_chi_hu_card(GRR._cards_index[i], GRR._weave_items[i], cbWeaveCount, card, chr,
	...
	protected boolean on_handler_game_finish(int seat_index, int reason) {
		...
		if (GRR != null) {
			...
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				GRR._start_hu_score[i] = (int) GRR._game_score[i];
			}
			for (int i = 0; i < getTablePlayerNumber(); i++) {
		...
		for (int i = 0; i < ting_count; i++) {
			...
			if (_logic.is_magic_card(_playerStatus[seat_index]._hu_out_card_ting[i])) {
				roomResponse.addOutCardTing(
						_playerStatus[seat_index]._hu_out_card_ting[i] + GameConstants.CARD_ESPECIAL_TYPE_JING);
			} else {
				roomResponse.addOutCardTing(
	...
	public void changeCard(int cards[]) {
		for (int j = 0; j < cards.length; j++) {
			...
			if (_logic.is_magic_card(cards[j])) {
				cards[j] += GameConstants.CARD_ESPECIAL_TYPE_JING;
			}
		}
	}
	...
	public int changeCard(int cards) {
		if (_logic.is_magic_card(cards)) {
			cards += GameConstants.CARD_ESPECIAL_TYPE_JING;
		}
	...
	public void test_cards() {
		int[] cards_of_player0 = new int[] { 0x01,0x01,0x01,0x02,0x02,0x06,0x07,0x11,0x12,0x13,0x25,0x25,0x25 };
		int[] cards_of_player1 = new int[] { 0x01,0x01,0x01,0x02,0x02,0x06,0x07,0x11,0x12,0x13,0x25,0x25,0x25};
		int[] cards_of_player3 = new int[] { 0x03, 0x03, 0x03, 0x07, 0x08, 0x16, 0x16, 0x23, 0x24, 0x26, 0x26, 0x26,
				0x06 };
		int[] cards_of_player2 = new int[] { 0x03, 0x03, 0x03, 0x07, 0x08, 0x16, 0x16, 0x23, 0x24, 0x26, 0x26, 0x26,
				0x06 };
	...
	public int get_real_card(int card) {
		...
		} else if (card > GameConstants.CARD_ESPECIAL_TYPE_JING && card < (GameConstants.CARD_ESPECIAL_TYPE_JING+GameConstants.CARD_ESPECIAL_TYPE_TING)) { // 山西乡宁金牌
