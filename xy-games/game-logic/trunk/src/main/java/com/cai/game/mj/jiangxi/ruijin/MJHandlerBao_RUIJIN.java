...
import com.cai.future.runnable.RevomeBaoMiddleCardRunnable;
 ...
 */
public class MJHandlerBao_RUIJIN extends AbstractMJHandler<MJTable_RUIJIN> {
	...
	public void exe(MJTable_RUIJIN table) {
		...
		if (index <= table._logic.switch_to_card_index(0x37)) {
			...
			table._logic.clean_magic_cards();
			for (int i = 0; i < 4; i++) {
				table._logic.add_magic_card_index(table._logic.switch_to_card_index(0x38)+i);
			}
		}
		...
		*/
		...
		/*for (int i = 0; i < 2; i++) {
			...
			table.GRR._especial_show_cards[table.GRR._especial_card_count] =  table._logic.switch_to_card_data(index);
			table.GRR._especial_card_count ++;
		//}
		

		// 延迟调度打点结束（发牌）
		GameSchedule.put(new RevomeBaoMiddleCardRunnable(table.getRoom_id(), _banker), 3, TimeUnit.SECONDS);

	}

	public int CalculateMasterCard(MJTable_RUIJIN table, int card) {
		...
		if (card_num > 0 && card_num < 9 && corlor < 3) { // 9的循环是
			...
			next_index = index - 2;
		} else if(index > table._logic.switch_to_card_index(0x37) && index != table._logic.switch_to_card_index(0x42)){
			next_index = index + 1;
		}else if(index == table._logic.switch_to_card_index(0x42)){
			next_index = index - 3;
		}
