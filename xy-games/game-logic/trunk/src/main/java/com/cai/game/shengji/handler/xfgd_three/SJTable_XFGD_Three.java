...
public class SJTable_XFGD_Three extends SJTable {
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_call_banker = new SJHandlerCallBanker_XFGD_THREE();
		_handler_out_card_operate = new SJHandlerOutCardOperate_XFGD_THREE();
	...
	protected void test_cards() {
		...
		int cards[] = new int[] { 29, 19, 20, 4, 17, 3, 50, 52, 58, 33, 1, 39, 53, 1, 18, 5, 55, 20, 18, 21, 43, 78, 58,
				4, 8, 36, 54, 39, 50, 22, 53, 28, 42, 25, 24, 41, 2, 33, 2, 44, 26, 49, 44, 24, 51, 54, 21, 9, 10, 29,
				49, 37, 45, 43, 22, 56, 56, 28, 59, 36, 52, 78, 61, 9, 35, 55, 13, 79, 38, 34, 59, 78, 57, 78, 51, 38,
				12, 57, 2, 40, 2, 1, 45, 1, 23, 41, 61, 23, 60, 12, 11, 7, 13, 19, 35, 42, 27, 11, 26, 6, 40, 27, 37,
				34, 25, 10, 79, 60 };
	...
	public int get_hand_card_count_max() {
		return GameConstants.XFGD_MAX_COUT;
	}

	@Override
	public int get_hand_card_count_init() {
		return get_hand_card_count_max() - 9;
	}
		...
		if (GRR != null) {
			...
			if (GRR._banker_player != GameConstants.INVALID_SEAT) {
				if (seat_index == GameConstants.INVALID_SEAT) {
					...
					if (xian_get_score >= ((10 - _select_dang[GRR._banker_player]) * 10 + 5) * 2
							&& has_rule(GameConstants.GAME_RULE_XFGD_DAOGUANG)) {
	...
	public void deal_tou_agree(int seat_index) {
		if (!has_rule(GameConstants.GAME_RULE_XFGD_TOU_XIANG) || GRR == null) {
			return;
		}
		if (!this._is_po_dang) {
			return;
			...
			for (int i = 0; i < this.getTablePlayerNumber(); i++) {
				for (int j = 0; j < GRR._card_count[i]; j++) {
					if (_logic.GetCardColor(GRR._cards_data[i][j]) == _zhu_type) {
						...
						if (_logic.GetCardValue(GRR._cards_data[i][j]) == 14
								|| _logic.GetCardValue(GRR._cards_data[i][j]) == 15) {
							GRR._cards_data[i][j] += _zhu_value;
						}
					}
