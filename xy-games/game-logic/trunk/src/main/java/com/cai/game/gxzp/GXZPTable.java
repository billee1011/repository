...
import com.cai.game.gxzp.GXZPGameLogic.AnalyseItem;
import com.cai.game.gxzp.handler.GXZPHandler;
import com.cai.game.gxzp.handler.GXZPHandlerChiPeng;
import com.cai.game.gxzp.handler.GXZPHandlerDispatchCard;
import com.cai.game.gxzp.handler.GXZPHandlerFinish;
import com.cai.game.gxzp.handler.GXZPHandlerGang;
import com.cai.game.gxzp.handler.GXZPHandlerOutCardOperate;
import com.cai.game.gxzp.handler.bayi.BYZPHandlerChiPeng_GL;
import com.cai.game.gxzp.handler.bayi.BYZPHandlerChuLiFirstCard_GL;
import com.cai.game.gxzp.handler.bayi.BYZPHandlerDispatchAddCard_GL;
import com.cai.game.gxzp.handler.bayi.BYZPHandlerDispatchCard_GL;
import com.cai.game.gxzp.handler.bayi.BYZPHandlerDispatchFirstCard_GL;
import com.cai.game.gxzp.handler.bayi.BYZPHandlerGang_GL;
import com.cai.game.gxzp.handler.bayi.BYZPHandlerOutCardOperate_GL;
import com.cai.game.gxzp.handler.guilin.GXZPHandlerChiPeng_GL;
import com.cai.game.gxzp.handler.guilin.GXZPHandlerChuLiFirstCard_GL;
import com.cai.game.gxzp.handler.guilin.GXZPHandlerDispatchAddCard_GL;
import com.cai.game.gxzp.handler.guilin.GXZPHandlerDispatchCard_GL;
import com.cai.game.gxzp.handler.guilin.GXZPHandlerDispatchFirstCard_GL;
import com.cai.game.gxzp.handler.guilin.GXZPHandlerGang_GL;
import com.cai.game.gxzp.handler.guilin.GXZPHandlerOutCardOperate_GL;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class GXZPTable extends AbstractRoom {
	/**
	 * 
	 */
	private static final long serialVersionUID = 7060061356475703643L;

	private static Logger logger = Logger.getLogger(GXZPTable.class);
	...
	public GXZPGameLogic _logic = null;
	...
	public GXZPHandler _handler;

	public GXZPHandlerDispatchCard _handler_dispath_card;
	public GXZPHandlerDispatchCard _handler_dispath_add_card;
	public GXZPHandlerOutCardOperate _handler_out_card_operate;
	public GXZPHandlerGang _handler_gang;
	public GXZPHandlerChiPeng _handler_chi_peng;
	public GXZPHandlerDispatchCard _handler_dispath_firstcards;
	public GXZPHandlerDispatchCard _handler_chuli_firstcards;

	// public HHHandlerDispatchLastCard_YX _handler_dispath_last_card;

	public GXZPHandlerFinish _handler_finish; // 结束

	public GXZPTable() {
		super(RoomType.HH, GameConstants.GAME_PLAYER);

		_logic = new GXZPGameLogic();
		...
		_gang_card_data = new CardsData(GameConstants.MAX_GXZP_COUNT);
	}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_PHZ_GUILIN_ZP)) {
			_handler_dispath_card = new GXZPHandlerDispatchCard_GL();
			_handler_out_card_operate = new GXZPHandlerOutCardOperate_GL();
			_handler_gang = new GXZPHandlerGang_GL();
			_handler_chi_peng = new GXZPHandlerChiPeng_GL();
			_handler_dispath_add_card = new GXZPHandlerDispatchAddCard_GL();
			_handler_chuli_firstcards = new GXZPHandlerChuLiFirstCard_GL();
			_handler_dispath_firstcards = new GXZPHandlerDispatchFirstCard_GL();
		}
		if (is_mj_type(GameConstants.GAME_TYPE_PHZ_BAYI_ZP)) {
			_handler_dispath_card = new BYZPHandlerDispatchCard_GL();
			_handler_out_card_operate = new BYZPHandlerOutCardOperate_GL();
			_handler_gang = new BYZPHandlerGang_GL();
			_handler_chi_peng = new BYZPHandlerChiPeng_GL();
			_handler_dispath_add_card = new BYZPHandlerDispatchAddCard_GL();
			_handler_chuli_firstcards = new BYZPHandlerChuLiFirstCard_GL();
			_handler_dispath_firstcards = new BYZPHandlerDispatchFirstCard_GL();
		}

		_handler_finish = new GXZPHandlerFinish();
	...
	public boolean reset_init_data() {
		...
		GRR = new GameRoundRecord(GameConstants.MAX_WEAVE_HH, GameConstants.MAX_GXZP_COUNT, GameConstants.MAX_HH_INDEX);
		GRR.setRoom(this);
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			_playerStatus[i] = new PlayerStatus(GameConstants.MAX_GXZP_COUNT);
		}
	...
	private boolean game_start_glzp() {
		...
		int hand_cards[][] = new int[playerCount][GameConstants.MAX_GXZP_COUNT];
		// 发送数据
		...
		for (int i = 0; i < playerCount; i++) {
			...
			for (int j = 0; j < GameConstants.MAX_GXZP_COUNT; j++) {
	...
	public void test_cards() {
		 ...
		 int[] realyCards = new int[] {
				 8, 6, 2, 5, 21, 17, 22, 4, 2, 26, 2, 4, 3, 4, 20, 21, 25, 2, 21, 9, 1, 7, 17, 8, 19, 1, 18, 24, 24, 17, 25, 23, 8, 23, 5, 10, 1, 7, 19, 24, 6, 6, 22, 24, 26, 19, 6, 9, 18, 3, 20, 8, 3, 9, 3, 10, 18, 25, 10, 22, 4, 21, 7, 1, 25, 22, 26, 17, 18, 23, 20, 10, 5, 9, 20, 7, 23, 26, 19, 5
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_PHZ_GUILIN_ZP)) {
			ret = this.handler_game_finish_gxzp(seat_index, reason);
		}
		if (is_mj_type(GameConstants.GAME_TYPE_PHZ_BAYI_ZP)) {
			ret = this.handler_game_finish_bayi(seat_index, reason);
		}
		if (is_sys()) {
	...
	public boolean handler_game_finish_gxzp(int seat_index, int reason) {
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL ||
			...
			for (int i = 0; i < count; i++) {
				...
				if (GRR._chi_hu_card[i][0] == 0 || reason != GameConstants.Game_End_NORMAL) {
					...
					game_end.addJettonScore(this._ben_xing_count[i]);
					game_end.addJettonScore(this._shang_xing_count[i]);
					game_end.addJettonScore(this._xia_xing_count[i]);
					game_end.addJettonScore(this._hu_xing_count[i]);
				}
	...
	public int analyse_chi_hu_card(int cards_index[], WeaveItem weaveItems[], int weave_count, int seat_index, int provider_index, int cur_card,
		...
		if (GameConstants.GAME_TYPE_PHZ_GUILIN_ZP == _game_type_index) {
			return analyse_chi_hu_card_glzp(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type,
					dispatch);
		}
		if (GameConstants.GAME_TYPE_PHZ_BAYI_ZP == _game_type_index) {
			return analyse_chi_hu_card_glzp(cards_index, weaveItems, weave_count, seat_index, provider_index, cur_card, chiHuRight, card_type,
					dispatch);
		}
		return 0;
	 ...
	 * @return
	 */
	public int analyse_chi_hu_card_glzp(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index, int provider_index, int cur_card,
		...
		boolean bValue = _logic.analyse_card_glzp(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index, cur_card, analyseItemArray,
		...
		if (cur_card != 0) {
			for (int i = 0; i < weaveCount; i++) {
				if ((cur_card == weaveItems[i].center_card) && ((weaveItems[i].weave_kind == GameConstants.WIK_PENG && dispatch == true)
					...
					boolean temp_bValue = _logic.analyse_card_glzp(cbCardIndexTemp, weaveItems, weaveCount, seat_index, provider_index, cur_card,
						...
						for (; temp_index < analyseItemArray.size(); temp_index++) {
							...
							for (int j = 0; j < 7; j++) {
								if ((cur_card == analyseItem.cbCenterCard[j]) && (analyseItem.cbWeaveKind[j] == GameConstants.WIK_PENG
										|| (analyseItem.cbWeaveKind[j] == GameConstants.WIK_SAO && provider_index != seat_index)))
									analyseItem.cbWeaveKind[j] = GameConstants.WIK_TUO;
								else if ((cur_card == analyseItem.cbCenterCard[j])
										&& (analyseItem.cbWeaveKind[j] == GameConstants.WIK_SAO && provider_index == seat_index))
									analyseItem.cbWeaveKind[j] = GameConstants.WIK_TI_LONG;
								analyseItem.hu_xi[j] = _logic.get_analyse_hu_xi(analyseItem.cbWeaveKind[j], analyseItem.cbCenterCard[j], false);
			...
			if (cards_index[cur_index] == 3) {
				...
				sao_WeaveItem.hu_xi = _logic.get_weave_hu_xi(sao_WeaveItem, false);
		...
		for (int i = 0; i < analyseItemArray.size(); i++) {
			...
			for (int j = 0; j < 7; j++) {
				...
				temp_hu_xi += _logic.get_weave_hu_xi(weave_items, false);
			}
		...
		if (has_rule(GameConstants.GAME_RULE_10_YOU_HU))
			if (max_hu_xi < 10) {
				chiHuRight.set_empty();
				return GameConstants.WIK_NULL;
			}
		if (has_rule(GameConstants.GAME_RULE_15_YOU_HU))
			if (max_hu_xi < 15) {
				chiHuRight.set_empty();
				return GameConstants.WIK_NULL;
			}
		...
		for (int j = 0; j < 7; j++) {
			...
			_hu_weave_items[seat_index][j].hu_xi = _logic.get_weave_hu_xi(_hu_weave_items[seat_index][j], false);
	...
	public int estimate_player_respond_glzp(int seat_index, int provider_index, int card_data, int pao_type[], boolean dispatch) {
		...
		if ((bAroseAction == GameConstants.WIK_NULL) && (dispatch == true)) {
			for (int weave_index = 0; weave_index < this.GRR._weave_count[seat_index]; weave_index++) {
				...
				// true, false);
				pao_type[0] = GameConstants.PAO_TYPE_MINE_PENG_PAO;
				bAroseAction = GameConstants.WIK_TUO;
			}
		}
		// 跑牌判断
		if ((bAroseAction == GameConstants.WIK_NULL) && (seat_index != provider_index)) {
			if (_logic.check_tuo(this.GRR._cards_index[seat_index], card_data) != GameConstants.WIK_NULL) {
				// this.exe_gang(seat_index, seat_index, card_data,
				// GameConstants.WIK_PAO,GameConstants.PAO_TYPE_OHTER_PAO ,
				// true, false);
				pao_type[0] = GameConstants.PAO_TYPE_OHTER_PAO;
	...
	public boolean add_lou_weave(int luoCode, int target_player, int target_card, int provide_player, LouWeaveItem lou_weave_item) {
		...
		for (int i = 0; i < 2; i++) {
			...
			case GameConstants.WIK_LEFT: {
				...
				this.GRR._weave_items[target_player][wIndex].hu_xi = _logic.get_weave_hu_xi(this.GRR._weave_items[target_player][wIndex], false);
				break;
			}
	...
	public boolean handler_player_be_in_room(int seat_index) {
		...
		if ((this._cur_round > 0 && this._player_ready[seat_index] == 0)||this.getRuleValue(GameConstants.GAME_RULE_CAN_LESS)==1)
	...
	public void countChiHuTimes_bayi(int _seat_index, boolean isZimo) {
		ChiHuRight chiHuRight = GRR._chi_hu_rights[_seat_index];
		_player_result.hu_pai_count[_seat_index]++;
		_player_result.ying_xi_count[_seat_index] += this._hu_xi[_seat_index];
		if (isZimo) {
	 ...
	 * @param zimo
	 */
	public void process_chi_hu_player_score_bayi(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		int zi_shu = 0;
		if (has_rule(GameConstants.GAME_RULE_10_YOU_HU)) {
			if (has_rule(GameConstants.GMAE_RULE_5_YOU_1_ZI)) {
				calculate_score += 1 + (all_hu_xi - 10) / 5;
			}
			if (has_rule(GameConstants.GAME_RULE_3_YOU_1_ZI)) {
				calculate_score += 1 + (all_hu_xi - 10) / 3;
			}
		}
		if (has_rule(GameConstants.GAME_RULE_15_YOU_HU)) {
			if (has_rule(GameConstants.GMAE_RULE_5_YOU_1_ZI)) {
				calculate_score += 1 + (all_hu_xi - 15) / 5;
			}
			if (has_rule(GameConstants.GAME_RULE_3_YOU_1_ZI)) {
		...
		if (has_rule(GameConstants.GAME_RULE_ZM_YI_ZI)) {
			if (!(chr.opr_and(GameConstants.CHR_ZI_MO)).is_empty()) {
				calculate_score += 1;
			}
			if (!(chr.opr_and(GameConstants.CHR_TIAN_HU)).is_empty()) {
		...
		zi_shu = calculate_score;
		if (!(chr.opr_and(GameConstants.CHR_HAI_HU)).is_empty()) {
			calculate_score += 3;
			zi_shu += 3;
		}

		// zi_shu += this._ben_xing_count;
		// zi_shu += this._shang_xing_count;
		// zi_shu += this._xia_xing_count;
		// zi_shu += this._hu_xing_count;
		this._tun_shu[seat_index] = zi_shu;
		int xing_shu = 0;
		xing_shu += this._ben_xing_count[seat_index];
		xing_shu += this._shang_xing_count[seat_index];
		xing_shu += this._xia_xing_count[seat_index];
		if (has_rule(GameConstants.Game_RULE_HU_XING))
			xing_shu += this._hu_xing_count[seat_index];

		this._fan_shu[seat_index] = wFanShu;
		calculate_score = wFanShu * calculate_score + xing_shu;
	 ...
	 * @param zimo
	 */
	public void process_chi_hu_player_score_glzp(int seat_index, int provide_index, int operate_card, boolean zimo) {
		GRR._chi_hu_card[seat_index][0] = operate_card;
		...
		calculate_score += this._ben_xing_count[seat_index];
		calculate_score += this._shang_xing_count[seat_index];
		calculate_score += this._xia_xing_count[seat_index];
		if (has_rule(GameConstants.Game_RULE_HU_XING))
			calculate_score += this._hu_xing_count[seat_index];
		zi_shu += this._ben_xing_count[seat_index];
		zi_shu += this._shang_xing_count[seat_index];
		zi_shu += this._xia_xing_count[seat_index];
		zi_shu += this._hu_xing_count[seat_index];
		this._tun_shu[seat_index] = zi_shu;
	...
	public boolean send_play_data(int seat_index) {
		...
		int hand_cards[] = new int[GameConstants.MAX_GXZP_COUNT];
