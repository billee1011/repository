...
public class PDKHandlerOutCardOperate_SR extends PDKHandlerOutCardOperate {
	@Override
	public void exe(PDKTable table) {
		...
		// 显示出牌
		table.operate_out_card(_out_card_player, table._turn_out_card_count, table._turn_out_card_data,
	...
	public int adjust_out_card_right(PDKTable table) {
		...
		if (cbCardType == GameConstants.PDK_CT_ERROR || cbCardType == GameConstants.PDK_CT_THREE_TAKE_ONE
		...
		// 黑桃必须先出
		if (table.has_rule(GameConstants.GAME_RULE_SHOU_JU_HEITAO_SAN) && table.GRR._cur_round == 0
				&& table._out_card_times[_out_card_player] == 0 && _out_card_player == table.GRR._banker_player) {
			if (_out_cards_data[0] != 0x33 && _out_cards_data[1] == 0x33 && _out_cards_data[2] == 0x33
					&& _out_cards_data[3] == 0x33) {
				return GameConstants.PDK_CT_ERROR;
			}
		}
		if (fang_zou_bao_pei(table)) {
