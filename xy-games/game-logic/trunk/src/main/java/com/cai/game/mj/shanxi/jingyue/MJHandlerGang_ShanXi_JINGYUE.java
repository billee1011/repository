...
public class MJHandlerGang_ShanXi_JINGYUE extends MJHandlerGang<MJTable_ShanXi_JINGYUE> {
	...
	public MJHandlerGang_ShanXi_JINGYUE() {
	}
	...
	public void exe(MJTable_ShanXi_JINGYUE table) {
		...
		} else if (GameConstants_SXJY.GANG_TYPE_AN_FENG_GANG == _type) {// 暗风杠
			table._playerStatus[_seat_index]._cards_feng_gang_count++;// 记录风杠的次数
			table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1,
					new long[] { GameConstants.WIK_AN_GANG }, 1, GameConstants.INVALID_SEAT);
		} else if (GameConstants_SXJY.GANG_TYPE_MING_FENG_GANG == _type) {// 明风杠
			table._playerStatus[_seat_index]._cards_feng_gang_count++;// 记录风杠的次数
			table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1,
		...
		if ((GameConstants.GANG_TYPE_AN_GANG == _type) || (GameConstants.GANG_TYPE_JIE_GANG == _type)
				|| (GameConstants_SXJY.GANG_TYPE_AN_FENG_GANG == _type)
				|| (GameConstants_SXJY.GANG_TYPE_MING_FENG_GANG == _type)) {
			this.exe_gang(table);
			return;
		}
		...
		if (_center_card == GameConstants_SXJY.DONG_FENG_CARD + GameConstants_SXJY.CARD_ESPECIAL_TYPE_GANG_NAN_FENG) {
			no_feng_operate_card = _center_card - GameConstants_SXJY.CARD_ESPECIAL_TYPE_GANG_NAN_FENG;
		}
	...
	@Override
	public boolean exe_gang(MJTable_ShanXi_JINGYUE table) {
		int no_feng_operate_card = _center_card;
		if (_center_card >= GameConstants_SXJY.DONG_FENG_CARD + GameConstants_SXJY.CARD_ESPECIAL_TYPE_GANG_NAN_FENG) {
			no_feng_operate_card = _center_card - GameConstants_SXJY.CARD_ESPECIAL_TYPE_GANG_NAN_FENG;
		}
		...
		// 勾选风胡玩法的时候 ，如果有玩家杠了“中”、发、“白”中的一个，那么所有玩家不能进行“东南西北”杠；
		if (table.has_rule(GameConstants_SXJY.GAME_RULE_SHANXI_QYS_YTL_QXD_FH_JF)) {
			if (no_feng_operate_card >= 53 && no_feng_operate_card <= 55) {
				table.is_feng_zfb_gang = true;
			}
		}
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type) {
			...
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
			table.feng_ed_gang[_seat_index] = true;
			table.all_player_gang_count++;
		} else if (GameConstants_SXJY.GANG_TYPE_MING_FENG_GANG == _type) {// 明风杠
			// 别人打的牌，谁点谁给
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
		...
		if (GameConstants.GANG_TYPE_AN_GANG == _type || GameConstants_SXJY.GANG_TYPE_AN_FENG_GANG == _type) {
			_p = false;
		} else {
			_p = true;
		}
		table.GRR._weave_items[_seat_index][cbWeaveIndex].public_card = _p == true ? 1 : 0;
		if (GameConstants_SXJY.GANG_TYPE_MING_FENG_GANG == _type
				|| GameConstants_SXJY.GANG_TYPE_AN_FENG_GANG == _type) {
		...
		// 把有杠的那几张牌从手上去掉
		if (GameConstants_SXJY.GANG_TYPE_AN_FENG_GANG == _type) {
			for (int i = 27; i < 31; i++) {
				table.GRR._cards_index[_seat_index][i]--;
			}
		} else if (GameConstants_SXJY.GANG_TYPE_MING_FENG_GANG == _type) {
			int center_index = table._logic.switch_to_card_index(no_feng_operate_card);
			for (int i = 27; i < 31; i++) {
				if (center_index == i) {
					continue;
				}
		...
		if (table.has_rule(GameConstants_SXJY.GAME_RULE_SHANXI_LAO_PAI)) {
	 ...
	 */
	@Override
	public boolean handler_operate_card(MJTable_ShanXi_JINGYUE table, int seat_index, int operate_code,
			int operate_card) {
		// 抢杠胡
