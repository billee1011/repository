...
public class HandlerBaoTing_ZhuZhouWang extends AbstractMJHandler<Table_ZhuZhouWang> {
	...
	public void exe(Table_ZhuZhouWang table) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			table._playerStatus[i]._hu_card_count = table.get_ting_card_bao_ting(table._playerStatus[i]._hu_cards,
					table.GRR._cards_index[i], table.GRR._weave_items[i], table.GRR._weave_count[i], i);
			int ting_cards[] = table._playerStatus[i]._hu_cards;
			int ting_count = table._playerStatus[i]._hu_card_count;
			if (ting_count > 0) {
				table.operate_chi_hu_cards(i, ting_count, ting_cards);
				table.qi_shou_ting[i] = true;
			} else {
				ting_cards[0] = 0;
				table.operate_chi_hu_cards(i, 0, ting_cards);
			}
			// table.qi_shou_ting[i] = true;
			// 能听牌的才在客户端弹出‘报听’的界面, 并且地牌张数小于3
			if (table.qi_shou_ting[i]) {
	...
	public boolean handler_operate_card(Table_ZhuZhouWang table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants.WIK_BAO_TING) {
			...
			set_bao_ting_card(table, seat_index);
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if ((table._playerStatus[i].is_respone() == false)
					&& (table._playerStatus[i].has_action_by_code(GameConstants.WIK_BAO_TING)))
				return false;
		}
	...
	public void set_bao_ting_card(Table_ZhuZhouWang table, int seat_index) {
		table.get_ting_card(table._playerStatus[seat_index]._hu_cards, table.GRR._cards_index[seat_index],
				table.GRR._weave_items[seat_index], table.GRR._weave_count[seat_index], seat_index);
		int ting_cards[] = table._playerStatus[seat_index]._hu_cards;
		int ting_count = table._playerStatus[seat_index]._hu_card_count;
		if (ting_count > 0) {
			table._bao_ting_count_qishou[seat_index] = ting_count;
			System.arraycopy(ting_cards, 0, table._bao_ting_card_qishou[seat_index], 0, ting_count);
		}
	}
