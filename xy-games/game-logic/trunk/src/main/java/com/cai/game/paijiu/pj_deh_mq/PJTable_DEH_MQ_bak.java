 ...
 * 
 */
package com.cai.game.paijiu.pj_deh_mq;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PJTable_DEH_MQ_bak extends PJTable {
	...
	public PJTable_DEH_MQ_bak() {
		super(PJType.GAME_TYPE_PJ_DEH_MQ);
	...
	protected boolean on_handler_game_start() {
		...
		if (_xiu_mang_cur_round <= _xiu_mang_total_round) {
			for (int i = 0; i < getTablePlayerNumber(); i++) {
				...
				if (_user_mang_pi_score[i] >= _mang_pi_score * 2) {
					break;
				}
				_user_mang_pi_score[i] += _mang_pi_score;
			}
		}
	...
	public boolean game_start_dehjd() {
		...
		game_end_deh.setRoomInfo(this.getRoomInfo());
	...
	private void test_cards() {
		...
		// 0x1d, 0x3d, 0x31, 0x32};
		int cards[] = new int[] { 21, 4, 58, 23, 35, 34, 10, 54, 26, 7, 44, 12, 79, 40, 57, 22, 24, 2, 56, 55, 52, 53, 42, 8, 39, 38, 27, 20, 25, 59,
				36, 6 };
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
	...
	public boolean deal_add_score(int seat_index, int jetton_score) {
		...
		if (_jetton_current_add_min == 0) {
			if (this._jetton_current + _jetton_current_add_min >= jetton_score || this._player_ready[seat_index] == 0 || _is_give_up[seat_index]) {
				return true;
			}
		} else {
	...
	public boolean deal_bobo_score(int seat_index, int jetton_score) {
		...
		for (int i = 0; i < this.getTablePlayerNumber(); i++) {
			...
			_mang_chi_score += _mang_pi_score;
			this._player_result.game_score[i] -= _mang_pi_score;
