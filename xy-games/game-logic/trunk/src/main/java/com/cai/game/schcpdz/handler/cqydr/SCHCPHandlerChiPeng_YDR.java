...
public class SCHCPHandlerChiPeng_YDR extends SCHCPDZHandlerChiPeng<SCHCPDZTable> {
	
	
	public SCHCPHandlerChiPeng_YDR(){
	...
	public boolean handler_operate_card(SCHCPDZTable table,int seat_index, int operate_code, int operate_card,int lou_pai){
		...
		if(operate_code == GameConstants.DZ_WIK_NULL){
			table.record_effect_action(seat_index,GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[]{GameConstants.DZ_WIK_NULL}, 1);
		}
		// 放弃操作
		if (operate_code == GameConstants.DZ_WIK_NULL) {
			// 用户状态
