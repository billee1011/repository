...
public class MJHandlerDispatchCard_YongZhou extends MJHandlerDispatchCard<MJTable_YongZhou> {
    ...
    public void exe(MJTable_YongZhou table) {
        ...
        if (action != GameConstants.WIK_NULL) {
            ...
            table.get_ting_card(table._playerStatus[_seat_index]._hu_cards, table.GRR._cards_index[_seat_index],
                    table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], _seat_index);
            if (table._playerStatus[_seat_index]._hu_cards[0] == -1) {
                if (table._logic.is_magic_card(_send_card_data)) {
                    chr.opr_or(GameConstants.CHR_HUNAN_PRIVATE_WANG_BAO_WANG);
                } else {
                    chr.opr_or(GameConstants.CHR_HUNAN_PRIVATE_WANG_BAO);
                }
            }
        } else {
            chr.set_empty();
        ...
        if (table.GRR._left_card_count > llcard) {
            ...
            int cbActionMask = table.analyse_gang_card_all(table.GRR._cards_index[_seat_index],
            ...
            if (cbActionMask != GameConstants.WIK_NULL) {// 有杠
                ...
                for (int i = 0; i < gangCount; i++) {
                    // 加上刚
                    if (m_gangCardResult.isPublic[i] == 1 && m_gangCardResult.cbCardData[i] != _send_card_data) {
