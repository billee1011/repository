...
public class GHZHandlerDispatchFirstCard_NX extends PHZHandlerDispatchCard<NXGHZTable> {
...
//		_send_card_data = 0x18;
		...
		while(loop < table.getTablePlayerNumber()){
			...
			if(table.is_tian_hu(i,table._current_player ,chr, table._send_card_data ) == 1)
			{
				...
				if(i == _seat_index){
					...
					curPlayerStatus.add_zi_mo(GameConstants.INVALID_CARD, i);
				}
				
				if(table.has_rule(GameConstants.GAME_RULE_QIANG_HU_PAI)){
					table.handler_operate_card(_seat_index, GameConstants.WIK_ZI_MO,_send_card_data, -1);
					return ;
				}
	 ...
	 */
	@Override
	public boolean handler_player_out_card(NXGHZTable table, int seat_index, int card) {
		// 错误断言
