...
public class HHTable_SYZP extends HHTable {
	...
	public int getTablePlayerNumber() {
		if (has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_FOUR)) {
			return GameConstants.GAME_PLAYER_HH + 1;
		}
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_dispath_card = new PHZHandlerDispatchCard_SYZP();
		_handler_out_card_operate = new PHZHandlerOutCardOperate_SYZP();
		_handler_gang = new PHZHandlerGang_SYZP();
		_handler_chi_peng = new PHZHandlerChiPeng_SYZP();
		_handler_chuli_firstcards = new PHZHandlerChuLiFirstCard_SYZP();
		_handler_dispath_firstcards = new PHZHandlerDispatchFirstCard_SYZP();
	}
...
//		_logic.random_card_data(repertory_card, mj_cards);
		...
		for (int i = 0; i < count; i++) {
			if (has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_FOUR)) {
				// 四人场最大手牌数15
				send_count = GameConstants_SYZP.MAX_FPHZ_COUNT - 1;
			} else {
				// 三人场最大手牌数21
				send_count = GameConstants_SYZP.MAX_HH_COUNT - 1;
			}
	...
	@Override
	public int analyse_chi_hu_card(int cards_index[], WeaveItem weaveItems[], int weaveCount, int seat_index,
			int provider_index, int cur_card, ChiHuRight chiHuRight, int card_type, int[] hu_xi_hh, boolean dispatch) {
							...
							for (int j = 0; j < 7; j++) {
								if ((cur_card == analyseItem.cbCenterCard[j])
										&& (analyseItem.cbWeaveKind[j] == GameConstants.WIK_PENG
												|| analyseItem.cbWeaveKind[j] == GameConstants.WIK_WEI)) //打出去的牌不能跑胡
		...
		if(has_rule(GameConstants_SYZP.GAME_RULE_PALYER_SYZP_HONHEIDIAN)){
		...
		// 地胡
		if (discard_num[seat_index] == 0 && seat_index != _cur_banker) {
			chiHuRight.opr_or(GameConstants.CHR_DI_HU);
		}
	...
	// 玩家出版的动作检测 跑
	public int estimate_player_respond_phz(int seat_index, int provider_index, int card_data, int pao_type[],
			boolean dispatch) {
		// 变量定义
	...
	public void process_chi_hu_player_score_phz(int seat_index, int provide_index, int operate_card, boolean zimo) {
		...
		countChiHuTimes(seat_index,zimo);
		// 计算胡息
		int all_hu_xi = 0;
		...
		// 息囤比
		int tun_num = 3;
		if (has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_XI5)) {
			tun_num = 5;
		}
		if (has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_XI1)) {
			tun_num = 1;
		}
		// 自摸囤
		int baseTun = 1;
		
		if (has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_ZIMO_JIAYITUN)
				&& !(chr.opr_and(GameConstants.CHR_ZI_MO)).is_empty()) {
			baseTun += 1;
		}
		int calculate_score = baseTun + (all_hu_xi - getQiHuXi()) / tun_num;
		...
		if (zimo) {
			// 自摸翻倍
			if (has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_ZIMO_DOUBLE) 
					&& !(chr.opr_and(GameConstants.CHR_ZI_MO)).is_empty()) {
				lChiHuScore *= 2;
				fan *= 2;
			}
	...
	public boolean on_room_game_finish(int seat_index, int reason) {
		...
		if (GRR != null) {// reason == MJGameConstants.Game_End_NORMAL ||
			...
			if (seat_index != -1) {
				...
				/*
				 * game_end.setTunShu(3); game_end.setFanShu(4);
				 */
			}
			...
			for (int j = 0; j < l; j++) {
				...
				if (GRR._chi_hu_rights[i].is_valid()) {
					if (type == GameConstants.CHR_ZI_MO) {
						if (has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_ZIMO_DOUBLE)) {
							des += ",自摸 X2";
						} else if (has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_ZIMO_JIAYITUN)) {
							des += ",自摸 +1";
						} else {
							des += ",自摸";
						}
					...
					if (des =="" && type == GameConstants.CHR_JEI_PAO_HU) {
						des += ",平胡";
					}
				}
			}
	...
	public void test_cards() {
		// 天胡-三提
	/*	int cards[] = new int[] { 0x01, 0x01, 0x01, 0x01, 0x02, 0x02, 0x02, 0x02, 0x03, 0x03, 0x05, 0x03, 0x05, 0x17,
		...
		// 0x16 };
		/*
		 * int cards[] = new int[] { 0x14, 0x14, 0x14, 0x12, 0x02, 0x02, 0x02,
		 * 0x017, 0x03, 0x03, 0x03, 0x1a, 0x05, 0x05, 0x05, 0x15, 0x16, 0x18,
		 * 0x18, 0x08 };
		 */
		/*for (int i = 0; i < getTablePlayerNumber(); i++) {
