...
public class PHZHandlerOutCardOperate_NingXiang extends HHHandlerOutCardOperate<NingXiangHHTable> {
	...
	public void exe(NingXiangHHTable table) {
		...
		NingXiangPHZUtils.ting_basic(table, _out_card_player);
		// 玩家出牌响应判断. 是否有跑 、提、扫
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if (ti_pao != GameConstants.WIK_NULL) {
				table.exe_gang(i, _out_card_player, _out_card_data, ti_pao, pao_type[0], false, true, false, table.time_for_operate_dragon);
				return;
			}
		}
		...
		if (bAroseAction == false) {
			...
			table.exe_add_discard(_out_card_player, 1, new int[] { _out_card_data }, true, table.time_for_add_discard);
