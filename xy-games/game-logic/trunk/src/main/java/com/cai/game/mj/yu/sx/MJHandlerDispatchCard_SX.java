...
public class MJHandlerDispatchCard_SX extends MJHandlerDispatchCard<Table_SX> {
	...
	public void exe(Table_SX table) {
		...
		if (ting_send_card) {
			...
			show_send_card += GameConstants_SX.CARD_ESPECIAL_TYPE_LAI_ZI;
		}
	...
	public boolean handler_operate_card(Table_SX table, int seat_index, int operate_code, int operate_card) {
		...
		if (operate_code == GameConstants_HZLZG.WIK_NULL) {
			...
			for (int i = 0; i < m_gangCardResult.cbCardCount; i++) {
				if (GameConstants_SX.GANG_TYPE_ADD_GANG == m_gangCardResult.type[i]) {
		...
		case GameConstants.WIK_ZI_MO: {
			...
			if (_type == GameConstants_SX.DispatchCard_Type_Tian_Hu && table.has_rule(GameConstants_SX.GAME_RULE_DIAN_PAO)) {
				table.GRR._chi_hu_rights[_seat_index].opr_or(GameConstants_SX.CHR_TIAN_HU);
			}
	...
	@Override
	public boolean handler_player_out_card(Table_SX table, int seat_index, int card) {
		...
		if (_type == GameConstants_SX.DispatchCard_Type_Tian_Hu) {
			// 出牌
			table.exe_out_card(_seat_index, card, GameConstants_SX.OutCard_Type_Di_Hu);
		} else {
			// 出牌
		...
		if ((out_ting_count > 0) && (seat_index == _seat_index)) {
			for (int j = 0; j < hand_card_count; j++) {
				...
				if (table._logic.is_magic_card(hand_cards[j])) {
					hand_cards[j] += GameConstants_SX.CARD_ESPECIAL_TYPE_LAI_ZI;
				}
			}
		}
