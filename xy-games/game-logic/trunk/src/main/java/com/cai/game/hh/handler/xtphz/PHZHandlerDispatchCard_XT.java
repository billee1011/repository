 ...
 *
 */
public class PHZHandlerDispatchCard_XT extends HHHandlerDispatchCard<HHTable> {
		...
		if (curPlayerStatus.has_action()) {// 有动作
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				if (bAroseAction == false) {
					...
					if (table.has_rule(GameConstants.GAME_RULE_ADD_SPEED) && sysParamModel1110 != null && sysParamModel1110.getVal4() > 0
					...
					if (table.has_rule(GameConstants.GAME_RULE_ADD_SPEED) && sysParamModel1110 != null && sysParamModel1110.getVal5() > 0
	...
	public boolean handler_player_out_card(HHTable table, int seat_index, int card) {
		...
		if (table.has_rule(GameConstants.GAME_RULE_DI_MING_WEI)
			...
			// if(table.GRR._can_ting_out_index[seat_index][table._logic.switch_to_card_index(card)]
			// == 0)
			// {
			// // 刷新手牌
			// int cards[] = new int[GameConstants.MAX_HH_COUNT];
			...
			// GameConstants.OUT_CARD_TYPE_MID,
			// GameConstants.INVALID_SEAT);
			// // 刷新自己手牌
			// int hand_card_count =
			...
			//
			// table.log_info(_seat_index+"出牌出错 HHHandlerDispatchCard
			// "+_seat_index);
			// return true;
			// }
			// else
