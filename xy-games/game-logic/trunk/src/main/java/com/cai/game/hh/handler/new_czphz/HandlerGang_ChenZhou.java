...
public class HandlerGang_ChenZhou extends HHHandlerGang<Table_New_ChenZhou> {
	...
	@Override
	protected boolean exe_gang(Table_New_ChenZhou table) {
		...
		if (an_long_count > 0) {
			...
			for (int x = 0; x < hand_card_count; x++) {
				if (table.is_card_has_wei(cards[x])) { // 如果是偎的牌
					// TODO
					// 射跑后，手牌强制偎或提了，出牌不需考虑打出后剩余牌是否听牌，第二次射跑及后续射跑也不需考虑打出后是否听牌状态。
					if (table.shoot_count[_seat_index] > 1) {
						cards[x] += GameConstants.CARD_ESPECIAL_TYPE_CAN_SHOOT;
					} else {
						// 判断打出这张牌是否能听牌
		...
		if ((table._long_count[_seat_index] <= 1 || GameConstants.SAO_TYPE_MINE_SAO == _type) && (table._is_xiang_gong[_seat_index] == false)) { // 一个提跑并且不是相公，判断是否能胡牌
			...
			if (action_hu != GameConstants.WIK_NULL) {
				...
				if (table.has_rule(Constants_New_ChenZhou.GAME_RULE_YOU_HU_BI_HU)
						|| (card_type == GameConstants.HU_CARD_TYPE_PAOHU && table.has_rule(Constants_New_ChenZhou.GAME_RULE_FANG_PAO_BI_HU))) {
					if (_seat_index != _provide_player) {
			...
			if (pai_count == 0) { // 如果是第1个提跑或者是偎牌，如果没牌可以出了，判断能不能胡，不能胡，设置成相公
				...
				if (b_hu_xi == true) { // 如果能胡
					...
					if (_seat_index != _provide_player) {
						chr.opr_or(Constants_New_ChenZhou.CHR_JEI_PAO_HU);
					} else {
						chr.opr_or(Constants_New_ChenZhou.CHR_ZI_MO);
					}

					if (table.has_rule(Constants_New_ChenZhou.GAME_RULE_HONG_HEI_DIAN)
					...
					if (table.has_rule(Constants_New_ChenZhou.GAME_RULE_YOU_HU_BI_HU)
							|| (_seat_index != _provide_player && table.has_rule(Constants_New_ChenZhou.GAME_RULE_FANG_PAO_BI_HU))) { // 如果有强制胡牌
						if (_seat_index != _provide_player) {
							GameSchedule.put(new Runnable() {
								@Override
								public void run() {
		...
		case GameConstants.WIK_ZI_MO: { // 偎提跑之后胡牌
			...
			if (target_action == GameConstants.WIK_ZI_MO) {
				...
				if (_depatch == true)
					...
					table._player_result.dian_pao_count[target_player]++;
				}
			}
