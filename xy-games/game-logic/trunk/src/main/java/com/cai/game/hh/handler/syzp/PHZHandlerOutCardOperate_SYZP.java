...
public class PHZHandlerOutCardOperate_SYZP extends HHHandlerOutCardOperate<HHTable_SYZP> {
	@Override
	public void exe(HHTable_SYZP table) {
		...
		if (table.GRR._cards_index[_out_card_player][table._logic.switch_to_card_index(_out_card_data)] >= 3) {
			table.log_info(_out_card_player + "出牌出错 HHHandlerOutCardOperate_SYZP" + _out_card_data);
			return;
		}
		//
		// 出牌记录
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if (table._playerStatus[i].has_chi_hu() 
					&& (table.has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_DIAN_PAO_BI_HU) 
							|| table.has_rule(GameConstants_SYZP.GAME_RULE_PLAYER_SYZP_YOU_PAO_BI_HU) )) {
				
				table.handler_operate_card(i, GameConstants.WIK_CHI_HU, table._out_card_data, -1);
				return ;
			}else if(table._playerStatus[i].has_chi_hu() ){
				bAroseAction =true;
			}
		}
		...
		boolean flag = table.estimate_player_out_card_respond_PHZ_syzp(_out_card_player, _out_card_data, false) ;// ,
		bAroseAction = flag == false ? bAroseAction : flag;
		...
		if (bAroseAction == false) {
			...
			// 延时发牌时间
			int discard_time = HHManager.getInstance().getSysParamValue(table.getGame_id(), HHManager.DEFAULT_2000,
					HHManager.VAL1,  HHManager.PARAM_ID1105);
			...
			// 延时发牌时间
			int dispatch_time = HHManager.getInstance().getSysParamValue(table.getGame_id(), HHManager.DEFAULT_2000,
					HHManager.VAL2,  HHManager.PARAM_ID1105);
	...
	public boolean handler_operate_card(HHTable_SYZP table, int seat_index, int operate_code, int operate_card, int luoCode) {
		...
		if (playerStatus.has_action() == false) {
			table.log_player_error(seat_index, "HHHandlerOutCardOperate_syzp 出牌,玩家操作已失效");
			return true;
		}

		// 是否已经响应
		if (playerStatus.is_respone()) {
			table.log_player_error(seat_index, "HHHandlerOutCardOperate_syzp 出牌,玩家已操作");
			return true;
		}

		if ((operate_code != GameConstants.WIK_NULL) && playerStatus.has_action_by_code(operate_code) == false)// 没有这个操作动作
		{
			table.log_player_error(seat_index, "HHHandlerOutCardOperate_syzp 出牌操作,没有动作");
			return true;
		}
		if ((operate_card != table._out_card_data)) {
			table.log_player_error(seat_index, "HHHandlerOutCardOperate_syzp 操作牌，与当前牌不一样");
			return true;
		}
		...
		case GameConstants.WIK_EQS:// 吃二七十
		{
			...
			table.add_lou_weave(target_lou_code, target_player, target_card, _out_card_player, table._lou_weave_item[target_player][table.getTablePlayerNumber()]);
			table.exe_chi_peng(target_player, _out_card_player, target_action, target_card, GameConstants.CHI_PENG_TYPE_OUT_CARD, target_lou_code);
			return true;
		}
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._cur_banker, GameConstants.Game_End_NORMAL), delay,
					TimeUnit.SECONDS);
