...
public class HandleOutCard_SongYuan extends AbstractMJHandler<MjTable_SongYuan> {
	...
	public boolean handler_operate_card(MjTable_SongYuan table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_CHI_TING_LEFT:{
		...
		case GameConstants.WIK_CHI_TING_RIGHT:{
			int cbRemoveCard[] = new int[] { operate_card - 1, operate_card - 2 };
			if (!table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2)) {
				table.log_player_error(seat_index, "吃牌删除出错");
				return false;
		...
		case GameConstants.WIK_CHI_TING_CENTER:{
			int cbRemoveCard[] = new int[] { operate_card - 1, operate_card + 1 };
			if (!table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2)) {
				table.log_player_error(seat_index, "吃牌删除出错");
				table.log_error("牌==" + operate_card );
				return false;
			}
		...
		case GameConstants.WIK_PENG_TING: {
