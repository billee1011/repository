...
public class SCHCPDZGameLogic {
	...
	public int get_magic_card_index(int index) {
		// m_cbMagicIndex
		return _magic_card_index[index];// MJGameConstants.DSS_MAX_CP_INDEX;
	}
	...
	public int check_nei_hua(int card_index[], int cur_card) {
		...
		// 碰牌判断
		return (card_index[switch_to_card_index(cur_card)] == 2) ? GameConstants.DZ_WIK_PENG : GameConstants.DZ_WIK_NULL;
	}
	...
	public boolean remove_card_by_index(int cards_index[], int card) {
		...
		if (card_index < 0 || card_index >= GameConstants.DSS_MAX_CP_INDEX) {
			return false;
		}
	...
	public int get_hong_pai_count(int weave_kind, int center_card) {
		...
		default:
			 return get_analyse_tuo_shu(weave_kind,center_card);
		}
		return 0;
	}
	...
	public int get_hei_pai_count(int weave_kind, int center_card) {
		...
		default:
			 return 3-get_analyse_tuo_shu(weave_kind,center_card);
		}
		return 0;
	}
	...
	public int get_analyse_tuo_shu(int cbWeaveKind, int center_card) {
		...
		case GameConstants.DZ_WIK_SINGLE_RIGHT: {
			...
			if (get_hong_dot(center_card) != 0)
				count = 2;
			if (get_hong_dot(this.get_kind_card(center_card, cbWeaveKind)) != 0)
				count += 1;
			return count;
		}
	...
	public int get_times_cards_ydr(int card) {
		int count = 0;
		switch (card) {
	...
	public int calculate_fan_count_ydr(WeaveItem weave_item[],int weave_count){
		int wFanShu = 0;
		for(int i = 0; i<weave_count;i++){
			switch (weave_item[i].weave_kind) {
			case GameConstants.DZ_WIK_PENG:
			case GameConstants.DZ_WIK_SHA_QIANG: {
				if (get_times_cards_ydr(weave_item[i].center_card) > 0) {
					wFanShu += 1;
				}
				break;
			}
	...
	public boolean isChengJu(int cards_index[], AnalyseItem analyseItem) {
		...
		int cbMagicCardIndex[] = new int[GameConstants.DSS_MAX_CP_INDEX];
		for (int i = 0; i < GameConstants.DSS_MAX_CP_INDEX; i++) {
		...
		int mj_count = GameConstants.DSS_MAX_CP_INDEX;
		for (int i = 0; i < mj_count; i++) {
			...
			if (cbMagicCardIndex[i] == 3) {
				...
				// 连牌判断
			if ((i < (GameConstants.DSS_MAX_CP_INDEX - 2)) && ((i % 3) == 0)) {
	...
	public int get_max_tuo_shu(WeaveItem weaveItem[], int cbWeaveCount,int cards_index[],int table_cards[]){
		int max_tuo_count = 0;
		int max_mao_shu = cbWeaveCount;
		int add_mao_count = 0;
		for(int i = 0; i < cbWeaveCount ;i++)
		{
			max_tuo_count += weaveItem[i].hu_xi;
		}
		for (int i = 0; i < GameConstants.MAX_CP_INDEX; i++) {
			if (cards_index[i] == 5) {
				max_tuo_count += get_analyse_tuo_shu(GameConstants.DZ_WIK_MTU_HUO, switch_to_card_data(i));
				max_mao_shu += 1;
				add_mao_count += 2;
			}
			if (cards_index[i] == 4) {
				max_tuo_count += get_analyse_tuo_shu(GameConstants.DZ_WIK_TUX_HUO, switch_to_card_data(i));
				max_mao_shu += 1;
				add_mao_count += 2;
			}
			if (cards_index[i] == 3 && cards_index[i]+table_cards[i] != 5) {
				max_tuo_count += get_analyse_tuo_shu(GameConstants.DZ_WIK_SHA,switch_to_card_data(i));
				max_mao_shu += 1;
				add_mao_count += 1;
			}
			if (cards_index[i] == 3 && cards_index[i]+table_cards[i] == 5) {
				max_tuo_count += get_analyse_tuo_shu(GameConstants.DZ_WIK_SHA_QIANG,switch_to_card_data(i));
				max_mao_shu += 1;
			}
		}
		// 变量定义
		int cbKindItemCount = 0;
		int cbLessKindItem = 7-max_mao_shu;
		int cbminKindItem = 7 - max_mao_shu-add_mao_count;
		int max_tuo_shu = 0;
		KindItem kindItem[] = new KindItem[50];
		...
		for (int i = 2; i < 13; i++) {
			int count = 0;
			int cards[] = new int[3];
			count = switch_to_value_to_card(i, cards);
			int count_next = 0;
			int cards_next[] = new int[3];
			count_next = switch_to_value_to_card(14 - i, cards_next);
			for(int j = 0; j<count ;j++){
				if(cards_index[switch_to_card_index(cards[j])] == 2&&cards_index[switch_to_card_index(cards[j])]+table_cards[switch_to_card_index(cards[j])]!=5){
					kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards[j]);
					kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards[j]);
					kindItem[cbKindItemCount].cbWeaveKind = GameConstants.DZ_WIK_PENG;
					kindItem[cbKindItemCount++].cbTuoShu = get_analyse_tuo_shu(GameConstants.DZ_WIK_SHA_QIANG,cards[j]);
				}
				else if(cards_index[switch_to_card_index(cards[j])] == 2){
					int  first_red = this.get_hong_dot(cards[j]);
				
					for (int k = 0; k < count_next; k++) {
						int next_red = this.get_hong_dot(cards_next[k]);
						if(cards[j] == cards_next[k])
							continue;
						if(first_red == 0 && next_red== 0)
							continue;
						if((first_red != 0||(cards_index[switch_to_card_index(cards_next[k])]>0&&cards_index[switch_to_card_index(cards_next[k])]+table_cards[switch_to_card_index(cards_next[k])]==5))&&table_cards[switch_to_card_index(cards_next[k])]!=5){
							int action = this.get_chi_action(cards[j], cards_next[k], cards[j]);
							kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards[j]);
							kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards[j]);
							kindItem[cbKindItemCount].cbWeaveKind = action;
							kindItem[cbKindItemCount++].cbTuoShu = get_analyse_tuo_shu(action,cards[j]);
						}
						else if(next_red != 0&& cards_index[switch_to_card_index(cards_next[k])]>0&&cards_index[switch_to_card_index(cards_next[k])]+table_cards[switch_to_card_index(cards_next[k])]!=5){
							int action = this.get_chi_action(cards[j], cards_next[k], cards_next[k]);
							kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards[j]);
							kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards_next[k]);
							kindItem[cbKindItemCount].cbWeaveKind = action;
							kindItem[cbKindItemCount++].cbTuoShu = get_analyse_tuo_shu(action,cards[j]);
						}else if(next_red != 0&&table_cards[switch_to_card_index(cards_next[k])]!=5){
							int action = this.get_chi_action(cards[j], cards_next[k], cards[j]);
							kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards[j]);
							kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards[j]);
							kindItem[cbKindItemCount].cbWeaveKind = action;
				...
				} else if(cards_index[switch_to_card_index(cards[j])] == 1){
					int  first_red = this.get_hong_dot(cards[j]);
					...
					for (int k = 0; k < count_next; k++) {
						...
						if(first_red == 0 && next_red == 0)
							continue;
						if(cards_index[switch_to_card_index(cards_next[k])] != 1 || cards_index[switch_to_card_index(cards[j])]!=1)
							continue;
						if(first_red != 0&&cards_index[switch_to_card_index(cards[j])]+table_cards[switch_to_card_index(cards[j])]!=5){
							int action = this.get_chi_action(cards[j], cards_next[k], cards[j]);
							kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards[j]);
							kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards_next[k]);
							kindItem[cbKindItemCount].cbWeaveKind = action;
						...
						else if(first_red != 0&&cards_index[switch_to_card_index(cards_next[k])]+table_cards[switch_to_card_index(cards_next[k])]!=5){
						...
						if(i == 7){
							if(first_red == 0 || next_red == 0)
								continue;
							if(table_cards[switch_to_card_index(0x07)]==5)
								continue;
							int action = this.get_chi_action(cards[j], cards_next[k], 0x07);
							kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards[j]);
							kindItem[cbKindItemCount].cbCardIndex[kindItem[cbKindItemCount].cbCardCount++] = switch_to_card_index(cards_next[k]);
							kindItem[cbKindItemCount].cbWeaveKind = action;
							kindItem[cbKindItemCount++].cbTuoShu = get_analyse_tuo_shu(action,cards[j]);
			...
			if (cbKindItemCount >= cbLessKindItem&&cbLessKindItem>0) {
				// 变量定义
				int cbCardIndexTemp[] = new int[GameConstants.DZ_MAX_CP_INDEX];
				...
				// 开始组合
				do {
					int hh_count = GameConstants.DZ_MAX_CP_INDEX;
					// 设置变量
					...
					for (int i = 0; i < cbLessKindItem; i++) {
						...
						pKindItem[i].cbCardCount = kindItem[cbIndex[i]].cbCardCount;
						pKindItem[i].cbTuoShu = kindItem[cbIndex[i]].cbTuoShu;
						for (int j = 0; j < kindItem[cbIndex[i]].cbCardCount; j++) {
					...
					for(int i = 0; i< cbLessKindItem;i++)
					{
						tuo_shu += pKindItem[i].cbTuoShu;
						for(int j = 0; j < pKindItem[i].cbCardCount; j++)
						{
							int cbCardIndex = pKindItem[i].cbCardIndex[j];
						...
						if(bEnoughCard == false)
							break;
					}

					// 胡牌判断
					if (bEnoughCard == true&&max_tuo_shu < max_tuo_count + tuo_shu) {

						max_tuo_shu = 	max_tuo_count + tuo_shu;
					}
					...
					if (cbIndex[cbLessKindItem - 1] == (cbKindItemCount - 1)) {
						...
						if (i == 0)
						{
							if((max_tuo_shu == 0||cbLessKindItem>cbminKindItem)&&cbLessKindItem>1)
							{
								for(int j = 0; j<cbLessKindItem;j++)
									cbIndex[j] = j;
								cbLessKindItem--;
							}
							else
				...
				for(int i = 0; i< cbLessKindItem;i++)
				{
					temp_tuo_shu +=   kindItem[i].cbTuoShu;
				}
				if(temp_tuo_shu + max_tuo_count > max_tuo_shu)
					max_tuo_shu = temp_tuo_shu + max_tuo_count;
			}
			if(max_tuo_shu < max_tuo_count)
				max_tuo_shu = max_tuo_count;
		return max_tuo_shu;	
	}
	// 分析扑克
	public boolean analyse_card(int cards_index[], WeaveItem weaveItem[], int cbWeaveCount, int seat_index, int provider_index, int cur_card,
			List<AnalyseItem> analyseItemArray, boolean dispatch,boolean is_peng) {
		// 计算数目
		int cbCardCount = get_card_count_by_index(cards_index);
		...
		for (int i = 0; i < GameConstants.DSS_MAX_CP_INDEX; i++) {
			if (cards_index[i] == 4) {
				analyseItem.cbWeaveKind[array_count] = GameConstants.DZ_WIK_TU_HUO;
				analyseItem.cbCenterCard[array_count] = this.switch_to_card_data(i);
				analyseItem.hu_xi[array_count++] = this.get_analyse_tuo_shu(GameConstants.DZ_WIK_TU_HUO, this.switch_to_card_data(i));
				temp_cards_index[i] = 0;
			} else if (cards_index[i] == 3) {
				if (dispatch == false && i == this.switch_to_card_index(cur_card)&&is_peng== true)
					analyseItem.cbWeaveKind[array_count] = GameConstants.DZ_WIK_PENG;
				else if(dispatch == false && i != this.switch_to_card_index(cur_card))
					analyseItem.cbWeaveKind[array_count] = GameConstants.DZ_WIK_SHA_QIANG;
				else if(dispatch == true)
					analyseItem.cbWeaveKind[array_count] = GameConstants.DZ_WIK_SHA_QIANG;
				if(analyseItem.cbWeaveKind[array_count] != 0)
				{
					analyseItem.cbCenterCard[array_count] = this.switch_to_card_data(i);
		...
		for (int i = 2; i <= 7; i++) {
			...
			int low_dot_count= 0;
			int height_dot_count = 0;
			for (int j = 0; j < card_count; j++) {
				if(temp_cards_index[this.switch_to_card_index(cards[j])] > 0 )
					low_dot_count ++;
			}
			for (int j = 0; j < card_count; j++) {
				if(temp_cards_index[this.switch_to_card_index(cards1[j])] > 0 )
					height_dot_count ++;
			}
			if(low_dot_count>=height_dot_count ){
				for (int j = 0; j < card_count; j++) {
					if (temp_cards_index[this.switch_to_card_index(cards[j])] == 0)
						continue;
					if (temp_cards_index[this.switch_to_card_index(cards[j])] == 1) {
					...
					if (temp_cards_index[this.switch_to_card_index(cards[j])] > 1) {
						...
						for (; k < card_count1; k++) {
							if (temp_cards_index[this.switch_to_card_index(cards1[k])] > 0) {
								...
								--temp_cards_index[this.switch_to_card_index(cards[j])];
								--temp_cards_index[this.switch_to_card_index(cards1[k])];
								int action = this.get_chi_action(cards[j], cards1[k], cards[j]);
								analyseItem.cbWeaveKind[array_count] = action;
								analyseItem.cbCenterCard[array_count] = cards[j];
								analyseItem.hu_xi[array_count++] = this.get_analyse_tuo_shu(action, cards[j]);
								if(temp_cards_index[this.switch_to_card_index(cards[j])]>0)
				...
				for (int j = 0; j < card_count1; j++) {
					if (temp_cards_index[this.switch_to_card_index(cards1[j])] == 0)
						continue;
					if (temp_cards_index[this.switch_to_card_index(cards1[j])] == 1) {
						int k = 0;
						for (; k < card_count; k++) {
							if (temp_cards_index[this.switch_to_card_index(cards[k])] > 1) {
								--temp_cards_index[this.switch_to_card_index(cards1[j])];
								--temp_cards_index[this.switch_to_card_index(cards[k])];
								--temp_cards_index[this.switch_to_card_index(cards[k])];
								int action = this.get_chi_action(cards1[j], cards[k], cards[k]);
								analyseItem.cbWeaveKind[array_count] = action;
								analyseItem.cbCenterCard[array_count] = cards1[j];
								analyseItem.hu_xi[array_count++] = this.get_analyse_tuo_shu(action, cards1[j]);
								if(temp_cards_index[this.switch_to_card_index(cards1[j])]>0)
								{
									j--;
									break;
					...
					if (temp_cards_index[this.switch_to_card_index(cards1[j])] > 1) {
						...
						for (; k < card_count; k++) {
							if (temp_cards_index[this.switch_to_card_index(cards[k])] > 0) {
								...
								--temp_cards_index[this.switch_to_card_index(cards1[j])];
								--temp_cards_index[this.switch_to_card_index(cards[k])];
								int action = this.get_chi_action(cards1[j], cards[k], cards1[j]);
								analyseItem.cbWeaveKind[array_count] = action;
								analyseItem.cbCenterCard[array_count] = cards1[j];
								analyseItem.hu_xi[array_count++] = this.get_analyse_tuo_shu(action, cards1[j]);
								if(temp_cards_index[this.switch_to_card_index(cards[j])]>0)
	...
	public int get_action_rank(int player_action) {
		// 自摸牌等级
		if (player_action == GameConstants.DZ_WIK_ZI_MO) {
			return 60;
		}
		if (player_action == GameConstants.DZ_WIK_CHI_HU) {
			return 60;
		}
		if (player_action == GameConstants.DZ_WIK_TUX_HUO) {
			return 50;
		}
		if (player_action == GameConstants.DZ_WIK_MTU_HUO) {
			return 50;
		}
		if (player_action == GameConstants.DZ_WIK_TU_HUO) {
			return 40;
		}
		if (player_action == GameConstants.DZ_WIK_SHA) {
			return 40;
		}
		if (player_action == GameConstants.DZ_WIK_PENG) {
			return 30;
		}

		// 上牌等级
		if (player_action == GameConstants.DZ_WIK_DUI_LEFT || player_action == GameConstants.DZ_WIK_SINGLE_LEFT
				|| player_action == GameConstants.DZ_WIK_DUI_CENTER || player_action == GameConstants.DZ_WIK_SINGLE_CENTER
				|| player_action == GameConstants.DZ_WIK_DUI_RIGHT || player_action == GameConstants.DZ_WIK_SINGLE_RIGHT
				 || player_action == GameConstants.DZ_WIK_OTHER) {
			return 10;
		}
	...
	public int get_action_list_rank(int action_count, int action[]) {
		int DSS_MAX_CP_INDEX = 0;

		for (int i = 0; i < action_count; i++) {
			int index = get_action_rank(action[i]);
			if (DSS_MAX_CP_INDEX < index) {
				DSS_MAX_CP_INDEX = index;
			}

		}

		return DSS_MAX_CP_INDEX;
	}
	...
	public int get_kind_card(int card, int type) {
		...
		case 0x04:
		case 0x13: {
			if (type == GameConstants.DZ_WIK_SINGLE_LEFT || type == GameConstants.DZ_WIK_DUI_LEFT)
				return 0x0a;
			if (type == GameConstants.DZ_WIK_SINGLE_CENTER || type == GameConstants.DZ_WIK_DUI_CENTER)
				return 0x46;
		}
		case 0x05:
		case 0x50: {
		...
		case 0x42: {
			...
			if (type == GameConstants.DZ_WIK_SINGLE_CENTER || type == GameConstants.DZ_WIK_DUI_CENTER)
				return 0xf8;
			if (type == GameConstants.DZ_WIK_SINGLE_RIGHT || type == GameConstants.DZ_WIK_DUI_RIGHT)
				return 0x80;
		}
		case 0x43:
		case 0x07:
		case 0x16: {
			...
			if (type == GameConstants.DZ_WIK_SINGLE_RIGHT || type == GameConstants.DZ_WIK_DUI_RIGHT)
				return 0x16;
			if (type == GameConstants.DZ_WIK_OTHER)
				return 0x43;
		}
		case 0xd8:
		case 0xf8:
