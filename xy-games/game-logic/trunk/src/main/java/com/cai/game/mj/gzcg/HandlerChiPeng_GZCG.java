...
public class HandlerChiPeng_GZCG extends MJHandlerChiPeng<Table_GZCG> {
	...
	public void exe(Table_GZCG table) {
		...
		if (_card == table.jing[0] || _card == table.jing[1]) {
			table.GRR._weave_items[_seat_index][wIndex].center_card = _card | 0x100;
		}
		...
		if (ting_count > 0) {
			...
			int tmp_hand_card_count = table._logic.switch_to_cards_data_gzcg(table.GRR._cards_index[_seat_index], tmp_cards);
			table.changeCard(tmp_cards, table.GRR._card_count[_seat_index]);
			for (int i = 0; i < tmp_hand_card_count; i++) {
				int card = table.getRealCard(tmp_cards[i]);
