...
public class PHZHandlerOutCardOperate_DY extends PHZHandlerOutCardOperate<HBPHZTable> {
	@Override
	public void exe(HBPHZTable table) {
		...
		if(table.GRR._cards_index[_out_card_player][table._logic.switch_to_card_index(_out_card_data)] >= 2)
		{
		...
		int card_count = table._logic.get_card_count_by_index(table.GRR._cards_index[_out_card_player]);
		if(card_count == 0 )
		{
			if (table._is_xiang_gong[_out_card_player] == false){
				table._is_xiang_gong[_out_card_player] = true;
				table.operate_player_xiang_gong_flag(_out_card_player, table._is_xiang_gong[_out_card_player]);
			}
		...
		while (loop < table.getTablePlayerNumber()) {
			...
			action_pao[i] = table.estimate_player_respond_hh(i, _out_card_player, _out_card_data, pao_type[i], false);
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				if (action_pao[i] != GameConstants.WIK_HBZP_ZHAO) {
					...
					for(int j= 0; j< GameConstants.MAX_HH_INDEX;j++){
						if(table.GRR._cards_index[i][j]<3)
							count += table.GRR._cards_index[i][j];
					}
					if(count != 0){
						tempPlayerStatus.add_action(GameConstants.WIK_HBZP_ZHAO);
						tempPlayerStatus.add_hbzp_zhao(_out_card_data,GameConstants.WIK_HBZP_ZHAO, _out_card_player);
					}
		...
		table._playerStatus[_out_card_player]._hu_card_count = table.get_hh_ting_card_twenty(table._playerStatus[_out_card_player]._hu_cards,
				table.GRR._cards_index[_out_card_player], table.GRR._weave_items[_out_card_player], table.GRR._weave_count[_out_card_player], _out_card_player,
				_out_card_player);
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			if ((action_pao[i] != GameConstants.WIK_NULL) && (bHupai == 0)) {
				...
				ti_pao = table.estimate_player_respond_hh(i, _out_card_player, _out_card_data, pao_type_out, false);
				...
				ti_pao = GameConstants.WIK_HBZP_ZHAO;
			}
		...
		if (bAroseAction == false && bHupai == 0 && ti_pao== GameConstants.WIK_NULL) {
			...
			_out_card_data = 0;
			//table.log_info("dispath = "+dispatch_time);
	...
	public boolean handler_operate_card(HBPHZTable table, int seat_index, int operate_code, int operate_card,int luoCode) {
		// 效验状态
...
//		}
		...
		int eat_type = GameConstants.WIK_HBZP_LEFT | GameConstants.WIK_HBZP_CENTER | GameConstants.WIK_HBZP_RIGHT
				| GameConstants.WIK_HBZP_DDX | GameConstants.WIK_HBZP_XXD | GameConstants.WIK_HBZP_EQS;
		// 用户状态
		if(target_action == GameConstants.WIK_NULL)
		{
			...
			for(int i = 0; i<table.getTablePlayerNumber();i++)
			{
				 ...
				 {
					if(table._playerStatus[i]._action[j] == GameConstants.WIK_HBZP_PENG)
					{
		...
		case GameConstants.WIK_HBZP_LEFT: // 上牌操作
		{
			// 删除扑克
			table._is_di_hu = true;
		...
		case GameConstants.WIK_HBZP_RIGHT: // 上牌操作
		{
			table._is_di_hu = true;
			// 删除扑克
			int cbRemoveCard[] = new int[] { target_card - 1, target_card - 2 };
		...
		case GameConstants.WIK_HBZP_CENTER: // 上牌操作
		{
			table._is_di_hu = true;
			// 删除扑克
		...
		case GameConstants.WIK_HBZP_EQS://吃二七十
		{
			table._is_di_hu = true;
			// 删除扑克
		...
		case GameConstants.WIK_NULL: {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				int pao_type[] = new int[1];
				int action = table.estimate_player_respond_hh(i, _out_card_player, _out_card_data, pao_type, false);
		...
		case GameConstants.CP_WIK_ZHAO: // 杠牌操作
		{

			int pao_type[] = new int[1];
			int action = table.estimate_player_respond_hh(target_player, this._out_card_player, operate_card, pao_type, false);
			if (action != GameConstants.WIK_NULL) {
				table.exe_gang(target_player, this._out_card_player, operate_card, action, pao_type[0], false, true, false, 300);
			}
...
//			}
			...
			table.process_chi_hu_player_score_dy(target_player, _out_card_player, _out_card_data, false);

			// 记录
			// table._player_result.jie_pao_count[target_player]++;
			table._player_result.dian_pao_count[_out_card_player]++;
			table.countChiHuTimes(target_player, _out_card_player,false);
	...
	public boolean handler_player_be_in_room(HBPHZTable table, int seat_index) {
		...
		{
			if(table._is_xiang_gong[i] == true)
				table.operate_player_xiang_gong_flag(i,table._is_xiang_gong[i]);
		}
