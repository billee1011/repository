 ...
 *
 */
public class PHHandlerDispatchCard_YX extends PHHandlerDispatchCard<PHTable> {
	...
	public void exe(PHTable table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				if (table.matchId != 0)
					temp_match_times = table.matchBase.getBase() * table.matchBase.getBaseScore() * table.matchBase.getTimes();

				table.GRR._game_score[i] = table._game_mid_score[i] * temp_match_times;
		...
		if (an_long_count > 0 && table._long_count[_seat_index] == 0) {
			...
			// 1, new long[]{_action}, 1,GameConstants.INVALID_SEAT);
			//
			for (int i = 0; i < an_long_count; i++) {
				// int cbWeaveIndex = table.GRR._weave_count[_seat_index];
				// table.GRR._weave_items[_seat_index][cbWeaveIndex].public_card
				// = 1;
		...
		if (ti_sao != GameConstants.WIK_NULL) {
			...
			table.exe_gang(_seat_index, _seat_index, _send_card_data, ti_sao, action_type[0], true, true, false, 1000);
			_send_card_data = 0;
			return;
		}
		...
		while (loop < table.getTablePlayerNumber()) {
			...
			action_hu[i] = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i], table.GRR._weave_count[i], i, _seat_index,
					card, chr[i], card_type, hu_xi_chi, true);// 自摸
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				if (table.has_rule(GameConstants.GAME_RULE_QZ_HU_PAI_ON) || table._warning[i] == 2) {
					...
					table._playerStatus[i].set_handler_status(GameConstants.PH_STATUS_DISPATCH);
					...
					GameSchedule.put(new HuPaiRunnable(table.getRoom_id(), i, GameConstants.WIK_ZI_MO, _send_card_data), dalay,
		...
		if (curPlayerStatus.has_action()) {// 有动作
			...
			if (bAroseAction == false) {
				...
				if (table.has_rule(GameConstants.GAME_RULE_QUICK_SPEED) && sysParamModel1110 != null && sysParamModel1110.getVal4() > 0
						&& sysParamModel1110.getVal4() < 10000) {
				...
				if (table.has_rule(GameConstants.GAME_RULE_QUICK_SPEED) && sysParamModel1110 != null && sysParamModel1110.getVal5() > 0
	...
	public boolean handler_status_operate_card(PHTable table, int seat_index, int operate_code, int operate_card, int luoCode, int handler_status) {
		...
		// assert handler_status == GameConstants.PH_STATUS_DISPATCH;
		if (handler_status != GameConstants.PH_STATUS_DISPATCH) {
			table.log_error(
					"DispatchCard handler_status = " + handler_status + "!= " + GameConstants.PH_STATUS_DISPATCH + "seat_index = " + seat_index);
			// return false;
		}
		...
		// {
		// table._guo_hu_pai_cards[seat_index][table._guo_hu_pai_count[seat_index]++]
		// = operate_card;
		...
		case GameConstants.WIK_NULL: {
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				...
				table.operate_effect_action(target_player, GameConstants.EFFECT_ACTTON_CACEL_DISPLAY_CARD, _seat_index,
						new long[] { GameConstants.WIK_NULL }, 1, GameConstants.INVALID_SEAT);
				// table.log_info("EFFECT_ACTTON_CACEL_DISPLAY_CARD" +
				// _seat_index);
				// table.operate_player_get_card(_seat_index, 0, null,
				// GameConstants.INVALID_SEAT,false);
				...
				if (table.has_rule(GameConstants.GAME_RULE_QUICK_SPEED) && sysParamModel1111 != null && sysParamModel1111.getVal3() > 0
						&& sysParamModel1111.getVal3() < 10000) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			if (!(chr.opr_and(GameConstants.CHR_PAO_HU_PH)).is_empty()) {
				for (int i = 0; i < table.GRR._weave_count[target_player]; i++) {
					if (table.GRR._weave_items[target_player][i].center_card == operate_card
							&& table.GRR._weave_items[target_player][i].weave_kind == GameConstants.WIK_PENG_OUT)
