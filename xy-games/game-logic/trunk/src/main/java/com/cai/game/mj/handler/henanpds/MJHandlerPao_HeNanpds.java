...
public class MJHandlerPao_HeNanpds extends MJHandlerPao_HeNan {
	...
	public void exe(MJTable table) {
		if (table.is_mj_type(GameConstants.GAME_TYPE_NEW_PING_DING_SHAN)) {
	...
	public boolean handler_pao_qiang(MJTable table, int seat_index, int pao, int qiang) {
		...
		if (table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU) && !table.is_mj_type(GameConstants.GAME_TYPE_NEW_PING_DING_SHAN)) {
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				table._playerStatus[i]._is_pao_qiang = false;
			}
			table.exe_kou();
		} else {
