...
public class HandlerDispatchCard_XinYang extends MJHandlerDispatchCard<Table_XinYang> {
	...
	public HandlerDispatchCard_XinYang() {
	...
	public void exe(Table_XinYang table) {
		...
		if (table._player_result.pao[_seat_index] != 0 || table._player_result.qiang[_seat_index] != 0) {
			if (_type != GameConstants.GANG_TYPE_HONG_ZHONG)
				show_send_card += GameConstants.CARD_ESPECIAL_TYPE_GUI;
		...
		// 闹庄
		if (_type == GameConstants.GANG_TYPE_HONG_ZHONG) {
			if (table.has_rule(Constants_MJ_XIN_YANG.GAME_RULE_NAO_ZHUANG) && table.continueBankerCount > 0 && table.isLastGameDianPao3) {
				table.set_handler(table._handler_nao);
				table._handler_nao.exe(table);
				return;
			} else {
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					table._player_result.nao[i] = -1;// 清掉 默认是-1
				}
			}
		}
	...
	public boolean handler_player_be_in_room(Table_XinYang table, int seat_index) {
		...
		int show_send_card = _send_card_data;
		if (table._player_result.pao[seat_index] != 0 || table._player_result.qiang[seat_index] != 0) {
			show_send_card += GameConstants.CARD_ESPECIAL_TYPE_GUI;
		} else if (ting_send_card) {
			show_send_card += GameConstants.CARD_ESPECIAL_TYPE_TING;
