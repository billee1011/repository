...
public class HandlerOutCardOperate_BD extends MJHandlerOutCardOperate<Table_BD> {
    ...
    public void exe(Table_BD table) {
        if (_type == GameConstants_HZLZG.OutCard_Type_Di_Hu)
        ...
        if (table.gen_zhuang_card != -1 && _out_card_data != table.gen_zhuang_card) {
            ...
            table.operate_effect_action(_out_card_player, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1,
                    new long[] { GameConstants_BD.WIK_GEN_ZHUANG }, 1, GameConstants.INVALID_SEAT);
        }

        // 跟庄啦
        if (table.gen_zhuang_card != -1 && _out_card_player == table.last_out_player_4_banker) {
            for (int i = 0; i < table.getTablePlayerNumber(); i++) {
                if (i == table._cur_banker)
                    continue;

                table._player_result.game_score[i] += 1;
                table._player_result.game_score[table._cur_banker] -= 1;
            }
    ...
    public boolean handler_operate_card(Table_BD table, int seat_index, int operate_code, int operate_card) {
        ...
        case GameConstants.WIK_GANG: {
            ...
            table.exe_gang(target_player, _out_card_player, target_card, target_action,
                    GameConstants.GANG_TYPE_JIE_GANG, false, flag);
            return true;
        }
