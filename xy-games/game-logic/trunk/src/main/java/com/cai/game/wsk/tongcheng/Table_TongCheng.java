...
import protobuf.clazz.tcdg.TcdgRsp.GameStart_Wsk_tcdg;
import protobuf.clazz.tcdg.TcdgRsp.LiangPai_Result_tcdg;
import protobuf.clazz.tcdg.TcdgRsp.Opreate_RequestWsk_tcdg;
import protobuf.clazz.tcdg.TcdgRsp.OutCardDataWsk_tcdg;
import protobuf.clazz.tcdg.TcdgRsp.PukeGameEndWsk_tcdg;
import protobuf.clazz.tcdg.TcdgRsp.RefreshCardData_tcdg;
import protobuf.clazz.tcdg.TcdgRsp.RefreshMingPai_tcdg;
import protobuf.clazz.tcdg.TcdgRsp.RefreshScore_tcdg;
import protobuf.clazz.tcdg.TcdgRsp.TableResponse_tcdg;

public class Table_TongCheng extends AbstractRoom {
	private static final long serialVersionUID = -2419887548488809773L;

	protected static Logger logger = Logger.getLogger(Table_TongCheng.class);
	...
	public GameLogic_TongCheng _logic = new GameLogic_TongCheng();
	...
	public AbstractHandler_TongCheng _handler;
	public HandlerOutCardOperate_TongCheng _handler_out_card_operate;
	public HandlerCallBnaker_TongCheng _handler_call_banker;
	public HandlerFinish_TongCheng _handler_finish;
	public HandlerPiao_TongCheng _handler_piao;
	...
	protected static final int GAME_OPREATE_TYPE_POSITION_SORT_CARD = 7; // 拖动理牌
	...
	public void on_init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_handler_out_card_operate = new HandlerOutCardOperate_TongCheng();
		_handler_call_banker = new HandlerCallBnaker_TongCheng();
		_handler_finish = new HandlerFinish_TongCheng();
		_handler_piao = new HandlerPiao_TongCheng();
		...
		hasRuleLiuZhaYiFen = has_rule(GameConstants.GAME_RULE_PK_TC_SIX_BOOM_REWARD);
		hasRuleBoomMustOut = has_rule(GameConstants.GAME_RULE_PK_TC_BOOM_MUST_OUT);
		hasRuleDisplayCount = has_rule(GameConstants.GAME_RULE_PK_TC_DISPLAY_COUNT);
		hasRulePiao = has_rule(GameConstants.GAME_RULE_PK_TC_PIAO);
	}
	...
	public int get_hand_card_count_max() {
		return GameConstants.WSK_MAX_COUNT;
	}
	...
	protected boolean on_game_start_real() {
		for (int play_index = 0; play_index < getTablePlayerNumber(); play_index++) {
			...
			// 发送数据
			GameStart_Wsk_tcdg.Builder gamestart = GameStart_Wsk_tcdg.newBuilder();
	...
	public void load_player_info_data_game_start(GameStart_Wsk_tcdg.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			rplayer = get_players()[i];
			if (rplayer == null)
	...
	protected void init_shuffle() {
		if (has_rule(GameConstants.GAME_RULE_PK_TC_ZERO_MAGIC)) {
			_repertory_card = new int[Constants_TongCheng.CARD_DATA_WSK.length];
			shuffle(_repertory_card, Constants_TongCheng.CARD_DATA_WSK);
		} else if (has_rule(GameConstants.GAME_RULE_PK_TC_ONE_MAGIC)) {
			_repertory_card = new int[Constants_TongCheng.CARD_DATA_WSK_ONE_MAGIC.length];
			shuffle(_repertory_card, Constants_TongCheng.CARD_DATA_WSK_ONE_MAGIC);
		} else if (has_rule(GameConstants.GAME_RULE_PK_TC_TWO_MAGIC)) {
			_repertory_card = new int[Constants_TongCheng.CARD_DATA_WSK_TWO_MAGIC.length];
			shuffle(_repertory_card, Constants_TongCheng.CARD_DATA_WSK_TWO_MAGIC);
		} else {
			_repertory_card = new int[Constants_TongCheng.CARD_DATA_WSK.length];
			shuffle(_repertory_card, Constants_TongCheng.CARD_DATA_WSK);
		}
	...
	protected boolean on_handler_game_finish(int seat_index, int reason) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_WSK_TCDG_GAME_END);
		GameEndResponse.Builder game_end = GameEndResponse.newBuilder();
		PukeGameEndWsk_tcdg.Builder game_end_wsk = PukeGameEndWsk_tcdg.newBuilder();
		game_end.setRoomInfo(getRoomInfo());
		game_end_wsk.setRoomInfo(getRoomInfo());

		// 计算分数
		if (reason == GameConstants.Game_End_NORMAL) {
			cal_score_wsk(_win_score);
			process_xi_qian();
		...
		if (GRR != null) {
			...
			if (_is_yi_da_san) {
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					if (i == GRR._banker_player) {
						game_end_wsk.addTeamNumber(1);
					} else {
						game_end_wsk.addTeamNumber(2);
					}
				}
			} else {
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					if (i == GRR._banker_player || i == _friend_seat[GRR._banker_player]) {
						game_end_wsk.addTeamNumber(1);
					} else {
						game_end_wsk.addTeamNumber(2);
					}
		...
		if (_is_yi_da_san) {
			end_score_type = 1;
		} else if (_win_score[0] == 1 || _win_score[0] == -1) {
			end_score_type = 2;
		} else if (_win_score[0] == 2 || _win_score[0] == -2) {
			end_score_type = 3;
		} else if (_win_score[0] == 4 || _win_score[0] == -4) {
			end_score_type = 4;
		}
		...
		game_end_wsk.setEndScoreType(end_score_type);

		for (int i = 0; i < getTablePlayerNumber(); i++) {
			_player_result.game_score[i] += _win_score[i] + _xi_qian_score[i] + _magic_card_score[i] + _punish_score[i] + _piao_score[i];
			get_players()[i].setGame_score((long) _player_result.game_score[i]);

			game_end.addGameScore(_win_score[i] + _xi_qian_score[i] + _magic_card_score[i] + _punish_score[i]);

			game_end_wsk.addZhuaFen(_get_score[i]); // 抓分
			game_end_wsk.addYingFenScore(_win_score[i]); // 赢分
			game_end_wsk.addXianQianScore(_xi_qian_score[i]); // 彩头
			game_end_wsk.addHuaPaiScore(_magic_card_score[i]); // 花牌
			game_end_wsk.addChengFaScore(_punish_score[i]); // 罚分
			game_end_wsk.addTotalPiaoScore(_piao_score[i]); // 飘分

			game_end_wsk.addWinOrder(win_order[i]);

			game_end_wsk.addEndScore(_win_score[i] + _xi_qian_score[i] + _magic_card_score[i] + _punish_score[i] + _piao_score[i]); // 总结算分
		}
		...
		if (reason == GameConstants.Game_End_NORMAL || reason == GameConstants.Game_End_DRAW) {
			if (_cur_round >= _game_round) {// 局数到了
				...
				for (int i = 0; i < getTablePlayerNumber(); i++) {
					...
					game_end_wsk.addBankerTimes(_solo_times[i]); // 独牌次数
					game_end_wsk.addTouYouTimes(_first_finish_times[i]); // 一游次数
					game_end_wsk.addLiuXiTimes(_six_boom_times[i]); // 六喜次数
					game_end_wsk.addQiXiTimes(_seven_boom_times[i]); // 七喜次数
					game_end_wsk.addBaXiTimes(_eight_boom_times[i]); // 八喜次数
		...
		if (!_is_yi_da_san) {
			...
			if (you_num == 1) {
				...
				if (_friend_seat[_chuwan_shunxu[0]] == _chuwan_shunxu[2]) {
					...
					// 一四游同队
					if (_chuwan_shunxu[0] == GRR._banker_player || _friend_seat[_chuwan_shunxu[0]] == GRR._banker_player)
						score_fence = 105;
	...
	public void refresh_ming_pai(int to_player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		RefreshMingPai_tcdg.Builder refresh_ming_pai = RefreshMingPai_tcdg.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_WSK_TCDG_REFRESH_MING_PAI);
	...
	public void load_player_info_data_reconnect(TableResponse_tcdg.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			rplayer = get_players()[i];
			if (rplayer == null)
	...
	public void load_player_info_data_game_end(PukeGameEndWsk_tcdg.Builder roomResponse) {
		Player rplayer;
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			rplayer = get_players()[i];
			if (rplayer == null)
	...
	public boolean operate_out_card(int seat_index, int count, int cards_data[], int type, int to_player, boolean is_deal) {
		...
		for (int index = 0; index < getTablePlayerNumber(); index++) {
			RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
			OutCardDataWsk_tcdg.Builder outcarddata = OutCardDataWsk_tcdg.newBuilder();
	...
	public void refresh_user_get_score(int to_player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		RefreshScore_tcdg.Builder refresh_user_getscore = RefreshScore_tcdg.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_WSK_TCDG_USER_GET_SCORE);
		Player rplayer;
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			rplayer = get_players()[i];
			if (rplayer == null)
	...
	public void set_handler(AbstractHandler_TongCheng handler) {
		_handler = handler;
	}
	...
	public boolean handler_requst_message_deal(Player player, int seat_index, RoomRequest room_rq, int type) {
		if (type == MsgConstants.REQUST_TCDG_OPERATE) {
			Opreate_RequestWsk_tcdg req = PBUtil.toObject(room_rq, Opreate_RequestWsk_tcdg.class);
			return handler_requst_opreate(seat_index, req.getOpreateType(), req.getCardData(), req.getSortCardList(), req.getCardIndex());
		}
	...
	public void deal_liang_pai(int seat_index, int card_data) {
		...
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_WSK_TCDG_LIANG_PAI_RESULT);
		// 发送数据
		LiangPai_Result_tcdg.Builder liang_pai_result = LiangPai_Result_tcdg.newBuilder();
	...
	public void auto_liang_pai(int seat_index) {
		...
		if (has_rule(GameConstants.GAME_RULE_PK_TC_ONE_MAGIC) && single_card_count == 1 && single_cards_data[0] == Constants_TongCheng.FLOWER_CARD) {
		...
		for (int i = 0; i < single_card_count; i++) {
			if (single_cards_data[i] != Constants_TongCheng.FLOWER_CARD && single_cards_data[i] != Constants_TongCheng.CARD_BIG_MAGIC
					&& single_cards_data[i] != Constants_TongCheng.CARD_SMALL_MAGIC) {
	...
	public boolean handler_requst_opreate(int seat_index, int opreate_type, int card_data, List<Integer> list, int position) {
		switch (opreate_type) {
		...
		case GAME_OPREATE_TYPE_POSITION_SORT_CARD: {
			deal_position_sort_card(seat_index, card_data, position);
			return true;
		}
		}
		return true;
	...
	public void deal_position_sort_card(int seat_index, int srcPosition, int position) {
		if (GRR == null)
			return;

		int card_count = GRR._card_count[seat_index];

		if (card_count == 0)
			return;

		if (position < 0 || position > card_count - 1)
			return;

		if (srcPosition < 0 || srcPosition > card_count - 1)
			return;
		...
		if (srcPosition < position) {
			int card = GRR._cards_data[seat_index][srcPosition];
			for (int j = srcPosition; j < position; j++) {
				GRR._cards_data[seat_index][j] = GRR._cards_data[seat_index][j + 1];
			}
			GRR._cards_data[seat_index][position] = card;
		}

		if (srcPosition > position) {
			int card = GRR._cards_data[seat_index][srcPosition];
			for (int j = srcPosition - 1; j >= position; j--) {
				GRR._cards_data[seat_index][j + 1] = GRR._cards_data[seat_index][j];
			}
			GRR._cards_data[seat_index][position] = card;
		}
	...
	public void deal_sort_card_custom(int seat_index, List<Integer> list) {
		...
		if (count == 0) {
			...
			for (int j = 0; j < GRR._card_count[seat_index]; j++) {
				if (GRR._cards_data[seat_index][j] > Constants_TongCheng.SPECIAL_CARD_TYPE) {
					hasCustomer = true;
					break;
				}
			}
			...
			if (flag_lost) {
				...
				for (int i = GRR._card_count[seat_index] - 1; i >= 0; i--) {
					if (GRR._cards_data[seat_index][i] > Constants_TongCheng.SPECIAL_CARD_TYPE) {
	...
	public void refresh_card(int to_player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_WSK_TCDG_REFRESH_CARD);
		// 发送数据
		RefreshCardData_tcdg.Builder refresh_card = RefreshCardData_tcdg.newBuilder();
		for (int i = 0; i < getTablePlayerNumber(); i++) {
			Int32ArrayResponse.Builder cards_card = Int32ArrayResponse.newBuilder();
			if (to_player == i) {
				for (int j = 0; j < GRR._card_count[i]; j++) {
	...
	public void refresh_pai_score(int to_player) {
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
		roomResponse.setType(MsgConstants.RESPONSE_WSK_TCDG_PAI_SCORE);
		roomResponse.setGameStatus(_game_status);
		roomResponse.setRoomInfo(getRoomInfo());
		// 发送数据
		...
		for (int i = 0; i < _out_pai_score_count; i++) {
			if (_logic.get_card_logic_value(_out_pai_score_card[i]) == Constants_TongCheng.CARD_THIRTEEN) {
		...
		for (int i = 0; i < _out_pai_score_count; i++) {
			if (_logic.get_card_logic_value(_out_pai_score_card[i]) == Constants_TongCheng.CARD_TEN) {
				cards.addItem(_out_pai_score_card[i]);
				count++;
			}
		}
		...
		for (int i = 0; i < _out_pai_score_count; i++) {
			if (_logic.get_card_logic_value(_out_pai_score_card[i]) == Constants_TongCheng.CARD_FIVE) {
				cards.addItem(_out_pai_score_card[i]);
				count++;
			}
		}
