 ...
 *
 */
public class SCHCPHandlerDispatchCard_YanZhong extends SCHCPHandlerDispatchCard<SCHCPTable_YanZhong> {
...
//		 _send_card_data = 0x66;
		...
		while(loop < table.getTablePlayerNumber()){
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				tempPlayerStatus.add_action(GameConstants.CP_WIK_ZI_MO);
		...
		for(int i = 0; i < table.getTablePlayerNumber();i++)
		{
			if(i == table._xiao_jia)
				continue;
			table._cannot_peng_index[i][table._logic.switch_to_card_index(_send_card_data)]++;
		}
...
//						table.operate_remove_discard(table._current_player, table.GRR._discard_count[table._current_player]);
	...
	public boolean handler_operate_card(SCHCPTable_YanZhong table, int seat_index, int operate_code, int operate_card,int luoCode) {
		...
		else if( table._logic.get_action_rank(target_action)<table._logic.get_action_rank(GameConstants.CP_WIK_CHE))
		{
			for(int i = 0; i<table.getTablePlayerNumber();i++)
			{
				 for(int j = 0; j< table._playerStatus[i]._action_count;j++)
