...
public class PHZHandlerGang_CS extends HHHandlerGang<HHTable_CS> {

	@Override
	public void exe(HHTable_CS table) {
		// 用户状态
	 ...
	 */
	@Override
	public boolean handler_operate_card(HHTable_CS table, int seat_index, int operate_code, int operate_card, int luoCode) {
		// 抢杠胡
	 ...
	 * 
	 ***/
	protected boolean exe_gang(HHTable_CS table) {
		...
		if ((_depatch == true) && (table._ti_two_long[_seat_index] == false) && (table._is_xiang_gong[_seat_index] == false)) {
			...
			// }
			// if (table.has_rule(GameConstants.GAME_RULE_DI_HANG_HANG_XI)) {
			// if (hang_hang_xi_count == 7) {
			// chr.opr_or(GameConstants.CHR_HANG_HANG_XI);
			// }
			// }
			// if (table.has_rule(GameConstants.GAME_RULE_DI_TUAN_YUAN)) {
			...
			// chr.opr_or(GameConstants.CHR_TUAN_YUAN);
			// }
			// }
	...
	@Override
	public boolean handler_player_be_in_room(HHTable_CS table, int seat_index) {
