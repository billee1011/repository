...
public class PHZHandlerDispatchFirstCard_AnHua extends HHHandlerDispatchCard<AnHuaHHTable> {
	...
	public void exe(AnHuaHHTable table) {
		...
		// 最后一张牌亮牌
		if (table.has_rule(GameConstants_AnHua.GAME_RULE_BANK_LINAG)) {
		...
		if (table.has_rule(GameConstants_AnHua.GAME_RULE_SAN_TI_WU_KAN)) {
		...
		if (is_hu == false) {
			...
			table.exe_chuli_first_card(_seat_index, GameConstants.WIK_NULL, 1250);
			_send_card_data = 0;
		} else {
			int huCount = 0;
			int huSeat = -1;
