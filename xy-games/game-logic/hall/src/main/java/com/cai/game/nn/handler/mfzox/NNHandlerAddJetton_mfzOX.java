...
public class NNHandlerAddJetton_mfzOX extends NNHandlerAddJetton{
		...
		for(int k = 0; k < GameConstants.GAME_PLAYER_OX;k++)
		{
			...
			if((k == seat_index)&&(table._add_Jetton[k] == 0)&&(table._player_status[k] == true))
			{
				...
				for(int i = 0;i<GameConstants.GAME_PLAYER_OX;i++)
				{
					...
					if((i!=table._cur_banker)&&(table._player_status[i] == true))
					{
						for(int j = 0; j< 4; j++){
							cards1.addItem(table._jetton_info_sever_ox[0][j]);
							table._jetton_info_cur[j] = table._jetton_info_sever_ox[0][j];
						}
						if(table._can_tuizhu_player[i]>0)
						{
							table._jetton_info_cur[4] = table._can_tuizhu_player[i];
							cards1.addItem(table._jetton_info_cur[4]);
						}
