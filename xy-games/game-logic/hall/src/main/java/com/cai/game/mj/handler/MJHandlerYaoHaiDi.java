...
//			_send_card_data=0x03;
		...
		for(int i=0; i< GameConstants.GAME_PLAYER; i++){
			...
			action = table.analyse_chi_hu_card(table.GRR._cards_index[i],table.GRR._weave_items[i], table.GRR._weave_count[i], _send_card_data, table.GRR._chi_hu_rights[i],GameConstants.HU_CARD_TYPE_PAOHU,i);// 自摸
	...
	public boolean handler_operate_card(MJTable table,int seat_index, int operate_code, int operate_card){
		...
		if(operate_code != GameConstants.WIK_ZI_MO){
			...
			// 执行判断
			for (int i = _seat_index; i <GameConstants.GAME_PLAYER; i++){
				int p =i % GameConstants.GAME_PLAYER;
				// 获取动作
