...
public class RoomHandler extends ClientHandler<RoomRequest> {
	...
	private boolean handler_requst_location(int room_id, long account_id, RoomRequest room_rq) {
		...
		Global.getService("").execute(new Runnable() {
			...
			public void run() {
				...
				if (random > 50 || org.apache.commons.lang.StringUtils.isEmpty(sysParamModel.getStr2())
						|| sysParamModel.getStr2().equals("0")) {
	...
	private boolean handler_request_goods(int room_id, long account_id, RoomRequest room_rq) {
		...
		if (room_rq.hasGoodsID()) {
			...
			String desc = String.format("使用道具ID:%d,game_id:%d,玩家id:%d,金币[-%d],金币值变化[%d]-[%d]", goodsid,
					table.getGame_id(), player.getAccount_id(), gmodel.getMoney(), player.getMoney(),
					player.getMoney() - gmodel.getMoney());

			ICenterRMIServer centerRMIServer = SpringService.getBean(ICenterRMIServer.class);
			AddMoneyResultModel addGoldResultModel = centerRMIServer.addAccountMoney(player.getAccount_id(),
					0 - gmodel.getMoney(), false, desc, EMoneyOperateType.USE_PROP);
