...
//		int cards[] = new int[] { 0x02,0x02,0x02,0x02,0x12,0x13,0x22,0x23,0x21,0x41,0x42,0x43,0x52,0x51,0x53,0x71,0x72,0x73};
		...
		// 0x73, 0x71, 0x72, 0x73, 0x71 };
		 ...
		 int cards[] = new int[] { 0x01, 0x02, 0x03, 0x01, 0x02, 0x03, 0x01,
		 0x02, 0x03, 0x71, 0x72, 0x73, 0x71, 0x72,
		 0x73, 0x71, 0x72, 0x73, 0x72 };
	...
	public int analyse_chi_hu_card_fls_twenty(int cards_index[], WeaveItem weaveItems[], int weaveCount, int cur_card,
		...
		if (!isEnough) {
			...
			for (int i = 0; i < analyseItemArray.size(); i++) {
				...
				if (analyseItem.totalScore == 0 && !analyseItem.eyeDui) {
		...
		if(effect_type == GameConstants.EFFECT_ACTION_TYPE_HU) {
			if(is_mj_type(GameConstants.GAME_TYPE_FLS_LX_TWENTY) ){
				...
				for (int i = 0; i < effect_count; i++) {
					if(effect_indexs[i]==GameConstants.CHR_FLS_HONG_HU4 || effect_indexs[i]==GameConstants.CHR_FLS_HONG_HU5||effect_indexs[i]==GameConstants.CHR_FLS_HONG_HU6||effect_indexs[i]==GameConstants.CHR_FLS_HONG_HU7) {
						continue;
					}
					realEffect++;
