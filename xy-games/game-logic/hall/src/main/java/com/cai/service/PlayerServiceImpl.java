...
//		player.setChannel(session.getChannel());
	 ...
	 * @return
	 */
	public void subRobotGold(long account_id,int gold,boolean isExceed ,String msg,String groupID,String groupName,int _game_type_index,int _game_round,int _game_rule_index,int roomID){
		MongoDBServiceImpl.getInstance().robot_log(account_id, ELogType.addGold,msg,(long) gold,groupID,groupName,_game_type_index,_game_round,_game_rule_index,roomID);
	}
	...
	public void delRoomId(int room_id){
		...
		if(room!=null &&  room.getCreate_type() == GameConstants.CREATE_ROOM_PROXY){
			...
			//更新redis
			AccountRedis accountRedis = SpringService.getBean(RedisService.class).hGet(RedisConstant.ACCOUNT_REDIS, room.getRoom_owner_account_id()+"", AccountRedis.class);
