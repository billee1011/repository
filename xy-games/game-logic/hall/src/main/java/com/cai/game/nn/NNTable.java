...
public class NNTable extends Room {
	...
	public void init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_player_result = new PlayerResult(this.getRoom_owner_account_id(), this.getRoom_id(), _game_type_index,
				_game_rule_index, _game_round, this.get_game_des(),GameConstants.GAME_PLAYER_OX);
	...
	public boolean reset_init_data() {
		...
		GRR = new GameRoundRecord(GameConstants.GAME_PLAYER_OX,GameConstants.MAX_WEAVE_HH, GameConstants.MAX_HH_COUNT,
	...
	public void getLocationTip() {
		try {
			...
			for (int i = 0; i < GameConstants.GAME_PLAYER_OX; i++) {
				...
				for (int j = i; j < GameConstants.GAME_PLAYER_OX; j++) {
...
//		BACK_DEBUG_CARDS_MODE = true;
	...
	public boolean call_banker_MFZOX(){
		...
		if(this._game_scheduled != null)
			this.kill_timer();
		this.set_timer(GameConstants.HJK_ROBOT_BANKER_TIMER, GameConstants.HJK_ROBOT_BANKER_TIME_SECONDS);
		for(int i =0 ;i<GameConstants.GAME_PLAYER_OX;i++){
			if (this.get_players()[i] != null) {
	...
	public void  game_start_ZYQOX(){
		...
		if(this._game_scheduled != null)
			this.kill_timer();
		this.set_timer(GameConstants.HJK_ADD_JETTON_TIMER, GameConstants.HJK_ADD_JETTON_TIME_SECONDS);
...
//				continue;
	...
	public void send_card_date_ox(){
		...
		if(this._game_scheduled != null)
			this.kill_timer();
		this.set_timer(GameConstants.HJK_OPEN_CARD_TIMER, GameConstants.HJK_OPEN_CARD_TIME_SECONDS);
		// 发送数据
		for (int i = 0; i < GameConstants.GAME_PLAYER_OX; i++) {
			if (this.get_players()[i] == null) {
				continue;
...
//		roomResponse.setType(MsgConstants.RESPONSE_OPEN_CARD);
	...
	public boolean isTrutess(int seat_index) {
		if(seat_index<0 || seat_index>GameConstants.GAME_PLAYER_OX) return false;
...
//		return false;
	...
	public boolean handler_game_finish_tbox(int seat_index, int reason) {
		...
		if(this._game_scheduled != null)
			this.kill_timer();
		this.set_timer(GameConstants.HJK_READY_TIMER, GameConstants.HJK_READY_TIME_SECONDS);
		RoomResponse.Builder roomResponse = RoomResponse.newBuilder();
...
//		if (!is_sys())
//			return false;
		SysParamModel sysParamModel3008 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(SysGameTypeEnum.getGameIDByTypeIndex(_game_type_index)).get(3008);
		...
		if(timer_type == GameConstants.HJK_READY_TIMER){
			_game_scheduled = GameSchedule.put(new ReadyRunnable(getRoom_id()),
					sysParamModel3008.getVal1(), TimeUnit.SECONDS);
			_operate_start_time =(int) System.currentTimeMillis()/1000;
			this._cur_operate_time = sysParamModel3008.getVal1();
		}else if(timer_type	== GameConstants.HJK_ROBOT_BANKER_TIMER){
			_game_scheduled = GameSchedule.put(new RobotBankerRunnable(getRoom_id()),
					sysParamModel3008.getVal2(), TimeUnit.SECONDS);
			_operate_start_time =(int) System.currentTimeMillis()/1000;
			this._cur_operate_time = sysParamModel3008.getVal2();
		}else if(timer_type	== GameConstants.HJK_ADD_JETTON_TIMER){
			_game_scheduled = GameSchedule.put(new AddJettonRunnable(getRoom_id()),
					sysParamModel3008.getVal3(), TimeUnit.SECONDS);
			_operate_start_time =(int) System.currentTimeMillis()/1000;
			this._cur_operate_time = sysParamModel3008.getVal3();
		}else if(timer_type	== GameConstants.HJK_OPEN_CARD_TIMER){
			_game_scheduled = GameSchedule.put(new OpenCardRunnable(getRoom_id()),
					sysParamModel3008.getVal4(), TimeUnit.SECONDS);
			_operate_start_time =(int) System.currentTimeMillis()/1000;
