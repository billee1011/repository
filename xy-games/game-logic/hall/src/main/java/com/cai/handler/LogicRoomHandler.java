...
public class LogicRoomHandler extends ClientHandler<LogicRoomRequest> {
		...
		if (type == CRATE_ROOM) {
			...
			if (table == null) {
				...
				RoomRedisModel roomRedisModel = SpringService.getBean(RedisService.class).hGet(RedisConstant.ROOM,
						r.getRoomId() + "", RoomRedisModel.class);
			...
			for (Map.Entry<Integer, Room> entry : roomMap.entrySet()) {
				...
				if (entry.getValue().is_sys() && entry.getValue().getPlayerCount() < max_player_num
						&& entry.getValue().getGameTypeIndex() == game_type_index
						&& entry.getValue().getGameRuleIndex() == game_rule_index) {
					if (suitableRoom != null) {
						suitableRoom = entry.getValue().getPlayerCount() > suitableRoom.getPlayerCount()
								? entry.getValue() : suitableRoom;
					} else {
	...
	private boolean handler_player_create_room(LogicRoomRequest r, int type, int room_id) {
		...
		table.handler_create_room(player, type, getMaxNumber(table));
	...
	public static boolean createRoomByBobot(long accountID, int roomID, int game_type_index, int game_rule_index,
...
//		table.handler_create_room(player, GameConstants.CREATE_ROOM_ROBOT, getMaxNumber(game_type_index));
		
		GameSchedule.put(new CreateTimeOutRunnable(roomID), GameConstants.CREATE_ROOM_PROXY_TIME_GAP,
				TimeUnit.MINUTES);
		...
		RoomRedisModel roomRedisModel = SpringService.getBean(RedisService.class).hGet(RedisConstant.ROOM,
				roomID + "", RoomRedisModel.class);
		roomRedisModel.setGameRuleDes(GameDescUtil.getGameDesc(game_type_index, game_rule_index));
		roomRedisModel.setRoomStatus(table._game_status);
		roomRedisModel.setPlayer_max(LogicRoomHandler.getMaxNumber(table));
		roomRedisModel.setGroupID(groupId);
		roomRedisModel.setIsInner(isInner);
		roomRedisModel.setGame_round(game_round);
		redisService.hSet(RedisConstant.ROOM, roomID + "", roomRedisModel);
	...
	private static Room createRoom(int game_type_index) {
		...
		if (game_type_index == GameConstants.GAME_TYPE_FLS_LX
				|| game_type_index == GameConstants.GAME_TYPE_FLS_LX_TWENTY) {
			table = new FLSTable();
		} else if (game_type_index == GameConstants.GAME_TYPE_HH_YX
				|| game_type_index == GameConstants.GAME_TYPE_PHZ_YX||game_type_index == GameConstants.GAME_TYPE_FPHZ_YX) {
			table = new HHTable();
		} else if (game_type_index == GameConstants.GAME_TYPE_SEVER_OX
				|| game_type_index == GameConstants.GAME_TYPE_SZOX || game_type_index == GameConstants.GAME_TYPE_LZOX
				|| game_type_index == GameConstants.GAME_TYPE_ZYQOX || game_type_index == GameConstants.GAME_TYPE_MSZOX
				|| game_type_index == GameConstants.GAME_TYPE_MFZOX
	...
	public static int getMaxNumber(Room table) {
		...
		if(game_type_index==GameConstants.GAME_TYPE_FLS_CS_LX) {
			if(FvMask.has_any(table._game_rule_index, FvMask.mask(GameConstants.GAME_RULE_HUNAN_THREE))) {
				maxNumber = 3;
			}
