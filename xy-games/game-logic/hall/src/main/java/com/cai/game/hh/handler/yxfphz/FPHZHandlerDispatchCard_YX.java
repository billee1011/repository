 ...
 *
 */
public class FPHZHandlerDispatchCard_YX extends HHHandlerDispatchCard {
...
//		 _send_card_data = 0x05;
		...
		for(int i = 0; i<table.getTablePlayerNumber();i++){
			...
			action_hu[i] = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i],
					table.GRR._weave_count[i],i,_seat_index,_send_card_data, chr[i], card_type,hu_xi_chi);// 自摸
	...
	public boolean handler_operate_card(HHTable table, int seat_index, int operate_code, int operate_card,int luoCode) {
		...
		case GameConstants.WIK_LEFT: // 上牌操作
		{
			...
			table.exe_chi_peng(target_player, _seat_index, target_action, target_card,
					GameConstants.CHI_PENG_TYPE_DISPATCH);
			return true;
		}
		...
		case GameConstants.WIK_DDX://吃大
		{
			...
			table.add_lou_weave(luoCode,target_player,target_card,_seat_index,table._lou_weave_item[target_player][5]);
