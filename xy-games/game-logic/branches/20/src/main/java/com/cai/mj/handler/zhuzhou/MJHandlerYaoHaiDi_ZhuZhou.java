	...
	public void exe(MJTable table) {
		...
		for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
			...
			action = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i],
					table.GRR._weave_count[i], _send_card_data, table.GRR._chi_hu_rights[i],
					GameConstants.HU_CARD_TYPE_PAOHU);// 自摸
		...
		if (has_action == false) {
			GameSchedule.put(
					new AddDiscardRunnable(table.getRoom_id(), _seat_index, 1, new int[] { _send_card_data }, true,table.getMaxCount()), 1,
					TimeUnit.SECONDS);
			GameSchedule.put(
					new GameFinishRunnable(table.getRoom_id(), table._banker_select, GameConstants.Game_End_DRAW), 2,
					TimeUnit.SECONDS);
		} else {
					...
					if (playerStatus.has_chi_hu()) {
						if (table.has_rule(GameConstants.GAME_RULE_HUNAN_JIANPAOHU)) {
							if (_seat_index == i) {// 庄家是自摸
								table.exe_jian_pao_hu(i, GameConstants.WIK_ZI_MO, _send_card_data);
							} else {
								// 见炮胡
								table.exe_jian_pao_hu(i, GameConstants.WIK_CHI_HU, _send_card_data);
							}
							return;
						} else {
							table.operate_player_action(i, false);
