	...
	public boolean handler_operate_card(FLSTable table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				table.process_chi_hu_player_operate_fls(i, new int[] { table._playerStatus[i]._operate_card }, 1,
						false);
				table.process_chi_hu_player_score_fls(_seat_index, target_player, target_card, false);

				table.countChiHuTimes(_seat_index,false);
				// 记录
