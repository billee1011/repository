...
public class FLSHandlerPiao_FLS extends FLSHandler {
	...
	public boolean handler_pao_qiang(FLSTable table, int seat_index, int pao, int qiang) {
		...
		int hand_cards[][] = new int[table.getTablePlayerNumber()][GameConstants.MAX_FLS_COUNT];
		// 发送数据
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			roomResponse.setCurrentPlayer(
					table._current_player == GameConstants.INVALID_SEAT ? table._resume_player : table._current_player);
		...
		// 检测听牌
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			table._playerStatus[i]._hu_card_count = table.get_fls_ting_card(table._playerStatus[i]._hu_cards,
					table.GRR._cards_index[i], table.GRR._weave_items[i], table.GRR._weave_count[i]);
	...
	private void player_reconnect(FLSTable table, int seat_index) {
