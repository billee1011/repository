...
public class MJHandlerGang_ZhuZhou_DispatchCard extends MJHandler {
	...
	public void exe(MJTable table) {
			...
			for (int k = 0; k < GameConstants.GAME_PLAYER; k++) {
				...
				if (k == _seat_index) {
					...
					int action = table.analyse_chi_hu_card(table.GRR._cards_index[k], table.GRR._weave_items[k],
							table.GRR._weave_count[k], bu_card, chr, GameConstants.HU_CARD_TYPE_ZIMO);// 自摸
						...
						if (cbActionMask != GameConstants.WIK_NULL) {// 有杠
							...
							for (int gc = 0; gc < m_gangCardResult.cbCardCount; gc++) {
								...
								if (is_ting == true) {
									...
									can_gang = table.is_zhuzhou_ting_card(table.GRR._cards_index[k],
					...
					if (k == chi_seat_index) {// 检查吃
						bAroseAction = table.estimate_gang_zhuzhou_respond(k, _seat_index, bu_card, _double, true);// ,
																													// EstimatKind.EstimatKind_OutCard
					} else {
						bAroseAction = table.estimate_gang_zhuzhou_respond(k, _seat_index, bu_card, _double, false);// ,
																													// EstimatKind.EstimatKind_OutCard
					}
		...
		if (has_action == false) {
			...
			table.exe_dispatch_card(table._current_player, GameConstants.WIK_NULL,
					GameConstants.GANG_CARD_CS_DELAY);
					...
					if (playerStatus.has_chi_hu()) {
						if (table.has_rule(GameConstants.GAME_RULE_HUNAN_JIANPAOHU)) {
							if (i == _seat_index) {// 庄家自摸
								table.exe_jian_pao_hu(i, GameConstants.WIK_ZI_MO, GameConstants.INVALID_VALUE);
							} else {
								// 见炮胡
								table.exe_jian_pao_hu(i, GameConstants.WIK_CHI_HU, GameConstants.INVALID_VALUE);
							}
							return;
						} else {
							table.operate_player_action(i, false);
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_GANG: // 杠牌操作
		{
			...
			if (add_card == -1) {
				table.runnable_add_discard(this._seat_index, GameConstants.ZHUZHOU_GANG_DRAW_COUNT,
						this._gang_card_data.get_cards(), true);
			} else {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			// 下局胡牌的是庄家
			table.set_niao_card(table.GRR._banker_player, GameConstants.INVALID_VALUE, false, 0);// 结束后设置鸟牌
			// 吃牌权位
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.set_niao_card(table.GRR._banker_player, operate_card, false, 0);
			for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
				if ((i == _seat_index) || (table.GRR._chi_hu_rights[i].is_valid() == false)) {
					continue;
				}
