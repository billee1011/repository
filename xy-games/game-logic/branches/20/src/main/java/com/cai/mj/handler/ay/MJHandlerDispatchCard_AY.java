...
public class MJHandlerDispatchCard_AY extends MJHandlerDispatchCard {
	...
	public void exe(MJTable table) {
		...
		if(table.DEBUG_CARDS_MODE)_send_card_data=0x09;
		...
		if((_send_card_data == GameConstants.CARD_CAI_SHENG) || table.GRR._cards_index[_seat_index][34]>0){
			...
			//显示出牌
			table.operate_out_card(_seat_index,1,new int[]{ GameConstants.CARD_CAI_SHENG},GameConstants.OUT_CARD_TYPE_MID,GameConstants.INVALID_SEAT);
			table.runnable_add_discard( _seat_index,  1, new int[]{GameConstants.CARD_CAI_SHENG},false);
			
			//效果
			table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[]{GameConstants.WIK_BU_HUA}, 1, GameConstants.INVALID_SEAT);
		...
		for(int i = 0; i < GameConstants.GAME_PLAYER; i++){
			...
			if(table.GRR._cards_index[i][34]>0){
				// 从牌堆拿出一张牌
				table._send_card_count++;
				int bu_hua = table._repertory_card[table._all_card_len-table.GRR._left_card_count];
				--table.GRR._left_card_count;
				
				
				table.GRR._cards_index[i][34]=0;
				table.GRR._chi_hu_rights[i].hua_count++;//花牌加一
				int send_card_index = table._logic.switch_to_card_index(bu_hua);
				
				table.GRR._cards_index[i][send_card_index]++;
				...
				//显示出牌
				table.operate_out_card(i,1,new int[]{ GameConstants.CARD_CAI_SHENG},GameConstants.OUT_CARD_TYPE_MID,GameConstants.INVALID_SEAT);
				table.runnable_add_discard( i,  1, new int[]{GameConstants.CARD_CAI_SHENG},false);
				
				//效果
				table.operate_effect_action(i, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[]{GameConstants.WIK_BU_HUA}, 1, GameConstants.INVALID_SEAT);
			}
		}
		...
		if(table._playerStatus[_seat_index].is_bao_ting()==true){
			...
			for(int i=0; i < table._playerStatus[_seat_index]._hu_card_count;i++){
				if(table._playerStatus[_seat_index]._hu_cards[i]==_send_card_data){
					action = table.analyse_chi_hu_card_ay(_seat_index,table.GRR._cards_index[_seat_index],
			...
			for( int i = 0; i < GameConstants.MAX_INDEX-GameConstants.CARD_HUA_COUNT; i++ )
			{
				...
				if(count>0){
					...
					//检查打出哪一张牌后可以听牌
					table._playerStatus[_seat_index]._hu_out_card_ting_count[ting_count] = table.get_ay_ting_card(_seat_index,table._playerStatus[_seat_index]._hu_out_cards[ting_count], table.GRR._cards_index[_seat_index], 
				...
				for(int i= 0; i < m_gangCardResult.cbCardCount; i++){
					if((m_gangCardResult.type[i] != GameConstants.GANG_TYPE_AN_GANG) ||
							(table._playerStatus[_seat_index].is_bao_ting() == false) ||
							 (table.check_gang_huan_zhang(GameConstants.GAME_TYPE_HENAN_AY,_seat_index,m_gangCardResult.cbCardData[i])==false)){
						has_gang=true;
						//加上杠
	...
	public boolean handler_operate_card(MJTable table,int seat_index, int operate_code, int operate_card){
		...
		switch (operate_code) {
			...
			case GameConstants.WIK_ZI_MO: // 自摸
			{
				...
				table.process_chi_hu_player_score_ay(_seat_index,_seat_index,operate_card, true);
