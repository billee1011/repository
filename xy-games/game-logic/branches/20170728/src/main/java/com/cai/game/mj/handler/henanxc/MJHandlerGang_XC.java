...
public class MJHandlerGang_XC extends MJHandlerGang<MJTable> {
	...
	public void exe(MJTable table) {
		...
		if(table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU)) {//可炮胡的时候 才能抢杠
	...
	protected boolean exe_gang(MJTable table){
		...
		if(GameConstants.GANG_TYPE_AN_GANG == _type){
			for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
				...
				//暗杠每人2分
				table.GRR._gang_score[_seat_index].scores[cbGangIndex][i] = -1*table.base_score;// 暗杠，其他玩家扣分
				table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += 1*table.base_score;//一共加分
			}
			
			table._player_result.an_gang_count[_seat_index]++;
		}else if(GameConstants.GANG_TYPE_JIE_GANG == _type){
			//放杠1分
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] = table.base_score;// m_pGameServiceOption->lCellScore*2;//配置参数乘
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_provide_player] =  -1*table.base_score;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
			
			table._player_result.ming_gang_count[_seat_index]++;
		}else if(GameConstants.GANG_TYPE_ADD_GANG == _type){
			//放杠1分
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] = table.base_score;// m_pGameServiceOption->lCellScore*2;//配置参数乘
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][table.GRR._weave_items[_seat_index][cbWeaveIndex].provide_player] =  -1*table.base_score;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
...
//			}
		...
		if(table._playerStatus[_seat_index].is_bao_ting()){
			table._playerStatus[_seat_index]._hu_card_count = table.get_xc_ting_card(_seat_index,
	...
	public boolean handler_player_be_in_room(MJTable table,int seat_index) {
		...
		for(int j=0; j < hand_card_count; j++){
			if( table._logic.is_magic_card(hand_cards[j])){
				hand_cards[j]+=GameConstants.CARD_ESPECIAL_TYPE_HUN;//将癞子转下
			}
		}
