...
import com.cai.game.btz.handler.BTZHandlerDispatchCard;
import com.cai.game.btz.handler.BTZHandlerFinish;
import com.cai.game.btz.handler.BTZHandlerOpenCard;
import com.cai.game.btz.handler.qzbtz.BTZHandlerAddJetton_qzbtz;
import com.cai.game.btz.handler.qzbtz.BTZHandlerCallBanker_qzbtz;
import com.cai.game.btz.handler.qzbtz.BTZHandlerOpenCard_qzbtz;
import com.cai.game.btz.handler.serverox.BTZHandlerAddJetton_server;
import com.cai.game.btz.handler.serverox.BTZHandlerOpenCard_server;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class BTZTable extends Room {
	/**
	 * 
	 */
	private static final long serialVersionUID = 7060061356475703643L;

	private static Logger logger = Logger.getLogger(BTZTable.class);
	...
	public BTZGameLogic _logic = null;
	...
	public BTZHandler _handler;

	public BTZHandlerDispatchCard _handler_dispath_card;
	public BTZHandlerCallBanker _handler_Call_banker;
	public BTZHandlerAddJetton _handler_add_jetton;
	public BTZHandlerOpenCard _handler_open_card;

	// public HHHandlerDispatchLastCard_YX _handler_dispath_last_card;

	public BTZHandlerFinish _handler_finish; // 结束


	public BTZTable() {
		super(RoomType.OX,GameConstants.GAME_PLAYER_SIX);

		_logic = new BTZGameLogic();
		// 玩家
		for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
			get_players()[i] = null;

		}
		_game_status = GameConstants.GS_OX_FREE;
		// 游戏变量
		_player_ready = new int[GameConstants.GAME_PLAYER_BTZ];
		_player_open_less = new int[GameConstants.GAME_PLAYER_BTZ];
		for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
			_player_ready[i] = 0;
		...
		_cur_game_timer = 0;
		_call_banker = new int[GameConstants.GAME_PLAYER_BTZ]; // 用户是否叫庄
		_add_Jetton = new int[GameConstants.GAME_PLAYER_BTZ]; // 用户下注
		_open_card = new boolean[GameConstants.GAME_PLAYER_BTZ]; // 用户摊牌
		_cur_call_banker = 0;
		_player_status = new boolean[GameConstants.GAME_PLAYER_BTZ];
		_ping_Player_ox = new boolean[GameConstants.GAME_PLAYER_BTZ];
		_win_player_ox = new boolean[GameConstants.GAME_PLAYER_BTZ];
		_call_banker_info = new int[5];
		_can_tuizhu_player = new int[GameConstants.GAME_PLAYER_BTZ];
		_card_type_ox = new int[GameConstants.GAME_PLAYER_BTZ];
		for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
			_call_banker[i] = -1;
			_add_Jetton[i] = 0;
			_open_card[i] = false;
	...
	public void init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		_jetton_info_cur = new int[GameConstants.GAME_PLAYER_BTZ][5];
		for (int i = 0; i < 5; i++) {
		...
		_player_result = new PlayerResult(this.getRoom_owner_account_id(), this.getRoom_id(), _game_type_index,
				_game_rule_index, _game_round, this.get_game_des(), GameConstants.GAME_PLAYER_BTZ);

		
		if (is_mj_type(GameConstants.GAME_TYPE_BTZ_YY) ) {
			_handler_add_jetton = new BTZHandlerAddJetton_server();
			_handler_open_card = new BTZHandlerOpenCard_server();
		}
		if (is_mj_type(GameConstants.GAME_TYPE_BTZ_YY) ) {
			_handler_add_jetton = new BTZHandlerAddJetton_qzbtz();
			_handler_open_card = new BTZHandlerOpenCard_qzbtz();
			_handler_Call_banker = new BTZHandlerCallBanker_qzbtz();
		}
	...
	public boolean reset_init_data() {
		...
		GRR = new GameRoundRecord(GameConstants.GAME_PLAYER_BTZ, GameConstants.MAX_WEAVE_HH, GameConstants.MAX_HH_COUNT,
		...
		// 新建
		_playerStatus = new PlayerStatus[GameConstants.GAME_PLAYER_BTZ];
		istrustee = new boolean[GameConstants.GAME_PLAYER_BTZ];
		for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
	...
	public boolean handler_game_start() {
		...
		_repertory_card = new int[GameConstants.CARD_COUNT_BTZ];
		shuffle(_repertory_card, GameConstants.CARD_TATA_BTZ);
	...
	public boolean call_banker_MFZOX() {
		...
		for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
			...
			for (int k = 0; k < GameConstants.GAME_PLAYER_BTZ; k++) {
...
//		this._handler = _handler_Call_banker;
	...
	public void game_start_ZYQOX() {
		...
		for (int j = _call_banker_info.length - 1; j >= 0; j--) {
			int chairID[] = new int[GameConstants.GAME_PLAYER_BTZ];
			int chair_count = 0;
			for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
	...
	public void open_card_ox(int seat_index) {
		...
		if (has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_ONE))
			times = _logic.get_times_two(GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT,
					_game_rule_index);
		if (has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_TWO))
			times = _logic.get_times_one(GRR._cards_data[seat_index], GameConstants.OX_MAX_CARD_COUNT,
					_game_rule_index);
	...
	public void process_tbox_calulate_end() {
		...
		for (int i = _win_player_oxtb + 1; i < GameConstants.GAME_PLAYER_BTZ; i++) {
			if (this._player_status[i] != true)
				continue;
			boolean first_ox = _logic.get_ox_card(GRR._cards_data[_win_player_oxtb], GameConstants.OX_MAX_CARD_COUNT,
					_game_rule_index);
			boolean next_ox = _logic.get_ox_card(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
			boolean action = _logic.compare_card(GRR._cards_data[_win_player_oxtb], GRR._cards_data[i],
	...
	public void process_ox_calulate_end() {
		...
		for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
			...
			if (this._player_status[i] != true)
				continue;
			boolean first_ox = _logic.get_ox_card(GRR._cards_data[this._cur_banker], GameConstants.OX_MAX_CARD_COUNT,
					_game_rule_index);
			boolean next_ox = _logic.get_ox_card(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
			boolean action = _logic.compare_card(GRR._cards_data[this._cur_banker], GRR._cards_data[i],
					GameConstants.OX_MAX_CARD_COUNT, first_ox, next_ox, _game_rule_index);
			//获取点数
	...
	private void countCardType(int card_type, int seat_index) {
		try {
			...
			if (card_type == GameConstants.OX_FIVE_KING) {
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.oxwuhuaox, "", 0,
						0l, this.getRoom_id());
			}

			if (card_type == GameConstants.OX_BOOM) {
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.oxboomox, "", 0, 0l,
						this.getRoom_id());
			}

			if (card_type == GameConstants.OX_WUXIAONIU) {
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.oxwuxiaoox, "", 0,
						0l, this.getRoom_id());
			}
	...
	public void process_chi_calulate_score() {
		...
		if (has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_ONE))
			calculate_score = _logic.get_times_two(GRR._cards_data[_win_player_oxtb], GameConstants.OX_MAX_CARD_COUNT,
					_game_rule_index);
		if (has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_TWO))
			calculate_score = _logic.get_times_one(GRR._cards_data[_win_player_oxtb], GameConstants.OX_MAX_CARD_COUNT,
					_game_rule_index);
		...
		for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
			...
			countCardType(card_type, card_type);
		}
		////////////////////////////////////////////////////// 自摸 算分
		for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
	...
	public void process_chi_calulate_score_ox() {
		...
		for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
			...
			if (_win_player_ox[i] == true) {
				if (has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_ONE))
					calculate_score = _logic.get_times_two(GRR._cards_data[_win_player_oxtb],
							GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
				if (has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_TWO))
					calculate_score = _logic.get_times_one(GRR._cards_data[_win_player_oxtb],
							GameConstants.OX_MAX_CARD_COUNT, _game_rule_index);
				lChiHuScore = calculate_score * _banker_times * _add_Jetton[i];
			} else {

				if (has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_ONE))
					calculate_score = _logic.get_times_two(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT,
							_game_rule_index);
				if (has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_TWO))
					calculate_score = _logic.get_times_one(GRR._cards_data[i], GameConstants.OX_MAX_CARD_COUNT,
							_game_rule_index);
				lChiHuScore = -calculate_score * _banker_times * _add_Jetton[i];
			}
			...
			this.log_error("用户：" + this.get_players()[this._cur_banker].getNick_name() + "分数 "
					+ GRR._game_score[this._cur_banker]);
		}
	...
	public boolean isTrutess(int seat_index) {
		if (seat_index < 0 || seat_index > GameConstants.GAME_PLAYER_BTZ)
			return false;
	...
	private void shuffle(int repertory_card[], int mj_cards[]) {
		...
		for (int i = 0; i < GameConstants.GAME_PLAYER_BTZ; i++) {
			for (int j = 0; j < GameConstants.BTZ_MAX_CARD_COUNT; j++) {
				GRR._cards_data[i][j] = repertory_card[i * GameConstants.BTZ_MAX_CARD_COUNT + j];
			}
		}
		// test_cards();
		// 记录初始牌型
...
//		 {0x39,0x11,0x1C,0x3D,0x28,0x23,0x37,0x1d,0x14,0x04,0x1b,0x0c,0x3d,0x03,0x13,0x1b,0x0c,0x3d,0x03,0x13,
//				 0x1b,0x0c,0x3d,0x03,0x13,0x1b,0x0c,0x3d,0x03,0x13};
//		 testRealyCard(realyCards);
		...
		if (BACK_DEBUG_CARDS_MODE) {
			if (debug_my_cards != null) {
				if (debug_my_cards.length > GameConstants.BTZ_MAX_CARD_COUNT) {
	...
	public boolean handler_game_finish_tbox(int seat_index, int reason) {
		...
		if (count == 0) {
			count = GameConstants.GAME_PLAYER_BTZ;
		}
		for (int i = 0; i < count; i++) {
			_player_ready[i] = 0;
		}
		...
		if (reason == GameConstants.Game_End_RELEASE_PLAY_TIME_OUT
				|| reason == GameConstants.Game_End_RELEASE_WAIT_TIME_OUT) {
			for (int j = 0; j < GameConstants.GAME_PLAYER_BTZ; j++) {
	...
	public boolean handler_enter_room(Player player) {
		...
		if (has_rule(GameConstants.GAME_RULE_START_FORBID_JOIN)) {
...
//		}
	...
	public boolean handler_requst_open_less(Player player, boolean openThree) {
		...
		// 牌桌不是3人
		if (count != (GameConstants.GAME_PLAYER_BTZ - 1)) {
			return false;
		}
		...
		if (openLess == readys && readys == GameConstants.GAME_PLAYER_BTZ - 1) {
			playerNumber = readys;
			handler_game_start();
	...
	public boolean handler_release_room(Player player, int opr_code) {
		...
		if (playerNumber == 0) {
			playerNumber = GameConstants.GAME_PLAYER_BTZ;
		}
...
//			}
	...
	public int getTablePlayerNumber() {
		...
		// }
		return GameConstants.GAME_PLAYER_BTZ;
	}
	...
	public boolean refresh_room_redis_data(int type, boolean notifyRedis) {
		if (type == GameConstants.PROXY_ROOM_UPDATE) {
			...
			roomRedisModel.setGameRuleDes(this.get_game_des());
			roomRedisModel.setRoomStatus(this._game_status);
			roomRedisModel.setPlayer_max(GameConstants.GAME_PLAYER_BTZ);
	...
	private boolean kou_dou_aa(Player cur_player, int seat_index, boolean create_room) {
		...
		// 判断房卡
		SysParamModel sysParamModel1007 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id)
				.get(1007);
		SysParamModel sysParamModel1008 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id)
				.get(1008);
		SysParamModel sysParamModel1009 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id)
				.get(1009);
		int check_gold = 0;
	...
	public boolean huan_dou_aa(int seat_index) {
		if (this._player_status[seat_index] == false) {
			...
			buf.append("开局失败" + ":" + this.getRoom_id()).append("game_id:" + this.getGame_id())
					...
					.append(",房主:" + this.getRoom_owner_account_id()).append(",豆+:" + cost_dou);
			// 把豆还给玩家
			AddGoldResultModel addresult = PlayerServiceImpl.getInstance().addGold(
					this.get_players()[seat_index].getAccount_id(), cost_dou, false, buf.toString(),
	...
	public boolean set_timer(int timer_type, int time, boolean makeDBtimer) {
		...
		_cur_game_timer = timer_type;
		if (timer_type == GameConstants.HJK_READY_TIMER) {
			_game_scheduled = GameSchedule.put(new ReadyRunnable(getRoom_id()), sysParamModel3008.getVal1(),
					TimeUnit.SECONDS);
			_operate_start_time = (int) (System.currentTimeMillis() / 1000L);
			this._cur_operate_time = sysParamModel3008.getVal1();
		} else if (timer_type == GameConstants.HJK_ROBOT_BANKER_TIMER) {
			_game_scheduled = GameSchedule.put(new RobotBankerRunnable(getRoom_id()), sysParamModel3008.getVal2(),
					TimeUnit.SECONDS);
			_operate_start_time = (int) (System.currentTimeMillis() / 1000L);
			this._cur_operate_time = sysParamModel3008.getVal2();
		} else if (timer_type == GameConstants.HJK_ADD_JETTON_TIMER) {
			_game_scheduled = GameSchedule.put(new AddJettonRunnable(getRoom_id()), sysParamModel3008.getVal3(),
					TimeUnit.SECONDS);
			_operate_start_time = (int) (System.currentTimeMillis() / 1000L);
			this._cur_operate_time = sysParamModel3008.getVal3();
		} else if (timer_type == GameConstants.HJK_OPEN_CARD_TIMER) {
			_game_scheduled = GameSchedule.put(new OpenCardRunnable(getRoom_id()), sysParamModel3008.getVal4(),
					TimeUnit.SECONDS);
			_operate_start_time = (int) (System.currentTimeMillis() / 1000L);
