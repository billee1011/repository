...
public class THKHandlerOutCardOperate_HY extends HHHandlerOutCardOperate {
	@Override
	public void exe(HHTable table) {
		...
		for(int i = 0; i<table.getTablePlayerNumber();i++){
			...
			action_pao[i] = table.estimate_player_respond_xiao(i,_out_card_player ,_out_card_data, pao_type[i],true);
			...
			if(i==_out_card_player) continue;
			 if((action_pao[i] != GameConstants.WIK_NULL)&& (bHupai == 0)){
				 int pao_type_out[] = new int[1];
					ti_pao = table.estimate_player_respond_xiao(i,_out_card_player ,_out_card_data, pao_type_out,false);
...
//		}
		...
		case GameConstants.WIK_PAO: // 杠牌操作
		{
			...
			int pao_type[] =  new int[1];
			int action = table.estimate_player_respond_xiao(target_player,this._out_card_player ,operate_card, pao_type,true);	
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_thk(target_player, this._out_card_player, operate_card, false);
