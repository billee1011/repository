...
public class MJHandlerGang_HuNan_ChangDe extends MJHandlerGang<MJTable_HuNan_ChangDe> {
	...
	public boolean handler_operate_card(MJTable_HuNan_ChangDe table, int seat_index, int operate_code,
		...
		if (playerStatus.has_action() == false) { // 是否已经响应
			table.log_player_error(seat_index, "出牌,玩家操作已失效");
			return false;
		}

		if (playerStatus.is_respone()) { // 是否已经响应
		...
		// 吃胡等待--因为胡牌的等级是一样的，可以多人抢杠，看看是不是还有能胡的
		for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
		...
		if (jie_pao_count > 0) {
			...
			if (jie_pao_count > 1) {
				...
				if (GameDescUtil.has_rule(table.getGameRuleIndexEx(),
					...
					table.set_niao_card(_provide_player, tmpCard, true, add_niao);
				} else {
					table.set_niao_card(_provide_player, GameConstants.INVALID_VALUE, true, add_niao);
				}

				table._banker_select = _provide_player;
			...
			table.process_chi_hu_player_score(target_player, _seat_index, _center_card, 1); // 抢杠胡算分
