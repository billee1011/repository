...
public class FLSHandlerOutCardOperate_LX_Twenty extends FLSHandlerOutCardOperate {
	@Override
	public void exe(FLSTable table) {
		...
		// 河南麻将,检查听牌
		table._playerStatus[_out_card_player]._hu_card_count = table.get_fls_ting_card_twenty(
		...
		// 玩家出牌 响应判断,是否有吃碰杠补胡
		boolean bAroseAction = table.estimate_player_out_card_respond_fls_twenty(_out_card_player, _out_card_data);// ,
...
//		int target_card = table._playerStatus[target_player]._operate_card;
		...
		case GameConstants.WIK_NULL: {
			...
			// 用户切换
			int _current_player = table._current_player = (_out_card_player + table.getTablePlayerNumber() + 1)
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			table.process_chi_hu_player_score_fls_twenty(target_player, _out_card_player, _out_card_data, false);

			// 记录
			table._player_result.dian_pao_count[_out_card_player]++;
			table.countChiHuTimesTwenty(target_player, false);
