...
public class PDKHandlerOutCardOperate_LZ extends PDKHandlerOutCardOperate {
	...
	public void exe(PDKTable table) {
		...
		//排列变换扑克
		table._logic.sort_card_date_list_by_type(table._turn_out_change_card_data, table._turn_out_card_count, cbCardType);
		//刷新玩家手牌
	...
	public void no_out_card(PDKTable table){
		if(table.has_rule(GameConstants.GAME_RULE_BI_XU_GUAN) && 
			table._logic.SearchOutCardLaiZi(table.GRR._cards_data[_out_card_player],table.GRR._card_count[_out_card_player],table._turn_out_card_data,
					table._turn_out_card_count,table._turn_out_change_card_data)){
			return;
		}
		...
		if(table._current_player== table._out_card_player){
			//炸弹分
			int cbCardType=table._logic.GetCardType(table._turn_out_change_card_data,table._turn_out_card_count,table._turn_out_card_data);
			if(cbCardType == GameConstants.PDK_CT_ERROR){
				return;
			}
			if(cbCardType == GameConstants.PDK_CT_BOMB_CARD){
	...
	public int adjust_out_card_right(PDKTable table){
		...
		if(table._turn_out_card_count>0){
			if(!table._logic.CompareCardLaizi(table._turn_out_change_card_data, change_data, table._turn_out_card_count, _out_card_count,_out_type)){
				return GameConstants.PDK_CT_ERROR;
			}
		}
