...
public class FLSHandlerOutCardOperate_LXDP extends FLSHandlerOutCardOperate {
	@Override
	public void exe(FLSTable table) {
		...
		// 河南麻将,检查听牌
		table._playerStatus[_out_card_player]._hu_card_count = table.get_fls_ting_card_dp(
...
/*		if (ting_count > 0) {
		...
		// 玩家出牌 响应判断,是否有吃碰杠补胡
		boolean bAroseAction = table.estimate_player_out_card_respond_fls_dp(_out_card_player, _out_card_data);// ,
		// 显示出牌
		table.operate_out_card(_out_card_player, 1, new int[] { _out_card_data }, GameConstants.OUT_CARD_TYPE_MID,
				GameConstants.INVALID_SEAT,bAroseAction);																											// EstimatKind.EstimatKind_OutCard
		if (ting_count > 0) {
			table.operate_chi_hu_cards(_out_card_player, ting_count, ting_cards);
		} else {
			ting_cards[0] = 0;
...
//		int target_card = table._playerStatus[target_player]._operate_card;
		...
		case GameConstants.WIK_RIGHT: // 上牌操作
		{
			...
			boolean normalEat = table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2);
			if(!normalEat) {
				cbRemoveCard = new int[] { _out_card_data - 1, _out_card_data + 1 };
				normalEat = table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2);
				if (!normalEat) {
					cbRemoveCard = new int[] { _out_card_data - 2, _out_card_data + 1 };
					normalEat = table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2);
					if(!normalEat) {
						table.log_player_error(seat_index, "吃牌删除出错");
						return false;
		...
		{
			// 删除扑克
			int cbRemoveCard[] = new int[] { _out_card_data - 1, _out_card_data + 1 };
			boolean normalEat = table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2);
			if(!normalEat) {
				cbRemoveCard = new int[] { _out_card_data - 1, _out_card_data + 2 };
				normalEat = table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2);
				if (!normalEat) {
					cbRemoveCard = new int[] { _out_card_data +1, _out_card_data + 2 };
					normalEat = table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2);
		...
		{
			// 是否有抢杠胡
			table.exe_gang(target_player, _out_card_player, _out_card_data, target_action,
