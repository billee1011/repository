...
public class FLSHandlerGang_LXDP_DispatchCard extends FLSHandlerGang_LX_DispatchCard {
	...
	public FLSHandlerGang_LXDP_DispatchCard() {
...
//						chr.opr_or(GameConstants.CHR_FLS_GANGSHANGHUA);
						...
						if (cbActionMask != GameConstants.WIK_NULL) {// 有杠
							...
							for (int gc = 0; gc < m_gangCardResult.cbCardCount; gc++) {
								...
								if (is_ting == true) {
									...
									can_gang = table.is_fls_ting_card_dp(table.GRR._cards_index[k],
					...
					if (k == chi_seat_index) {
						bAroseAction = table.estimate_gang_fls_respond_dp(k, _seat_index, bu_card, _double, true);// ,
																												// EstimatKind.EstimatKind_OutCard
					} else {
						bAroseAction = table.estimate_gang_fls_respond_dp(k, _seat_index, bu_card, _double, false);// ,
																												// EstimatKind.EstimatKind_OutCard
					}
	...
	public boolean handler_operate_card(FLSTable table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_LEFT: // 上牌操作
		{
			...
			boolean normalEat = table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2);
			if(!normalEat) {
				cbRemoveCard = new int[] { target_card + 1, target_card + 3 };
				normalEat = table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2);
				if (!normalEat) {
			...
			copy[copy.length-1]=target_card;
		...
		case GameConstants.WIK_RIGHT: // 上牌操作
		{
			...
			boolean normalEat = table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2);
			if(!normalEat) {
				cbRemoveCard = new int[] { target_card - 1, target_card + 1 };
				normalEat = table._logic.remove_cards_by_index(table.GRR._cards_index[target_player], cbRemoveCard, 2);
				if (!normalEat) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			table.process_chi_hu_player_score_fls(_seat_index, _seat_index, target_card, true);
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				...
				table.process_chi_hu_player_operate_fls(i, new int[] { target_card}, 1,
						false);
				table.process_chi_hu_player_score_fls(i, _seat_index, target_card, false);
...
//				table._player_result.da_hu_jie_pao[i]++;
	...
	public boolean handler_player_be_in_room(FLSTable table, int seat_index) {
		...
		table.operate_effect_action(_seat_index, GameConstants.EFFECT_ACTION_TYPE_ACTION, 1,
				new long[] { GameConstants.WIK_GANG }, 1, seat_index,0,true,false);
	...
	public boolean handler_be_set_trustee(FLSTable table, int seat_index){
		...
		if(curPlayerStatus.has_action()){
			...
			if(curPlayerStatus.has_zi_mo()){
				table.exe_jian_pao_hu(seat_index, GameConstants.WIK_ZI_MO, this._gang_card_data.get_cards()[0]);
			}else if(curPlayerStatus.has_chi_hu()){
				table.exe_jian_pao_hu(seat_index, GameConstants.WIK_CHI_HU, this._gang_card_data.get_cards()[0]);
			}else{
			...
			int send_index = table._logic.switch_to_card_index(this._gang_card_data.get_cards()[0]);
			if(send_index != GameConstants.MAX_FLS_INDEX_DP && table.GRR._cards_index[seat_index][send_index] > 0){//手牌有
				out_card = this._gang_card_data.get_cards()[0];
			}else{
