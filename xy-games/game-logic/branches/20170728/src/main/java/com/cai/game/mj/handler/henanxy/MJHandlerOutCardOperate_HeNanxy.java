...
public class MJHandlerOutCardOperate_HeNanxy extends MJHandlerOutCardOperate<MJTable> {
	...
	public void exe(MJTable table) {
		...
		if(table.GRR._xianchu_count[_out_card_player]>0){
			for(int i=0;i<table.GRR._xianchu_count[_out_card_player];i++){
				if(table.GRR._xianchu_cards[_out_card_player][i] == _out_card_data){
					if(i == table.GRR._xianchu_count[_out_card_player]-1){
						table.GRR._xianchu_cards[_out_card_player][i]=GameConstants.INVALID_CARD;
					}else{
						table.GRR._xianchu_cards[_out_card_player][i]=table.GRR._xianchu_cards[_out_card_player][i+1];
					}
		...
		table.GRR._xianchu_count[_out_card_player]--;
		...
		if(table.GRR._xianchu_count[_out_card_player]>0){
			...
			for(int i=0;i<GameConstants.MAX_COUNT;i++){
				xianchu_cards[i]=table.GRR._xianchu_cards[_out_card_player][i];
			}
			for(int i=GameConstants.MAX_COUNT-1;i>=0;i--){
				boolean bxianchu=false;
				for(int j=0;j<table.GRR._xianchu_count[_out_card_player];j++){
...
//		}
		...
		//信阳麻将,检查听牌
		table._playerStatus[_out_card_player]._hu_card_count = table.get_henanxy_ting_card(table._playerStatus[_out_card_player]._hu_cards, table.GRR._cards_index[_out_card_player], 
		...
		// 玩家出牌 响应判断,是否有吃碰杠补胡
		boolean bAroseAction = table.estimate_player_out_card_respond_henan_xy(_out_card_player, _out_card_data);//, EstimatKind.EstimatKind_OutCard
...
//		if (operate_code == MJGameConstants.WIK_NULL && table._logic.is_magic_card(operate_card)) {
...
//					// 庄家切换
//					table._banker_select =  (table.GRR._banker_player + MJtable.getTablePlayerNumber() + 1) % MJtable.getTablePlayerNumber();
//				}else{
...
//				}
				...
				table.process_chi_hu_player_score_henanxy(target_player,_out_card_player, _out_card_data,false);
