...
public class PDK_LZ_Table extends PDKTable {
	...
	public boolean handler_game_start() {
		...
		if(this.has_rule(GameConstants.GAME_RULE_FIFTEEN_COUNT)){
			int bPosition = (int) (RandomUtil.getRandomNumber(Integer.MAX_VALUE) % GameConstants.CARD_COUNT_PDK_FIFTEEN);
			this._logic._laizi = GameConstants.CARD_DATA_PDK_FIFTEEN[bPosition];
		}else{
			int bPosition = (int) (RandomUtil.getRandomNumber(Integer.MAX_VALUE) % GameConstants.CARD_COUNT_PDK_JD);
			this._logic._laizi = GameConstants.CARD_DATA_PDK_JD[bPosition];
		}
		this._logic._laizi=0x03;
		//
		if(this.has_rule(GameConstants.GAME_RULE_FIFTEEN_COUNT)){
...
//					return i;
//		}
//		if(this.has_rule(GameConstants.GAME_RULE_TWO_PLAY) && seat == GameConstants.INVALID_SEAT){
//			//没有黑桃三，牌最小的玩家出
...
//		}
	...
	private void test_cards() {
		int cards[] = new int[] { 0x03,0x23,0x0D,0x0D,0x0D,0x0C,0x0C,0x0C,0x0B,0x0A,0x09,
