...
public class MJHandlerOutCardOperate_lygc extends MJHandlerOutCardOperate<MJTable> {
	...
	public void exe(MJTable table) {
		...
		for (int j = 0; j < hand_card_count; j++) {
			if (table._logic.is_ci_card(table._logic.switch_to_card_index(cards[j])) 
					&& !table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU)) {
		...
		for(int i = 0; i < table.getTablePlayerNumber(); i++){
			...
			for(int m = 0 ; m < weave_count; m++ ){
				...
				weaves[m].provide_player = table.GRR._weave_items[i][m].provide_player
			...
			if(flag){
				/*// 刷新手牌
				int cards_other[] = new int[GameConstants.MAX_COUNT];
				// 刷新其他玩家手牌
				int hand_card_count_other = table._logic.switch_to_cards_data(table.GRR._cards_index[i], cards_other);
				// 次牌
				for (int j = 0; j < hand_card_count_other; j++) {
					if (table._logic.is_ci_card(table._logic.switch_to_card_index(cards_other[j])) 
							&& !table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU)) {
						cards_other[j] += GameConstants.CARD_ESPECIAL_TYPE_CI;
					}
				}*/
				//刷新其他玩家牌手牌
		...
		if(!table.has_rule(GameConstants.GAME_RULE_HENAN_YCI)){//WalkerGeek 硬次玩法不听牌
			// 河南麻将,检查听牌
			table._playerStatus[_out_card_player]._hu_card_count = table.get_henan_ting_card_lygc(
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_LYGC_CI: // 次操作
		{
			...
			table.exe_gang(target_player, _out_card_player, target_card, GameConstants.WIK_GANG,
					GameConstants.GANG_TYPE_JIE_GANG, false, false);
			return true;
		}
		case GameConstants.WIK_NULL: {
			...
			int real_card = _out_card_data;
			if ( table._logic.is_ci_card(table._logic.switch_to_card_index(_out_card_data))) {// 癞子替换
				real_card += GameConstants.CARD_ESPECIAL_TYPE_CI;
			}
			// 加到牌堆 没有人要
		...
		case GameConstants.WIK_CHI_HU: // 胡
		{
			...
			// MJtable.getTablePlayerNumber() + 1) %
			// MJtable.getTablePlayerNumber();
			// }else{
			// table._banker_select = table.GRR._banker_player;
			// }
			...
			table.process_chi_hu_player_score_henan_lygc(target_player, _out_card_player, _out_card_data,
					GameConstants.HU_CARD_TYPE_PAOHU,true);
			// table.process_chi_hu_player_score_henan(target_player,_out_card_player,
			// _out_card_data,false);
	...
	public boolean handler_player_be_in_room(MJTable table, int seat_index) {
		...
		int real_card = _out_card_data;
		if( table._logic.is_ci_card(table._logic.switch_to_card_index(_out_card_data)) ){
			real_card+=GameConstants.CARD_ESPECIAL_TYPE_CI;
		}
		// 出牌
		table.operate_out_card(_out_card_player, 1, new int[] { real_card }, GameConstants.OUT_CARD_TYPE_MID,
