...
public class MJHandlerOutCardOperate_HeNan_smx extends MJHandlerOutCardOperate<MJTable> {
	...
	public void exe(MJTable table) {
		...
		if(table._playerStatus[_current_player]._biaoyan_count>0){
			if(table.isbiaoyancontinue(table.GRR._cards_index[_out_card_player],
					table.GRR._weave_items[_out_card_player], table.GRR._weave_count[_out_card_player],_out_card_data)){
				table._playerStatus[_current_player]._biaoyan_count++;
			}
			else{
				table._playerStatus[_current_player]._biaoyan_count=0;
			}
		}
...
//		}
		...
		//河南麻将,检查听牌
		table._playerStatus[_out_card_player]._hu_card_count = table.get_henan_smx_ting_card(table._playerStatus[_out_card_player]._hu_cards, table.GRR._cards_index[_out_card_player], 
		...
		// 玩家出牌 响应判断,是否有吃碰杠补胡
		boolean bAroseAction = table.estimate_player_out_card_respond_henan_smx(_out_card_player, _out_card_data);//, EstimatKind.EstimatKind_OutCard
...
//				}
				...
				table.process_chi_hu_player_score_henan_smx(target_player,_out_card_player, _out_card_data,false);
