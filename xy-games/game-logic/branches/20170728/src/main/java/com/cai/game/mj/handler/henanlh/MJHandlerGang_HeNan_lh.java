package com.cai.game.mj.handler.henanlh;
...
public class MJHandlerGang_HeNan_lh extends MJHandlerGang<MJTable> {
	@Override
	public void exe(MJTable table) {
		...
		if (table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU)) {// 可炮胡的时候
																			// 才能抢杆
			bAroseAction = table.estimate_gang_respond_henan_lh(_seat_index, _center_card);
		}
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		if (jie_pao_count > 0) {
			...
			table.process_chi_hu_player_score_he_nan_lh(target_player, _seat_index, _center_card, false);
	...
	protected boolean exe_gang(MJTable table) {
		...
		// 河南麻将,检查听牌
		table._playerStatus[_seat_index]._hu_card_count = table.get_henan_lh_ting_card(table._playerStatus[_seat_index]._hu_cards,
