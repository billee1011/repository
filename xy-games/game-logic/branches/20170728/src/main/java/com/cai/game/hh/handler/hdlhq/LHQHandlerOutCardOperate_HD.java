...
public class LHQHandlerOutCardOperate_HD extends HHHandlerOutCardOperate {
	@Override
	public void exe(HHTable table) {
		...
		for(int i = 0; i<table.getTablePlayerNumber();i++){
			...
			action_pao[i] = table.estimate_player_respond_phz_chd(i,_out_card_player ,_out_card_data, pao_type[i],true);
			...
			if (action_hu[i] != GameConstants.WIK_NULL) {
				...
				if(action_pao[i] != GameConstants.WIK_PAO&& (table.has_rule(GameConstants.GAME_RULE_QIANG_ZHI_HU_PAI)==false)){
					tempPlayerStatus.add_action(GameConstants.WIK_NULL);
					tempPlayerStatus.add_pass(_out_card_data, _out_card_player);
				}
				else if(table.has_rule(GameConstants.GAME_RULE_QIANG_ZHI_HU_PAI)==false){
		...
		if(ti_pao == GameConstants.WIK_NULL)
		{
			if(bHupai == 1&&table.has_rule(GameConstants.GAME_RULE_QIANG_ZHI_HU_PAI)==true)
				bAroseAction = false;
			else
...
//		}
		...
		case GameConstants.WIK_NULL: {
			...
			for(int i = 0; i< table.getTablePlayerNumber();i++){
				int pao_type[] =  new int[1];
				int action = table.estimate_player_respond_phz_chd(i,_out_card_player ,_out_card_data, pao_type,true);	
		...
		case GameConstants.WIK_PAO: // 杠牌操作
		{
			...
			int pao_type[] =  new int[1];
			int action = table.estimate_player_respond_phz_chd(target_player,this._out_card_player ,operate_card, pao_type,true);	
			if(action != GameConstants.WIK_NULL){
