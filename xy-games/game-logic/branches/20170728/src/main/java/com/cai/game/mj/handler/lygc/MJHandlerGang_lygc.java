...
//					}
	...
	protected boolean exe_gang(MJTable table){
		...
		//包次开始数量变更
		if(table.has_rule(GameConstants.GAME_RULE_HENAN_BAOCI) 
				&& bao_ci_change != GameConstants.INVALID_SEAT && table._bao_ci_start != GameConstants.LYGC_BAO_CI_SATRT){
		...
		if(!table.has_rule(GameConstants.GAME_RULE_HENAN_YCI)){//WalkerGeek 硬次玩法不听牌
			table._playerStatus[_seat_index]._hu_card_count = table.get_henan_ting_card_lygc(
		...
		if(!table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU) ){
			...
			if(bAroseAction && table.LYGC_CI_STATE){
				...
				int out_card_palyer = GameConstants.INVALID_SEAT;
				// 记录
				boolean bao_ci = true;
				if(GameConstants.GANG_TYPE_AN_GANG == _type){
					...
					out_card_palyer = _provide_player;
				}else if(GameConstants.GANG_TYPE_ADD_GANG == _type){//放碰的人给分
					table._player_result.jie_pao_count[_seat_index]++;
					table._player_result.dian_pao_count[table.GRR._weave_items[_seat_index][cbWeaveIndex].provide_player]++;
					out_card_palyer = table.GRR._weave_items[_seat_index][cbWeaveIndex].provide_player;
					bao_ci = false;
				}
				...
				table.process_chi_hu_player_score_henan_lygc(_seat_index,out_card_palyer,card, GameConstants.HU_CARD_TYPE_GANG_CI,bao_ci);
