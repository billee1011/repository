...
public class MJHandlerOutCardOperate_YYZXZ extends MJHandlerOutCardOperate<MJTable_YYZXZ> {
	...
	public void exe(MJTable_YYZXZ table) {
		...
		// 用户切换
		int next_player =  (_out_card_player + GameConstants.GAME_PLAYER_HH + 1) % GameConstants.GAME_PLAYER_HH;
		...
		// 玩家出牌 响应判断,是否有吃碰杠补胡
		boolean bAroseAction = table.estimate_player_out_card_respond(_out_card_player, _out_card_data);//, EstimatKind.EstimatKind_OutCard
...
//					MJGameConstants.DELAY_ADD_CARD_DELAY, TimeUnit.MILLISECONDS);
	...
	public boolean handler_operate_card(MJTable_YYZXZ table,int seat_index, int operate_code, int operate_card){
		...
		// 执行判断
		for (int p = 0; p <GameConstants.GAME_PLAYER_HH; p++){
			int i =(_out_card_player+p) % GameConstants.GAME_PLAYER_HH;
			// 获取动作
		...
		switch (target_action) {
			...
			case GameConstants.WIK_CHI_HU: //胡
			{
				...
				for (int i = 0; i < GameConstants.GAME_PLAYER; i++) {
					...
					hupai_index = checkHuIndex(_out_card_player, i, hupai_index);
					jie_pao_count++;
				}
				if (jie_pao_count > 0) {
