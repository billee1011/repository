...
public class MJHandlerNao_HenNanxy extends MJHandler {
...
//		return;
	...
	public boolean handler_nao(MJTable table,int seat_index, int nao) {
		...
		for(int i = 0; i < table.getTablePlayerNumber(); i++){
			table._playerStatus[i]._is_nao_zhuang=false;
		}
		table._is_xy_nao_zhuang=true;
		//都ok了
		//游戏开始
		if (table.has_rule(GameConstants.GAME_RULE_HENAN_DAI_PAO)) {
			table.set_handler(table._handler_pao_henna_xy);
			table._handler.exe(table);
			return true;
		}
		table.GRR._banker_player = table._current_player=table._pre_bangker_player = table._banker_select;
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++){
			table.GRR._xianchu_count[i]=0;
			for(int j=0;j<GameConstants.MAX_COUNT;j++){
				table.GRR._xianchu_cards[i][j]=GameConstants.INVALID_CARD;
			}
		}
		...
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			...
			for (int j = 0; j < GameConstants.MAX_COUNT; j++) {
				if(table._player_result.nao[i]>0 && j<=11){
					hand_cards[i][j]+=8000;
				}
				if(table._player_result.nao[i]>0){
					table.GRR._xianchu_cards[i][table.GRR._xianchu_count[i]]=hand_cards[i][12];
					table.GRR._xianchu_count[i]=1;
				}
	...
	public boolean handler_player_be_in_room(MJTable table,int seat_index) {
		...
		}else if(table._pre_bangker_player != GameConstants.INVALID_SEAT){
			tableResponse.setBankerPlayer(table._pre_bangker_player);
		}else{
