...
public class MJHandlerDispatchCard_HeNan_smx extends MJHandlerDispatchCard<MJTable> {
	...
	public void exe(MJTable table) {
		...
		if (table.GRR._left_card_count <= llcard) {
			...
			table._banker_select=(table.GRR._banker_player + table.getTablePlayerNumber() + 1) % table.getTablePlayerNumber();//若荒装，则当局庄家的下家继续坐庄。
			// 流局
		...
		int action = table.analyse_chi_hu_card_henan_smx(table.GRR._cards_index[_seat_index],
				table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], _send_card_data, chr,card_type);// 自摸
		if(table._playerStatus[_seat_index]._biaoyan_count == 0){
			if(table.isonbiaoyan(table.GRR._cards_index[_seat_index],
					table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], _send_card_data)){
				table._playerStatus[_seat_index]._biaoyan_count++;
			}
		}
	...
	public boolean handler_operate_card(MJTable table,int seat_index, int operate_code, int operate_card){
		...
		switch (operate_code) {
			...
			case GameConstants.WIK_ZI_MO: // 自摸
			{
				...
				table.process_chi_hu_player_score_henan_smx(_seat_index,_seat_index,operate_card, true);
