...
//				continue;
	...
	public void caluate_score(int seat_index,boolean b_time){
		...
		{
			if(this._can_tuizhu_player[seat_index] == 0)
		
			{
				...
				{
					int temp = this._add_Jetton[seat_index] * 2;
					if(has_rule(GameConstants.GAME_RULE_MAX_TWENTY_FOUR)){
						if(temp >24)
							temp = 24;
					}
	...
	private void countCardType(int card_type, int seat_index) {
		try {
			if (card_type == GameConstants.HJK_CARD_TYPE_AA) {
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.hjkaa, "", 0, 0l,
						this.getRoom_id());
			}

			if (card_type == GameConstants.HJK_CARD_TYPE_HJK) {
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.hjkhjk, "", 0,
						0l, this.getRoom_id());
			}

			if (card_type == GameConstants.HJK_CARD_TYPE_SEVEN) {
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.hjk777, "", 0,
						0l, this.getRoom_id());
			}

			if (card_type == GameConstants.HJK_CARD_TYPE_WUXIAOLONG) {
				MongoDBServiceImpl.getInstance().card_log(this.get_players()[seat_index], ECardType.hjkwuxiaolong, "", 0,
						0l, this.getRoom_id());
			}
...
//		 int[] realyCards = new int[] {0x11,0x0a,0x01,0x0b,0x21,0x2a,0x31,0x2c,0x22,0x3c,
//				 5, 17, 58, 33, 10, 54, 19, 4, 37, 51, 39, 237, 43, 52, 9, 23, 3, 18,
//				 20, 2, 53, 40, 55, 1, 35, 61, 59, 24, 45, 44, 60, 25, 13, 22, 36, 12,
//				 42, 8, 41, 7};
...
//		}
	...
	private void record_game_room() {
		...
		_gameRoomRecord.setGame_id(GameConstants.GAME_ID_HJK);
...
//		this._handler.exe(this);
	...
	public boolean refresh_room_redis_data(int type,boolean notifyRedis){
		...
		if(type==GameConstants.PROXY_ROOM_UPDATE){
			...
			roomRedisModel.setGameRuleDes(this.get_game_des());
			roomRedisModel.setRoomStatus(this._game_status);
			roomRedisModel.setPlayer_max(GameConstants.GAME_PLAYER_HJK);
...
//		// 判断房卡
//		// 判断房卡
//		SysParamModel sysParamModel1003 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id)
//				.get(1003);
//		SysParamModel sysParamModel1004 = SysParamDict.getInstance().getSysParamModelDictionaryByGameId(game_id)
//				.get(1004);
//
//		int check_gold = 0;
//		boolean create_result = true;
//
//		if (_game_round == sysParamModel1003.getVal1()) {
//			check_gold = sysParamModel1003.getVal2();
//		} else if (_game_round == sysParamModel1004.getVal1()) {
//			check_gold = sysParamModel1004.getVal2();
//		}  
//		// 注意游戏ID不一样
//		if (check_gold == 0) {
//			create_result = false;
...
//		return true;
	...
	public  boolean open_card_timer(){
		if(this._open_card[this._cur_banker] == false)
		{
			...
			GameSchedule.put(new GameFinishRunnable(this.getRoom_id(), this._cur_banker, GameConstants.Game_End_NORMAL),
