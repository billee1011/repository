...
public class FLSHandlerGang_LX_DispatchCard extends FLSHandler {
	...
	public FLSHandlerGang_LX_DispatchCard() {
						...
						if (cbActionMask != GameConstants.WIK_NULL) {// 有杠
							...
							for (int gc = 0; gc < m_gangCardResult.cbCardCount; gc++) {
								...
								if (is_ting == true) {
									...
									can_gang = table.is_fls_ting_card(table.GRR._cards_index[k],
...
//				table._player_result.da_hu_jie_pao[i]++;
	...
	public boolean handler_be_set_trustee(FLSTable table, int seat_index){
		...
		if(curPlayerStatus.has_action()){
			...
			if(send_index != GameConstants.MAX_FLS_INDEX && table.GRR._cards_index[seat_index][send_index] > 0){//手牌有
