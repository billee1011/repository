...
public class MJHandlerGang_CSLX_DispatchCard extends MJHandler {
...
//	private boolean _self;
	...
	public MJHandlerGang_CSLX_DispatchCard(){
			...
			for(int k=0; k < table.getTablePlayerNumber();k++){
				...
				if(k==_seat_index){
					...
					int action = table.analyse_chi_hu_card(table.GRR._cards_index[k],
							table.GRR._weave_items[k], table.GRR._weave_count[k], bu_card, chr,GameConstants.HU_CARD_TYPE_ZIMO,k);//自摸
...
//						}
					...
					if (table.GRR._left_card_count > 2) {
						...
						if (table.has_rule(GameConstants.GAME_RULE_HUNAN_ONE_GANG) || curPlayerStatus.lock_huan_zhang() == false) {
					...
					if(k==chi_seat_index){
						bAroseAction = table.estimate_gang_cs_lx_respond(k, _seat_index,bu_card,_double,true);//, EstimatKind.EstimatKind_OutCard
					}else{
						bAroseAction = table.estimate_gang_cs_lx_respond(k, _seat_index,bu_card,_double,false);//, EstimatKind.EstimatKind_OutCard
					}
...
//						cbWeaveItemCount, target_card, table.GRR._chi_hu_rights[_seat_index],MJGameConstants.HU_CARD_TYPE_ZIMO);
				...
				if(table.GRR._chi_hu_rights[_seat_index].is_mul(GameConstants.CHR_ZI_MO)){
					...
					table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index],
								table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], this._gang_card_data.get_card(0), chrtemp,GameConstants.HU_CARD_TYPE_ZIMO,_seat_index);//自摸
					ChiHuRight chrtemp2 =new ChiHuRight();
					table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index],
								table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], this._gang_card_data.get_card(1), chrtemp2,GameConstants.HU_CARD_TYPE_ZIMO,_seat_index);//自摸
					
					if(chrtemp.single_da_hu==chrtemp2.single_da_hu) {
						table.GRR._chi_hu_rights[_seat_index].set_empty();
						table.GRR._chi_hu_rights[_seat_index].copy(chrtemp2);
						...
						ChiHuRight temp = chrtemp.single_da_hu>chrtemp2.single_da_hu?chrtemp:chrtemp2;
						table.GRR._chi_hu_rights[_seat_index].set_empty();
						table.GRR._chi_hu_rights[_seat_index].copy(temp);
						table.GRR._chi_hu_rights[_seat_index].opr_or(GameConstants.CHR_HUNAN_GANG_KAI); 
					}
					table.GRR._chi_hu_rights[_seat_index].set_valid(true);
...
//					}
					...
					if(table.GRR._chi_hu_rights[i].is_mul(GameConstants.CHR_SHU_FAN)){
						...
						table.analyse_chi_hu_card(table.GRR._cards_index[i],
									table.GRR._weave_items[i], table.GRR._weave_count[i], this._gang_card_data.get_card(0), chrtemp,GameConstants.HU_CARD_TYPE_ZIMO,i);//自摸
						ChiHuRight chrtemp2 =new ChiHuRight();
						table.analyse_chi_hu_card(table.GRR._cards_index[i],
									table.GRR._weave_items[i], table.GRR._weave_count[i], this._gang_card_data.get_card(1), chrtemp2,GameConstants.HU_CARD_TYPE_ZIMO,i);//自摸
						
						if(chrtemp.single_da_hu==chrtemp2.single_da_hu) {
							table.GRR._chi_hu_rights[i].set_empty();
							table.GRR._chi_hu_rights[i].copy(chrtemp2);
							...
							ChiHuRight temp = chrtemp.single_da_hu>chrtemp2.single_da_hu?chrtemp:chrtemp2;
							table.GRR._chi_hu_rights[i].set_empty();
							table.GRR._chi_hu_rights[i].copy(temp);
							table.GRR._chi_hu_rights[i].opr_or(GameConstants.CHR_HUNAN_GANG_SHANG_PAO); 
						}
