...
//			_send_card_data=0x03;
		...
		table.operate_effect_action(_seat_index,GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[]{GameConstants.WIK_YAO_HAI_DI}, 0,GameConstants.INVALID_SEAT,0,true);
		...
		for(int i=0; i< table.getTablePlayerNumber(); i++){
			...
			action = table.analyse_chi_hu_card(table.GRR._cards_index[i],table.GRR._weave_items[i], table.GRR._weave_count[i], _send_card_data, table.GRR._chi_hu_rights[i],GameConstants.HU_CARD_TYPE_PAOHU,_seat_index);// 自摸
...
//		}
	...
	public boolean handler_player_be_in_room(FLSTable table,int seat_index) {
		...
		table.operate_effect_action(_seat_index,GameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[]{GameConstants.WIK_YAO_HAI_DI}, 0,seat_index,0,true,false);
