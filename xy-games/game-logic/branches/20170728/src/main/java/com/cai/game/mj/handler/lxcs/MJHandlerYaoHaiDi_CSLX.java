...
public class MJHandlerYaoHaiDi_CSLX extends MJHandlerYaoHaiDi<MJTable> {

	private static Logger logger = Logger.getLogger(MJHandlerYaoHaiDi_CSLX.class);
	...
	public void exe(MJTable table) {
		...
		int action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index],table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], _send_card_data, chr,GameConstants.HU_CARD_TYPE_ZIMO,_seat_index);// 自摸
		...
		for(int i=0; i<table.getTablePlayerNumber(); i++){
			...
			chr.set_empty();
			action = table.analyse_chi_hu_card(table.GRR._cards_index[i],table.GRR._weave_items[i], table.GRR._weave_count[i], _send_card_data, chr,GameConstants.HU_CARD_TYPE_PAOHU,i);// 自摸
...
//				table.process_chi_hu_player_operate_cs(seat_index, new int[]{operate_card},1,false);--先不弹效果 可能自己会胡
		...
		switch (target_action) {
			...
			case GameConstants.WIK_ZI_MO: // 自摸
			{
				...
				// 下局胡牌的是庄家
				table.set_niao_card_cs(table.GRR._banker_player,_send_card_data,false,0,false);// 结束后设置鸟牌
				// 吃牌权位
...
//				{
			...
			case GameConstants.WIK_CHI_HU: //胡
			{
				...
				if(jie_pao_count>1){
					table._banker_select = _seat_index;
					table.set_niao_card_cs(table.GRR._banker_player,_send_card_data,false,0,true);
				}else{
					table._banker_select = target_player;
					table.set_niao_card_cs(table.GRR._banker_player,_send_card_data,false,0,false);
				}
