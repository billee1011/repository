	...
	public boolean game_start_ddzjd(){
		...
		for(int index=0;index<this.getTablePlayerNumber();index++){
			...
			for(int play_index=0;play_index<this.getTablePlayerNumber();play_index++){
				for(int i=0;i<this.getTablePlayerNumber();i++){
					if(this.has_rule(GameConstants.GAME_RULE_DISPLAY_CARD)){
						...
						if(i == play_index){
							...
							gameStartResponse.addCardCount(-1);
						}
					}
				...
				gameStartResponse.setCardsData(play_index, cards_card);
			}
			
			gameStartResponse.setCellScore(1);
			roomResponse_ddz.setGameStart(gameStartResponse);


			roomResponse.setCommResponse(PBUtil.toByteString(roomResponse_ddz));
