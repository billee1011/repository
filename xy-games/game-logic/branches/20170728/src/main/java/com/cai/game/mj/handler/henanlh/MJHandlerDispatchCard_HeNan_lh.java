...
public class MJHandlerDispatchCard_HeNan_lh extends MJHandlerDispatchCard<MJTable> {
	...
	public void exe(MJTable table) {
			...
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				for (int j = 0; j < table.GRR._gang_score[i].gang_count; j++) {
					for (int k = 0; k < table.getTablePlayerNumber(); k++) {
						table.GRR._gang_score[i].scores[j][k] = 0;// 杠牌，每个人的分数
					}
				}
			}
		...
		int action = table.analyse_chi_hu_card_henan_lh(table.GRR._cards_index[_seat_index], table.GRR._weave_items[_seat_index],
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		case GameConstants.WIK_ZI_MO: // 自摸
		{
			...
			table.process_chi_hu_player_score_he_nan_lh(_seat_index, _seat_index, operate_card, true);
