 ...
 * 
 */
package com.cai.game.pdk.handler.fifteenpdk;
...
///////////////////////////////////////////////////////////////////////////////////////////////
public class PDK_FIFTEEN_Table extends PDKTable {

	public PDK_FIFTEEN_Table() {
	}
	...
	public void init_table(int game_type_index, int game_rule_index, int game_round) {
		...
		if (is_mj_type(GameConstants.GAME_TYPE_PDK_JD)) {
			...
			_handler_out_card_operate = new PDKHandlerOutCardOperate_FIFTEEN();
...
//		BACK_DEBUG_CARDS_MODE = true;
		...
		if (is_mj_type(GameConstants.GAME_TYPE_PDK_SW)) {
	...
	public int find_banker(){
		...
		if(this.has_rule(GameConstants.GAME_RULE_TWO_PLAY)){
			//牌最小的玩家出
			if(this.GRR._cards_data[0][this.GRR._card_count[0]-1] > this.GRR._cards_data[1][this.GRR._card_count[1]-1]){
				return 1;
			}else{
				return 0;
			}
	...
	private void shuffle(int repertory_card[], int mj_cards[]) {
		...
		for (int i = 0; i < count; i++) {
			for (int j = 0; j < GameConstants.MAX_PDK_COUNT_SW; j++) {
				GRR._cards_data[i][j] = repertory_card[i *  GameConstants.MAX_PDK_COUNT_SW + j];
				if(this.has_rule(GameConstants.GAME_RULE_HONGTAO10_ZANIAO) && GRR._cards_data[i][j] == 0x29){
					_hong_tao_palyer=i;
				}
			}
			GRR._card_count[i] = GameConstants.MAX_PDK_COUNT_SW;
	...
	private void test_cards() {
		int cards[] = new int[] { 0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,
	...
	public boolean handler_game_finish_pdk_jd(int seat_index, int reason) {
		...
		if(reason == GameConstants.Game_End_NORMAL){
			...
			//春天玩家
			for(int i=0;i<this.getTablePlayerNumber();i++){
				if(this.has_rule(GameConstants.GAME_RULE_KE_FAN_DE) && i == GRR._banker_player && this._out_card_times[i]==1 && GRR._card_count[i] != 0){
