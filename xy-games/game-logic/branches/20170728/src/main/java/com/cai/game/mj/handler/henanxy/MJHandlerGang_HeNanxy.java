...
public class MJHandlerGang_HeNanxy extends MJHandlerGang<MJTable> {
	@Override
...
//		}
		...
		if(table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU)) {//可炮胡的时候 才能抢杆
			bAroseAction =table.estimate_gang_respond_henan_xy(_seat_index, _center_card);
		}
...
//					}
	...
	public boolean handler_operate_card(MJTable table,int seat_index, int operate_code, int operate_card){
		...
		if(jie_pao_count>0){
			...
			table.process_chi_hu_player_score_henanxy(target_player,_seat_index, _center_card,false);
	...
	protected boolean exe_gang(MJTable table){
		...
		//不能出的牌
		if(table.GRR._xianchu_count[_seat_index]>0){
			int xianchu_cards[]=new int[GameConstants.MAX_COUNT];
			for(int i=0;i<GameConstants.MAX_COUNT;i++){
				xianchu_cards[i]=table.GRR._xianchu_cards[_seat_index][i];
			}
			for(int i=GameConstants.MAX_COUNT-1;i>=0;i--){
				boolean bxianchu=false;
				for(int j=0;j<table.GRR._xianchu_count[_seat_index];j++){
		...
		if(table.has_rule(GameConstants.GAME_RULE_HENAN_TUIDAOHU)){
			...
			if(GameConstants.GANG_TYPE_AN_GANG == _type){
				for (int i = 0; i < table.getTablePlayerNumber(); i++) {
					...
					//暗杠每人2分
					table.GRR._gang_score[_seat_index].scores[cbGangIndex][i] = -1*GameConstants.CELL_SCORE;// 暗杠，其他玩家扣分
					table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += 1*GameConstants.CELL_SCORE;//一共加分
				}
				...
				table.GRR._gang_score[_seat_index].scores[cbGangIndex][table.GRR._weave_items[_seat_index][cbWeaveIndex].provide_player] =  -1*GameConstants.CELL_SCORE;	
				table._player_result.ming_gang_count[_seat_index]++;
			}
		}
		...
		//河南麻将,检查听牌
		table._playerStatus[_seat_index]._hu_card_count = table.get_henanxy_ting_card(
