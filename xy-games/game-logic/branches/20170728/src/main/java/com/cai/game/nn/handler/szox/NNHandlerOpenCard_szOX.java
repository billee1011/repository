			...
			if (table._player_status[k] != true) {
				...
				if (k == seat_index) {
					...
					if (table.has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_ONE))
						times = table._logic.get_times_two(table.GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, table._game_rule_index);
					if (table.has_rule(GameConstants.GAME_RULE_SELECT_OX_VALUSE_TWO))
						times = table._logic.get_times_one(table.GRR._cards_data[k], GameConstants.OX_MAX_CARD_COUNT, table._game_rule_index);
