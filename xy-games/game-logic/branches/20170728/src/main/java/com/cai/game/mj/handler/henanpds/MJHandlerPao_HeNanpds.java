...
public class MJHandlerPao_HeNanpds extends MJHandlerPao_HeNan {

	@Override
	public void exe(MJTable table) {

		if (table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU)) {
			table._game_status = GameConstants.GS_MJ_PAO;// 点炮胡设置状态
		} else {
			table._game_status = GameConstants.GS_MJ_PAO_KOU;// 自摸胡设置状态
		}
	...
	public boolean handler_pao_qiang(MJTable table, int seat_index, int pao,
			int qiang) {
		...
		table.operate_player_data();
		for (int i = 0; i < table.getTablePlayerNumber(); i++) {
			if (table._playerStatus[i]._is_pao == false) {
				return true;
			}
		}
		...
		if (table.has_rule(GameConstants.GAME_RULE_HENAN_HENAN_PAO_HU)) {
			// 等到跑头都选择完毕后，切换到选择跑扣的状态
			// table._game_status = GameConstants.GS_MJ_PIAO_KOU;
			// 切换到跑扣处理器
			table.exe_kou();
		} else {
			// 都ok了
			// 游戏开始 当选择是自摸胡时，直接选择完跑扣 游戏开始
			table._game_status = GameConstants.GS_MJ_PLAY;// 设置状态
			GameStartResponse.Builder gameStartResponse = GameStartResponse
					.newBuilder();
			...
			// 发送数据
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				int hand_card_count = table._logic.switch_to_cards_data(
						table.GRR._cards_index[i], hand_cards[i]);
				gameStartResponse.addCardsCount(hand_card_count);
			}

			// 发送数据
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				Int32ArrayResponse.Builder cards = Int32ArrayResponse
						.newBuilder();
				...
				roomResponse
						.setCurrentPlayer(table._current_player == GameConstants.INVALID_SEAT ? table._resume_player
			...
			// 检测听牌
			for (int i = 0; i < table.getTablePlayerNumber(); i++) {
				table._playerStatus[i]._hu_card_count = table
						.get_henan_ting_card(table._playerStatus[i]._hu_cards,
								table.GRR._cards_index[i],
								table.GRR._weave_items[i],
								table.GRR._weave_count[i]);
				if (table._playerStatus[i]._hu_card_count > 0) {
					table.operate_chi_hu_cards(i,
							table._playerStatus[i]._hu_card_count,
							table._playerStatus[i]._hu_cards);
				}
			}

			table.exe_dispatch_card(table._current_player,
					GameConstants.WIK_NULL, GameConstants.DELAY_SEND_CARD_DELAY);
	...
	private void player_reconnect(MJTable table, int seat_index) {
		if (table._playerStatus[seat_index]._is_pao == true) {
			return;
		}
