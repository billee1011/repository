...
public class MJHandlerDaDian extends MJHandler {
	...
	public void exe(MJTable table) {
		...
		if (table.is_mj_type(MJGameConstants.GAME_TYPE_HZ)) {
			_da_dian_card = table._repertory_card_zz[table._all_card_len-table.GRR._left_card_count];
		} else {
			_da_dian_card = table._repertory_card_cs[table._all_card_len-table.GRR._left_card_count];
		}
		--table.GRR._left_card_count;
		
		if(table.DEBUG_CARDS_MODE)_da_dian_card=0x04;//
		//显示
		table.operate_show_card(_banker,MJGameConstants.Show_Card_Center, 1,new int[]{_da_dian_card+MJGameConstants.CARD_ESPECIAL_TYPE_DING_GUI},MJGameConstants.INVALID_SEAT);
	...
	public boolean handler_player_be_in_room(MJTable table,int seat_index) {
		...
		//显示
		table.operate_show_card(_banker,MJGameConstants.Show_Card_Center, 1,new int[_da_dian_card],seat_index);
