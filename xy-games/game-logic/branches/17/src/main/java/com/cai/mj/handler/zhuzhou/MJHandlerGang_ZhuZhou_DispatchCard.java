...
public class MJHandlerGang_ZhuZhou_DispatchCard extends MJHandler {
...
//	private boolean _self;
	...
	public MJHandlerGang_ZhuZhou_DispatchCard(){
...
//									//只有杠的这张才算
									...
									boolean is_ting = table.is_zhuzhou_ting_card(table.GRR._cards_index[k],
...
//								}
					...
					if(k==chi_seat_index){
						bAroseAction = table.estimate_gang_zhuzhou_respond(k, _seat_index,bu_card,_double,true);//, EstimatKind.EstimatKind_OutCard
					}else{
						bAroseAction = table.estimate_gang_zhuzhou_respond(k, _seat_index,bu_card,_double,false);//, EstimatKind.EstimatKind_OutCard
					}
				...
				if(playerStatus.has_action()){
					if(playerStatus.has_chi_hu()){
						if(table.has_rule(MJGameConstants.GAME_TYPE_ZZ_JIANPAOHU)){
							if(i==_seat_index) {//庄家自摸
								table.exe_jian_pao_hu(i,MJGameConstants.WIK_ZI_MO,MJGameConstants.INVALID_VALUE);
							}else {
								//见炮胡
								table.exe_jian_pao_hu(i,MJGameConstants.WIK_CHI_HU,MJGameConstants.INVALID_VALUE);
							}
							return;
						}else{
							table.operate_player_action(i, false);
...
//			}
		...
		switch (target_action) {
			...
			case MJGameConstants.WIK_ZI_MO: // 自摸
			{
				...
				if(operate_card == MJGameConstants.INVALID_VALUE){
					int bu_card;
					ChiHuRight ch=null;
					int wFanShu=0;
					for(int i=0; i < MJGameConstants.CS_GANG_DRAW_COUNT; i++){
						...
						int fan=0;
						//有两张牌,所以要重新算一次
						int ddd  = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index], pWeaveItem,
								cbWeaveItemCount, bu_card, ch,MJGameConstants.HU_CARD_TYPE_ZIMO);
						
						if(ddd !=0){
							if(table.has_rule(MJGameConstants.GAME_TYPE_SCORE_MUTIP)) {
								fan = table._logic.get_chi_hu_action_rank_zhuzhou_mutip(ch);
							}else {
								fan = table._logic.get_chi_hu_action_rank_zhuzhou(ch);
							}
						}
						if(fan>wFanShu){
							table.GRR._chi_hu_kind[_seat_index] = ddd;
							target_card=bu_card;
							wFanShu = fan;
							table.GRR._chi_hu_rights[_seat_index].copy(ch);
						}
					}
				}else{
//					//有两张牌,所以要重新算一次
			...
			case MJGameConstants.WIK_CHI_HU: //胡
			{
				...
				//放炮
				// 检查接炮
				int cbWeaveItemCount = table.GRR._weave_count[target_player];
				WeaveItem pWeaveItem[] = table.GRR._weave_items[target_player];
				if(operate_card == MJGameConstants.INVALID_VALUE){
					int bu_card;
					ChiHuRight ch = null;
					int wFanShu=0;
					for(int i=0; i < MJGameConstants.CS_GANG_DRAW_COUNT; i++){
						...
						int fan=0;
						//有两张牌,所以要重新算一次
						int ddd  = table.analyse_chi_hu_card(table.GRR._cards_index[target_player], pWeaveItem,
								cbWeaveItemCount, bu_card, ch,MJGameConstants.HU_CARD_TYPE_PAOHU);
						...
						if(fan>wFanShu){
							table.GRR._chi_hu_kind[target_player] = ddd;
							target_card=bu_card;
							wFanShu = fan;
							table.GRR._chi_hu_rights[target_player].copy(ch);
						}
					}
				}else{
					table.GRR._chi_hu_kind[target_player] = table.analyse_chi_hu_card(table.GRR._cards_index[target_player], pWeaveItem,
					cbWeaveItemCount, table._playerStatus[target_player]._operate_card, table.GRR._chi_hu_rights[target_player],MJGameConstants.HU_CARD_TYPE_PAOHU);
				}
				
				
				table.GRR._chi_hu_rights[target_player].opr_or(MJGameConstants.CHR_FANG_PAO);
