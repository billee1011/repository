...
public class MJHandlerDispatchCard_XTHH extends MJHandlerDispatchCard {
	...
	public MJHandlerDispatchCard_XTHH(){
	...
	public void exe(MJTable table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			//最后四张牌。看谁能自摸。不用打
			table.operate_effect_action(MJGameConstants.INVALID_SEAT, MJGameConstants.Effect_Action_Other, 1, new long[]{MJGameConstants.EFFECT_LAST_FOUR_CARD}, 1, MJGameConstants.INVALID_SEAT);
			//延迟3秒
			table.exe_dispatch_last_card(_seat_index,MJGameConstants.WIK_NULL,3000);
			return;
		}
		...
		if (table.GRR._left_card_count > 0) {
			...
			if(cbActionMask!=MJGameConstants.WIK_NULL){//有笑
	...
	public boolean handler_player_out_card(MJTable table,int seat_index, int card) {
		// 错误断言
		if(card>MJGameConstants.CARD_ESPECIAL_TYPE_GUI && card<MJGameConstants.CARD_ESPECIAL_TYPE_LAI_ZI ){
			card-=MJGameConstants.CARD_ESPECIAL_TYPE_GUI;
		}else if(card>MJGameConstants.CARD_ESPECIAL_TYPE_LAI_ZI && card<MJGameConstants.CARD_ESPECIAL_TYPE_DING_GUI){
			card-=MJGameConstants.CARD_ESPECIAL_TYPE_LAI_ZI;
			
		}else if(card>MJGameConstants.CARD_ESPECIAL_TYPE_DING_GUI && card<MJGameConstants.CARD_ESPECIAL_TYPE_LAI_GEN){
			card-=MJGameConstants.CARD_ESPECIAL_TYPE_DING_GUI;
			
		}
		else if(card>MJGameConstants.CARD_ESPECIAL_TYPE_LAI_GEN ){
			card-=MJGameConstants.CARD_ESPECIAL_TYPE_LAI_GEN;
...
//		}
	...
	public boolean handler_operate_card(MJTable table,int seat_index, int operate_code, int operate_card){
		...
		switch (operate_code) {
			...
			case MJGameConstants.WIK_DA_CHAO_TIAN: // 杠牌操作
			{
				for(int i= 0; i < m_gangCardResult.cbCardCount; i++){
					if(operate_card == m_gangCardResult.cbCardData[i]){
						//是否有抢杠胡
			...
			case MJGameConstants.WIK_MENG_XIAO: 
			{
				for(int i= 0; i < m_gangCardResult.cbCardCount; i++){
					if(operate_card == m_gangCardResult.cbCardData[i]){
						//是否有抢杠胡
