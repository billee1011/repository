...
public class MJHandlerGang extends MJHandler {
	...
	public void exe(MJTable table) {
		...
		if(bAroseAction == false){
			...
			for (int i = 0; i < MJGameConstants.GAME_PLAYER; i++) {
				...
				if(playerStatus.has_chi_hu() && table.has_rule(MJGameConstants.GAME_TYPE_ZZ_JIANPAOHU)){
					continue;
				}
	...
	public boolean handler_operate_card(MJTable table,int seat_index, int operate_code, int operate_card){
		...
		if(jie_pao_count>0){
			...
			if(jie_pao_count>1){
				//红中通炮就不加鸟了
				table.set_niao_card(_provide_player,MJGameConstants.INVALID_VALUE,true,add_niao);// 结束后设置鸟牌
				...
				table.set_niao_card(seat_index,MJGameConstants.INVALID_VALUE,true,add_niao);// 结束后设置鸟牌 
	...
	protected boolean exe_gang(MJTable table){
		...
		table.operate_player_cards(_seat_index, hand_card_count, cards, table.GRR._weave_count[_seat_index], table.GRR._weave_items[_seat_index]);
			...
			/////////////////////////////////////////////////////算分
		if(table.is_mj_type(MJGameConstants.GAME_TYPE_CS)){
			//长沙麻将杠没有分
			...
			if(_self){
				...
				for (int i = 0; i < MJGameConstants.GAME_PLAYER; i++) {
					...
					if(_p == true){
						...
						//暗杠每人2分
						table.GRR._gang_score[_seat_index].scores[cbGangIndex][i] = -2*MJGameConstants.CELL_SCORE;// 暗杠，其他玩家扣分
						table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] += 2*MJGameConstants.CELL_SCORE;//一共加分
					}
				...
				//放杠三分
				table.GRR._gang_score[_seat_index].scores[cbIndex][_seat_index] = 3 * MJGameConstants.CELL_SCORE;// m_pGameServiceOption->lCellScore*2;//配置参数乘
				table.GRR._gang_score[_seat_index].scores[cbIndex][_provide_player] = -3 * MJGameConstants.CELL_SCORE;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
			}
...
//			}
		...
		//效果
		table.operate_effect_action(_seat_index,MJGameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[]{_action}, 1,seat_index);
