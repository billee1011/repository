...
public class MJHandlerOutCardOperate_XTHH extends MJHandlerOutCardOperate {
	...
	public void exe(MJTable table) {
		...
		//显示出牌
		table.operate_out_card(_out_card_player,1,new int[]{real_card},MJGameConstants.OUT_CARD_TYPE_MID,MJGameConstants.INVALID_SEAT);
...
//			}
			...
			table.GRR._especial_txt_type = MJGameConstants.Especial_Txt_Type_PIAO_LAI;
			
			//效果
			table.operate_effect_action(_out_card_player, MJGameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[]{MJGameConstants.WIK_PIAO_LAI}, 1, MJGameConstants.INVALID_SEAT);
			
			if(table.has_rule(MJGameConstants.GAME_TYPE_PIAO_LAI_YOU_JIANG)){
				//飘赖有奖
				int lai_count[] = new int[MJGameConstants.GAME_PLAYER];
				float score[] = new float[ MJGameConstants.GAME_PLAYER];
			...
			if(_type == MJGameConstants.WIK_MENG_XIAO ||
					_type == MJGameConstants.WIK_DIAN_XIAO||
					_type == MJGameConstants.WIK_HUI_TOU_XIAO||
					_type == MJGameConstants.WIK_XIAO_CHAO_TIAN||
					_type == MJGameConstants.WIK_DA_CHAO_TIAN){
				// 玩家出牌 响应判断,是否有吃碰杠补胡
				bAroseAction = table.estimate_player_out_card_respond_xthh(_out_card_player, _out_card_data,MJGameConstants.HU_CARD_TYPE_RE_CHONG);//, EstimatKind.EstimatKind_OutCard
			}else{
				bAroseAction = table.estimate_player_out_card_respond_xthh(_out_card_player, _out_card_data,MJGameConstants.HU_CARD_TYPE_PAOHU);//, EstimatKind.EstimatKind_OutCard
			}
...
//					MJGameConstants.DELAY_ADD_CARD_DELAY, TimeUnit.MILLISECONDS);
			...
			if(piao_lai==true){
				...
				//发牌
				table.exe_dispatch_card(_out_card_player,MJGameConstants.WIK_PIAO_LAI,MJGameConstants.DELAY_SEND_CARD_DELAY);
			}else{
				// 用户切换
	...
	public boolean handler_operate_card(MJTable table,int seat_index, int operate_code, int operate_card){
		...
		switch (target_action) {
			...
			case MJGameConstants.WIK_PENG: // 碰牌操作
				...
				break;
			case MJGameConstants.WIK_DIAN_XIAO: // 杠牌操作
			{
				//是否有抢杠胡
				table.exe_gang(target_player, _out_card_player, target_card, target_action, MJGameConstants.WIK_DIAN_XIAO, false,false);
				return true;
			}
			case MJGameConstants.WIK_XIAO_CHAO_TIAN: // 小朝天 能不能抢杠胡？
			{
				//是否有抢杠胡
				table.exe_gang(target_player, _out_card_player, target_card, target_action, MJGameConstants.WIK_XIAO_CHAO_TIAN, false,false);
				return true;
			}
	...
	public boolean handler_player_be_in_room(MJTable table,int seat_index) {
		...
		//出牌
		table.operate_out_card(_out_card_player, 1, new int[]{real_card},MJGameConstants.OUT_CARD_TYPE_MID,seat_index);
