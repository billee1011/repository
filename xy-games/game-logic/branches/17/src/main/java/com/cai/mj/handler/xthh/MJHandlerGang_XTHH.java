...
public class MJHandlerGang_XTHH extends MJHandlerGang {
	...
	public void reset_status(int seat_index,int provide_player, int center_card,int action, int type, boolean self,boolean d){
		...
		_action = action;
		_type = type;
		if(MJGameConstants.WIK_MENG_XIAO == _type || MJGameConstants.WIK_DA_CHAO_TIAN == _type){
			_p = false;
		}else{
			_p = true;
		}
	...
	public void exe(MJTable table) {
		...
		if((MJGameConstants.WIK_MENG_XIAO == _type)||
				(MJGameConstants.WIK_DIAN_XIAO == _type) ||
				(MJGameConstants.WIK_DA_CHAO_TIAN == _type)||
				(MJGameConstants.WIK_XIAO_CHAO_TIAN == _type)){
			this.exe_gang(table);
			return;
		}
	...
	protected boolean exe_gang(MJTable table){
		...
		if(MJGameConstants.WIK_MENG_XIAO == _type){
		...
		}else if(MJGameConstants.WIK_DIAN_XIAO == _type){
			//别人打的牌
		...
		}else if(MJGameConstants.WIK_HUI_TOU_XIAO == _type){
			// 看看是不是有碰的牌，明杠
			// 寻找组合
		...
		}else if(MJGameConstants.WIK_DA_CHAO_TIAN == _type){
			//别人打的牌
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
			...
			//table.operate_out_card(_provide_player, 0, null,MJGameConstants.OUT_CARD_TYPE_MID,MJGameConstants.INVALID_SEAT);
			//table.operate_remove_discard(this._provide_player, table.GRR._discard_count[_provide_player]);
		}else if(MJGameConstants.WIK_XIAO_CHAO_TIAN == _type){
			//别人打的牌
			cbWeaveIndex = table.GRR._weave_count[_seat_index];
			table.GRR._weave_count[_seat_index]++;
		...
		for (int i = 0; i < weave_count; i++) {
			...
			weaves[i].provide_player = table.GRR._weave_items[_seat_index][i].provide_player+MJGameConstants.WEAVE_SHOW_DIRECT;
			//癞子
		...
		if(MJGameConstants.WIK_MENG_XIAO == _type || MJGameConstants.WIK_DA_CHAO_TIAN== _type ){
			...
			if( MJGameConstants.WIK_DA_CHAO_TIAN== _type ){
				// 出牌
				table.exe_chi_peng(_seat_index,_action);
				return true;
			}
