...
public class MJHandlerYaoHaiDi extends MJHandler {

private static Logger logger = Logger.getLogger(MJHandlerYaoHaiDi.class);
	...
	public void exe(MJTable table) {
		...
		//if(table.DEBUG_CARDS_MODE)_send_card_data=0x17;
		...
		int action = table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index],table.GRR._weave_items[_seat_index], table.GRR._weave_count[_seat_index], _send_card_data, chr,MJGameConstants.HU_CARD_TYPE_ZIMO);// 自摸
		...
		for(int i=0; i< MJGameConstants.GAME_PLAYER; i++){
			...
			action = table.analyse_chi_hu_card(table.GRR._cards_index[i],table.GRR._weave_items[i], table.GRR._weave_count[i], _send_card_data, table.GRR._chi_hu_rights[i],MJGameConstants.HU_CARD_TYPE_PAOHU);// 自摸
		...
		if(has_action==false){
			GameSchedule.put(new AddDiscardRunnable(table.getRoom_id(), _seat_index, 1, new int[]{_send_card_data},true),
					1, TimeUnit.SECONDS);
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._banker_select, MJGameConstants.Game_End_DRAW),
					2, TimeUnit.SECONDS);
		}else{
			for (int i = 0; i < MJGameConstants.GAME_PLAYER; i++) {
	...
	public boolean handler_operate_card(MJTable table,int seat_index, int operate_code, int operate_card){
		...
		if(operate_code != MJGameConstants.WIK_ZI_MO){
			...
			// 执行判断
			for (int i = _seat_index; i <MJGameConstants.GAME_PLAYER; i++){
				int p =i % MJGameConstants.GAME_PLAYER;
				// 获取动作
