...
public class MJHandlerGang_HZ extends MJHandlerGang {
	@Override
	...
	public boolean handler_operate_card(MJTable table,int seat_index, int operate_code, int operate_card){
		...
		if(jie_pao_count>0){
			...
			if(jie_pao_count>1){
				...
				if(table.GRR._cards_index[seat_index][table._logic.get_magic_card_index(0)]==0){
					//自己的手上没有红中 加1鸟
					add_niao=1;
				}
	...
	protected boolean exe_gang(MJTable table){
		...
		if(MJGameConstants.GANG_TYPE_AN_GANG == _type){
			...
			//放杠三分
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_seat_index] = 3 * MJGameConstants.CELL_SCORE;// m_pGameServiceOption->lCellScore*2;//配置参数乘
			table.GRR._gang_score[_seat_index].scores[cbGangIndex][_provide_player] = -3 * MJGameConstants.CELL_SCORE;// -m_pGameServiceOption->lCellScore*2;//配置参数乘
