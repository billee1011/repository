 ...
 *
 */
public class MJHandlerDispatchCard_CS extends MJHandlerDispatchCard {

	private GangCardResult m_gangCardResult;
	
	public MJHandlerDispatchCard_CS(){
	...
	public void exe(MJTable table) {
		...
		if (table.GRR._left_card_count == 0) {
			...
			if(table.has_rule(MJGameConstants.GAME_TYPE_HZ)==false){
		...
		//海底
		if(table.is_mj_type(MJGameConstants.GAME_TYPE_CS) && table.GRR._left_card_count==1){
		...
		if(table.DEBUG_CARDS_MODE)_send_card_data=0x11;
			...
			if(cbActionMask!=MJGameConstants.WIK_NULL){//有杠
				...
				for(int i= 0; i < m_gangCardResult.cbCardCount; i++){
					...
					if (table.GRR._left_card_count > 2) {
						...
						is_ting = table.is_cs_ting_card(table.GRR._cards_index[_seat_index],
		...
		if(curPlayerStatus.has_action()){//有动作
			...
			if(table._playerStatus[_seat_index].lock_huan_zhang()){
				 GameSchedule.put(new OutCardRunnable(table.getRoom_id(), _seat_index, _send_card_data),
						MJGameConstants.DELAY_AUTO_OUT_CARD, TimeUnit.MILLISECONDS);
			}else{
