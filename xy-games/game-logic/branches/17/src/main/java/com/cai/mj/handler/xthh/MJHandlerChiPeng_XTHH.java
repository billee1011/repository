...
public class MJHandlerChiPeng_XTHH extends MJHandlerChiPeng {
	private GangCardResult m_gangCardResult;
	
	public MJHandlerChiPeng_XTHH(){
	...
	public void exe(MJTable table) {
		...
		if (table.GRR._left_card_count > 0) {
			//只检测手上的牌能不能杠
			int cbActionMask = table._logic.analyse_gang_card_all_xthh(table.GRR._cards_index[_current_player],
					table.GRR._weave_items[_current_player], table.GRR._weave_count[_current_player], m_gangCardResult,false);
			
			if(cbActionMask!=0){
				for(int i= 0; i < m_gangCardResult.cbCardCount; i++){
					//加上刚
					curPlayerStatus.add_action(m_gangCardResult.type[i]);//转转就是杠
					curPlayerStatus.add_xiao(m_gangCardResult.cbCardData[i],m_gangCardResult.type[i], _current_player, m_gangCardResult.isPublic[i]);
				}
			}
		}
	...
	public boolean handler_player_out_card(MJTable table,int seat_index, int card) {
		if(card>MJGameConstants.CARD_ESPECIAL_TYPE_LAI_ZI){
