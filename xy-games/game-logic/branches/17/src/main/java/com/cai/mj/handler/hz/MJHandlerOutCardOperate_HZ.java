...
public class MJHandlerOutCardOperate_HZ extends MJHandlerOutCardOperate {
	...
	public void exe(MJTable table) {
		...
		// 玩家出牌 响应判断,是否有吃碰杠补胡
		boolean bAroseAction = table.estimate_player_out_card_respond_hz(_out_card_player, _out_card_data);//, EstimatKind.EstimatKind_OutCard
			...
			case MJGameConstants.WIK_CHI_HU: //胡
			{
				...
				if(jie_pao_count>0){
					...
					if(jie_pao_count>1){
						// 结束后设置鸟牌
						//红中通炮就不加鸟了
						table.set_niao_card(_out_card_player,MJGameConstants.INVALID_VALUE,true,add_niao);
						//结束
						...
						// 结束后设置鸟牌 
						table.set_niao_card(seat_index,MJGameConstants.INVALID_VALUE,true,add_niao);
