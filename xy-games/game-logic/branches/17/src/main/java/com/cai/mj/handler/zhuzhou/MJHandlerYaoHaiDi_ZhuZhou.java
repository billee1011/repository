...
public class MJHandlerYaoHaiDi_ZhuZhou extends MJHandlerYaoHaiDi {

	private static Logger logger = Logger.getLogger(MJHandlerYaoHaiDi_ZhuZhou.class);
...
//			_send_card_data = 0x03;
		...
		table.operate_effect_action(_seat_index, MJGameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { MJGameConstants.WIK_YAO_HAI_DI }, 0, MJGameConstants.INVALID_SEAT);
		table.operate_out_card(_seat_index, 1, new int[] { _send_card_data }, MJGameConstants.OUT_CARD_TYPE_LEFT, MJGameConstants.INVALID_SEAT);
		...
		for (int i = 0; i < MJGameConstants.GAME_PLAYER; i++) {
			...
			action = table.analyse_chi_hu_card(table.GRR._cards_index[i], table.GRR._weave_items[i], table.GRR._weave_count[i], _send_card_data, table.GRR._chi_hu_rights[i],
					MJGameConstants.HU_CARD_TYPE_PAOHU);// 自摸

			if (action != MJGameConstants.WIK_NULL) {
				// 添加动作
				table._playerStatus[i].add_action(MJGameConstants.WIK_CHI_HU);
				table._playerStatus[i].add_chi_hu(_send_card_data, _seat_index);
				table.GRR._chi_hu_rights[i].opr_or(MJGameConstants.CHR_HAI_DI_PAO);// 海底炮
		...
		if (has_action == false) {
			GameSchedule.put(new AddDiscardRunnable(table.getRoom_id(), _seat_index, 1, new int[] { _send_card_data },true), 1, TimeUnit.SECONDS);
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._banker_select, MJGameConstants.Game_End_DRAW), 2, TimeUnit.SECONDS);
		} else {
			// for (int i = 0; i < MJGameConstants.GAME_PLAYER; i++) {
				...
				if (playerStatus.has_action()) {
					if (playerStatus.has_chi_hu()) {
						if (table.has_rule(MJGameConstants.GAME_TYPE_ZZ_JIANPAOHU)) {
							if (_seat_index == i) {// 庄家是自摸
								table.exe_jian_pao_hu(i, MJGameConstants.WIK_ZI_MO, _send_card_data);
							} else {
								// 见炮胡
								table.exe_jian_pao_hu(i, MJGameConstants.WIK_CHI_HU, _send_card_data);
							}
							return;
						} else {
							table.operate_player_action(i, false);
...
//			}
			...
			for (int p = 0; p <MJGameConstants.GAME_PLAYER; p++){
				...
				if(table._playerStatus[i].has_action()){
					if(table._playerStatus[i].is_respone()){
						//获取已经执行的动作的优先级
						cbUserActionRank =  table._logic.get_action_rank(table._playerStatus[i].get_perform())+MJGameConstants.GAME_PLAYER-p;
					}else{
						//获取最大的动作的优先级
						cbUserActionRank =  table._logic.get_action_list_rank(table._playerStatus[i]._action_count,table._playerStatus[i]._action)+MJGameConstants.GAME_PLAYER-p;
					}
					...
					logger.info("cbUserActionRank:"+cbUserActionRank + "  cbTargetActionRank:"+cbTargetActionRank);
					// 动作判断 优先级最高的人和动作
		...
		case MJGameConstants.WIK_NULL: {
			...
			GameSchedule.put(new AddDiscardRunnable(table.getRoom_id(), _seat_index, 1, new int[] { _send_card_data },true), 1, TimeUnit.SECONDS);
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._banker_select, MJGameConstants.Game_End_NORMAL), 2, TimeUnit.SECONDS);
		...
		case MJGameConstants.WIK_ZI_MO: // 自摸
		{
			...
			// 下局胡牌的是庄家
			table.set_niao_card(table.GRR._banker_player, MJGameConstants.INVALID_VALUE, false, 0);// 结束后设置鸟牌
			// 吃牌权位
			...
			//
			// table.GRR._chi_hu_kind[_seat_index] =
			// table.analyse_chi_hu_card(table.GRR._cards_index[_seat_index],
			// pWeaveItem,
			// cbWeaveItemCount, target_card,
			// table.GRR._chi_hu_rights[_seat_index],true);
			//
			...
			// 要删除手上的牌,因为这张海底并没有入手
			table.process_chi_hu_player_operate(_seat_index, _send_card_data, false);
			...
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), table._banker_select, MJGameConstants.Game_End_NORMAL), MJGameConstants.GAME_FINISH_DELAY,
					TimeUnit.SECONDS);
		...
		case MJGameConstants.WIK_CHI_HU: // 胡
		{
			...
			for (int i = 0; i < MJGameConstants.GAME_PLAYER; i++) {
				...
				//
				// table.GRR._chi_hu_kind[i] =
				// table.analyse_chi_hu_card(table.GRR._cards_index[i],
				// pWeaveItem,
				// cbWeaveItemCount, table._playerStatus[i]._operate_card,
				// table.GRR._chi_hu_rights[seat_index],true);
				//
...
//				table.process_chi_hu_player_score(i, _seat_index, _send_card_data, false);
	...
	public boolean handler_player_be_in_room(MJTable table, int seat_index) {
		...
		table.operate_effect_action(_seat_index, MJGameConstants.EFFECT_ACTION_TYPE_ACTION, 1, new long[] { MJGameConstants.WIK_YAO_HAI_DI }, 0, seat_index);
		table.operate_out_card(_seat_index, 1, new int[] { _send_card_data }, MJGameConstants.OUT_CARD_TYPE_LEFT, seat_index);
