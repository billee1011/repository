...
//			table.exe_di_hu(_seat_index);
		...
		if (curPlayerStatus.has_action()) {// 有动作
			...
			if (table._playerStatus[_seat_index].lock_huan_zhang()) {
				GameSchedule.put(new OutCardRunnable(table.getRoom_id(), _seat_index, _send_card_data), MJGameConstants.DELAY_AUTO_OUT_CARD, TimeUnit.MILLISECONDS);
			} else {
	...
	public boolean handler_operate_card(MJTable table, int seat_index, int operate_code, int operate_card) {
		...
		case MJGameConstants.WIK_ZI_MO: // 自摸
		{
			...
			table.GRR._chi_hu_rights[_seat_index].opr_or(MJGameConstants.CHR_TIAN_HU);
			...
			GameSchedule.put(new GameFinishRunnable(table.getRoom_id(), _seat_index, MJGameConstants.Game_End_NORMAL), MJGameConstants.GAME_FINISH_DELAY, TimeUnit.SECONDS);
