...
public class MJHandlerDiHu_ZhuZhou extends MJHandlerDiHu {

	private static Logger logger = Logger.getLogger(MJHandlerDiHu_ZhuZhou.class);
	...
	public boolean handler_operate_card(MJTable table,int seat_index, int operate_code, int operate_card){
		...
		if(operate_code == MJGameConstants.WIK_CHI_HU){
			...
			table.GRR._chi_hu_rights[seat_index].opr_or(MJGameConstants.CHR_DI_HU);
			//效果
...
//						int hu_player =  -1;
//						for(int i=0; i<MJGameConstants.GAME_PLAYER;i++){
//							int next_player =  (_out_card_player + MJGameConstants.GAME_PLAYER + 1+i) % MJGameConstants.GAME_PLAYER;
//							if(huIndex[next_player]){
//								if(hu_player<0) {
//									hu_player=next_player;
//									//效果
//									table.process_chi_hu_player_operate(hu_player, operate_card,false);
//								}else {
//									table.GRR._chi_hu_rights[next_player].set_empty();
//								}
//							}
//						}
						...
						// 结束后设置鸟牌 
						table.set_niao_card(target_player,MJGameConstants.INVALID_VALUE,true,0);
					}
					for(int i=0; i<MJGameConstants.GAME_PLAYER;i++){
						if((i == _out_card_player) ||(table.GRR._chi_hu_rights[i].is_valid()==false )){
							continue;
...
//		}
		...
		// 删除扑克
		if (table._logic.remove_card_by_index(table.GRR._cards_index[_out_card_player], card) == false) {
