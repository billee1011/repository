...
public class MJGameLogic {
	...
	public MJGameLogic(){
		_magic_card_count = 0;
		_magic_card_index = new int[MJGameConstants.MAX_COUNT];
		_lai_gen=0;
		_ding_gui=0;
	}
	...
	//洗牌
	public void random_card_data(int return_cards[]){
			...
			if ((value_index>=excursion[i])&&((value_index-excursion[i])<=6))
			{
				...
				if(this._magic_card_count>0){
					for(int m=0; m<this._magic_card_count; i++){
						//吃牌不能包含有王霸
						if( get_magic_card_index(m) != MJGameConstants.MAX_INDEX &&
								get_magic_card_index(m) >= first_index && get_magic_card_index(m) <= first_index+2 ) continue;
	...
	public int check_peng_xthh(int card_index[], int cur_card)
	{
		...
		if(card_index[switch_to_card_index(cur_card)]>=2){
			if(this.is_lai_gen_card(cur_card)==false){
				//不是赖根
				return MJGameConstants.WIK_PENG;
			}
		}
		return MJGameConstants.WIK_NULL;
	}
...
//			return MJGameConstants.WIK_NULL;
	...
	public int analyse_gang_card_all(int cards_index[], WeaveItem WeaveItem[], int cbWeaveCount, GangCardResult gangCardResult,boolean check_weave)
	{
		...
		for (int i=0;i<MJGameConstants.MAX_INDEX;i++)
		{
			...
			if (cards_index[i]==4)
			{
				...
				gangCardResult.type[index] = MJGameConstants.GANG_TYPE_AN_GANG;
			}
		}
			...
			for (int i=0;i<cbWeaveCount;i++)
			{
				if (WeaveItem[i].weave_kind==MJGameConstants.WIK_PENG)
				{
					if (cards_index[switch_to_card_index(WeaveItem[i].center_card)]==1)
					{
						...
						gangCardResult.type[index] = MJGameConstants.GANG_TYPE_ADD_GANG;
					}
				}
			}
	 ...
	 * @return
	 */
	public int analyse_gang_card_all_xthh(int cards_index[], WeaveItem WeaveItem[], int cbWeaveCount, GangCardResult gangCardResult,boolean check_weave)
	{
		//设置变量
		int cbActionMask=MJGameConstants.WIK_NULL;
		...
		for (int i=0;i<MJGameConstants.MAX_INDEX;i++)
		{
			...
			if (cards_index[i]==4)
			{
				cbActionMask|=MJGameConstants.WIK_MENG_XIAO;
				int index = gangCardResult.cbCardCount++;
				gangCardResult.cbCardData[index]=switch_to_card_data(i);
				gangCardResult.isPublic[index] = 0;//安刚
				gangCardResult.type[index] = MJGameConstants.WIK_MENG_XIAO;
			}else if((cards_index[i]==3) && (this.is_lai_gen_card(this.switch_to_card_data(i)))){
				cbActionMask|=MJGameConstants.WIK_DA_CHAO_TIAN;
				int index = gangCardResult.cbCardCount++;
				gangCardResult.cbCardData[index]=switch_to_card_data(i);
				gangCardResult.isPublic[index] = 0;//安刚
				gangCardResult.type[index] = MJGameConstants.WIK_DA_CHAO_TIAN;
		...
		if(check_weave==true){
			...
			for (int i=0;i<cbWeaveCount;i++)
			{
				if (WeaveItem[i].weave_kind==MJGameConstants.WIK_PENG)
				{
					if (cards_index[switch_to_card_index(WeaveItem[i].center_card)]==1)
					{
						cbActionMask|=MJGameConstants.WIK_HUI_TOU_XIAO;
						
						int index = gangCardResult.cbCardCount++;
						gangCardResult.cbCardData[index]=WeaveItem[i].center_card;
						gangCardResult.isPublic[index] = 1;//明刚
						gangCardResult.type[index] = MJGameConstants.WIK_HUI_TOU_XIAO;
					}
				}
			}
	...
	public int analyse_gang_by_card_hand_card(int cards_index[],WeaveItem WeaveItem[], int cbWeaveCount, GangCardResult gangCardResult)
	{
		...
		for (int i=0;i<MJGameConstants.MAX_INDEX;i++)
		{
			...
			if (cards_index[i]==4)
			{
				...
				gangCardResult.cbCardData[index]=switch_to_card_data(i);
				gangCardResult.isPublic[index] = 0;//安刚
				gangCardResult.type[i] = MJGameConstants.GANG_TYPE_AN_GANG;
			}
		}
	 ...
	 * @return
	 */
	public int analyse_xiao_by_dispacth_card(int cards_index[], int card, WeaveItem WeaveItem[], int cbWeaveCount, GangCardResult gangCardResult)
	{
		//设置变量
		int cbActionMask=MJGameConstants.WIK_NULL;
	...
	//是否听牌
//	public boolean is_ting_card( int cards_index[], WeaveItem weaveItem[] ,int cbWeaveCount )
//	{
...
//		for (int i = 0; i < MJGameConstants.MAX_INDEX - 7; i++) {
//			int cbCurrentCard = switch_to_card_data(i);
//			if (MJGameConstants.WIK_CHI_HU == analyse_chi_hu_card_zhuzhou(cbCardIndexTemp, weaveItem, cbWeaveCount, cbCurrentCard, chr, true))
//				return true;
//		}
//		return false;
//	}
...
//	// 解析吃胡 株洲玩法
//	public int analyse_chi_hu_card_zhuzhou(int cards_index[], WeaveItem weaveItem[], int weaveCount, int cur_card, ChiHuRight chiHuRight, boolean bSelfSendCard) {
//		// 变量定义
//		// int cbChiHuKind = MJGameConstants.WIK_NULL;
//		// 设置变量
//		List<AnalyseItem> analyseItemArray = new ArrayList<AnalyseItem>();
...
//
//		// 分析扑克
//		boolean bValue = analyse_card(cbCardIndexTemp, weaveItem, weaveCount, analyseItemArray);
//		if (bSelfSendCard) {
...
//		}
	...
	public int is_qi_xiao_dui(int cards_index[], WeaveItem weaveItem[],int cbWeaveCount,int cur_card){
		...
		//组合判断
		if (cbWeaveCount!=0) return MJGameConstants.WIK_NULL;
		...
		for (int i=0;i<MJGameConstants.MAX_INDEX;i++)
		{
			...
			if(this._magic_card_count>0){
				for(int m=0; m<_magic_card_count;m++){
					//王牌过滤
					if( i == get_magic_card_index(m) ) continue;
...
//				return MJGameConstants.WIK_NULL;
			...
			if(cbReplaceCount > 0)return MJGameConstants.WIK_NULL;
		}

		if (nGenCount > 0) {
			if(nGenCount>1){
				//双豪华七小对
				return MJGameConstants.CHR_SHUANG_HAO_HUA_QI_XIAO_DUI;
			}
			return MJGameConstants.CHR_HAOHUA_QI_XIAO_DUI;
		} else {
			return MJGameConstants.CHR_QI_XIAO_DUI;
		}
	...
	public boolean is_jiangjiang_hu( int cards_index[], WeaveItem weaveItem[],int weaveCount, int cur_card ){
		...
		for(int i=0;i<weaveCount;i++){
			if(weaveItem[i].weave_kind == MJGameConstants.WIK_LEFT || 
					weaveItem[i].weave_kind == MJGameConstants.WIK_CENTER||
					weaveItem[i].weave_kind == MJGameConstants.WIK_RIGHT){
	...
	//分析扑克
	public boolean analyse_card(int cards_index[], WeaveItem weaveItem[],int cbWeaveCount, List<AnalyseItem> analyseItemArray)
	{
		//计算数目
...
//					if(this.is_magic_index(i) || this.is_magic_index(i+1) || this.is_magic_index(i+1)){
//						
//					}else{
...
//						if((get_magic_card_index()!=MJGameConstants.MAX_INDEX) && (cbCardIndexTemp[get_magic_card_index()]>0)){
				...
				if (bEnoughCard==true)
				{
					...
					for (int i=0;i<MJGameConstants.MAX_INDEX;i++)
					{
						if (cbCardIndexTemp[i]==2)
						{
							...
							if( this.is_magic_index(i) ) bMagicEye = true;
							break;
						}
...
//						}
	...
	{
		//自摸牌等级
		if (player_action == MJGameConstants.WIK_ZI_MO) { return 50; }
		
		//吃胡牌等级
		if (player_action == MJGameConstants.WIK_CHI_HU) { return 40; }
		
		//地胡牌等级
		if (player_action == MJGameConstants.WIK_CHI_HU) { return 40; }
		
		//杠牌等级
		if (player_action == MJGameConstants.WIK_GANG) { return 30; }
		
		//补张牌等级
		if (player_action == MJGameConstants.WIK_BU_ZHNAG) { return 30; }
		
		//笑
		if (player_action == MJGameConstants.WIK_MENG_XIAO) { return 30; }
		if (player_action == MJGameConstants.WIK_DIAN_XIAO) { return 30; }
		if (player_action == MJGameConstants.WIK_HUI_TOU_XIAO) { return 30; }
		if (player_action == MJGameConstants.WIK_XIAO_CHAO_TIAN) { return 30; }
		if (player_action == MJGameConstants.WIK_DA_CHAO_TIAN) { return 30; }
		
		//碰牌等级
		if (player_action == MJGameConstants.WIK_PENG) { return 20; }
		
		//上牌等级
		if (player_action ==MJGameConstants.WIK_RIGHT ||
			player_action ==MJGameConstants.WIK_CENTER||
			player_action ==MJGameConstants.WIK_LEFT) { return 10; }
	...
	public int get_chi_hu_action_rank_zz( ChiHuRight chiHuRight)
	{
		...
		if(!(chiHuRight.opr_and(MJGameConstants.CHR_QIANG_GANG_HU)).is_empty()){
			//抢杠胡
			wFanShu = 1;
		}
		
		if(!(chiHuRight.opr_and(MJGameConstants.CHR_QI_XIAO_DUI)).is_empty()){
			wFanShu = 1;
		}
		
		if(!(chiHuRight.opr_and(MJGameConstants.CHR_SHU_FAN)).is_empty()){
			//素胡
			wFanShu = 1;
		}
		
		if(!(chiHuRight.opr_and(MJGameConstants.CHR_ZI_MO)).is_empty()){
			wFanShu = 2;
		}
...
//		}
	...
	public int get_chi_hu_action_rank_xthh( ChiHuRight chiHuRight)
	{
		...
		if(!(chiHuRight.opr_and(MJGameConstants.CHR_HH_HEI_MO)).is_empty()){
			wFanShu = 4;
		}else if(!(chiHuRight.opr_and(MJGameConstants.CHR_HH_RUAN_MO)).is_empty()){
			wFanShu = 2;
		}else if(!(chiHuRight.opr_and(MJGameConstants.CHR_HH_ZHUO_CHONG)).is_empty()){
			wFanShu = 2;
		}else if(!(chiHuRight.opr_and(MJGameConstants.CHR_HH_RE_CHONG)).is_empty()){
			wFanShu = 2;//加杠钱
		}else if(!(chiHuRight.opr_and(MJGameConstants.CHR_HH_QIANG_GANG_HU)).is_empty()){
			wFanShu = 3;
		}
	...
	public int get_chi_hu_action_rank_zhuzhou_mutip(ChiHuRight chiHuRight) {
		...
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_PENGPENG_HU)).is_empty()) {
			if (!(chiHuRight.opr_and(MJGameConstants.CHR_258_JIANG)).is_empty()) {
				wFanShu += 4;
			} else {
				wFanShu += 3;
			}
		...
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_QI_XIAO_DUI)).is_empty())
			wFanShu += 4;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_HAOHUA_QI_XIAO_DUI)).is_empty())
			wFanShu += 8;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_SHUANG_HAO_HUA_QI_XIAO_DUI)).is_empty())
			wFanShu += 16;
		
		boolean hasPengOrQi = wFanShu>0?true:false;//是否有 碰碰胡 七小对 一种  上面牌型互斥 只会有一种
		
		boolean haQingyiSe = false;
		//清一色
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_QING_YI_SE)).is_empty()) {
			if(hasPengOrQi) {
		...
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_TIAN_HU)).is_empty())
			wFanShu += 8;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_DI_HU)).is_empty())
			wFanShu += 4;
	...
	private boolean isHasHaiDiOrGang(ChiHuRight chiHuRight) {
		boolean has = false;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_HAI_DI_LAO)).is_empty())
			has=true;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_HAI_DI_PAO)).is_empty())
			has=true;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_GANG_KAI)).is_empty())
			has=true;
		// if
		...
		// wFanShu += 6;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_GANG_SHANG_PAO)).is_empty())
			has=true;
		// if
		// (!(chiHuRight.opr_and(MJGameConstants.CHR_QUAN_QIU_REN)).is_empty())
		// wFanShu += 6;
	
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_SHUANG_GANG_KAI)).is_empty())
			has=true;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_SHUANG_GANG_SHANG_PAO)).is_empty())
			has=true;
		return has;
	}
...
//		if (!(chiHuRight.opr_and(MJGameConstants.CHR_JIANGJIANG_HU)).is_empty())
//			wFanShu += 6;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_TIAN_HU)).is_empty())
			wFanShu += 8;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_DI_HU)).is_empty())
		...
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_GANG_SHANG_PAO)).is_empty())
			wFanShu += 4;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_QUAN_QIU_REN)).is_empty())
			wFanShu += 6;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_SHUANG_HAO_HUA_QI_XIAO_DUI)).is_empty())
			wFanShu += 16;
		if (!(chiHuRight.opr_and(MJGameConstants.CHR_SHUANG_GANG_KAI)).is_empty())
		...
		// 小胡
//		if (!(chiHuRight.opr_and(MJGameConstants.CHR_XIAO_DA_SI_XI)).is_empty())
//			wFanShu += 1;
//		if (!(chiHuRight.opr_and(MJGameConstants.CHR_XIAO_BAN_BAN_HU)).is_empty())// 8000
//			wFanShu += 1;
//		if (!(chiHuRight.opr_and(MJGameConstants.CHR_XIAO_LIU_LIU_SHUN)).is_empty())
//			wFanShu += 1;
//		if (!(chiHuRight.opr_and(MJGameConstants.CHR_XIAO_QUE_YI_SE)).is_empty())// 10000
//			wFanShu += 1;
	...
	public int get_weave_card(int cbWeaveKind, int cbCenterCard, int cbCardBuffer[])
	{
		...
		case MJGameConstants.WIK_MENG_XIAO:		//杠牌操作
		case MJGameConstants.WIK_DIAN_XIAO:	
		case MJGameConstants.WIK_HUI_TOU_XIAO:	
		{
			//设置变量
			cbCardBuffer[0]=cbCenterCard;
			cbCardBuffer[1]=cbCenterCard;
		...
		case MJGameConstants.WIK_XIAO_CHAO_TIAN:		//杠牌操作
		case MJGameConstants.WIK_DA_CHAO_TIAN:	
		{
			//设置变量
			cbCardBuffer[0]=cbCenterCard;
			cbCardBuffer[1]=cbCenterCard;
...
//		cards_index[switch_to_card_index(0x11)]++;
//		cards_index[switch_to_card_index(0x25)]++;
		return card_count;
	}
