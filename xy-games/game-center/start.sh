...
# memory options
MEM_CONFIG="-Xms$TOTAL_AVAILABLE_MEMORY -Xmx$TOTAL_AVAILABLE_MEMORY -Xmn$YOUNG_EDEN -XX:+UseConcMarkSweepGC -XX:+UseCMSCompactAtFullCollection -XX:+ExplicitGCInvokesConcurrent -XX:CMSInitiatingOccupancyFraction=75 -XX:CMSFullGCsBeforeCompaction=1"
...
#GC_LOG
GC_LOG="-XX:+PrintGC -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+PrintHeapAtGC -XX:+PrintTenuringDistribution -Xloggc:./gc.log"

# encoding config options
ENCODEING_CONFIG="-Dfile.encoding=UTF-8"

# Classpath config options
CLASSPATH="lib/*:./game-center.jar"

# main class
MAIN_CLASS="GoCenterServer"
