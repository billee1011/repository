 ...
 *
 */
public class DownRecommendLevelRunnble implements Runnable {

	private static Logger logger = Logger.getLogger(DownRecommendLevelRunnble.class);

	private long account_id = 0;
	private int receive = 0;

	public DownRecommendLevelRunnble(long account_id, int receive) {
		...
		try {
			...
			if (level == 3) {
				if (count < sysParamModel2253.getVal1()) {
					...
					log_down_record(account_id, 0, 3, "业绩未达标白银降级为见习推广员");
				}
			} else if (level == 2) {
				if (count < sysParamModel2253.getVal2()) {
					...
					log_down_record(account_id, 3, 2, "业绩未达标黄金降级为白银推广员");
				}
			} else if (level == 1) {
				if (count < sysParamModel2253.getVal3()) {
					...
					log_down_record(account_id, 2, 1, "业绩未达标钻石降级为黄金推广员");
				}
			} else {
				return;
			}
			...
			logger.error(account_id + " 推广员降级失败", e);
		}
	}
