 ...
 * 
 *
 * @author DIY date: 2017年11月10日 上午11:55:44 <br/>
 */
public class DbBatchRunnable implements Runnable {

	private static Logger logger = LoggerFactory.getLogger(DbBatchRunnable.class);
	...
	public void run() {
		try {
			...
			for (Account account : PublicServiceImpl.getInstance().getAccountIdMap().values()) {
				...
				for (AccountParamModel m : account.getAccountParamModelMap().values()) {
					...
					if (m.isNewAddValue()) {
						...
						m.setNeedDB(false);
						DataThreadPool.getInstance().addTask(
								new DBUpdateDto(DbStoreType.PUBLIC, DbOpType.INSERT, "insertAccountParamModel", m));
					}
			...
			if (accountModelList.size() > 0) {
				DataThreadPool.getInstance().addTask(new DBUpdateDto(DbStoreType.PUBLIC, DbOpType.BATCH_UPDATE,
						"updateAccountModel", accountModelList));
			...
			if (accountWeiXinModelList.size() > 0) {
				DataThreadPool.getInstance().addTask(new DBUpdateDto(DbStoreType.PUBLIC, DbOpType.BATCH_UPDATE,
						"updateAccountWeixinModel", accountWeiXinModelList));
			...
			if (accountParamModelList.size() > 0) {
				DataThreadPool.getInstance().addTask(new DBUpdateDto(DbStoreType.PUBLIC, DbOpType.BATCH_UPDATE,
						"updateAccountParamModel", accountParamModelList));
		...
		try {
			List<RoomGeneratorModel> roomGeneratorModels = Lists.newArrayList();
			roomGeneratorModels.addAll(IDServiceImpl.getInstance().getRoomGeneratorModelToDB());
			if(roomGeneratorModels.size()>0) {
				DataThreadPool.getInstance().addTask(new DBUpdateDto(DbStoreType.PUBLIC, DbOpType.BATCH_UPDATE,
						"updateRoomGeneratorModel", roomGeneratorModels));
			}
