...
public class RedisService implements IRedisService {
	...
	public long del(final String... keys) {
		...
		if (cost > WARN_TIME) {
			...
			if(cost>WARN_DB_TIME) {
				MongoDBServiceImpl.getInstance().server_error_log(0, ELogType.redisTimeLong, "redis耗时 del"+cost,
						0L, keyStr);
			}
	...
	public void set(final byte[] key, final byte[] value, final long liveTime) {
		...
		if (cost > WARN_TIME) {
			...
			if(cost>WARN_DB_TIME) {
				MongoDBServiceImpl.getInstance().server_error_log(0, ELogType.redisTimeLong, "redis耗时--set"+cost,
						0L, keyStr);
			}
		}
	}
